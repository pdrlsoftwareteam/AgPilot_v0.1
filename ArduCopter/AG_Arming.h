...
class AG_Arming_Copter : public AG_Arming
{
public:
    friend class Copter;
    friend class ToyMode;

    AG_Arming_Copter() : AG_Arming()
    {
    ...
    /* Do not allow copies */
    CLASS_NO_COPY(AG_Arming_Copter);

    bool rc_calibration_checks(bool display_failure) override;

    bool disarm(AG_Arming::Method method, bool do_disarm_checks=true) override;
...
protected:
    ...
    bool arm_checks(AG_Arming::Method method) override;

    // mandatory checks that cannot be bypassed.  This function will only be called if ARMING_CHECK is zero or arming forced
    bool mandatory_checks(bool display_failure) override;

    // NOTE! the following check functions *DO* call into AG_Arming:
    ...
    // NOTE! the following check functions *DO NOT* call into AG_Arming!
