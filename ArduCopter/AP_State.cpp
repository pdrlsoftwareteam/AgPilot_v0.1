    ...
    if (simple_mode != b) {
        switch (b) {
            case SimpleMode::NONE:
                ...
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SIMPLE mode off");
                break;
            case SimpleMode::SIMPLE:
                AP::logger().Write_Event(LogEvent::SET_SIMPLE_ON);
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SIMPLE mode on");
                break;
            case SimpleMode::SUPERSIMPLE:
                // initialise super simple heading
                update_super_simple_bearing(true);
                AP::logger().Write_Event(LogEvent::SET_SUPERSIMPLE_ON);
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SUPERSIMPLE mode on");
                break;
        }
        simple_mode = b;
    }
