    ...
    switch (stage) {
        ...
        case STORING_POINTS:
            if (ab_dest == Destination::A) {
                // store point A
                dest_A = curr_pos;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: point A stored");
                AP::logger().Write_Event(LogEvent::ZIGZAG_STORE_A);
            } else {
                // store point B
                dest_B = curr_pos;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: point B stored");
            ...
            if (calculate_next_dest(ab_dest, stage == AUTO, next_dest, terr_alt)) {
                ...
                if (wp_nav->set_wp_destination(next_dest, terr_alt)) {
                    ...
                    if (is_auto == false || line_num == ZIGZAG_LINE_INFINITY) {
                        gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: moving to %s", (ab_dest == Destination::A) ? "A" : "B");
                    } else {
                        line_count++;
                        gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: moving to %s (line %d/%d)", (ab_dest == Destination::A) ? "A" : "B", line_count, line_num);
                    }
                }
            }
            break;
    ...
    if (!dest_A.is_zero() && !dest_B.is_zero() && !is_zero((dest_B - dest_A).length_squared())) {
        ...
        if (calculate_side_dest(next_dest, terr_alt)) {
            ...
            if (wp_nav->set_wp_destination(next_dest, terr_alt)) {
                ...
                gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: moving to %s", dir[(uint8_t)zigzag_direction]);
            }
        }
    }
}
...
void ModeZigZag::return_to_manual_control(bool maintain_target)
{
    if (stage == AUTO) {
        ...
        is_auto = false;
        gcs().send_text(AGPILOT_SEVERITY_INFO, "ZigZag: manual control");
    }
}
