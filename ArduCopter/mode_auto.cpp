...
bool ModeAuto::wp_start(const Location& dest_loc)
{
	...
	// Store the previous location only if it's valid (not zero)
	if (!is_zero(Next.x) && !is_zero(Next.y)) {

		Vector2f check_loc;
		if(dest_loc.get_vector_xy_from_origin_NE(check_loc)){}

		if(is_zero(check_loc.x - Next.x))
		{
			goto LABEL;
		}
		else
	...
	LABEL:
	// Get the next waypoint location using dest_loc and store it in Next
	if (dest_loc.get_vector_xy_from_origin_NE(Next)) { }
...
bool ModeAuto::Check_Range()
{
	// Line vector
	float line_vec_x = Next.x - Prev.x;
	float line_vec_y = Next.y - Prev.y;

	// Length of the line segment
	float line_length_squared = line_vec_x * line_vec_x + line_vec_y * line_vec_y;

	Vector2f current_Loc = inertial_nav.get_position_xy_cm();

	// Vector from Prev to current_Loc
	float Prev_to_current_Loc_x = current_Loc.x - Prev.x;
	float Prev_to_current_Loc_y = current_Loc.y - Prev.y;

	// Projection factor
	float t = (Prev_to_current_Loc_x * line_vec_x + Prev_to_current_Loc_y * line_vec_y) / line_length_squared;
	t = MAX(0.0f, MIN(1.0f, t)); // Clamps t between 0 and 1
	...
	const float threshold = 100.0; // Adjust this threshold as needed
...
void ModeAuto::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
	...
	uint16_t spray_param = 0;
	if(LOWBYTE(cmd.p1) == 0)
		spray_param = 1;

	mission.do_sprayer_in_auto(spray_param);
	gcs().send_text(MAV_SEVERITY_INFO, "do_sprayer_in_auto - %d",spray_param);
