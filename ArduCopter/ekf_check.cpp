        ...
        if (!ekf_check_state.bad_variance) {
            ...
            if (ekf_check_state.fail_count >= EKF_CHECK_ITERATIONS_MAX) {
                ...
                if ((AP_HAL::millis() - ekf_check_state.last_warn_time) > EKF_CHECK_WARNING_TIME) {
                    gcs().send_text(AGPILOT_SEVERITY_CRITICAL,"EKF variance");
...
void Copter::failsafe_ekf_event()
{
    ...
    gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "EKF Failsafe: changed to %s Mode", flightmode->name());
}
...
void Copter::failsafe_ekf_off_event(void)
{
    ...
    if (AP_Notify::flags.failsafe_ekf) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "EKF Failsafe Cleared");
    }
...
void Copter::check_ekf_reset()
{
    ...
    if ((ahrs.get_primary_core_index() != ekf_primary_core) && (ahrs.get_primary_core_index() != -1)) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "EKF primary changed:%d", (unsigned)ekf_primary_core);
    }
}
...
void Copter::check_vibration()
{
    ...
    if (!vibration_check.high_vibes) {
        ...
        if (now - vibration_check.start_ms > 1000) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "Vibration compensation ON");
        }
    } else {
        // initialise timer
        if (do_bad_vibe_actions) {
        ...
        if (now - vibration_check.clear_ms > 15000) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "Vibration compensation OFF");
        }
    }
