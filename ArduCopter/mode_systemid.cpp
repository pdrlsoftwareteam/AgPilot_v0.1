...
bool ModeSystemId::init(bool ignore_checks)
{
    // check if enabled
    if (axis == 0) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "No axis selected, SID_AXIS = 0");
        return false;
    }
    ...
    gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Starting: axis=%d", (unsigned)axis);
...
void ModeSystemId::run()
{
    ...
    if ((systemid_state == SystemIDModeState::SYSTEMID_STATE_TESTING) &&
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Parameter Error");
    }
    ...
    switch (systemid_state) {
        ...
        case SystemIDModeState::SYSTEMID_STATE_TESTING:
            ...
            if (copter.ap.land_complete) {
                systemid_state = SystemIDModeState::SYSTEMID_STATE_STOPPED;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Stopped: Landed");
                break;
            }
            if (attitude_control->lean_angle_deg()*100 > attitude_control->lean_angle_max_cd()) {
                systemid_state = SystemIDModeState::SYSTEMID_STATE_STOPPED;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Stopped: lean=%f max=%f", (double)attitude_control->lean_angle_deg(), (double)attitude_control->lean_angle_max_cd());
                break;
            }
            if (waveform_time > SYSTEM_ID_DELAY + time_fade_in + time_const_freq + time_record + time_fade_out) {
                systemid_state = SystemIDModeState::SYSTEMID_STATE_STOPPED;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Finished");
                break;
            }
            ...
            switch ((AxisType)axis.get()) {
                case AxisType::NONE:
                    systemid_state = SystemIDModeState::SYSTEMID_STATE_STOPPED;
                    gcs().send_text(AGPILOT_SEVERITY_INFO, "SystemID Stopped: axis = 0");
                    break;
