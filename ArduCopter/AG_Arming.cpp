...
bool AG_Arming_Copter::pre_arm_checks(bool display_failure)
{
...
bool AG_Arming_Copter::run_pre_arm_checks(bool display_failure)
{
    ...
    return parameter_checks(display_failure)
        ...
        & AG_Arming::airspeed_checks(display_failure)
#endif
        & AG_Arming::pre_arm_checks(display_failure);
}

bool AG_Arming_Copter::rc_throttle_failsafe_checks(bool display_failure) const
{
...
bool AG_Arming_Copter::barometer_checks(bool display_failure)
{
    if (!AG_Arming::barometer_checks(display_failure)) {
        return false;
    }
...
bool AG_Arming_Copter::ins_checks(bool display_failure)
{
    bool ret = AG_Arming::ins_checks(display_failure);
...
bool AG_Arming_Copter::board_voltage_checks(bool display_failure)
{
    if (!AG_Arming::board_voltage_checks(display_failure)) {
        return false;
    }
...
// all data loaded
bool AG_Arming_Copter::terrain_database_required() const
{

    if (copter.wp_nav->get_terrain_source() == AG_WPNav::TerrainSource::TERRAIN_FROM_RANGEFINDER) {
        // primary terrain source is from rangefinder, allow arming without terrain database
        return false;
    }

    if (copter.wp_nav->get_terrain_source() == AG_WPNav::TerrainSource::TERRAIN_FROM_TERRAINDATABASE &&
        copter.mode_rtl.get_alt_type() == ModeRTL::RTLAltType::RTL_ALTTYPE_TERRAIN) {
        return true;
    }
    return AG_Arming::terrain_database_required();
}

bool AG_Arming_Copter::parameter_checks(bool display_failure)
{
    ...
    if (check_enabled(ARMING_CHECK_PARAMETERS)) {
        ...
        if (copter.mode_rtl.get_alt_type() == ModeRTL::RTLAltType::RTL_ALTTYPE_TERRAIN) {
            ...
            case AG_WPNav::TerrainSource::TERRAIN_FROM_TERRAINDATABASE:
                // these checks are done in AG_Arming
                break;
            }
        }
#endif
...
bool AG_Arming_Copter::oa_checks(bool display_failure)
{
...
bool AG_Arming_Copter::rc_calibration_checks(bool display_failure)
{
    ...
    copter.ap.pre_arm_rc_check = rc_checks_copter_sub(display_failure, channels)
        & AG_Arming::rc_calibration_checks(display_failure);

    return copter.ap.pre_arm_rc_check;
}

// performs pre_arm gps related checks and returns true if passed
bool AG_Arming_Copter::gps_checks(bool display_failure)
{
    ...
    if (mode_requires_gps) {
        if (!AG_Arming::gps_checks(display_failure)) {
            AG_Notify::flags.pre_arm_gps_check = false;
            return false;
        }
    }
    ...
    if (!mode_requires_gps) {
        AG_Notify::flags.pre_arm_gps_check = true;
        return true;
    }
...
bool AG_Arming_Copter::pre_arm_ekf_attitude_check()
{
...
bool AG_Arming_Copter::proximity_checks(bool display_failure) const
{
#if HAL_PROXIMITY_ENABLED

    if (!AG_Arming::proximity_checks(display_failure)) {
        return false;
    }
...
bool AG_Arming_Copter::mandatory_gps_checks(bool display_failure)
{
    // check if flight mode requires GPS
...
// Check GCS failsafe
bool AG_Arming_Copter::gcs_failsafe_check(bool display_failure)
{
...
// check winch
bool AG_Arming_Copter::winch_checks(bool display_failure) const
{
#if AP_WINCH_ENABLED
...
bool AG_Arming_Copter::alt_checks(bool display_failure)
{
...
bool AG_Arming_Copter::arm_checks(AG_Arming::Method method)
{
    const auto &ahrs = AP::ahrs();
    ...
    if (check_enabled(ARMING_CHECK_RC)) {
        ...
        if (!((method == AG_Arming::Method::MAVLINK || method == AG_Arming::Method::SCRIPTING) && (copter.flightmode->mode_number() == Mode::Number::GUIDED || copter.flightmode->mode_number() == Mode::Number::AUTO))) {
    ...
    return AG_Arming::arm_checks(method);
}

// mandatory checks that will be run if ARMING_CHECK is zero or arming forced
bool AG_Arming_Copter::mandatory_checks(bool display_failure)
{
    ...
    AG_Notify::flags.pre_arm_gps_check = result;
    ...
    return result & AG_Arming::mandatory_checks(display_failure);
}

void AG_Arming_Copter::set_pre_arm_check(bool b)
{
    copter.ap.pre_arm_check = b;
    AG_Notify::flags.pre_arm_check = b;
}

bool AG_Arming_Copter::arm(const AG_Arming::Method method, const bool do_arming_checks)
{
    ...
    if (!AG_Arming::arm(method, do_arming_checks)) {
        AG_Notify::events.arming_failed = true;
        in_arm_motors = false;
        return false;
    }
...
bool AG_Arming_Copter::disarm(const AG_Arming::Method method, bool do_disarm_checks)
{
    ...
    if (do_disarm_checks &&
        method == AG_Arming::Method::MAVLINK &&
        !copter.ap.land_complete) {
        return false;
    }

    if (!AG_Arming::disarm(method, do_disarm_checks)) {
        return false;
    }
