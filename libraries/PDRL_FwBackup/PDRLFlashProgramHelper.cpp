...
bool PDRL_FlashProgramHelper::init()
{
...
void PDRL_FlashProgramHelper::reset_device()
{
...
bool PDRL_FlashProgramHelper::detect_device()
{
    AP_HAL::QSPIDevice::CommandHeader cmd;
    ...
    // Read and parse the param table
    {
        ...
        if (_desc.is_xip_supported) {
            if (SFDP_GET_BIT(param_table[14],16)) {
                _desc.entry_method = PDRL_FlashProgramHelper::XIP_ENTRY_METHOD_1;
            } else if (SFDP_GET_BIT(param_table[14],17)) {
                _desc.entry_method = PDRL_FlashProgramHelper::XIP_ENTRY_METHOD_2;
            } else {
...
bool PDRL_FlashProgramHelper::configure_device()
{
    // Enable 1-4-4 mode
...
bool PDRL_FlashProgramHelper::write_enable()
{
...
bool PDRL_FlashProgramHelper::write_disable()
{
    if (_desc.write_enable_ins) {
        wait_ready();
        write_enable_called = true;
...
bool PDRL_FlashProgramHelper::modify_reg(uint8_t read_ins, uint8_t write_ins,
...
bool PDRL_FlashProgramHelper::read_reg(uint8_t read_ins, uint8_t &read_val)
{
    AP_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = read_ins;
    cmd.cfg    =  AP_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
...
bool PDRL_FlashProgramHelper::write_reg(uint8_t read_ins, uint8_t write_val)
{
    AP_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = read_ins;
    cmd.cfg    =  AP_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
...
bool PDRL_FlashProgramHelper::send_cmd(uint8_t ins)
{
    AP_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = ins;
    cmd.cfg    =  AP_HAL::QSPI::CFG_CMD_MODE_ONE_LINE;
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_mass_erase(uint32_t &delay_ms, uint32_t &timeout_ms)
{
    write_enable();
    wait_ready();
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_sector_erase(uint32_t sector, uint32_t &delay_ms, uint32_t &timeout_ms)
{
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_erase_offset(uint32_t offset, uint32_t size, uint32_t &erasing,
 ...
 *
 */
bool PDRL_FlashProgramHelper::verify_sector_erase(uint32_t sector)
{
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_program_page(uint32_t page, const uint8_t* data,
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_program_offset(uint32_t offset, const uint8_t* data, uint32_t size, uint32_t &programming,
        uint32_t &delay_us, uint32_t &timeout_us)
{
 ...
 *
 */
bool PDRL_FlashProgramHelper::read(uint32_t offset, uint8_t* data, uint32_t size)
{
 ...
 *
 */
bool PDRL_FlashProgramHelper::is_device_busy()
{
    uint8_t status;
...
void PDRL_FlashProgramHelper::wait_ready()
{
 ...
 *
 */
bool PDRL_FlashProgramHelper::start_xip_mode(void** addr)
{
    wait_ready();
    ...
    switch(_desc.entry_method) {
        case PDRL_FlashProgramHelper::XIP_ENTRY_METHOD_1:
        {
        ...
        case PDRL_FlashProgramHelper::XIP_ENTRY_METHOD_2:
        {
...
bool PDRL_FlashProgramHelper::stop_xip_mode()
{
