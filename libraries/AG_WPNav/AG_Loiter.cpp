...
const AG_Param::GroupInfo AG_Loiter::var_info[] = {
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ANG_MAX",  1, AG_Loiter, _angle_max, 0.0f),
    ...
    // @Increment: 50
    // @User: Standard
    AP_GROUPINFO("SPEED", 2, AG_Loiter, _speed_cms, LOITER_SPEED_DEFAULT),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ACC_MAX", 3, AG_Loiter, _accel_cmss, LOITER_ACCEL_MAX_DEFAULT),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("BRK_ACCEL", 4, AG_Loiter, _brake_accel_cmss, LOITER_BRAKE_ACCEL_DEFAULT),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("BRK_JERK", 5, AG_Loiter, _brake_jerk_max_cmsss, LOITER_BRAKE_JERK_DEFAULT),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("BRK_DELAY",  6, AG_Loiter, _brake_delay, LOITER_BRAKE_START_DELAY_DEFAULT),
...
// their values.
//
AG_Loiter::AG_Loiter(const AG_InertialNav& inav, const AG_AHRS_View& ahrs, AG_PosControl& pos_control, const AG_AttitudeControl& attitude_control) :
    _inav(inav),
    _ahrs(ahrs),
...
void AG_Loiter::init_target(const Vector2f& position)
{
    sanity_check_params();
...
void AG_Loiter::soften_for_landing()
{
...
void AG_Loiter::set_pilot_desired_acceleration(float euler_roll_angle_cd, float euler_pitch_angle_cd)
{
...
void AG_Loiter::get_stopping_point_xy(Vector2f& stopping_point) const
{
    Vector2p stop;
...
float AG_Loiter::get_angle_max_cd() const
{
...
void AG_Loiter::update(bool avoidance_on)
{
...
void AG_Loiter::sanity_check_params()
{
...
void AG_Loiter::calc_desired_velocity(bool avoidance_on)
{
    ...
    if (!is_zero(desired_speed)) {
        ...
        if (_desired_accel.is_zero()) {
            if ((AG_HAL::millis() - _brake_timer) > _brake_delay * 1000.0f) {
