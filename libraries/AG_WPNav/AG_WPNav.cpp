...
const AG_Param::GroupInfo AG_WPNav::var_info[] = {
    ...
    // @Increment: 50
    // @User: Standard
    AP_GROUPINFO("SPEED",       0, AG_WPNav, _wp_speed_cms, WPNAV_WP_SPEED),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("RADIUS",      1, AG_WPNav, _wp_radius_cm, WPNAV_WP_RADIUS),
    ...
    // @Increment: 50
    // @User: Standard
    AP_GROUPINFO("SPEED_UP",    2, AG_WPNav, _wp_speed_up_cms, WPNAV_WP_SPEED_UP),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("SPEED_DN",    3, AG_WPNav, _wp_speed_down_cms, WPNAV_WP_SPEED_DOWN),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("ACCEL",       5, AG_WPNav, _wp_accel_cmss, WPNAV_ACCELERATION),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("ACCEL_Z",     6, AG_WPNav, _wp_accel_z_cmss, WPNAV_WP_ACCEL_Z_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("RFND_USE",   10, AG_WPNav, _rangefinder_use, 1),
    ...
    // @Range: 1 20
    // @User: Standard
    AP_GROUPINFO("JERK",   11, AG_WPNav, _wp_jerk, 1.0f),
    ...
    // @Range: 0.1 100
    // @User: Advanced
    AP_GROUPINFO("TER_MARGIN",  12, AG_WPNav, _terrain_margin, 10.0),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("ACCEL_C",     13, AG_WPNav, _wp_accel_c_cmss, 0.0),
...
// their values.
//
AG_WPNav::AG_WPNav(const AG_InertialNav& inav, const AG_AHRS_View& ahrs, AG_PosControl& pos_control, const AG_AttitudeControl& attitude_control) :
    _inav(inav),
    _ahrs(ahrs),
...
AG_WPNav::TerrainSource AG_WPNav::get_terrain_source() const
{
    // use range finder if connected
    if (_rangefinder_available && _rangefinder_use) {
        return AG_WPNav::TerrainSource::TERRAIN_FROM_RANGEFINDER;
    }
#if AP_TERRAIN_AVAILABLE
    const AG_Terrain *terrain = AP::terrain();
    if (terrain != nullptr && terrain->enabled()) {
        return AG_WPNav::TerrainSource::TERRAIN_FROM_TERRAINDATABASE;
    } else {
        return AG_WPNav::TerrainSource::TERRAIN_UNAVAILABLE;
    }
#else
    return AG_WPNav::TerrainSource::TERRAIN_UNAVAILABLE;
#endif
...
void AG_WPNav::wp_and_spline_init(float speed_cms, Vector3f stopping_point)
{    
    ...
    // mark as active
    _wp_last_update = AG_HAL::millis();
}

/// set_speed_xy - allows main code to pass target horizontal velocity for wp navigation
void AG_WPNav::set_speed_xy(float speed_cms)
{
...
void AG_WPNav::set_speed_up(float speed_up_cms)
{
...
void AG_WPNav::set_speed_down(float speed_down_cms)
{
...
bool AG_WPNav::set_wp_destination_loc(const Location& destination)
{
    bool terr_alt;
    Vector3f dest_neu;
...
///     returns false if conversion from location to vector from ekf origin cannot be calculated
bool AG_WPNav::set_wp_destination_next_loc(const Location& destination)
{
    bool terr_alt;
    Vector3f dest_neu;
...
bool AG_WPNav::get_wp_destination_loc(Location& destination) const
{
...
bool AG_WPNav::set_wp_destination(const Vector3f& destination, bool terrain_alt)
{
...
bool AG_WPNav::set_wp_destination_next(const Vector3f& destination, bool terrain_alt)
{
...
bool AG_WPNav::set_wp_destination_NED(const Vector3f& destination_NED)
{
...
/// set waypoint destination using NED position vector from ekf origin in meters
bool AG_WPNav::set_wp_destination_next_NED(const Vector3f& destination_NED)
{
    // convert NED to NEU and do not use terrain following
...
void AG_WPNav::shift_wp_origin_and_destination_to_current_pos_xy()
{
...
///     relies on set_wp_destination or set_wp_origin_and_destination having been called first
void AG_WPNav::shift_wp_origin_and_destination_to_stopping_point_xy()
{
...
void AG_WPNav::get_wp_stopping_point_xy(Vector2f& stopping_point) const
{
    Vector2p stop;
...
void AG_WPNav::get_wp_stopping_point(Vector3f& stopping_point) const
{
    Vector3p stop;
...
void AG_WPNav::setSemiAutoOverrideAltitude(bool isSet,bool resetZoffset)
{
...
void AG_WPNav::resetWaypointZ(float zVal)
{
	 _lastZOffset = zVal;
}

/// advance_wp_target_along_track - move target location along track from origin to destination
bool AG_WPNav::advance_wp_target_along_track(float dt)
{
...
void AG_WPNav::update_track_with_speed_accel_limits()
{
    // update this leg
    if (_this_leg_is_spline) {
...
float AG_WPNav::check_avoidance_status()
{
...
bool AG_WPNav::is_active() const
{
    return (AG_HAL::millis() - _wp_last_update) < 200;
}

// get terrain's altitude (in cm above the ekf origin) at the current position (+ve means terrain below vehicle is above ekf origin's altitude)
bool AG_WPNav::get_terrain_offset(float& offset_cm)
{
...
bool AG_WPNav::set_spline_destination_loc(const Location& destination, const Location& next_destination, bool next_is_spline)
{
    // convert destination location to vector
    Vector3f dest_neu;
    bool dest_terr_alt;
...
bool AG_WPNav::set_spline_destination_next_loc(const Location& next_destination, const Location& next_next_destination, bool next_next_is_spline)
{
...
bool AG_WPNav::set_spline_destination(const Vector3f& destination, bool terrain_alt, const Vector3f& next_destination, bool next_terrain_alt, bool next_is_spline)
{
    // re-initialise if previous destination has been interrupted
    if (!is_active() || !_flags.reached_destination) {
        wp_and_spline_init(_wp_desired_speed_xy_cms);
...
bool AG_WPNav::set_spline_destination_next(const Vector3f& next_destination, bool next_terrain_alt, const Vector3f& next_next_destination, bool next_next_terrain_alt, bool next_next_is_spline)
{
    // do not add next point if alt types don't match
...
bool AG_WPNav::get_vector_NEU(const Location &loc, Vector3f &vec, bool &terrain_alt)
{
...
void AG_WPNav::calc_scurve_jerk_and_snap()
{
    // calculate jerk
