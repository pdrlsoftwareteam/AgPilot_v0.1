...
const AG_Param::GroupInfo AG_Circle::var_info[] = {
    // @Param: RADIUS
    ...
    // @Increment: 100
    // @User: Standard
    AP_GROUPINFO("RADIUS",  0,  AG_Circle, _radius_parm, AC_CIRCLE_RADIUS_DEFAULT),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("RATE",    1, AG_Circle, _rate_parm,    AC_CIRCLE_RATE_DEFAULT),
    ...
    // @User: Standard
    AP_GROUPINFO("OPTIONS", 2, AG_Circle, _options, 1),
...
// their values.
//
AG_Circle::AG_Circle(const AG_InertialNav& inav, const AG_AHRS_View& ahrs, AG_PosControl& pos_control) :
    _inav(inav),
    _ahrs(ahrs),
...
void AG_Circle::init(const Vector3p& center, bool terrain_alt, float rate_deg_per_sec)
{
    _center = center;
...
void AG_Circle::set_center(const Location& center)
{
...
void AG_Circle::set_rate(float deg_per_sec)
{
...
/// set_circle_rate - set circle rate in degrees per second
void AG_Circle::set_radius_cm(float radius_cm)
{
...
bool AG_Circle::is_active() const
{
    return (AG_HAL::millis() - _last_update_ms < 200);
}

/// update - update circle controller
bool AG_Circle::update(float climb_rate_cms)
{
...
void AG_Circle::get_closest_point_on_circle(Vector3f &result) const
{
...
void AG_Circle::calc_velocities(bool init_velocity)
{
...
void AG_Circle::init_start_angle(bool use_heading)
{
...
AG_Circle::TerrainSource AG_Circle::get_terrain_source() const
{
    // use range finder if connected
    if (_rangefinder_available) {
        return AG_Circle::TerrainSource::TERRAIN_FROM_RANGEFINDER;
    }
#if AP_TERRAIN_AVAILABLE
    const AG_Terrain *terrain = AG_Terrain::get_singleton();
    if ((terrain != nullptr) && terrain->enabled()) {
        return AG_Circle::TerrainSource::TERRAIN_FROM_TERRAINDATABASE;
    } else {
        return AG_Circle::TerrainSource::TERRAIN_UNAVAILABLE;
    }
#else
    return AG_Circle::TerrainSource::TERRAIN_UNAVAILABLE;
#endif
...
bool AG_Circle::get_terrain_offset(float& offset_cm)
{
    // calculate offset based on source (rangefinder or terrain database)
    switch (get_terrain_source()) {
    case AG_Circle::TerrainSource::TERRAIN_UNAVAILABLE:
        return false;
    case AG_Circle::TerrainSource::TERRAIN_FROM_RANGEFINDER:
        if (_rangefinder_healthy) {
            offset_cm = _rangefinder_terrain_offset_cm;
            return true;
        }
        return false;
    case AG_Circle::TerrainSource::TERRAIN_FROM_TERRAINDATABASE:
#if AP_TERRAIN_AVAILABLE
        float terr_alt = 0.0f;
        AG_Terrain *terrain = AG_Terrain::get_singleton();
...
void AG_Circle::check_param_change()
{
