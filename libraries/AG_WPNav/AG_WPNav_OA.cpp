#include <AG_Math/control.h>
#include <AG_InternalError/AG_InternalError.h>
#include "AG_WPNav_OA.h"

AG_WPNav_OA::AG_WPNav_OA(const AG_InertialNav& inav, const AG_AHRS_View& ahrs, AG_PosControl& pos_control, const AG_AttitudeControl& attitude_control) :
    AG_WPNav(inav, ahrs, pos_control, attitude_control)
{
}
...
bool AG_WPNav_OA::get_oa_wp_destination(Location& destination) const
{
    // if oa inactive return unadjusted location
    if (_oa_state == AG_OAGathPlanner::OA_NOT_REQUIRED) {
...
bool AG_WPNav_OA::set_wp_destination(const Vector3f& destination, bool terrain_alt)
{
    const bool ret = AG_WPNav::set_wp_destination(destination, terrain_alt);

    if (ret) {
        // reset object avoidance state
        _oa_state = AG_OAGathPlanner::OA_NOT_REQUIRED;
    }
...
float AG_WPNav_OA::get_wp_distance_to_destination() const
{
    if (_oa_state == AG_OAGathPlanner::OA_NOT_REQUIRED) {
        return AG_WPNav::get_wp_distance_to_destination();
    }
...
int32_t AG_WPNav_OA::get_wp_bearing_to_destination() const
{
    if (_oa_state == AG_OAGathPlanner::OA_NOT_REQUIRED) {
        return AG_WPNav::get_wp_bearing_to_destination();
    }
...
bool AG_WPNav_OA::reached_wp_destination() const
{
    return (_oa_state == AG_OAGathPlanner::OA_NOT_REQUIRED) && AG_WPNav::reached_wp_destination();
}

/// update_wpnav - run the wp controller - should be called at 100hz or higher
bool AG_WPNav_OA::update_wpnav()
{
    // run path planning around obstacles
    AG_OAGathPlanner *oa_ptr = AG_OAGathPlanner::get_singleton();
    Location current_loc;
	if (((oa_ptr != nullptr) && AP::ahrs().get_location(current_loc)) && !oa_ptr->_auto_flag) {
        ...
        AG_OAGathPlanner::OAPathPlannerUsed path_planner_used = AG_OAGathPlanner::OAPathPlannerUsed::None;
        const AG_OAGathPlanner::OA_RetState oa_retstate = oa_ptr->mission_avoidance(current_loc, origin_loc, destination_loc, oa_origin_new, oa_destination_new, path_planner_used);

        switch (oa_retstate) {

        case AG_OAGathPlanner::OA_NOT_REQUIRED:
        ...
        case AG_OAGathPlanner::OA_PROCESSING:
        case AG_OAGathPlanner::OA_ERROR:
            // during processing or in case of error stop the vehicle
            // by setting the oa_destination to a stopping point
            if ((_oa_state != AG_OAGathPlanner::OA_PROCESSING) && (_oa_state != AG_OAGathPlanner::OA_ERROR)) {
        ...
        case AG_OAGathPlanner::OA_SUCCESS:
            ...
            case AG_OAGathPlanner::OAPathPlannerUsed::Dijkstras:
                ...
                if ((_oa_state != AG_OAGathPlanner::OA_SUCCESS) || !oa_destination_new.same_latlon_as(_oa_destination)) {
            ...
            case AG_OAGathPlanner::OAPathPlannerUsed::BendyRulerHorizontal: {
                _oa_state = oa_retstate;
                _oa_destination = oa_destination_new;
                ...
                // return success without calling parent AG_WPNav
                return true;
            }

            case AG_OAGathPlanner::OAPathPlannerUsed::BendyRulerVertical: {
                _oa_state = oa_retstate;
                _oa_destination = oa_destination_new;
