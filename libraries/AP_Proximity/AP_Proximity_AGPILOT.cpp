...
#define PROXIMITY_AGPILOT_TIMEOUT_MS    500 // distance messages must arrive within this many milliseconds
...
void AP_Proximity_AGPILOT::update(void)
{
    // check for timeout and set health status
    if ((_last_update_ms == 0 || (AP_HAL::millis() - _last_update_ms > PROXIMITY_AGPILOT_TIMEOUT_MS)) &&
        (_last_upward_update_ms == 0 || (AP_HAL::millis() - _last_upward_update_ms > PROXIMITY_AGPILOT_TIMEOUT_MS))) {
...
bool AP_Proximity_AGPILOT::get_upward_distance(float &distance) const
{
    if ((_last_upward_update_ms != 0) && (AP_HAL::millis() - _last_upward_update_ms <= PROXIMITY_AGPILOT_TIMEOUT_MS)) {
...
void AP_Proximity_AGPILOT::handle_msg(const mavlink_message_t &msg)
{   
    switch (msg.msgid) {
...
void AP_Proximity_AGPILOT::handle_distance_sensor_msg(const mavlink_message_t &msg)
{
    mavlink_distance_sensor_t packet;
    mavlink_msg_distance_sensor_decode(&msg, &packet);

    // store distance to appropriate sector based on orientation field
    if (packet.orientation <= AGPILOT_SENSOR_ROTATION_YAW_315) {
    ...
    if (packet.orientation == AGPILOT_SENSOR_ROTATION_PITCH_90) {
...
void AP_Proximity_AGPILOT::handle_obstacle_distance_msg(const mavlink_message_t &msg)
{
    ...
    const uint8_t total_distances = MIN(((360.0f / fabsf(increment)) + 0.5f), AGPILOTLINK_MSG_OBSTACLE_DISTANCE_FIELD_DISTANCES_LEN); // usually 72
...
void AP_Proximity_AGPILOT::handle_obstacle_distance_3d_msg(const mavlink_message_t &msg)
{
