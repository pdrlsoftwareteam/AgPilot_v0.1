...
#include "AG_WindVane.h"

#include "AG_WindVane_Home.h"
#include "AG_WindVane_Analog.h"
#include "AG_WindVane_ModernDevice.h"
#include "AG_WindVane_Airspeed.h"
#include "AG_WindVane_RPM.h"
#include "AG_WindVane_SITL.h"
#include "AG_WindVane_NMEA.h"
...
const AG_Param::GroupInfo AG_WindVane::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("TYPE", 1, AG_WindVane, _direction_type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Standard
    AP_GROUPINFO("DIR_PIN", 3, AG_WindVane, _dir_analog_pin, WINDVANE_DEFAULT_PIN),
    ...
    // @Range: 0 5.0
    // @User: Standard
    AP_GROUPINFO("DIR_V_MIN", 4, AG_WindVane, _dir_analog_volt_min, 0.0f),
    ...
    // @Range: 0 5.0
    // @User: Standard
    AP_GROUPINFO("DIR_V_MAX", 5, AG_WindVane, _dir_analog_volt_max, 3.3f),
    ...
    // @Range: 0 360
    // @User: Standard
    AP_GROUPINFO("DIR_OFS", 6, AG_WindVane, _dir_analog_bearing_offset, 0.0f),
    ...
    // @Units: Hz
    // @User: Standard
    AP_GROUPINFO("DIR_FILT", 7, AG_WindVane, _dir_filt_hz, 0.5f),
    ...
    // @User: Standard
    AP_GROUPINFO("CAL", 8, AG_WindVane, _calibration, 0),
    ...
    // @Range: 0 360
    // @User: Standard
    AP_GROUPINFO("DIR_DZ", 9, AG_WindVane, _dir_analog_deadzone, 0),
    ...
    // @Range: 0 5
    // @User: Standard
    AP_GROUPINFO("SPEED_MIN", 10, AG_WindVane, _dir_speed_cutoff, 0),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("SPEED_TYPE", 11, AG_WindVane, _speed_sensor_type,  0),
    ...
    // @Values: 11:Pixracer,13:Pixhawk ADC4,14:Pixhawk ADC3,15:Pixhawk ADC6/Pixhawk2 ADC,50:AUX1,51:AUX2,52:AUX3,53:AUX4,54:AUX5,55:AUX6,103:Pixhawk SBUS
    // @User: Standard
    AP_GROUPINFO("SPEED_PIN", 12, AG_WindVane, _speed_sensor_speed_pin,  WINDSPEED_DEFAULT_SPEED_PIN),
    ...
    // @Values: 11:Pixracer,13:Pixhawk ADC4,14:Pixhawk ADC3,15:Pixhawk ADC6/Pixhawk2 ADC,50:AUX1,51:AUX2,52:AUX3,53:AUX4,54:AUX5,55:AUX6,103:Pixhawk SBUS
    // @User: Standard
    AP_GROUPINFO("TEMP_PIN", 13, AG_WindVane, _speed_sensor_temp_pin,  WINDSPEED_DEFAULT_TEMP_PIN),
    ...
    // @Range: 0 3.3
    // @User: Standard
    AP_GROUPINFO("SPEED_OFS", 14, AG_WindVane, _speed_sensor_voltage_offset, WINDSPEED_DEFAULT_VOLT_OFFSET),
    ...
    // @Units: Hz
    // @User: Standard
    AP_GROUPINFO("SPEED_FILT", 15, AG_WindVane, _speed_filt_hz, 0.5f),
    ...
    // @Units: Hz
    // @User: Standard
    AP_GROUPINFO("TRUE_FILT", 16, AG_WindVane, _true_filt_hz, 0.05f),

    AP_GROUPEND
};

// constructor
AG_WindVane::AG_WindVane()
{
    AG_Param::setup_object_defaults(this, var_info);
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton) {
        AG_HAL::panic("Too many Wind Vane sensors");
    }
#endif
    _singleton = this;
}
...
/*
 * Get the AG_WindVane singleton
 */
AG_WindVane *AG_WindVane::get_singleton()
{
    return _singleton;
}

// return true if wind vane is enabled
bool AG_WindVane::enabled() const
{
    return _direction_type != WINDVANE_NONE;
}

// return true if wind speed is enabled
bool AG_WindVane::wind_speed_enabled() const
{
    return (_speed_sensor_type != WINDSPEED_NONE);
}

// Initialize the Wind Vane object and prepare it for use
void AG_WindVane::init(const AG_SerialManager& serial_manager)
{
    ...
    switch (_direction_type) {
        ...
        case WindVaneType::WINDVANE_PWM_PIN:
            _direction_driver = new AG_WindVane_Home(*this);
            break;
        case WindVaneType::WINDVANE_ANALOG_PIN:
            _direction_driver = new AG_WindVane_Analog(*this);
            break;
        case WindVaneType::WINDVANE_SITL_TRUE:
        case WindVaneType::WINDVANE_SITL_APPARENT:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            _direction_driver = new AG_WindVane_SITL(*this);
#endif
            break;
        case WindVaneType::WINDVANE_NMEA:
            _direction_driver = new AG_WindVane_NMEA(*this);
    ...
    switch (_speed_sensor_type) {
        ...
        case Speed_type::WINDSPEED_AIRSPEED:
            _speed_driver = new AG_WindVane_Airspeed(*this);
            break;
        case Speed_type::WINDVANE_WIND_SENSOR_REV_P:
            _speed_driver = new AG_WindVane_ModernDevice(*this);
            break;
        ...
        case Speed_type::WINDSPEED_SITL_APPARENT:
            ...
            if (_direction_type != _speed_sensor_type) {
                _speed_driver = new AG_WindVane_SITL(*this);
            } else {
        ...
        case Speed_type::WINDSPEED_NMEA:
            ...
            if (_direction_type != WindVaneType::WINDVANE_NMEA) {
                _speed_driver = new AG_WindVane_NMEA(*this);
        ...
        case Speed_type::WINDSPEED_RPM:
            _speed_driver = new AG_WindVane_RPM(*this);
            break;
    }
}

// update wind vane, expected to be called at 20hz
void AG_WindVane::update()
{
...
void AG_WindVane::record_home_heading()
{
    _home_heading = AP::ahrs().yaw;
}

// to start direction calibration from mavlink or other
bool AG_WindVane::start_direction_calibration()
{
...
bool AG_WindVane::start_speed_calibration()
{
    if (enabled() && (_calibration == 0)) {
        _calibration.set(2);
        return true;
...
void AG_WindVane::send_wind(mavlink_channel_t chan) const
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
void AG_WindVane::update_true_wind_speed_and_direction()
{
...
void AG_WindVane::update_apparent_wind_dir_from_true()
{
    // if no vehicle speed, can't do calcs
    Vector3f veh_velocity;
    if (!AP::ahrs().get_velocity_NED(veh_velocity)) {
...
AG_WindVane *AG_WindVane::_singleton = nullptr;

namespace AP {
    AG_WindVane *windvane()
    {
        return AG_WindVane::get_singleton();
    }
};
