...
void AG_WindVane_NMEA::init(const AG_SerialManager& serial_manager)
{
    uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_WindVane, 0);
    if (uart != nullptr) {
        uart->begin(serial_manager.find_baudrate(AG_SerialManager::SerialProtocol_WindVane, 0));
    }
}

void AG_WindVane_NMEA::update_direction()
{
...
void AG_WindVane_NMEA::update_speed()
{
    update();
}

void AG_WindVane_NMEA::update()
{
    if (uart == nullptr) {
        return;
    }
...
bool AG_WindVane_NMEA::decode(char c)
{
    switch (c) {
    case ',':
...
bool AG_WindVane_NMEA::decode_latest_term()
{
