...
// constructor
AG_WindVane_Analog::AG_WindVane_Analog(AG_WindVane &frontend) :
    AG_WindVane_Backend(frontend)
{
    _dir_analog_source = hal.analogin->channel(ANALOG_INPUT_NONE);
}

void AG_WindVane_Analog::update_direction()
{
...
void AG_WindVane_Analog::calibrate()
{

    // start calibration
    if (_cal_start_ms == 0) {
        _cal_start_ms = AG_HAL::millis();
    ...
    if ((AG_HAL::millis() - _cal_start_ms) > 30000) {
