...
AG_UAVCAN_DNA_Server::AG_UAVCAN_DNA_Server(AG_UAVCAN *ap_uavcan, StorageAccess _storage) :
...
void AG_UAVCAN_DNA_Server::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (AllocationListener == nullptr) {
        AG_BoardConfig::allocation_error("AG_UAVCAN_DNA: AllocationListener");
    }
    ...
    if (alloc_listener_res < 0) {
        AG_HAL::panic("Allocation Subscriber start problem\n\r");
        return;
    }
    ...
    if (NodeStatusListener == nullptr) {
        AG_BoardConfig::allocation_error("AG_UAVCAN_DNA: NodeStatusListener");
    }
    ...
    if (nodestatus_listener_res < 0) {
        AG_HAL::panic("NodeStatus Subscriber start problem\n\r");
        return;
    }
}
...
void AG_UAVCAN_DNA_Server::getHash(NodeData &node_data, const uint8_t unique_id[], uint8_t size) const
{
...
bool AG_UAVCAN_DNA_Server::readNodeData(NodeData &data, uint8_t node_id)
{
...
bool AG_UAVCAN_DNA_Server::writeNodeData(const NodeData &data, uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
bool AG_UAVCAN_DNA_Server::setOccupationMask(uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
bool AG_UAVCAN_DNA_Server::freeNodeID(uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
Server Records. */
void AG_UAVCAN_DNA_Server::setVerificationMask(uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return;
    }
...
bool AG_UAVCAN_DNA_Server::isNodeIDOccupied(uint8_t node_id) const
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
bool AG_UAVCAN_DNA_Server::isNodeIDVerified(uint8_t node_id) const
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
uint8_t AG_UAVCAN_DNA_Server::getNodeIDForUniqueID(const uint8_t unique_id[], uint8_t size)
{
    uint8_t node_id = 255;
...
bool AG_UAVCAN_DNA_Server::addNodeIDForUniqueID(uint8_t node_id, const uint8_t unique_id[], uint8_t size)
{
    NodeData node_data;
...
bool AG_UAVCAN_DNA_Server::isValidNodeDataAvailable(uint8_t node_id)
{
    NodeData node_data;
...
Server Record. */
bool AG_UAVCAN_DNA_Server::init()
{
    ...
    if (allocation_pub[driver_index] == nullptr) {
        AG_BoardConfig::allocation_error("AG_UAVCAN_DNA: allocation_pub[%d]", driver_index);
    }
    ...
    if (getNodeInfo_client[driver_index] == nullptr) {
        AG_BoardConfig::allocation_error("AG_UAVCAN_DNA: getNodeInfo_client[%d]", driver_index);
    }
    ...
    if (_ap_uavcan->check_and_reset_option(AG_UAVCAN::Options::DNA_CLEAR_DATABASE)) {
...
void AG_UAVCAN_DNA_Server::reset()
{
    NodeData node_data;
    memset(&node_data, 0, sizeof(node_data));
...
uint8_t AG_UAVCAN_DNA_Server::findFreeNodeID(uint8_t preferred)
{
    // Search up
...
bool AG_UAVCAN_DNA_Server::isNodeSeen(uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return false;
    }
...
void AG_UAVCAN_DNA_Server::addToSeenNodeMask(uint8_t node_id)
{
    if (node_id > MAX_NODE_ID) {
        return;
    }
...
on the bus. */
void AG_UAVCAN_DNA_Server::verify_nodes()
{
...
void AG_UAVCAN_DNA_Server::handleNodeStatus(uint8_t node_id, const NodeStatusCb &cb)
{
    if (node_id > MAX_NODE_ID) {
        return;
    }
    if ((cb.msg->health != uavcan::protocol::NodeStatus::HEALTH_OK ||
        ...
        !_ap_uavcan->option_is_set(AG_UAVCAN::Options::DNA_IGNORE_UNHEALTHY_NODE)) {
...
void AG_UAVCAN_DNA_Server::trampoline_handleNodeStatus(AG_UAVCAN* ap_uavcan, uint8_t node_id, const NodeStatusCb &cb)
{
    if (ap_uavcan == nullptr) {
        return;
    }
...
received Unique ID */
void AG_UAVCAN_DNA_Server::handleNodeInfo(uint8_t node_id, uint8_t unique_id[], char name[], uint8_t major, uint8_t minor, uint32_t vcs_commit)
{
    if (node_id > MAX_NODE_ID) {
        return;
    }
    ...
    if (isNodeIDOccupied(node_id)) {
        ...
        if (node_id == getNodeIDForUniqueID(unique_id, 16)) {
            ...
            setVerificationMask(node_id);
        } else if (!_ap_uavcan->option_is_set(AG_UAVCAN::Options::DNA_IGNORE_DUPLICATE_NODE)) {
...
void AG_UAVCAN_DNA_Server::trampoline_handleNodeInfo(AG_UAVCAN* ap_uavcan, uint8_t node_id, const GetNodeInfoCb& resp)
{
...
void AG_UAVCAN_DNA_Server::handleAllocation(uint8_t node_id, const AllocationCb &cb)
{
    if (allocation_pub[driver_index] == nullptr) {
    ...
    if (rcvd_unique_id_offset) {
        debug_uavcan(AG_CANManager::LOG_DEBUG, "TIME: %ld  -- Accepting Followup part! %u\n",
                     long(uavcan::SystemClock::instance().getMonotonic().toUSec()/1000),
                     unsigned((now - last_alloc_msg_ms)));
    } else {
        debug_uavcan(AG_CANManager::LOG_DEBUG, "TIME: %ld  -- Accepting First part! %u\n",
                     long(uavcan::SystemClock::instance().getMonotonic().toUSec()/1000),
                     unsigned((now - last_alloc_msg_ms)));
    }
...
void AG_UAVCAN_DNA_Server::trampoline_handleAllocation(AG_UAVCAN* ap_uavcan, uint8_t node_id, const AllocationCb &cb)
{
    if (ap_uavcan == nullptr) {
        return;
    }
...
bool AG_UAVCAN_DNA_Server::prearm_check(char* fail_msg, uint8_t fail_msg_len) const
{
    switch (server_state) {
    case HEALTHY:
        return true;
    ...
    case DUPLICATE_NODES: {
        if (_ap_uavcan->option_is_set(AG_UAVCAN::Options::DNA_IGNORE_DUPLICATE_NODE)) {
            // ignore error
            return true;
        }
    ...
    case NODE_STATUS_UNHEALTHY: {
        if (_ap_uavcan->option_is_set(AG_UAVCAN::Options::DNA_IGNORE_UNHEALTHY_NODE)) {
            // ignore error
            return true;
        }
