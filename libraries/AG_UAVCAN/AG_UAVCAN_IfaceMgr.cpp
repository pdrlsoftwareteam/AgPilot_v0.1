...
int16_t CanIface::send(const CanFrame& frame, MonotonicTime tx_deadline, CanIOFlags flags)
{
    ...
    return can_iface_->send(AG_HAL::CANFrame(frame.id, frame.data, AG_HAL::CANFrame::dlcToDataLength(frame.dlc), frame.isCanFDFrame()), tx_deadline.toUSec(), flags);
}
                          ...
                          CanIOFlags& out_flags)
{
    ...
    out_frame = CanFrame(frame.id, (const uint8_t*)frame.data, AG_HAL::CANFrame::dlcToDataLength(frame.dlc), frame.canfd);
    out_flags = flags;
    if (rx_timestamp != 0) {
...
bool CanIfaceMgr::add_interface(AG_HAL::CANIface *can_iface)
{
    if (num_ifaces > HAL_NUM_CAN_IFACES) {
        AP::can().log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "UAVCANIfaceMgr: Num Ifaces Exceeded\n");
        return false;
    }
    if (can_iface == nullptr) {
        AP::can().log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "UAVCANIfaceMgr: Iface Null\n");
        return false;
    }
    if (ifaces[num_ifaces] != nullptr) {
        AP::can().log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "UAVCANIfaceMgr: Iface already added\n");
        return false;
    }
    ifaces[num_ifaces] = new CanIface(can_iface);
    if (ifaces[num_ifaces] == nullptr) {
        AP::can().log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "UAVCANIfaceMgr: Can't alloc uavcan::iface\n");
        return false;
    }
    if (!ifaces[num_ifaces]->can_iface_->set_event_handle(&_event_handle)) {
        AP::can().log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "UAVCANIfaceMgr: Setting event handle failed\n");
        return false;
    }
    AP::can().log_text(AG_CANManager::LOG_INFO, LOG_TAG, "UAVCANIfaceMgr: Successfully added interface %d\n", int(num_ifaces));
    num_ifaces++;
    return true;
}
/**
...
CanSelectMasks CanIfaceMgr::makeSelectMasks(const CanSelectMasks in_mask, const CanFrame* (& pending_tx)[MaxCanIfaces]) const
{
    ...
    for (uint8_t i = 0; i < num_ifaces; i++) {
        ...
        if (pending_tx[i] == UAVCAN_NULLPTR) {
            ...
            AG_HAL::CANFrame frame {pending_tx[i]->id, pending_tx[i]->data, AG_HAL::CANFrame::dlcToDataLength(pending_tx[i]->dlc)};
