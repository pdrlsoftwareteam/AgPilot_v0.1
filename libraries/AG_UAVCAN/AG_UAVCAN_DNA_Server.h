...
class AG_UAVCAN_DNA_Server
{
    ...
    static void trampoline_handleNodeInfo(AG_UAVCAN* ap_uavcan, uint8_t node_id, const GetNodeInfoCb& resp);
    static void trampoline_handleAllocation(AG_UAVCAN* ap_uavcan, uint8_t node_id, const AllocationCb &cb);
    static void trampoline_handleNodeStatus(AG_UAVCAN* ap_uavcan, uint8_t node_id, const NodeStatusCb &cb);
...
public:
    AG_UAVCAN_DNA_Server(AG_UAVCAN *ap_uavcan, StorageAccess _storage);


    // Do not allow copies
    CLASS_NO_COPY(AG_UAVCAN_DNA_Server);
