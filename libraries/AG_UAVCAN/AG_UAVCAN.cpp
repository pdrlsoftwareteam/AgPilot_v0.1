...
#include <AG_Baro/AG_Baro_UAVCAN.h>
#include <AG_RangeFinder/AG_RangeFinder_UAVCAN.h>
#include <AG_EFI/AG_EFI_DroneCAN.h>
#include <AG_GPS/AG_GPS_UAVCAN.h>
#include <AG_GPS/AG_GPS.h>
#include <AG_BattMonitor/AG_BattMonitor_UAVCAN.h>
#include <AG_Compass/AG_Compass_UAVCAN.h>
#include <AG_Airspeed/AG_Airspeed_UAVCAN.h>
#include <SRV_Channel/SRV_Channel.h>
#include <AG_OpticalFlow/AG_OpticalFlow_HereFlow.h>
#include <AG_ADSB/AG_ADSB.h>
#include "AG_UAVCAN_DNA_Server.h"
#include <AG_Logger/AG_Logger.h>
#include <AG_Notify/AG_Notify.h>
#include <AG_OpenDroneID/AG_OpenDroneID.h>
#include "AG_UAVCAN_pool.h"
#include <AG_Proximity/AG_Proximity_DroneCAN.h>
...
const AG_Param::GroupInfo AG_UAVCAN::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("SRV_BM", 2, AG_UAVCAN, _servo_bm, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("ESC_BM", 3, AG_UAVCAN, _esc_bm, 0),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_GROUPINFO("SRV_RT", 4, AG_UAVCAN, _servo_rate_hz, 50),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTION", 5, AG_UAVCAN, _options, 0),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_GROUPINFO("NTF_RT", 6, AG_UAVCAN, _notify_state_hz, 20),
    ...
    // @Range: 0 18
    // @User: Advanced
    AP_GROUPINFO("ESC_OF", 7, AG_UAVCAN, _esc_offset, 0),
    ...
    // @Range: 1024 16384
    // @User: Advanced
    AP_GROUPINFO("POOL", 8, AG_UAVCAN, _pool_size, UAVCAN_NODE_POOL_SIZE),
...
AG_UAVCAN::AG_UAVCAN()
{
    ...
    debug_uavcan(AG_CANManager::LOG_INFO, "AG_UAVCAN constructed\n\r");
}
...
AG_UAVCAN *AG_UAVCAN::get_uavcan(uint8_t driver_index)
{
    if (driver_index >= AP::can().get_num_drivers() ||
        AP::can().get_driver_type(driver_index) != AG_CANManager::Driver_Type_UAVCAN) {
        return nullptr;
    }
    return static_cast<AG_UAVCAN*>(AP::can().get_driver(driver_index));
}

bool AG_UAVCAN::add_interface(AG_HAL::CANIface* can_iface) {
    ...
    if (_iface_mgr == nullptr) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: can't create UAVCAN interface manager\n\r");
        return false;
    }

    if (!_iface_mgr->add_interface(can_iface)) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: can't add UAVCAN interface\n\r");
        return false;   
    }
    return true;
}
...
#pragma GCC diagnostic error "-Wframe-larger-than=2300"
void AG_UAVCAN::init(uint8_t driver_index, bool enable_filters)
{
    _driver_index = driver_index;

    if (_initialized) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: init called more than once\n\r");
        return;
    }

    if (_iface_mgr == nullptr) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: can't get UAVCAN interface driver\n\r");
        return;
    }
    ...
    if (_allocator == nullptr || !_allocator->init()) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: couldn't allocate node pool\n");
        return;
    }
    ...
    if (_node == nullptr) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: couldn't allocate node\n\r");
        return;
    }

    if (_node->isStarted()) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: node was already started?\n\r");
        return;
    }
    {
    ...
    }
    {
        ...
        sw_version.major = AG_UAVCAN_SW_VERS_MAJOR;
        sw_version.minor = AG_UAVCAN_SW_VERS_MINOR;
        _node->setSoftwareVersion(sw_version);

        uavcan::protocol::HardwareVersion hw_version; // Standard type uavcan.protocol.HardwareVersion

        hw_version.major = AG_UAVCAN_HW_VERS_MAJOR;
        hw_version.minor = AG_UAVCAN_HW_VERS_MINOR;
    ...
    if (start_res < 0) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: node start problem, error %d\n\r", start_res);
        return;
    }

    _dna_server = new AG_UAVCAN_DNA_Server(this, StorageAccess(StorageManager::StorageCANDNA));
    if (_dna_server == nullptr) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: couldn't allocate DNA server\n\r");
        return;
    }

    //Start Servers
    if (!_dna_server->init()) {
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: Failed to start DNA Server\n\r");
        return;
    }

    // Roundup all subscribers from supported drivers
    AG_UAVCAN_DNA_Server::subscribe_msgs(this);
    AG_GPS_UAVCAN::subscribe_msgs(this);
#if AP_COMPASS_UAVCAN_ENABLED
    AG_Compass_UAVCAN::subscribe_msgs(this);
#endif
#if AP_BARO_UAVCAN_ENABLED
    AG_Baro_UAVCAN::subscribe_msgs(this);
#endif
    AG_BattMonitor_UAVCAN::subscribe_msgs(this);
#if AP_AIRSPEED_UAVCAN_ENABLED
    AG_Airspeed_UAVCAN::subscribe_msgs(this);
#endif
#if AP_OPTICALFLOW_HEREFLOW_ENABLED
    AG_OpticalFlow_HereFlow::subscribe_msgs(this);
#endif
#if AP_RANGEFINDER_UAVCAN_ENABLED
    AG_RangeFinder_UAVCAN::subscribe_msgs(this);
#endif
#if AG_EFI_DRONECAN_ENABLED
    AG_EFI_DroneCAN::subscribe_msgs(this);
#endif

#if AP_PROXIMITY_DRONECAN_ENABLED
    AG_Proximity_DroneCAN::subscribe_msgs(this);
#endif
    ...
    param_get_set_client[driver_index] = new uavcan::ServiceClient<uavcan::protocol::param::GetSet, ParamGetSetCb>(*_node, ParamGetSetCb(this, &AG_UAVCAN::handle_param_get_set_response));

    param_execute_opcode_client[driver_index] = new uavcan::ServiceClient<uavcan::protocol::param::ExecuteOpcode, ParamExecuteOpcodeCb>(*_node, ParamExecuteOpcodeCb(this, &AG_UAVCAN::handle_param_save_response));
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_UAVCAN::loop, void), _thread_name, UAVCAN_STACK_SIZE, AG_HAL::Scheduler::PRIORITY_CAN, 0)) {
        _node->setModeOfflineAndPublish();
        debug_uavcan(AG_CANManager::LOG_ERROR, "UAVCAN: couldn't create thread\n\r");
        return;
    }

    _initialized = true;
    debug_uavcan(AG_CANManager::LOG_INFO, "UAVCAN: init done\n\r");
}
#pragma GCC diagnostic pop

void AG_UAVCAN::loop(void)
{
    while (true) {
        if (!_initialized) {
        ...
        if (_SRV_armed) {
            ...
            if (_servo_bm > 0) {
                ...
                uint32_t now = AG_HAL::native_micros();
...
#if AP_DRONECAN_SEND_GPS
        if (option_is_set(AG_UAVCAN::Options::SEND_GNSS) && !AG_GPS_UAVCAN::instance_exists(this)) {
            // send if enabled and this interface/driver is not used by the AG_GPS driver
            gnss_send_fix();
            gnss_send_yaw();
        }
#endif
...
void AG_UAVCAN::SRV_send_actuator(void)
{
 ...
 */
void AG_UAVCAN::SRV_send_himark(void)
{
    WITH_SEMAPHORE(SRV_sem);
...
void AG_UAVCAN::SRV_send_esc(void)
{
 ...
 */
void AG_UAVCAN::SRV_send_esc_hobbywing(void)
{
...
void AG_UAVCAN::SRV_push_servos()
{
    WITH_SEMAPHORE(SRV_sem);
    ...
    _SRV_armed = hal.util->safety_switch_state() != AG_HAL::Util::SAFETY_DISARMED;
}


///// LED /////

void AG_UAVCAN::led_out_send()
{
    uint64_t now = AG_HAL::native_micros64();
...
bool AG_UAVCAN::led_write(uint8_t led_index, uint8_t red, uint8_t green, uint8_t blue)
{
    if (_led_conf.devices_count >= AG_UAVCAN_MAX_LED_DEVICES) {
        return false;
    }
...
// buzzer support
void AG_UAVCAN::set_buzzer_tone(float frequency, float duration_s)
{
    WITH_SEMAPHORE(_buzzer.sem);
...
// notify state send
void AG_UAVCAN::notify_state_send()
{
    ...
    msg.vehicle_state = 0;
    if (AG_Notify::flags.initialising) {
    ...
    _last_notify_state_ms = AG_HAL::native_millis();
}

#if AP_DRONECAN_SEND_GPS
void AG_UAVCAN::gnss_send_fix()
{
    ...
    */
    ...
    pkt.timestamp.usec = AG_HAL::native_micros64();
    ...
    switch (gps.status()) {
    case AG_GPS::GPS_Status::NO_GPS:
    case AG_GPS::GPS_Status::NO_FIX:
    ...
    const uint32_t now_ms = AG_HAL::native_millis();
...
void AG_UAVCAN::rtcm_stream_send()
{
...
// SafetyState send
void AG_UAVCAN::safety_state_send()
{
    uint32_t now = AG_HAL::native_millis();
...
*/
void AG_UAVCAN::send_RTCMStream(const uint8_t *data, uint32_t len)
{
    WITH_SEMAPHORE(_rtcm_stream.sem);
    if (_rtcm_stream.buf == nullptr) {
 ...
 */
void AG_UAVCAN::handle_button(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ButtonCb &cb)
{
    switch (cb.msg->button) {
    case ardupilot::indication::Button::BUTTON_SAFETY: {
        AG_BoardConfig *brdconfig = AG_BoardConfig::get_singleton();
        if (brdconfig && brdconfig->safety_button_handle_pressed(cb.msg->press_time)) {
            AG_HAL::Util::safety_state state = hal.util->safety_switch_state();
            if (state == AG_HAL::Util::SAFETY_ARMED) {
 ...
 */
void AG_UAVCAN::handle_traffic_report(AG_UAVCAN* ap_uavcan, uint8_t node_id, const TrafficReportCb &cb)
{
#if HAL_ADSB_ENABLED
    ...
    AG_ADSB::adsb_vehicle_t vehicle;
    ...
    vehicle.last_update_ms = AG_HAL::native_millis() - (vehicle.info.tslc * 1000);
 ...
 */
void AG_UAVCAN::handle_actuator_status(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ActuatorStatusCb &cb)
{
    // log as CSRV message
    AP::logger().Write_ServoStatus(AG_HAL::native_micros64(),
                                   cb.msg->actuator_id,
                                   cb.msg->position,
                                   cb.msg->force,
                                   cb.msg->speed,
 ...
 */
void AG_UAVCAN::handle_himark_servoinfo(AG_UAVCAN* ap_uavcan, uint8_t node_id, const HimarkServoInfoCb &cb)
{
    // log as CSRV message
    AP::logger().Write_ServoStatus(AG_HAL::native_micros64(),
                                   cb.msg->servo_id,
...
#if AP_DRONECAN_VOLZ_FEEDBACK_ENABLED
void AG_UAVCAN::handle_actuator_status_Volz(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ActuatorStatusVolzCb &cb)
{
 ...
 */
void AG_UAVCAN::handle_ESC_status(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ESCStatusCb &cb)
{
    ...
    ap_uavcan->update_telem_data(esc_index, t,
        (isnan(cb.msg->current) ? 0 : AG_ESC_Telem_Backend::TelemetryType::CURRENT)
            | (isnan(cb.msg->voltage) ? 0 : AG_ESC_Telem_Backend::TelemetryType::VOLTAGE)
            | (isnan(cb.msg->temperature) ? 0 : AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE));
#endif
}

bool AG_UAVCAN::is_esc_data_index_valid(const uint8_t index) {
 ...
 */
void AG_UAVCAN::handle_debug(AG_UAVCAN* ap_uavcan, uint8_t node_id, const DebugCb &cb)
{
#if HAL_LOGGING_ENABLED
    const auto &msg = *cb.msg;
    if (AP::can().get_log_level() != AG_CANManager::LOG_NONE) {
...
void AG_UAVCAN::send_parameter_request()
{
...
bool AG_UAVCAN::set_parameter_on_node(uint8_t node_id, const char *name, float value, ParamGetSetFloatCb *cb)
{
    WITH_SEMAPHORE(_param_sem);
...
bool AG_UAVCAN::set_parameter_on_node(uint8_t node_id, const char *name, int32_t value, ParamGetSetIntCb *cb)
{
    WITH_SEMAPHORE(_param_sem);
    if (param_int_cb != nullptr ||
        param_float_cb != nullptr) {
...
bool AG_UAVCAN::get_parameter_on_node(uint8_t node_id, const char *name, ParamGetSetFloatCb *cb)
{
    WITH_SEMAPHORE(_param_sem);
    if (param_int_cb != nullptr ||
        param_float_cb != nullptr) {
...
bool AG_UAVCAN::get_parameter_on_node(uint8_t node_id, const char *name, ParamGetSetIntCb *cb)
{
    WITH_SEMAPHORE(_param_sem);
    if (param_int_cb != nullptr ||
        param_float_cb != nullptr) {
...
void AG_UAVCAN::handle_param_get_set_response(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ParamGetSetCb &cb)
{
...
void AG_UAVCAN::send_parameter_save_request()
{
...
bool AG_UAVCAN::save_parameters_on_node(uint8_t node_id, ParamSaveCb *cb)
{
    WITH_SEMAPHORE(_param_save_sem);
...
void AG_UAVCAN::handle_param_save_response(AG_UAVCAN* ap_uavcan, uint8_t node_id, const ParamExecuteOpcodeCb &cb)
{
...
void AG_UAVCAN::send_reboot_request(uint8_t node_id)
{
    if (_node == nullptr) {
        return;
    }
...
bool AG_UAVCAN::check_and_reset_option(Options option)
{
...
// handle prearm check
bool AG_UAVCAN::prearm_check(char* fail_msg, uint8_t fail_msg_len) const
{
 ...
 */
void AG_UAVCAN::hobbywing_ESC_update(void)
{
 ...
 */
void AG_UAVCAN::handle_hobbywing_GetEscID(AG_UAVCAN* ap_uavcan, uint8_t node_id, const HobbywingESCIDCb &cb)
{
    const auto &msg = *cb.msg;
 ...
 */
bool AG_UAVCAN::hobbywing_find_esc_index(uint8_t node_id, uint8_t &esc_index) const
{
 ...
 */
void AG_UAVCAN::handle_hobbywing_StatusMsg1(AG_UAVCAN* ap_uavcan, uint8_t node_id, const HobbywingStatus1Cb &cb)
{
    uint8_t esc_index;
 ...
 */
void AG_UAVCAN::handle_hobbywing_StatusMsg2(AG_UAVCAN* ap_uavcan, uint8_t node_id, const HobbywingStatus2Cb &cb)
{
    uint8_t esc_index;
    if (ap_uavcan->hobbywing_find_esc_index(node_id, esc_index)) {
        TelemetryData t {
        ...
        ap_uavcan->update_telem_data(esc_index, t,
                                     AG_ESC_Telem_Backend::TelemetryType::CURRENT|
                                     AG_ESC_Telem_Backend::TelemetryType::VOLTAGE|
                                     AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE);
    }
