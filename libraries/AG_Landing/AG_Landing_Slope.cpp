...
/*
 *   AG_Landing_Slope.cpp - Landing logic handler for ArduPlane for STANDARD_GLIDE_SLOPE
 */
...
void AG_Landing::type_slope_do_land(const AG_Mission::Mission_Command& cmd, const float relative_altitude)
{
    initial_slope = 0;
    slope = 0;
...
void AG_Landing::type_slope_verify_abort_landing(const Location &prev_WP_loc, Location &next_WP_loc, bool &throttle_suppressed)
{
...
/*
  ...
  final flare
 */
bool AG_Landing::type_slope_verify_land(const Location &prev_WP_loc, Location &next_WP_loc, const Location &current_loc,
    ...
    */
    ...
    if ((on_approach_stage && below_flare_alt) ||
        ...
        if (type_slope_stage != SlopeStage::FINAL) {
            ...
            if (is_flying && (AG_HAL::millis()-last_flying_ms) > 3000) {
            ...
            // If not - go around
            AG_LandingGear *LG_inst = AG_LandingGear::get_singleton();
...
void AG_Landing::type_slope_adjust_landing_slope_for_rangefinder_bump(AG_FixedWing::Rangefinder_State &rangefinder_state, Location &prev_WP_loc, Location &next_WP_loc, const Location &current_loc, const float wp_distance, int32_t &target_altitude_offset_cm)
{
...
bool AG_Landing::type_slope_request_go_around(void)
{
    flags.commanded_go_around = true;
    return true;
}
 ...
 */
void AG_Landing::type_slope_setup_landing_glide_slope(const Location &prev_WP_loc, const Location &next_WP_loc, const Location &current_loc, int32_t &target_altitude_offset_cm)
{
...
int32_t AG_Landing::type_slope_get_target_airspeed_cm(void)
{
    ...
    const uint32_t max_airspeed_cm = AG_Landing::allow_max_airspeed_on_land() ? aparm.airspeed_max*100 : aparm.airspeed_cruise_cm;
    
    return constrain_int32(target_airspeed_cm + head_wind_compensation_cm, target_airspeed_cm, max_airspeed_cm);
    
}

int32_t AG_Landing::type_slope_constrain_roll(const int32_t desired_roll_cd, const int32_t level_roll_limit_cd)
{
    if (type_slope_stage == SlopeStage::FINAL) {
...
bool AG_Landing::type_slope_is_flaring(void) const
{
    return (type_slope_stage == SlopeStage::FINAL);
}

bool AG_Landing::type_slope_is_on_approach(void) const
{
    return (type_slope_stage == SlopeStage::APPROACH ||
            type_slope_stage == SlopeStage::PREFLARE);
}

bool AG_Landing::type_slope_is_expecting_impact(void) const
{
    return (type_slope_stage == SlopeStage::PREFLARE ||
            type_slope_stage == SlopeStage::FINAL);
}

bool AG_Landing::type_slope_is_complete(void) const
{
    return (type_slope_stage == SlopeStage::FINAL);
}

void AG_Landing::type_slope_log(void) const
{
// @LoggerMessage: LAND
...
bool AG_Landing::type_slope_is_throttle_suppressed(void) const
{
    return type_slope_stage == SlopeStage::FINAL;
}
