...
/*
 *   AG_Landing.cpp - Landing logic handler for ArduPlane
 */

#include "AG_Landing.h"
#include <GCS_MAVLink/GCS.h>
#include <AG_AHRS/AG_AHRS.h>
#include <AG_Fence/AG_Fence.h>

// table of user settable parameters
const AG_Param::GroupInfo AG_Landing::var_info[] = {
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_GROUPINFO("SLOPE_RCALC", 1, AG_Landing, slope_recalc_shallow_threshold, 2.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("ABORT_DEG", 2, AG_Landing, slope_recalc_steep_threshold_to_abort, 0),
    ...
    // @Increment: 10
    // @User: Advanced
    AP_GROUPINFO("PITCH_CD", 3, AG_Landing, pitch_cd, 0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("FLARE_ALT", 4, AG_Landing, flare_alt, 3.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("FLARE_SEC", 5, AG_Landing, flare_sec, 2.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("PF_ALT", 6, AG_Landing, pre_flare_alt, 10.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("PF_SEC", 7, AG_Landing, pre_flare_sec, 6.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("PF_ARSPD", 8, AG_Landing, pre_flare_airspeed, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("THR_SLEW", 9, AG_Landing, throttle_slewrate, 0),
    ...
    // @Range: 0 127
    // @User: Advanced
    AP_GROUPINFO("DISARMDELAY", 10, AG_Landing, disarm_delay, 20),
    ...
    // @User: Advanced
    AP_GROUPINFO("THEN_NEUTRL", 11, AG_Landing, then_servos_neutral, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("ABORT_THR", 12, AG_Landing, abort_throttle_enable, 0),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("FLAP_PERCNT", 13, AG_Landing, flap_percent, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 16, AG_Landing, _options, 0),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("FLARE_AIM", 17, AG_Landing, flare_effectivness_pct, 50),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("WIND_COMP", 18, AG_Landing, wind_comp, 50),
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("TYPE",    14, AG_Landing, type, TYPE_STANDARD_GLIDE_SLOPE, AP_PARAM_FLAG_ENABLE),

#if HAL_LANDING_DEEPSTALL_ENABLED
    // @Group: DS_
    // @Path: AG_Landing_Deepstall.cpp
    AP_SUBGROUPINFO(deepstall, "DS_", 15, AG_Landing, AG_Landing_Deepstall),
#endif
    ...
    // constructor
AG_Landing::AG_Landing(AG_Mission &_mission, AG_AHRS &_ahrs, AG_TECS *_tecs_Controller, AG_Navigation *_nav_controller, AG_FixedWing &_aparm,
...
void AG_Landing::do_land(const AG_Mission::Mission_Command& cmd, const float relative_altitude)
{
...
/*
  ...
  final flare
 */
bool AG_Landing::verify_land(const Location &prev_WP_loc, Location &next_WP_loc, const Location &current_loc,
...
bool AG_Landing::verify_abort_landing(const Location &prev_WP_loc, Location &next_WP_loc, const Location &current_loc,
...
void AG_Landing::adjust_landing_slope_for_rangefinder_bump(AG_FixedWing::Rangefinder_State &rangefinder_state, Location &prev_WP_loc, Location &next_WP_loc, const Location &current_loc, const float wp_distance, int32_t &target_altitude_offset_cm)
{
    switch (type) {
    case TYPE_STANDARD_GLIDE_SLOPE:
...
bool AG_Landing::send_landing_message(mavlink_channel_t chan) {
...
bool AG_Landing::is_flaring(void) const
{
    if (!flags.in_progress) {
        return false;
    }
...
bool AG_Landing::is_on_approach(void) const
{
    if (!flags.in_progress) {
        return false;
    }
...
bool AG_Landing::is_ground_steering_allowed(void) const
{
    if (!flags.in_progress) {
        return true;
    }
...
bool AG_Landing::is_expecting_impact(void) const
{
    if (!flags.in_progress) {
        return false;
    }
...
bool AG_Landing::override_servos(void) {
    if (!flags.in_progress) {
        return false;
    }
...
const AG_PIDInfo* AG_Landing::get_pid_info(void) const
{
    switch (type) {
#if HAL_LANDING_DEEPSTALL_ENABLED
    case TYPE_DEEPSTALL:
...
void AG_Landing::setup_landing_glide_slope(const Location &prev_WP_loc, const Location &next_WP_loc, const Location &current_loc, int32_t &target_altitude_offset_cm)
{
    switch (type) {
    case TYPE_STANDARD_GLIDE_SLOPE:
 ...
 */
bool AG_Landing::restart_landing_sequence()
{
...
int32_t AG_Landing::constrain_roll(const int32_t desired_roll_cd, const int32_t level_roll_limit_cd)
{
    switch (type) {
    case TYPE_STANDARD_GLIDE_SLOPE:
...
bool AG_Landing::get_target_altitude_location(Location &location)
{
    if (!flags.in_progress) {
        return false;
    }
 ...
 */
float AG_Landing::wind_alignment(const float heading_deg)
{
 ...
 */
float AG_Landing::head_wind(void)
{
 ...
 */
int32_t AG_Landing::get_target_airspeed_cm(void)
{
    if (!flags.in_progress) {
 ...
 */
bool AG_Landing::request_go_around(void)
{
    bool success = false;
...
void AG_Landing::handle_flight_stage_change(const bool _in_landing_stage)
{
 ...
 */
bool AG_Landing::is_complete(void) const
{
    switch (type) {
    case TYPE_STANDARD_GLIDE_SLOPE:
...
void AG_Landing::Log(void) const
{
    switch (type) {
    case TYPE_STANDARD_GLIDE_SLOPE:
        type_slope_log();
 ...
 */
bool AG_Landing::is_throttle_suppressed(void) const
{
    if (!flags.in_progress) {
        return false;
    }
...
bool AG_Landing::use_thr_min_during_flare(void) const
{
    return (OptionsMask::ON_LANDING_FLARE_USE_THR_MIN & _options) != 0;
}

//defaults to false, but _options bit zero enables it.
bool AG_Landing::allow_max_airspeed_on_land(void) const
{
 ...
 */
bool AG_Landing::is_flying_forward(void) const
{
    if (!flags.in_progress) {
        return true;
    }
 ...
 */
bool AG_Landing::terminate(void) {
    switch (type) {
#if HAL_LANDING_DEEPSTALL_ENABLED
    case TYPE_DEEPSTALL:
