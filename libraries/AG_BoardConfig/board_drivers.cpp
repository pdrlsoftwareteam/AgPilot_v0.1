 ...
 */
/*
 *   AG_BoardConfig - driver loading and setup
 */
...
/*
  init safety state
 */
void AG_BoardConfig::board_init_safety()
{
    ...
    if (force_safety_off) {
        ...
        uint8_t count = 20;
        while (hal.util->safety_switch_state() != AG_HAL::Util::SAFETY_ARMED && count--) {
 ...
 */
void AG_BoardConfig::board_init_debug()
{
...
AG_BoardConfig::px4_board_type AG_BoardConfig::px4_configured_board;

void AG_BoardConfig::board_setup_drivers(void)
{
 ...
 */
bool AG_BoardConfig::spi_check_register(const char *devname, uint8_t regnum, uint8_t value, uint8_t read_flag)
{
...
/*
  check a SPI device for a register value
 */
bool AG_BoardConfig::spi_check_register_inv2(const char *devname, uint8_t regnum, uint8_t value, uint8_t read_flag)
{
    auto dev = hal.spi->get_device(devname);
    if (!dev) {
#if SPI_PROBE_DEBUG
...
bool AG_BoardConfig::check_ms5611(const char* devname) {
    auto dev = hal.spi->get_device(devname);
    if (!dev) {
#if SPI_PROBE_DEBUG
        hal.console->printf("%s: no device\n", devname);
    ...
    AG_HAL::Semaphore *dev_sem = dev->get_semaphore();
 ...
 */
void AG_BoardConfig::validate_board_type(void)
{
 ...
 */
void AG_BoardConfig::board_autodetect(void)
{
#if defined(HAL_VALIDATE_BOARD)
 ...
 */
void AG_BoardConfig::board_setup_uart()
{
#if AP_FEATURE_RTSCTS
#ifdef HAL_HAVE_RTSCTS_SERIAL1
    if (hal.serial(1) != nullptr) {
        hal.serial(1)->set_flow_control((AG_HAL::UARTDriver::flow_control)state.ser_rtscts[1].get());
    }
#endif
#ifdef HAL_HAVE_RTSCTS_SERIAL2
    if (hal.serial(2) != nullptr) {
        hal.serial(2)->set_flow_control((AG_HAL::UARTDriver::flow_control)state.ser_rtscts[2].get());
    }
#endif
#ifdef HAL_HAVE_RTSCTS_SERIAL3
    if (hal.serial(3) != nullptr) {
        hal.serial(3)->set_flow_control((AG_HAL::UARTDriver::flow_control)state.ser_rtscts[3].get());
    }
#endif
#ifdef HAL_HAVE_RTSCTS_SERIAL4
    if (hal.serial(4) != nullptr) {
        hal.serial(4)->set_flow_control((AG_HAL::UARTDriver::flow_control)state.ser_rtscts[4].get());
    }
#endif
#ifdef HAL_HAVE_RTSCTS_SERIAL5
    if (hal.serial(5) != nullptr) {
        hal.serial(5)->set_flow_control((AG_HAL::UARTDriver::flow_control)state.ser_rtscts[5].get());
    }
#endif
#endif
}
...
/*
  setup SBUS
 */
void AG_BoardConfig::board_setup_sbus(void)
{
 ...
 */
void AG_BoardConfig::board_setup()
{
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
 ...
 */
void AG_BoardConfig::detect_fmuv6_variant()
{
    if (((spi_check_register_inv2("icm20649", INV2REG_WHOAMI, INV2_WHOAMI_ICM20649) ||
        ...
        AG_Param::load_defaults_file("@ROMFS/param/CUAV_V6X_defaults.parm", false);
    }
