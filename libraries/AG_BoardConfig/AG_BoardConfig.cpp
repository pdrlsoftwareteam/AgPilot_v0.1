 ...
 */
/*
 *   AG_BoardConfig - board specific configuration
 */

#include "AG_BoardConfig.h"
...
AG_BoardConfig *AG_BoardConfig::_singleton;

// constructor
AG_BoardConfig::AG_BoardConfig()
...
const AG_Param::GroupInfo AG_BoardConfig::var_info[] = {
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SER1_RTSCTS",    1, AG_BoardConfig, state.ser_rtscts[1], BOARD_SER1_RTSCTS_DEFAULT),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SER2_RTSCTS",    2, AG_BoardConfig, state.ser_rtscts[2], 2),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SER3_RTSCTS",    23, AG_BoardConfig, state.ser_rtscts[3], 2),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SER4_RTSCTS",    24, AG_BoardConfig, state.ser_rtscts[4], 2),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SER5_RTSCTS",    25, AG_BoardConfig, state.ser_rtscts[5], 2),
#endif
#endif
    ...
    // @RebootRequired: True
    // @User: Standard
    AP_GROUPINFO("SAFETY_DEFLT",   3, AG_BoardConfig, state.safety_enable, BOARD_SAFETY_ENABLE_DEFAULT),
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SBUS_OUT",   4, AG_BoardConfig, state.sbus_out_rate, 0),
#endif
    ...
    // @User: Standard
    AP_GROUPINFO("SERIAL_NUM", 5, AG_BoardConfig, vehicleSerialNumber, 0),
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SAFETY_MASK", 7, AG_BoardConfig, state.ignore_safety_channels, 0),
    ...
    // @Units: degC
    // @User: Advanced
    AP_GROUPINFO("HEAT_TARG", 8, AG_BoardConfig, heater.imu_target_temperature, HAL_IMU_TEMP_DEFAULT),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("TYPE", 9, AG_BoardConfig, state.board_type, BOARD_TYPE_DEFAULT),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("IO_ENABLE", 10, AG_BoardConfig, state.io_enable, 1),
#endif

#if HAL_RCINPUT_WITH_AP_RADIO
    // @Group: RADIO
    // @Path: ../AG_Radio/AG_Radio.cpp
    AP_SUBGROUPINFO(_radio, "RADIO", 11, AG_BoardConfig, AG_Radio),
#endif
    ...
    // @User: Standard
    AP_GROUPINFO("SAFETYOPTION",   13, AG_BoardConfig, state.safety_option, BOARD_SAFETY_OPTION_DEFAULT),

    // @Group: RTC
    // @Path: ../AG_RTC/AG_RTC.cpp
    AP_SUBGROUPINFO(rtc, "RTC", 14, AG_BoardConfig, AG_RTC),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("VBUS_MIN",    15,     AG_BoardConfig,  _vbus_min,  BOARD_CONFIG_BOARD_VOLTAGE_MIN),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("VSERVO_MIN",    16,     AG_BoardConfig, _vservo_min,  0),
#endif
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("SD_SLOWDOWN",  17,     AG_BoardConfig, _sdcard_slowdown,  0),
#endif
    ...
    // @Values: 0:3.3V,1:5V
    // @User: Advanced
    AP_GROUPINFO("PWM_VOLT_SEL", 18, AG_BoardConfig, _pwm_volt_sel, 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 19, AG_BoardConfig, _options, HAL_BRD_OPTIONS_DEFAULT),
    ...
    // @Units: ms
    // @User: Advanced
    AP_GROUPINFO("BOOT_DELAY", 20, AG_BoardConfig, _boot_delay_ms, HAL_DEFAULT_BOOT_DELAY),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_SUBGROUPINFO(heater.pi_controller, "HEAT_",  21, AG_BoardConfig, AG_PI),
#endif
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("ALT_CONFIG", 22, AG_BoardConfig, _alt_config, 0),
    ...
    // @Units: degC
    // @User: Advanced
    AP_GROUPINFO("HEAT_LOWMGN", 23, AG_BoardConfig, heater.imu_arming_temperature_margin_low, HAL_IMU_TEMP_MARGIN_LOW_DEFAULT),
#endif
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("SD_MISSION", 24, AG_BoardConfig, sdcard_storage.mission_kb, 0),
#endif
    
    AP_GROUPEND
};

void AG_BoardConfig::init()
{
    ...
    AP::rtc().set_utc_usec(hal.util->get_hw_rtc(), AG_RTC::SOURCE_HW);
...
void AG_BoardConfig::set_default_safety_ignore_mask(uint32_t mask)
{
    state.ignore_safety_channels.set_default(mask);
}

void AG_BoardConfig::init_safety()
{
    board_init_safety();
    board_init_debug();
}
...
*/
bool AG_BoardConfig::_in_error_loop;

void AG_BoardConfig::throw_error(const char *err_type, const char *fmt, va_list arg)
{
    _in_error_loop = true;
    /*
...
void AG_BoardConfig::allocation_error(const char *fmt, ...)
{
    va_list arg_list;
...
void AG_BoardConfig::config_error(const char *fmt, ...)
{
    va_list arg_list;
    va_start(arg_list, fmt);
 ...
 */
bool AG_BoardConfig::safety_button_handle_pressed(uint8_t press_count)
{
    if (press_count != 10) {
        return false;
    }
    ...
    AG_HAL::Util::safety_state safety_state = hal.util->safety_switch_state();
    if (safety_state == AG_HAL::Util::SAFETY_DISARMED &&
        !(safety_options & BOARD_SAFETY_OPTION_BUTTON_ACTIVE_SAFETY_OFF)) {
        return false;
    }
    if (safety_state == AG_HAL::Util::SAFETY_ARMED &&
...
namespace AP {
    AG_BoardConfig *boardConfig(void) {
        return AG_BoardConfig::get_singleton();
    }
};
