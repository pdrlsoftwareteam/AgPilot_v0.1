...
AG_Hott_Telem *AG_Hott_Telem::singleton;

AG_Hott_Telem::AG_Hott_Telem(void)
{
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (singleton != nullptr) {
        AG_HAL::panic("AG_Hott_Telem must be singleton");
    }
#endif
    singleton = this;
}
 ...
 * initialise uart
 */
void AG_Hott_Telem::init()
{
    const AG_SerialManager &serial_manager = AP::serialmanager();

    uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Hott, 0);
    if (uart) {
        // register thread
        if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_Hott_Telem::loop, void),
                                          "Hott",
 ...
 */
void AG_Hott_Telem::send_EAM(void)
{
    // EAM message
    struct PACKED {
 ...
 */
void AG_Hott_Telem::GPS_to_DDM(float decimal, uint8_t &sign, uint16_t &dm, uint16_t &sec) const
{
    sign = decimal>=0?0:1;
...
/*
  send GPS packet
 */
void AG_Hott_Telem::send_GPS(void)
{
    // GPS message
    struct PACKED {
...
/*
  send Vario
 */
void AG_Hott_Telem::send_Vario(void)
{
    // Vario message
    struct PACKED {
        uint8_t start_byte = 0x7C;   //#01 start uint8_t
    ...
    if (hal.util->get_soft_armed()) {
        ...
        const char *ck = AG_Notify::flags.pre_arm_check ? "CK:PASS" : "CK:FAIL";
...
/*
  send a packet out
 */
void AG_Hott_Telem::send_packet(const uint8_t *b, uint8_t len)
{
    // initial delay
...
namespace AP {
    AG_Hott_Telem *hott_telem() {
        return AG_Hott_Telem::get_singleton();
    }
};
