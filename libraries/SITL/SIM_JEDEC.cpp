...
void JEDEC::page_erase (uint32_t addr)
{
    ...
    if (addr + fill_length > get_storage_size()) {
        AG_HAL::panic("trying to write outside memory");
    }
...
{
    if (len<4) {
        AG_HAL::panic("address too short");
    }
...
int JEDEC::rdwr(uint8_t count, SPI::spi_ioc_transfer *&tfrs)
{
    ...
    for (uint8_t i=0; i<count; i++) {
        ...
        case State::WAITING: {
            ...
            default:
                AG_HAL::panic("Unhandled command received");
            }
            break;
        }
        ...
        case State::READING: {
            if (xfr_addr + tfr.len > get_storage_size()) {
                AG_HAL::panic("trying to read outside memory");
            }
