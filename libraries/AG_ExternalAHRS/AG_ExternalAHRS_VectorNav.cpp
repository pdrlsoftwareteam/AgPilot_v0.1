...
// constructor
AG_ExternalAHRS_VectorNav::AG_ExternalAHRS_VectorNav(AG_ExternalAHRS *_frontend,
                                                     AG_ExternalAHRS::state_t &_state) :
    AG_ExternalAHRS_backend(_frontend, _state)
{
    ...
    if (!pktbuf || !last_pkt1 || !last_pkt2) {
        AG_BoardConfig::allocation_error("ExternalAHRS");
    }

    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_ExternalAHRS_VectorNav::update_thread, void), "AHRS", 2048, AG_HAL::Scheduler::PRIORITY_SPI, 0)) {
        AG_HAL::panic("Failed to start ExternalAHRS update thread");
    }
...
bool AG_ExternalAHRS_VectorNav::check_uart()
{
    if (!setup_complete) {
        return false;
    }
...
void AG_ExternalAHRS_VectorNav::wait_register_responce(const uint8_t register_num)
{
...
bool AG_ExternalAHRS_VectorNav::decode(char c)
{
    switch (c) {
    case ',':
...
bool AG_ExternalAHRS_VectorNav::decode_latest_term()
{
...
void AG_ExternalAHRS_VectorNav::update_thread()
{
...
const char* AG_ExternalAHRS_VectorNav::get_name() const
{
    if (setup_complete) {
        return model_name;
    }
 ...
 */
void AG_ExternalAHRS_VectorNav::process_packet1(const uint8_t *b)
{
    const struct VN_packet1 &pkt1 = *(struct VN_packet1 *)b;
    const struct VN_packet2 &pkt2 = *last_pkt2;

    last_pkt1_ms = AG_HAL::millis();
    *last_pkt1 = pkt1;

    const bool use_uncomp = option_is_set(AG_ExternalAHRS::OPTIONS::VN_UNCOMP_IMU);
    ...
    {
        AG_ExternalAHRS::baro_data_message_t baro;
        baro.instance = 0;
 ...
 */
void AG_ExternalAHRS_VectorNav::process_packet2(const uint8_t *b)
{
    const struct VN_packet2 &pkt2 = *(struct VN_packet2 *)b;
    const struct VN_packet1 &pkt1 = *last_pkt1;

    last_pkt2_ms = AG_HAL::millis();
    *last_pkt2 = pkt2;
 ...
 */
void AG_ExternalAHRS_VectorNav::process_packet_VN_100(const uint8_t *b)
{
...
int8_t AG_ExternalAHRS_VectorNav::get_port(void) const
{
    if (!uart) {
        return -1;
    }
...
bool AG_ExternalAHRS_VectorNav::healthy(void) const
{
    const uint32_t now = AG_HAL::millis();
    if (type == TYPE::VN_100) {
...
bool AG_ExternalAHRS_VectorNav::initialised(void) const
{
    if (!setup_complete) {
        return false;
    }
...
bool AG_ExternalAHRS_VectorNav::pre_arm_check(char *failure_msg, uint8_t failure_msg_len) const
{
    if (!setup_complete) {
 ...
 */
void AG_ExternalAHRS_VectorNav::get_filter_status(nav_filter_status &status) const
{
...
void AG_ExternalAHRS_VectorNav::send_status_report(GCS_MAVLINK &link) const
{
    if (!last_pkt1) {
        return;
    }
