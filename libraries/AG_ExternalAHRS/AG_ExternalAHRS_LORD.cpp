...
AG_ExternalAHRS_LORD::AG_ExternalAHRS_LORD(AG_ExternalAHRS *_frontend,
        AG_ExternalAHRS::state_t &_state): AG_ExternalAHRS_backend(_frontend, _state)
{
    auto &sm = AP::serialmanager();
    uart = sm.find_serial(AG_SerialManager::SerialProtocol_AHRS, 0);

    baudrate = sm.find_baudrate(AG_SerialManager::SerialProtocol_AHRS, 0);
    port_num = sm.find_portnum(AG_SerialManager::SerialProtocol_AHRS, 0);
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_ExternalAHRS_LORD::update_thread, void), "AHRS", 2048, AG_HAL::Scheduler::PRIORITY_SPI, 0)) {
        AG_BoardConfig::allocation_error("Failed to allocate ExternalAHRS update thread");
    }
...
void AG_ExternalAHRS_LORD::update_thread(void)
{
    if (!port_open) {
        port_open = true;
        uart->begin(baudrate);
...
void AG_ExternalAHRS_LORD::build_packet()
{
    WITH_SEMAPHORE(sem);
...
bool AG_ExternalAHRS_LORD::valid_packet(const LORD_Packet & packet) const
{
...
void AG_ExternalAHRS_LORD::handle_packet(const LORD_Packet& packet)
{
...
void AG_ExternalAHRS_LORD::handle_imu(const LORD_Packet& packet)
{
    last_ins_pkt = AG_HAL::millis();
...
void AG_ExternalAHRS_LORD::post_imu() const
{
    {
    ...
    {
        AG_ExternalAHRS::ins_data_message_t ins {
    ...
    {
        AG_ExternalAHRS::mag_data_message_t mag {
            field: imu_data.mag
        };
    ...
    {
        const AG_ExternalAHRS::baro_data_message_t baro {
            instance: 0,
...
void AG_ExternalAHRS_LORD::handle_gnss(const LORD_Packet &packet)
{
    last_gps_pkt = AG_HAL::millis();
...
void AG_ExternalAHRS_LORD::handle_filter(const LORD_Packet &packet)
{
    last_filter_pkt = AG_HAL::millis();
...
void AG_ExternalAHRS_LORD::post_filter() const
{
    {
        WITH_SEMAPHORE(state.sem);
    ...
    AG_ExternalAHRS::gps_data_message_t gps {
...
int8_t AG_ExternalAHRS_LORD::get_port(void) const
{
    if (!uart) {
        return -1;
    }
...
// Get model/type name
const char* AG_ExternalAHRS_LORD::get_name() const
{
    return "LORD";
}

bool AG_ExternalAHRS_LORD::healthy(void) const
{
    uint32_t now = AG_HAL::millis();
    return (now - last_ins_pkt < 40 && now - last_gps_pkt < 500 && now - last_filter_pkt < 500);
}

bool AG_ExternalAHRS_LORD::initialised(void) const
{
    return last_ins_pkt != 0 && last_gps_pkt != 0 && last_filter_pkt != 0;
}

bool AG_ExternalAHRS_LORD::pre_arm_check(char *failure_msg, uint8_t failure_msg_len) const
{
    if (!healthy()) {
...
void AG_ExternalAHRS_LORD::get_filter_status(nav_filter_status &status) const
{
...
void AG_ExternalAHRS_LORD::send_status_report(GCS_MAVLINK &link) const
{
    // prepare flags
    uint16_t flags = 0;
...
Vector3f AG_ExternalAHRS_LORD::populate_vector3f(const uint8_t *data, uint8_t offset) const
{
    return Vector3f {
...
Quaternion AG_ExternalAHRS_LORD::populate_quaternion(const uint8_t *data, uint8_t offset) const
{
    return Quaternion {
        extract_float(data, offset),
        extract_float(data, offset+4),
...
float AG_ExternalAHRS_LORD::extract_float(const uint8_t *data, uint8_t offset) const
{
...
double AG_ExternalAHRS_LORD::extract_double(const uint8_t *data, uint8_t offset) const
{
