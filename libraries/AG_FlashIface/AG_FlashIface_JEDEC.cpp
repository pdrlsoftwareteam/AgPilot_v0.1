...
#include "AG_FlashIface_JEDEC.h"
#include <AG_Math/AG_Math.h>
#ifdef HAL_BOOTLOADER_BUILD
#include <AG_HAL_ChibiOS/QSPIDevice.h>
...
bool AG_FlashIface_JEDEC::init()
{
    ...
    if (!_dev) {
        AG_HAL::panic("Ext Flash Not Found!");
    }
...
void AG_FlashIface_JEDEC::reset_device()
{
    // Get chip out of XIP mode
    AG_HAL::QSPIDevice::CommandHeader cmd;
    ...
    cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE;
    cmd.addr   =  0;
    cmd.alt    =  0;
    cmd.dummy  =  0;
...
bool AG_FlashIface_JEDEC::detect_device()
{
    AG_HAL::QSPIDevice::CommandHeader cmd;

    {
        ...
        cmd.cmd    =  CMD_READ_ID;
        cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
                      AG_HAL::QSPI::CFG_DATA_MODE_ONE_LINE;
        cmd.addr   =  0;
        cmd.alt    =  0;
        cmd.dummy  =  0;
    ...
    // Read SFDP header to get information Ref. JESD216D 4 and 6.2
    {
        ...
        cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
                      AG_HAL::QSPI::CFG_ADDR_MODE_ONE_LINE |
                      AG_HAL::QSPI::CFG_ADDR_SIZE_24 |
                      AG_HAL::QSPI::CFG_DATA_MODE_ONE_LINE;
        cmd.addr   =  0;
        cmd.alt    =  0;
    ...
    // Read and parse the param table
    {
        ...
        if (_desc.is_xip_supported) {
            if (SFDP_GET_BIT(param_table[14],16)) {
                _desc.entry_method = AG_FlashIface_JEDEC::XIP_ENTRY_METHOD_1;
            } else if (SFDP_GET_BIT(param_table[14],17)) {
                _desc.entry_method = AG_FlashIface_JEDEC::XIP_ENTRY_METHOD_2;
            } else {
...
bool AG_FlashIface_JEDEC::configure_device()
{
    // Enable 1-4-4 mode
    if (_desc.quad_mode_enable == QUAD_ENABLE_B1R2) {
        ...
        AG_HAL::QSPIDevice::CommandHeader cmd {
            .cmd    = 0x01,
            .cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
                       AG_HAL::QSPI::CFG_DATA_MODE_ONE_LINE,
            .addr   =  0,
            .alt    =  0,
            .dummy  =  0
...
bool AG_FlashIface_JEDEC::write_enable()
{
...
bool AG_FlashIface_JEDEC::write_disable()
{
    if (_desc.write_enable_ins) {
        wait_ready();
        write_enable_called = true;
...
bool AG_FlashIface_JEDEC::modify_reg(uint8_t read_ins, uint8_t write_ins,
...
bool AG_FlashIface_JEDEC::read_reg(uint8_t read_ins, uint8_t &read_val)
{
    AG_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = read_ins;
    cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
...
bool AG_FlashIface_JEDEC::write_reg(uint8_t read_ins, uint8_t write_val)
{
    AG_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = read_ins;
    cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
...
bool AG_FlashIface_JEDEC::send_cmd(uint8_t ins)
{
    AG_HAL::QSPIDevice::CommandHeader cmd;
    cmd.cmd = ins;
    cmd.cfg    =  AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE;
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_mass_erase(uint32_t &delay_ms, uint32_t &timeout_ms)
{
    write_enable();
    wait_ready();
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_sector_erase(uint32_t sector, uint32_t &delay_ms, uint32_t &timeout_ms)
{
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_erase_offset(uint32_t offset, uint32_t size, uint32_t &erasing,
 ...
 *
 */
bool AG_FlashIface_JEDEC::verify_sector_erase(uint32_t sector)
{
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_program_page(uint32_t page, const uint8_t* data,
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_program_offset(uint32_t offset, const uint8_t* data, uint32_t size, uint32_t &programming,
        uint32_t &delay_us, uint32_t &timeout_us)
{
 ...
 *
 */
bool AG_FlashIface_JEDEC::read(uint32_t offset, uint8_t* data, uint32_t size)
{
    ...
    for (read_ptr = offset; read_ptr < (offset+size); read_ptr+=MAX_READ_SIZE) {
        ...
        cmd.alt     =   0;
        cmd.cfg     =   AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
                        AG_HAL::QSPI::CFG_ADDR_MODE_FOUR_LINES |
                        AG_HAL::QSPI::CFG_ADDR_SIZE_24 |
                        AG_HAL::QSPI::CFG_DATA_MODE_FOUR_LINES |
                        AG_HAL::QSPI::CFG_ALT_SIZE_8 |
                        AG_HAL::QSPI::CFG_ALT_MODE_FOUR_LINES;
 ...
 *
 */
bool AG_FlashIface_JEDEC::is_device_busy()
{
    uint8_t status;
...
void AG_FlashIface_JEDEC::wait_ready()
{
 ...
 *
 */
bool AG_FlashIface_JEDEC::start_xip_mode(void** addr)
{
    wait_ready();
    ...
    switch(_desc.entry_method) {
        case AG_FlashIface_JEDEC::XIP_ENTRY_METHOD_1:
        {
            // Set QSPI module for XIP mode
            AG_HAL::QSPIDevice::CommandHeader cmd;
            cmd.cmd = _desc.fast_read_ins;
            cmd.alt = 0xA5;
            cmd.cfg = AG_HAL::QSPI::CFG_ADDR_SIZE_24 |
                      AG_HAL::QSPI::CFG_CMD_MODE_ONE_LINE |
                      AG_HAL::QSPI::CFG_ADDR_MODE_FOUR_LINES |
                      AG_HAL::QSPI::CFG_DATA_MODE_FOUR_LINES |
                      AG_HAL::QSPI::CFG_ALT_MODE_FOUR_LINES |  /* Always 4 lines, note.*/
                      AG_HAL::QSPI::CFG_ALT_SIZE_8;
            cmd.addr = 0;
            cmd.dummy = _desc.fast_read_dummy_cycles;
            _dev->set_cmd_header(cmd);
        ...
        case AG_FlashIface_JEDEC::XIP_ENTRY_METHOD_2:
        {
...
bool AG_FlashIface_JEDEC::stop_xip_mode()
{
