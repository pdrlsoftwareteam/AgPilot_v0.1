...
/*
  ...
  Several SPort packets make up a AGPILOTlite message.
...
#define AGPILOTLITE_MAX_PAYLOAD_LEN                 31 // 7 float params + cmd_id + options
#define AGPILOTLITE_MSG_SPORT_PACKETS_COUNT(LEN)    static_cast<uint8_t>(1 + ceilf((LEN-2)/5.0f)) // number of sport packets required to transport a message with LEN payload
#define SPORT_PACKET_QUEUE_LENGTH               static_cast<uint8_t>(30U*AGPILOTLITE_MSG_SPORT_PACKETS_COUNT(AGPILOTLITE_MAX_PAYLOAD_LEN))
