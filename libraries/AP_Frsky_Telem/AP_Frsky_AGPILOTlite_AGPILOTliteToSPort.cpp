...
void AP_Frsky_AGPILOTlite_AGPILOTliteToSPort::reset()
{
    checksum = 0;
...
void AP_Frsky_AGPILOTlite_AGPILOTliteToSPort::update_checksum(const uint8_t c)
{
    checksum += c; //0-1FF
    checksum += checksum >> 8;
    checksum &= 0xFF;
 ...
 */
bool AP_Frsky_AGPILOTlite_AGPILOTliteToSPort::process(ObjectBuffer_TS<AP_Frsky_SPort::sport_packet_t> &queue, const AP_Frsky_AGPILOTlite_Message &msg)
{
    // let's check if there's enough room to send it
    if (queue.space() < AGPILOTLITE_MSG_SPORT_PACKETS_COUNT(msg.len)) {
        return false;
    }
    reset();
...
void AP_Frsky_AGPILOTlite_AGPILOTliteToSPort::process_byte(const uint8_t b, ObjectBuffer_TS<AP_Frsky_SPort::sport_packet_t> &queue)
{
    if (packet_offs == 2) {
