...
class AP_BattMonitor_CAN_BMS : public AP_BattMonitor_Backend, public CANSensor {
public:
    // Constructor
    AP_BattMonitor_CAN_BMS(AP_BattMonitor &mon,
    ...
    bool get_unique_id(const uint8_t* &uniq_id) const;
    bool get_firmware_info(uint16_t &firm_info) const;
    bool get_state_of_health(uint16_t &state_of_health) const;
    bool get_state_of_charge(uint16_t &state_of_charge) const;
    bool get_capacity(uint32_t &cap) const;
    uint8_t get_cell_count(uint8_t &count_cell) const;
    bool get_temperature2(int16_t &temp) const;

    bool get_battery_info(uint16_t &firm_info,const uint8_t* &uniq_id) const;
    ...
    bool get_faults_and_warnings(uint32_t &fault_flags, uint32_t &warning_flags) const ;
    ...
    bool get_remaining_capacity(uint32_t &rem_cap) const ;
...
class TEST_CAN : public CANSensor {
public:
    // Singleton access
    static TEST_CAN* getInstance() {
        if (instance == nullptr) {
            instance = new TEST_CAN();
        }
        return instance;
    }
    ...
    void set_monitor(AP_BattMonitor_CAN_BMS* mon) { monitor = mon; }
...
private:
    static TEST_CAN* instance;
    AP_BattMonitor_CAN_BMS* monitor = nullptr;
};
