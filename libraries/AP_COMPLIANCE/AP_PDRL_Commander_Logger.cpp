...
int AP_PDRL_Logger::start_new_geofence_log(char* m_write_filename)
{
	...
	if (file_exists(write_filename)) {
		...
		asprintf(&currentFileName,"%s.json",(char*)write_filename);	//delete json file for current PA
...
int AP_PDRL_Logger::makeJsonAndSignLogFileLatest()
{
	...
	if(isOldHashAvailable)
	{
		...
		for(int i = 0; i < 32 ; i++)
		{
			asprintf(&hexVal,"%02x",oldLogFileHash[i]);
...
size_t AP_PDRL_Logger::encryptHash(char* buffer,size_t dataLenght)
{
    ...
    if(keyStore->isPasswordGenarated() != 0)
    {
        keyStore->getPlainPassword(password);
        keyStore->generateKeyFromPassword(password);
        if(true == keyStore->getGrant(password,16))
        {
            memset(keyStore->privateKeyBuf,'-',sizeof(keyStore->privateKeyBuf));
            keyStore->readKey(keyStore->privateKeyBuf ,&keyLen,KEY_TYPE_PRIVATE);
            mbedtls_pk_context pk;
            mbedtls_pk_init(&pk);
            ret = mbedtls_pk_parse_key(&pk,keyStore->privateKeyBuf,keyLen,NULL,0);
