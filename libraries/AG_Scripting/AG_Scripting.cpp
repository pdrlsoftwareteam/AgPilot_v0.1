...
const AG_Param::GroupInfo AG_Scripting::var_info[] = {
    // @Param: ENABLE
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_Scripting, _enable, SCRIPTING_ENABLE_DEFAULT, AP_PARAM_FLAG_ENABLE),
    ...
    // @Increment: 10000
    // @User: Advanced
    AP_GROUPINFO("VM_I_COUNT", 2, AG_Scripting, _script_vm_exec_count, 10000),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("HEAP_SIZE", 3, AG_Scripting, _script_heap_size, SCRIPTING_HEAP_SIZE),
    ...
    // @User: Advanced
    AP_GROUPINFO("DEBUG_OPTS", 4, AG_Scripting, _debug_options, 0),

    // @Param: USER1
    // @DisplayName: Scripting User Parameter1
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER1", 5, AG_Scripting, _user[0], 0.0),

    // @Param: USER2
    // @DisplayName: Scripting User Parameter2
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER2", 6, AG_Scripting, _user[1], 0.0),

    // @Param: USER3
    // @DisplayName: Scripting User Parameter3
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER3", 7, AG_Scripting, _user[2], 0.0),

    // @Param: USER4
    // @DisplayName: Scripting User Parameter4
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER4", 8, AG_Scripting, _user[3], 0.0),

    // @Param: USER5
    // @DisplayName: Scripting User Parameter5
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER5", 10, AG_Scripting, _user[4], 0.0),

    // @Param: USER6
    // @DisplayName: Scripting User Parameter6
    // @Description: General purpose user variable input for scripts
    // @User: Standard
    AP_GROUPINFO("USER6", 11, AG_Scripting, _user[5], 0.0),
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("DIR_DISABLE", 9, AG_Scripting, _dir_disable, 0),

    AP_GROUPEND
};

AG_Scripting::AG_Scripting() {
    AG_Param::setup_object_defaults(this, var_info);

#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton != nullptr) {
        AG_HAL::panic("Scripting must be a singleton");
    }
#endif // CONFIG_HAL_BOARD == HAL_BOARD_SITL
    _singleton = this;
}

void AG_Scripting::init(void) {
    if (!_enable) {
        return;
    }
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_Scripting::thread, void),
                                      "Scripting", SCRIPTING_STACK_SIZE, AG_HAL::Scheduler::PRIORITY_SCRIPTING, 0)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "Scripting: %s", "failed to start");
        _thread_failed = true;
    }
}

MAV_RESULT AG_Scripting::handle_command_int_packet(const mavlink_command_int_t &packet) {
...
bool AG_Scripting::repl_start(void) {
...
void AG_Scripting::repl_stop(void) {
...
void AG_Scripting::handle_mission_command(const AG_Mission::Mission_Command& cmd_in)
{
    if (!_enable) {
        return;
    }
    ...
    if (mission_data == nullptr) {
        // load buffer
        mission_data = new ObjectBuffer<struct AG_Scripting::scripting_mission_cmd>(mission_cmd_queue_size);
...
bool AG_Scripting::arming_checks(size_t buflen, char *buffer) const
{
...
void AG_Scripting::restart_all()
{
    _restart = true;
    _stop = true;
}

AG_Scripting *AG_Scripting::_singleton = nullptr;

namespace AP {
    AG_Scripting *scripting() {
        return AG_Scripting::get_singleton();
    }
}
