...
lua_scripts::lua_scripts(const AP_Int32 &vm_steps, const AP_Int32 &heap_size, const AP_Int8 &debug_options, struct AG_Scripting::terminal_s &_terminal)
    : _vm_steps(vm_steps),
...
lua_scripts::script_info *lua_scripts::load_script(lua_State *L, char *filename) {
    ...
    const uint32_t loadStart = AG_HAL::micros();
    ...
    const uint32_t loadEnd = AG_HAL::micros();
    ...
    new_script->next_run_ms = AG_HAL::millis64() - 1; // force the script to be stale
...
void lua_scripts::run_next_script(lua_State *L) {
    if (scripts == nullptr) {
        ...
        AG_HAL::panic("Lua: Attempted to run a script without any scripts queued");
#endif // defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
        return;
    }

    uint64_t start_time_ms = AG_HAL::millis64();
...
void lua_scripts::reschedule_script(script_info *script) {
    if (script == nullptr) {
#if defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
       AG_HAL::panic("Lua: Attempted to schedule a null pointer");
#endif // defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
       return;
    }
...
void lua_scripts::run(void) {
    ...
    uint16_t dir_disable = AG_Scripting::get_singleton()->get_disabled_dir();
    bool loaded = false;
    if ((dir_disable & uint16_t(AG_Scripting::SCR_DIR::SCRIPTS)) == 0) {
        load_all_scripts_in_dir(L, SCRIPTING_DIRECTORY);
        loaded = true;
    }
    if ((dir_disable & uint16_t(AG_Scripting::SCR_DIR::ROMFS)) == 0) {
    ...
    while (AG_Scripting::get_singleton()->should_run()) {
...
#if defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
        if (lua_gettop(L) != 0) {
            AG_HAL::panic("Lua: Stack should be empty before running scripts");
        }
#endif // defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
        ...
        if (scripts != nullptr) {
              ...
              while (sanity->next != nullptr) {
                  if (sanity->next_run_ms > sanity->next->next_run_ms) {
                      AG_HAL::panic("Lua: Script tasking order has been violated");
                  }
                  sanity = sanity->next;
              }
#endif // defined(AP_SCRIPTING_CHECKS) && AP_SCRIPTING_CHECKS >= 1
            ...
            // compute delay time
            uint64_t now_ms = AG_HAL::millis64();
            ...
            const uint32_t runEnd = AG_HAL::micros();
            const int endMem = lua_gc(L, LUA_GCCOUNT, 0) * 1024 + lua_gc(L, LUA_GCCOUNTB, 0);
        ...
        if ((print_error_count < error_prints) && (AG_HAL::millis() - last_print_ms > 10000)) {
