...
#include <AG_Scripting/lua_generated_bindings.h>
...
int lua_millis(lua_State *L) {
    ...
    new_uint32_t(L);
    *check_uint32_t(L, -1) = AG_HAL::millis();
...
int lua_micros(lua_State *L) {
    ...
    new_uint32_t(L);
    *check_uint32_t(L, -1) = AG_HAL::micros();
...
int lua_mission_receive(lua_State *L) {
    ...
    ObjectBuffer<struct AG_Scripting::scripting_mission_cmd> *input = AP::scripting()->mission_data;
    ...
    struct AG_Scripting::scripting_mission_cmd cmd;
...
int AG_Logger_Write(lua_State *L) {
    AG_Logger * AP_logger = AG_Logger::get_singleton();
    ...
    struct AG_Logger::log_write_fmt *f;
    if (!have_units) {
...
int lua_get_i2c_device(lua_State *L) {
    ...
    scripting->_i2c_dev[scripting->num_i2c_devices] = new AG_HAL::OwnPtr<AG_HAL::I2CDevice>;
    ...
    *scripting->_i2c_dev[scripting->num_i2c_devices] = std::move(hal.i2c_mgr->get_device(bus, address, bus_clock, use_smbus));
    ...
    new_AG_HAL__I2CDevice(L);
    *((AG_HAL::I2CDevice**)luaL_checkudata(L, -1, "AG_HAL::I2CDevice")) = scripting->_i2c_dev[scripting->num_i2c_devices]->get();
...
int AG_HAL__I2CDevice_read_registers(lua_State *L) {
    ...
    AG_HAL::I2CDevice * ud = *check_AG_HAL__I2CDevice(L, 1);
...
int lua_get_CAN_device(lua_State *L) {
    ...
    if (scripting->_CAN_dev == nullptr) {
        scripting->_CAN_dev = new ScriptingCANSensor(AG_CANManager::Driver_Type::Driver_Type_Scripting);
        if (scripting->_CAN_dev == nullptr) {
...
int lua_get_CAN_device2(lua_State *L) {
    ...
    if (scripting->_CAN_dev2 == nullptr) {
        scripting->_CAN_dev2 = new ScriptingCANSensor(AG_CANManager::Driver_Type::Driver_Type_Scripting2);
        if (scripting->_CAN_dev2 == nullptr) {
            return luaL_argerror(L, 1, "CAN device nullptr");
        }
    }
...
int SRV_Channels_get_safety_state(lua_State *L) {
    binding_argcheck(L, 1);
    const bool data = hal.util->safety_switch_state() !=  AG_HAL::Util::SAFETY_ARMED;
...
int lua_get_PWMSource(lua_State *L) {
    ...
    scripting->_pwm_source[scripting->num_pwm_source] = new AG_HAL::PWMSource;
    if (scripting->_pwm_source[scripting->num_pwm_source] == nullptr) {
        return luaL_argerror(L, 1, "PWMSources device nullptr");
    }

    new_AG_HAL__PWMSource(L);
    *((AG_HAL::PWMSource**)luaL_checkudata(L, -1, "AG_HAL::PWMSource")) = scripting->_pwm_source[scripting->num_pwm_source];
