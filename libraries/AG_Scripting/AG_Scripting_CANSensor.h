...
public:
    ...
    ScriptingCANSensor(AG_CANManager::Driver_Type dtype)
        : CANSensor("Script") {
        register_driver(dtype);
    }

    // handler for outgoing frames, using uint32
    bool write_frame(AG_HAL::CANFrame &out_frame, const uint32_t timeout_us);
...
public:
    ...
    bool read_frame(AG_HAL::CANFrame &frame);

    // recursively add frame to buffer
    void handle_frame(AG_HAL::CANFrame &frame);
...
private:
    ...
    ObjectBuffer<AG_HAL::CANFrame> buffer;
