 ...
 */
#include "AG_Scripting_CANSensor.h"

#if HAL_MAX_CAN_PROTOCOL_DRIVERS

// handler for outgoing frames, using uint32
bool ScriptingCANSensor::write_frame(AG_HAL::CANFrame &out_frame, const uint32_t timeout_us)
{
    return CANSensor::write_frame(out_frame, timeout_us);
};

// handler for incoming frames, add to buffers
void ScriptingCANSensor::handle_frame(AG_HAL::CANFrame &frame)
{
    WITH_SEMAPHORE(sem);
...
bool ScriptingCANBuffer::write_frame(AG_HAL::CANFrame &out_frame, const uint32_t timeout_us)
{
    return sensor.write_frame(out_frame, timeout_us);
};

// read a frame from the buffer
bool ScriptingCANBuffer::read_frame(AG_HAL::CANFrame &frame)
{
    return buffer.pop(frame);
}

// recursively add frame to buffer
void ScriptingCANBuffer::handle_frame(AG_HAL::CANFrame &frame)
{
    WITH_SEMAPHORE(sem);
    buffer.push(frame);
    if (next != nullptr) {
