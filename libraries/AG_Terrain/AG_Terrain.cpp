...
AG_Terrain *AG_Terrain::singleton;
...
const AG_Param::GroupInfo AG_Terrain::var_info[] = {
    // @Param: ENABLE
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE", 0, AG_Terrain, enable, TERRAIN_ENABLE_DEFAULT, AP_PARAM_FLAG_ENABLE),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("SPACING",   1, AG_Terrain, grid_spacing, 100),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS",   2, AG_Terrain, options, 0),
    ...
    // @Range: 0.05 50000
    // @User: Advanced
    AP_GROUPINFO("MARGIN",   3, AG_Terrain, margin, 0.05),
    ...
    // @Range: 0 50
    // @User: Advanced
    AP_GROUPINFO("OFS_MAX",  4, AG_Terrain, offset_max, 30),
    
    AP_GROUPEND
};

// constructor
AG_Terrain::AG_Terrain() :
    ...
    fd(-1)
{
    ...
    if (singleton != nullptr) {
        AG_HAL::panic("Terrain must be singleton");
    }
#endif
    singleton = this;
}
 ...
 */
bool AG_Terrain::height_amsl(const Location &loc, float &height, bool corrected)
{
    if (!allocate()) {
        return false;
    }
...
*/
bool AG_Terrain::height_terrain_difference_home(float &terrain_difference, bool extrapolate)
{
    const AG_AHRS &ahrs = AP::ahrs();
...
*/
bool AG_Terrain::height_above_terrain(float &terrain_altitude, bool extrapolate)
{
...
/* 
   ...
   terrain altitude based on the last available data
*/
bool AG_Terrain::height_relative_home_equivalent(float terrain_altitude, 
                                                 float &relative_home_altitude,
                                                 bool extrapolate)
{
    float terrain_difference;
...
*/
float AG_Terrain::lookahead(float bearing, float distance, float climb_ratio)
{
...
bool AG_Terrain::update_surrounding_tiles(const Location &loc)
{
...
bool AG_Terrain::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len) const
{
...
void AG_Terrain::log_terrain_data()
{
    if (!allocate()) {
        return;
    }
...
*/
void AG_Terrain::set_reference_location(void)
{
...
/*
  ...
  reference location
 */
void AG_Terrain::update_reference_offset(void)
{
...
AG_Terrain *terrain()
{
    return AG_Terrain::get_singleton();
}
