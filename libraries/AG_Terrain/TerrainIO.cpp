 ...
 */
void AG_Terrain::check_disk_read(void)
{
 ...
 */
void AG_Terrain::check_disk_write(void)
{
    for (uint16_t i=0; i<cache_size; i++) {
 ...
 */
void AG_Terrain::schedule_disk_io(void)
{
    ...
    if (!timer_setup) {
        timer_setup = true;
        hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&AG_Terrain::io_timer, void));
    }
    ...
    case DiskIoDoneRead: {
        ...
        if (cache_idx != -1) {
            ...
            cache[cache_idx].last_access_ms = AG_HAL::millis();
        }
 ...
 */
void AG_Terrain::open_file(void)
{
 ...
 */
uint32_t AG_Terrain::east_blocks(struct grid_block &block) const
{
    Location loc1, loc2;
 ...
 */
void AG_Terrain::seek_offset(void)
{
    struct grid_block &block = disk_block.block;
 ...
 */
void AG_Terrain::write_block(void)
{
    seek_offset();
    if (io_failure) {
        return;
...
/*
  read in disk_block
 */
void AG_Terrain::read_block(void)
{
    seek_offset();
    if (io_failure) {
        return;
