 ...
 */
bool AG_Terrain::request_missing(mavlink_channel_t chan, struct grid_cache &gcache)
{
    ...
    last_request_time_ms[chan] = AG_HAL::millis();
 ...
 */
bool AG_Terrain::request_missing(mavlink_channel_t chan, const struct grid_info &info)
{
    // find the grid
 ...
 */
bool AG_Terrain::send_cache_request(mavlink_channel_t chan)
{
 ...
 */
void AG_Terrain::send_request(mavlink_channel_t chan)
{
    if (!allocate()) {
        // not enabled
        return;
    ...
    if (AG_HAL::millis() - last_request_time_ms[chan] < 2000) {
...
*/
uint8_t AG_Terrain::bitcount64(uint64_t b) const
{
...
*/
void AG_Terrain::get_statistics(uint16_t &pending, uint16_t &loaded) const
{
    pending = 0;
    loaded = 0;
    for (uint16_t i=0; i<cache_size; i++) {
 ...
 */
void AG_Terrain::handle_data(mavlink_channel_t chan, const mavlink_message_t &msg)
{
 ...
 */
void AG_Terrain::send_terrain_report(mavlink_channel_t chan, const Location &loc, bool extrapolate)
{
 ...
 */
void AG_Terrain::handle_terrain_check(mavlink_channel_t chan, const mavlink_message_t &msg)
{
 ...
 */
void AG_Terrain::handle_terrain_data(const mavlink_message_t &msg)
{
