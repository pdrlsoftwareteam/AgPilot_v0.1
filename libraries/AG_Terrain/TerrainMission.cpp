 ...
 */
void AG_Terrain::update_mission_data(void)
{
    ...
    if (pending && AP::gps().status() >= AG_GPS::GPS_OK_FIX_3D) {
 ...
 */
void AG_Terrain::update_rally_data(void)
{
    const AG_Rally *rally = AP::rally();
    if (rally == nullptr) {
        return;
    }
