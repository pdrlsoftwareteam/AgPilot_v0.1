...
*/
uint8_t AG_Terrain::grid_bitnum(uint8_t idx_x, uint8_t idx_y)
{    
...
/*
  ...
  in the bitmap)
 */
bool AG_Terrain::check_bitmap(const struct grid_block &grid, uint8_t idx_x, uint8_t idx_y)
{
...
/*
  ...
  grid indices
*/
void AG_Terrain::calculate_grid_info(const Location &loc, struct grid_info &info) const
{
 ...
 */
AG_Terrain::grid_cache &AG_Terrain::find_grid_cache(const struct grid_info &info)
{
    uint16_t oldest_i = 0;
    ...
    for (uint16_t i=0; i<cache_size; i++) {
        if (TERRAIN_LATLON_EQUAL(cache[i].grid.lat,info.grid_lat) &&
            ...
            cache[i].last_access_ms = AG_HAL::millis();
            return cache[i];
        }
    ...
    grid.last_access_ms = AG_HAL::millis();
 ...
 */
int16_t AG_Terrain::find_io_idx(enum GridCacheState state)
{
...
/*
  get CRC for a block
 */
uint16_t AG_Terrain::get_block_crc(struct grid_block &block)
{
