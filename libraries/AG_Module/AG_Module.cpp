...
#include <AG_Module/AG_Module_Structures.h>

struct AG_Module::hook_list *AG_Module::hooks[NUM_HOOKS];

const char *AG_Module::hook_names[AG_Module::NUM_HOOKS] = {
    "ap_hook_setup_start",
 ...
 */
void AG_Module::module_scan(const char *path)
{
#if defined(HAVE_LIBDL)
    ...
    for (uint16_t i=0; i<NUM_HOOKS; i++) {
        ...
        if (s != nullptr) {
            ...
            if (h == nullptr) {
                AG_HAL::panic("Failed to allocate hook for %s", hook_names[i]);
            }
            h->next = hooks[i];
            h->symbol = s;
            hooks[i] = h;
    ...
    if (found_hooks == 0) {
        ...
        printf("AG_Module: Loaded %u hooks from %s\n", (unsigned)found_hooks, path);
    }
#endif
}

/*
  initialise AG_Module, looking for shared libraries in the given module path
*/
void AG_Module::init(const char *module_path)
{
#if AP_MODULE_SUPPORTED
...
*/
void AG_Module::call_hook_setup_start(void)
{
#if AP_MODULE_SUPPORTED
...
*/
void AG_Module::call_hook_setup_complete(void)
{
#if AP_MODULE_SUPPORTED
    uint64_t now = AG_HAL::micros64();
...
*/
void AG_Module::call_hook_AHRS_update(const AG_AHRS &ahrs)
{
#if AP_MODULE_SUPPORTED
...
*/
void AG_Module::call_hook_gyro_sample(uint8_t instance, float dt, const Vector3f &gyro)
{
#if AP_MODULE_SUPPORTED
...
*/
void AG_Module::call_hook_accel_sample(uint8_t instance, float dt, const Vector3f &accel, bool fsync_set)
{
#if AP_MODULE_SUPPORTED
