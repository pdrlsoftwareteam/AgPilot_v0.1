...
void AG_Mount_Backend::set_angle_target(float roll_deg, float pitch_deg, float yaw_deg, bool yaw_is_earth_frame)
{
    // set angle targets
...
void AG_Mount_Backend::set_rate_target(float roll_degs, float pitch_degs, float yaw_degs, bool yaw_is_earth_frame)
{
    // set rate targets
...
void AG_Mount_Backend::set_roi_target(const Location &target_loc)
{
...
void AG_Mount_Backend::set_target_sysid(uint8_t sysid)
{
    _target_sysid = sysid;
...
void AG_Mount_Backend::handle_mount_configure(const mavlink_mount_configure_t &packet)
{
    set_mode((MAV_MOUNT_MODE)packet.mount_mode);
}

// send a GIMBAL_DEVICE_ATTITUDE_STATUS message to GCS
void AG_Mount_Backend::send_gimbal_device_attitude_status(mavlink_channel_t chan)
{
    ...
    mavlink_msg_gimbal_device_attitude_status_send(chan,
                                                   0,   // target system
                                                   0,   // target component
                                                   AG_HAL::millis(),    // autopilot system time
...
void AG_Mount_Backend::handle_mount_control(const mavlink_mount_control_t &packet)
{
    switch (get_mode()) {
...
MAV_RESULT AG_Mount_Backend::handle_command_do_mount_control(const mavlink_command_long_t &packet)
{
...
bool AG_Mount_Backend::handle_global_position_int(uint8_t msg_sysid, const mavlink_global_position_int_t &packet)
{
...
void AG_Mount_Backend::get_rc_input(float& roll_in, float& pitch_in, float& yaw_in) const
{
...
bool AG_Mount_Backend::get_rc_rate_target(MountTarget& rate_rads) const
{
...
bool AG_Mount_Backend::get_rc_angle_target(MountTarget& angle_rad) const
{
...
bool AG_Mount_Backend::get_angle_target_to_location(const Location &loc, MountTarget& angle_rad) const
{
...
// returns true on success, false on failure
bool AG_Mount_Backend::get_angle_target_to_roi(MountTarget& angle_rad) const
{
    if (!_roi_target_set) {
        return false;
    }
...
float AG_Mount_Backend::get_bf_yaw_angle(const MountTarget& angle_rad) const
{
    if (angle_rad.yaw_is_ef) {
...
float AG_Mount_Backend::get_ef_yaw_angle(const MountTarget& angle_rad) const
{
    if (angle_rad.yaw_is_ef) {
...
void AG_Mount_Backend::update_angle_target_from_rate(const MountTarget& rate_rad, MountTarget& angle_rad) const
{
...
uint16_t AG_Mount_Backend::get_gimbal_device_flags() const
{
...
// returns true on success, false on failure
bool AG_Mount_Backend::get_angle_target_to_home(MountTarget& angle_rad) const
{
...
// returns true on success, false on failure
bool AG_Mount_Backend::get_angle_target_to_sysid(MountTarget& angle_rad) const
{
...
void AG_Mount_Backend::send_warning_to_GCS(const char* warning_str)
{
