...
void AG_Mount_Siyi::init()
{
    const AG_SerialManager& serial_manager = AP::serialmanager();

    _uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Gimbal, 0);
    if (_uart != nullptr) {
        _initialised = true;
...
bool AG_Mount_Siyi::healthy() const
{
    ...
    if (AG_HAL::millis() - _last_current_angle_rad_ms > 1000) {
        return false;
    }
...
bool AG_Mount_Siyi::get_attitude_quaternion(Quaternion& att_quat)
{
...
void AG_Mount_Siyi::read_incoming_packets()
{
...
void AG_Mount_Siyi::process_packet()
{
#if AP_MOUNT_SIYI_DEBUG
    ...
    case SiyiCommandId::ACQUIRE_GIMBAL_ATTITUDE: {
        ...
        _last_current_angle_rad_ms = AG_HAL::millis();
...
bool AG_Mount_Siyi::send_packet(SiyiCommandId cmd_id, const uint8_t* databuff, uint8_t databuff_len)
{
    if (!_initialised) {
        return false;
    }
...
// returns true on success, false if outgoing serial buffer is full
bool AG_Mount_Siyi::send_1byte_packet(SiyiCommandId cmd_id, uint8_t data_byte)
{
...
void AG_Mount_Siyi::rotate_gimbal(int8_t pitch_scalar, int8_t yaw_scalar, bool yaw_is_ef)
{
...
// center gimbal
void AG_Mount_Siyi::center_gimbal()
{
...
void AG_Mount_Siyi::set_lock(bool lock)
{
...
void AG_Mount_Siyi::send_target_rates(float pitch_rads, float yaw_rads, bool yaw_is_ef)
{
...
void AG_Mount_Siyi::send_target_angles(float pitch_rad, float yaw_rad, bool yaw_is_ef)
{
...
bool AG_Mount_Siyi::take_picture()
{
...
bool AG_Mount_Siyi::record_video(bool start_recording)
{
...
bool AG_Mount_Siyi::set_zoom_step(int8_t zoom_step)
{
...
bool AG_Mount_Siyi::set_manual_focus_step(int8_t focus_step)
{
    return send_1byte_packet(SiyiCommandId::MANUAL_FOCUS, (uint8_t)focus_step);
}

// auto focus.  returns true on success
bool AG_Mount_Siyi::set_auto_focus()
{
