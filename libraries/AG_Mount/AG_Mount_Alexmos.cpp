...
void AG_Mount_Alexmos::init()
{
    const AG_SerialManager& serial_manager = AP::serialmanager();

    // check for alexmos protcol
    if ((_port = serial_manager.find_serial(AG_SerialManager::SerialProtocol_AlexMos, 0))) {
        _initialised = true;
        get_boardinfo();
...
bool AG_Mount_Alexmos::has_pan_control() const
{
    return _gimbal_3axis && yaw_range_valid();
}

// get attitude as a quaternion.  returns true on success
bool AG_Mount_Alexmos::get_attitude_quaternion(Quaternion& att_quat)
{
    if (!_initialised) {
        return false;
    }
 ...
 * get_angles
 */
void AG_Mount_Alexmos::get_angles()
{
 ...
 */
void AG_Mount_Alexmos::set_motor(bool on)
{
    if (on) {
        uint8_t data[1] = {(uint8_t)1};
 ...
 */
void AG_Mount_Alexmos::get_boardinfo()
{
    if (_board_version != 0) {
        return;
    }
...
*/
void AG_Mount_Alexmos::control_axis(const MountTarget& angle_target_rad)
{
...
*/
void AG_Mount_Alexmos::read_params(uint8_t profile_id)
{
...
*/
void AG_Mount_Alexmos::write_params()
{
    if (!_param_read_once) {
        return;
    }
...
*/
void AG_Mount_Alexmos::send_command(uint8_t cmd, uint8_t* data, uint8_t size)
{
 ...
 */
void AG_Mount_Alexmos::parse_body()
{
    switch (_command_id ) {
        case CMD_BOARD_INFO:
 ...
 */
void AG_Mount_Alexmos::read_incoming()
{
    uint8_t data;
    int16_t numc;
