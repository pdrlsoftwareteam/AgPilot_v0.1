...
void AG_Mount_SToRM32::update()
{
    ...
    if (resend_now || ((AG_HAL::millis() - _last_send) > AP_MOUNT_STORM32_RESEND_MS)) {
        send_do_mount_control(_angle_rad);
    }
}

// get attitude as a quaternion.  returns true on success
bool AG_Mount_SToRM32::get_attitude_quaternion(Quaternion& att_quat)
{
...
void AG_Mount_SToRM32::find_gimbal()
{
    ...
    if (AG_HAL::millis() > AP_MOUNT_STORM32_SEARCH_MS) {
        return;
    }
...
void AG_Mount_SToRM32::send_do_mount_control(const MountTarget& angle_target_rad)
{
    // exit immediately if not initialised
    if (!_initialised) {
        return;
