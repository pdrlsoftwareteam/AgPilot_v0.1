...
void AG_Mount_SToRM32_serial::init()
{
    const AG_SerialManager& serial_manager = AP::serialmanager();

    _port = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Gimbal, 0);
    if (_port) {
        _initialised = true;
...
void AG_Mount_SToRM32_serial::update()
{
    ...
    resend_now = resend_now || ((AG_HAL::millis() - _last_send) > AP_MOUNT_STORM32_SERIAL_RESEND_MS);

    if ((AG_HAL::millis() - _last_send) > AP_MOUNT_STORM32_SERIAL_RESEND_MS*2) {
...
bool AG_Mount_SToRM32_serial::get_attitude_quaternion(Quaternion& att_quat)
{
    att_quat.from_euler(radians(_current_angle.x * 0.01f), radians(_current_angle.y * 0.01f), radians(_current_angle.z * 0.01f));
    return true;
}

bool AG_Mount_SToRM32_serial::can_send(bool with_control) {
    uint16_t required_tx = 1;
    if (with_control) {
        required_tx += sizeof(AG_Mount_SToRM32_serial::cmd_set_angles_struct);
    }
...
// send_target_angles
void AG_Mount_SToRM32_serial::send_target_angles(const MountTarget& angle_target_rad)
{
    ...
    // store time of send
    _last_send = AG_HAL::millis();
}

void AG_Mount_SToRM32_serial::get_angles() {
    // exit immediately if not initialised
    if (!_initialised) {
        return;
    }
...
uint8_t AG_Mount_SToRM32_serial::get_reply_size(ReplyType reply_type) {
    switch (reply_type) {
        case ReplyType_DATA:
...
void AG_Mount_SToRM32_serial::read_incoming() {
    uint8_t data;
    int16_t numc;
...
void AG_Mount_SToRM32_serial::parse_reply() {
    uint16_t crc;
    bool crc_ok;
