...
#include "AG_Mount_Servo.h"
#include "AG_Mount_SoloGimbal.h"
#include "AG_Mount_Alexmos.h"
#include "AG_Mount_SToRM32.h"
#include "AG_Mount_SToRM32_serial.h"
#include "AG_Mount_Gremsy.h"
#include "AG_Mount_Siyi.h"
#include "AG_Mount_Scripting.h"
#include <stdio.h>
#include <AG_Math/location.h>
#include <SRV_Channel/SRV_Channel.h>

const AG_Param::GroupInfo AG_Mount::var_info[] = {

    // @Group: 1
    // @Path: AG_Mount_Params.cpp
    AP_SUBGROUPINFO(_params[0], "1", 43, AG_Mount, AG_Mount_Params),

#if AP_MOUNT_MAX_INSTANCES > 1
    // @Group: 2
    // @Path: AG_Mount_Params.cpp
    AP_SUBGROUPINFO(_params[1], "2", 44, AG_Mount, AG_Mount_Params),
#endif
...
AG_Mount::AG_Mount()
{
    if (_singleton != nullptr) {
        ...
        AG_HAL::panic("Mount must be singleton");
#endif
        return;
    }
    _singleton = this;
...
void AG_Mount::init()
{
    ...
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        ...
        if (mount_type == Mount_Type_Servo) {
            ...
            _backends[instance] = new AG_Mount_Servo(*this, _params[instance], true, instance);
            _num_instances++;
#endif
        ...
        // check for Solo mounts
            ...
            _backends[instance] = new AG_Mount_SoloGimbal(*this, _params[instance], instance);
            _num_instances++;
        ...
        // check for Alexmos mounts
            ...
            _backends[instance] = new AG_Mount_Alexmos(*this, _params[instance], instance);
            _num_instances++;
#endif
        ...
        // check for SToRM32 mounts using MAVLink protocol
            ...
            _backends[instance] = new AG_Mount_SToRM32(*this, _params[instance], instance);
            _num_instances++;
#endif
        ...
        // check for SToRM32 mounts using serial protocol
            ...
            _backends[instance] = new AG_Mount_SToRM32_serial(*this, _params[instance], instance);
            _num_instances++;
#endif
        ...
        // check for Gremsy mounts
            ...
            _backends[instance] = new AG_Mount_Gremsy(*this, _params[instance], instance);
            _num_instances++;
        ...
        // check for BrushlessPWM mounts (uses Servo backend)
            ...
            _backends[instance] = new AG_Mount_Servo(*this, _params[instance], false, instance);
            _num_instances++;
#endif
        ...
        // check for Siyi gimbal
            ...
            _backends[instance] = new AG_Mount_Siyi(*this, _params[instance], instance);
            _num_instances++;
        ...
        // check for Scripting gimbal
            ...
            _backends[instance] = new AG_Mount_Scripting(*this, _params[instance], instance);
            _num_instances++;
...
AG_Mount::MountType AG_Mount::get_mount_type(uint8_t instance) const
{
...
bool AG_Mount::has_pan_control(uint8_t instance) const
{
...
MAV_MOUNT_MODE AG_Mount::get_mode(uint8_t instance) const
{
    const auto *backend = get_instance(instance);
    if (backend == nullptr) {
...
void AG_Mount::set_mode_to_default(uint8_t instance)
{
...
void AG_Mount::set_mode(uint8_t instance, enum MAV_MOUNT_MODE mode)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
void AG_Mount::set_yaw_lock(uint8_t instance, bool yaw_lock)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
void AG_Mount::set_angle_target(uint8_t instance, float roll_deg, float pitch_deg, float yaw_deg, bool yaw_is_earth_frame)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
void AG_Mount::set_rate_target(uint8_t instance, float roll_degs, float pitch_degs, float yaw_degs, bool yaw_lock)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
MAV_RESULT AG_Mount::handle_command_do_mount_configure(const mavlink_command_long_t &packet)
{
...
MAV_RESULT AG_Mount::handle_command_do_mount_control(const mavlink_command_long_t &packet)
{
    auto *backend = get_primary();
    if (backend == nullptr) {
        return MAV_RESULT_FAILED;
...
MAV_RESULT AG_Mount::handle_command_do_gimbal_manager_pitchyaw(const mavlink_command_long_t &packet)
{
    AG_Mount_Backend *backend;
...
MAV_RESULT AG_Mount::handle_command_long(const mavlink_command_long_t &packet)
{
...
void AG_Mount::handle_global_position_int(const mavlink_message_t &msg)
{
...
/// Change the configuration of the mount
void AG_Mount::handle_mount_configure(const mavlink_message_t &msg)
{
    auto *backend = get_primary();
    if (backend == nullptr) {
        return;
...
void AG_Mount::handle_mount_control(const mavlink_message_t &msg)
{
    auto *backend = get_primary();
    if (backend == nullptr) {
        return;
...
void AG_Mount::send_gimbal_device_attitude_status(mavlink_channel_t chan)
{
...
bool AG_Mount::get_attitude_euler(uint8_t instance, float& roll_deg, float& pitch_deg, float& yaw_bf_deg)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len)
{
...
bool AG_Mount::get_rate_target(uint8_t instance, float& roll_degs, float& pitch_degs, float& yaw_degs, bool& yaw_is_earth_frame)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::get_angle_target(uint8_t instance, float& roll_deg, float& pitch_deg, float& yaw_deg, bool& yaw_is_earth_frame)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::get_location_target(uint8_t instance, Location& target_loc)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
void AG_Mount::set_attitude_euler(uint8_t instance, float roll_deg, float pitch_deg, float yaw_bf_deg)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
bool AG_Mount::get_camera_state(uint8_t instance, uint16_t& pic_count, bool& record_video, int8_t& zoom_step, int8_t& focus_step, bool& auto_focus)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
void AG_Mount::set_target_sysid(uint8_t instance, uint8_t sysid)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
void AG_Mount::set_roi_target(uint8_t instance, const Location &target_loc)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
bool AG_Mount::take_picture(uint8_t instance)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::record_video(uint8_t instance, bool start_recording)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::set_zoom_step(uint8_t instance, int8_t zoom_step)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::set_manual_focus_step(uint8_t instance, int8_t focus_step)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Mount::set_auto_focus(uint8_t instance)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
AG_Mount_Backend *AG_Mount::get_primary() const
{
    return get_instance(_primary);
}

AG_Mount_Backend *AG_Mount::get_instance(uint8_t instance) const
{
...
void AG_Mount::handle_gimbal_report(mavlink_channel_t chan, const mavlink_message_t &msg)
{
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
...
void AG_Mount::handle_message(mavlink_channel_t chan, const mavlink_message_t &msg)
{
    switch (msg.msgid) {
    ...
    default:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Unhandled mount case");
#endif
        break;
    }
}
...
// handle PARAM_VALUE
void AG_Mount::handle_param_value(const mavlink_message_t &msg)
{
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
...
void AG_Mount::handle_gimbal_device_information(const mavlink_message_t &msg)
{
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
...
void AG_Mount::handle_gimbal_device_attitude_status(const mavlink_message_t &msg)
{
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
...
void AG_Mount::convert_params()
{
    ...
    int8_t mnt_type = 0;
    IGNORE_RETURN(AG_Param::get_param_by_index(this, 19, AP_PARAM_INT8, &mnt_type));
    if (mnt_type > 0) {
        int8_t stab_roll = 0;
        int8_t stab_pitch = 0;
        IGNORE_RETURN(AG_Param::get_param_by_index(this, 4, AP_PARAM_INT8, &stab_roll));
        IGNORE_RETURN(AG_Param::get_param_by_index(this, 5, AP_PARAM_INT8, &stab_pitch));
    ...
    int8_t jstick_spd = 0;
    if (AG_Param::get_param_by_index(this, 16, AP_PARAM_INT8, &jstick_spd) && (jstick_spd > 0)) {
        _params[0].rc_rate_max.set_and_save(jstick_spd * 0.3);
    }

    // find Mount's top level key
    uint16_t k_param_mount_key;
    if (!AG_Param::find_top_level_key_by_pointer(this, k_param_mount_key)) {
        return;
    }

    // table of mount parameters to be converted without scaling
    static const AG_Param::ConversionInfo mnt_param_conversion_info[] {
    ...
    for (uint8_t i=0; i<table_size; i++) {
        AG_Param::convert_old_parameter(&mnt_param_conversion_info[i], 1.0f);
    }

    // mount parameters conversion from centi-degrees to degrees
    static const AG_Param::ConversionInfo mnt_param_deg_conversion_info[] {
    ...
    for (uint8_t i=0; i<table_size; i++) {
        AG_Param::convert_old_parameter(&mnt_param_deg_conversion_info[i], 0.01f);
    }
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(mnt_rc_conversion_table); i++) {
        int8_t mnt_rcin = 0;
        if (AG_Param::get_param_by_index(this, mnt_rc_conversion_table[i].old_rcin_idx, AP_PARAM_INT8, &mnt_rcin) && (mnt_rcin > 0)) {
            // get pointers to the appropriate RCx_OPTION parameter
            char pname[17];
            enum ap_var_type ptype;
            snprintf(pname, sizeof(pname), "RC%u_OPTION", (unsigned)mnt_rcin);
            AP_Int16 *rcx_option = (AP_Int16 *)AG_Param::find(pname, &ptype);
...
// singleton instance
AG_Mount *AG_Mount::_singleton;

namespace AP {

AG_Mount *mount()
{
    return AG_Mount::get_singleton();
}
