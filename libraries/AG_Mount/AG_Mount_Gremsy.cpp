...
bool AG_Mount_Gremsy::healthy() const
{
    ...
    if (AG_HAL::millis() - _last_attitude_status_ms > 1000) {
        return false;
    }
...
bool AG_Mount_Gremsy::get_attitude_quaternion(Quaternion& att_quat)
{
...
void AG_Mount_Gremsy::find_gimbal()
{
...
void AG_Mount_Gremsy::handle_gimbal_device_information(const mavlink_message_t &msg)
{
...
void AG_Mount_Gremsy::handle_gimbal_device_attitude_status(const mavlink_message_t &msg)
{
    // exit immediately if this is not our message
    if (msg.sysid != _sysid || msg.compid != _compid) {
        return;
    ...
    _last_attitude_status_ms = AG_HAL::millis();
}

// request GIMBAL_DEVICE_INFORMATION message
void AG_Mount_Gremsy::request_gimbal_device_information() const
{
    if (_link == nullptr) {
        return;
    }
...
bool AG_Mount_Gremsy::start_sending_attitude_to_gimbal()
{
...
void AG_Mount_Gremsy::send_gimbal_device_retract() const
{
...
void AG_Mount_Gremsy::send_gimbal_device_set_rate(float roll_rads, float pitch_rads, float yaw_rads, bool earth_frame) const
{
    // prepare flags
...
void AG_Mount_Gremsy::send_gimbal_device_set_attitude(float roll_rad, float pitch_rad, float yaw_rad, bool earth_frame) const
{
    // exit immediately if not initialised
    if (!_initialised) {
        return;
