...
public:
    ...
    class AG_DAL_RangeFinder_Backend *get_backend(uint8_t id) const;
...
private:
    ...
    AG_DAL_RangeFinder_Backend **_backend;
};


class AG_DAL_RangeFinder_Backend {
public:

    AG_DAL_RangeFinder_Backend(struct log_RRNI &RRNI);
    ...
    AG_DAL_RangeFinder::Status status() const {
        return (AG_DAL_RangeFinder::Status)_RRNI.status;
    }
    ...
    // DAL methods:
    void start_frame(AG_RangeFinder_Backend *backend);
