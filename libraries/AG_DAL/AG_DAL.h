...
#include "AG_DAL_InertialSensor.h"
#include "AG_DAL_Baro.h"
#include "AG_DAL_GPS.h"
#include "AG_DAL_RangeFinder.h"
#include "AG_DAL_Compass.h"
#include "AG_DAL_Airspeed.h"
...
public:
    ...
    // must remain the same as AG_AHRS_VehicleClass numbers-wise
    ...
    static AG_DAL *get_singleton() {
        if (!_singleton) {
            _singleton = new AG_DAL();
        }
        return _singleton;
    }
    ...
    AG_DAL_InertialSensor &ins() { return _ins; }
    AG_DAL_Baro &baro() { return _baro; }
    AG_DAL_GPS &gps() { return _gps; }

    AG_DAL_RangeFinder *rangefinder() {
        return _rangefinder;
    }
    AG_DAL_Airspeed *airspeed() {
        return _airspeed;
    }
#if AP_BEACON_ENABLED
    AG_DAL_Beacon *beacon() {
        return _beacon;
    }
#endif
#if HAL_VISUALODOM_ENABLED
    AG_DAL_VisualOdom *visualodom() {
        return _visualodom;
    }
#endif

    AG_DAL_Compass &compass() { return _compass; }

    // random methods that AG_NavEKF3 wants to call on AHRS:
    ...
    void handle_message(const log_RRNH &msg) {
        if (_rangefinder == nullptr) {
            _rangefinder = new AG_DAL_RangeFinder;
        }
    ...
    void handle_message(const log_RVOH &msg) {
        ...
        if (_visualodom == nullptr) {
            _visualodom = new AG_DAL_VisualOdom;
        }
...
private:
    ...
    AG_DAL_InertialSensor _ins;
    AG_DAL_Baro _baro;
    AG_DAL_GPS _gps;
    AG_DAL_RangeFinder *_rangefinder;
    AG_DAL_Compass _compass;
    AG_DAL_Airspeed *_airspeed;
#if AP_BEACON_ENABLED
    AG_DAL_Beacon *_beacon;
#endif
#if HAL_VISUALODOM_ENABLED
    AG_DAL_VisualOdom *_visualodom;
#endif
...
#define WRITE_REPLAY_BLOCK(sname,v) AG_DAL::WriteLogMessage(LOG_## sname ##_MSG, &v, nullptr, offsetof(log_ ##sname, _end))
#define WRITE_REPLAY_BLOCK_IFCHANGED(sname,v,old) do { static_assert(sizeof(v) == sizeof(old), "types must match"); \
                                                      AG_DAL::WriteLogMessage(LOG_## sname ##_MSG, &v, &old, offsetof(log_ ##sname, _end)); } \
                                                 while (0)
