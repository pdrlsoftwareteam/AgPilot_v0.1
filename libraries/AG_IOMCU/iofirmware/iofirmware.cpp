...
void AG_IOMCU_FW::pwm_out_update()
{
...
void AG_IOMCU_FW::heater_update()
{
    uint32_t now = last_ms;
    if (!has_heater) {
...
void AG_IOMCU_FW::process_io_packet()
{
 ...
 */
void AG_IOMCU_FW::page_status_update(void)
{
...
bool AG_IOMCU_FW::handle_code_read()
{
...
bool AG_IOMCU_FW::handle_code_write()
{
    switch (rx_io_packet.page) {
    case PAGE_SETUP:
...
void AG_IOMCU_FW::schedule_reboot(uint32_t time_ms)
{
    do_reboot = true;
    reboot_time = last_ms + time_ms;
}

void AG_IOMCU_FW::calculate_fw_crc(void)
{
...
/*
  update safety state
 */
void AG_IOMCU_FW::safety_update(void)
{
    uint32_t now = last_ms;
 ...
 */
void AG_IOMCU_FW::rcout_mode_update(void)
{
    bool use_oneshot = (reg_setup.features & P_SETUP_FEATURES_ONESHOT) != 0;
    if (use_oneshot && !oneshot_enabled) {
        oneshot_enabled = true;
        hal.rcout->set_output_mode(reg_setup.pwm_rates, AG_HAL::RCOutput::MODE_PWM_ONESHOT);
    }
    ...
    if (use_brushed && !brushed_enabled) {
        ...
        hal.rcout->set_output_mode(reg_setup.pwm_rates, AG_HAL::RCOutput::MODE_PWM_BRUSHED);
        hal.rcout->set_freq(reg_setup.pwm_rates, reg_setup.pwm_altrate);
    }
}
 ...
 */
void AG_IOMCU_FW::fill_failsafe_pwm(void)
{
