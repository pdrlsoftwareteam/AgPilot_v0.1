...
void sbus_out_write(uint16_t *channels, uint8_t nchannels)
{
    if (sd3_config == 0) {
        uint8_t buffer[25];
        AG_SBusOut::sbus_format_frame(channels, nchannels, buffer);
 ...
 */
void AG_IOMCU_FW::rcin_serial_init(void)
{
    sdStart(&SD1, &dsm_cfg);
    ...
    // SBUS and FPORT
    AP::RC().disable_for_pulses(AG_RCProtocol::SBUS);
    AP::RC().disable_for_pulses(AG_RCProtocol::SBUS_NI);
    AP::RC().disable_for_pulses(AG_RCProtocol::FPORT);
}
 ...
 */
void AG_IOMCU_FW::rcin_serial_update(void)
{
    uint8_t b[16];
    uint32_t n;
 ...
 */
void AG_IOMCU_FW::dsm_bind_step(void)
{
