...
#ifndef AG_IOMCU_FORCE_ENABLE_HEATER
#define AG_IOMCU_FORCE_ENABLE_HEATER 0
#endif

AG_IOMCU::AG_IOMCU(AG_HAL::UARTDriver &_uart) :
    uart(_uart)
{
    singleton = this;
}
...
AG_IOMCU *AG_IOMCU::singleton;

/*
  initialise library, starting thread
 */
void AG_IOMCU::init(void)
{
    ...
    AG_BoardConfig *boardconfig = AG_BoardConfig::get_singleton();
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_IOMCU::thread_main, void), "IOMCU",
                                      1024, AG_HAL::Scheduler::PRIORITY_BOOST, 1)) {
        AG_HAL::panic("Unable to allocate IOMCU thread");
    }
    initialised = true;
}
 ...
 */
void AG_IOMCU::event_failed(uint32_t event_mask)
{
...
/*
  main IO thread loop
 */
void AG_IOMCU::thread_main(void)
{
    ...
    while (!do_shutdown) {
        ...
        if (last_reg_read_ms != 0 && now_ms - last_reg_read_ms > 1000U) {
            INTERNAL_ERROR(AG_InternalError::error_t::iomcu_reset);
            last_reg_read_ms = 0;
        }
...
#if AG_IOMCU_FORCE_ENABLE_HEATER
        ...
        if (now - last_rc_read_ms > 20) {
            ...
            read_rc_input();
            last_rc_read_ms = AG_HAL::millis();
        }

        if (now - last_status_read_ms > 50) {
            // read status at 20Hz
            read_status();
            last_status_read_ms = AG_HAL::millis();
            write_log();
        }
        ...
        if (now - last_servo_read_ms > 50) {
            ...
            read_servo();
            last_servo_read_ms = AG_HAL::millis();
        }
 ...
 */
void AG_IOMCU::send_servo_out()
{
#if 0
    // simple method to test IO failsafe
    if (AG_HAL::millis() > 30000) {
        return;
    }
#endif
...
void AG_IOMCU::read_rc_input()
{
    ...
    if (rc_input.flags_rc_ok && !rc_input.flags_failsafe) {
        rc_last_input_ms = AG_HAL::millis();
    }
}
...
void AG_IOMCU::read_status()
{
    ...
    if (reg_status.flag_safety_off == 0) {
        ...
        if (safety_forced_off && (options & AG_BoardConfig::BOARD_SAFETY_OPTION_BUTTON_ACTIVE_SAFETY_ON) == 0) {
...
void AG_IOMCU::write_log()
{
    uint32_t now = AG_HAL::millis();
    if (now - last_log_ms >= 1000U) {
        last_log_ms = now;
        if (AG_Logger::get_singleton()) {
// @LoggerMessage: IOMC
 ...
 */
void AG_IOMCU::discard_input(void)
{
    uart.discard_input();
}
 ...
 */
size_t AG_IOMCU::write_wait(const uint8_t *pkt, uint8_t len)
{
...
*/
bool AG_IOMCU::read_registers(uint8_t page, uint8_t offset, uint8_t count, uint16_t *regs)
{
    ...
    if (!uart.wait_timeout(count*2+4, 10)) {
        debug("t=%u timeout read page=%u offset=%u count=%u\n",
              AG_HAL::millis(), page, offset, count);
        protocol_fail_count++;
        return false;
    }
    ...
    if (got_crc != expected_crc) {
        debug("t=%u bad crc %02x should be %02x n=%u %u/%u/%u\n",
              AG_HAL::millis(), got_crc, expected_crc,
              n, page, offset, count);
        protocol_fail_count++;
        return false;
    }
    ...
    protocol_count++;
    last_reg_read_ms = AG_HAL::millis();
    return true;
}
...
*/
bool AG_IOMCU::write_registers(uint8_t page, uint8_t offset, uint8_t count, const uint16_t *regs)
{
    while (count > PKT_MAX_REGS) {
...
bool AG_IOMCU::modify_register(uint8_t page, uint8_t offset, uint16_t clearbits, uint16_t setbits)
{
    uint16_t v = 0;
...
void AG_IOMCU::write_channel(uint8_t chan, uint16_t pwm)
{
...
// trigger an ioevent
void AG_IOMCU::trigger_event(uint8_t event)
{
...
AG_HAL::Util::safety_state AG_IOMCU::get_safety_switch_state(void) const
{
    return reg_status.flag_safety_off?AG_HAL::Util::SAFETY_ARMED:AG_HAL::Util::SAFETY_DISARMED;
}

// force safety on
bool AG_IOMCU::force_safety_on(void)
{
...
// force safety off
void AG_IOMCU::force_safety_off(void)
{
...
uint16_t AG_IOMCU::read_channel(uint8_t chan)
{
    return pwm_in.pwm[chan];
}

// cork output
void AG_IOMCU::cork(void)
{
    corked = true;
}

// push output
void AG_IOMCU::push(void)
{
...
void AG_IOMCU::set_freq(uint16_t chmask, uint16_t freq)
{
...
uint16_t AG_IOMCU::get_freq(uint16_t chan)
{
...
// enable SBUS out
bool AG_IOMCU::enable_sbus_out(uint16_t rate_hz)
{
...
*/
bool AG_IOMCU::check_rcinput(uint32_t &last_frame_us, uint8_t &num_channels, uint16_t *channels, uint8_t max_chan)
{
...
void AG_IOMCU::set_heater_duty_cycle(uint8_t duty_cycle)
{
...
void AG_IOMCU::set_default_rate(uint16_t rate_hz)
{
...
void AG_IOMCU::set_oneshot_mode(void)
{
...
void AG_IOMCU::set_brushed_mode(void)
{
...
void AG_IOMCU::update_safety_options(void)
{
    AG_BoardConfig *boardconfig = AG_BoardConfig::get_singleton();
    if (!boardconfig) {
        return;
    ...
    uint16_t options = boardconfig->get_safety_button_options();
    if (!(options & AG_BoardConfig::BOARD_SAFETY_OPTION_BUTTON_ACTIVE_SAFETY_OFF)) {
        desired_options |= P_SETUP_ARMING_SAFETY_DISABLE_OFF;
    }
    if (!(options & AG_BoardConfig::BOARD_SAFETY_OPTION_BUTTON_ACTIVE_SAFETY_ON)) {
        desired_options |= P_SETUP_ARMING_SAFETY_DISABLE_ON;
    }
    if (!(options & AG_BoardConfig::BOARD_SAFETY_OPTION_BUTTON_ACTIVE_ARMED) && AP::arming().is_armed()) {
...
void AG_IOMCU::send_rc_protocols()
{
 ...
 */
bool AG_IOMCU::check_crc(void)
{
    // flash size minus 4k bootloader
	const uint32_t flash_size = 0x10000 - 0x1000;

    fw = AG_ROMFS::find_decompress(fw_name, fw_size);
    if (!fw) {
    ...
    if (!upload_fw()) {
        AG_ROMFS::free(fw);
        fw = nullptr;
        AG_BoardConfig::config_error("Failed to update IO firmware");
    }
 ...
 */
void AG_IOMCU::set_failsafe_pwm(uint16_t chmask, uint16_t period_us)
{
    bool changed = false;
...
void AG_IOMCU::set_safety_mask(uint16_t chmask)
{
 ...
 */
void AG_IOMCU::bind_dsm(uint8_t mode)
{
 ...
 */
bool AG_IOMCU::setup_mixing(RCMapper *rcmap, int8_t override_chan,
 ...
 */
const char *AG_IOMCU::get_rc_protocol(void)
{
    if (!is_chibios_backend) {
        return nullptr;
    }
    return AG_RCProtocol::protocol_name_from_protocol((AG_RCProtocol::rcprotocol_t)rc_input.rc_protocol);
}
...
/*
  ...
  to a watchdog).
 */
void AG_IOMCU::handle_repeated_failures(void)
{
    ...
    INTERNAL_ERROR(AG_InternalError::error_t::iomcu_fail);
}

/*
  check for IOMCU reset (possibly due to a watchdog).
 */
void AG_IOMCU::check_iomcu_reset(void)
{
    ...
    debug("IOMCU reset t=%u %u %u dt=%u\n",
          unsigned(AG_HAL::millis()), unsigned(ts1), unsigned(reg_status.timestamp_ms), unsigned(dt_ms));
    ...
    if (last_safety_off && !reg_status.flag_safety_off && AP::arming().is_armed()) {
        ...
        if (safety_forced_off || (options & AG_BoardConfig::BOARD_SAFETY_OPTION_BUTTON_ACTIVE_ARMED) == 0) {
...
bool AG_IOMCU::valid_GPIO_pin(uint8_t pin) const
{
...
bool AG_IOMCU::convert_pin_number(uint8_t& pin) const
{
    if (pin < 101 || pin > 108) {
        return false;
    }
...
void AG_IOMCU::set_GPIO_mask(uint8_t mask)
{
...
void AG_IOMCU::write_GPIO(uint8_t pin, bool value)
{
    if (!convert_pin_number(pin)) {
        return;
    }
...
// toggle a output pin
void AG_IOMCU::toggle_GPIO(uint8_t pin)
{
    if (!convert_pin_number(pin)) {
        return;
    }
