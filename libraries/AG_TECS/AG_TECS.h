...
class AG_TECS {
public:
    AG_TECS(AG_AHRS &ahrs, const AG_FixedWing &parms, const AG_Landing &landing, const uint32_t log_bitmask)
        : _ahrs(ahrs)
        , aparm(parms)
        , _landing(landing)
...
private:
    ...
    const AG_FixedWing &aparm;

    // reference to const AG_Landing to access it's params
    const AG_Landing &_landing;
    ...
    enum AG_FixedWing::FlightStage _flight_stage;
