...
AG_Airspeed_DLVR::AG_Airspeed_DLVR(AG_Airspeed &_frontend, uint8_t _instance, const float _range_inH2O) :
 ...
 */
AG_Airspeed_Backend *AG_Airspeed_DLVR::probe(AG_Airspeed &_frontend,
                                             uint8_t _instance,
                                             ...
                                             const float _range_inH2O)
{
    ...
    AG_Airspeed_DLVR *sensor = new AG_Airspeed_DLVR(_frontend, _instance, _range_inH2O);
    if (!sensor) {
        return nullptr;
    }
...
void AG_Airspeed_DLVR::setup()
{
    ...
    dev->register_periodic_callback(1000000UL/50U,
                                    FUNCTOR_BIND_MEMBER(&AG_Airspeed_DLVR::timer, void));
}

// probe and initialise the sensor
bool AG_Airspeed_DLVR::init()
{
...
bool AG_Airspeed_DLVR::get_differential_pressure(float &_pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AG_Airspeed_DLVR::get_temperature(float &_temperature)
{
    WITH_SEMAPHORE(sem);
