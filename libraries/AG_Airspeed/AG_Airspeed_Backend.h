...
public:
    AG_Airspeed_Backend(AG_Airspeed &frontend, uint8_t instance);
    virtual ~AG_Airspeed_Backend();
...
protected:
    ...
    bool disable_voltage_correction(void) const {
        return (frontend._options.get() & AG_Airspeed::OptionsMask::DISABLE_VOLTAGE_CORRECTION) != 0;
    }

    AG_Airspeed::pitot_tube_order get_tube_order(void) const {
#ifndef HAL_BUILD_AP_PERIPH
        return AG_Airspeed::pitot_tube_order(frontend.param[instance].tube_order.get());
#else
        return AG_Airspeed::pitot_tube_order::PITOT_TUBE_ORDER_AUTO;
#endif
    }
