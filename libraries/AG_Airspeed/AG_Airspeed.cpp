 ...
 */
/*
 *   AG_Airspeed.cpp - airspeed (pitot) driver
 */
...
// Dummy the AG_Airspeed class to allow building Airspeed only for plane, rover, sub, and copter & heli 2MB boards
// This could be removed once the build system allows for APM_BUILD_TYPE in header files
// Note that this is also defined in AG_Airspeed_Params.cpp
...
#include <utility>
#include "AG_Airspeed_MS4525.h"
#include "AG_Airspeed_MS5525.h"
#include "AG_Airspeed_SDP3X.h"
#include "AG_Airspeed_DLVR.h"
#include "AG_Airspeed_analog.h"
#include "AG_Airspeed_ASP5033.h"
#include "AG_Airspeed_Backend.h"
#include "AG_Airspeed_UAVCAN.h"
#include "AG_Airspeed_NMEA.h"
#include "AG_Airspeed_MSP.h"
#include "AG_Airspeed_SITL.h"
...
#define OPTIONS_DEFAULT AG_Airspeed::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_DO_DISABLE | AG_Airspeed::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_RECOVERY_DO_REENABLE | AG_Airspeed::OptionsMask::USE_EKF_CONSISTENCY

#define ENABLE_PARAMETER !(APM_BUILD_TYPE(APM_BUILD_ArduPlane) || defined(HAL_BUILD_AP_PERIPH))

// table of user settable parameters
const AG_Param::GroupInfo AG_Airspeed::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("_ENABLE", 30, AG_Airspeed, _enable, 0, AP_PARAM_FLAG_ENABLE),
#endif
    // slots 0-9 (and 63) were previously used by params before being refactored into AG_Airspeed_Params
    ...
    // @User: Advanced
    AP_GROUPINFO("_PRIMARY", 10, AG_Airspeed, primary_sensor, 0),
#endif

    // 11-20 were previously used by second sensor params before being refactored into AG_Airspeed_Params
    ...
    // @User: Advanced
    AP_GROUPINFO("_OPTIONS", 21, AG_Airspeed, _options, OPTIONS_DEFAULT),
    ...
    // @Units: m/s
    // @User: Advanced
    AP_GROUPINFO("_WIND_MAX", 22, AG_Airspeed, _wind_max, 0),
    ...
    // @Units: m/s
    // @User: Advanced
    AP_GROUPINFO("_WIND_WARN", 23, AG_Airspeed, _wind_warn, 0),
    ...
    // @Range: 0.0 10.0
    // @User: Advanced
    AP_GROUPINFO("_WIND_GATE", 26, AG_Airspeed, _wind_gate, 5.0f),
    ...
    // @Units: %
    // @User: Advanced
    AP_GROUPINFO_FRAME("_OFF_PCNT", 27, AG_Airspeed, max_speed_pcnt, 0, AP_PARAM_FRAME_PLANE),    

#endif

    // @Group: _
    // @Path: AG_Airspeed_Params.cpp
    AP_SUBGROUPINFO(param[0], "_", 28, AG_Airspeed, AG_Airspeed_Params),

#if AIRSPEED_MAX_SENSORS > 1
    // @Group: 2_
    // @Path: AG_Airspeed_Params.cpp
    AP_SUBGROUPINFO(param[1], "2_", 29, AG_Airspeed, AG_Airspeed_Params),
#endif
...
AG_Airspeed::AG_Airspeed()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_Airspeed must be singleton");
    }
    _singleton = this;
}

void AG_Airspeed::set_fixedwing_parameters(const AG_FixedWing *_fixed_wing_parameters)
{
...
bool AG_Airspeed::add_backend(AG_Airspeed_Backend *backend)
{
    if (!backend) {
        return false;
    }
    if (num_sensors >= AIRSPEED_MAX_SENSORS) {
        AG_HAL::panic("Too many airspeed drivers");
    }
...
void AG_Airspeed::convert_per_instance()
{
    AG_Param::ConversionInfo info;
#ifndef HAL_BUILD_AP_PERIPH
    // Vehicle conversion
    if (!AG_Param::find_key_by_pointer(this, info.old_key)) {
        return;
    }
    ...
    for (const auto & elem : conversion_table) {
        ...
        for (uint8_t i=0; i < MIN(AIRSPEED_MAX_SENSORS,2); i++) {
            ...
            AG_Param::convert_old_parameter(&info, 1.0, 0);
        }
    }
}

void AG_Airspeed::init()
{
...
void AG_Airspeed::allocate()
{
    ...
    for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
        ...
        case TYPE_I2C_MS4525:
            ...
            sensor[i] = new AG_Airspeed_MS4525(*this, i);
#endif
            break;
        case TYPE_SITL:
#if AP_AIRSPEED_SITL_ENABLED
            sensor[i] = new AG_Airspeed_SITL(*this, i);
#endif
            break;
        case TYPE_ANALOG:
#if AP_AIRSPEED_ANALOG_ENABLED
            sensor[i] = new AG_Airspeed_Analog(*this, i);
#endif
            break;
        case TYPE_I2C_MS5525:
#if AP_AIRSPEED_MS5525_ENABLED
            sensor[i] = new AG_Airspeed_MS5525(*this, i, AG_Airspeed_MS5525::MS5525_ADDR_AUTO);
#endif
            break;
        case TYPE_I2C_MS5525_ADDRESS_1:
#if AP_AIRSPEED_MS5525_ENABLED
            sensor[i] = new AG_Airspeed_MS5525(*this, i, AG_Airspeed_MS5525::MS5525_ADDR_1);
#endif
            break;
        case TYPE_I2C_MS5525_ADDRESS_2:
#if AP_AIRSPEED_MS5525_ENABLED
            sensor[i] = new AG_Airspeed_MS5525(*this, i, AG_Airspeed_MS5525::MS5525_ADDR_2);
#endif
            break;
        case TYPE_I2C_SDP3X:
#if AP_AIRSPEED_SDP3X_ENABLED
            sensor[i] = new AG_Airspeed_SDP3X(*this, i);
#endif
            break;
        case TYPE_I2C_DLVR_5IN:
#if AP_AIRSPEED_DLVR_ENABLED
            sensor[i] = new AG_Airspeed_DLVR(*this, i, 5);
#endif
            break;
        case TYPE_I2C_DLVR_10IN:
#if AP_AIRSPEED_DLVR_ENABLED
            sensor[i] = new AG_Airspeed_DLVR(*this, i, 10);
#endif
            break;
        case TYPE_I2C_DLVR_20IN:
#if AP_AIRSPEED_DLVR_ENABLED
            sensor[i] = new AG_Airspeed_DLVR(*this, i, 20);
#endif
            break;
        case TYPE_I2C_DLVR_30IN:
#if AP_AIRSPEED_DLVR_ENABLED
            sensor[i] = new AG_Airspeed_DLVR(*this, i, 30);
#endif
            break;
        case TYPE_I2C_DLVR_60IN:
#if AP_AIRSPEED_DLVR_ENABLED
            sensor[i] = new AG_Airspeed_DLVR(*this, i, 60);
#endif  // AP_AIRSPEED_DLVR_ENABLED
            break;
        case TYPE_I2C_ASP5033:
#if AP_AIRSPEED_ASP5033_ENABLED
            sensor[i] = new AG_Airspeed_ASP5033(*this, i);
#endif
            break;
        case TYPE_UAVCAN:
#if AP_AIRSPEED_UAVCAN_ENABLED
            sensor[i] = AG_Airspeed_UAVCAN::probe(*this, i, uint32_t(param[i].bus_id.get()));
#endif
            break;
        case TYPE_NMEA_WATER:
#if AP_AIRSPEED_NMEA_ENABLED
#if APM_BUILD_TYPE(APM_BUILD_Rover) || APM_BUILD_TYPE(APM_BUILD_ArduSub) 
            sensor[i] = new AG_Airspeed_NMEA(*this, i);
#endif
#endif
            break;
        case TYPE_MSP:
#if AP_AIRSPEED_MSP_ENABLED
            sensor[i] = new AG_Airspeed_MSP(*this, i, 0);
#endif
            break;
        }
    ...
    for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
        if (sensor[i] == nullptr && (enum airspeed_type)param[i].type.get() == TYPE_UAVCAN) {
            sensor[i] = AG_Airspeed_UAVCAN::probe(*this, i, 0);
            if (sensor[i] != nullptr) {
                num_sensors = i+1;
            }
        }
...
float AG_Airspeed::get_pressure(uint8_t i)
{
    if (!enabled(i)) {
        return 0;
    }
...
bool AG_Airspeed::get_temperature(uint8_t i, float &temperature)
{
    if (!enabled(i)) {
        return false;
    }
...
void AG_Airspeed::calibrate(bool in_startup)
{
#ifndef HAL_BUILD_AP_PERIPH
    if (!lib_enabled()) {
        return;
    ...
    for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
        ...
        state[i].cal.start_ms = AG_HAL::millis();
        state[i].cal.count = 0;
        state[i].cal.sum = 0;
...
*/
void AG_Airspeed::update_calibration(uint8_t i, float raw_pressure)
{
#ifndef HAL_BUILD_AP_PERIPH
    ...
    if (AG_HAL::millis() - state[i].cal.start_ms >= 1000 &&
...
AG_Airspeed::CalibrationState AG_Airspeed::get_calibration_state() const
{
    for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
...
void AG_Airspeed::read(uint8_t i)
{
    if (!enabled(i) || !sensor[i]) {
        return;
    }
    state[i].last_update_ms = AG_HAL::millis();
 ...
 */
void AG_Airspeed::handle_msp(const MSP::msp_airspeed_data_message_t &pkt)
{
    if (!lib_enabled()) {
        return;
    }
...
bool AG_Airspeed::use(uint8_t i) const
{
#ifndef HAL_BUILD_AP_PERIPH
    if (!lib_enabled()) {
        return false;
 ...
 */
bool AG_Airspeed::all_healthy(void) const
{
    for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
...
bool AG_Airspeed::lib_enabled() const {
#if ENABLE_PARAMETER
    return _enable > 0;
#endif
    return true;
...
bool AG_Airspeed::enabled(uint8_t i) const {
    if (!lib_enabled()) {
        return false;
    }
...
bool AG_Airspeed::healthy(uint8_t i) const {
...
float AG_Airspeed::get_airspeed(uint8_t i) const {
    if (!enabled(i)) {
...
float AG_Airspeed::get_raw_airspeed(uint8_t i) const {
    if (!enabled(i)) {
        // we can't have negative airspeed so sending an obviously invalid value
        return -1.0;
    }
...
float AG_Airspeed::get_differential_pressure(uint8_t i) const {
    if (!enabled(i)) {
        return 0.0;
    }
...
float AG_Airspeed::get_corrected_pressure(uint8_t i) const {
    if (!enabled(i)) {
        return 0.0;
    }
...
#if AP_AIRSPEED_HYGROMETER_ENABLE
bool AG_Airspeed::get_hygrometer(uint8_t i, uint32_t &last_sample_ms, float &temperature, float &humidity) const
{
    if (!enabled(i) || sensor[i] == nullptr) {
        return false;
    }
...
const AG_Param::GroupInfo AG_Airspeed::var_info[] = { AP_GROUPEND };

void AG_Airspeed::update() {};
bool AG_Airspeed::get_temperature(uint8_t i, float &temperature) { return false; }
void AG_Airspeed::calibrate(bool in_startup) {}
AG_Airspeed::CalibrationState AG_Airspeed::get_calibration_state() const { return CalibrationState::NOT_STARTED; }
bool AG_Airspeed::use(uint8_t i) const { return false; }
bool AG_Airspeed::enabled(uint8_t i) const { return false; }
bool AG_Airspeed::healthy(uint8_t i) const { return false; }
float AG_Airspeed::get_airspeed(uint8_t i) const { return 0.0; }
float AG_Airspeed::get_differential_pressure(uint8_t i) const { return 0.0; }

#if AP_AIRSPEED_MSP_ENABLED
void AG_Airspeed::handle_msp(const MSP::msp_airspeed_data_message_t &pkt) {}
#endif

bool AG_Airspeed::all_healthy(void) const { return false; }
void AG_Airspeed::init(void) {};
AG_Airspeed::AG_Airspeed() {}

#endif // #if AP_AIRSPEED_DUMMY_METHODS_ENABLED

// singleton instance
AG_Airspeed *AG_Airspeed::_singleton;

namespace AP {

AG_Airspeed *airspeed()
{
    return AG_Airspeed::get_singleton();
}
