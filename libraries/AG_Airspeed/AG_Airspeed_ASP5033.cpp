...
bool AG_Airspeed_ASP5033::init()
{
    ...
    for (uint8_t address : addresses) {
        ...
        dev->register_periodic_callback(1000000UL/80U,
                                        FUNCTOR_BIND_MEMBER(&AG_Airspeed_ASP5033::timer, void));
        return true;
    }
...
/*
  ...
  expected behaviour
*/
bool AG_Airspeed_ASP5033::confirm_sensor_id(void)
{
    uint8_t part_id;
...
void AG_Airspeed_ASP5033::timer()
{
...
bool AG_Airspeed_ASP5033::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);

    if (AG_HAL::millis() - last_sample_ms > 100) {
        return false;
    }
...
bool AG_Airspeed_ASP5033::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
