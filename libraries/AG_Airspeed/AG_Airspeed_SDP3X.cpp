...
AG_Airspeed_SDP3X::AG_Airspeed_SDP3X(AG_Airspeed &_frontend, uint8_t _instance) :
 ...
 */
bool AG_Airspeed_SDP3X::_send_command(uint16_t cmd)
{
...
bool AG_Airspeed_SDP3X::init()
{
    ...
    _dev->register_periodic_callback(20000,
                                     FUNCTOR_BIND_MEMBER(&AG_Airspeed_SDP3X::_timer, void));
    return true;
}

// read the values from the sensor. Called at 50Hz
void AG_Airspeed_SDP3X::_timer()
{
 ...
 */
float AG_Airspeed_SDP3X::_correct_pressure(float press)
{
    float sign = 1.0f;
    
    // fix for tube order
    AG_Airspeed::pitot_tube_order tube_order = get_tube_order();
    switch (tube_order) {
    case AG_Airspeed::PITOT_TUBE_ORDER_NEGATIVE:
        press = -press;
        sign = -1.0f;
        break;
    case AG_Airspeed::PITOT_TUBE_ORDER_POSITIVE:
        break;
    case AG_Airspeed::PITOT_TUBE_ORDER_AUTO:
    default:
        if (press < 0.0f) {
            sign = -1.0f;
            press = -press;
    ...
    AG_Baro *baro = AG_Baro::get_singleton();
...
bool AG_Airspeed_SDP3X::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AG_Airspeed_SDP3X::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
 ...
 */
bool AG_Airspeed_SDP3X::_crc(const uint8_t data[], uint8_t size, uint8_t checksum)
{
