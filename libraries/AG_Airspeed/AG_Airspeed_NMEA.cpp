...
AG_Airspeed_NMEA::AG_Airspeed_NMEA(AG_Airspeed &_frontend, uint8_t _instance) :
    AG_Airspeed_Backend(_frontend, _instance)
{
}

bool AG_Airspeed_NMEA::init()
{
    const AG_SerialManager& serial_manager = AP::serialmanager();

    _uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_AirSpeed, 0);
    if (_uart == nullptr) {
        return false;
    }

    set_bus_id(AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_SERIAL,0,0,0));

    _uart->begin(serial_manager.find_baudrate(AG_SerialManager::SerialProtocol_AirSpeed, 0));
...
bool AG_Airspeed_NMEA::get_airspeed(float &airspeed)
{
    if (_uart == nullptr) {
        return false;
    }
...
bool AG_Airspeed_NMEA::get_temperature(float &temperature)
{
    if (_uart == nullptr) {
        return false;
    }
...
bool AG_Airspeed_NMEA::decode(char c)
{
    switch (c) {
    case ',':
...
bool AG_Airspeed_NMEA::decode_latest_term()
{
