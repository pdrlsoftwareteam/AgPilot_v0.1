...
AG_Airspeed_MS4525::AG_Airspeed_MS4525(AG_Airspeed &_frontend, uint8_t _instance) :
    AG_Airspeed_Backend(_frontend, _instance)
{
}

// probe for a sensor
bool AG_Airspeed_MS4525::probe(uint8_t bus, uint8_t address)
{
...
found_sensor:
    ...
    _dev->register_periodic_callback(20000,
                                     FUNCTOR_BIND_MEMBER(&AG_Airspeed_MS4525::_timer, void));
    return true;
}

// start a measurement
void AG_Airspeed_MS4525::_measure()
{
    _measurement_started_ms = 0;
    uint8_t cmd = 0;
    if (_dev->transfer(&cmd, 1, nullptr, 0)) {
        _measurement_started_ms = AG_HAL::millis();
    }
}
...
*/
float AG_Airspeed_MS4525::_get_pressure(int16_t dp_raw) const
{
 ...
 */
float AG_Airspeed_MS4525::_get_temperature(int16_t dT_raw) const
{
...
void AG_Airspeed_MS4525::_collect()
{
    uint8_t data[4];
    uint8_t data2[4];
    ...
    _last_sample_time_ms = AG_HAL::millis();
}
 ...
 */
void AG_Airspeed_MS4525::_voltage_correction(float &diff_press_pa, float &temperature)
{
...
// 50Hz timer
void AG_Airspeed_MS4525::_timer()
{
    if (_measurement_started_ms == 0) {
        _measure();
        return;
    }
    if ((AG_HAL::millis() - _measurement_started_ms) > 10) {
        _collect();
...
bool AG_Airspeed_MS4525::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AG_Airspeed_MS4525::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
