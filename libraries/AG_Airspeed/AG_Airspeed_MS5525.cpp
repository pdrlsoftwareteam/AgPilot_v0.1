...
AG_Airspeed_MS5525::AG_Airspeed_MS5525(AG_Airspeed &_frontend, uint8_t _instance, MS5525_ADDR address) :
...
bool AG_Airspeed_MS5525::init()
{
    ...
    dev->register_periodic_callback(1000000UL/80U,
                                    FUNCTOR_BIND_MEMBER(&AG_Airspeed_MS5525::timer, void));
    return true;
}
 ...
 */
uint16_t AG_Airspeed_MS5525::crc4_prom(void)
{
    return crc_crc4(prom);
}

bool AG_Airspeed_MS5525::read_prom(void)
{
...
/*
  read from the ADC
 */
int32_t AG_Airspeed_MS5525::read_adc()
{
    uint8_t val[3];
 ...
 */
void AG_Airspeed_MS5525::calculate(void)
{
    ...
    press_count++;
    temp_count++;
    last_sample_time_ms = AG_HAL::millis();
}

// 80Hz timer
void AG_Airspeed_MS5525::timer()
{
    if (AG_HAL::micros() - command_send_us < 10000) {
...
bool AG_Airspeed_MS5525::get_differential_pressure(float &_pressure)
{
    WITH_SEMAPHORE(sem);

    if ((AG_HAL::millis() - last_sample_time_ms) > 100) {
        return false;
    }
...
bool AG_Airspeed_MS5525::get_temperature(float &_temperature)
{
    WITH_SEMAPHORE(sem);
