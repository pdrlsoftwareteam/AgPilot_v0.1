...
void AG_Airspeed::check_sensor_failures()
{
...
void AG_Airspeed::check_sensor_ahrs_wind_max_failures(uint8_t i)
{
#ifndef HAL_BUILD_AP_PERIPH
    ...
    if (gps.status() < AG_GPS::GPS_Status::GPS_OK_FIX_3D) {
    ...
    if (is_positive(_wind_gate) && (AG_Airspeed::OptionsMask::USE_EKF_CONSISTENCY & _options) != 0) {
    ...
    if (param[i].use > 0) {
        if (((AG_Airspeed::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_DO_DISABLE & _options) != 0) &&
    ...
    } else if (((AG_Airspeed::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_RECOVERY_DO_REENABLE & _options) != 0) &&
