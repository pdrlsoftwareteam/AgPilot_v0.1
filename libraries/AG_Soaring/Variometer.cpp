...
Variometer::Variometer(const AG_FixedWing &parms, const PolarParams &polarParams) :
    _aparm(parms),
...
void Variometer::update(const float thermal_bank)
{
    ...
    float dt = (float)(AG_HAL::micros64() - _prev_update_time)/1e6;

    // Logic borrowed from AG_TECS.cpp
