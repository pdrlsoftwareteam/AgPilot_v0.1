...
// ArduSoar parameters
const AG_Param::GroupInfo SoaringController::var_info[] = {
    // @Param: ENABLE
...
SoaringController::SoaringController(AG_TECS &tecs, const AG_FixedWing &parms) :
    _tecs(tecs),
...
bool SoaringController::suppress_throttle()
{
    ...
    if (_throttle_suppressed && (alt < alt_min)) {
        ...
        _cruise_start_time_us = AG_HAL::micros64();
...
bool SoaringController::check_thermal_criteria()
{
    return (_last_update_status == ActiveStatus::AUTO_MODE_CHANGE
            && ((AG_HAL::micros64() - _cruise_start_time_us) > ((unsigned)min_cruise_s * 1e6))
...
SoaringController::LoiterStatus SoaringController::check_cruise_criteria(Vector2f prev_wp, Vector2f next_wp)
{
    ...
    } else if ((AG_HAL::micros64() - _thermal_start_time_us) > ((unsigned)min_thermal_s * 1e6)) {
...
void SoaringController::init_thermalling()
{
    ...
    _prev_update_time = AG_HAL::micros64();
    _thermal_start_time_us = AG_HAL::micros64();
...
{
    float deltaT = (AG_HAL::micros64() - _prev_update_time) * 1e-6;
