...
#include <AG_RCMapper/AG_RCMapper.h>
...
const AG_Param::GroupInfo AG_Arming::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS_FRAME("REQUIRE",     0,      AG_Arming,  require, float(Required::YES_MIN_PWM),
    ...
    // @Range: 0.25 3.0
    // @User: Advanced
    AP_GROUPINFO("ACCTHRESH",    3,     AG_Arming,  accel_error_threshold,  AP_ARMING_ACCEL_ERROR_THRESHOLD),

    // index 4 was VOLT_MIN, moved to AG_BattMonitor
    // index 5 was VOLT2_MIN, moved to AG_BattMonitor
    ...
    // @User: Advanced
    AP_GROUPINFO_FRAME("RUDDER",  6,     AG_Arming, _rudder_arming, ARMING_RUDDER_DEFAULT, AP_PARAM_FRAME_PLANE |
    ...
    // @User: Advanced
    AP_GROUPINFO("MIS_ITEMS",    7,     AG_Arming, _required_mission_items, 0),
    ...
    // @User: Standard
    AP_GROUPINFO("CHECK",        8,     AG_Arming,  checks_to_perform,       ARMING_CHECK_ALL),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 9,   AG_Arming, _arming_options, 0),
...
extern AG_IOMCU iomcu;
#endif

AG_Arming::AG_Arming()
{
    if (_singleton) {
        AG_HAL::panic("Too many AG_Arming instances");
    }
    _singleton = this;
...
uint16_t AG_Arming::compass_magfield_expected() const
{
    return AP_ARMING_COMPASS_MAGFIELD_EXPECTED;
}

bool AG_Arming::is_armed() const
{
 ...
 */
bool AG_Arming::is_armed_and_safety_off() const
{
    return is_armed() && hal.util->safety_switch_state() != AG_HAL::Util::SAFETY_DISARMED;
}

uint32_t AG_Arming::get_enabled_checks() const
{
    return checks_to_perform;
}

bool AG_Arming::check_enabled(const enum AG_Arming::ArmingChecks check) const
{
...
void AG_Arming::check_failed(const enum AG_Arming::ArmingChecks check, bool report, const char *fmt, ...) const
{
    if (!report) {
        return;
    }
...
void AG_Arming::check_failed(bool report, const char *fmt, ...) const
{
    if (!report) {
        return;
    }
...
bool AG_Arming::barometer_checks(bool report)
{
...
bool AG_Arming::airspeed_checks(bool report)
{
...
bool AG_Arming::logging_checks(bool report)
{
...
bool AG_Arming::ins_accels_consistent(const AG_InertialSensor &ins)
{
...
bool AG_Arming::ins_gyros_consistent(const AG_InertialSensor &ins)
{
...
bool AG_Arming::ins_checks(bool report)
{
...
bool AG_Arming::compass_checks(bool report)
{
...
bool AG_Arming::gps_checks(bool report)
{
    ...
    if (check_enabled(ARMING_CHECK_GPS)) {
        ...
        for (uint8_t i = 0; i < gps.num_sensors(); i++) {
            ...
            if (
#endif
                    (gps.get_type(i) == AG_GPS::GPS_Type::GPS_TYPE_NONE)) {
            ...
            //GPS OK?
            if (gps.status(i) < AG_GPS::GPS_OK_FIX_3D) {
...
bool AG_Arming::battery_checks(bool report)
{
...
bool AG_Arming::hardware_safety_check(bool report) 
{
...
bool AG_Arming::rc_arm_checks(AG_Arming::Method method)
{
    ...
    if ((last_input_ms == 0) || ((AG_HAL::millis() - last_input_ms) > 1000)) {
        return true;
    }
...
bool AG_Arming::rc_calibration_checks(bool report)
{
    bool check_passed = true;
...
bool AG_Arming::rc_in_calibration_check(bool report)
{
    if (rc().calibrating()) {
...
bool AG_Arming::manual_transmitter_checks(bool report)
{
...
bool AG_Arming::mission_checks(bool report)
{
...
bool AG_Arming::rangefinder_checks(bool report)
{
...
bool AG_Arming::servo_checks(bool report) const
{
...
#if HAL_WITH_IO_MCU
    if (!iomcu.healthy() && AG_BoardConfig::io_enabled()) {
...
bool AG_Arming::board_voltage_checks(bool report)
{
    // check board voltage
    if (check_enabled(ARMING_CHECK_VOLTAGE)) {
#if HAL_HAVE_BOARD_VOLTAGE
        const float bus_voltage =  hal.analogin->board_voltage();
        const float vbus_min = AG_BoardConfig::get_minimum_board_voltage();
       ...
       const float vservo_min = AG_BoardConfig::get_minimum_servo_voltage();
...
bool AG_Arming::heater_min_temperature_checks(bool report)
{
    if (checks_to_perform & ARMING_CHECK_ALL) {
        AG_BoardConfig *board = AP::boardConfig();
        if (board) {
            float temperature;
 ...
 */
bool AG_Arming::system_checks(bool report)
{
    char buffer[MAVLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN+1] {};
    ...
    if (check_enabled(ARMING_CHECK_SYSTEM)) {
        ...
        const AG_Terrain *terrain = AG_Terrain::get_singleton();
        ...
        const AG_Scripting *scripting = AG_Scripting::get_singleton();
...
bool AG_Arming::terrain_database_required() const
{
    AG_Mission *mission = AP::mission();
    if (mission == nullptr) {
        // no mission support?
...
bool AG_Arming::terrain_checks(bool report) const
{
    if (!check_enabled(ARMING_CHECK_PARAMETERS)) {
        return true;
    }
...
bool AG_Arming::proximity_checks(bool report) const
{
#if HAL_PROXIMITY_ENABLED
    const AG_Proximity *proximity = AP::proximity();
...
bool AG_Arming::can_checks(bool report)
{
...
// To be replaced with macro saying if KDECAN library is included
                ...
                case AG_CANManager::Driver_Type_EFI_NWPMU:
                case AG_CANManager::Driver_Type_USD1:
                case AG_CANManager::Driver_Type_None:
                case AG_CANManager::Driver_Type_Scripting:
                case AG_CANManager::Driver_Type_Scripting2:
                case AG_CANManager::Driver_Type_Benewake:
                    break;
            }
        }
    }
...
bool AG_Arming::fence_checks(bool display_failure)
{
#if AP_FENCE_ENABLED
...
bool AG_Arming::camera_checks(bool display_failure)
{
...
bool AG_Arming::osd_checks(bool display_failure) const
{
#if OSD_PARAM_ENABLED && OSD_ENABLED 
    if (check_enabled(ARMING_CHECK_CAMERA)) {
        const AG_OSD *osd = AP::osd();
        if (osd == nullptr) {
            return true;
        }
...
bool AG_Arming::mount_checks(bool display_failure) const
{
...
bool AG_Arming::fettec_checks(bool display_failure) const
{
#if AP_FETTEC_ONEWIRE_ENABLED
    const AG_FETtecOneWire *f = AG_FETtecOneWire::get_singleton();
    if (f == nullptr) {
        return true;
    }
...
bool AG_Arming::get_aux_auth_id(uint8_t& auth_id)
{
...
void AG_Arming::set_aux_auth_passed(uint8_t auth_id)
{
    WITH_SEMAPHORE(aux_auth_sem);
...
void AG_Arming::set_aux_auth_failed(uint8_t auth_id, const char* fail_msg)
{
    WITH_SEMAPHORE(aux_auth_sem);
...
bool AG_Arming::aux_auth_checks(bool display_failure)
{
    // handle error cases
    if (aux_auth_error) {
        if (aux_auth_fail_msg == nullptr) {
...
bool AG_Arming::generator_checks(bool display_failure) const
{
#if HAL_GENERATOR_ENABLED
    const AG_Generator *generator = AP::generator();
...
// OpenDroneID Checks
bool AG_Arming::opendroneid_checks(bool display_failure)
{
...
bool AG_Arming::serial_protocol_checks(bool display_failure)
{
    if (AP::serialmanager().have_serial(AG_SerialManager::SerialProtocol_RCIN, 1)) {
...
bool AG_Arming::pre_arm_checks(bool report)
{
...
bool AG_Arming::arm_checks(AG_Arming::Method method)
{
    if (check_enabled(ARMING_CHECK_RC)) {
    ...
    // note that this will prepare AG_Logger to start logging
...
bool AG_Arming::mandatory_checks(bool report)
{
    bool ret = true;
#if AP_OPENDRONEID_ENABLED
...
bool AG_Arming::arm(AG_Arming::Method method, const bool do_arming_checks)
{
    ...
    if (armed) {
        ...
        if (fence != nullptr) {
            ...
            if (fence->auto_enabled() == AG_Fence::AutoEnable::ONLY_WHEN_ARMED) {
                fence->enable(true);
...
bool AG_Arming::disarm(const AG_Arming::Method method, bool do_disarm_checks)
{
    ...
    AG_BoardConfig *board_cfg = AG_BoardConfig::get_singleton();
    if ((board_cfg != nullptr) &&
        (board_cfg->get_safety_button_options() & AG_BoardConfig::BOARD_SAFETY_OPTION_SAFETY_ON_DISARM)) {
...
AG_Arming::Required AG_Arming::arming_required() const
{
#if AP_OPENDRONEID_ENABLED
    // cannot be disabled if OpenDroneID is present
    if (AG_OpenDroneID::get_singleton() != nullptr && AP::opendroneid().enabled()) {
...
bool AG_Arming::rc_checks_copter_sub(const bool display_failure, const RC_Channel *channels[4]) const
{
...
bool AG_Arming::visodom_checks(bool display_failure) const
{
...
bool AG_Arming::disarm_switch_checks(bool display_failure) const
{
...
void AG_Arming::Log_Write_Arm(const bool forced, const AG_Arming::Method method)
{
...
void AG_Arming::Log_Write_Disarm(const bool forced, const AG_Arming::Method method)
{
    const struct log_Arm_Disarm pkt {
        LOG_PACKET_HEADER_INIT(LOG_ARM_DISARM_MSG),
        time_us                 : AG_HAL::micros64(),
...
void AG_Arming::check_forced_logging(const AG_Arming::Method method)
{
...
AG_Arming *AG_Arming::_singleton = nullptr;

/*
 * Get the AG_InertialSensor singleton
 */
AG_Arming *AG_Arming::get_singleton()
{
    return AG_Arming::_singleton;
}
...
AG_Arming &arming()
{
    return *AG_Arming::get_singleton();
}
