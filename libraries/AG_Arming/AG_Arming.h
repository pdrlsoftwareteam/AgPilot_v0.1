...
public:
    ...
    CLASS_NO_COPY(AG_Arming);  /* Do not allow copies */

    static AG_Arming *get_singleton();
    ...
    virtual bool arm(AG_Arming::Method method, bool do_arming_checks=true);
    virtual bool disarm(AG_Arming::Method method, bool do_disarm_checks=true);
    bool is_armed() const;
    ...
    virtual bool arm_checks(AG_Arming::Method method);
...
protected:
    ...
    bool rc_arm_checks(AG_Arming::Method method);
    ...
    bool check_enabled(const enum AG_Arming::ArmingChecks check) const;
    // handle the case where a check fails
    void check_failed(const enum AG_Arming::ArmingChecks check, bool report, const char *fmt, ...) const FMT_PRINTF(4, 5);
    void check_failed(bool report, const char *fmt, ...) const FMT_PRINTF(3, 4);

    void Log_Write_Arm(bool forced, AG_Arming::Method method);
    void Log_Write_Disarm(bool forced, AG_Arming::Method method);

private:

    static AG_Arming *_singleton;

    bool ins_accels_consistent(const class AG_InertialSensor &ins);
    bool ins_gyros_consistent(const class AG_InertialSensor &ins);

    // check if we should keep logging after disarming
    void check_forced_logging(const AG_Arming::Method method);
