...
void AG_Logger_W25N01GV::Init()
{
    dev = hal.spi->get_device("dataflash");
    if (!dev) {
        AG_HAL::panic("PANIC: AG_Logger W25N01GV device not found");
        return;
    }
    ...
    AG_Logger_Block::Init();
}

/*
  wait for busy flag to be cleared
 */
void AG_Logger_W25N01GV::WaitReady()
{
    if (flash_died) {
        return;
    }

    uint32_t t = AG_HAL::millis();
    while (Busy()) {
        hal.scheduler->delay_microseconds(100);
        if (AG_HAL::millis() - t > 5000) {
...
bool AG_Logger_W25N01GV::getSectorCount(void)
{
    WaitReady();
...
uint8_t AG_Logger_W25N01GV::ReadStatusRegBits(uint8_t bits)
{
    WITH_SEMAPHORE(dev_sem);
...
void AG_Logger_W25N01GV::WriteStatusReg(uint8_t reg, uint8_t bits)
{
    WaitReady();
    WITH_SEMAPHORE(dev_sem);
...
*/
void AG_Logger_W25N01GV::send_command_addr(uint8_t command, uint32_t PageAdr)
{
    uint8_t cmd[4];
    cmd[0] = command;
    cmd[1] = 0;    // dummy
...
void AG_Logger_W25N01GV::PageToBuffer(uint32_t pageNum)
{
...
void AG_Logger_W25N01GV::BufferToPage(uint32_t pageNum)
{
    if (pageNum == 0 || pageNum > df_NumPages+1) {
...
*/
void AG_Logger_W25N01GV::SectorErase(uint32_t blockNum)
{
    WriteEnable();
    WITH_SEMAPHORE(dev_sem);
...
/*
  erase one 4k sector
*/
void AG_Logger_W25N01GV::Sector4kErase(uint32_t sectorNum)
{
    SectorErase(sectorNum);
}

void AG_Logger_W25N01GV::StartErase()
{
    WriteEnable();
...
void AG_Logger_W25N01GV::WriteEnable(void)
{
    WaitReady();
    WITH_SEMAPHORE(dev_sem);
