 ...
 */
void AG_Logger_Backend::Fill_Format(const struct LogStructure *s, struct log_Format &pkt)
{
 ...
 */
void AG_Logger_Backend::Fill_Format_Units(const struct LogStructure *s, struct log_Format_Units &pkt)
{
    memset(&pkt, 0, sizeof(pkt));
    pkt.head1 = HEAD_BYTE1;
    pkt.head2 = HEAD_BYTE2;
    pkt.msgid = LOG_FORMAT_UNITS_MSG;
    pkt.time_us = AG_HAL::micros64();
 ...
 */
bool AG_Logger_Backend::Write_Format(const struct LogStructure *s)
{
 ...
 */
bool AG_Logger_Backend::Write_Unit(const struct UnitStructure *s)
{
    struct log_Unit pkt{
 ...
 */
bool AG_Logger_Backend::Write_Multiplier(const struct MultiplierStructure *s)
{
 ...
 */
bool AG_Logger_Backend::Write_Format_Units(const struct LogStructure *s)
{
 ...
 */
bool AG_Logger_Backend::Write_Parameter(const char *name, float value, float default_val)
{
...
/*
  write a parameter to the log
 */
bool AG_Logger_Backend::Write_Parameter(const AG_Param *ap,
                                            const AG_Param::ParamToken &token,
...
void AG_Logger::Write_RCIN(void)
{
    uint16_t values[16] = {};
...
void AG_Logger::Write_RCOUT(void)
{
...
void AG_Logger::Write_RSSI()
{
    AG_RSSI *rssi = AP::rssi();
    if (rssi == nullptr) {
        return;
    }
...
void AG_Logger::Write_Command(const mavlink_command_int_t &packet,
...
bool AG_Logger_Backend::Write_Mission_Cmd(const AG_Mission &mission,
                                              const AG_Mission::Mission_Command &cmd)
{
    mavlink_mission_item_int_t mav_cmd = {};
    AG_Mission::mission_cmd_to_mavlink_int(cmd,mav_cmd);
...
bool AG_Logger_Backend::Write_EntireMission()
{
...
bool AG_Logger_Backend::Write_Message(const char *message)
{
...
void AG_Logger::Write_Power(void)
{
...
void AG_Logger::Write_Radio(const mavlink_radio_t &packet)
{
...
void AG_Logger::Write_Compass_instance(const uint64_t time_us, const uint8_t mag_instance)
{
...
// Write a mode packet.
bool AG_Logger_Backend::Write_Mode(uint8_t mode, const ModeReason reason)
{
 ...
 */
void AG_Logger::Write_ServoStatus(uint64_t time_us, uint8_t id, float position, float force, float speed, uint8_t power_pct,
...
void AG_Logger::Write_PID(uint8_t msg_type, const AG_PIDInfo &info)
{
...
void AG_Logger::Write_SRTL(bool active, uint16_t num_points, uint16_t max_points, uint8_t action, const Vector3f& breadcrumb)
{
...
void AG_Logger::Write_Winch(bool healthy, bool thread_end, bool moving, bool clutch, uint8_t mode, float desired_length, float length, float desired_rate, uint16_t tension, float voltage, int8_t temp)
{
...
void AG_Logger::Write_PSCx(LogMessages id, float pos_target, float pos, float vel_desired, float vel_target, float vel, float accel_desired, float accel_target, float accel)
{
...
void AG_Logger::Write_PSCN(float pos_target, float pos, float vel_desired, float vel_target, float vel, float accel_desired, float accel_target, float accel)
{
    Write_PSCx(LOG_PSCN_MSG, pos_target, pos, vel_desired, vel_target, vel, accel_desired, accel_target, accel);
}

void AG_Logger::Write_PSCE(float pos_target, float pos, float vel_desired, float vel_target, float vel, float accel_desired, float accel_target, float accel)
{
    Write_PSCx(LOG_PSCE_MSG, pos_target, pos, vel_desired, vel_target, vel, accel_desired, accel_target, accel);
}

void AG_Logger::Write_PSCD(float pos_target, float pos, float vel_desired, float vel_target, float vel, float accel_desired, float accel_target, float accel)
{
