/* 
   AG_Logger logging - file oriented variant
...
/*
  constructor
 */
AG_Logger_File::AG_Logger_File(AG_Logger &front,
                               LoggerMessageWriter_DFLogStart *writer) :
    AG_Logger_Backend(front, writer),
...
void AG_Logger_File::ensure_log_directory_exists()
{
    int ret;
    struct stat st;
...
void AG_Logger_File::Init()
{
    ...
    DEV_PRINTF("AG_Logger_File: buffer size=%u\n", (unsigned)bufsize);
...
bool AG_Logger_File::file_exists(const char *filename) const
{
    struct stat st;
    EXPECT_DELAY_MS(3000);
...
bool AG_Logger_File::log_exists(const uint16_t lognum) const
{
...
void AG_Logger_File::periodic_1Hz()
{
    AG_Logger_Backend::periodic_1Hz();
    ...
    if (!io_thread_alive()) {
        if (io_thread_warning_decimation_counter == 0 && _initialised) {
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "AG_Logger: stuck thread (%s)", last_io_operation);
        }
    ...
    if (rate_limiter == nullptr &&
        ...
        rate_limiter = new AG_Logger_RateLimiter(_front, _front._params.file_ratemax, _front._params.disarm_ratemax);
    }
}

void AG_Logger_File::periodic_fullrate()
{
    AG_Logger_Backend::push_log_blocks();
}

uint32_t AG_Logger_File::bufferspace_available()
{
...
bool AG_Logger_File::recent_open_error(void) const
{
    if (_open_error_ms == 0) {
        return false;
    }
    return AG_HAL::millis() - _open_error_ms < LOGGER_FILE_REOPEN_MS;
}

// return true for CardInserted() if we successfully initialized
bool AG_Logger_File::CardInserted(void) const
{
...
// returns -1 on error
int64_t AG_Logger_File::disk_space_avail()
{
...
// returns -1 on error
int64_t AG_Logger_File::disk_space()
{
 ...
 */
bool AG_Logger_File::dirent_to_log_num(const dirent *de, uint16_t &log_num) const
{
...
uint16_t AG_Logger_File::find_oldest_log()
{
...
void AG_Logger_File::Prep_MinSpace()
{
    ...
    do {
        ...
        if (count++ > MAX_LOG_FILES+10) {
            ...
            INTERNAL_ERROR(AG_InternalError::error_t::logger_too_many_deletions);
            break;
        }
        char *filename_to_remove = _log_file_name(log_to_remove);
        if (filename_to_remove == nullptr) {
            INTERNAL_ERROR(AG_InternalError::error_t::logger_bad_getfilename);
            break;
        }
 ...
 */
char *AG_Logger_File::_log_file_name_short(const uint16_t log_num) const
{
    char *buf = nullptr;
...
/*
  ...
  Note: Caller must free.
 */
char *AG_Logger_File::_log_file_name_long(const uint16_t log_num) const
{
    char *buf = nullptr;
...
/*
  ...
  Note: Caller must free.
 */
char *AG_Logger_File::_log_file_name(const uint16_t log_num) const
{
...
/*
  ...
  Note: Caller must free.
 */
char *AG_Logger_File::_lastlog_file_name(void) const
{
    char *buf = nullptr;
...
bool AG_Logger_File::WritesOK() const
{
    if (_write_fd == -1) {
        return false;
    }
...
bool AG_Logger_File::StartNewLogOK() const
{
    if (recent_open_error()) {
        return false;
    }
...
#endif
    return AG_Logger_Backend::StartNewLogOK();
}

/* Write a block of data at current offset */
bool AG_Logger_File::_WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical)
{
 ...
 */
uint16_t AG_Logger_File::find_last_log()
{
    unsigned ret = 0;
...
uint32_t AG_Logger_File::_get_log_size(const uint16_t log_num)
{
...
uint32_t AG_Logger_File::_get_log_time(const uint16_t log_num)
{
    char *fname = _log_file_name(log_num);
    if (fname == nullptr) {
        return 0;
 ...
 */
void AG_Logger_File::get_log_boundaries(const uint16_t list_entry, uint32_t & start_page, uint32_t & end_page)
{
 ...
 */
int16_t AG_Logger_File::get_log_data(const uint16_t list_entry, const uint16_t page, const uint32_t offset, const uint16_t len, uint8_t *data)
{
 ...
 */
void AG_Logger_File::get_log_info(const uint16_t list_entry, uint32_t &size, uint32_t &time_utc)
{
 ...
 */
uint16_t AG_Logger_File::get_num_logs()
{
    auto *d = AP::FS().opendir(_log_directory);
    if (d == nullptr) {
        return 0;
...
/*
  stop logging
 */
void AG_Logger_File::stop_logging(void)
{
 ...
 */
void AG_Logger_File::PrepForArming_start_logging()
{
    if (logging_started()) {
        return;
    }
    ...
    while (AG_HAL::millis() - start_ms < open_limit_ms) {
        if (logging_started()) {
            break;
        }
 ...
 */
void AG_Logger_File::start_new_log(void)
{
    if (recent_open_error()) {
    ...
    last_log_is_marked_discard = _front._params.log_disarmed == AG_Logger::LogDisarmed::LOG_WHILE_DISARMED_DISCARD;
 ...
 */
bool AG_Logger_File::write_lastlog_file(uint16_t log_num)
{
    // now update lastlog.txt with the new log number
    char *fname = _lastlog_file_name();
...
void AG_Logger_File::flush(void)
#if APM_BUILD_TYPE(APM_BUILD_Replay)
{
    ...
    if (write_fd_semaphore.take(1)) {
        ...
        INTERNAL_ERROR(AG_InternalError::error_t::logger_flushing_without_sem);
    }
}
#else
{
...
void AG_Logger_File::io_timer(void)
{
    uint32_t tnow = AG_HAL::millis();
    ...
    if (tnow - _free_space_last_check_time > _free_space_check_interval) {
        ...
        if (disk_space_avail() < _free_space_min_avail && disk_space() > 0) {
            DEV_PRINTF("Out of space for logging\n");
            stop_logging();
            _open_error_ms = AG_HAL::millis(); // prevent logging starting again for 5s
            last_io_operation = "";
            return;
        }
        last_io_operation = "";
...
bool AG_Logger_File::io_thread_alive() const
{
...
#endif
    return (AG_HAL::millis() - _io_timer_heartbeat) < timeout_ms;
}

bool AG_Logger_File::logging_failed() const
{
    if (!_initialised) {
        return true;
    }
 ...
 */
void AG_Logger_File::erase_next(void)
{
