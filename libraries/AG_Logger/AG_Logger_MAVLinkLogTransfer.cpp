 ...
 */
void AG_Logger::handle_log_message(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
    if (!WritesEnabled()) {
    ...
    _last_mavlink_log_transfer_message_handled_ms = AG_HAL::millis();
    switch (msg.msgid) {
...
/**
   handle all types of log download requests from the GCS
 */
void AG_Logger::handle_log_request_list(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
 ...
 */
void AG_Logger::handle_log_request_data(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
    WITH_SEMAPHORE(_log_send_sem);
 ...
 */
void AG_Logger::handle_log_request_erase(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
 ...
 */
void AG_Logger::handle_log_request_end(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
    WITH_SEMAPHORE(_log_send_sem);
 ...
 */
void AG_Logger::handle_log_send()
{
    WITH_SEMAPHORE(_log_send_sem);
...
void AG_Logger::handle_log_sending()
{
    WITH_SEMAPHORE(_log_send_sem);
...
/**
   trigger sending of log messages if there are some pending
 */
void AG_Logger::handle_log_send_listing()
{
    WITH_SEMAPHORE(_log_send_sem);
    ...
    if (AG_HAL::millis() - _log_sending_link->get_last_heartbeat_time() > 3000) {
 ...
 */
bool AG_Logger::handle_log_send_data()
{
    WITH_SEMAPHORE(_log_send_sem);
