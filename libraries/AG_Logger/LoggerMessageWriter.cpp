...
void LoggerMessageWriter_DFLogStart::reset()
{
    ...
    ap = AG_Param::first(&token, &type, &param_default);
}
...
bool LoggerMessageWriter_DFLogStart::check_process_limit(uint32_t start_us)
{
    ...
    if (AG_HAL::micros() - start_us > limit_us) {
        return true;
    }
    return false;
}
...
void LoggerMessageWriter_DFLogStart::process()
{
    ...
    const uint32_t start_us = AG_HAL::micros();
    ...
    case Stage::PARMS: {
        while (ap) {
            ...
            param_default = AP::logger().quiet_nanf();
            ap = AG_Param::next_scalar(&token, &type, &param_default);
            if (check_process_limit(start_us)) {
                return; // call me again!
            }
        }
...
void LoggerMessageWriter_WriteSysInfo::process() {
    ...
    case Stage::PARAM_SPACE_USED:
        if (! _logger_backend->Write_MessageF("Param space used: %u/%u", AG_Param::storage_used(), AG_Param::storage_size())) {
            return; // call me again
        }
...
{
    const AG_Rally *_rally = AP::rally();
    if (_rally == nullptr) {
        _finished = true;
        return;
    }
...
void LoggerMessageWriter_WriteEntireMission::process() {
    const AG_Mission *_mission = AP::mission();
    if (_mission == nullptr) {
        _finished = true;
        return;
    }
