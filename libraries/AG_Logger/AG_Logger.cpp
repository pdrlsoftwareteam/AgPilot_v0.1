...
#include "AG_Logger_Backend.h"

#include "AG_Logger_File.h"
#include "AG_Logger_DataFlash.h"
#include "AG_Logger_W25N01GV.h"
#include "AG_Logger_MAVLink.h"
...
AG_Logger *AG_Logger::_singleton;
...
#define HAL_LOGGING_DATAFLASH_DRIVER AG_Logger_DataFlash
#endif
...
const AG_Param::GroupInfo AG_Logger::var_info[] = {
    // @Param: _BACKEND_TYPE
    // @DisplayName: AG_Logger Backend Storage type
    ...
    // @User: Standard
    AP_GROUPINFO("_BACKEND_TYPE",  0, AG_Logger, _params.backend_types,       uint8_t(HAL_LOGGING_BACKENDS_DEFAULT)),

    // @Param: _FILE_BUFSIZE
    // @DisplayName: Maximum AG_Logger File and Block Backend buffer size (in kilobytes)
    ...
    // @User: Standard
    AP_GROUPINFO("_FILE_BUFSIZE",  1, AG_Logger, _params.file_bufsize,       HAL_LOGGING_FILE_BUFSIZE),
    ...
    // @User: Standard
    AP_GROUPINFO("_DISARMED",  2, AG_Logger, _params.log_disarmed,       0),
    ...
    // @User: Standard
    AP_GROUPINFO("_REPLAY",  3, AG_Logger, _params.log_replay,       0),
    ...
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    AP_GROUPINFO("_FILE_DSRMROT",  4, AG_Logger, _params.file_disarm_rot,       0),

#if HAL_LOGGING_MAVLINK_ENABLED
    // @Param: _MAV_BUFSIZE
    // @DisplayName: Maximum AG_Logger MAVLink Backend buffer size
    // @Description: Maximum amount of memory to allocate to AG_Logger-over-mavlink
    // @User: Advanced
    // @Units: kB
    AP_GROUPINFO("_MAV_BUFSIZE",  5, AG_Logger, _params.mav_bufsize,       HAL_LOGGING_MAV_BUFSIZE),
#endif
    ...
    // @User: Standard
    // @Units: s
    AP_GROUPINFO("_FILE_TIMEOUT",  6, AG_Logger, _params.file_timeout,     HAL_LOGGING_FILE_TIMEOUT),
    ...
    // @Range: 10 1000
    // @User: Standard
    AP_GROUPINFO("_FILE_MB_FREE",  7, AG_Logger, _params.min_MB_free, 500),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("_FILE_RATEMAX",  8, AG_Logger, _params.file_ratemax, 0),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("_MAV_RATEMAX",  9, AG_Logger, _params.mav_ratemax, 0),
#endif
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("_BLK_RATEMAX", 10, AG_Logger, _params.blk_ratemax, 0),
#endif
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("_DARM_RATEMAX",  11, AG_Logger, _params.disarm_ratemax, 0),
...
AG_Logger::AG_Logger(const AP_Int32 &log_bitmask)
    : _log_bitmask(log_bitmask)
{
    AG_Param::setup_object_defaults(this, var_info);
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_Logger must be singleton");
    }

    _singleton = this;
}

void AG_Logger::Init(const struct LogStructure *structures, uint8_t num_types)
{
    ...
    static const struct {
        Backend_Type type;
        AG_Logger_Backend* (*probe_fn)(AG_Logger&, LoggerMessageWriter_DFLogStart*);
    } backend_configs[] {
#if HAL_LOGGING_FILESYSTEM_ENABLED
        { Backend_Type::FILESYSTEM, AG_Logger_File::probe },
#endif
#if HAL_LOGGING_DATAFLASH_ENABLED
        { Backend_Type::BLOCK, HAL_LOGGING_DATAFLASH_DRIVER::probe },
#endif
#if HAL_LOGGING_MAVLINK_ENABLED
        { Backend_Type::MAVLINK, AG_Logger_MAVLink::probe },
#endif
};
...
const char* AG_Logger::unit_name(const uint8_t unit_id)
{
...
double AG_Logger::multiplier_name(const uint8_t multiplier_id)
{
...
void AG_Logger::dump_structure_field(const struct LogStructure *logstructure, const char *label, const uint8_t fieldnum)
{
...
void AG_Logger::dump_structures(const struct LogStructure *logstructures, const uint8_t num_types)
{
...
bool AG_Logger::labels_string_is_good(const char *labels) const
{
    bool passed = true;
...
bool AG_Logger::validate_structure(const struct LogStructure *logstructure, const int16_t offset)
{
    bool passed = true;
...
void AG_Logger::validate_structures(const struct LogStructure *logstructures, const uint8_t num_types)
{
    ...
    if (!passed) {
        AG_BoardConfig::config_error("See console: Log structures invalid");
    }
}

#endif // CONFIG_HAL_BOARD == HAL_BOARD_SITL

const struct LogStructure *AG_Logger::structure(uint16_t num) const
{
    return &_structures[num];
}

bool AG_Logger::logging_present() const
{
    return _next_backend != 0;
}
bool AG_Logger::logging_enabled() const
{
    if (_next_backend == 0) {
        return false;
    }
...
bool AG_Logger::logging_failed() const
{
    if (_next_backend < 1) {
...
void AG_Logger::Write_MessageF(const char *fmt, ...)
{
...
void AG_Logger::backend_starting_new_log(const AG_Logger_Backend *backend)
{
    _log_start_count++;
...
bool AG_Logger::should_log(const uint32_t mask) const
{
 ...
 */
bool AG_Logger::in_log_download() const
{
...
const struct UnitStructure *AG_Logger::unit(uint16_t num) const
{
    return &_units[num];
}

const struct MultiplierStructure *AG_Logger::multiplier(uint16_t num) const
{
...
void AG_Logger::PrepForArming()
{
    FOR_EACH_BACKEND(PrepForArming());
}

void AG_Logger::setVehicle_Startup_Writer(vehicle_startup_message_Writer writer)
{
    _vehicle_messages = writer;
}

void AG_Logger::set_vehicle_armed(const bool armed_state)
{
...
/*
  ...
  replay
*/
void AG_Logger::save_format_Replay(const void *pBuffer)
{
...
void AG_Logger::WriteBlock(const void *pBuffer, uint16_t size) {
#if APM_BUILD_TYPE(APM_BUILD_Replay)
...
bool AG_Logger::WriteBlock_first_succeed(const void *pBuffer, uint16_t size) 
{
    if (_next_backend == 0) {
        return false;
    }
...
bool AG_Logger::WriteReplayBlock(uint8_t msg_id, const void *pBuffer, uint16_t size) {
    bool ret = true;
    if (log_replay()) {
        uint8_t buf[3+size];
        buf[0] = HEAD_BYTE1;
    ...
    if (!ret && log_while_disarmed()) {
        AG_HAL::panic("Failed to log replay block");
    }
#endif
    return ret;
}

void AG_Logger::WriteCriticalBlock(const void *pBuffer, uint16_t size) {
    FOR_EACH_BACKEND(WriteCriticalBlock(pBuffer, size));
}

void AG_Logger::WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical) {
    FOR_EACH_BACKEND(WritePrioritisedBlock(pBuffer, size, is_critical));
}

// change me to "DoTimeConsumingPreparations"?
void AG_Logger::EraseAll() {
    FOR_EACH_BACKEND(EraseAll());
}
// change me to "LoggingAvailable"?
bool AG_Logger::CardInserted(void) {
    for (uint8_t i=0; i< _next_backend; i++) {
...
void AG_Logger::StopLogging()
{
    FOR_EACH_BACKEND(stop_logging());
}

uint16_t AG_Logger::find_last_log() const {
    if (_next_backend == 0) {
        return 0;
    }
    return backends[0]->find_last_log();
}
void AG_Logger::get_log_boundaries(uint16_t log_num, uint32_t & start_page, uint32_t & end_page) {
    if (_next_backend == 0) {
        return;
    }
    backends[0]->get_log_boundaries(log_num, start_page, end_page);
}
void AG_Logger::get_log_info(uint16_t log_num, uint32_t &size, uint32_t &time_utc) {
    if (_next_backend == 0) {
        return;
    }
    backends[0]->get_log_info(log_num, size, time_utc);
}
int16_t AG_Logger::get_log_data(uint16_t log_num, uint16_t page, uint32_t offset, uint16_t len, uint8_t *data) {
    if (_next_backend == 0) {
        return 0;
    }
    return backends[0]->get_log_data(log_num, page, offset, len, data);
}
uint16_t AG_Logger::get_num_logs(void) {
    if (_next_backend == 0) {
        return 0;
    }
...
bool AG_Logger::logging_started(void) {
    for (uint8_t i=0; i< _next_backend; i++) {
...
void AG_Logger::handle_mavlink_msg(GCS_MAVLINK &link, const mavlink_message_t &msg)
{
    switch (msg.msgid) {
...
void AG_Logger::periodic_tasks() {
    ...
    // currently only AG_Logger_File support this:
void AG_Logger::flush(void) {
     FOR_EACH_BACKEND(flush());
}
#endif


void AG_Logger::Write_EntireMission()
{
    FOR_EACH_BACKEND(Write_EntireMission());
}

void AG_Logger::Write_Message(const char *message)
{
    FOR_EACH_BACKEND(Write_Message(message));
}

void AG_Logger::Write_Mode(uint8_t mode, const ModeReason reason)
{
    FOR_EACH_BACKEND(Write_Mode(mode, reason));
}

void AG_Logger::Write_Parameter(const char *name, float value)
{
    FOR_EACH_BACKEND(Write_Parameter(name, value, quiet_nanf()));
}

void AG_Logger::Write_Mission_Cmd(const AG_Mission &mission,
                                            const AG_Mission::Mission_Command &cmd)
{
    FOR_EACH_BACKEND(Write_Mission_Cmd(mission, cmd));
}

#if HAL_RALLY_ENABLED
void AG_Logger::Write_RallyPoint(uint8_t total,
                                 uint8_t sequence,
...
void AG_Logger::Safe_Write_Emit_FMT(log_write_fmt *f)
{
    for (uint8_t i=0; i<_next_backend; i++) {
        if (!(f->sent_mask & (1U<<i))) {
...
uint32_t AG_Logger::num_dropped() const
{
    if (_next_backend == 0) {
        return 0;
    }
...
/* Write support */
void AG_Logger::Write(const char *name, const char *labels, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::Write(const char *name, const char *labels, const char *units, const char *mults, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::WriteStreaming(const char *name, const char *labels, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::WriteStreaming(const char *name, const char *labels, const char *units, const char *mults, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::WriteCritical(const char *name, const char *labels, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::WriteCritical(const char *name, const char *labels, const char *units, const char *mults, const char *fmt, ...)
{
    va_list arg_list;
...
void AG_Logger::WriteV(const char *name, const char *labels, const char *units, const char *mults, const char *fmt, va_list arg_list,
...
#if !APM_BUILD_TYPE(APM_BUILD_Replay)
        INTERNAL_ERROR(AG_InternalError::error_t::logger_mapfailure);
#endif
        return;
    }
 ...
 */
bool AG_Logger::allow_start_ekf() const
{
...
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
bool AG_Logger::assert_same_fmt_for_name(const AG_Logger::log_write_fmt *f,
                                               const char *name,
                                               const char *labels,
                                               const char *units,
                                               const char *mults,
                                               const char *fmt) const
{
    ...
    if (!passed) {
        AG_BoardConfig::config_error("See console: Format definition must be consistent for every call of Write");
    }
#endif
    return passed;
}
...
AG_Logger::log_write_fmt *AG_Logger::msg_fmt_for_name(const char *name, const char *labels, const char *units, const char *mults, const char *fmt, const bool direct_comp, const bool copy_strings)
{
    ...
    if (!validate_structure(&ls, (int16_t)-1)) {
        AG_BoardConfig::config_error("See console: Log structure invalid");
    }
#endif
...
const struct LogStructure *AG_Logger::structure_for_msg_type(const uint8_t msg_type) const
{
...
const struct AG_Logger::log_write_fmt *AG_Logger::log_write_fmt_for_msg_type(const uint8_t msg_type) const
{
    struct log_write_fmt *f;
    for (f = log_write_fmts; f; f=f->next) {
...
bool AG_Logger::msg_type_in_use(const uint8_t msg_type) const
{
...
int16_t AG_Logger::find_free_msg_type() const
{
 ...
 */
bool AG_Logger::fill_log_write_logstructure(struct LogStructure &logstruct, const uint8_t msg_type) const
{
...
int16_t AG_Logger::Write_calc_msg_len(const char *fmt) const
{
    ...
    for (uint8_t i=0; i<strlen(fmt); i++) {
        ...
        default:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("Unknown format specifier (%c)", fmt[i]);
#endif
            return -1;
        }
    }
 ...
 */
bool AG_Logger::check_crash_dump_save(void)
{
...
void AG_Logger::io_thread(void)
{
    uint32_t last_run_us = AG_HAL::micros();
...
void AG_Logger::start_io_thread(void)
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_Logger::io_thread, void), "log_io", HAL_LOGGING_STACK_SIZE, AG_HAL::Scheduler::PRIORITY_IO, 1)) {
        AG_HAL::panic("Failed to start Logger IO thread");
    }
...
void AG_Logger::Write_Event(LogEvent id)
{
...
void AG_Logger::Write_Error(LogErrorSubsystem sub_system,
...
void AG_Logger::createGeoFenceBreachLogFile()
{
...
void AG_Logger::sendGeoFenceAndTimeBreachToLog()
{
}
 ...
 */
bool AG_Logger::in_log_persistance(void) const
{
 ...
 */
bool AG_Logger::log_while_disarmed(void) const
{
...
#if HAL_LOGGER_FILE_CONTENTS_ENABLED
void AG_Logger::prepare_at_arming_sys_file_logging()
{
...
void AG_Logger::FileContent::reset()
{
    WITH_SEMAPHORE(sem);
...
void AG_Logger::FileContent::remove_and_free(file_list *victim)
{
    WITH_SEMAPHORE(sem);
 ...
 */
void AG_Logger::log_file_content(const char *filename)
{
    log_file_content(normal_file_content, filename);
}

void AG_Logger::log_file_content(FileContent &file_content, const char *filename)
{
 ...
 */
void AG_Logger::file_content_update(void)
{
...
void AG_Logger::file_content_update(FileContent &file_content)
{
...
AG_Logger &logger()
{
    return *AG_Logger::get_singleton();
}
