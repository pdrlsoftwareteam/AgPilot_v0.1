/* 
   AG_Logger Remote(via MAVLink) logging
*/
...
bool AG_Logger_MAVLink::logging_failed() const
{
    return !_sending_to_client;
}

uint32_t AG_Logger_MAVLink::bufferspace_available() {
    return (_blockcount_free * 200 + remaining_space_in_current_block());
}

uint8_t AG_Logger_MAVLink::remaining_space_in_current_block() const {
    // note that _current_block *could* be NULL ATM.
    return (MAVLINK_MSG_REMOTE_LOG_DATA_BLOCK_FIELD_DATA_LEN - _latest_block_len);
}

void AG_Logger_MAVLink::enqueue_block(dm_block_queue_t &queue, struct dm_block *block)
{
...
struct AG_Logger_MAVLink::dm_block *AG_Logger_MAVLink::dequeue_seqno(AG_Logger_MAVLink::dm_block_queue_t &queue, uint32_t seqno)
{
...
bool AG_Logger_MAVLink::free_seqno_from_queue(uint32_t seqno, dm_block_queue_t &queue)
{
...
bool AG_Logger_MAVLink::WritesOK() const
{
...
bool AG_Logger_MAVLink::_WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical)
{
    ...
    while (copied < size) {
        if (_current_block == nullptr) {
            ...
            if (_current_block == nullptr) {
                ...
                INTERNAL_ERROR(AG_InternalError::error_t::logger_bad_current_block);
                semaphore.give();
                return false;
            }
        }
...
//Get a free block
struct AG_Logger_MAVLink::dm_block *AG_Logger_MAVLink::next_block()
{
    AG_Logger_MAVLink::dm_block *ret = _blocks_free;
    if (ret != nullptr) {
...
void AG_Logger_MAVLink::free_all_blocks()
{
...
void AG_Logger_MAVLink::stop_logging()
{
    if (_sending_to_client) {
        _sending_to_client = false;
        _last_response_time = AG_HAL::millis();
    }
}

void AG_Logger_MAVLink::handle_ack(const GCS_MAVLINK &link,
...
void AG_Logger_MAVLink::remote_log_block_status_msg(const GCS_MAVLINK &link,
                                                    const mavlink_message_t& msg)
{
...
void AG_Logger_MAVLink::handle_retry(uint32_t seqno)
{
...
void AG_Logger_MAVLink::stats_init() {
    _dropped = 0;
    stats.resends = 0;
    stats_reset();
}
void AG_Logger_MAVLink::stats_reset() {
    stats.state_free = 0;
...
void AG_Logger_MAVLink::Write_logger_MAV(AG_Logger_MAVLink &logger_mav)
{
...
void AG_Logger_MAVLink::stats_log()
{
    if (!_initialised) {
        return;
    }
...
uint8_t AG_Logger_MAVLink::stack_size(struct dm_block *stack)
{
    uint8_t ret = 0;
...
uint8_t AG_Logger_MAVLink::queue_size(dm_block_queue_t queue)
{
    return stack_size(queue.oldest);
}

void AG_Logger_MAVLink::stats_collect()
{
    if (!_initialised) {
        return;
    }
    ...
    if (sfree != _blockcount_free) {
        INTERNAL_ERROR(AG_InternalError::error_t::logger_blockcount_mismatch);
    }
    semaphore.give();
...
*/
bool AG_Logger_MAVLink::send_log_blocks_from_queue(dm_block_queue_t &queue)
{
    ...
    while (queue.oldest != nullptr) {
        ...
        queue.sent_count++;
        struct AG_Logger_MAVLink::dm_block *tmp = dequeue_seqno(queue,queue.oldest->seqno);
        if (tmp != nullptr) { // should never be nullptr
            enqueue_block(_blocks_sent, tmp);
        } else {
            INTERNAL_ERROR(AG_InternalError::error_t::logger_dequeue_failure);
        }
    }
    return true;
}

void AG_Logger_MAVLink::push_log_blocks()
{
    if (!_initialised || !_sending_to_client) {
        return;
    }

    AG_Logger_Backend::WriteMoreStartupMessages();
...
void AG_Logger_MAVLink::do_resends(uint32_t now)
{
    if (!_initialised || !_sending_to_client) {
        return;
    }
...
// appropriately!
void AG_Logger_MAVLink::periodic_10Hz(const uint32_t now)
{
    do_resends(now);
    stats_collect();
}
void AG_Logger_MAVLink::periodic_1Hz()
{
    if (rate_limiter == nullptr &&
        ...
        rate_limiter = new AG_Logger_RateLimiter(_front, _front._params.mav_ratemax, _front._params.disarm_ratemax);
    }
...
bool AG_Logger_MAVLink::send_log_block(struct dm_block &block)
{
    if (!_initialised) {
       return false;
    }
