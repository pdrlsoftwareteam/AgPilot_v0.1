...
class AG_Logger_RateLimiter
{
public:
    AG_Logger_RateLimiter(const AG_Logger &_front, const AP_Float &_limit_hz, const AP_Float &_disarm_limit_hz);
...
public:
    ...
    AG_Logger_Backend(AG_Logger &front,
     ...
     * immediately - e.g. AG_Logger_MAVLink might get another start
   ...
   virtual void periodic_tasks();
    ...
    bool Write_FencePoint(uint8_t total, uint8_t sequence, const AG_PolyFenceItem &fence_point);
    bool Write_Fence();
#endif
    ...
    bool Write_Mission_Cmd(const AG_Mission &mission,
    ...
    bool Write_Parameter(const AG_Param *ap,
...
protected:
    ...
    */
    ...
    AG_Logger_RateLimiter *rate_limiter;
...
private:
    ...
    void Write_AG_Logger_Stats_File(const struct df_stats &_stats);
