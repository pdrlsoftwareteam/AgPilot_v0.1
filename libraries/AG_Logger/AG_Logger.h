/* ************************************************************ */
/* Test for AG_Logger Log library                               */
/* ************************************************************ */
#pragma once

#include "AG_Logger_config.h"
...
#include <AG_Vehicle/ModeReason.h>
...
class AG_Logger
{
    friend class AG_Logger_Backend; // for _num_types
    friend class AG_Logger_RateLimiter;

public:
    FUNCTOR_TYPEDEF(vehicle_startup_message_Writer, void);

    AG_Logger(const AP_Int32 &log_bitmask);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_Logger);

    // get singleton instance
    static AG_Logger *get_singleton(void) {
        return _singleton;
    }
    ...
    void Write_Mission_Cmd(const AG_Mission &mission,
    ...
    void Write_PID(uint8_t msg_type, const class AG_PIDInfo &info);
    ...
    void arming_failure() {
        _last_arming_failure_ms = AG_HAL::millis();
...
private:
    ...
    AG_Logger_Backend *backends[LOGGER_MAX_BACKENDS];
    ...
    void backend_starting_new_log(const AG_Logger_Backend *backend);

    static AG_Logger *_singleton;
