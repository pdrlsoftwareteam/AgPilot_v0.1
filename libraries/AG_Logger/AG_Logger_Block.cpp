...
AG_Logger_Block::AG_Logger_Block(AG_Logger &front, LoggerMessageWriter_DFLogStart *writer) :
...
void AG_Logger_Block::Init(void)
{
    // buffer is used for both reads and writes so access must always be within the semaphore
    buffer = (uint8_t *)hal.util->malloc_type(df_PageSize, AG_HAL::Util::MEM_DMA_SAFE);
    if (buffer == nullptr) {
        AG_HAL::panic("Out of DMA memory for logging");
    }
    ...
    if (CardInserted()) {
        ...
        while (!writebuf.set_size(bufsize) && bufsize >= df_PageSize * df_PagePerBlock) {
            DEV_PRINTF("AG_Logger_Block: Couldn't set buffer size to=%u\n", (unsigned)bufsize);
            bufsize >>= 1;
        }
        ...
        DEV_PRINTF("AG_Logger_Block: buffer size=%u\n", (unsigned)bufsize);
        _initialised = true;
    }
...
uint32_t AG_Logger_Block::bufferspace_available()
{
    // because AG_Logger_Block devices are ring buffers, we *always*
    // have room...
    return df_NumPages * df_PageSize;
}

// *** LOGGER PUBLIC FUNCTIONS ***
void AG_Logger_Block::StartWrite(uint32_t PageAdr)
{
    df_PageAdr    = PageAdr;
}

void AG_Logger_Block::FinishWrite(void)
{
...
bool AG_Logger_Block::WritesOK() const
{
...
bool AG_Logger_Block::_WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical)
{
...
uint16_t AG_Logger_Block::StartRead(uint32_t PageAdr)
{
...
uint16_t AG_Logger_Block::ReadHeaders()
{
...
bool AG_Logger_Block::ReadBlock(void *pBuffer, uint16_t size)
{
    if (erase_started) {
        return false;
    }
...
void AG_Logger_Block::StartLogFile(uint16_t FileNumber)
{
...
uint16_t AG_Logger_Block::GetFileNumber() const
{
    return df_FileNumber;
}

void AG_Logger_Block::EraseAll()
{
...
void AG_Logger_Block::periodic_1Hz()
{
    ...
    if (rate_limiter == nullptr &&
        ...
        rate_limiter = new AG_Logger_RateLimiter(_front, _front._params.blk_ratemax, _front._params.disarm_ratemax);
    }
    ...
    if (!io_thread_alive()) {
        if (warning_decimation_counter == 0 && _initialised) {
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "AG_Logger: IO thread died");
        }
...
void AG_Logger_Block::periodic_10Hz(const uint32_t now)
{
 ...
 */
bool AG_Logger_Block::NeedErase(void)
{
    uint32_t version = 0;
 ...
 */
void AG_Logger_Block::validate_log_structure()
{
    WITH_SEMAPHORE(sem);
 ...
 */
int16_t AG_Logger_Block::get_log_data_raw(uint16_t log_num, uint32_t page, uint32_t offset, uint16_t len, uint8_t *data)
{
    WITH_SEMAPHORE(sem);
 ...
 */
int16_t AG_Logger_Block::get_log_data(uint16_t list_entry, uint16_t page, uint32_t offset, uint16_t len, uint8_t *data)
{
...
// This function determines the number of whole log files in the AG_Logger
// partial logs are rejected as without the headers they are relatively useless
uint16_t AG_Logger_Block::get_num_logs(void)
{
    WITH_SEMAPHORE(sem);
    uint32_t lastpage;
    uint32_t last;
...
void AG_Logger_Block::stop_logging(void)
{
    WITH_SEMAPHORE(sem);
...
void AG_Logger_Block::stop_logging_async(void)
{
    stop_log_pending = true;
}

// This function starts a new log file in the AG_Logger
// no actual data should be written to the storage here
// that should all be handled by the IO thread
void AG_Logger_Block::start_new_log(void)
{
    if (erase_started) {
        // already erasing
        return;
...
// The first page may be greater than the last page if the AG_Logger has been filled and partially overwritten.
void AG_Logger_Block::get_log_boundaries(uint16_t list_entry, uint32_t & start_page, uint32_t & end_page)
{
    const uint16_t log_num = log_num_from_list_entry(list_entry);
    if (log_num == 0) {
        // that failed - probably no logs
...
bool AG_Logger_Block::is_wrapped(void)
{
...
uint16_t AG_Logger_Block::find_last_log(void)
{
    WITH_SEMAPHORE(sem);
    uint32_t last_page = find_last_page();
...
uint32_t AG_Logger_Block::find_last_page(void)
{
    uint32_t look;
    uint32_t bottom = 1;
...
uint32_t AG_Logger_Block::find_last_page_of_log(uint16_t log_number)
{
    uint32_t look;
    uint32_t bottom;
    uint32_t top;
...
void AG_Logger_Block::get_log_info(uint16_t list_entry, uint32_t &size, uint32_t &time_utc)
{
    uint32_t start, end;
...
bool AG_Logger_Block::BlockRead(uint16_t IntPageAdr, void *pBuffer, uint16_t size)
{
    memcpy(pBuffer, &buffer[IntPageAdr], size);
    return true;
}

bool AG_Logger_Block::logging_failed() const
{
    if (!_initialised) {
        return true;
    }
...
bool AG_Logger_Block::io_thread_alive() const
{
    // if the io thread hasn't had a heartbeat in 3s it is dead
    return (AG_HAL::millis() - io_timer_heartbeat) < 3000U || !hal.scheduler->is_system_initialized();
}
 ...
 */
void AG_Logger_Block::io_timer(void)
{
    ...
    if (df_EraseFrom > 0) {
        ...
        while (next_sector < aligned_sector) {
            ...
            io_timer_heartbeat = AG_HAL::millis();
            next_sector++;
        }
...
void AG_Logger_Block::write_log_page()
{
    struct PageHeader ph;
...
void AG_Logger_Block::flash_test()
{
