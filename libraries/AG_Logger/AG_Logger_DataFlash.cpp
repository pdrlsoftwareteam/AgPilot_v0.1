...
void AG_Logger_DataFlash::Init()
{
    dev = hal.spi->get_device("dataflash");
    if (!dev) {
        AG_HAL::panic("PANIC: AG_Logger SPIDeviceDriver not found");
        return;
    }
 ...
 */
void AG_Logger_DataFlash::WaitReady()
{
    if (flash_died) {
        return;
    }
...
bool AG_Logger_DataFlash::getSectorCount(void)
{
    WaitReady();
...
uint8_t AG_Logger_DataFlash::ReadStatusReg()
{
    WITH_SEMAPHORE(dev_sem);
...
bool AG_Logger_DataFlash::Busy()
{
    return (ReadStatusReg() & (JEDEC_STATUS_BUSY | JEDEC_STATUS_SRP0)) != 0;
}

void AG_Logger_DataFlash::Enter4ByteAddressMode(void)
{
    WITH_SEMAPHORE(dev_sem);
...
*/
void AG_Logger_DataFlash::send_command_addr(uint8_t command, uint32_t PageAdr)
{
    uint8_t cmd[5];
    cmd[0] = command;
    if (use_32bit_address) {
...
void AG_Logger_DataFlash::PageToBuffer(uint32_t pageNum)
{
...
void AG_Logger_DataFlash::BufferToPage(uint32_t pageNum)
{
    if (pageNum == 0 || pageNum > df_NumPages+1) {
...
*/
void AG_Logger_DataFlash::SectorErase(uint32_t blockNum)
{
    WriteEnable();
...
/*
  erase one 4k sector
*/
void AG_Logger_DataFlash::Sector4kErase(uint32_t sectorNum)
{
    WriteEnable();
...
void AG_Logger_DataFlash::StartErase()
{
    WriteEnable();
...
bool AG_Logger_DataFlash::InErase()
{
...
void AG_Logger_DataFlash::WriteEnable(void)
{
    WaitReady();
    WITH_SEMAPHORE(dev_sem);
