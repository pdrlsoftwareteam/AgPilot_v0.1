...
AG_Logger_Backend::AG_Logger_Backend(AG_Logger &front,
...
uint8_t AG_Logger_Backend::num_types() const
{
    return _front._num_types;
}

const struct LogStructure *AG_Logger_Backend::structure(uint8_t num) const
{
    return _front.structure(num);
}

uint8_t AG_Logger_Backend::num_units() const
{
    return _front._num_units;
}

const struct UnitStructure *AG_Logger_Backend::unit(uint8_t num) const
{
    return _front.unit(num);
}

uint8_t AG_Logger_Backend::num_multipliers() const
{
    return _front._num_multipliers;
}

const struct MultiplierStructure *AG_Logger_Backend::multiplier(uint8_t num) const
{
    return _front.multiplier(num);
}

AG_Logger_Backend::vehicle_startup_message_Writer AG_Logger_Backend::vehicle_message_writer() const {
    return _front._vehicle_messages;
}

void AG_Logger_Backend::periodic_10Hz(const uint32_t now)
{
}

void AG_Logger_Backend::periodic_1Hz()
{
...
void AG_Logger_Backend::periodic_fullrate()
{
    push_log_blocks();
}

void AG_Logger_Backend::periodic_tasks()
{
...
void AG_Logger_Backend::start_new_log_reset_variables()
{
    _dropped = 0;
...
bool AG_Logger_Backend::allow_start_ekf() const
{
...
// for example, in AG_Logger_MAVLink we may push messages into the UART.
void AG_Logger_Backend::push_log_blocks() {
...
bool AG_Logger_Backend::WriteBlockCheckStartupMessages()
{
...
void AG_Logger_Backend::WriteMoreStartupMessages()
{
#if APM_BUILD_TYPE(APM_BUILD_Replay)
    return;
#endif
...
bool AG_Logger_Backend::Write_Emit_FMT(uint8_t msg_type)
{
#if APM_BUILD_TYPE(APM_BUILD_Replay)
    ...
    struct AG_Logger::log_write_fmt_strings ls = {};
    ...
    if (!_front.fill_log_write_logstructure(logstruct, msg_type)) {
        ...
        INTERNAL_ERROR(AG_InternalError::error_t::logger_missing_logstructure);
        return false;
    }
...
bool AG_Logger_Backend::Write(const uint8_t msg_type, va_list arg_list, bool is_critical, bool is_streaming)
{
    ...
    if (fmt == nullptr) {
        INTERNAL_ERROR(AG_InternalError::error_t::logger_logwrite_missingfmt);
        return false;
    }
...
bool AG_Logger_Backend::StartNewLogOK() const
{
    if (logging_started()) {
        return false;
    }
...
void AG_Logger_Backend::validate_WritePrioritisedBlock(const void *pBuffer,
                                                       uint16_t size)
{
    ...
    // checks.
    if (size < 3) {
        AG_HAL::panic("Short prioritised block");
    }
    if (((uint8_t*)pBuffer)[0] != HEAD_BYTE1 ||
        ((uint8_t*)pBuffer)[1] != HEAD_BYTE2) {
        AG_HAL::panic("Not passed a message");
    }
    const uint8_t type = ((uint8_t*)pBuffer)[2];
    uint8_t type_len;
    const struct LogStructure *s = _front.structure_for_msg_type(type);
    if (s == nullptr) {
        const struct AG_Logger::log_write_fmt *t = _front.log_write_fmt_for_msg_type(type);
        if (t == nullptr) {
            AG_HAL::panic("No structure for msg_type=%u", type);
        }
        type_len = t->msg_len;
    } else {
        type_len = s->msg_len;
    ...
    if (type_len != size) {
        ...
        AG_HAL::panic("Size mismatch for %u (%s) (expected=%u got=%u)\n",
                      type, name, type_len, size);
    }
}
#endif

bool AG_Logger_Backend::WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical, bool writev_streaming)
{
...
bool AG_Logger_Backend::ShouldLog(bool is_critical)
{
    ...
    if (_front.in_log_download() &&
        ...
        if (AG_HAL::millis() - _front._last_mavlink_log_transfer_message_handled_ms < 10000) {
...
void AG_Logger_Backend::PrepForArming()
{
    if (_rotate_pending) {
        _rotate_pending = false;
        stop_logging();
...
bool AG_Logger_Backend::Write_MessageF(const char *fmt, ...)
{
...
// Write rally points
bool AG_Logger_Backend::Write_RallyPoint(uint8_t total,
                                         uint8_t sequence,
...
// Write rally points
bool AG_Logger_Backend::Write_Rally()
{
...
// Write a fence point
bool AG_Logger_Backend::Write_FencePoint(uint8_t total, uint8_t sequence, const AG_PolyFenceItem &fence_point)
{
...
bool AG_Logger_Backend::Write_Fence()
{
    // kick off asynchronous write:
...
bool AG_Logger_Backend::Write_VER()
{
...
/*
  ...
  number of logs.
*/
uint16_t AG_Logger_Backend::log_num_from_list_entry(const uint16_t list_entry)
{
...
uint16_t AG_Logger_Backend::find_oldest_log()
{
...
void AG_Logger_Backend::vehicle_was_disarmed()
{
...
bool AG_Logger_Backend::logging_enabled() const
{
...
void AG_Logger_Backend::Write_AG_Logger_Stats_File(const struct df_stats &_stats)
{
...
void AG_Logger_Backend::df_stats_gather(const uint16_t bytes_written, uint32_t space_remaining)
{
...
void AG_Logger_Backend::df_stats_clear() {
    memset(&stats, '\0', sizeof(stats));
    stats.buf_space_min = -1;
}

void AG_Logger_Backend::df_stats_log() {
    Write_AG_Logger_Stats_File(stats);
    df_stats_clear();
}


// class to handle rate limiting of log messages
AG_Logger_RateLimiter::AG_Logger_RateLimiter(const AG_Logger &_front, const AP_Float &_limit_hz, const AP_Float &_disarm_limit_hz)
    : front(_front),
 ...
 */
bool AG_Logger_RateLimiter::should_log_streaming(uint8_t msgid, float rate_hz)
{
    if (front._log_pause) {
        return false;
    }
    const uint16_t now = AG_HAL::millis16();
 ...
 */
bool AG_Logger_RateLimiter::should_log(uint8_t msgid, bool writev_streaming)
{
