...
#include "AG_Beacon_Pozyx.h"
#include "AG_Beacon_Marvelmind.h"
#include "AG_Beacon_Nooploop.h"
#include "AG_Beacon_SITL.h"
...
const AG_Param::GroupInfo AG_Beacon::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("_TYPE",    0, AG_Beacon, _type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: -90 90
    // @User: Advanced
    AP_GROUPINFO("_LATITUDE", 1, AG_Beacon, origin_lat, 0),
    ...
    // @Range: -180 180
    // @User: Advanced
    AP_GROUPINFO("_LONGITUDE", 2, AG_Beacon, origin_lon, 0),
    ...
    // @Range: 0 10000
    // @User: Advanced
    AP_GROUPINFO("_ALT", 3, AG_Beacon, origin_alt, 0),
    ...
    // @Range: -180 +180
    // @User: Advanced
    AP_GROUPINFO("_ORIENT_YAW", 4, AG_Beacon, orient_yaw, 0),
...
// initialise the AG_Beacon class
void AG_Beacon::init(void)
{
    if (_driver != nullptr) {
    ...
    // create backend
    if (_type == AG_BeaconType_Pozyx) {
        _driver = new AG_Beacon_Pozyx(*this);
    } else if (_type == AG_BeaconType_Marvelmind) {
        _driver = new AG_Beacon_Marvelmind(*this);
    } else if (_type == AG_BeaconType_Nooploop) {
        _driver = new AG_Beacon_Nooploop(*this);
    }
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_type == AG_BeaconType_SITL) {
        _driver = new AG_Beacon_SITL(*this);
    }
#endif
}

// return true if beacon feature is enabled
bool AG_Beacon::enabled(void) const
{
    return (_type != AG_BeaconType_None);
}

// return true if sensor is basically healthy (we are receiving data)
bool AG_Beacon::healthy(void) const
{
    if (!device_ready()) {
        return false;
    }
...
bool AG_Beacon::get_origin(Location &origin_loc) const
{
    if (!device_ready()) {
        return false;
    }
...
bool AG_Beacon::get_vehicle_position_ned(Vector3f &position, float& accuracy_estimate) const
{
    if (!device_ready()) {
        return false;
    }
    ...
    // check for timeout
    if (AG_HAL::millis() - veh_pos_update_ms > AP_BEACON_TIMEOUT_MS) {
        return false;
    }
...
bool AG_Beacon::get_beacon_data(uint8_t beacon_instance, struct BeaconState& state) const
{
...
uint8_t AG_Beacon::beacon_id(uint8_t beacon_instance) const
{
...
bool AG_Beacon::beacon_healthy(uint8_t beacon_instance) const
{
    if (beacon_instance >= num_beacons) {
        return false;
    }
...
float AG_Beacon::beacon_distance(uint8_t beacon_instance) const
{
...
Vector3f AG_Beacon::beacon_position(uint8_t beacon_instance) const
{
    if (!device_ready() || beacon_instance >= num_beacons) {
        Vector3f temp = {};
        return temp;
...
uint32_t AG_Beacon::beacon_last_update_ms(uint8_t beacon_instance) const
{
    if (_type == AG_BeaconType_None || beacon_instance >= num_beacons) {
        return 0;
    }
...
void AG_Beacon::update_boundary_points()
{
...
bool AG_Beacon::get_next_boundary_point(const Vector2f* boundary_pts, uint8_t num_points, uint8_t current_index, float start_angle, uint8_t& next_index, float& next_angle)
{
    // sanity check
...
const Vector2f* AG_Beacon::get_boundary_points(uint16_t& num_points) const
{
    if (!device_ready()) {
        num_points = 0;
        return nullptr;
...
bool AG_Beacon::device_ready(void) const
{
    return ((_driver != nullptr) && (_type != AG_BeaconType_None));
}
...
// singleton instance
AG_Beacon *AG_Beacon::_singleton;

namespace AP {

AG_Beacon *beacon()
{
    return AG_Beacon::get_singleton();
}
