...
void AG_Beacon_Marvelmind::process_position_datagram()
{
...
void AG_Beacon_Marvelmind::process_position_highres_datagram()
{
...
AG_Beacon_Marvelmind::StationaryBeaconPosition* AG_Beacon_Marvelmind::get_or_alloc_beacon(uint8_t address)
{
...
void AG_Beacon_Marvelmind::process_beacons_positions_datagram()
{
...
void AG_Beacon_Marvelmind::process_beacons_positions_highres_datagram()
{
    const uint8_t n = input_buffer[5]; // number of beacons in packet
    StationaryBeaconPosition *stationary_beacon;
...
void AG_Beacon_Marvelmind::process_beacons_distances_datagram()
{
...
int8_t AG_Beacon_Marvelmind::find_beacon_instance(uint8_t address) const
{
    for (uint8_t i = 0; i < hedge.positions_beacons.num_beacons; i++) {
        if (hedge.positions_beacons.beacons[i].address == address) {
            return i;
...
void AG_Beacon_Marvelmind::update(void)
{
    if (uart == nullptr) {
        return;
    }
...
bool AG_Beacon_Marvelmind::healthy()
{
    // healthy if we have parsed a message within the past 300ms
    return ((AG_HAL::millis() - last_update_ms) < AP_BEACON_TIMEOUT_MS);
}

void AG_Beacon_Marvelmind::set_stationary_beacons_positions()
{
    bool set = false;
...
void AG_Beacon_Marvelmind::order_stationary_beacons()
{
    if (hedge.positions_beacons.updated) {
        bool swapped = false;
