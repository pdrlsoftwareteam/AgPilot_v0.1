...
*/
AG_Beacon_Backend::AG_Beacon_Backend(AG_Beacon &frontend) :
    _frontend(frontend)
{
    const AG_SerialManager &serialmanager = AP::serialmanager();
    uart = serialmanager.find_serial(AG_SerialManager::SerialProtocol_Beacon, 0);
    if (uart == nullptr) {
        return;
    }

    uart->begin(serialmanager.find_baudrate(AG_SerialManager::SerialProtocol_Beacon, 0));
}

// set vehicle position
// pos should be in meters in NED frame from the beacon's local origin
// accuracy_estimate is also in meters
void AG_Beacon_Backend::set_vehicle_position(const Vector3f& pos, float accuracy_estimate)
{
    _frontend.veh_pos_update_ms = AG_HAL::millis();
...
void AG_Beacon_Backend::set_beacon_distance(uint8_t beacon_instance, float distance)
{
    ...
    _frontend.beacon_state[beacon_instance].distance_update_ms = AG_HAL::millis();
...
void AG_Beacon_Backend::set_beacon_position(uint8_t beacon_instance, const Vector3f& pos)
{
    // sanity check instance
    if (beacon_instance >= AP_BEACON_MAX_BEACONS) {
        return;
...
Vector3f AG_Beacon_Backend::correct_for_orient_yaw(const Vector3f &vector)
{
