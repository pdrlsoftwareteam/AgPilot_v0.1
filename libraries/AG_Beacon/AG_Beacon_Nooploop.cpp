...
bool AG_Beacon_Nooploop::healthy()
{
    // healthy if we have parsed a message within the past 300ms
    return ((AG_HAL::millis() - _last_update_ms) < AP_BEACON_TIMEOUT_MS);
}

// update the state of the sensor
void AG_Beacon_Nooploop::update(void)
{
        ...
        if (b >= 0 ) {
            ...
            if (type == MsgType::NODE_FRAME2) {
                if (_anchor_pos_avail) {
                    parse_node_frame2();
                } else if (AG_HAL::millis() - _last_request_setting_ms > 2000) {
                    _last_request_setting_ms = AG_HAL::millis();
                    request_setting();
                }
...
void AG_Beacon_Nooploop::request_setting()
{
...
AG_Beacon_Nooploop::MsgType AG_Beacon_Nooploop::parse_byte(uint8_t b)
{
...
void AG_Beacon_Nooploop::parse_node_frame2()
{
...
void AG_Beacon_Nooploop::parse_setting_frame0()
{
