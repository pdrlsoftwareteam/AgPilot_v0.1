...
const AG_Param::GroupInfo AG_NMEA_Output::var_info[] = {
    ...
    // @Units: ms
    // @User: Standard
    AP_GROUPINFO("RATE_MS", 1, AG_NMEA_Output, _interval_ms, 100),
    ...
    // @User: Standard
    AP_GROUPINFO("MSG_EN", 2, AG_NMEA_Output, _message_enable_bitmask, AP_NMEA_OUTPUT_MESSAGE_ENABLED_DEFAULT),

    AP_GROUPEND
};

void AG_NMEA_Output::init()
{
    const AG_SerialManager& sm = AP::serialmanager();

    _num_outputs = 0;
    for (uint8_t i = 0; i < ARRAY_SIZE(_uart); i++) {
        _uart[i] = sm.find_serial(AG_SerialManager::SerialProtocol_NMEAOutput, i);
...
void AG_NMEA_Output::update()
{
...
#else
    const bool pos_valid = (gps_status >= AG_GPS::GPS_OK_FIX_3D);
    loc = gps.location();
#endif
    ...
    if ((_message_enable_bitmask.get() & static_cast<int16_t>(Enabled_Messages::PASHR)) != 0) {
        ...
        const uint8_t gps_status_flag = (gps_status >= AG_GPS::GPS_OK_FIX_3D_RTK_FIXED) ? 2 :
                                                    (gps_status >= AG_GPS::GPS_OK_FIX_2D ? 1 : 0);
