...
#include "AG_OpticalFlow_Pixart_SROM.h"
#include <stdio.h>
...
// constructor
AG_OpticalFlow_Pixart::AG_OpticalFlow_Pixart(const char *devname, AG_OpticalFlow &_frontend) :
...
// detect the device
AG_OpticalFlow_Pixart *AG_OpticalFlow_Pixart::detect(const char *devname, AG_OpticalFlow &_frontend)
{
    AG_OpticalFlow_Pixart *sensor = new AG_OpticalFlow_Pixart(devname, _frontend);
    if (!sensor) {
        return nullptr;
    }
...
// setup the device
bool AG_OpticalFlow_Pixart::setup_sensor(void)
{
    if (!_dev) {
        return false;
    }
    ...
    integral.last_frame_us = AG_HAL::micros();

    _dev->register_periodic_callback(2000, FUNCTOR_BIND_MEMBER(&AG_OpticalFlow_Pixart::timer, void));
    return true;
}

// write an 8 bit register
void AG_OpticalFlow_Pixart::reg_write(uint8_t reg, uint8_t value)
{
...
uint8_t AG_OpticalFlow_Pixart::reg_read(uint8_t reg)
{
    uint8_t v = 0;
    _dev->set_chip_select(true);
    _dev->transfer(&reg, 1, nullptr, 0);
...
uint16_t AG_OpticalFlow_Pixart::reg_read16u(uint8_t reg)
{
...
int16_t AG_OpticalFlow_Pixart::reg_read16s(uint8_t reg)
{
    return (int16_t)reg_read16u(reg);
}

void AG_OpticalFlow_Pixart::srom_download(void)
{
    reg_write(0x39, 0x02);
...
void AG_OpticalFlow_Pixart::load_configuration(const RegData *init_data, uint16_t n)
{
...
void AG_OpticalFlow_Pixart::motion_burst(void)
{
...
void AG_OpticalFlow_Pixart::timer(void)
{
    if (AG_HAL::micros() - last_burst_us < 500) {
        return;
    }
    motion_burst();
    last_burst_us = AG_HAL::micros();
...
void AG_OpticalFlow_Pixart::update(void)
{
    uint32_t now = AG_HAL::millis();
