...
// constructor
AG_OpticalFlow_CXOF::AG_OpticalFlow_CXOF(AG_OpticalFlow &_frontend, AG_HAL::UARTDriver *_uart) :
...
// detect the device
AG_OpticalFlow_CXOF *AG_OpticalFlow_CXOF::detect(AG_OpticalFlow &_frontend)
{
    AG_SerialManager *serial_manager = AP::serialmanager().get_singleton();
    if (serial_manager == nullptr) {
        return nullptr;
    }

    // look for first serial driver with protocol defined as OpticalFlow
    AG_HAL::UARTDriver *uart = serial_manager->find_serial(AG_SerialManager::SerialProtocol_OpticalFlow, 0);
    if (uart == nullptr) {
        return nullptr;
    }

    // we have found a serial port so use it
    AG_OpticalFlow_CXOF *sensor = new AG_OpticalFlow_CXOF(_frontend, uart);
    return sensor;
}

// initialise the sensor
void AG_OpticalFlow_CXOF::init()
{
    // sanity check uart
    if (uart == nullptr) {
        return;
    ...
    last_frame_us = AG_HAL::micros();
}

// read latest values from sensor and fill in x,y and totals.
void AG_OpticalFlow_CXOF::update(void)
{
    // sanity check uart
    if (uart == nullptr) {
        return;
