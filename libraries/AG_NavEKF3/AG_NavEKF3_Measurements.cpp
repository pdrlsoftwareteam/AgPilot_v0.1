...
void NavEKF3_core::readRangeFinder(void)
{
    ...
    if ((imuSampleTime_ms - lastRngMeasTime_ms) > frontend->sensorIntervalMin_ms) {
        ...
        for (uint8_t sensorIndex = 0; sensorIndex < ARRAY_SIZE(rngMeasIndex); sensorIndex++) {
            ...
            if ((sensor->orientation() == ROTATION_PITCH_270) && (sensor->status() == AG_DAL_RangeFinder::Status::Good)) {
...
void NavEKF3_core::readGpsData()
{
    ...
    if (gps.status(selected_gps) < AG_DAL_GPS::GPS_OK_FIX_3D) {
    ...
    if (gpsDataNew.have_vz && frontend->sources.useVelZSource(AG_NavEKF_Source::SourceZ::GPS)) {
        useGpsVertVel = true;
    } else {
        useGpsVertVel = false;
    }
...
void NavEKF3_core::correctEkfOriginHeight()
{
    ...
    } else if (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER) {
...
void NavEKF3_core::readRngBcnData()
{
    ...
    const AG_DAL_Beacon *beacon = dal.beacon();
...
void NavEKF3_core::update_gps_selection(void)
{
    ...
    if (frontend->_affinity & EKF_AFFINITY_GPS) {
        ...
        if (gps.status(preferred_gps) >= AG_DAL_GPS::GPS_OK_FIX_3D) {
...
void NavEKF3_core::updateMovementCheck(void)
{
    ...
    const bool runCheck = onGround && (yaw_source == AG_NavEKF_Source::SourceYaw::GPS || yaw_source == AG_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK ||
                                       yaw_source == AG_NavEKF_Source::SourceYaw::EXTNAV || yaw_source == AG_NavEKF_Source::SourceYaw::GSF || !use_compass());
    if (!runCheck)
    {
...
{
    table_earth_field_ga = AG_Declination::get_earth_field_ga(loc).toftype();
    table_declination = radians(AG_Declination::get_declination(loc.lat*1.0e-7,
                                                                loc.lng*1.0e-7));
