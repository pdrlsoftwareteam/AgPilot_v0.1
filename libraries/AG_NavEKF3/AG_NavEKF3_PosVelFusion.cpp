...
#include "AG_NavEKF3.h"
#include "AG_NavEKF3_core.h"
...
void NavEKF3_core::ResetHeight(void)
{
    ...
    if (inFlight &&
        ...
        frontend->sources.useVelZSource(AG_NavEKF_Source::SourceZ::GPS) &&
        gpsDataNew.have_vz &&
        (imuSampleTime_ms - gpsDataDelayed.time_ms < 500)) {
        stateStruct.velocity.z =  gpsDataNew.vel.z;
#if EK3_FEATURE_EXTERNAL_NAV
    } else if (inFlight && useExtNavVel && (activeHgtSource == AG_NavEKF_Source::SourceZ::EXTNAV)) {
...
{
    if (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER || !onGround) {
...
void NavEKF3_core::SelectVelPosFusion()
{
    ...
    const AG_NavEKF_Source::SourceXY posxy_source = frontend->sources.getPosXYSource();
    if (posxy_source != posxy_source_last) {
        posxy_source_reset = (posxy_source != AG_NavEKF_Source::SourceXY::NONE);
    ...
    if (gpsDataToFuse && (PV_AidingMode == AID_ABSOLUTE) && (posxy_source == AG_NavEKF_Source::SourceXY::GPS)) {

        // Don't fuse velocity data if GPS doesn't support it
        fuseVelData = frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::GPS);
        fusePosData = true;
#if EK3_FEATURE_EXTERNAL_NAV
        ...
        velPosObs[4] = posxy.y;
#if EK3_FEATURE_EXTERNAL_NAV
    } else if (extNavDataToFuse && (PV_AidingMode == AID_ABSOLUTE) && (posxy_source == AG_NavEKF_Source::SourceXY::EXTNAV)) {
    ...
    if (extNavVelToFuse && frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::EXTNAV)) {
        fuseVelData = true;
    ...
    if (gpsDataToFuse && (PV_AidingMode == AID_ABSOLUTE) && (posxy_source == AG_NavEKF_Source::SourceXY::GPS) && (gpsDataDelayed.sensor_idx != last_gps_idx || posxy_source_reset)) {
        ...
        if (activeHgtSource == AG_NavEKF_Source::SourceZ::GPS) {
    ...
    if (extNavDataToFuse && (PV_AidingMode == AID_ABSOLUTE) && (posxy_source == AG_NavEKF_Source::SourceXY::EXTNAV) && (extNavDataDelayed.posReset || posxy_source_reset)) {
        // mark a source reset as consumed
        posxy_source_reset = false;
        ResetPositionNE(extNavDataDelayed.pos.x, extNavDataDelayed.pos.y);
        if (activeHgtSource == AG_NavEKF_Source::SourceZ::EXTNAV) {
            ResetPositionD(-hgtMea);
        }
    }
#endif // EK3_FEATURE_EXTERNAL_NAV
...
void NavEKF3_core::FuseVelPosNED()
{
    ...
    if (fuseVelData || fusePosData || fuseHgtData) {
        ...
        if (gpsDataDelayed.have_vz && fuseVelData && (frontend->sources.getPosZSource() != AG_NavEKF_Source::SourceZ::GPS)) {
        ...
        if (fuseHgtData) {
            ...
            if (hgtTestRatio < maxTestRatio) {
                hgtCheckPassed = true;
                lastHgtPassTime_ms = imuSampleTime_ms;
            } else if ((frontend->_gpsGlitchRadiusMax <= 0) && !onGroundNotNavigating && (activeHgtSource == AG_NavEKF_Source::SourceZ::GPS)) {
                // Handle the special case where the glitch radius parameter has been set to a non-positive number.
                // The innovation variance is increased to limit the state update to an amount corresponding
                // to a test ratio of 1.
                posCheckPassed = true;
        ...
        for (obsIndex=0; obsIndex<=5; obsIndex++) {
            if (fuseData[obsIndex]) {
                ...
                if (obsIndex <= 2) {
                    ...
                    if ((dal.get_touchdown_expected() || dal.get_takeoff_expected()) && activeHgtSource == AG_NavEKF_Source::SourceZ::BARO) {
...
void NavEKF3_core::selectHeightForFusion()
{
    ...
    if ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::NONE)) {
        // user has specified no height sensor
        activeHgtSource = AG_NavEKF_Source::SourceZ::NONE;
    } else if ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::RANGEFINDER) && _rng && rangeFinderDataIsFresh) {
        // user has specified the range finder as a primary height source
        activeHgtSource = AG_NavEKF_Source::SourceZ::RANGEFINDER;
    } else if ((frontend->_useRngSwHgt > 0) && ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::BARO) || (frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::GPS)) && _rng && rangeFinderDataIsFresh) {
        ...
        */
        if ((aboveUpperSwHgt || dontTrustTerrain) && (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER)) {
            // cannot trust terrain or range finder so stop using range finder height
            if (frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::BARO) {
                activeHgtSource = AG_NavEKF_Source::SourceZ::BARO;
            } else if (frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::GPS) {
                activeHgtSource = AG_NavEKF_Source::SourceZ::GPS;
            }
        } else if (belowLowerSwHgt && trustTerrain && (prevTnb.c.z >= 0.7f)) {
            // reliable terrain and range finder so start using range finder height
            activeHgtSource = AG_NavEKF_Source::SourceZ::RANGEFINDER;
        }
    } else if (frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::BARO) {
        activeHgtSource = AG_NavEKF_Source::SourceZ::BARO;
    } else if ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::GPS) && ((imuSampleTime_ms - lastTimeGpsReceived_ms) < 500) && validOrigin && gpsAccuracyGood) {
        activeHgtSource = AG_NavEKF_Source::SourceZ::GPS;
#if EK3_FEATURE_BEACON_FUSION
    } else if ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::BEACON) && validOrigin && rngBcnGoodToAlign) {
        activeHgtSource = AG_NavEKF_Source::SourceZ::BEACON;
#endif
#if EK3_FEATURE_EXTERNAL_NAV
    } else if ((frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::EXTNAV) && extNavDataIsFresh) {
        activeHgtSource = AG_NavEKF_Source::SourceZ::EXTNAV;
#endif
    }

    // Use Baro alt as a fallback if we lose range finder, GPS, external nav or Beacon
    bool lostRngHgt = ((activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER) && !rangeFinderDataIsFresh);
    bool lostGpsHgt = ((activeHgtSource == AG_NavEKF_Source::SourceZ::GPS) && ((imuSampleTime_ms - lastTimeGpsReceived_ms) > 2000 || !gpsAccuracyGoodForAltitude));
#if EK3_FEATURE_BEACON_FUSION
    bool lostRngBcnHgt = ((activeHgtSource == AG_NavEKF_Source::SourceZ::BEACON) && ((imuSampleTime_ms - rngBcnDataDelayed.time_ms) > 2000));
#endif
    bool fallback_to_baro =
        ...
        ;
#if EK3_FEATURE_EXTERNAL_NAV
    bool lostExtNavHgt = ((activeHgtSource == AG_NavEKF_Source::SourceZ::EXTNAV) && !extNavDataIsFresh);
    ...
    if (baroDataToFuse) {
        ...
        if (activeHgtSource != AG_NavEKF_Source::SourceZ::BARO) {
            calcFiltBaroOffset();
        }
    ...
    if (gpsDataToFuse &&
            (((frontend->_originHgtMode & (1 << 0)) && (activeHgtSource == AG_NavEKF_Source::SourceZ::BARO)) ||
            ((frontend->_originHgtMode & (1 << 1)) && (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER)))
            ) {
...
#if EK3_FEATURE_EXTERNAL_NAV
    if (extNavDataToFuse && (activeHgtSource == AG_NavEKF_Source::SourceZ::EXTNAV)) {
...
#endif // EK3_FEATURE_EXTERNAL_NAV
        if (rangeDataToFuse && (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER)) {
    ...
    } else if (gpsDataToFuse && (activeHgtSource == AG_NavEKF_Source::SourceZ::GPS)) {
        // using GPS data
    ...
    } else if (baroDataToFuse && (activeHgtSource == AG_NavEKF_Source::SourceZ::BARO)) {
        // using Baro data
        ...
        velPosObs[5] = -hgtMea;
    } else if ((activeHgtSource == AG_NavEKF_Source::SourceZ::NONE && imuSampleTime_ms - lastHgtPassTime_ms > 70)) {
...
void NavEKF3_core::SelectBodyOdomFusion()
{
    ...
    if (bodyOdomDataToFuse && frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::EXTNAV)) {
    ...
    if (wheelOdomDataToFuse && frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::WHEEL_ENCODER)) {
