...
void NavEKF3_core::SelectFlowFusion()
{
    ...
    gndOffsetValid = ((imuSampleTime_ms - gndHgtValidTime_ms) < 5000) || (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER);
    // Perform tilt check
    ...
    if (flowDataToFuse && tiltOK) {
        const bool fuse_optflow = (frontend->_flowUse == FLOW_USE_NAV) && frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::OPTFLOW);
...
void NavEKF3_core::EstimateTerrainOffset(const of_elements &ofDataDelayed)
{
    ...
    if ((!rangeDataToFuse && cantFuseFlowData) || (activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER)) {
        // skip update
