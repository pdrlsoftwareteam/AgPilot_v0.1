...
const AG_Param::GroupInfo NavEKF3::var_info[] = {
...
const AG_Param::GroupInfo NavEKF3::var_info2[] = {

    // @Group: SRC
    // @Path: ../AG_NavEKF/AG_NavEKF_Source.cpp
    AP_SUBGROUPINFO(sources, "SRC", 1, NavEKF3, AG_NavEKF_Source),
...
bool NavEKF3::InitialiseFilter(void)
{
    ...
    AP::dal().start_frame(AG_DAL::FrameType::InitialiseFilterEKF3);
...
{
    AP::dal().start_frame(AG_DAL::FrameType::UpdateFilterEKF3);
    ...
    for (uint8_t i=0; i<num_cores; i++) {
        ...
        if (core[i].getFramesSincePredict() < (_framesPerPrediction+3) &&
            AP::dal().ekf_low_time_remaining(AG_DAL::EKFType::EKF3, i)) {
...
{
    AP::dal().log_event3(AG_DAL::Event::checkLaneSwitch);
...
{
    AP::dal().log_event3(AG_DAL::Event::requestYawReset);
...
void NavEKF3::setPosVelYawSourceSet(uint8_t source_set_idx)
{
    if (source_set_idx < AP_NAKEKF_SOURCE_SET_MAX) {
        AP::dal().log_event3(AG_DAL::Event(uint8_t(AG_DAL::Event::setSourceSet0)+source_set_idx));
    }
...
bool NavEKF3::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
    ...
    const AG_NavEKF_Source::SourceYaw yaw_source = sources.getYawSource();
    if (((magCalParamVal == 5) || (magCalParamVal == 6)) && (yaw_source != AG_NavEKF_Source::SourceYaw::GPS)) {
...
{
    AP::dal().log_event3(AG_DAL::Event::resetGyroBias);
...
{
    AP::dal().log_event3(AG_DAL::Event::resetHeightDatum);
...
bool NavEKF3::setOriginLLH(const Location &loc)
{
    ...
    if ((sources.getPosXYSource() == AG_NavEKF_Source::SourceXY::GPS) || common_origin_valid) {
...
bool NavEKF3::getVelInnovationsAndVariancesForSource(AG_NavEKF_Source::SourceXY source, Vector3f &innovations, Vector3f &variances) const
{
    if (core) {
...
bool NavEKF3::configuredToUseGPSForPosXY(void) const
{
    ...
    return  (sources.getPosXYSource() == AG_NavEKF_Source::SourceXY::GPS);
}
...
void NavEKF3::convert_parameters()
{
    ...
    if (!AG_Param::find_top_level_key_by_pointer(this, k_param_ekf3)) {
        return;
    }

    // use EK3_GPS_TYPE to set EK3_SRC1_POSXY, EK3_SRC1_VELXY, EK3_SRC1_VELZ
    const AG_Param::ConversionInfo gps_type_info = {k_param_ekf3, 1, AP_PARAM_INT8, "EK3_GPS_TYPE"};
    AP_Int8 gps_type_old;
    const bool found_gps_type = AG_Param::find_old_parameter(&gps_type_info, &gps_type_old);
    if (found_gps_type) {
        switch (gps_type_old.get()) {
        case 0:
            // EK3_GPS_TYPE == 0 (GPS 3D Vel and 2D Pos)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSXY", (int8_t)AG_NavEKF_Source::SourceXY::GPS);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELXY", (int8_t)AG_NavEKF_Source::SourceXY::GPS);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELZ", (int8_t)AG_NavEKF_Source::SourceZ::GPS);
            break;
        case 1:
            // EK3_GPS_TYPE == 1 (GPS 2D Vel and 2D Pos) then EK3_SRC1_POSXY = GPS(1), EK3_SRC1_VELXY = GPS(1), EK3_SRC1_VELZ = NONE(0)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSXY", (int8_t)AG_NavEKF_Source::SourceXY::GPS);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELXY", (int8_t)AG_NavEKF_Source::SourceXY::GPS);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELZ", (int8_t)AG_NavEKF_Source::SourceZ::NONE);
            break;
        case 2:
            // EK3_GPS_TYPE == 2 (GPS 2D Pos) then EK3_SRC1_POSXY = GPS(1), EK3_SRC1_VELXY = None(0), EK3_SRC1_VELZ = NONE(0)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSXY", (int8_t)AG_NavEKF_Source::SourceXY::GPS);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELXY", (int8_t)AG_NavEKF_Source::SourceXY::NONE);
            AG_Param::set_and_save_by_name("EK3_SRC1_VELZ", (int8_t)AG_NavEKF_Source::SourceZ::NONE);
            break;
        case 3:
        default:
    ...
    const AG_Param::ConversionInfo alt_source_info = {k_param_ekf3, 9, AP_PARAM_INT8, "EK3_ALT_SOURCE"};
    AP_Int8 alt_source_old;
    if (AG_Param::find_old_parameter(&alt_source_info, &alt_source_old)) {
        ...
        case 1:
            ...
            AG_Param::set_and_save_by_name("EK3_SRC1_POSZ", (int8_t)AG_NavEKF_Source::SourceZ::RANGEFINDER);
            break;
        case 2:
            // EK3_ALT_SOURCE == 2 (GPS)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSZ", (int8_t)AG_NavEKF_Source::SourceZ::GPS);
            break;
        case 3:
            // EK3_ALT_SOURCE == 3 (Beacon)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSZ", (int8_t)AG_NavEKF_Source::SourceZ::BEACON);
            break;
        case 4:
            // EK3_ALT_SOURCE == 4 (ExtNav)
            AG_Param::set_and_save_by_name("EK3_SRC1_POSZ", (int8_t)AG_NavEKF_Source::SourceZ::EXTNAV);
            break;
        default:
            // do nothing
            break;
    ...
    case 5:
        ...
        AG_Param::set_and_save_by_name("EK3_SRC1_YAW", (int8_t)AG_NavEKF_Source::SourceYaw::GPS);
        break;
    case 6:
        // EK3_MAG_CAL = 6 (ExtYaw with Compass fallback).  We rely on effective_magCal to interpret old "6" values as "When Flying"
        AG_Param::set_and_save_by_name("EK3_SRC1_YAW", (int8_t)AG_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK);
        break;
    default:
        // do nothing
        break;
    ...
    if (AP::dal().opticalflow_enabled() && (!found_gps_type || (gps_type_old.get() <= 2))) {
        AG_Param::set_and_save_by_name("EK3_SRC2_VELXY", (int8_t)AG_NavEKF_Source::SourceXY::OPTFLOW);
    }
}
...
{
    if (val) {
        AP::dal().log_event3(AG_DAL::Event::setTerrainHgtStable);
    } else {
        AP::dal().log_event3(AG_DAL::Event::unsetTerrainHgtStable);
    }
