...
void NavEKF3_core::setAidingMode()
{
    ...
    if (yaw_source_last == AG_NavEKF_Source::SourceYaw::NONE &&
        !motorsArmed &&
        onGround &&
        PV_AidingMode != AID_NONE)
    {
...
bool NavEKF3_core::readyToUseOptFlow(void) const
{
    ...
    if (!frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::OPTFLOW)) {
        return false;
    }
...
bool NavEKF3_core::readyToUseBodyOdm(void) const
{
    ...
    if (!frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::EXTNAV) &&
        !frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::WHEEL_ENCODER)) {
...
{
    if (frontend->sources.getPosXYSource() != AG_NavEKF_Source::SourceXY::GPS) {
        return false;
    }
...
{
#if EK3_FEATURE_BEACON_FUSION
    if (frontend->sources.getPosXYSource() != AG_NavEKF_Source::SourceXY::BEACON) {
        return false;
    }
...
{
#if EK3_FEATURE_EXTERNAL_NAV
    if (frontend->sources.getPosXYSource() != AG_NavEKF_Source::SourceXY::EXTNAV) {
        return false;
    }
...
{
    const AG_NavEKF_Source::SourceYaw yaw_source = frontend->sources.getYawSource();
    if ((yaw_source != AG_NavEKF_Source::SourceYaw::COMPASS) &&
        (yaw_source != AG_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK)) {
...
bool NavEKF3_core::using_noncompass_for_yaw(void) const
{
    const AG_NavEKF_Source::SourceYaw yaw_source = frontend->sources.getYawSource();
#if EK3_FEATURE_EXTERNAL_NAV
    if (yaw_source == AG_NavEKF_Source::SourceYaw::EXTNAV) {
        return ((imuSampleTime_ms - last_extnav_yaw_fusion_ms < 5000) || (imuSampleTime_ms - lastSynthYawTime_ms < 5000));
    }
#endif
    if (yaw_source == AG_NavEKF_Source::SourceYaw::GPS || yaw_source == AG_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK ||
        yaw_source == AG_NavEKF_Source::SourceYaw::GSF || !use_compass()) {
...
{
#if EK3_FEATURE_EXTERNAL_NAV
    if (frontend->sources.getYawSource() == AG_NavEKF_Source::SourceYaw::EXTNAV) {
        return ((imuSampleTime_ms - last_extnav_yaw_fusion_ms < 5000) || (imuSampleTime_ms - lastSynthYawTime_ms < 5000));
    }
#endif
    return false;
...
bool NavEKF3_core::assume_zero_sideslip(void) const
{
    ...
    // traction is lost
    return dal.get_fly_forward() && dal.get_vehicle_class() != AG_DAL::VehicleClass::GROUND;
}
...
{
    if ((PV_AidingMode == AID_ABSOLUTE) && (frontend->sources.getPosXYSource() == AG_NavEKF_Source::SourceXY::GPS)) {
...
void NavEKF3_core::checkGyroCalStatus(void)
{
    ...
    const AG_NavEKF_Source::SourceYaw yaw_source = frontend->sources.getYawSource();
    if (!use_compass() && (yaw_source != AG_NavEKF_Source::SourceYaw::GPS) && (yaw_source != AG_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK) &&
        (yaw_source != AG_NavEKF_Source::SourceYaw::EXTNAV)) {
...
void  NavEKF3_core::updateFilterStatus(void)
{
    ...
    bool hgtNotAccurate = (frontend->sources.getPosZSource() == AG_NavEKF_Source::SourceZ::GPS) && !validOrigin;
    ...
    status.flags.gps_glitching = !gpsAccuracyGood && (PV_AidingMode == AID_ABSOLUTE) && (frontend->sources.getPosXYSource() == AG_NavEKF_Source::SourceXY::GPS); // GPS glitching is affecting navigation accuracy
...
void NavEKF3_core::runYawEstimatorPrediction()
{
    ...
    if (frontend->sources.getPosXYSource() != AG_NavEKF_Source::SourceXY::GPS ||
        !frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::GPS)) {
        return;
    }
