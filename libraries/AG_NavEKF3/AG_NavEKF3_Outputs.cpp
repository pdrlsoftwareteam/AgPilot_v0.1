...
bool NavEKF3_core::getHeightControlLimit(float &height) const
{
    ...
    if (frontend->sources.useVelXYSource(AG_NavEKF_Source::SourceXY::OPTFLOW) && (PV_AidingMode == AID_RELATIVE) && flowDataValid) {
        ...
        if (frontend->sources.getPosZSource() != AG_NavEKF_Source::SourceZ::RANGEFINDER) {
            height -= terrainState;
        }
        return true;
    } else {
...
bool NavEKF3_core::getPosNE(Vector2f &posNE) const
{
    ...
    if (PV_AidingMode != AID_NONE) {
        ...
        if(validOrigin) {
            auto &gps = dal.gps();
            if ((gps.status(selected_gps) >= AG_DAL_GPS::GPS_OK_FIX_2D)) {
...
bool NavEKF3_core::getVelInnovationsAndVariancesForSource(AG_NavEKF_Source::SourceXY source, Vector3f &innovations, Vector3f &variances) const
{
    switch (source) {
    case AG_NavEKF_Source::SourceXY::GPS:
        // check for timeouts
    ...
    case AG_NavEKF_Source::SourceXY::EXTNAV:
        // check for timeouts
    ...
    case AG_NavEKF_Source::SourceXY::OPTFLOW:
        // check for timeouts
...
void NavEKF3_core::send_status_report(GCS_MAVLINK &link) const
{
    ...
    float temp = 0;
    if (((frontend->_useRngSwHgt > 0) && activeHgtSource == AG_NavEKF_Source::SourceZ::RANGEFINDER) || (PV_AidingMode == AID_RELATIVE && flowDataValid)) {
