...
const AG_Param::GroupInfo AG_Relay::var_info[] = {
    // @Param: PIN
    ...
    AP_GROUPINFO("PIN",  0, AG_Relay, _pin[0], RELAY1_PIN_DEFAULT),
    ...
    AP_GROUPINFO("PIN2",  1, AG_Relay, _pin[1], RELAY2_PIN_DEFAULT),
    ...
    AP_GROUPINFO("PIN3",  2, AG_Relay, _pin[2], RELAY3_PIN_DEFAULT),
    ...
    AP_GROUPINFO("PIN4",  3, AG_Relay, _pin[3], RELAY4_PIN_DEFAULT),
    ...
    AP_GROUPINFO("DEFAULT",  4, AG_Relay, _default, 0),
    ...
    AP_GROUPINFO("PIN5",  5, AG_Relay, _pin[4], RELAY5_PIN_DEFAULT),
    ...
    AP_GROUPINFO("PIN6",  6, AG_Relay, _pin[5], RELAY6_PIN_DEFAULT),

    AP_GROUPEND
};

AG_Relay *AG_Relay::singleton;

extern const AG_HAL::HAL& hal;

AG_Relay::AG_Relay(void)
{
    AG_Param::setup_object_defaults(this, var_info);

#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (singleton != nullptr) {
        AG_HAL::panic("AG_Relay must be singleton");
    }
#endif
    singleton = this;
}
...
void AG_Relay::set(const uint8_t instance, const bool value)
{
...
void AG_Relay::toggle(uint8_t instance)
{
...
bool AG_Relay::arming_checks(size_t buflen, char *buffer) const
{
    for (uint8_t i=0; i<AP_RELAY_NUM_RELAYS; i++) {
