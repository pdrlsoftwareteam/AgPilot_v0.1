...
#include <AG_RCTelemetry/AG_RCTelemetry.h>

#include "AG_Frsky_SPortParser.h"
#include "AG_Frsky_MAVlite.h"

#include "AG_Frsky_Telem.h"

#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
#include "AG_Frsky_MAVlite_SPortToMAVlite.h"
#include "AG_Frsky_MAVlite_MAVliteToSPort.h"
#include "AG_Frsky_MAVliteMsgHandler.h"

#define SPORT_TX_PACKET_DUPLICATES          1   // number of duplicates packets we send (fport only)
#endif

class AG_Frsky_SPort_Passthrough : public AG_Frsky_SPort, public AG_RCTelemetry
{
public:

    AG_Frsky_SPort_Passthrough(AG_HAL::UARTDriver *port, bool use_external_data, AG_Frsky_Parameters *&frsky_parameters) :
        AG_Frsky_SPort(port),
        AG_RCTelemetry(WFQ_LAST_ITEM),
    ...
    static AG_Frsky_SPort_Passthrough *get_singleton(void) {
        return singleton;
    }
...
private:
    ...
    AG_Frsky_Parameters *&_frsky_parameters;
    ...
    struct {
        ...
        ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> rx_packet_queue{SPORT_PACKET_QUEUE_LENGTH};
        ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> tx_packet_queue{SPORT_PACKET_QUEUE_LENGTH};
    } _SPort_bidir;

    AG_Frsky_SPortParser _sport_handler;
    AG_Frsky_MAVlite_SPortToMAVlite sport_to_mavlite;
    AG_Frsky_MAVlite_MAVliteToSPort mavlite_to_sport;

    void set_sensor_id(AP_Int8 idx, uint8_t &sensor);
    // tx/rx sport packet processing
    void queue_rx_packet(const AG_Frsky_SPort::sport_packet_t sp);
    ...
    bool send_message(const AG_Frsky_MAVlite_Message &txmsg);
    AG_Frsky_MAVliteMsgHandler mavlite{FUNCTOR_BIND_MEMBER(&AG_Frsky_SPort_Passthrough::send_message, bool, const AG_Frsky_MAVlite_Message &)};
#endif
    ...
    static AG_Frsky_SPort_Passthrough *singleton;
};
