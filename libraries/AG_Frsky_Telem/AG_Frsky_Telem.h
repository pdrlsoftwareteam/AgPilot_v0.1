...
public:
    ...
    /* Do not allow copies */
    CLASS_NO_COPY(AG_Frsky_Telem);

    // init - perform required initialisation
    bool init(bool use_external_data=false);

    static AG_Frsky_Telem *get_singleton(void) {
        return singleton;
    }

    // get next telemetry data for external consumers of SPort data
    static bool get_telem_data(AG_Frsky_Backend::sport_packet_t* packet_array, uint8_t &packet_count, const uint8_t max_size);
...
private:
    ...
    AG_Frsky_Backend *_backend;
    AG_Frsky_Parameters* _frsky_parameters;

    // get next telemetry data for external consumers of SPort data (internal function)
    bool _get_telem_data(AG_Frsky_Backend::sport_packet_t* packet_array, uint8_t &packet_count, const uint8_t max_size);
#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
    ...
    static AG_Frsky_Telem *singleton;
