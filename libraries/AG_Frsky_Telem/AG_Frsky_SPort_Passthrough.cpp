...
AG_Frsky_SPort_Passthrough *AG_Frsky_SPort_Passthrough::singleton;

bool AG_Frsky_SPort_Passthrough::init()
{
    if (!AG_RCTelemetry::init()) {
        return false;
    }
    return AG_Frsky_SPort::init();
}

bool AG_Frsky_SPort_Passthrough::init_serial_port()
{
    if (_use_external_data) {
        return true;
    }
    return AG_Frsky_SPort::init_serial_port();
}

void  AG_Frsky_SPort_Passthrough::send_sport_frame(uint8_t frame, uint16_t appid, uint32_t data)
{
    if (_use_external_data) {
    ...
    return AG_Frsky_SPort::send_sport_frame(frame, appid, data);
}

/*
  setup ready for passthrough telem
 */
void AG_Frsky_SPort_Passthrough::setup_wfq_scheduler(void)
{
    ...
    // initialize sport
    hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&AG_Frsky_SPort_Passthrough::process_rx_queue, void));
#endif //HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
}
...
*/
void AG_Frsky_SPort_Passthrough::adjust_packet_weight(bool queue_empty)
{
    /*
...
// WFQ scheduler
bool AG_Frsky_SPort_Passthrough::is_packet_ready(uint8_t idx, bool queue_empty)
{
    ...
    case WAYPOINT:
        {
            const AG_Mission *mission = AP::mission();
 ...
 */
void AG_Frsky_SPort_Passthrough::process_packet(uint8_t idx)
{
    // send packet
    switch (idx) {
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_SPort_Passthrough::send(void)
{
...
#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
        AG_Frsky_SPort::sport_packet_t sp;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
bool AG_Frsky_SPort_Passthrough::get_next_msg_chunk(void)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_param(void)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_gps_status(void)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_batt(uint8_t instance)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
bool AG_Frsky_SPort_Passthrough::is_passthrough_byte(const uint8_t byte) const
{
#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_ap_status(void)
{
    ...
    // is_flying flag
    ap_status |= (uint8_t)(AG_Notify::flags.flying) << AP_FLYING_OFFSET;
    // armed flag
    ap_status |= (uint8_t)(AG_Notify::flags.armed)<<AP_ARMED_OFFSET;
    // battery failsafe flag
    ap_status |= (uint8_t)(AG_Notify::flags.failsafe_battery)<<AP_BATT_FS_OFFSET;
    // bad ekf flag
    ap_status |= (uint8_t)(AG_Notify::flags.ekf_bad)<<AP_EKF_FS_OFFSET;
    // generic failsafe
    ap_status |= (uint8_t)(AG_Notify::flags.failsafe_battery||AG_Notify::flags.failsafe_ekf||AG_Notify::flags.failsafe_gcs||AG_Notify::flags.failsafe_radio)<<AP_FS_OFFSET;
#if AP_FENCE_ENABLED
    // fence status
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_home(void)
{
    uint32_t home = 0;
    Location loc;
    Location home_loc;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_velandyaw(void)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_attiandrng(void)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_rpm(void)
{
#if AG_RPM_ENABLED
    const AG_RPM *ap_rpm = AP::rpm();
    if (ap_rpm == nullptr) {
        return 0;
    }
    uint32_t value = 0;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_terrain(void)
{
    uint32_t value = 0;
#if AP_TERRAIN_AVAILABLE
    ...
    value |= (uint8_t)(terrain->status() == AG_Terrain::TerrainStatus::TerrainStatusUnhealthy) << TERRAIN_UNHEALTHY_OFFSET;
#endif
    return value;
}
 ...
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_wind(void)
{
#if !APM_BUILD_TYPE(APM_BUILD_Rover)
    Vector3f v;
    {
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
uint32_t AG_Frsky_SPort_Passthrough::calc_waypoint(void)
{
    const AG_Mission *mission = AP::mission();
 ...
 */
bool AG_Frsky_SPort_Passthrough::get_telem_data(sport_packet_t* packet_array, uint8_t &packet_count, const uint8_t max_size)
{
    if (!_use_external_data) {
        return false;
    }
 ...
 */
bool AG_Frsky_SPort_Passthrough::set_telem_data(const uint8_t frame, const uint16_t appid, const uint32_t data)
{
    // queue only Uplink packets
    if (frame == SPORT_UPLINK_FRAME || frame == SPORT_UPLINK_FRAME_RW) {
        const AG_Frsky_SPort::sport_packet_t sp {
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_SPort_Passthrough::queue_rx_packet(const AG_Frsky_SPort::sport_packet_t packet)
{
    // queue only Uplink packets
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_SPort_Passthrough::process_rx_queue()
{
    ...
    while (_SPort_bidir.rx_packet_queue.pop(packet) && loop_count++ < MAVLITE_MSG_SPORT_PACKETS_COUNT(MAVLITE_MAX_PAYLOAD_LEN)) {
        AG_Frsky_MAVlite_Message rxmsg;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_SPort_Passthrough::process_tx_queue()
{
    AG_Frsky_SPort::sport_packet_t packet;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_SPort_Passthrough::set_sensor_id(AP_Int8 param_idx, uint8_t &sensor)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
bool AG_Frsky_SPort_Passthrough::send_message(const AG_Frsky_MAVlite_Message &txmsg)
{
...
namespace AP
{
AG_Frsky_SPort_Passthrough *frsky_passthrough_telem()
{
    return AG_Frsky_SPort_Passthrough::get_singleton();
}
};
