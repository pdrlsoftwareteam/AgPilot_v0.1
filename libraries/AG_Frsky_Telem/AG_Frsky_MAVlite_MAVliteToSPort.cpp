...
void AG_Frsky_MAVlite_MAVliteToSPort::reset()
{
    checksum = 0;
...
void AG_Frsky_MAVlite_MAVliteToSPort::update_checksum(const uint8_t c)
{
    checksum += c; //0-1FF
    checksum += checksum >> 8;
    checksum &= 0xFF;
 ...
 */
bool AG_Frsky_MAVlite_MAVliteToSPort::process(ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> &queue, const AG_Frsky_MAVlite_Message &msg)
{
...
void AG_Frsky_MAVlite_MAVliteToSPort::process_byte(const uint8_t b, ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> &queue)
{
    if (packet_offs == 2) {
