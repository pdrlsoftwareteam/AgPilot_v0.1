#include "AG_Frsky_MAVliteMsgHandler.h"
...
void AG_Frsky_MAVliteMsgHandler::handle_command_long(const AG_Frsky_MAVlite_Message &rxmsg)
{
    ...
    uint8_t param_count = AG_Frsky_MAVlite_Message::bit8_unpack(cmd_options, 3, 0);               // first 3 bits
...
MAV_RESULT AG_Frsky_MAVliteMsgHandler::handle_command(const mavlink_command_long_t &mav_command_long)
{
...
MAV_RESULT AG_Frsky_MAVliteMsgHandler::handle_command_do_set_mode(const mavlink_command_long_t &mav_command_long)
{
...
void AG_Frsky_MAVliteMsgHandler::send_command_ack(const MAV_RESULT mav_result, const uint16_t cmdid)
{
    AG_Frsky_MAVlite_Message txmsg;
...
MAV_RESULT AG_Frsky_MAVliteMsgHandler::handle_command_preflight_calibration_baro(const mavlink_command_long_t &mav_command_long)
{
...
MAV_RESULT AG_Frsky_MAVliteMsgHandler::handle_command_do_fence_enable(const mavlink_command_long_t &mav_command_long)
{
#if AP_FENCE_ENABLED
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_MAVliteMsgHandler::handle_param_request_read(const AG_Frsky_MAVlite_Message &rxmsg)
{
    float param_value;
    ...
    // find existing param
    if (!AG_Param::get(param_name,param_value)) {
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_MAVliteMsgHandler::handle_param_set(const AG_Frsky_MAVlite_Message &rxmsg)
{
    float param_value;
    char param_name[AP_MAX_NAME_SIZE+1];
    ...
    vp = AG_Param::find(param_name, &var_type, &parameter_flags);
    ...
    } else if (!AG_Param::set_and_save_by_name(param_name, param_value)) {
 ...
 */
MAV_RESULT AG_Frsky_MAVliteMsgHandler::handle_command_preflight_reboot(const mavlink_command_long_t &mav_command_long)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AG_Frsky_MAVliteMsgHandler::process_message(const AG_Frsky_MAVlite_Message &rxmsg)
{
    switch (rxmsg.msgid) {
 ...
 */
bool AG_Frsky_MAVliteMsgHandler::send_message(AG_Frsky_MAVlite_Message &txmsg)
{
