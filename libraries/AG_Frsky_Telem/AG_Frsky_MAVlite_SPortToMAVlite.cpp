...
void AG_Frsky_MAVlite_SPortToMAVlite::reset(void)
{
    checksum = 0;
...
void AG_Frsky_MAVlite_SPortToMAVlite::update_checksum(const uint8_t c)
{
    checksum += c; //0-1FF
    checksum += checksum >> 8;
    checksum &= 0xFF;
 ...
 */
bool AG_Frsky_MAVlite_SPortToMAVlite::process(AG_Frsky_MAVlite_Message &rxmsg, const AG_Frsky_SPort::sport_packet_t &packet)
{
...
void AG_Frsky_MAVlite_SPortToMAVlite::parse(uint8_t byte)
{
    switch (parse_state) {
