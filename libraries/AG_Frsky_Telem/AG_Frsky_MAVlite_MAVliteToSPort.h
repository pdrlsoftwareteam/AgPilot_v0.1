...
class AG_Frsky_MAVlite_MAVliteToSPort {
public:

    // insert sport packets calculated from mavlite msg into queue
    bool process(ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> &queue,
                 const AG_Frsky_MAVlite_Message &msg) WARN_IF_UNUSED;
...
private:
    ...
    void process_byte(uint8_t byte, ObjectBuffer_TS<AG_Frsky_SPort::sport_packet_t> &queue);
