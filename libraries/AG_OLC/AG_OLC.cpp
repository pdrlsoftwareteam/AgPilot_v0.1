 ...
 *
 * AG_OLC is based on INAV olc.c implemention, thanks @fiam and other contributors.
 */
...
const int32_t AG_OLC::initial_exponent  = floorf(logf(2 * (LON_MAX / OLC_DEG_MULTIPLIER)) / logf(ENCODING_BASE));
const int32_t AG_OLC::grid_size = (1 / powf(ENCODING_BASE, PAIR_CODE_LEN / 2 - (initial_exponent + 1))) * OLC_DEG_MULTIPLIER;
const int32_t AG_OLC::initial_resolution = powf(ENCODING_BASE, initial_exponent) * OLC_DEG_MULTIPLIER;

constexpr char AG_OLC::olc_alphabet[];
...
float AG_OLC::compute_precision_for_length(int length)
{
    // Magic numbers!
...
int32_t AG_OLC::adjust_latitude(int32_t lat, size_t code_len)
{
...
int32_t AG_OLC::normalize_longitude(int32_t lon)
{
    while (lon < -LON_MAX) {
        lon += LON_MAX;
        lon += LON_MAX;
...
// the previous code.
unsigned AG_OLC::encode_pairs(uint32_t lat, uint32_t lon, size_t length, char *buf, size_t bufsize)
{
    if ((length + 1) >= bufsize) {
        buf[0] = '\0';
        return 0;
...
// character.
int AG_OLC::encode_grid(uint32_t lat, uint32_t lon, size_t length,
...
int AG_OLC::olc_encode(int32_t lat, int32_t lon, size_t length, char *buf, size_t bufsize)
{
    int pos = 0;
