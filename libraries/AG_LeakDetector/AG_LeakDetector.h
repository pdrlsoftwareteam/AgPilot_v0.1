...
class AG_LeakDetector {

    friend class AG_LeakDetector_Analog;
    friend class AG_LeakDetector_Digital;

public:
    AG_LeakDetector();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_LeakDetector);
...
private:
    AG_LeakDetector_Backend *_drivers[LEAKDETECTOR_MAX_INSTANCES];
