...
const AG_Param::GroupInfo AG_LeakDetector::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("1_PIN", 1, AG_LeakDetector, _pin[0], -1),
    ...
    // @Values: 0:Low,1:High
    // @User: Standard
    AP_GROUPINFO("1_LOGIC", 2, AG_LeakDetector, _default_reading[0], 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("1_TYPE", 7, AG_LeakDetector, _type[0], DISABLED), 
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("2_PIN", 3, AG_LeakDetector, _pin[1], -1),
    ...
    // @Values: 0:Low,1:High
    // @User: Standard
    AP_GROUPINFO("2_LOGIC", 4, AG_LeakDetector, _default_reading[1], 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("2_TYPE", 8, AG_LeakDetector, _type[1], DISABLED),
#endif
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("3_PIN", 5, AG_LeakDetector, _pin[2], -1),
    ...
    // @Values: 0:Low,1:High
    // @User: Standard
    AP_GROUPINFO("3_LOGIC", 6, AG_LeakDetector, _default_reading[2], 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("3_TYPE", 9, AG_LeakDetector, _type[2], DISABLED),
#endif

    AP_GROUPEND
};

AG_LeakDetector::AG_LeakDetector() :
    _status(false),
    _last_detect_ms(0)
{
    ...
    for (int i = 0; i < LEAKDETECTOR_MAX_INSTANCES; i++) {
     ...
     CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_CHIBIOS_FMUV5)
        ...
        switch(_type[i]) {
            case ANALOG:
                _state[i].instance = i;
                _drivers[i] = new AG_LeakDetector_Analog(*this, _state[i]);
                break;
            case DIGITAL:
                _state[i].instance = i;
                _drivers[i] = new AG_LeakDetector_Digital(*this, _state[i]);
                break;
        }
    }
}

bool AG_LeakDetector::update()
{
...
void AG_LeakDetector::set_detect()
{
    _last_detect_ms = AG_HAL::millis();
}
