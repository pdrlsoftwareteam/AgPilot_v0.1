...
static AG_InternalError instance;

void AG_InternalError::error(const AG_InternalError::error_t e, uint16_t line) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL && defined(HAL_DEBUG_BUILD)
    switch (e) {
    case AG_InternalError::error_t::watchdog_reset:
    case AG_InternalError::error_t::main_loop_stuck:
    case AG_InternalError::error_t::params_restored:
        // don't panic on these to facilitate watchdog testing
        break;
    default:
        char buffer[50];
        AP::internalerror().error_to_string(buffer, ARRAY_SIZE(buffer), e);
        AG_HAL::panic("AG_InternalError::error_t::%s", buffer);
    }
#endif
...
static_assert((1U<<(ARRAY_SIZE(error_bit_descriptions))) == uint32_t(AG_InternalError::error_t::__LAST__), "too few descriptions for bits");

void AG_InternalError::error_to_string(char *buffer, const uint16_t len, error_t error_code) const
{
    uint32_t temp = log2f(int(error_code));
    strncpy(buffer, error_bit_descriptions[temp], len - 1);
}

void AG_InternalError::errors_as_string(uint8_t *buffer, const uint16_t len) const
{
    buffer[0] = 0;
...
AG_InternalError &internalerror()
{
    return instance;
}
...
void AP_stack_overflow(const char *thread_name)
{
    ...
    INTERNAL_ERROR(AG_InternalError::error_t::stack_overflow);
...
{
    INTERNAL_ERROR(AG_InternalError::error_t::mem_guard);
    if (!hal.util->get_soft_armed()) {
        ::printf("memory guard error size=%u\n", unsigned(size));
        AG_HAL::panic("memory guard size=%u\n", unsigned(size));
    }
}
