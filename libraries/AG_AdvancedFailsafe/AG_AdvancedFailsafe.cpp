...
AG_AdvancedFailsafe *AG_AdvancedFailsafe::_singleton;

extern const AG_HAL::HAL& hal;

// table of user settable parameters
const AG_Param::GroupInfo AG_AdvancedFailsafe::var_info[] = {
    // @Param: ENABLE
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE",       11, AG_AdvancedFailsafe, _enable, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    AP_GROUPINFO("MAN_PIN",     0, AG_AdvancedFailsafe, _manual_pin,    -1),
    ...
    // @User: Advanced
    AP_GROUPINFO("TERM_ACTION", 6, AG_AdvancedFailsafe, _terminate_action, 0),
    ...
    // @User: Advanced
    // @Values: -1:Disabled,49:BB Blue GP0 pin 4,50:AUXOUT1,51:AUXOUT2,52:AUXOUT3,53:AUXOUT4,54:AUXOUT5,55:AUXOUT6,57:BB Blue GP0 pin 3,113:BB Blue GP0 pin 6,116:BB Blue GP0 pin 5
    AP_GROUPINFO("TERM_PIN",    7, AG_AdvancedFailsafe, _terminate_pin,    -1),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("AMSL_LIMIT",   8, AG_AdvancedFailsafe, _amsl_limit,    0),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("AMSL_ERR_GPS", 9, AG_AdvancedFailsafe, _amsl_margin_gps,  -1),
    ...
    // @Units: hPa
    // @User: Advanced
    AP_GROUPINFO("QNH_PRESSURE", 10, AG_AdvancedFailsafe, _qnh_pressure,    0),
    ...
    // @User: Advanced
    AP_GROUPINFO("MAX_GPS_LOSS", 13, AG_AdvancedFailsafe, _max_gps_loss, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("MAX_COM_LOSS", 14, AG_AdvancedFailsafe, _max_comms_loss, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("GEOFENCE",     15, AG_AdvancedFailsafe, _enable_geofence_fs, 1),
    ...
    // @User: Advanced
    AP_GROUPINFO("RC",           16, AG_AdvancedFailsafe, _enable_RC_fs, 1),
    ...
    // @User: Advanced
    AP_GROUPINFO("RC_MAN_ONLY",    17, AG_AdvancedFailsafe, _rc_term_manual_only, 1),
    ...
    // @User: Advanced
    AP_GROUPINFO("DUAL_LOSS",      18, AG_AdvancedFailsafe, _enable_dual_loss, 1),
    ...
    // @User: Advanced
    // @Units: s
    AP_GROUPINFO("RC_FAIL_TIME",   19, AG_AdvancedFailsafe, _rc_fail_time_seconds,    0),
    ...
    // @User: Advanced
    // @Units: km
    AP_GROUPINFO("MAX_RANGE",   20, AG_AdvancedFailsafe, _max_range_km,    0),
...
// ArduPlane code
void
AG_AdvancedFailsafe::check(uint32_t last_valid_rc_ms)
{
    if (!_enable) {
        return;
    }
    ...
    if(_enable_geofence_fs) {
        const AG_Fence *ap_fence = AP::fence();
    ...
    if (_state != STATE_PREFLIGHT && !_terminate && _enable_RC_fs &&
        ...
        _rc_fail_time_seconds > 0 &&
            (AG_HAL::millis() - last_valid_rc_ms) > (_rc_fail_time_seconds * 1000.0f)) {
...
void
AG_AdvancedFailsafe::heartbeat(void)
{    
    if (!_enable) {
        return;
    }
...
bool
AG_AdvancedFailsafe::check_altlimit(void)
{    
    if (!_enable) {
        return false;
    }
    ...
    const AG_Baro &baro = AP::baro();
    const AG_GPS &gps = AP::gps();
    if (AG_HAL::millis() - baro.get_last_update() > 5000) {
 ...
 */
bool AG_AdvancedFailsafe::should_crash_vehicle(void)
{
    if (!_enable) {
        return false;
    }
...
bool AG_AdvancedFailsafe::gcs_terminate(bool should_terminate, const char *reason) {
    if (!_enable) {
 ...
 */
void AG_AdvancedFailsafe::max_range_update(void)
{
    if (_max_range_km <= 0) {
        return;
    }
    ...
    if (!_have_first_location) {
        if (AP::gps().status() < AG_GPS::GPS_OK_FIX_3D) {
            // wait for 3D fix
            return;
        }
    ...
    if (AP::gps().status() < AG_GPS::GPS_OK_FIX_2D) {
...
AG_AdvancedFailsafe *advancedfailsafe()
{
    return AG_AdvancedFailsafe::get_singleton();
}
