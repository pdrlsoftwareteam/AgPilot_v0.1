/*
  Code for handling AGPILOTLink2 signing
...
// storage object
StorageAccess GCS_AGPILOTLINK::_signing_storage(StorageManager::StorageKeys);
...
mavlink_signing_streams_t GCS_AGPILOTLINK::signing_streams;

// last time we saved the timestamp
uint32_t GCS_AGPILOTLINK::last_signing_save_ms;

bool GCS_AGPILOTLINK::signing_key_save(const struct SigningKey &key)
{
...
bool GCS_AGPILOTLINK::signing_key_load(struct SigningKey &key)
{
    if (_signing_storage.size() < sizeof(key)) {
        return false;
    }
 ...
 */
void GCS_AGPILOTLINK::handle_setup_signing(const mavlink_message_t &msg) const
{
    // setting up signing key when armed generally not useful /
    // possibly not a good idea
    if (hal.util->get_soft_armed()) {
        send_text(AGPILOT_SEVERITY_WARNING, "ERROR: Won't setup signing when armed");
        return;
    }
    ...
    if (!signing_key_save(key)) {
        send_text(AGPILOT_SEVERITY_WARNING, "ERROR: Failed to save signing key");
        return;
    }

    // activate it immediately on all links:
    for (uint8_t i=0; i<AGPILOTLINK_COMM_NUM_BUFFERS; i++) {
        GCS_AGPILOTLINK *backend = gcs().chan(i);
        if (backend == nullptr) {
            return;
        }
...
static const uint32_t accept_list[] = {
    AGPILOTLINK_MSG_ID_RADIO_STATUS,
    AGPILOTLINK_MSG_ID_RADIO
};
    
static bool accept_unsigned_callback(const mavlink_status_t *status, uint32_t msgId)
{
    if (status == mavlink_get_channel_status(AGPILOTLINK_COMM_0)) {
...
/*
  load signing key
 */
void GCS_AGPILOTLINK::load_signing_key(void)
{
    struct SigningKey key;
    ...
    signing.flags = AGPILOTLINK_SIGNING_FLAG_SIGN_OUTGOING;
 ...
 */
void GCS_AGPILOTLINK::update_signing_timestamp(uint64_t timestamp_usec)
{
    ...
    for (uint8_t i=0; i<AGPILOTLINK_COMM_NUM_BUFFERS; i++) {
        mavlink_channel_t chan = (mavlink_channel_t)(AGPILOTLINK_COMM_0 + i);
        mavlink_status_t *status = mavlink_get_channel_status(chan);
 ...
 */
void GCS_AGPILOTLINK::save_signing_timestamp(bool force_save_now)
{
 ...
 */
bool GCS_AGPILOTLINK::signing_enabled(void) const
{
    const mavlink_status_t *status = mavlink_get_channel_status(chan);
    if (status->signing && (status->signing->flags & AGPILOTLINK_SIGNING_FLAG_SIGN_OUTGOING)) {
        return true;
    }
    return false;
}
 ...
 */
uint8_t GCS_AGPILOTLINK::packet_overhead_chan(mavlink_channel_t chan)
{
    /*
    ...
    const mavlink_status_t *status = mavlink_get_channel_status(chan);
    if (status->signing && (status->signing->flags & AGPILOTLINK_SIGNING_FLAG_SIGN_OUTGOING)) {
        return AGPILOTLINK_NUM_NON_PAYLOAD_BYTES + AGPILOTLINK_SIGNATURE_BLOCK_LEN + reserved_space;
    }
    return AGPILOTLINK_NUM_NON_PAYLOAD_BYTES + reserved_space;
}
