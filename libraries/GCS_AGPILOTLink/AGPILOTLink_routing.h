...
/// @brief	handle routing of AGPILOTLink packets by ID
#pragma once
...
// we make more extensive use of AGPILOTLink forwarding
#define AGPILOTLINK_MAX_ROUTES 20

/*
  object to handle AGPILOTLink packet routing
 */
class AGPILOTLink_routing
{
    friend class GCS_AGPILOTLINK;
    
public:
    AGPILOTLink_routing(void);
    ...
    */
    bool check_and_forward(class GCS_AGPILOTLINK &link, const mavlink_message_t &msg);
...
private:
    ...
    struct route {
        ...
        uint8_t mavtype;
    } routes[AGPILOTLINK_MAX_ROUTES];
    
    // a channel mask to block routing as required
    uint8_t no_route_mask;
    
    // learn new routes
    void learn_route(GCS_AGPILOTLINK &link, const mavlink_message_t &msg);
    ...
    void handle_heartbeat(GCS_AGPILOTLINK &link, const mavlink_message_t &msg);
