...
{
    uint8_t ret = 0;
    ret |= GCS_AGPILOTLINK::active_channel_mask();
    ret |= GCS_AGPILOTLINK::streaming_channel_mask();
    ret &= ~GCS_AGPILOTLINK::private_channel_mask();
    return ret;
}
 ...
 */
void GCS::send_textv(AGPILOT_SEVERITY severity, const char *fmt, va_list arg_list)
{
...
void GCS::send_text(AGPILOT_SEVERITY severity, const char *fmt, ...)
{
    va_list arg_list;
...
void GCS::send_named_float(const char *name, float value) const
{
    ...
    packet.value = value;
    memcpy(packet.name, name, MIN(strlen(name), (uint8_t)AGPILOTLINK_MSG_NAMED_VALUE_FLOAT_FIELD_NAME_LEN));

    gcs().send_to_active_channels(AGPILOTLINK_MSG_ID_NAMED_VALUE_FLOAT,
                                  (const char *)&packet);
}
...
void GCS::enable_high_latency_connections(bool enabled)
{
    ...
    gcs().send_text(AGPILOT_SEVERITY_NOTICE, "High Latency %s", enabled ? "enabled" : "disabled");
}
...
/*
  ...
  protocol to take over the link if AGPILOTLink goes idle. It is used to
  allow for the AP_BLHeli pass-thru protocols to run on hal.serial(0)
 */
bool GCS::install_alternative_protocol(mavlink_channel_t c, GCS_AGPILOTLINK::protocol_handler_fn_t handler)
{
    GCS_AGPILOTLINK *link = chan(c);
    if (link == nullptr) {
        return false;
    }
...
void GCS::update_sensor_status_flags()
{
    ...
    control_sensors_present |= AGPILOT_SYS_STATUS_AHRS;
    if (ahrs.initialised()) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_AHRS;
        if (ahrs.healthy()) {
            if (!ahrs.have_inertial_nav() || ins.accel_calibrated_ok_all()) {
                control_sensors_health |= AGPILOT_SYS_STATUS_AHRS;
            }
        }
    }
#endif
    ...
    if (AP::compass().available()) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_3D_MAG;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_3D_MAG;
    }
    if (compass.available() && compass.healthy()) {
        control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_3D_MAG;
    }
#endif
    ...
    if (barometer.num_instances() > 0) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE;
        if (barometer.all_healthy()) {
            control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE;
        }
    }
#endif
    ...
    if (gps.status() > AP_GPS::NO_GPS) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_GPS;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_GPS;
    }
    if (gps.is_healthy() && gps.status() >= min_status_for_gps_healthy()) {
        control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_GPS;
    }
#endif

#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
    const AP_BattMonitor &battery = AP::battery();
    control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_BATTERY;
    if (battery.num_instances() > 0) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_BATTERY;
    }
    if (battery.healthy() && !battery.has_failsafed()) {
        control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_BATTERY;
    }
#endif

#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_AHRS)
    control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_3D_GYRO;
    control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_3D_ACCEL;
    if (!ins.calibrating()) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_3D_ACCEL;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_3D_GYRO;
        if (ins.get_accel_health_all()) {
            control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_3D_ACCEL;
        }
        if (ins.get_gyro_health_all() && ins.gyro_calibrated_ok_all()) {
            control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_3D_GYRO;
        }
    }
#endif
...
#endif
    if (logging_present) {
        control_sensors_present |= AGPILOT_SYS_STATUS_LOGGING;
    }
    if (logging_enabled) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_LOGGING;
    }
    if (logging_healthy) {
        control_sensors_health |= AGPILOT_SYS_STATUS_LOGGING;
    }
#endif  // HAL_LOGGING_ENABLED

    // set motors outputs as enabled if safety switch is not disarmed (i.e. either NONE or ARMED)
#if !defined(HAL_BUILD_AP_PERIPH)
    control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_MOTOR_OUTPUTS;
    if (hal.util->safety_switch_state() != AP_HAL::Util::SAFETY_DISARMED) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_MOTOR_OUTPUTS;
    }
    control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_MOTOR_OUTPUTS;
#endif
    ...
    if (ahrs.get_ekf_type() == 10) {
        ...
        control_sensors_health |= AGPILOT_SYS_STATUS_AHRS | AGPILOT_SYS_STATUS_SENSOR_GPS | AGPILOT_SYS_STATUS_SENSOR_3D_ACCEL | AGPILOT_SYS_STATUS_SENSOR_3D_GYRO;
    }
#endif
    ...
    if (fence != nullptr) {
        if (fence->sys_status_enabled()) {
            control_sensors_enabled |= AGPILOT_SYS_STATUS_GEOFENCE;
        }
        if (fence->sys_status_present()) {
            control_sensors_present |= AGPILOT_SYS_STATUS_GEOFENCE;
        }
        if (!fence->sys_status_failed()) {
            control_sensors_health |= AGPILOT_SYS_STATUS_GEOFENCE;
        }
    }
#endif
    ...
    if (airspeed && airspeed->enabled()) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE;
        const bool use = airspeed->use();
        const bool enabled = AP::ahrs().airspeed_sensor_enabled();
        if (use) {
            control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE;
        }
        if (airspeed->all_healthy() && (!use || enabled)) {
            control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE;
        }
    }
#endif
    ...
    if (optflow && optflow->enabled()) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_OPTICAL_FLOW;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_OPTICAL_FLOW;
    }
    if (optflow && optflow->healthy()) {
        control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_OPTICAL_FLOW;
    }
#endif
    ...
    if (visual_odom && visual_odom->enabled()) {
        control_sensors_present |= AGPILOT_SYS_STATUS_SENSOR_VISION_POSITION;
        control_sensors_enabled |= AGPILOT_SYS_STATUS_SENSOR_VISION_POSITION;
        if (visual_odom->healthy()) {
            control_sensors_health |= AGPILOT_SYS_STATUS_SENSOR_VISION_POSITION;
        }
    }
#endif
...
#if !defined(HAL_BUILD_AP_PERIPH)
    control_sensors_present |= AGPILOT_SYS_STATUS_PREARM_CHECK;
    if (AP::arming().get_enabled_checks()) {
        control_sensors_enabled |= AGPILOT_SYS_STATUS_PREARM_CHECK;
        if (hal.util->get_soft_armed() || AP_Notify::flags.pre_arm_check) {
            control_sensors_health |= AGPILOT_SYS_STATUS_PREARM_CHECK;
        }
    }
#endif
...
{
    GCS_AGPILOTLINK *link = gcs().chan(chan);
    if (link == nullptr) {
        return;
    }
 ...
 */
bool GCS_AGPILOTLINK::check_payload_size(uint16_t max_payload_len)
{
