...
#ifndef HAL_AGPILOTLINK_INTERVALS_FROM_FILES_ENABLED
#define HAL_AGPILOTLINK_INTERVALS_FROM_FILES_ENABLED (HAVE_FILESYSTEM_SUPPORT && BOARD_FLASH_SIZE > 1024)
#endif
...
#define PAYLOAD_SIZE(chan, id) (unsigned(GCS_AGPILOTLINK::packet_overhead_chan(chan)+AGPILOTLINK_MSG_ID_ ## id ## _LEN))
...
// GCS_AGPILOTLink object's methods.  It inserts code which will
...
// output
#define CHECK_PAYLOAD_SIZE(id) if (!check_payload_size(AGPILOTLINK_MSG_ID_ ## id ## _LEN)) return false
...
#define AGPILOT_STREAM_ENTRY(stream_name)           \
    {                                           \
        GCS_AGPILOTLINK::stream_name,               \
        stream_name ## _msgs,                   \
        ARRAY_SIZE(stream_name ## _msgs)        \
    }
#define AGPILOT_STREAM_TERMINATOR { (streams)0, nullptr, 0 }
...
#define GCS_AGPILOTLINK_CHAN_METHOD_DEFINITIONS(subclass_name) \
...
#define GCS_AGPILOTLINK_NUM_STREAM_RATES 10
class GCS_AGPILOTLINK_Parameters
{
public:

    GCS_AGPILOTLINK_Parameters();

    static const struct AP_Param::GroupInfo        var_info[];

    // saveable rate of each stream
    AP_Int16        streamRates[GCS_AGPILOTLINK_NUM_STREAM_RATES];
};
...
class GCS_AGPILOTLINK_InProgress
{
public:
    enum class Type {
        NONE,
    ...
    bool conclude(AGPILOT_RESULT result);
    ...
    static class GCS_AGPILOTLINK_InProgress *get_task(AGPILOT_CMD cmd, Type t, uint8_t sysid, uint8_t compid, mavlink_channel_t chan);
...
private:
    ...
    bool send_ack(AGPILOT_RESULT result);

    static GCS_AGPILOTLINK_InProgress in_progress_tasks[1];
...
/// @brief	AGPILOTLink transport control class
///
class GCS_AGPILOTLINK
{
public:
    ...
    GCS_AGPILOTLINK(GCS_AGPILOTLINK_Parameters &parameters, AP_HAL::UARTDriver &uart);
    virtual ~GCS_AGPILOTLINK() {}

    void        update_receive(uint32_t max_time_us=1000);
    void        update_send();
    bool        init(uint8_t instance);
    void        send_message(enum ap_message id);
    void        send_text(AGPILOT_SEVERITY severity, const char *fmt, ...) const FMT_PRINTF(3, 4);
    ...
    void send_mission_ack(const mavlink_message_t &msg,
                          AGPILOT_MISSION_TYPE mission_type,
                          AGPILOT_MISSION_RESULT result) const {
    ...
    // send a mavlink_message_t out this GCS_AGPILOTLINK connection.
    ...
    // GCS_AGPILOTLINK_Parameters to be able to use it.  This is an
    // extensive change, so we 'll just keep them in sync with a
    // static assert for now:
    static_assert(NUM_STREAMS == GCS_AGPILOTLINK_NUM_STREAM_RATES, "num streams must equal num stream rates");

    bool is_high_bandwidth() { return chan == AGPILOTLINK_COMM_0; }
    // return true if this channel has hardware flow control
    bool have_flow_control();

    bool is_active() const {
        return GCS_AGPILOTLINK::active_channel_mask() & (1 << (chan-AGPILOTLINK_COMM_0));
    }
    ...
    // lock a channel, preventing use by AGPILOTLink
    void lock(bool _lock) {
        _locked = _lock;
    }
    // returns true if this channel isn't available for AGPILOTLink
    bool locked() const {
        return _locked;
    }
    ...
    /*
      send a AGPILOTLink message to all components with this vehicle's system id
    ...
    */
    static void disable_channel_routing(mavlink_channel_t chan) { routing.no_route_mask |= (1U<<(chan-AGPILOTLINK_COMM_0)); }
    ...
    // same as above, but returns a pointer to the GCS_AGPILOTLINK object
    // corresponding to the channel
    static GCS_AGPILOTLINK *find_by_mavtype_and_compid(uint8_t mav_type, uint8_t compid, uint8_t &sysid);
    ...
    AGPILOT_RESULT set_message_interval(uint32_t msg_id, int32_t interval_us);

protected:

    bool mavlink_coordinate_frame_to_location_alt_frame(AGPILOT_FRAME coordinate_frame,
    ...
    virtual AGPILOT_MODE base_mode() const = 0;
    AGPILOT_STATE system_status() const;
    virtual AGPILOT_STATE vehicle_system_status() const = 0;

    virtual AGPILOT_VTOL_STATE vtol_state() const { return AGPILOT_VTOL_STATE_UNDEFINED; }
    virtual AGPILOT_LANDED_STATE landed_state() const { return AGPILOT_LANDED_STATE_UNDEFINED; }

    // return a AGPILOTLink parameter type given a AP_Param type
    static AGPILOT_PARAM_TYPE mav_param_type(enum ap_var_type t);
    ...
    AGPILOT_RESULT handle_command_int_do_set_home(const mavlink_command_int_t &packet);
    virtual AGPILOT_RESULT handle_command_int_packet(const mavlink_command_int_t &packet);

    virtual bool set_home_to_current_location(bool lock) = 0;
    virtual bool set_home(const Location& loc, bool lock) = 0;

    virtual AGPILOT_RESULT handle_command_component_arm_disarm(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_set_home(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_aux_function(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_storage_format(const mavlink_command_int_t &packet, const mavlink_message_t &msg);
    ...
    virtual AGPILOT_RESULT handle_preflight_reboot(const mavlink_command_long_t &packet, const mavlink_message_t &msg);

    // reset a message interval via mavlink:
    AGPILOT_RESULT handle_command_set_message_interval(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_get_message_interval(const mavlink_command_long_t &packet);
    bool get_ap_message_interval(ap_message id, uint16_t &interval_ms) const;
    AGPILOT_RESULT handle_command_request_message(const mavlink_command_long_t &packet);

    AGPILOT_RESULT handle_rc_bind(const mavlink_command_long_t &packet);

    virtual AGPILOT_RESULT handle_flight_termination(const mavlink_command_long_t &packet);

    void handle_send_autopilot_version(const mavlink_message_t &msg);
    AGPILOT_RESULT handle_command_request_autopilot_capabilities(const mavlink_command_long_t &packet);
    ...
    AGPILOT_RESULT handle_command_run_prearm_checks(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_preflight_set_sensor_offsets(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_flash_bootloader(const mavlink_command_long_t &packet);

    // generally this should not be overridden; Plane overrides it to ensure
    // failsafe isn't triggered during calibration
    virtual AGPILOT_RESULT handle_command_preflight_calibration(const mavlink_command_long_t &packet, const mavlink_message_t &msg);

    virtual AGPILOT_RESULT _handle_command_preflight_calibration(const mavlink_command_long_t &packet, const mavlink_message_t &msg);
    virtual AGPILOT_RESULT _handle_command_preflight_calibration_baro(const mavlink_message_t &msg);

    AGPILOT_RESULT handle_command_preflight_can(const mavlink_command_long_t &packet);

    virtual AGPILOT_RESULT handle_command_do_set_mission_current(const mavlink_command_long_t &packet);

    AGPILOT_RESULT handle_command_battery_reset(const mavlink_command_long_t &packet);
    void handle_command_long(const mavlink_message_t &msg);
    AGPILOT_RESULT handle_command_accelcal_vehicle_pos(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_set_roi_sysid(const uint8_t sysid);
    AGPILOT_RESULT handle_command_do_set_roi_sysid(const mavlink_command_int_t &packet);
    AGPILOT_RESULT handle_command_do_set_roi_sysid(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_set_roi_none();

    virtual AGPILOT_RESULT handle_command_mount(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_mag_cal(const mavlink_command_long_t &packet);
    virtual AGPILOT_RESULT handle_command_long_packet(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_camera(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_send_banner(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_set_roi(const mavlink_command_int_t &packet);
    AGPILOT_RESULT handle_command_do_set_roi(const mavlink_command_long_t &packet);
    virtual AGPILOT_RESULT handle_command_do_set_roi(const Location &roi_loc);
    AGPILOT_RESULT handle_command_do_gripper(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_sprayer(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_set_mode(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_get_home_position(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_do_fence_enable(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_debug_trap(const mavlink_command_long_t &packet);
    AGPILOT_RESULT handle_command_set_ekf_source_set(const mavlink_command_long_t &packet);

    /*
      handle AGPILOT_CMD_CAN_FORWARD and CAN_FRAME messages for CAN over AGPILOTLink
     */
    void can_frame_callback(uint8_t bus, const AP_HAL::CANFrame &);
    AGPILOT_RESULT handle_can_forward(const mavlink_command_long_t &packet, const mavlink_message_t &msg);
    void handle_can_frame(const mavlink_message_t &msg) const;

    void handle_optical_flow(const mavlink_message_t &msg);

    AGPILOT_RESULT handle_fixed_mag_cal_yaw(const mavlink_command_long_t &packet);
    ...
    AGPILOT_RESULT handle_control_high_latency(const mavlink_command_long_t &packet);
    ...
    static void convert_COMMAND_LONG_to_COMMAND_INT(const mavlink_command_long_t &in, mavlink_command_int_t &out, AGPILOT_FRAME frame = AGPILOT_FRAME_GLOBAL_RELATIVE_ALT);

    // methods to extract a Location object from a command_long or command_int
    bool location_from_command_t(const mavlink_command_long_t &in, AGPILOT_FRAME in_frame, Location &out);
...
private:
    ...
    AGPILOT_RESULT _set_mode_common(const AGPILOT_MODE base_mode, const uint32_t custom_mode);
    ...
    AGPILOT_RESULT handle_servorelay_message(const mavlink_command_long_t &packet);

    static bool command_long_stores_location(const AGPILOT_CMD command);
    ...
    uint16_t get_interval_for_stream(GCS_AGPILOTLINK::streams id) const;
    ...
    void initialise_message_intervals_for_stream(GCS_AGPILOTLINK::streams id);
    ...
    static AGPILOTLink_routing routing;
    ...
    struct {
        GCS_AGPILOTLINK::protocol_handler_fn_t handler;
    ...
    // true if we should NOT do AGPILOTLink on this port (usually because
...
public:
    ...
    virtual AGPILOT_TYPE frame_type() const = 0;
    ...
    void send_text(AGPILOT_SEVERITY severity, const char *fmt, ...) FMT_PRINTF(3, 4);
    void send_textv(AGPILOT_SEVERITY severity, const char *fmt, va_list arg_list);
    virtual void send_textv(AGPILOT_SEVERITY severity, const char *fmt, va_list arg_list, uint8_t mask);
    uint8_t statustext_send_channel_mask() const;

    virtual GCS_AGPILOTLINK *chan(const uint8_t ofs) = 0;
    virtual const GCS_AGPILOTLINK *chan(const uint8_t ofs) const = 0;
    ...
    // AGPILOT_MISSION_TYPE, taking advantage of the fact that fence,
    ...
    class MissionItemProtocol *get_prot_for_mission_type(const AGPILOT_MISSION_TYPE mission_type) const;
    void try_send_queued_message_for_type(AGPILOT_MISSION_TYPE type) const;
    ...
    bool install_alternative_protocol(mavlink_channel_t chan, GCS_AGPILOTLINK::protocol_handler_fn_t handler);

    // get the VFR_HUD throttle
    int16_t get_hud_throttle(void) const {
        const GCS_AGPILOTLINK *link = chan(0);
        if (link == nullptr) {
            return 0;
        }
    ...
    AGPILOT_RESULT set_message_interval(uint8_t port_num, uint32_t msg_id, int32_t interval_us);
...
protected:
    ...
    virtual GCS_AGPILOTLINK *new_gcs_mavlink_backend(GCS_AGPILOTLINK_Parameters &params,
    ...
    GCS_AGPILOTLINK_Parameters chan_parameters[AGPILOTLINK_COMM_NUM_BUFFERS];
    uint8_t _num_gcs;
    GCS_AGPILOTLINK *_chan[AGPILOTLINK_COMM_NUM_BUFFERS];

private:

    static GCS *_singleton;

    void create_gcs_mavlink_backend(GCS_AGPILOTLINK_Parameters &params,
    ...
    // this contains the index of the GCS_AGPILOTLINK backend we will
