/*
   GCS AGPILOTLink functions related to FTP
...
struct GCS_AGPILOTLINK::ftp_state GCS_AGPILOTLINK::ftp;

// timeout for session inactivity
#define FTP_SESSION_TIMEOUT 3000

bool GCS_AGPILOTLINK::ftp_init(void) {
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&GCS_AGPILOTLINK::ftp_worker, void),
...
failed:
    ...
    gcs().send_text(AGPILOT_SEVERITY_WARNING, "failed to initialize AGPILOTFTP");

    return false;
}

void GCS_AGPILOTLINK::handle_file_transfer_protocol(const mavlink_message_t &msg) {
    if (ftp_init()) {
...
bool GCS_AGPILOTLINK::send_ftp_reply(const pending_ftp &reply)
{
    /*
...
void GCS_AGPILOTLINK::ftp_error(struct pending_ftp &response, FTP_ERROR error) {
...
void GCS_AGPILOTLINK::ftp_push_replies(pending_ftp &reply)
{
...
void GCS_AGPILOTLINK::ftp_worker(void) {
    pending_ftp request;
    pending_ftp reply = {};
        ...
        if (request.session != ftp.current_session &&
            ...
            switch (request.opcode) {
                ...
                default:
                    ...
                    gcs().send_text(AGPILOT_SEVERITY_DEBUG, "Unsupported FTP: %d", static_cast<int>(request.opcode));
                    ftp_error(reply, FTP_ERROR::Fail);
                    break;
            }
        }
...
int GCS_AGPILOTLINK::gen_dir_entry(char *dest, size_t space, const char *path, const struct dirent * entry) {
...
void GCS_AGPILOTLINK::ftp_list_dir(struct pending_ftp &request, struct pending_ftp &response) {
