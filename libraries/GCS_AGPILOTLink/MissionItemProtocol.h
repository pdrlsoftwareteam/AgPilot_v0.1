...
// for rally point uploads.  These objects are static in GCS_AGPILOTLINK
...
public:
    ...
    void handle_mission_request_list(const class GCS_AGPILOTLINK &link,
                                     const mavlink_mission_request_list_t &packet,
                                     const mavlink_message_t &msg);
    void handle_mission_request_int(GCS_AGPILOTLINK &link,
                                    const mavlink_mission_request_int_t &packet,
                                    const mavlink_message_t &msg);
    void handle_mission_request(GCS_AGPILOTLINK &link,
                                const mavlink_mission_request_t &packet,
                                const mavlink_message_t &msg);

    void handle_mission_count(class GCS_AGPILOTLINK &link,
                              const mavlink_mission_count_t &packet,
                              const mavlink_message_t &msg);
    void handle_mission_write_partial_list(GCS_AGPILOTLINK &link,
                                           const mavlink_message_t &msg,
    ...
    void handle_mission_clear_all(const GCS_AGPILOTLINK &link,
                                  const mavlink_message_t &msg);

    void queued_request_send();
    void update();

    bool active_link_is(const GCS_AGPILOTLINK *_link) const { return _link == link; };

    virtual AGPILOT_MISSION_TYPE mission_type() const = 0;

    bool receiving; // currently sending requests and expecting items

    // a method for GCS_AGPILOTLINK to send warnings about received
    // MISSION_ITEM messages (we should be getting MISSION_ITEM_INT)
    void send_mission_item_warning();

protected:

    GCS_AGPILOTLINK *link; // link currently receiving waypoints on
...
private:
    ...
    virtual AGPILOT_MISSION_RESULT get_item(const GCS_AGPILOTLINK &_link,
                                        const mavlink_message_t &msg,
                                        const mavlink_mission_request_int_t &packet,
                                        mavlink_mission_item_int_t &ret_packet) = 0;

    void init_send_requests(GCS_AGPILOTLINK &_link,
                            const mavlink_message_t &msg,
                            const int16_t _request_first,
                            const int16_t _request_last);
    virtual AGPILOT_MISSION_RESULT allocate_receive_resources(const uint16_t count) WARN_IF_UNUSED {
        return AGPILOT_MISSION_ACCEPTED;
    }
    virtual AGPILOT_MISSION_RESULT allocate_update_resources() WARN_IF_UNUSED {
        return AGPILOT_MISSION_ACCEPTED;
    }
    virtual void free_upload_resources() { }

    void send_mission_ack(const mavlink_message_t &msg, AGPILOT_MISSION_RESULT result) const;
    void send_mission_ack(const GCS_AGPILOTLINK &link, const mavlink_message_t &msg, AGPILOT_MISSION_RESULT result) const;

    virtual uint16_t item_count() const = 0;
    virtual uint16_t max_items() const = 0;

    virtual AGPILOT_MISSION_RESULT replace_item(const mavlink_mission_item_int_t &mission_item_int) = 0;
    virtual AGPILOT_MISSION_RESULT append_item(const mavlink_mission_item_int_t &mission_item_int) = 0;

    // complete - method called when transfer is complete - backends
    // are expected to override this method to do any required tidy up.
    virtual AGPILOT_MISSION_RESULT complete(const GCS_AGPILOTLINK &_link) {
        return AGPILOT_MISSION_ACCEPTED;
    };
    // transfer_is_complete - tidy up after a transfer is complete;
    // this method will call complete() so the backends can do their
    // bit.
    void transfer_is_complete(const GCS_AGPILOTLINK &_link, const mavlink_message_t &msg);
    ...
    bool mavlink2_requirement_met(const GCS_AGPILOTLINK &_link, const mavlink_message_t &msg) const;
};
