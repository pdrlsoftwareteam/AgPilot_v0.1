/*
  AGPILOTLink SERIAL_CONTROL handling
 */
 ...
 */
void GCS_AGPILOTLINK::handle_serial_control(const mavlink_message_t &msg)
{
    ...
    case SERIAL_CONTROL_DEV_TELEM1: {
        GCS_AGPILOTLINK *link = gcs().chan(1);
        if (link == nullptr) {
            break;
        }
    ...
    case SERIAL_CONTROL_DEV_TELEM2: {
        GCS_AGPILOTLINK *link = gcs().chan(2);
        if (link == nullptr) {
            break;
        }
        stream = port = link->get_uart();
    ...
    case SERIAL_CONTROL_SERIAL0 ... SERIAL_CONTROL_SERIAL9: {
        ...
        for (uint8_t i=0; i<gcs().num_gcs(); i++) {
            GCS_AGPILOTLINK *link = gcs().chan(i);
...
more_data:
    ...
    _mav_finalize_message_chan_send(chan, 
                                    AGPILOTLINK_MSG_ID_SERIAL_CONTROL,
                                    (const char *)&packet,
                                    AGPILOTLINK_MSG_ID_SERIAL_CONTROL_MIN_LEN,
                                    AGPILOTLINK_MSG_ID_SERIAL_CONTROL_LEN,
                                    AGPILOTLINK_MSG_ID_SERIAL_CONTROL_CRC);
