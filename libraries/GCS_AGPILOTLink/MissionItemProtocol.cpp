...
void MissionItemProtocol::init_send_requests(GCS_AGPILOTLINK &_link,
...
void MissionItemProtocol::handle_mission_clear_all(const GCS_AGPILOTLINK &_link,
                                                   const mavlink_message_t &msg)
{
    bool success = true;
    success = success && !receiving;
    success = success && clear_all_items();
    send_mission_ack(_link, msg, success ? AGPILOT_MISSION_ACCEPTED : AGPILOT_MISSION_ERROR);
}

bool MissionItemProtocol::mavlink2_requirement_met(const GCS_AGPILOTLINK &_link, const mavlink_message_t &msg) const
{
    // need mavlink2 to do mission types other than mission:
    if (mission_type() == AGPILOT_MISSION_TYPE_MISSION) {
        return true;
    }
    if (!_link.sending_mavlink1()) {
        return true;
    }
    gcs().send_text(AGPILOT_SEVERITY_WARNING, "Need mavlink2 for item transfer");
    send_mission_ack(_link, msg, AGPILOT_MISSION_UNSUPPORTED);
    return false;
}

void MissionItemProtocol::handle_mission_count(
    GCS_AGPILOTLINK &_link,
    ...
    const mavlink_message_t &msg)
{
    ...
    if (receiving) {
        ...
        if (msg.sysid != dest_sysid || msg.compid != dest_compid) {
            ...
            send_mission_ack(_link, msg, AGPILOT_MISSION_DENIED);
            return;
        }
    ...
    if (packet.count > max_items()) {
        ...
        send_mission_ack(_link, msg, AGPILOT_MISSION_NO_SPACE);
        GCS_SEND_TEXT(AGPILOT_SEVERITY_WARNING, "Only %u items are supported", (unsigned)max_items());
        return;
    }

    AGPILOT_MISSION_RESULT ret_alloc = allocate_receive_resources(packet.count);
    if (ret_alloc != AGPILOT_MISSION_ACCEPTED) {
...
void MissionItemProtocol::handle_mission_request_int(GCS_AGPILOTLINK &_link,
                                                     ...
                                                     const mavlink_message_t &msg)
{
    ...
    const AGPILOT_MISSION_RESULT result_code = get_item(_link, msg, packet, ret_packet);

    if (result_code != AGPILOT_MISSION_ACCEPTED) {
        // send failure message
        send_mission_ack(_link, msg, result_code);
        return;
    }

    _link.send_message(AGPILOTLINK_MSG_ID_MISSION_ITEM_INT, (const char*)&ret_packet);
}

void MissionItemProtocol::handle_mission_request(GCS_AGPILOTLINK &_link,
...
)
{
    ...
    AGPILOT_MISSION_RESULT ret = get_item(_link, msg, request_int, item_int);
    ...
    if (!mission_request_warning_sent) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "got MISSION_REQUEST; use MISSION_REQUEST_INT!");
    }

    // buffer space is checked by send_message
    _link.send_message(AGPILOTLINK_MSG_ID_MISSION_ITEM, (const char*)&ret_packet);
}
...
void MissionItemProtocol::send_mission_item_warning()
{
    ...
    gcs().send_text(AGPILOT_SEVERITY_WARNING, "got MISSION_ITEM; GCS should send MISSION_ITEM_INT");
}

void MissionItemProtocol::handle_mission_write_partial_list(GCS_AGPILOTLINK &_link,
                                                            const mavlink_message_t &msg,
                                                            const mavlink_mission_write_partial_list_t &packet)
{
    ...
    if ((unsigned)packet.start_index > item_count() ||
        ...
        gcs().send_text(AGPILOT_SEVERITY_WARNING,"Flight plan update rejected"); // FIXME: Remove this anytime after 2020-01-22
        send_mission_ack(_link, msg, AGPILOT_MISSION_ERROR);
        return;
    }

    AGPILOT_MISSION_RESULT ret_alloc = allocate_update_resources();
    if (ret_alloc != AGPILOT_MISSION_ACCEPTED) {
        send_mission_ack(_link, msg, ret_alloc);
        return;
    }
...
void MissionItemProtocol::handle_mission_item(const mavlink_message_t &msg, const mavlink_mission_item_int_t &cmd)
{
    ...
    if (cmd.seq != request_i) {
        send_mission_ack(msg, AGPILOT_MISSION_INVALID_SEQUENCE);
        return;
    }
    // make sure the item is coming from the system that initiated the upload
    if (msg.sysid != dest_sysid) {
        send_mission_ack(msg, AGPILOT_MISSION_DENIED);
        return;
    }
...
void MissionItemProtocol::transfer_is_complete(const GCS_AGPILOTLINK &_link, const mavlink_message_t &msg)
{
    const AGPILOT_MISSION_RESULT result = complete(_link);
...
void MissionItemProtocol::send_mission_ack(const GCS_AGPILOTLINK &_link,
                                           const mavlink_message_t &msg,
                                           AGPILOT_MISSION_RESULT result) const
{
    ...
    if (tnow - timelast_receive_ms > upload_timeout_ms) {
        ...
        if (HAVE_PAYLOAD_SPACE(chan, MISSION_ACK)) {
            mavlink_msg_mission_ack_send(chan,
                                         dest_sysid,
                                         dest_compid,
                                         AGPILOT_MISSION_OPERATION_CANCELLED,
                                         mission_type());
        }
        link = nullptr;
        free_upload_resources();
