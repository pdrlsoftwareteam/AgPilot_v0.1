                                                         ...
                                                         mavlink_mission_item_int_t &ret_packet)
{
    ...
    AGPILOT_CMD ret_cmd = AGPILOT_CMD_NAV_FENCE_POLYGON_VERTEX_INCLUSION; // initialised to avoid compiler warning
    float p1 = 0;
    switch (fenceitem.type) {
    case AC_PolyFenceType::POLYGON_INCLUSION:
        ret_cmd = AGPILOT_CMD_NAV_FENCE_POLYGON_VERTEX_INCLUSION;
        p1 = fenceitem.vertex_count;
        break;
    case AC_PolyFenceType::POLYGON_EXCLUSION:
        ret_cmd = AGPILOT_CMD_NAV_FENCE_POLYGON_VERTEX_EXCLUSION;
        p1 = fenceitem.vertex_count;
        break;
    case AC_PolyFenceType::RETURN_POINT:
        ret_cmd = AGPILOT_CMD_NAV_FENCE_RETURN_POINT;
        break;
    case AC_PolyFenceType::CIRCLE_EXCLUSION:
        ret_cmd = AGPILOT_CMD_NAV_FENCE_CIRCLE_EXCLUSION;
        p1 = fenceitem.radius;
        break;
    case AC_PolyFenceType::CIRCLE_INCLUSION:
        ret_cmd = AGPILOT_CMD_NAV_FENCE_CIRCLE_INCLUSION;
        p1 = fenceitem.radius;
        break;
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::get_item(const GCS_AGPILOTLINK &_link,
...
static AGPILOT_MISSION_RESULT convert_MISSION_ITEM_INT_to_AC_PolyFenceItem(const mavlink_mission_item_int_t &mission_item_int, AC_PolyFenceItem &ret)
{
    if (mission_item_int.frame != AGPILOT_FRAME_GLOBAL &&
        mission_item_int.frame != AGPILOT_FRAME_GLOBAL_INT &&
        mission_item_int.frame != AGPILOT_FRAME_GLOBAL_RELATIVE_ALT &&
        mission_item_int.frame != AGPILOT_FRAME_GLOBAL_RELATIVE_ALT_INT &&
        mission_item_int.frame != AGPILOT_FRAME_GLOBAL_TERRAIN_ALT &&
        mission_item_int.frame != AGPILOT_FRAME_GLOBAL_TERRAIN_ALT_INT) {
        return AGPILOT_MISSION_UNSUPPORTED_FRAME;
    }

    switch (mission_item_int.command) {
    case AGPILOT_CMD_NAV_FENCE_POLYGON_VERTEX_INCLUSION:
        ret.type = AC_PolyFenceType::POLYGON_INCLUSION;
        ret.vertex_count = mission_item_int.param1;
        break;
    case AGPILOT_CMD_NAV_FENCE_POLYGON_VERTEX_EXCLUSION:
        ret.type = AC_PolyFenceType::POLYGON_EXCLUSION;
        ret.vertex_count = mission_item_int.param1;
        break;
    case AGPILOT_CMD_NAV_FENCE_RETURN_POINT:
        ret.type = AC_PolyFenceType::RETURN_POINT;
        break;
    case AGPILOT_CMD_NAV_FENCE_CIRCLE_EXCLUSION:
        ret.type = AC_PolyFenceType::CIRCLE_EXCLUSION;
        ret.radius = mission_item_int.param1;
        break;
    case AGPILOT_CMD_NAV_FENCE_CIRCLE_INCLUSION:
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::replace_item(const mavlink_mission_item_int_t &mission_item_int)
{
    ...
    const AGPILOT_MISSION_RESULT ret = convert_MISSION_ITEM_INT_to_AC_PolyFenceItem(mission_item_int, _new_items[mission_item_int.seq]);
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::append_item(const mavlink_mission_item_int_t &mission_item_int)
{
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::complete(const GCS_AGPILOTLINK &_link)
{
    if (_updated_mask != nullptr) {
        ...
        for (uint16_t i=0; i<_new_items_count; i++) {
            if (!(_updated_mask[i/8] & (1U<<(i%8)))) {
                if (!_fence.polyfence().get_item(i, _new_items[i])) {
                    _link.send_text(AGPILOT_SEVERITY_INFO, "Error replacing item (%u)", i);
                    return AGPILOT_MISSION_ERROR;
                }
            }
        }
...
{
    link->send_text(AGPILOT_SEVERITY_WARNING, "Fence upload timeout");
}
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::allocate_receive_resources(const uint16_t count)
{
    if (_new_items != nullptr) {
    ...
    if (allocation_size != 0) {
        ...
        if (_new_items == nullptr) {
            gcs().send_text(AGPILOT_SEVERITY_WARNING, "Out of memory for upload");
            return AGPILOT_MISSION_ERROR;
        }
    }
...
AGPILOT_MISSION_RESULT MissionItemProtocol_Fence::allocate_update_resources()
{
    ...
    AGPILOT_MISSION_RESULT ret = allocate_receive_resources(_item_count);
    if (ret != AGPILOT_MISSION_ACCEPTED) {
        delete[] _updated_mask;
        _updated_mask = nullptr;
        return ret;
