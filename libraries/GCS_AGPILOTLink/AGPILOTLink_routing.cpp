...
/// @file	AGPILOTLink_routing.h
/// @brief	handle routing of AGPILOTLink packets by sysid/componentid
...
// constructor
AGPILOTLink_routing::AGPILOTLink_routing(void) : num_routes(0) {}

/*
  forward a AGPILOTLink message to the right port. This also
  automatically learns the route for the sender if it is not
  already known.
  
  This returns true if the message should be processed locally

  Theory of AGPILOTLink routing:
...
*/
bool AGPILOTLink_routing::check_and_forward(GCS_AGPILOTLINK &in_link, const mavlink_message_t &msg)
{
    ...
    if (msg.msgid == AGPILOTLINK_MSG_ID_HEARTBEAT) {
    ...
    if (msg.msgid == AGPILOTLINK_MSG_ID_ADSB_VEHICLE) {
    ...
    bool sent_to_chan[AGPILOTLINK_COMM_NUM_BUFFERS];
    ...
    for (uint8_t i=0; i<num_routes; i++) {
        ...
        GCS_AGPILOTLINK *out_link = gcs().chan(routes[i].channel);
        if (out_link == nullptr) {
            // this is bad
            continue;
        }
...
*/
void AGPILOTLink_routing::send_to_components(uint32_t msgid, const char *pkt, uint8_t pkt_len)
{
...
void AGPILOTLink_routing::send_to_components(const char *pkt, const mavlink_msg_entry_t *entry, const uint8_t pkt_len)
{
    bool sent_to_chan[AGPILOTLINK_COMM_NUM_BUFFERS] {};
    ...
    for (uint8_t i=0; i<num_routes; i++) {
        ...
        if (comm_get_txspace(routes[i].channel) <
            ((uint16_t)entry->max_msg_len) + GCS_AGPILOTLINK::packet_overhead_chan(routes[i].channel)) {
 ...
 */
bool AGPILOTLink_routing::find_by_mavtype(uint8_t mavtype, uint8_t &sysid, uint8_t &compid, mavlink_channel_t &channel)
{
    // check learned routes
    for (uint8_t i=0; i<num_routes; i++) {
...
/*
  ...
  returns true if a match is found
 */
bool AGPILOTLink_routing::find_by_mavtype_and_compid(uint8_t mavtype, uint8_t compid, uint8_t &sysid, mavlink_channel_t &channel) const
{
    for (uint8_t i=0; i<num_routes; i++) {
...
*/
void AGPILOTLink_routing::learn_route(GCS_AGPILOTLINK &in_link, const mavlink_message_t &msg)
{
    uint8_t i;
    if (msg.sysid == 0) {
    ...
    if (msg.sysid == mavlink_system.sysid &&
        msg.compid == AGPILOT_COMP_ID_ALL) {
    ...
    for (i=0; i<num_routes; i++) {
        if (routes[i].sysid == msg.sysid &&
            ...
            if (routes[i].mavtype == 0 && msg.msgid == AGPILOTLINK_MSG_ID_HEARTBEAT) {
                routes[i].mavtype = mavlink_msg_heartbeat_get_type(&msg);
            }
            break;
        }
    }
    if (i == num_routes && i<AGPILOTLINK_MAX_ROUTES) {
...
*/
void AGPILOTLink_routing::handle_heartbeat(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
    uint16_t mask = GCS_AGPILOTLINK::active_channel_mask() & ~GCS_AGPILOTLINK::private_channel_mask();

    const mavlink_channel_t in_channel = link.get_chan();

    // don't send on the incoming channel. This should only matter if
    // the routing table is full
    mask &= ~(1U<<(in_channel-AGPILOTLINK_COMM_0));
    ...
    for (uint8_t i=0; i<num_routes; i++) {
        if (routes[i].sysid == msg.sysid && routes[i].compid == msg.compid) {
            mask &= ~(1U<<((unsigned)(routes[i].channel-AGPILOTLINK_COMM_0)));
        }
    }
    ...
    for (uint8_t i=0; i<AGPILOTLINK_COMM_NUM_BUFFERS; i++) {
        if (mask & (1U<<i)) {
            mavlink_channel_t channel = (mavlink_channel_t)(AGPILOTLINK_COMM_0 + i);
            if (comm_get_txspace(channel) >= ((uint16_t)msg.len) +
                GCS_AGPILOTLINK::packet_overhead_chan(channel)) {
#if ROUTING_DEBUG
...
*/
void AGPILOTLink_routing::get_targets(const mavlink_message_t &msg, int16_t &sysid, int16_t &compid)
{
    const mavlink_msg_entry_t *msg_entry = mavlink_get_msg_entry(msg.msgid);
    if (msg_entry == nullptr) {
        return;
    }
    if (msg_entry->flags & AGPILOT_MSG_ENTRY_FLAG_HAVE_TARGET_SYSTEM) {
        sysid = _AGPILOT_RETURN_uint8_t(&msg,  msg_entry->target_system_ofs);
    }
    if (msg_entry->flags & AGPILOT_MSG_ENTRY_FLAG_HAVE_TARGET_COMPONENT) {
        compid = _AGPILOT_RETURN_uint8_t(&msg,  msg_entry->target_component_ofs);
    }
}
