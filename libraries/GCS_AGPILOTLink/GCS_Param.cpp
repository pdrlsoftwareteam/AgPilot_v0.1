/*
   GCS AGPILOTLink functions related to parameter handling
...
ObjectBuffer<GCS_AGPILOTLINK::pending_param_request> GCS_AGPILOTLINK::param_requests(20);
ObjectBuffer<GCS_AGPILOTLINK::pending_param_reply> GCS_AGPILOTLINK::param_replies(5);

bool GCS_AGPILOTLINK::param_timer_registered;
 ...
 */
void
GCS_AGPILOTLINK::queued_param_send()
{
    ...
    const uint16_t size_for_one_param_value_msg = AGPILOTLINK_MSG_ID_PARAM_VALUE_LEN + packet_overhead();
 ...
 */
bool GCS_AGPILOTLINK::have_flow_control(void)
{
    if (_port == nullptr) {
        return false;
    }
...
/*
  ...
  ground station.
 */
void GCS_AGPILOTLINK::handle_request_data_stream(const mavlink_message_t &msg)
{
    ...
    case AGPILOT_DATA_STREAM_ALL:
        ...
        break;
    case AGPILOT_DATA_STREAM_RAW_SENSORS:
        stream_id = STREAM_RAW_SENSORS;
        break;
    case AGPILOT_DATA_STREAM_EXTENDED_STATUS:
        stream_id = STREAM_EXTENDED_STATUS;
        break;
    case AGPILOT_DATA_STREAM_RC_CHANNELS:
        stream_id = STREAM_RC_CHANNELS;
        break;
    case AGPILOT_DATA_STREAM_RAW_CONTROLLER:
        stream_id = STREAM_RAW_CONTROLLER;
        break;
    case AGPILOT_DATA_STREAM_POSITION:
        stream_id = STREAM_POSITION;
        break;
    case AGPILOT_DATA_STREAM_EXTRA1:
        stream_id = STREAM_EXTRA1;
        break;
    case AGPILOT_DATA_STREAM_EXTRA2:
        stream_id = STREAM_EXTRA2;
        break;
    case AGPILOT_DATA_STREAM_EXTRA3:
...
void GCS_AGPILOTLINK::handle_param_request_list(const mavlink_message_t &msg)
{
    if (!params_ready()) {
        return;
    }
...
void GCS_AGPILOTLINK::handle_param_request_read(const mavlink_message_t &msg)
{
    ...
    if (!param_timer_registered) {
        ...
        hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&GCS_AGPILOTLINK::param_io_timer, void));
    }
}

void GCS_AGPILOTLINK::handle_param_set(const mavlink_message_t &msg)
{
    ...
    if ((parameter_flags & AP_PARAM_FLAG_INTERNAL_USE_ONLY) || vp->is_read_only()) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "Param write denied (%s)", key);
...
void GCS_AGPILOTLINK::send_parameter_value(const char *param_name, ap_var_type param_type, float param_value)
{
    if (!HAVE_PAYLOAD_SPACE(chan, PARAM_VALUE)) {
        return;
    }
...
/*
  send a parameter value message to all active AGPILOTLink connections
 */
void GCS::send_parameter_value(const char *param_name, ap_var_type param_type, float param_value)
{
    ...
    packet.param_type = GCS_AGPILOTLINK::mav_param_type(param_type);
    packet.param_count = AP_Param::count_parameters();
    packet.param_index = -1;

    gcs().send_to_active_channels(AGPILOTLINK_MSG_ID_PARAM_VALUE,
                                  (const char *)&packet);
 ...
 */
void GCS_AGPILOTLINK::param_io_timer(void)
{
    struct pending_param_request req;
 ...
 */
uint8_t GCS_AGPILOTLINK::send_parameter_async_replies()
{
...
void GCS_AGPILOTLINK::handle_common_param_message(const mavlink_message_t &msg)
{
    switch (msg.msgid) {
