/*
  Common GCS AGPILOTLink functions for all vehicle types
...
struct GCS_AGPILOTLINK::LastRadioStatus GCS_AGPILOTLINK::last_radio_status;
uint8_t GCS_AGPILOTLINK::mavlink_active = 0;
uint8_t GCS_AGPILOTLINK::chan_is_streaming = 0;
uint32_t GCS_AGPILOTLINK::reserve_param_space_start_ms;

// private channels are ones used for point-to-point protocols, and
// don't get broadcasts or fwded packets
uint8_t GCS_AGPILOTLINK::mavlink_private = 0;

GCS *GCS::_singleton = nullptr;

GCS_AGPILOTLINK_InProgress GCS_AGPILOTLINK_InProgress::in_progress_tasks[1];
uint32_t GCS_AGPILOTLINK_InProgress::last_check_ms;

GCS_AGPILOTLINK::GCS_AGPILOTLINK(GCS_AGPILOTLINK_Parameters &parameters,
...
bool GCS_AGPILOTLINK::init(uint8_t instance)
{
    // get associated mavlink channel
    chan = (mavlink_channel_t)(AGPILOTLINK_COMM_0 + instance);
    if (!valid_channel(chan)) {
        return false;
    }

    // find instance of AGPILOTLink protocol; the protocol_match method in
    // AP_SerialManager means this will match AGPILOTLink2 and AGPILOTLinkHL,
    // too:
    uartstate = AP::serialmanager().find_protocol_instance(AP_SerialManager::SerialProtocol_AGPILOTLink, instance);
    ...
    if (uartstate->option_enabled(AP_HAL::UARTDriver::OPTION_AGPILOTLINK_NO_FORWARD)) {
    ...
    if (mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLink2 ||
        mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLinkHL) {
        // load signing key
        load_signing_key();
    } else {
        // user has asked to only send AGPILOTLink1
        status->flags |= AGPILOTLINK_STATUS_FLAG_OUT_AGPILOTLINK1;
    }

#if HAL_HIGH_LATENCY2_ENABLED
    if (mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLinkHL) {
...
void GCS_AGPILOTLINK::send_meminfo(void)
{
    unsigned __brkval = 0;
...
void GCS_AGPILOTLINK::send_power_status(void)
{
...
void GCS_AGPILOTLINK::send_mcu_status(void)
{
    if (!gcs().vehicle_initialised()) {
        // avoid unnecessary errors being reported to user
        return;
...
int8_t GCS_AGPILOTLINK::battery_remaining_pct(const uint8_t instance) const {
#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
    uint8_t percentage;
...
void GCS_AGPILOTLINK::send_battery_status(const uint8_t instance) const
{
#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
    // catch the battery backend not supporting the required number of cells
    static_assert(sizeof(AP_BattMonitor::cells) >= (sizeof(uint16_t) * AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN),
                  "Not enough battery cells for the AGPILOTLink message");
    ...
    uint16_t cell_mvolts[AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN];
    uint16_t cell_mvolts_ext[AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_EXT_LEN];
    ...
    if (battery.has_cell_voltages(instance)) {
        ...
        for (uint8_t i = 0; i < AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_EXT_LEN; i++) {
            if (AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN+i < uint8_t(ARRAY_SIZE(batt_cells.cells))) {
                cell_mvolts_ext[i] = batt_cells.cells[AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN+i];
            } else {
                cell_mvolts_ext[i] = 0;
            }
        }
        ...
        */
        ...
        for (uint8_t i=0; i<AGPILOTLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN; i++) {
    ...
    mavlink_msg_battery_status_send(chan,
                                    instance, // id
                                    AGPILOT_BATTERY_FUNCTION_UNKNOWN, // function
                                    AGPILOT_BATTERY_TYPE_UNKNOWN, // type
...
bool GCS_AGPILOTLINK::send_battery_status()
{
#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
    const AP_BattMonitor &battery = AP::battery();
...
void GCS_AGPILOTLINK::send_distance_sensor(const AP_RangeFinder_Backend *sensor, const uint8_t instance) const
{
    if (!sensor->has_data()) {
        return;
    }
    ...
    mavlink_msg_distance_sensor_send(
        ...
        sensor->get_mav_distance_sensor_type(),  // type from AGPILOT_DISTANCE_SENSOR enum
        instance,                                // onboard ID of the sensor == instance
        sensor->orientation(),                   // direction the sensor faces from AGPILOT_SENSOR_ORIENTATION enum
        0,                                       // Measurement covariance in centimeters, 0 for unknown / invalid readings
        0,                                       // horizontal FOV
        0,                                       // vertical FOV
        (const float *)nullptr,                  // quaternion of sensor orientation for AGPILOT_SENSOR_ROTATION_CUSTOM
...
void GCS_AGPILOTLINK::send_distance_sensor()
{
...
void GCS_AGPILOTLINK::send_rangefinder() const
{
    RangeFinder *rangefinder = RangeFinder::get_singleton();
    if (rangefinder == nullptr) {
        return;
...
#if HAL_PROXIMITY_ENABLED
void GCS_AGPILOTLINK::send_proximity()
{
    AP_Proximity *proximity = AP_Proximity::get_singleton();
    if (proximity == nullptr) {
        ...
        if (proximity->get_horizontal_distances(dist_array)) {
            for (uint8_t i = 0; i < PROXIMITY_MAX_DIRECTION; i++) {
                ...
                mavlink_msg_distance_sensor_send(
                        ...
                        AGPILOT_DISTANCE_SENSOR_LASER,                      // type from AGPILOT_DISTANCE_SENSOR enum
                        PROXIMITY_SENSOR_ID_START + i,                  // onboard ID of the sensor
                        dist_array.orientation[i],                      // direction the sensor faces from AGPILOT_SENSOR_ORIENTATION enum
                        0,                                              // Measurement covariance in centimeters, 0 for unknown / invalid readings
                        0, 0, nullptr, 0);
            }
        }
    ...
    if (proximity->get_upward_distance(dist_up)) {
        ...
        mavlink_msg_distance_sensor_send(
                ...
                AGPILOT_SENSOR_ROTATION_PITCH_90,                             // direction upwards
                0,                                                        // Measurement covariance in centimeters, 0 for unknown / invalid readings
                0, 0, nullptr, 0);
    }
}
...
// report AHRS2 state
void GCS_AGPILOTLINK::send_ahrs2()
{
...
MissionItemProtocol *GCS::get_prot_for_mission_type(const AGPILOT_MISSION_TYPE mission_type) const
{
...
void GCS_AGPILOTLINK::handle_mission_request_list(const mavlink_message_t &msg)
{
    // decode
    mavlink_mission_request_list_t packet;
    mavlink_msg_mission_request_list_decode(&msg, &packet);

    MissionItemProtocol *prot = gcs().get_prot_for_mission_type((AGPILOT_MISSION_TYPE)packet.mission_type);
    if (prot == nullptr) {
        mavlink_msg_mission_ack_send(chan,
                                     msg.sysid,
                                     msg.compid,
                                     AGPILOT_MISSION_UNSUPPORTED,
                                     packet.mission_type);
        return;
    }
 ...
 */
void GCS_AGPILOTLINK::handle_mission_request_int(const mavlink_message_t &msg)
{
        // decode
...
void GCS_AGPILOTLINK::handle_mission_request(const mavlink_message_t &msg)
{
        // decode
...
/*
  ...
  AGPILOT_CMD_DO_SET_MISSION_CURRENT which provides an acknowledgement
  ...
  do.
 */
void GCS_AGPILOTLINK::handle_mission_set_current(AP_Mission &mission, const mavlink_message_t &msg)
{
    // decode
 ...
 */
void GCS_AGPILOTLINK::handle_mission_count(const mavlink_message_t &msg)
{
    // decode
 ...
 */
void GCS_AGPILOTLINK::handle_mission_clear_all(const mavlink_message_t &msg)
{
    // decode
    mavlink_mission_clear_all_t packet;
    mavlink_msg_mission_clear_all_decode(&msg, &packet);

    const AGPILOT_MISSION_TYPE mission_type = (AGPILOT_MISSION_TYPE)packet.mission_type;
    MissionItemProtocol *prot = gcs().get_prot_for_mission_type(mission_type);
    if (prot == nullptr) {
        send_mission_ack(msg, mission_type, AGPILOT_MISSION_UNSUPPORTED);
        return;
    }
...
bool GCS_AGPILOTLINK::requesting_mission_items() const
{
...
void GCS_AGPILOTLINK::handle_mission_write_partial_list(const mavlink_message_t &msg)
{
    // decode
    mavlink_mission_write_partial_list_t packet;
    mavlink_msg_mission_write_partial_list_decode(&msg, &packet);

    MissionItemProtocol *use_prot = gcs().get_prot_for_mission_type((AGPILOT_MISSION_TYPE)packet.mission_type);
    if (use_prot == nullptr) {
        send_mission_ack(msg, (AGPILOT_MISSION_TYPE)packet.mission_type, AGPILOT_MISSION_UNSUPPORTED);
        return;
    }
 ...
 */
void GCS_AGPILOTLINK::handle_mount_message(const mavlink_message_t &msg)
{
 ...
 */
void GCS_AGPILOTLINK::handle_param_value(const mavlink_message_t &msg)
{
#if HAL_MOUNT_ENABLED
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
...
void GCS_AGPILOTLINK::send_text(AGPILOT_SEVERITY severity, const char *fmt, ...) const
{
    va_list arg_list;
...
float GCS_AGPILOTLINK::telemetry_radio_rssi()
{
...
void GCS_AGPILOTLINK::handle_radio_status(const mavlink_message_t &msg, bool log_radio)
{
...
void GCS_AGPILOTLINK::handle_mission_item(const mavlink_message_t &msg)
{
    mavlink_mission_item_int_t mission_item_int;
    bool send_mission_item_warning = false;
    if (msg.msgid == AGPILOTLINK_MSG_ID_MISSION_ITEM) {
        mavlink_mission_item_t mission_item;
        mavlink_msg_mission_item_decode(&msg, &mission_item);
        AGPILOT_MISSION_RESULT ret = AP_Mission::convert_MISSION_ITEM_to_MISSION_ITEM_INT(mission_item, mission_item_int);
        if (ret != AGPILOT_MISSION_ACCEPTED) {
            const AGPILOT_MISSION_TYPE type = (AGPILOT_MISSION_TYPE)mission_item_int.mission_type;
    ...
    if (type == AGPILOT_MISSION_TYPE_MISSION && (current == 2 || current == 3)) {
        struct AP_Mission::Mission_Command cmd = {};
        AGPILOT_MISSION_RESULT result = AP_Mission::mavlink_int_to_mission_cmd(mission_item_int, cmd);
        if (result != AGPILOT_MISSION_ACCEPTED) {
            //decode failed
            send_mission_ack(msg, AGPILOT_MISSION_TYPE_MISSION, result);
            return;
        }
        ...
        if (current == 2) {
            ...
            result = (handle_guided_request(cmd) ? AGPILOT_MISSION_ACCEPTED
                      : AGPILOT_MISSION_ERROR) ;
        } else if (current == 3) {
            ...
            result = AGPILOT_MISSION_ACCEPTED;
        }
        send_mission_ack(msg, AGPILOT_MISSION_TYPE_MISSION, result);
        return;
    }
    ...
    if (prot == nullptr) {
        send_mission_ack(msg, type, AGPILOT_MISSION_UNSUPPORTED);
        return;
    }
    ...
    if (!prot->receiving) {
        send_mission_ack(msg, type, AGPILOT_MISSION_ERROR);
        return;
    }
...
ap_message GCS_AGPILOTLINK::mavlink_id_to_ap_message_id(const uint32_t mavlink_id) const
{
    ...
    static const struct {
        ...
        { AGPILOTLINK_MSG_ID_HEARTBEAT,             MSG_HEARTBEAT},
        { AGPILOTLINK_MSG_ID_ATTITUDE,              MSG_ATTITUDE},
        { AGPILOTLINK_MSG_ID_ATTITUDE_QUATERNION,   MSG_ATTITUDE_QUATERNION},
        { AGPILOTLINK_MSG_ID_GLOBAL_POSITION_INT,   MSG_LOCATION},
        { AGPILOTLINK_MSG_ID_HOME_POSITION,         MSG_HOME},
        { AGPILOTLINK_MSG_ID_GPS_GLOBAL_ORIGIN,     MSG_ORIGIN},
        { AGPILOTLINK_MSG_ID_SYS_STATUS,            MSG_SYS_STATUS},
        { AGPILOTLINK_MSG_ID_POWER_STATUS,          MSG_POWER_STATUS},
#if HAL_WITH_MCU_MONITORING
        { AGPILOTLINK_MSG_ID_MCU_STATUS,            MSG_MCU_STATUS},
#endif
        { AGPILOTLINK_MSG_ID_MEMINFO,               MSG_MEMINFO},
        { AGPILOTLINK_MSG_ID_NAV_CONTROLLER_OUTPUT, MSG_NAV_CONTROLLER_OUTPUT},
        { AGPILOTLINK_MSG_ID_MISSION_CURRENT,       MSG_CURRENT_WAYPOINT},
        { AGPILOTLINK_MSG_ID_VFR_HUD,               MSG_VFR_HUD},
        { AGPILOTLINK_MSG_ID_SERVO_OUTPUT_RAW,      MSG_SERVO_OUTPUT_RAW},
        { AGPILOTLINK_MSG_ID_RC_CHANNELS,           MSG_RC_CHANNELS},
        { AGPILOTLINK_MSG_ID_RC_CHANNELS_RAW,       MSG_RC_CHANNELS_RAW},
        { AGPILOTLINK_MSG_ID_RAW_IMU,               MSG_RAW_IMU},
        { AGPILOTLINK_MSG_ID_SCALED_IMU,            MSG_SCALED_IMU},
        { AGPILOTLINK_MSG_ID_SCALED_IMU2,           MSG_SCALED_IMU2},
        { AGPILOTLINK_MSG_ID_SCALED_IMU3,           MSG_SCALED_IMU3},
        { AGPILOTLINK_MSG_ID_SCALED_PRESSURE,       MSG_SCALED_PRESSURE},
        { AGPILOTLINK_MSG_ID_SCALED_PRESSURE2,      MSG_SCALED_PRESSURE2},
        { AGPILOTLINK_MSG_ID_SCALED_PRESSURE3,      MSG_SCALED_PRESSURE3},
        { AGPILOTLINK_MSG_ID_GPS_RAW_INT,           MSG_GPS_RAW},
        { AGPILOTLINK_MSG_ID_GPS_RTK,               MSG_GPS_RTK},
#if GPS_MAX_RECEIVERS > 1
        { AGPILOTLINK_MSG_ID_GPS2_RAW,              MSG_GPS2_RAW},
        { AGPILOTLINK_MSG_ID_GPS2_RTK,              MSG_GPS2_RTK},
#endif
        { AGPILOTLINK_MSG_ID_SYSTEM_TIME,           MSG_SYSTEM_TIME},
        { AGPILOTLINK_MSG_ID_RC_CHANNELS_SCALED,    MSG_SERVO_OUT},
        { AGPILOTLINK_MSG_ID_PARAM_VALUE,           MSG_NEXT_PARAM},
        { AGPILOTLINK_MSG_ID_FENCE_STATUS,          MSG_FENCE_STATUS},
        { AGPILOTLINK_MSG_ID_AHRS,                  MSG_AHRS},
#if AP_SIM_ENABLED
        { AGPILOTLINK_MSG_ID_SIMSTATE,              MSG_SIMSTATE},
        { AGPILOTLINK_MSG_ID_SIM_STATE,             MSG_SIM_STATE},
#endif
        { AGPILOTLINK_MSG_ID_AHRS2,                 MSG_AHRS2},
        { AGPILOTLINK_MSG_ID_HWSTATUS,              MSG_HWSTATUS},
        { AGPILOTLINK_MSG_ID_WIND,                  MSG_WIND},
        { AGPILOTLINK_MSG_ID_RANGEFINDER,           MSG_RANGEFINDER},
        { AGPILOTLINK_MSG_ID_DISTANCE_SENSOR,       MSG_DISTANCE_SENSOR},
            // request also does report:
        { AGPILOTLINK_MSG_ID_TERRAIN_REQUEST,       MSG_TERRAIN},
#if AP_AGPILOTLINK_BATTERY2_ENABLED
        { AGPILOTLINK_MSG_ID_BATTERY2,              MSG_BATTERY2},
#endif
        { AGPILOTLINK_MSG_ID_CAMERA_FEEDBACK,       MSG_CAMERA_FEEDBACK},
#if HAL_MOUNT_ENABLED
        { AGPILOTLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS, MSG_GIMBAL_DEVICE_ATTITUDE_STATUS},
        { AGPILOTLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE, MSG_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE},
#endif
#if AP_OPTICALFLOW_ENABLED
        { AGPILOTLINK_MSG_ID_OPTICAL_FLOW,          MSG_OPTICAL_FLOW},
#endif
        { AGPILOTLINK_MSG_ID_MAG_CAL_PROGRESS,      MSG_MAG_CAL_PROGRESS},
        { AGPILOTLINK_MSG_ID_MAG_CAL_REPORT,        MSG_MAG_CAL_REPORT},
        { AGPILOTLINK_MSG_ID_EKF_STATUS_REPORT,     MSG_EKF_STATUS_REPORT},
        { AGPILOTLINK_MSG_ID_LOCAL_POSITION_NED,    MSG_LOCAL_POSITION},
        { AGPILOTLINK_MSG_ID_PID_TUNING,            MSG_PID_TUNING},
        { AGPILOTLINK_MSG_ID_VIBRATION,             MSG_VIBRATION},
#if AP_RPM_ENABLED
        { AGPILOTLINK_MSG_ID_RPM,                   MSG_RPM},
#endif
        { AGPILOTLINK_MSG_ID_MISSION_ITEM_REACHED,  MSG_MISSION_ITEM_REACHED},
        { AGPILOTLINK_MSG_ID_ATTITUDE_TARGET,       MSG_ATTITUDE_TARGET},
        { AGPILOTLINK_MSG_ID_POSITION_TARGET_GLOBAL_INT,  MSG_POSITION_TARGET_GLOBAL_INT},
        { AGPILOTLINK_MSG_ID_POSITION_TARGET_LOCAL_NED,  MSG_POSITION_TARGET_LOCAL_NED},
        { AGPILOTLINK_MSG_ID_ADSB_VEHICLE,          MSG_ADSB_VEHICLE},
        { AGPILOTLINK_MSG_ID_BATTERY_STATUS,        MSG_BATTERY_STATUS},
        { AGPILOTLINK_MSG_ID_AOA_SSA,               MSG_AOA_SSA},
        { AGPILOTLINK_MSG_ID_DEEPSTALL,             MSG_LANDING},
        { AGPILOTLINK_MSG_ID_EXTENDED_SYS_STATE,    MSG_EXTENDED_SYS_STATE},
        { AGPILOTLINK_MSG_ID_AUTOPILOT_VERSION,     MSG_AUTOPILOT_VERSION},
#if HAL_EFI_ENABLED
        { AGPILOTLINK_MSG_ID_EFI_STATUS,            MSG_EFI_STATUS},
#endif
#if HAL_GENERATOR_ENABLED
        { AGPILOTLINK_MSG_ID_GENERATOR_STATUS,      MSG_GENERATOR_STATUS},
#endif
        { AGPILOTLINK_MSG_ID_WINCH_STATUS,          MSG_WINCH_STATUS},
#if HAL_WITH_ESC_TELEM
        { AGPILOTLINK_MSG_ID_ESC_TELEMETRY_1_TO_4,  MSG_ESC_TELEMETRY},
#endif
#if APM_BUILD_TYPE(APM_BUILD_Rover)
        { AGPILOTLINK_MSG_ID_WATER_DEPTH,           MSG_WATER_DEPTH},
#endif
#if HAL_HIGH_LATENCY2_ENABLED
        { AGPILOTLINK_MSG_ID_HIGH_LATENCY2,         MSG_HIGH_LATENCY2},
#endif
#if AP_AIS_ENABLED
        { AGPILOTLINK_MSG_ID_AIS_VESSEL,            MSG_AIS_VESSEL},
#endif
#if HAL_ADSB_ENABLED
        { AGPILOTLINK_MSG_ID_UAVIONIX_ADSB_OUT_STATUS, MSG_UAVIONIX_ADSB_OUT_STATUS},
#endif
        { AGPILOTLINK_MSG_ID_DATA_TRANSFER,         MSG_DATA_TRANSFER},
        { AGPILOTLINK_MSG_ID_ACK_FOR_COMMAND,       MSG_ACK_FOR_COMMAND},
        { AGPILOTLINK_MSG_ID_COMMAND_TRANSFER,      MSG_COMMAND_TRANSFER},
            };
...
bool GCS_AGPILOTLINK::set_mavlink_message_id_interval(const uint32_t mavlink_id,
                                                  const uint16_t interval_ms)
{
    const ap_message id = mavlink_id_to_ap_message_id(mavlink_id);
    if (id == MSG_LAST) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "No ap_message for mavlink id (%u)", (unsigned int)mavlink_id);
        return false;
    }
    return set_ap_message_interval(id, interval_ms);
}

bool GCS_AGPILOTLINK::should_send_message_in_delay_callback(const ap_message id) const
{
...
uint16_t GCS_AGPILOTLINK::get_reschedule_interval_ms(const deferred_message_bucket_t &deferred) const
{
...
void GCS_AGPILOTLINK::find_next_bucket_to_send(uint16_t now16_ms)
{
#if GCS_DEBUG_SEND_MESSAGE_TIMINGS
...
ap_message GCS_AGPILOTLINK::next_deferred_bucket_message_to_send(uint16_t now16_ms)
{
    if (sending_bucket_id == no_bucket_to_send) {
...
bool GCS_AGPILOTLINK::do_try_send_message(const ap_message id)
{
...
int8_t GCS_AGPILOTLINK::get_deferred_message_index(const ap_message id) const
{
...
int8_t GCS_AGPILOTLINK::deferred_message_to_send_index(uint16_t now16_ms)
{
...
bool GCS_AGPILOTLINK_InProgress::send_ack(AGPILOT_RESULT result)
{
...
bool GCS_AGPILOTLINK_InProgress::send_in_progress()
{
    return send_ack(AGPILOT_RESULT_IN_PROGRESS);
}

bool GCS_AGPILOTLINK_InProgress::conclude(AGPILOT_RESULT result)
{
    if (!send_ack(result)) {
        return false;
    }
...
GCS_AGPILOTLINK_InProgress *GCS_AGPILOTLINK_InProgress::get_task(AGPILOT_CMD mav_cmd, GCS_AGPILOTLINK_InProgress::Type t, uint8_t sysid, uint8_t compid, mavlink_channel_t chan)
{
...
void GCS_AGPILOTLINK_InProgress::check_tasks()
{
    // run these checks only intermittently (rate-limits the
    // AGPILOT_RESULT_IN_PROGRESS messages):
    ...
    for (auto &task : in_progress_tasks) {
        ...
        case Type::AIRSPEED_CAL: {
            ...
            case AP_Airspeed::CalibrationState::NOT_STARTED:
                ...
                task.conclude(AGPILOT_RESULT_FAILED);
                break;
            ...
            case AP_Airspeed::CalibrationState::SUCCESS:
                task.conclude(AGPILOT_RESULT_ACCEPTED);
                break;
            }
#endif
            }
...
void GCS_AGPILOTLINK::update_send()
{
...
#if HAL_AGPILOTLINK_INTERVALS_FROM_FILES_ENABLED
    ...
    GCS_AGPILOTLINK_InProgress::check_tasks();
...
void GCS_AGPILOTLINK::remove_message_from_bucket(int8_t bucket, ap_message id)
{
...
bool GCS_AGPILOTLINK::set_ap_message_interval(enum ap_message id, uint16_t interval_ms)
{
...
// mavlink work!)
void GCS_AGPILOTLINK::send_message(enum ap_message id)
{
    if (id == MSG_HEARTBEAT || id == MSG_HIGH_LATENCY2) {
        save_signing_timestamp(false);
        // update the mask of all streaming channels
        if (is_streaming()) {
            GCS_AGPILOTLINK::chan_is_streaming |= (1U<<(chan-AGPILOTLINK_COMM_0));
        } else {
            GCS_AGPILOTLINK::chan_is_streaming &= ~(1U<<(chan-AGPILOTLINK_COMM_0));
        }
    }
...
void GCS_AGPILOTLINK::packetReceived(const mavlink_status_t &status,
                                 const mavlink_message_t &msg)
{
    // we exclude radio packets because we historically used this to
    // make it possible to use the CLI over the radio
    if (msg.msgid != AGPILOTLINK_MSG_ID_RADIO && msg.msgid != AGPILOTLINK_MSG_ID_RADIO_STATUS) {
        mavlink_active |= (1U<<(chan-AGPILOTLINK_COMM_0));
    }
    const auto mavlink_protocol = uartstate->get_protocol();
    if (!(status.flags & AGPILOTLINK_STATUS_FLAG_IN_AGPILOTLINK1) &&
        (status.flags & AGPILOTLINK_STATUS_FLAG_OUT_AGPILOTLINK1) &&
        (mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLink2 ||
         mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLinkHL)) {
        // if we receive any AGPILOTLink2 packets on a connection
        // currently sending AGPILOTLink1 then switch to sending
        // AGPILOTLink2
        mavlink_status_t *cstatus = mavlink_get_channel_status(chan);
        if (cstatus != nullptr) {
            cstatus->flags &= ~AGPILOTLINK_STATUS_FLAG_OUT_AGPILOTLINK1;
        }
    }
    ...
    // if ((msg.sysid != sysid_my_gcs()) || AP_PDRL_COMMANDER::getInstance()->isGcsUnlocked() || (msg.msgid == AGPILOTLINK_MSG_ID_COMMAND_TRANSFER))
    handleMessage(msg);
}

void
GCS_AGPILOTLINK::update_receive(uint32_t max_time_us)
{
    ...
    for (uint16_t i=0; i<nbytes; i++)
    {
        ...
        if (alternative.handler &&
            ...
            /*
              ...
              haven't parsed a AGPILOTLink packet for 4 seconds. Try
            ...
            /*
              we may also try parsing as AGPILOTLink if we haven't had a
...
*/
void GCS_AGPILOTLINK::log_mavlink_stats()
{
    mavlink_status_t *status = mavlink_get_channel_status(chan);
    if (status == nullptr) {
        return;
    ...
    const struct log_AGPILOT pkt{
    LOG_PACKET_HEADER_INIT(LOG_AGPILOT_MSG),
 ...
 */
void GCS_AGPILOTLINK::send_system_time() const
{
 ...
 */
void GCS_AGPILOTLINK::send_rc_channels() const
{
    uint16_t values[18] = {};
...
bool GCS_AGPILOTLINK::sending_mavlink1() const
{
    ...
    return ((status->flags & AGPILOTLINK_STATUS_FLAG_OUT_AGPILOTLINK1) != 0);
}

void GCS_AGPILOTLINK::send_rc_channels_raw() const
{
...
void GCS_AGPILOTLINK::send_raw_imu()
{
...
void GCS_AGPILOTLINK::send_scaled_imu(uint8_t instance, void (*send_fn)(mavlink_channel_t chan, uint32_t time_ms, int16_t xacc, int16_t yacc, int16_t zacc, int16_t xgyro, int16_t ygyro, int16_t zgyro, int16_t xmag, int16_t ymag, int16_t zmag, int16_t temperature))
{
#if AP_INERTIALSENSOR_ENABLED
    const AP_InertialSensor &ins = AP::ins();
    const Compass &compass = AP::compass();
...
void GCS_AGPILOTLINK::send_scaled_pressure_instance(uint8_t instance, void (*send_fn)(mavlink_channel_t chan, uint32_t time_boot_ms, float press_abs, float press_diff, int16_t temperature, int16_t temperature_press_diff))
{
...
/*
    send a statustext text string to specific AGPILOTLink bitmask
*/
void GCS::send_textv(AGPILOT_SEVERITY severity, const char *fmt, va_list arg_list, uint8_t dest_bitmask)
{
    char first_piece_of_text[AGPILOTLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN+1]{};
...
/*
    send a statustext message to specific AGPILOTLink connections in a bitmask

    must be called with semaphore held
 */
void GCS_AGPILOTLINK::service_statustext(void)
{
...
void GCS::update_send()
{
    ...
    if (!initialised_missionitemprotocol_objects) {
        ...
        if (mission != nullptr) {
            missionitemprotocols[AGPILOT_MISSION_TYPE_MISSION] = new MissionItemProtocol_Waypoints(*mission);
        }
#endif
#if HAL_RALLY_ENABLED
        AP_Rally *rally = AP::rally();
        if (rally != nullptr) {
            missionitemprotocols[AGPILOT_MISSION_TYPE_RALLY] = new MissionItemProtocol_Rally(*rally);
        }
#endif
#if AP_FENCE_ENABLED
        AC_Fence *fence = AP::fence();
        if (fence != nullptr) {
            missionitemprotocols[AGPILOT_MISSION_TYPE_FENCE] = new MissionItemProtocol_Fence(*fence);
        }
#endif
    }
    ...
    // round-robin the GCS_AGPILOTLINK backend that gets to go first so
...
{
    AP_HAL::UARTDriver *uart = AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_AGPILOTLink, 0);
    if (uart == nullptr) {
...
GCS_AGPILOTLINK_Parameters::GCS_AGPILOTLINK_Parameters()
{
    AP_Param::setup_object_defaults(this, var_info);
}

void GCS::create_gcs_mavlink_backend(GCS_AGPILOTLINK_Parameters &params, AP_HAL::UARTDriver &uart)
{
...
{
    for (uint8_t i = 1; i < AGPILOTLINK_COMM_NUM_BUFFERS; i++) {
        if (i >= ARRAY_SIZE(chan_parameters)) {
            // should not happen
            break;
        }
        AP_HAL::UARTDriver *uart = AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_AGPILOTLink, i);
        if (uart == nullptr) {
...
void GCS_AGPILOTLINK::send_battery2()
{
#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
    const AP_BattMonitor &battery = AP::battery();
...
#endif  // AP_AGPILOTLINK_BATTERY2_ENABLED

/*
  handle a SET_MODE AGPILOTLink message
 */
void GCS_AGPILOTLINK::handle_set_mode(const mavlink_message_t &msg)
{
    mavlink_set_mode_t packet;
    mavlink_msg_set_mode_decode(&msg, &packet);

    const AGPILOT_MODE _base_mode = (AGPILOT_MODE)packet.base_mode;
    const uint32_t _custom_mode = packet.custom_mode;

    const AGPILOT_RESULT result = _set_mode_common(_base_mode, _custom_mode);
    ...
    if (HAVE_PAYLOAD_SPACE(chan, COMMAND_ACK)) {
        mavlink_msg_command_ack_send(chan, AGPILOTLINK_MSG_ID_SET_MODE, result,
                                     0, 0,
                                     msg.sysid,
                                     msg.compid);
    }
...
*/
AGPILOT_RESULT GCS_AGPILOTLINK::_set_mode_common(const AGPILOT_MODE _base_mode, const uint32_t _custom_mode)
{
    // only accept custom modes because there is no easy mapping from Mavlink flight modes to AC flight modes
    if (uint32_t(_base_mode) & AGPILOT_MODE_FLAG_CUSTOM_MODE_ENABLED) {
    ...
    if (_base_mode == (AGPILOT_MODE)AGPILOT_MODE_FLAG_DECODE_POSITION_SAFETY) {
 ...
 */
void GCS_AGPILOTLINK::send_opticalflow()
{
 ...
 */
void GCS_AGPILOTLINK::send_autopilot_version() const
{
...
#endif
    char flight_custom_version[AGPILOTLINK_MSG_AUTOPILOT_VERSION_FIELD_FLIGHT_CUSTOM_VERSION_LEN]{};
    char middleware_custom_version[AGPILOTLINK_MSG_AUTOPILOT_VERSION_FIELD_MIDDLEWARE_CUSTOM_VERSION_LEN]{};
    char os_custom_version[AGPILOTLINK_MSG_AUTOPILOT_VERSION_FIELD_OS_CUSTOM_VERSION_LEN]{};
...
#endif
    uint64_t uid = 0;
    uint8_t  uid2[AGPILOTLINK_MSG_AUTOPILOT_VERSION_FIELD_UID2_LEN] = {0};
 ...
 */
void GCS_AGPILOTLINK::send_local_position() const
{
    const AP_AHRS &ahrs = AP::ahrs();
 ...
 */
void GCS_AGPILOTLINK::send_vibration() const
{
#if AP_INERTIALSENSOR_ENABLED
    const AP_InertialSensor &ins = AP::ins();
...
void GCS_AGPILOTLINK::send_named_float(const char *name, float value) const
{
    char float_name[AGPILOTLINK_MSG_NAMED_VALUE_FLOAT_FIELD_NAME_LEN+1] {};
    strncpy(float_name, name, AGPILOTLINK_MSG_NAMED_VALUE_FLOAT_FIELD_NAME_LEN);
    mavlink_msg_named_value_float_send(chan, AP_HAL::millis(), float_name, value);
}

void GCS_AGPILOTLINK::send_home_position() const
{
...
void GCS_AGPILOTLINK::send_gps_global_origin() const
{
    Location ekf_origin;
...
AGPILOT_STATE GCS_AGPILOTLINK::system_status() const
{
    AGPILOT_STATE _system_status = vehicle_system_status();
    if (_system_status < AGPILOT_STATE_CRITICAL) {
        ...
        if (AP::internalerror().errors()) {
            _system_status = AGPILOT_STATE_CRITICAL;
        }
    }
...
/*
  Send AGPILOTLink heartbeat
 */
void GCS_AGPILOTLINK::send_heartbeat() const
{
    mavlink_msg_heartbeat_send(
        chan,
        gcs().frame_type(),
        AGPILOT_AUTOPILOT_ARDUPILOTMEGA,
        base_mode(),
        gcs().custom_mode(),
        system_status());
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_aux_function(const mavlink_command_long_t &packet)
{
    if (packet.param2 > 2) {
        return AGPILOT_RESULT_DENIED;
    }
    ...
    if (!rc().run_aux_function(aux_func, position, RC_Channel::AuxFuncTriggerSource::AGPILOTLINK)) {
...
void GCS_AGPILOTLINK::send_key_tranfer() const
{
...
void GCS_AGPILOTLINK::send_ack_for_command() const
{
...
void GCS_AGPILOTLINK::send_command_tranfer()const
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_set_message_interval(const mavlink_command_long_t &packet)
{
    return set_message_interval((uint32_t)packet.param1, (int32_t)packet.param2);
}

AGPILOT_RESULT GCS_AGPILOTLINK::set_message_interval(uint32_t msg_id, int32_t interval_us)
{
    const ap_message id = mavlink_id_to_ap_message_id(msg_id);
    if (id == MSG_LAST) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "No ap_message for mavlink id (%u)", (unsigned int)msg_id);
        return AGPILOT_RESULT_DENIED;
    }
 ...
 */
AGPILOT_RESULT GCS::set_message_interval(uint8_t port_num, uint32_t msg_id, int32_t interval_us)
{
    uint8_t channel = get_channel_from_port_number(port_num);

    GCS_AGPILOTLINK *link = chan(channel);
    if (link == nullptr) {
        return AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_request_message(const mavlink_command_long_t &packet)
{
...
bool GCS_AGPILOTLINK::get_ap_message_interval(ap_message id, uint16_t &interval_ms) const
{
    // check if it's a specially-handled message:
    const int8_t deferred_offset = get_deferred_message_index(id);
    if (deferred_offset != -1) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_get_message_interval(const mavlink_command_long_t &packet)
{
...
bool GCS_AGPILOTLINK::telemetry_delayed() const
{
    uint32_t tnow = AP_HAL::millis() >> 10;
    if (tnow > telem_delay()) {
        return false;
    }
    if (chan == AGPILOTLINK_COMM_0 && hal.gpio->usb_connected()) {
 ...
 */
void GCS_AGPILOTLINK::send_servo_output_raw()
{
...
void GCS_AGPILOTLINK::send_accelcal_vehicle_position(uint32_t position)
{
    if (HAVE_PAYLOAD_SPACE(chan, COMMAND_LONG)) {
        mavlink_msg_command_long_send(
            ...
            0,
            0,
            AGPILOT_CMD_ACCELCAL_VEHICLE_POS,
            0,
            (float) position,
            0, 0, 0, 0, 0, 0);
    }
...
float GCS_AGPILOTLINK::vfr_hud_airspeed() const
{
#if AP_AIRSPEED_ENABLED
    AP_Airspeed *airspeed = AP_Airspeed::get_singleton();
...
float GCS_AGPILOTLINK::vfr_hud_climbrate() const
{
    Vector3f velned;
...
float GCS_AGPILOTLINK::vfr_hud_alt() const
{
    return global_position_current_loc.alt * 0.01f; // cm -> m
}

void GCS_AGPILOTLINK::send_vfr_hud()
{
 ...
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_preflight_reboot(const mavlink_command_long_t &packet, const mavlink_message_t &msg)
{
    if (is_equal(packet.param1, 42.0f) &&
        ...
        if (is_equal(packet.param4, 93.0f)) {
            ...
            // functionality
            while (true) {
                send_text(AGPILOT_SEVERITY_WARNING,"entering lockup");
                hal.scheduler->delay(250);
            }
        }
        if (is_equal(packet.param4, 94.0f)) {
            // the following text is unlikely to make it out...
            send_text(AGPILOT_SEVERITY_WARNING,"deferencing a bad thing");
...
// esp32 can't do this bit, skip it, return an error
        ...
        if (is_equal(packet.param4, 95.0f)) {
            // the following text is unlikely to make it out...
            send_text(AGPILOT_SEVERITY_WARNING,"calling AP_HAL::panic(...)");
        ...
        if (is_equal(packet.param4, 96.0f)) {
            ...
            send_text(AGPILOT_SEVERITY_WARNING,"wiping parameter storage header");
        ...
        if (is_equal(packet.param4, 97.0f)) {
            ...
            send_text(AGPILOT_SEVERITY_WARNING,"Creating long loop");
            // 250ms:
        ...
        if (is_equal(packet.param4, 98.0f)) {
            send_text(AGPILOT_SEVERITY_WARNING,"Creating internal error");
            INTERNAL_ERROR(AP_InternalError::error_t::flow_of_control);
            return AGPILOT_RESULT_ACCEPTED;
        }
        ...
        if (is_equal(packet.param4, 99.0f)) {
            ...
            send_text(AGPILOT_SEVERITY_ERROR, "Refusing DFU for secure firmware");
            return AGPILOT_RESULT_FAILED;
#else
            send_text(AGPILOT_SEVERITY_WARNING, "Entering DFU mode");
    ...
    mavlink_msg_command_ack_send(chan, packet.command, AGPILOT_RESULT_ACCEPTED,
                                 0, 0,
                                 msg.sysid,
                                 msg.compid);
 ...
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_flight_termination(const mavlink_command_long_t &packet)
{
 ...
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_rc_bind(const mavlink_command_long_t &packet)
{
...
uint64_t GCS_AGPILOTLINK::timesync_receive_timestamp_ns() const
{
...
uint64_t GCS_AGPILOTLINK::timesync_timestamp_ns() const
{
 ...
 */
void GCS_AGPILOTLINK::handle_timesync(const mavlink_message_t &msg)
{
 ...
 */
void GCS_AGPILOTLINK::send_timesync()
{
...
void GCS_AGPILOTLINK::handle_statustext(const mavlink_message_t &msg) const
{
    AP_Logger *logger = AP_Logger::get_singleton();
    if (logger == nullptr) {
        return;
    ...
    const uint8_t text_len = AGPILOTLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN+1+max_prefix_len;
    ...
    memcpy(&text[offset], packet.text, AGPILOTLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN);
 ...
 */
void GCS_AGPILOTLINK::handle_named_value(const mavlink_message_t &msg) const
{
...
void GCS_AGPILOTLINK::handle_system_time_message(const mavlink_message_t &msg)
{
    mavlink_system_time_t packet;
    mavlink_msg_system_time_decode(&msg, &packet);

    AP::rtc().set_utc_usec(packet.time_unix_usec, AP_RTC::SOURCE_AGPILOTLINK_SYSTEM_TIME);
}

#if AP_CAMERA_ENABLED
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_camera(const mavlink_command_long_t &packet)
{
...
void GCS_AGPILOTLINK::set_ekf_origin(const Location& loc)
{
...
void GCS_AGPILOTLINK::handle_set_gps_global_origin(const mavlink_message_t &msg)
{
 ...
 */
void GCS_AGPILOTLINK::handle_data_packet(const mavlink_message_t &msg)
{
#if HAL_RCINPUT_WITH_AP_RADIO
    mavlink_data96_t m;
...
void GCS_AGPILOTLINK::handle_vision_position_delta(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_vision_position_estimate(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_global_vision_position_estimate(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_vicon_position_estimate(const mavlink_message_t &msg)
{
...
/*
  ...
  and attitude data
 */
void GCS_AGPILOTLINK::handle_odometry(const mavlink_message_t &msg)
{
    AP_VisualOdom *visual_odom = AP::visualodom();
    if (visual_odom == nullptr) {
        return;
    ...
    if (m.frame_id != AGPILOT_FRAME_LOCAL_FRD ||
        m.child_frame_id != AGPILOT_FRAME_BODY_FRD) {
...
// these packets
void GCS_AGPILOTLINK::handle_common_vision_position_estimate_data(const uint64_t usec,
                                                              const float x,
                                                              const float y,
                                                              const float z,
                                                              const float roll,
...
void GCS_AGPILOTLINK::handle_att_pos_mocap(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_vision_speed_estimate(const mavlink_message_t &msg)
{
    AP_VisualOdom *visual_odom = AP::visualodom();
    if (visual_odom == nullptr) {
        return;
...
void GCS_AGPILOTLINK::handle_command_ack(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_rc_channels_override(const mavlink_message_t &msg)
{
    if(msg.sysid != sysid_my_gcs()) {
    ...
    for (uint8_t i=0; i<8; i++) {
        // Per AGPILOTLink spec a value of UINT16_MAX means to ignore this field.
        if (override_data[i] != UINT16_MAX) {
            RC_Channels::set_override(i, override_data[i], tnow);
        }
    }
    for (uint8_t i=8; i<ARRAY_SIZE(override_data); i++) {
        // Per AGPILOTLink spec a value of zero or UINT16_MAX means to
        // ignore this field.
...
#if AP_OPTICALFLOW_ENABLED
void GCS_AGPILOTLINK::handle_optical_flow(const mavlink_message_t &msg)
{
...
/*
  handle AGPILOT_CMD_FIXED_MAG_CAL_YAW
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_fixed_mag_cal_yaw(const mavlink_command_long_t &packet)
{
...
void GCS_AGPILOTLINK::handle_data_transfer(const mavlink_message_t &msg)
{
    //  AP_KEYSTORE* keyStore = AP_KEYSTORE::getInstance();
...
void GCS_AGPILOTLINK::handle_command_transfer(const mavlink_message_t &msg)
{
    AP_PDRL_COMMANDER* commander = AP_PDRL_COMMANDER::getInstance();
...
/*
  handle AGPILOT_CMD_CAN_FORWARD
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_can_forward(const mavlink_command_long_t &packet, const mavlink_message_t &msg)
{
#if HAL_CANMANAGER_ENABLED
    return AP::can().handle_can_forward(chan, packet, msg) ? AGPILOT_RESULT_ACCEPTED : AGPILOT_RESULT_FAILED;
#else
    return AGPILOT_RESULT_UNSUPPORTED;
#endif
}
 ...
 */
void GCS_AGPILOTLINK::handle_can_frame(const mavlink_message_t &msg) const
{
#if HAL_CANMANAGER_ENABLED
...
void GCS_AGPILOTLINK::handle_distance_sensor(const mavlink_message_t &msg)
{
...
void GCS_AGPILOTLINK::handle_obstacle_distance(const mavlink_message_t &msg)
{
#if HAL_PROXIMITY_ENABLED
    AP_Proximity *proximity = AP::proximity();
    if (proximity != nullptr) {
...
void GCS_AGPILOTLINK::handle_obstacle_distance_3d(const mavlink_message_t &msg)
{
#if HAL_PROXIMITY_ENABLED
    AP_Proximity *proximity = AP::proximity();
    if (proximity != nullptr) {
...
void GCS_AGPILOTLINK::handle_adsb_message(const mavlink_message_t &msg)
{
#if HAL_ADSB_ENABLED
...
void GCS_AGPILOTLINK::handle_osd_param_config(const mavlink_message_t &msg) const
{
...
void GCS_AGPILOTLINK::handle_heartbeat(const mavlink_message_t &msg) const
{
 ...
 */
void GCS_AGPILOTLINK::handle_common_message(const mavlink_message_t &msg)
{
    switch (msg.msgid) {
    ...
    case AGPILOTLINK_MSG_ID_COMMAND_ACK: {
        handle_command_ack(msg);
        break;
    }

    case AGPILOTLINK_MSG_ID_SETUP_SIGNING:
    ...
    case AGPILOTLINK_MSG_ID_SET_GPS_GLOBAL_ORIGIN:
        handle_set_gps_global_origin(msg);
        break;

    case AGPILOTLINK_MSG_ID_DEVICE_OP_READ:
        handle_device_op_read(msg);
        break;
    case AGPILOTLINK_MSG_ID_DEVICE_OP_WRITE:
        handle_device_op_write(msg);
        break;
    case AGPILOTLINK_MSG_ID_TIMESYNC:
        handle_timesync(msg);
        break;
    ...
    case AGPILOTLINK_MSG_ID_FILE_TRANSFER_PROTOCOL:
        handle_file_transfer_protocol(msg);
        break;

#if AP_CAMERA_ENABLED
    case AGPILOTLINK_MSG_ID_DIGICAM_CONTROL:
    case AGPILOTLINK_MSG_ID_GOPRO_HEARTBEAT: // heartbeat from a GoPro in Solo gimbal
        {
            AP_Camera *camera = AP::camera();
            if (camera == nullptr) {
                return;
    ...
    case AGPILOTLINK_MSG_ID_SET_MODE:
        handle_set_mode(msg);
        break;

    case AGPILOTLINK_MSG_ID_AUTOPILOT_VERSION_REQUEST:
        handle_send_autopilot_version(msg);
        break;

    case AGPILOTLINK_MSG_ID_MISSION_WRITE_PARTIAL_LIST:
    case AGPILOTLINK_MSG_ID_MISSION_REQUEST_LIST:
    case AGPILOTLINK_MSG_ID_MISSION_COUNT:
    case AGPILOTLINK_MSG_ID_MISSION_CLEAR_ALL:
    case AGPILOTLINK_MSG_ID_MISSION_ITEM:
    case AGPILOTLINK_MSG_ID_MISSION_ITEM_INT:
    case AGPILOTLINK_MSG_ID_MISSION_REQUEST_INT:
    case AGPILOTLINK_MSG_ID_MISSION_REQUEST:
    case AGPILOTLINK_MSG_ID_MISSION_ACK:
    case AGPILOTLINK_MSG_ID_MISSION_SET_CURRENT:
    ...
    case AGPILOTLINK_MSG_ID_COMMAND_INT:
        handle_command_int(msg);
        break;

    case AGPILOTLINK_MSG_ID_FENCE_POINT:
    case AGPILOTLINK_MSG_ID_FENCE_FETCH_POINT:
        handle_fence_message(msg);
        break;

#if HAL_MOUNT_ENABLED
    case AGPILOTLINK_MSG_ID_MOUNT_CONFIGURE: // deprecated. Use AGPILOT_CMD_DO_MOUNT_CONFIGURE
        send_received_message_deprecation_warning("MOUNT_CONFIGURE");
        handle_mount_message(msg);
        break;
    case AGPILOTLINK_MSG_ID_MOUNT_CONTROL: // deprecated. Use AGPILOT_CMD_DO_MOUNT_CONTROL
...
#if HAL_RALLY_ENABLED
    case AGPILOTLINK_MSG_ID_RALLY_POINT:
    case AGPILOTLINK_MSG_ID_RALLY_FETCH_POINT:
        handle_common_rally_message(msg);
        break;
#endif

    case AGPILOTLINK_MSG_ID_REQUEST_DATA_STREAM:
    ...
    case AGPILOTLINK_MSG_ID_DATA96:
        handle_data_packet(msg);
        break;        

#if HAL_VISUALODOM_ENABLED
    case AGPILOTLINK_MSG_ID_VISION_POSITION_DELTA:
        handle_vision_position_delta(msg);
        break;

    case AGPILOTLINK_MSG_ID_VISION_POSITION_ESTIMATE:
        handle_vision_position_estimate(msg);
        break;

    case AGPILOTLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE:
        handle_global_vision_position_estimate(msg);
        break;

    case AGPILOTLINK_MSG_ID_VICON_POSITION_ESTIMATE:
        handle_vicon_position_estimate(msg);
        break;

    case AGPILOTLINK_MSG_ID_ODOMETRY:
        handle_odometry(msg);
        break;

    case AGPILOTLINK_MSG_ID_ATT_POS_MOCAP:
        handle_att_pos_mocap(msg);
        break;

    case AGPILOTLINK_MSG_ID_VISION_SPEED_ESTIMATE:
        handle_vision_speed_estimate(msg);
        break;
#endif  // HAL_VISUALODOM_ENABLED

    case AGPILOTLINK_MSG_ID_SYSTEM_TIME:
        handle_system_time_message(msg);
        break;

    case AGPILOTLINK_MSG_ID_RC_CHANNELS_OVERRIDE:
        handle_rc_channels_override(msg);
        break;

#if AP_OPTICALFLOW_ENABLED
    case AGPILOTLINK_MSG_ID_OPTICAL_FLOW:
        handle_optical_flow(msg);
        break;
#endif

    case AGPILOTLINK_MSG_ID_DISTANCE_SENSOR:
        handle_distance_sensor(msg);
        break;

    case AGPILOTLINK_MSG_ID_OBSTACLE_DISTANCE:
        handle_obstacle_distance(msg);
        break;
    
    case AGPILOTLINK_MSG_ID_OBSTACLE_DISTANCE_3D:
    ...
    case AGPILOTLINK_MSG_ID_LANDING_TARGET:
        handle_landing_target(msg);
        break;

    case AGPILOTLINK_MSG_ID_NAMED_VALUE_FLOAT:
    ...
    case AGPILOTLINK_MSG_ID_CAN_FILTER_MODIFY:
#if HAL_CANMANAGER_ENABLED
    ...
    case AGPILOTLINK_MSG_ID_SECURE_COMMAND_REPLY:
        AP_CheckFirmware::handle_msg(chan, msg);
        break;
#endif

    case AGPILOTLINK_MSG_ID_DATA_TRANSFER:
        handle_data_transfer(msg);
    break;

    case AGPILOTLINK_MSG_ID_COMMAND_TRANSFER:
...
void GCS_AGPILOTLINK::handle_common_mission_message(const mavlink_message_t &msg)
{
    switch (msg.msgid) {
    case AGPILOTLINK_MSG_ID_MISSION_WRITE_PARTIAL_LIST: // AGPILOT ID: 38
    {
    ...
    case AGPILOTLINK_MSG_ID_MISSION_ITEM:           // AGPILOT ID: 39
    case AGPILOTLINK_MSG_ID_MISSION_ITEM_INT:
    ...
    case AGPILOTLINK_MSG_ID_MISSION_SET_CURRENT:    // AGPILOT ID: 41
    {
    ...
    case AGPILOTLINK_MSG_ID_MISSION_REQUEST_LIST:       // AGPILOT ID: 43
    {
    ...
    //  individual commands will then be sent from the GCS using the AGPILOTLINK_MSG_ID_MISSION_ITEM message
    case AGPILOTLINK_MSG_ID_MISSION_COUNT:          // AGPILOT ID: 44
    {
        handle_mission_count(msg);
        break;
    }

    case AGPILOTLINK_MSG_ID_MISSION_CLEAR_ALL:      // AGPILOT ID: 45
    {
...
void GCS_AGPILOTLINK::handle_send_autopilot_version(const mavlink_message_t &msg)
{
    send_message(MSG_AUTOPILOT_VERSION);
}

void GCS_AGPILOTLINK::send_banner()
{
    // mark the firmware version in the tlog
    const AP_FWVersion &fwver = AP::fwversion();

    send_text(AGPILOT_SEVERITY_INFO, "%s", fwver.fw_string);

    if (fwver.middleware_name && fwver.os_name) {
        send_text(AGPILOT_SEVERITY_INFO, "%s: %s %s: %s",
                  fwver.middleware_name, fwver.middleware_hash_str,
                  fwver.os_name, fwver.os_hash_str);
    } else if (fwver.os_name) {
        send_text(AGPILOT_SEVERITY_INFO, "%s: %s",
                  fwver.os_name, fwver.os_hash_str);
    }
    ...
    if (hal.util->get_system_id(sysid)) {
        send_text(AGPILOT_SEVERITY_INFO, "%s", sysid);
    }

    // send RC output mode info if available
    char banner_msg[50];
    if (hal.rcout->get_output_mode_banner(banner_msg, sizeof(banner_msg))) {
        send_text(AGPILOT_SEVERITY_INFO, "%s", banner_msg);
    }
...
#if AP_SIM_ENABLED
void GCS_AGPILOTLINK::send_simstate() const
{
...
void GCS_AGPILOTLINK::send_sim_state() const
{
    SITL::SIM *sitl = AP::sitl();
    if (sitl == nullptr) {
        return;
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_flash_bootloader(const mavlink_command_long_t &packet)
{
    if (uint32_t(packet.param5) != 290876) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Magic not set");
        return AGPILOT_RESULT_FAILED;
    }
    ...
    case AP_HAL::Util::FlashBootloader::NOT_SIGNED:
        gcs().send_text(AGPILOT_SEVERITY_ERROR, "Bootloader not signed");
        break;
#endif
    default:
        break;
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_preflight_set_sensor_offsets(const mavlink_command_long_t &packet)
{
    Compass &compass = AP::compass();
...
bool GCS_AGPILOTLINK::calibrate_gyros()
{
#if AP_INERTIALSENSOR_ENABLED
    AP::ins().init_gyro();
...
AGPILOT_RESULT GCS_AGPILOTLINK::_handle_command_preflight_calibration_baro(const mavlink_message_t &msg)
{
    ...
    AP_Airspeed *airspeed = AP_Airspeed::get_singleton();
    if (airspeed != nullptr) {
        GCS_AGPILOTLINK_InProgress *task = GCS_AGPILOTLINK_InProgress::get_task(AGPILOT_CMD_PREFLIGHT_CALIBRATION, GCS_AGPILOTLINK_InProgress::Type::AIRSPEED_CAL, msg.sysid, msg.compid, chan);
        if (task == nullptr) {
            return AGPILOT_RESULT_TEMPORARILY_REJECTED;
        }
        airspeed->calibrate(false);
        return AGPILOT_RESULT_IN_PROGRESS;
    }
#endif
...
AGPILOT_RESULT GCS_AGPILOTLINK::_handle_command_preflight_calibration(const mavlink_command_long_t &packet, const mavlink_message_t &msg)
{
    AGPILOT_RESULT ret = AGPILOT_RESULT_UNSUPPORTED;
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_preflight_calibration(const mavlink_command_long_t &packet, const mavlink_message_t &msg)
{
    if (hal.util->get_soft_armed()) {
        // *preflight*, remember?
        gcs().send_text(AGPILOT_SEVERITY_NOTICE, "Disarm to allow calibration");
        return AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_run_prearm_checks(const mavlink_command_long_t &packet)
{
    if (hal.util->get_soft_armed()) {
        return AGPILOT_RESULT_TEMPORARILY_REJECTED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_preflight_can(const mavlink_command_long_t &packet)
{
#if HAL_CANMANAGER_ENABLED
    if (hal.util->get_soft_armed()) {
        // *preflight*, remember?
...
// To be replaced with macro saying if KDECAN library is included
    ...
    AGPILOT_RESULT ack = AGPILOT_RESULT_DENIED;
    if (can_exists) {
        ack = result ? AGPILOT_RESULT_ACCEPTED : AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_mission_current(const mavlink_command_long_t &packet)
{
    AP_Mission *mission = AP::mission();
    if (mission == nullptr) {
        return AGPILOT_RESULT_UNSUPPORTED;
    ...
    if (packet.command == AGPILOT_CMD_DO_JUMP_TAG) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_battery_reset(const mavlink_command_long_t &packet)
{
#if !defined(HAL_BUILD_AP_PERIPH) || defined(HAL_PERIPH_ENABLE_BATTERY)
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_mag_cal(const mavlink_command_long_t &packet)
{
#if COMPASS_CAL_ENABLED
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_request_autopilot_capabilities(const mavlink_command_long_t &packet)
{
    if (!is_equal(packet.param1,1.0f)) {
        return AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_send_banner(const mavlink_command_long_t &packet)
{
    send_banner();
    return AGPILOT_RESULT_ACCEPTED;
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_mode(const mavlink_command_long_t &packet)
{
    const AGPILOT_MODE _base_mode = (AGPILOT_MODE)packet.param1;
    const uint32_t _custom_mode = (uint32_t)packet.param2;

    return _set_mode_common(_base_mode, _custom_mode);
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_get_home_position(const mavlink_command_long_t &packet)
{
    if (!AP::ahrs().home_is_set()) {
        return AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_debug_trap(const mavlink_command_long_t &packet)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_set_ekf_source_set(const mavlink_command_long_t &packet)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_gripper(const mavlink_command_long_t &packet)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_sprayer(const mavlink_command_long_t &packet)
{
...
#if HAL_INS_ACCELCAL_ENABLED
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_accelcal_vehicle_pos(const mavlink_command_long_t &packet)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_mount(const mavlink_command_long_t &packet)
{
#if HAL_MOUNT_ENABLED
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_home(const mavlink_command_long_t &packet)
{
    ...
    if (!location_from_command_t(packet, AGPILOT_FRAME_GLOBAL, new_home_loc)) {
        return AGPILOT_RESULT_DENIED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_component_arm_disarm(const mavlink_command_long_t &packet)
{
    if (is_equal(packet.param1,1.0f)) {
        ...
        if (AP::arming().arm(AP_Arming::Method::AGPILOTLINK, do_arming_checks)) {
            return AGPILOT_RESULT_ACCEPTED;
        }
        return AGPILOT_RESULT_FAILED;
    }
    if (is_zero(packet.param1))  {
        ...
        if (AP::arming().disarm(AP_Arming::Method::AGPILOTLINK, !forced)) {
            return AGPILOT_RESULT_ACCEPTED;
        }
        return AGPILOT_RESULT_FAILED;
    }
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_long_packet(const mavlink_command_long_t &packet)
{
    ...
    case AGPILOT_CMD_DO_SEND_BANNER:
...
#if HAL_HIGH_LATENCY2_ENABLED
    case AGPILOT_CMD_CONTROL_HIGH_LATENCY:
    ...
    case AGPILOT_CMD_DO_SET_ROI_NONE:
    ...
    case AGPILOT_CMD_DO_SET_MISSION_CURRENT:
...
#if HAL_ADSB_ENABLED
    case AGPILOT_CMD_DO_ADSB_OUT_IDENT:
    ...
    case AGPILOT_CMD_RUN_PREARM_CHECKS:
    ...
    case AGPILOT_CMD_DEBUG_TRAP:
        result = handle_command_debug_trap(packet);
        break;

    case AGPILOT_CMD_SET_EKF_SOURCE_SET:
    ...
    case AGPILOT_CMD_PREFLIGHT_STORAGE:
        if (is_equal(packet.param1, 2.0f)) {
            AP_Param::erase_all();
            send_text(AGPILOT_SEVERITY_WARNING, "All parameters reset, reboot board");
            result= AGPILOT_RESULT_ACCEPTED;
        }
        break;
    ...
    case AGPILOT_CMD_DO_REPEAT_SERVO:
...
bool GCS_AGPILOTLINK::location_from_command_t(const mavlink_command_long_t &in, AGPILOT_FRAME in_frame, Location &out)
{
...
bool GCS_AGPILOTLINK::location_from_command_t(const mavlink_command_int_t &in, Location &out)
{
    if (!command_long_stores_location((AGPILOT_CMD)in.command)) {
        return false;
    }
    ...
    if (!mavlink_coordinate_frame_to_location_alt_frame((AGPILOT_FRAME)in.frame, frame)) {
...
bool GCS_AGPILOTLINK::command_long_stores_location(const AGPILOT_CMD command)
{
    switch(command) {
    case AGPILOT_CMD_DO_SET_HOME:
    case AGPILOT_CMD_DO_SET_ROI:
    ...
    case AGPILOT_CMD_DO_REPOSITION:
        return true;
    default:
        return false;
    }
...
void GCS_AGPILOTLINK::convert_COMMAND_LONG_to_COMMAND_INT(const mavlink_command_long_t &in, mavlink_command_int_t &out, AGPILOT_FRAME frame)
{
...
void GCS_AGPILOTLINK::handle_command_long(const mavlink_message_t &msg)
{
    // decode packet
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi(const Location &roi_loc)
{
#if HAL_MOUNT_ENABLED
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
    ...
    if (roi_loc.lat == 0 && roi_loc.lng == 0 && roi_loc.alt == 0) {
        ...
        if (mount->get_mode() == AGPILOT_MOUNT_MODE_GPS_POINT) {
...
void GCS_AGPILOTLINK::handle_landing_target(const mavlink_message_t &msg)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_int_do_set_home(const mavlink_command_int_t &packet)
{
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi_none()
{
#if HAL_MOUNT_ENABLED
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi_sysid(const uint8_t sysid)
{
#if HAL_MOUNT_ENABLED
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi_sysid(const mavlink_command_int_t &packet)
{
    return handle_command_do_set_roi_sysid((uint8_t)packet.param1);
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi_sysid(const mavlink_command_long_t &packet)
{
    return handle_command_do_set_roi_sysid((uint8_t)packet.param1);
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi(const mavlink_command_int_t &packet)
{
    // be aware that this method is called for both AGPILOT_CMD_DO_SET_ROI
    // and AGPILOT_CMD_DO_SET_ROI_LOCATION.  If you intend to support any
    // of the extra fields in the former then you will need to split
    // off support for AGPILOT_CMD_DO_SET_ROI_LOCATION (which doesn't
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_do_set_roi(const mavlink_command_long_t &packet)
{
    // be aware that this method is called for both AGPILOT_CMD_DO_SET_ROI
    // and AGPILOT_CMD_DO_SET_ROI_LOCATION.  If you intend to support any
    // of the extra fields in the former then you will need to split
    ...
    // support the extra fields).
    Location roi_loc;
    if (!location_from_command_t(packet, AGPILOT_FRAME_GLOBAL_RELATIVE_ALT, roi_loc)) {
        return AGPILOT_RESULT_DENIED;
    }
    return handle_command_do_set_roi(roi_loc);
}

AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_storage_format(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    if (!is_equal(packet.param1, 1.0f) ||
        !is_equal(packet.param2, 1.0f)) {
        return AGPILOT_RESULT_UNSUPPORTED;
    }
    GCS_AGPILOTLINK_InProgress *task = GCS_AGPILOTLINK_InProgress::get_task(AGPILOT_CMD_STORAGE_FORMAT, GCS_AGPILOTLINK_InProgress::Type::SD_FORMAT, msg.sysid, msg.compid, chan);
    if (task == nullptr) {
        return AGPILOT_RESULT_TEMPORARILY_REJECTED;
    }
    if (!AP::FS().format()) {
...
AGPILOT_RESULT GCS_AGPILOTLINK::handle_command_int_packet(const mavlink_command_int_t &packet)
{
    switch (packet.command) {
    case AGPILOT_CMD_DO_SET_ROI:
    case AGPILOT_CMD_DO_SET_ROI_LOCATION:
...
void GCS_AGPILOTLINK::handle_command_int(const mavlink_message_t &msg)
{
    // decode packet
    ...
    // special handling of messages that need the mavlink_message_t
    switch (packet.command) {
    case AGPILOT_CMD_STORAGE_FORMAT:
...
void GCS::try_send_queued_message_for_type(AGPILOT_MISSION_TYPE type) const {
...
bool GCS_AGPILOTLINK::try_send_mission_message(const enum ap_message id)
{
    switch (id) {
    ...
    case MSG_NEXT_MISSION_REQUEST_WAYPOINTS:
        ...
        gcs().try_send_queued_message_for_type(AGPILOT_MISSION_TYPE_MISSION);
        break;
#if HAL_RALLY_ENABLED
    case MSG_NEXT_MISSION_REQUEST_RALLY:
        CHECK_PAYLOAD_SIZE(MISSION_REQUEST);
        gcs().try_send_queued_message_for_type(AGPILOT_MISSION_TYPE_RALLY);
        break;
#endif
#if AP_FENCE_ENABLED
    case MSG_NEXT_MISSION_REQUEST_FENCE:
        CHECK_PAYLOAD_SIZE(MISSION_REQUEST);
        gcs().try_send_queued_message_for_type(AGPILOT_MISSION_TYPE_FENCE);
        break;
#endif
    default:
        break;
...
void GCS_AGPILOTLINK::send_hwstatus()
{
...
#if AP_RPM_ENABLED
void GCS_AGPILOTLINK::send_rpm() const
{
    AP_RPM *rpm = AP::rpm();
    if (rpm == nullptr) {
        return;
...
void GCS_AGPILOTLINK::send_sys_status()
{
...
void GCS_AGPILOTLINK::send_extended_sys_state() const
{
    mavlink_msg_extended_sys_state_send(chan, vtol_state(), landed_state());
}

void GCS_AGPILOTLINK::send_attitude() const
{
    const AP_AHRS &ahrs = AP::ahrs();
...
void GCS_AGPILOTLINK::send_attitude_quaternion() const
{
    const AP_AHRS &ahrs = AP::ahrs();
    Quaternion quat;
...
int32_t GCS_AGPILOTLINK::global_position_int_alt() const {
    return global_position_current_loc.alt * 10UL;
}
int32_t GCS_AGPILOTLINK::global_position_int_relative_alt() const {
    float posD;
    AP::ahrs().get_relative_position_D_home(posD);
    posD *= -1000.0f; // change from down to up and metres to millimeters
    return posD;
}
void GCS_AGPILOTLINK::send_global_position_int()
{
    AP_AHRS &ahrs = AP::ahrs();
...
#if HAL_MOUNT_ENABLED
void GCS_AGPILOTLINK::send_gimbal_device_attitude_status() const
{
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
        return;
...
void GCS_AGPILOTLINK::send_set_position_target_global_int(uint8_t target_system, uint8_t target_component, const Location& loc)
{
    ...
    mavlink_msg_set_position_target_global_int_send(
            ...
            target_system,
            target_component,
            AGPILOT_FRAME_GLOBAL_RELATIVE_ALT_INT,
            type_mask,
            loc.lat,
            loc.lng,
            rel_alt,
...
#if HAL_GENERATOR_ENABLED
void GCS_AGPILOTLINK::send_generator_status() const
{
...
#if APM_BUILD_TYPE(APM_BUILD_Rover)
void GCS_AGPILOTLINK::send_water_depth() const
{
...
#if HAL_ADSB_ENABLED
void GCS_AGPILOTLINK::send_uavionix_adsb_out_status() const
{
    AP_ADSB *adsb = AP::ADSB();
    if (adsb != nullptr) {
...
void GCS_AGPILOTLINK::send_autopilot_state_for_gimbal_device() const
{
    // get attitude
    const AP_AHRS &ahrs = AP::ahrs();
    Quaternion quat;
    ...
    mavlink_msg_autopilot_state_for_gimbal_device_send(
        ...
        0);     // landed_state (see AGPILOT_LANDED_STATE)
}

void GCS_AGPILOTLINK::send_received_message_deprecation_warning(const char * message)
{
    ...
    send_text(AGPILOT_SEVERITY_WARNING, "Received message (%s) is deprecated", message);
}

bool GCS_AGPILOTLINK::try_send_message(const enum ap_message id)
{
    bool ret = true;
    ...
    default:
        ...
        gcs().send_text(AGPILOT_SEVERITY_DEBUG, "Sending unknown message (%u)", id);
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
...
uint16_t GCS_AGPILOTLINK::get_interval_for_stream(GCS_AGPILOTLINK::streams id) const
{
...
void GCS_AGPILOTLINK::initialise_message_intervals_for_stream(GCS_AGPILOTLINK::streams id)
{
    for (uint8_t i=0; all_stream_entries[i].ap_message_ids != nullptr; i++) {
        const GCS_AGPILOTLINK::stream_entries &entries = all_stream_entries[i];
...
void GCS_AGPILOTLINK::get_intervals_from_filepath(const char *path, DefaultIntervalsFromFiles &intervals)
{
...
void GCS_AGPILOTLINK::initialise_message_intervals_from_config_files()
{
...
void GCS_AGPILOTLINK::initialise_message_intervals_from_streamrates()
{
    ...
    if (!is_high_latency_link) {
        set_mavlink_message_id_interval(AGPILOTLINK_MSG_ID_HEARTBEAT, 1000);
    } else {
        set_mavlink_message_id_interval(AGPILOTLINK_MSG_ID_HIGH_LATENCY2, 5000);
    }
#else
    set_mavlink_message_id_interval(AGPILOTLINK_MSG_ID_HEARTBEAT, 1000);
#endif
...
bool GCS_AGPILOTLINK::get_default_interval_for_ap_message(const ap_message id, uint16_t &interval) const
{
    if (id == MSG_HEARTBEAT) {
 ...
 */
uint32_t GCS_AGPILOTLINK::correct_offboard_timestamp_usec_to_ms(uint64_t offboard_usec, uint16_t payload_size)
{
    uint64_t local_us;
 ...
 */
bool GCS_AGPILOTLINK::accept_packet(const mavlink_status_t &status,
                                const mavlink_message_t &msg) const
{
    ...
    if (msg.msgid == AGPILOTLINK_MSG_ID_RADIO ||
        msg.msgid == AGPILOTLINK_MSG_ID_RADIO_STATUS) {
        return true;
    }
...
void GCS::update_passthru(void)
{
    ...
    if (enabled && !_passthru.enabled) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Passthru enabled");
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Passthru disabled");
    } else if (enabled &&
               _passthru.timeout_s &&
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Passthru timed out");
    }
}
...
bool GCS_AGPILOTLINK::mavlink_coordinate_frame_to_location_alt_frame(const AGPILOT_FRAME coordinate_frame, Location::AltFrame &frame)
{
    switch (coordinate_frame) {
    case AGPILOT_FRAME_GLOBAL_RELATIVE_ALT: // solo shot manager incorrectly sends RELATIVE_ALT instead of RELATIVE_ALT_INT
    ...
    default:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Unknown mavlink coordinate frame %u", coordinate_frame);
#endif
        return false;
    }
}

uint64_t GCS_AGPILOTLINK::capabilities() const
{
    uint64_t ret = AGPILOT_PROTOCOL_CAPABILITY_PARAM_FLOAT |
        AGPILOT_PROTOCOL_CAPABILITY_COMPASS_CALIBRATION;

    const auto mavlink_protocol = uartstate->get_protocol();
    if (mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLink2 || mavlink_protocol == AP_SerialManager::SerialProtocol_AGPILOTLinkHL) {
        ret |= AGPILOT_PROTOCOL_CAPABILITY_AGPILOTLINK2;
    }
    ...
    if (failsafe != nullptr && failsafe->enabled()) {
        ...
        ret |= AGPILOT_PROTOCOL_CAPABILITY_FLIGHT_TERMINATION;
    }
#endif

#if HAL_RALLY_ENABLED
    if (AP::rally()) {
        ret |= AGPILOT_PROTOCOL_CAPABILITY_MISSION_RALLY;
    }
#endif

#if AP_FENCE_ENABLED
    if (AP::fence()) {
        ret |= AGPILOT_PROTOCOL_CAPABILITY_MISSION_FENCE;
    }
#endif

    if (!AP_BoardConfig::ftp_disabled()){  //if ftp disable board option is not set
        ret |= AGPILOT_PROTOCOL_CAPABILITY_FTP;
    }
...
void GCS_AGPILOTLINK::manual_override(RC_Channel *c, int16_t value_in, const uint16_t offset, const float scaler, const uint32_t tnow, const bool reversed)
{
    if (c == nullptr) {
        return;
    }
...
void GCS_AGPILOTLINK::handle_manual_control(const mavlink_message_t &msg)
{
    if (msg.sysid != sysid_my_gcs()) {
        return; // only accept control from our gcs
    }
...
uint8_t GCS_AGPILOTLINK::receiver_rssi() const
{
 ...
 */
#if HAL_HIGH_LATENCY2_ENABLED
void GCS_AGPILOTLINK::send_high_latency2() const
{
    AP_AHRS &ahrs = AP::ahrs();
    ...
    static const struct PACKED status_map_t {
        AGPILOT_SYS_STATUS_SENSOR sensor;
        HL_FAILURE_FLAG failure_flag;
    } status_map[] {
        { AGPILOT_SYS_STATUS_SENSOR_GPS, HL_FAILURE_FLAG_GPS },
        { AGPILOT_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE, HL_FAILURE_FLAG_DIFFERENTIAL_PRESSURE },
        { AGPILOT_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE, HL_FAILURE_FLAG_ABSOLUTE_PRESSURE },
        { AGPILOT_SYS_STATUS_SENSOR_3D_ACCEL, HL_FAILURE_FLAG_3D_ACCEL },
        { AGPILOT_SYS_STATUS_SENSOR_3D_GYRO, HL_FAILURE_FLAG_3D_GYRO },
        { AGPILOT_SYS_STATUS_SENSOR_3D_MAG, HL_FAILURE_FLAG_3D_MAG },
        { AGPILOT_SYS_STATUS_TERRAIN, HL_FAILURE_FLAG_TERRAIN },
        { AGPILOT_SYS_STATUS_SENSOR_BATTERY, HL_FAILURE_FLAG_BATTERY },
        { AGPILOT_SYS_STATUS_SENSOR_RC_RECEIVER, HL_FAILURE_FLAG_RC_RECEIVER },
        { AGPILOT_SYS_STATUS_GEOFENCE, HL_FAILURE_FLAG_GEOFENCE },
        { AGPILOT_SYS_STATUS_AHRS, HL_FAILURE_FLAG_ESTIMATOR },
    };
    ...
    //send_text(AGPILOT_SEVERITY_INFO, "Yaw: %u", (((uint16_t)ahrs.yaw_sensor / 100) % 360));

    mavlink_msg_high_latency2_send(chan, 
        AP_HAL::millis(), //[ms] Timestamp (milliseconds since boot or Unix epoch)
        gcs().frame_type(), // Type of the AGPILOT (quadrotor, helicopter, etc.)
        AGPILOT_AUTOPILOT_ARDUPILOTMEGA, // Autopilot type / class. Use AGPILOT_AUTOPILOT_INVALID for components that are not flight controllers.
...
int8_t GCS_AGPILOTLINK::high_latency_air_temperature() const
{
#if AP_AIRSPEED_ENABLED
...
/*
  handle a AGPILOT_CMD_CONTROL_HIGH_LATENCY command 

  Enable or disable any marked (via SERIALn_PROTOCOL) high latency connections
 */
AGPILOT_RESULT GCS_AGPILOTLINK::handle_control_high_latency(const mavlink_command_long_t &packet)
{
    ...
    hl_msg.command = AGPILOT_CMD_CONTROL_HIGH_LATENCY;
    hl_msg.param1 = packet.param1;
    GCS_AGPILOTLINK::send_to_components(AGPILOTLINK_MSG_ID_COMMAND_LONG, (char*)&hl_msg, sizeof(hl_msg));
