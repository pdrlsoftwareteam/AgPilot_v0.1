...
/// @file	GCS_AGPILOTLink.cpp

/*
This provides some support code and variables for AGPILOTLink enabled sketches
...
#ifdef AGPILOTLINK_SEPARATE_HELPERS
// Shut up warnings about missing declarations; TODO: should be fixed on
// mavlink/pymavlink project for when AGPILOTLINK_SEPARATE_HELPERS is defined
...
AP_HAL::UARTDriver	*mavlink_comm_port[AGPILOTLINK_COMM_NUM_BUFFERS];
bool gcs_alternative_active[AGPILOTLINK_COMM_NUM_BUFFERS];

// per-channel lock
static HAL_Semaphore chan_locks[AGPILOTLINK_COMM_NUM_BUFFERS];
static bool chan_discard[AGPILOTLINK_COMM_NUM_BUFFERS];

mavlink_system_t mavlink_system = {7,1};

// routing table
AGPILOTLink_routing GCS_AGPILOTLINK::routing;

GCS_AGPILOTLINK *GCS_AGPILOTLINK::find_by_mavtype_and_compid(uint8_t mav_type, uint8_t compid, uint8_t &sysid) {
...
void GCS_AGPILOTLINK::set_channel_private(mavlink_channel_t _chan)
{
...
AGPILOT_PARAM_TYPE GCS_AGPILOTLINK::mav_param_type(enum ap_var_type t)
{
    if (t == AP_PARAM_INT8) {
	    return AGPILOT_PARAM_TYPE_INT8;
    }
    if (t == AP_PARAM_INT16) {
	    return AGPILOT_PARAM_TYPE_INT16;
    }
    if (t == AP_PARAM_INT32) {
	    return AGPILOT_PARAM_TYPE_INT32;
    }
    // treat any others as float
    return AGPILOT_PARAM_TYPE_REAL32;
}
...
/*
  send a buffer out a AGPILOTLink channel
 */
