...
#include <AR_Motors/AG_MotorsUGV.h>
...
#define SCHED_TASK(func, rate_hz, max_time_micros, prio) SCHED_TASK_CLASS(AG_Vehicle, &vehicle, func, rate_hz, max_time_micros, prio)

/*
  2nd group of parameters
 */
const AG_Param::GroupInfo AG_Vehicle::var_info[] = {
#if HAL_RUNCAM_ENABLED
    // @Group: CAM_RC_
    // @Path: ../AG_Camera/AG_RunCam.cpp
    AP_SUBGROUPINFO(runcam, "CAM_RC_", 1, AG_Vehicle, AG_RunCam),
#endif

#if HAL_GYROFFT_ENABLED
    // @Group: FFT_
    // @Path: ../AG_GyroFFT/AG_GyroFFT.cpp
    AP_SUBGROUPINFO(gyro_fft, "FFT_",  2, AG_Vehicle, AG_GyroFFT),
#endif

#if HAL_VISUALODOM_ENABLED
    // @Group: VISO
    // @Path: ../AG_VisualOdom/AG_VisualOdom.cpp
    AP_SUBGROUPINFO(visual_odom, "VISO",  3, AG_Vehicle, AG_VisualOdom),
#endif

#if AP_VIDEOTX_ENABLED
    // @Group: VTX_
    // @Path: ../AG_VideoTX/AG_VideoTX.cpp
    AP_SUBGROUPINFO(vtx, "VTX_",  4, AG_Vehicle, AG_VideoTX),
#endif

#if HAL_MSP_ENABLED
    // @Group: MSP
    // @Path: ../AG_MSP/AG_MSP.cpp
    AP_SUBGROUPINFO(msp, "MSP",  5, AG_Vehicle, AG_MSP),
#endif

#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
    // @Group: FRSKY_
    // @Path: ../AG_Frsky_Telem/AG_Frsky_Parameters.cpp
    AP_SUBGROUPINFO(frsky_parameters, "FRSKY_", 6, AG_Vehicle, AG_Frsky_Parameters),
#endif

#if HAL_GENERATOR_ENABLED
    // @Group: GEN_
    // @Path: ../AG_Generator/AG_Generator.cpp
    AP_SUBGROUPINFO(generator, "GEN_", 7, AG_Vehicle, AG_Generator),
#endif

#if HAL_EXTERNAL_AHRS_ENABLED
    // @Group: EAHRS
    // @Path: ../AG_ExternalAHRS/AG_ExternalAHRS.cpp
    AP_SUBGROUPINFO(externalAHRS, "EAHRS", 8, AG_Vehicle, AG_ExternalAHRS),
#endif

#if HAL_EFI_ENABLED
    // @Group: EFI
    // @Path: ../AG_EFI/AG_EFI.cpp
    AP_SUBGROUPINFO(efi, "EFI", 9, AG_Vehicle, AG_EFI),
#endif

#if AP_AIRSPEED_ENABLED
    // @Group: ARSPD
    // @Path: ../AG_Airspeed/AG_Airspeed.cpp
    AP_SUBGROUPINFO(airspeed, "ARSPD", 10, AG_Vehicle, AG_Airspeed),
#endif

    // @Group: CUST_ROT
    // @Path: ../AG_CustomRotations/AG_CustomRotations.cpp
    AP_SUBGROUPINFO(custom_rotations, "CUST_ROT", 11, AG_Vehicle, AG_CustomRotations),

#if HAL_WITH_ESC_TELEM
    // @Group: ESC_TLM
    // @Path: ../AG_ESC_Telem/AG_ESC_Telem.cpp
    AP_SUBGROUPINFO(esc_telem, "ESC_TLM", 12, AG_Vehicle, AG_ESC_Telem),
#endif

#if AG_AIS_ENABLED
    // @Group: AIS_
    // @Path: ../AG_AIS/AG_AIS.cpp
    AP_SUBGROUPINFO(ais, "AIS_",  13, AG_Vehicle, AG_AIS),
#endif

#if AP_FENCE_ENABLED
    // @Group: FENCE_
    // @Path: ../AG_Fence/AG_Fence.cpp
    AP_SUBGROUPINFO(fence, "FENCE_", 14, AG_Vehicle, AG_Fence),
#endif

#if AP_OPENDRONEID_ENABLED
    // @Group: DID_
    // @Path: ../AG_OpenDroneID/AG_OpenDroneID.cpp
    AP_SUBGROUPINFO(opendroneid, "DID_", 15, AG_Vehicle, AG_OpenDroneID),
#endif

#if AP_TEMPERATURE_SENSOR_ENABLED
    // @Group: TEMP
    // @Path: ../AG_TemperatureSensor/AG_TemperatureSensor.cpp
    AP_SUBGROUPINFO(temperature_sensor, "TEMP", 16, AG_Vehicle, AG_TemperatureSensor),
#endif

#if HAL_NMEA_OUTPUT_ENABLED
    // @Group: NMEA_
    // @Path: ../AG_NMEA_Output/AG_NMEA_Output.cpp
    AP_SUBGROUPINFO(nmea, "NMEA_", 17, AG_Vehicle, AG_NMEA_Output),
#endif
...
AG_Vehicle& vehicle = *AG_Vehicle::get_singleton();
#else
extern AG_Vehicle& vehicle;
#endif

/*
  setup is called when the sketch starts
 */
void AG_Vehicle::setup()
{
...
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
    if (AG_BoardConfig::get_sdcard_slowdown() != 0) {
    ...
    AG_Param::show_all(hal.console, true);
#endif
 ...
 */
const AG_Scheduler::Task AG_Vehicle::scheduler_tasks[] = {
#if HAL_GYROFFT_ENABLED
    FAST_TASK_CLASS(AG_GyroFFT,    &vehicle.gyro_fft,       sample_gyros),
#endif
#if AP_AIRSPEED_ENABLED
    SCHED_TASK_CLASS(AG_Airspeed,  &vehicle.airspeed,       update,                   10, 100, 41),    // NOTE: the priority number here should be right before Plane's calc_airspeed_errors
#endif
#if COMPASS_CAL_ENABLED
    SCHED_TASK_CLASS(Compass,      &vehicle.compass,        cal_update,     100, 200, 75),
#endif
#if HAL_NMEA_OUTPUT_ENABLED
    SCHED_TASK_CLASS(AG_NMEA_Output, &vehicle.nmea,         update,                   50, 50, 180),
#endif
#if HAL_RUNCAM_ENABLED
    SCHED_TASK_CLASS(AG_RunCam,    &vehicle.runcam,         update,                   50, 50, 200),
#endif
#if HAL_GYROFFT_ENABLED
    SCHED_TASK_CLASS(AG_GyroFFT,   &vehicle.gyro_fft,       update,                  400, 50, 205),
    SCHED_TASK_CLASS(AG_GyroFFT,   &vehicle.gyro_fft,       update_parameters,         1, 50, 210),
#endif
    SCHED_TASK(update_dynamic_notch_at_specified_rate,      LOOP_RATE,                    200, 215),
#if AP_VIDEOTX_ENABLED
    SCHED_TASK_CLASS(AG_VideoTX,   &vehicle.vtx,            update,                    2, 100, 220),
#endif
#if AP_TRAMP_ENABLED
    SCHED_TASK_CLASS(AG_Tramp,     &vehicle.tramp,          update,                   50,  50, 225),
#endif
    SCHED_TASK(send_watchdog_reset_statustext,         0.1,     20, 225),
#if HAL_WITH_ESC_TELEM
    SCHED_TASK_CLASS(AG_ESC_Telem, &vehicle.esc_telem,      update,                  100,  50, 230),
#endif
#if HAL_GENERATOR_ENABLED
    SCHED_TASK_CLASS(AG_Generator, &vehicle.generator,      update,                   10,  50, 235),
#endif
#if AP_OPENDRONEID_ENABLED
    SCHED_TASK_CLASS(AG_OpenDroneID, &vehicle.opendroneid,  update,                   10,  50, 236),
#endif
#if OSD_ENABLED
    SCHED_TASK(publish_osd_info, 1, 10, 240),
#endif
#if AP_TEMPERATURE_SENSOR_ENABLED
    SCHED_TASK_CLASS(AG_TemperatureSensor, &vehicle.temperature_sensor, update,        5, 50, 242),
#endif
#if HAL_INS_ACCELCAL_ENABLED
    SCHED_TASK(accel_cal_update,                                                      10, 100, 245),
#endif
#if AP_FENCE_ENABLED
    SCHED_TASK_CLASS(AG_Fence,     &vehicle.fence,          update,                   10, 100, 248),
#endif
#if AG_AIS_ENABLED
    SCHED_TASK_CLASS(AG_AIS,       &vehicle.ais,            update,                    5, 100, 249),
#endif
#if HAL_EFI_ENABLED
    SCHED_TASK_CLASS(AG_EFI,       &vehicle.efi,            update,                   50, 200, 250),
#endif
#if HAL_INS_ACCELCAL_ENABLED
...
void AG_Vehicle::get_common_scheduler_tasks(const AG_Scheduler::Task*& tasks, uint8_t& num_tasks)
{
 ...
 *  complete
 */
void AG_Vehicle::scheduler_delay_callback()
{
#if APM_BUILD_TYPE(APM_BUILD_Replay)
...
void AG_Vehicle::send_watchdog_reset_statustext()
{
    if (!hal.util->was_watchdog_reset()) {
        return;
    }
    const AG_HAL::Util::PersistentData &pd = hal.util->last_persistent_data;
...
bool AG_Vehicle::is_crashed() const
{
    if (AP::arming().is_armed()) {
        return false;
    }
    return AP::arming().last_disarm_method() == AG_Arming::Method::CRASH;
}

// update the harmonic notch filter for throttle based notch
void AG_Vehicle::update_throttle_notch(AG_InertialSensor::HarmonicNotch &notch)
{
...
void AG_Vehicle::update_dynamic_notch(AG_InertialSensor::HarmonicNotch &notch)
{
#if APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI||APM_BUILD_TYPE(APM_BUILD_Rover)
...
#if APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI
    const AG_Motors* motors = AP::motors();
    if (motors != nullptr && motors->get_spool_state() == AG_Motors::SpoolState::SHUT_DOWN) {
...
void AG_Vehicle::update_dynamic_notch_at_specified_rate()
{
...
void AG_Vehicle::notify_no_such_mode(uint8_t mode_number)
{
...
void AG_Vehicle::reboot(bool hold_in_bootloader)
{
    ...
    AG_Notify::flags.firmware_update = 1;
    notify.update();
...
#if OSD_ENABLED
void AG_Vehicle::publish_osd_info()
{
...
void AG_Vehicle::get_osd_roll_pitch_rad(float &roll, float &pitch) const
{
    roll = ahrs.roll;
    pitch = ahrs.pitch;
}
...
/*
  update accel cal
 */
void AG_Vehicle::accel_cal_update()
{
 ...
 */
void AG_Vehicle::one_Hz_update(void)
{
    one_Hz_counter++;
...
void AG_Vehicle::check_motor_noise()
{
    ...
    for (uint8_t i = 0; i<numf; i++) {
        ...
        if (energy > 40.0f && AG_HAL::millis() - last_motor_noise_ms > 5000) {
    ...
    if (output_error) {
        last_motor_noise_ms = AG_HAL::millis();
    }
#endif
}

AG_Vehicle *AG_Vehicle::_singleton = nullptr;

AG_Vehicle *AG_Vehicle::get_singleton()
{
    return _singleton;
}
...
AG_Vehicle *vehicle()
{
    return AG_Vehicle::get_singleton();
}
