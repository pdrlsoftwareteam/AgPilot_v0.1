...
function update()
    ...
    if rcin_select ~= rcin_select_pre then
        if rcin_select == 0 then
            ...
            gcs:send_text(6, 'LUA: Gimbal-Cam mode manual')  -- AGPILOT_SEVERITY_INFO
        elseif rcin_select == 1 then
            ...
            gcs:send_text(6, 'LUA: Gimbal-Cam mode auto/manual')  -- AGPILOT_SEVERITY_INFO
        else
            ...
            gcs:send_text(6, 'LUA: Gimbal-Cam mode auto')  -- AGPILOT_SEVERITY_INFO
...
--        gcs:send_text(6, string.format('LUA: %s = %i',param_name,value))  -- AGPILOT_SEVERITY_INFO
--        gcs:send_text(6, 'LUA: Gimbal-Cam mode disabled')  -- AGPILOT_SEVERITY_INFO
...
--        return  -- stop running until reboot of FC
    ...
    if (select_switch_ch == nil) then  -- NORC
        gcs:send_text(3, 'LUA: RC channels not set')  -- AGPILOT_SEVERITY_ERROR
...
function set_save_param(str,val,save)
    if save then
        ...
        if param:set_and_save(str,val) then
            gcs:send_text(2, 'LUA: params saved')  -- AGPILOT_SEVERITY_CRITICAL
        else
            gcs:send_text(2, 'LUA: param set failed')  -- AGPILOT_SEVERITY_CRITICAL
        end
    else
        -- Just set
...
gcs:send_text(6, 'LUA: Gimbal-Cam mode started')  -- AGPILOT_SEVERITY_INFO
