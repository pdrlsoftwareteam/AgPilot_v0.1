...
The script will, however, automatically enable AGPILOTLink High Latency Control upon start
...
local function AGPILOTLinkProcessor()
    ...
    function self.createAGPILOTLink(message, msgid)
        ...
        if not message_map then
            ...
            gcs:send_text(3, "Unknown AGPILOTLink message " .. msgid)
            return nil
        end
...
function HLSatcom()
    -- read in any bytes from GCS and and send to AGPILOTLink processor
    -- only read in 1 packet at a time to avoid time overruns
    while port:available() > 0 do
        local byte = port:read()
        if mavlink.parseAGPILOTLink(byte) then break end
    end
