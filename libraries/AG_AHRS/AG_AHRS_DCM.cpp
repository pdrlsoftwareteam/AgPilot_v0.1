...
void
AG_AHRS_DCM::reset_gyro_drift(void)
{
    _omega_I.zero();
    _omega_I_sum.zero();
    _omega_I_sum_time = 0;
...
void AG_AHRS::load_watchdog_home()
{
    const AG_HAL::Util::PersistentData &pd = hal.util->persistent_data;
...
void AG_AHRS_DCM::get_results(AG_AHRS_Backend::Estimates &results)
{
 ...
 */
void AG_AHRS_DCM::backup_attitude(void)
{
    AG_HAL::Util::PersistentData &pd = hal.util->persistent_data;
    pd.roll_rad = roll;
    pd.pitch_rad = pitch;
    pd.yaw_rad = yaw;
}
...
void AG_AHRS_DCM::matrix_update(void)
{
 ...
 */
void
AG_AHRS_DCM::reset(bool recover_eulers)
{
    ...
    if (hal.util->was_watchdog_reset() && AG_HAL::millis() < 10000) {
        const AG_HAL::Util::PersistentData &pd = hal.util->persistent_data;
        roll = pd.roll_rad;
        pitch = pd.pitch_rad;
        yaw = pd.yaw_rad;
    ...
    _last_startup_ms = AG_HAL::millis();
}
 ...
 */
void
AG_AHRS_DCM::check_matrix(void)
{
    if (_dcm_matrix.is_nan()) {
        //Serial.printf("ERROR: DCM matrix NAN\n");
        AG_AHRS_DCM::reset(true);
        return;
    }
...
bool
AG_AHRS_DCM::renorm(Vector3f const &a, Vector3f &result)
{
...
AG_AHRS_DCM::normalize(void)
{
    ...
    if (!renorm(t0, _dcm_matrix.a) ||
        ...
        _last_failure_ms = AG_HAL::millis();
        AG_AHRS_DCM::reset(true);
    }
}
...
float
AG_AHRS_DCM::yaw_error_compass(Compass &compass)
{
...
// paper from Bill.
float
AG_AHRS_DCM::_P_gain(float spin_rate)
{
...
float
AG_AHRS_DCM::_yaw_gain(void) const
{
...
bool AG_AHRS_DCM::have_gps(void) const
{
    if (AP::gps().status() <= AG_GPS::NO_FIX || _gps_use == GPSUse::Disable) {
        return false;
    }
    return true;
}
 ...
 */
bool AG_AHRS_DCM::use_fast_gains(void) const
{
    return !hal.util->get_soft_armed() && (AG_HAL::millis() - _last_startup_ms) < 20000U;
}


// return true if we should use the compass for yaw correction
bool AG_AHRS_DCM::use_compass(void)
{
...
bool AG_AHRS_DCM::get_quaternion(Quaternion &quat) const
{
...
void
AG_AHRS_DCM::drift_correction_yaw(void)
{
    bool new_value = false;
    float yaw_error;
    float yaw_deltat;

    const AG_GPS &_gps = AP::gps();
    ...
    if (_kp_yaw < AG_AHRS_YAW_P_MIN) {
        _kp_yaw.set(AG_AHRS_YAW_P_MIN);
    }
 ...
 */
Vector3f AG_AHRS_DCM::ra_delayed(uint8_t instance, const Vector3f &ra)
{
 ...
 */
bool AG_AHRS_DCM::should_correct_centrifugal() const
{
...
void
AG_AHRS_DCM::drift_correction(float deltat)
{
    Vector3f velocity;
    ...
    if (!have_gps() ||
            _gps.status() < AG_GPS::GPS_OK_FIX_3D ||
        ...
        last_correction_time = AG_HAL::millis();
...
#define YAW_INDEPENDENT_DRIFT_CORRECTION 0
    ...
    if (_kp < AG_AHRS_RP_P_MIN) {
        _kp.set(AG_AHRS_RP_P_MIN);
    }
    ...
    if (fly_forward && _gps.status() >= AG_GPS::GPS_OK_FIX_2D &&
            _gps.ground_speed() < GPS_SPEED_MIN &&
            _ins.get_accel().x >= 7 &&
...
void AG_AHRS_DCM::estimate_wind(void)
{
...
bool AG_AHRS_DCM::get_location(Location &loc) const
{
    loc.lat = _last_lat;
    loc.lng = _last_lng;
...
bool AG_AHRS_DCM::airspeed_estimate(float &airspeed_ret) const
{
#if AP_AIRSPEED_ENABLED
...
bool AG_AHRS_DCM::airspeed_estimate(uint8_t airspeed_index, float &airspeed_ret) const
{
...
//               Or if none of the above, fills-in using the previous airspeed estimate
// Return false: if we are using the previous airspeed estimate
bool AG_AHRS_DCM::get_unconstrained_airspeed_estimate(uint8_t airspeed_index, float &airspeed_ret) const
{
#if AP_AIRSPEED_ENABLED
...
bool AG_AHRS::set_home(const Location &loc)
{
...
*/
bool AG_AHRS_DCM::healthy(void) const
{
    // consider ourselves healthy if there have been no failures for 5 seconds
    return (_last_failure_ms == 0 || AG_HAL::millis() - _last_failure_ms > 5000);
}

/*
  return NED velocity if we have GPS lock
 */
bool AG_AHRS_DCM::get_velocity_NED(Vector3f &vec) const
{
    const AG_GPS &_gps = AP::gps();
    if (_gps.status() < AG_GPS::GPS_OK_FIX_3D) {
        return false;
    }
    vec = _gps.velocity();
    return true;
...
bool AG_AHRS_DCM::get_vert_pos_rate(float &velocity) const
{
    Vector3f velned;
...
bool AG_AHRS_DCM::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
    if (!healthy()) {
...
/*
  relative-origin functions for fallback in AG_InertialNav
*/
bool AG_AHRS_DCM::get_origin(Location &ret) const
{
    ret = last_origin;
    if (ret.is_zero()) {
...
bool AG_AHRS_DCM::get_relative_position_NED_origin(Vector3f &posNED) const
{
    Location origin;
    if (!AG_AHRS_DCM::get_origin(origin)) {
        return false;
    }
    Location loc;
    if (!AG_AHRS_DCM::get_location(loc)) {
        return false;
    }
...
bool AG_AHRS_DCM::get_relative_position_NE_origin(Vector2f &posNE) const
{
    Vector3f posNED;
    if (!AG_AHRS_DCM::get_relative_position_NED_origin(posNED)) {
        return false;
    }
    posNE = posNED.xy();
    return true;
...
bool AG_AHRS_DCM::get_relative_position_D_origin(float &posD) const
{
    Vector3f posNED;
    if (!AG_AHRS_DCM::get_relative_position_NED_origin(posNED)) {
        return false;
...
void AG_AHRS_DCM::send_ekf_status_report(GCS_MAVLINK &link) const
{
}

// return true if DCM has a yaw source available
bool AG_AHRS_DCM::yaw_source_available(void) const
{
    return AP::compass().use_for_yaw();
}

void AG_AHRS_DCM::get_control_limits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const
{
