...
#include "AG_AHRS_config.h"

#include <AG_HAL/Semaphores.h>

#include <AG_NavEKF2/AG_NavEKF2.h>
#include <AG_NavEKF3/AG_NavEKF3.h>
#include <AG_NavEKF/AG_Nav_Common.h>              // definitions shared by inertial and ekf nav filters

#include "AG_AHRS_DCM.h"
#include "AG_AHRS_SIM.h"

// forward declare view class
class AG_AHRS_View;

#define AG_AHRS_NAVEKF_SETTLE_TIME_MS 20000     // time in milliseconds the ekf needs to settle after being started
...
public:
    ...
    // Constructor
    AG_AHRS(uint8_t flags = 0);

    // initialise
    void init(void);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_AHRS);

    // get singleton instance
    static AG_AHRS *get_singleton() {
        return _singleton;
    }
    ...
    // create a view
    AG_AHRS_View *create_view(enum Rotation rotation, float pitch_trim_deg=0);
     ...
     * from AG_AHRS by other libraries
     */
    ...
    // get the view
    AG_AHRS_View *get_view(void) const { return _view; };
...
private:
    ...
    // poke AG_Notify based on values from status
    void update_notify_from_filter_status(const nav_filter_status &status);

    /*
     *  backends (and their results)
     */
    AG_AHRS_DCM dcm{_kp_yaw, _kp, gps_gain, beta, _gps_use, _gps_minsats};
    struct AG_AHRS_Backend::Estimates dcm_estimates;
#if AG_AHRS_SIM_ENABLED
#if HAL_NAVEKF3_AVAILABLE
    AG_AHRS_SIM sim{EKF3};
#else
    AG_AHRS_SIM sim;
#endif
    struct AG_AHRS_Backend::Estimates sim_estimates;
#endif
     ...
     */
    void copy_estimates_from_backend_estimates(const AG_AHRS_Backend::Estimates &results);
