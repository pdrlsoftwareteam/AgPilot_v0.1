...
AG_AHRS_View::AG_AHRS_View(AG_AHRS &_ahrs, enum Rotation _rotation, float pitch_trim_deg) :
    rotation(_rotation),
    ahrs(_ahrs)
{
    switch (rotation) {
    ...
    case ROTATION_PITCH_270:
        ...
        break;
    default:
        AG_HAL::panic("Unsupported AHRS view %u\n", (unsigned)rotation);
    }
...
// apply pitch trim
void AG_AHRS_View::set_pitch_trim(float trim_deg) {
...
Vector3f AG_AHRS_View::get_gyro_latest(void) const {
    return rot_view * ahrs.get_gyro_latest();
}

// rotate a 2D vector from earth frame to body frame
Vector2f AG_AHRS_View::earth_to_body2D(const Vector2f &ef) const
{
...
// rotate a 2D vector from earth frame to body frame
Vector2f AG_AHRS_View::body_to_earth2D(const Vector2f &bf) const
{
...
void AG_AHRS_View::rotate(Vector3f &vec) const
{
    vec = rot_view * vec;
}
