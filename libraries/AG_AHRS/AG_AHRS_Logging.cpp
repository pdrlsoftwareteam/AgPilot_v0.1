...
#include <AG_AttitudeControl/AG_AttitudeControl.h>
#include <AG_AttitudeControl/AG_PosControl.h>


// Write an AHRS2 packet
void AG_AHRS::Write_AHRS2() const
{
    Vector3f euler;
    Location loc;
    Quaternion quat;
...
// Write AOA and SSA
void AG_AHRS::Write_AOA_SSA(void) const
{
...
void AG_AHRS::Write_Attitude(const Vector3f &targets) const
{
...
void AG_AHRS::Write_Origin(LogOriginType origin_type, const Location &loc) const
{
...
void AG_AHRS::write_video_stabilisation() const
{
...
void AG_AHRS_View::Write_AttitudeView(const Vector3f &targets) const
{
    const struct log_Attitude pkt{
        LOG_PACKET_HEADER_INIT(LOG_ATTITUDE_MSG),
        time_us         : AG_HAL::micros64(),
...
// Write a rate packet
void AG_AHRS_View::Write_Rate(const AG_Motors &motors, const AG_AttitudeControl &attitude_control,
                                const AG_PosControl &pos_control) const
{
    const Vector3f &rate_targets = attitude_control.rate_bf_targets();
    const Vector3f &accel_target = pos_control.get_accel_target_cmss();
    const auto timeus = AG_HAL::micros64();
    ...
    if (scale != AG_AttitudeControl::VECTORF_111 || pd_scale != AG_AttitudeControl::VECTORF_111) {
