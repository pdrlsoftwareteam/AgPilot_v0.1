#include "AG_AHRS_SIM.h"

#if AG_AHRS_SIM_ENABLED

#include "AG_AHRS.h"

bool AG_AHRS_SIM::get_location(Location &loc) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_velocity_NED(Vector3f &vec) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::wind_estimate(Vector3f &wind) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::airspeed_estimate(float &airspeed_ret) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::airspeed_estimate(uint8_t index, float &airspeed_ret) const
{
    return airspeed_estimate(airspeed_ret);
}

bool AG_AHRS_SIM::get_quaternion(Quaternion &quat) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
Vector2f AG_AHRS_SIM::groundspeed_vector(void)
{
    if (_sitl == nullptr) {
        return Vector2f{};
    }
...
bool AG_AHRS_SIM::get_vert_pos_rate(float &velocity) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_hagl(float &height) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_relative_position_NED_origin(Vector3f &vec) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_relative_position_NE_origin(Vector2f &posNE) const
{
    Location loc, orgn;
    if (!get_location(loc) ||
        !get_origin(orgn)) {
...
bool AG_AHRS_SIM::get_relative_position_D_origin(float &posD) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_filter_status(nav_filter_status &status) const
{
...
void AG_AHRS_SIM::get_control_limits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const
{
...
bool AG_AHRS_SIM::get_mag_offsets(uint8_t mag_idx, Vector3f &magOffsets) const
{
    magOffsets.zero();
...
void AG_AHRS_SIM::send_ekf_status_report(GCS_MAVLINK &link) const
{
...
bool AG_AHRS_SIM::get_origin(Location &ret) const
{
    if (_sitl == nullptr) {
        return false;
    }
...
bool AG_AHRS_SIM::get_innovations(Vector3f &velInnov, Vector3f &posInnov, Vector3f &magInnov, float &tasInnov, float &yawInnov) const
{
    velInnov.zero();
    posInnov.zero();
    magInnov.zero();
...
bool AG_AHRS_SIM::get_variances(float &velVar, float &posVar, float &hgtVar, Vector3f &magVar, float &tasVar) const
{
    velVar = 0;
    posVar = 0;
    hgtVar = 0;
...
void AG_AHRS_SIM::get_results(AG_AHRS_Backend::Estimates &results)
{
    if (_sitl == nullptr) {
        _sitl = AP::sitl();
        if (_sitl == nullptr) {
    ...
    if (_sitl->odom_enable) {
        ...
        uint32_t timeStamp_ms = AG_HAL::millis();
