...
void AG_AHRS_Backend::init()
{
}

// return a smoothed and corrected gyro vector using the latest ins data (which may not have been consumed by the EKF yet)
Vector3f AG_AHRS::get_gyro_latest(void) const
{
...
// set_trim
void AG_AHRS::set_trim(const Vector3f &new_trim)
{
    const Vector3f trim {
        constrain_float(new_trim.x, ToRad(-AG_AHRS_TRIM_LIMIT), ToRad(AG_AHRS_TRIM_LIMIT)),
        constrain_float(new_trim.y, ToRad(-AG_AHRS_TRIM_LIMIT), ToRad(AG_AHRS_TRIM_LIMIT)),
        constrain_float(new_trim.z, ToRad(-AG_AHRS_TRIM_LIMIT), ToRad(AG_AHRS_TRIM_LIMIT))
    };
    _trim.set_and_save(trim);
}

// add_trim - adjust the roll and pitch trim up to a total of 10 degrees
void AG_AHRS::add_trim(float roll_in_radians, float pitch_in_radians, bool save_to_eeprom)
{
    Vector3f trim = _trim.get();

    // add new trim
    trim.x = constrain_float(trim.x + roll_in_radians, ToRad(-AG_AHRS_TRIM_LIMIT), ToRad(AG_AHRS_TRIM_LIMIT));
    trim.y = constrain_float(trim.y + pitch_in_radians, ToRad(-AG_AHRS_TRIM_LIMIT), ToRad(AG_AHRS_TRIM_LIMIT));
...
void AG_AHRS::update_orientation()
{
...
Vector2f AG_AHRS_DCM::groundspeed_vector(void)
{
    ...
    const bool gotGPS = (AP::gps().status() >= AG_GPS::GPS_OK_FIX_2D);
    if (gotAirspeed) {
 ...
 */
void AG_AHRS::calc_trig(const Matrix3f &rot,
 ...
 */
void AG_AHRS::update_cd_values(void)
{
...
/*
  create a rotated view of AG_AHRS with optional pitch trim
 */
AG_AHRS_View *AG_AHRS::create_view(enum Rotation rotation, float pitch_trim_deg)
{
    if (_view != nullptr) {
        // can only have one
        return nullptr;
    }
    _view = new AG_AHRS_View(*this, rotation, pitch_trim_deg);
    return _view;
}
 ...
 */
void AG_AHRS::update_AOA_SSA(void)
{
...
Vector2f AG_AHRS::earth_to_body2D(const Vector2f &ef) const
{
...
// rotate a 2D vector from earth frame to body frame
Vector2f AG_AHRS::body_to_earth2D(const Vector2f &bf) const
{
...
void AG_AHRS::Log_Write_Home_And_Origin()
{
...
float AG_AHRS_Backend::get_EAS2TAS(void) const {
    return AP::baro().get_EAS2TAS();
}

// return current vibration vector for primary IMU
Vector3f AG_AHRS::get_vibration(void) const
{
    return AP::ins().get_vibration_levels();
}

void AG_AHRS::set_takeoff_expected(bool b)
{
    takeoff_expected = b;
    takeoff_expected_start_ms = AG_HAL::millis();
}

void AG_AHRS::set_touchdown_expected(bool b)
{
    touchdown_expected = b;
    touchdown_expected_start_ms = AG_HAL::millis();
}

/*
  update takeoff/touchdown flags
 */
void AG_AHRS::update_flags(void)
{
    const uint32_t timeout_ms = 1000;
    if (takeoff_expected && AG_HAL::millis() - takeoff_expected_start_ms > timeout_ms) {
        takeoff_expected = false;
    }
    if (touchdown_expected && AG_HAL::millis() - touchdown_expected_start_ms > timeout_ms) {
