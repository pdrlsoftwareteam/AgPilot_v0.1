...
const AG_Param::GroupInfo AG_AHRS::var_info[] = {
    ...
    // @Increment: .01
    // @User: Advanced
    AP_GROUPINFO("GPS_GAIN",  2, AG_AHRS, gps_gain, 1.0f),
    ...
    // @User: Advanced
    AP_GROUPINFO("GPS_USE",  3, AG_AHRS, _gps_use, float(GPSUse::Enable)),
    ...
    // @Increment: .01
    // @User: Advanced
    AP_GROUPINFO("YAW_P", 4,    AG_AHRS, _kp_yaw, 0.2f),
    ...
    // @Increment: .01
    // @User: Advanced
    AP_GROUPINFO("RP_P",  5,    AG_AHRS, _kp, 0.2f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("WIND_MAX",  6,    AG_AHRS, _wind_max, 0.0f),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("TRIM", 8, AG_AHRS, _trim, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("ORIENTATION", 9, AG_AHRS, _board_orientation, 0),
    ...
    // @Increment: .01
    // @User: Advanced
    AP_GROUPINFO("COMP_BETA",  10, AG_AHRS, beta, 0.1f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("GPS_MINSATS", 11, AG_AHRS, _gps_minsats, 6),
    ...
    // @User: Advanced
    AP_GROUPINFO("EKF_TYPE",  14, AG_AHRS, _ekf_type, HAL_AHRS_EKF_TYPE_DEFAULT),
    ...
    _ekf_flags(flags)
{
    ...
    _ekf_flags |= AG_AHRS::FLAG_ALWAYS_USE_EKF;
#endif
...
void AG_AHRS::init()
{
    ...
    if (_ekf_type.get() == 1) {
        AG_BoardConfig::config_error("EKF1 not available");
    }
    ...
    if (_board_orientation == ROTATION_CUSTOM_OLD) {
        ...
        if (AG_Param::find_top_level_key_by_pointer(this, info.old_key)) {
            ...
            for (info.old_group_element=15; info.old_group_element<=17; info.old_group_element++) {
                if (AG_Param::find_old_parameter(&info, &rpy_param)) {
...
void AG_AHRS::update_trim_rotation_matrices()
{
...
const Vector3f &AG_AHRS::get_gyro(void) const
{
    return _gyro_estimate;
}

const Matrix3f &AG_AHRS::get_rotation_body_to_ned(void) const
{
    return _dcm_matrix;
}

const Vector3f &AG_AHRS::get_gyro_drift(void) const
{
    return _gyro_drift;
}
...
void AG_AHRS::reset_gyro_drift(void)
{
...
void AG_AHRS::update(bool skip_ins_update)
{
    ...
    AG_Module::call_hook_AHRS_update(*this);
#endif
...
/*
 * copy results from a backend over AG_AHRS canonical results.
 * This updates member variables like roll and pitch, as well as
 * updating derived values like sin_roll and sin_pitch.
 */
void AG_AHRS::copy_estimates_from_backend_estimates(const AG_AHRS_Backend::Estimates &results)
{
...
void AG_AHRS::update_notify_from_filter_status(const nav_filter_status &status)
{
    AG_Notify::flags.gps_fusion = status.flags.using_gps; // Drives AG_Notify flag for usable GPS.
    AG_Notify::flags.gps_glitching = status.flags.gps_glitching;
    AG_Notify::flags.have_pos_abs = status.flags.horiz_pos_abs;
}

#if HAL_NAVEKF2_AVAILABLE
void AG_AHRS::update_EKF2(void)
{
    if (!_ekf2_started) {
        ...
        if (!hal.util->was_watchdog_reset()) {
            if (AG_HAL::millis() - start_time_ms < 5000) {
        ...
        if (AG_HAL::millis() - start_time_ms > startup_delay_ms) {
    ...
    if (_ekf2_started) {
        ...
        if (active_EKF_type() == EKFType::TWO) {
            ...
            const AG_InertialSensor &_ins = AP::ins();
...
#if HAL_EXTERNAL_AHRS_ENABLED
void AG_AHRS::update_external(void)
{
...
bool AG_AHRS::get_location(Location &loc) const
{
...
float AG_AHRS::get_error_rp(void) const
{
    return dcm.get_error_rp();
}

float AG_AHRS::get_error_yaw(void) const
{
    return dcm.get_error_yaw();
}

// return a wind estimation vector, in m/s
bool AG_AHRS::wind_estimate(Vector3f &wind) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
Vector3f AG_AHRS::wind_estimate(void) const
{
    Vector3f ret;
 ...
 */
bool AG_AHRS::airspeed_sensor_enabled(void) const
{
...
bool AG_AHRS::airspeed_estimate(float &airspeed_ret) const
{
    bool ret = false;
    ...
    if (airspeed_sensor_enabled()) {
        ...
        if (_wind_max > 0 && AP::gps().status() >= AG_GPS::GPS_OK_FIX_2D) {
...
bool AG_AHRS::airspeed_estimate_true(float &airspeed_ret) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
bool AG_AHRS::airspeed_vector_true(Vector3f &vec) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
bool AG_AHRS::airspeed_health_data(float &innovation, float &innovationVariance, uint32_t &age_ms) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
// on failure.
bool AG_AHRS::synthetic_airspeed(float &ret) const
{
    return dcm.synthetic_airspeed(ret);
}

// true if compass is being used
bool AG_AHRS::use_compass(void)
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
bool AG_AHRS::get_quaternion(Quaternion &quat) const
{
...
bool AG_AHRS::get_secondary_attitude(Vector3f &eulers) const
{
...
bool AG_AHRS::get_secondary_quaternion(Quaternion &quat) const
{
    EKFType secondary_ekf_type;
    if (!get_secondary_EKF_type(secondary_ekf_type)) {
        return false;
...
bool AG_AHRS::get_secondary_position(Location &loc) const
{
    EKFType secondary_ekf_type;
    if (!get_secondary_EKF_type(secondary_ekf_type)) {
        return false;
...
Vector2f AG_AHRS::groundspeed_vector(void)
{
    Vector3f vec;
...
float AG_AHRS::groundspeed(void)
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
bool AG_AHRS::set_origin(const Location &loc)
{
    WITH_SEMAPHORE(_rsem);
#if HAL_NAVEKF2_AVAILABLE
...
bool AG_AHRS::have_inertial_nav(void) const
{
...
bool AG_AHRS::get_velocity_NED(Vector3f &vec) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
bool AG_AHRS::get_mag_field_NED(Vector3f &vec) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        return false;
...
bool AG_AHRS::get_mag_field_correction(Vector3f &vec) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        return false;
...
bool AG_AHRS::get_vert_pos_rate(float &velocity) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
bool AG_AHRS::get_hagl(float &height) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        return false;
...
bool AG_AHRS::get_relative_position_NED_origin(Vector3f &vec) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
// North/East/Down order.
bool AG_AHRS::get_relative_position_NED_home(Vector3f &vec) const
{
    Location originLLH;
    Vector3f originNED;
...
bool AG_AHRS::get_relative_position_NE_origin(Vector2f &posNE) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
// return a relative ground position to the home in meters
// North/East order.
bool AG_AHRS::get_relative_position_NE_home(Vector2f &posNE) const
{
    Location originLLH;
    Vector2f originNE;
...
bool AG_AHRS::get_relative_position_D_origin(float &posD) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
void AG_AHRS::get_relative_position_D_home(float &posD) const
{
    if (!_home_is_set) {
    ...
    if (!get_relative_position_D_origin(originD) ||
        ...
        if (_gps_use == GPSUse::EnableWithHeight &&
            gps.status() >= AG_GPS::GPS_OK_FIX_3D) {
 ...
 */
AG_AHRS::EKFType AG_AHRS::ekf_type(void) const
{
...
AG_AHRS::EKFType AG_AHRS::active_EKF_type(void) const
{
    ...
    if (ret != EKFType::NONE &&
        ...
        if (hal.util->get_soft_armed() &&
            ...
            should_use_gps &&
            AP::gps().status() >= AG_GPS::GPS_OK_FIX_3D) {
...
bool AG_AHRS::get_secondary_EKF_type(EKFType &secondary_ekf_type) const
{
...
*/
bool AG_AHRS::healthy(void) const
{
...
bool AG_AHRS::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
    bool ret = true;
    if (!healthy()) {
...
bool AG_AHRS::initialised(void) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
        return true;
    ...
    case EKFType::TWO:
        ...
        return (_ekf2_started && (AG_HAL::millis() - start_time_ms > AG_AHRS_NAVEKF_SETTLE_TIME_MS));
#endif

#if HAL_NAVEKF3_AVAILABLE
    case EKFType::THREE:
        // initialisation complete 10sec after ekf has started
        return (_ekf3_started && (AG_HAL::millis() - start_time_ms > AG_AHRS_NAVEKF_SETTLE_TIME_MS));
#endif
...
bool AG_AHRS::get_filter_status(nav_filter_status &status) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
...
void  AG_AHRS::writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates, const Vector2f &rawGyroRates, const uint32_t msecFlowMeas, const Vector3f &posOffset, float heightOverride)
{
#if HAL_NAVEKF2_AVAILABLE
...
bool AG_AHRS::getOptFlowSample(uint32_t& timeStamp_ms, Vector2f& flowRate, Vector2f& bodyRate, Vector2f& losPred) const
{
#if HAL_NAVEKF3_AVAILABLE
...
void  AG_AHRS::writeBodyFrameOdom(float quality, const Vector3f &delPos, const Vector3f &delAng, float delTime, uint32_t timeStamp_ms, uint16_t delay_ms, const Vector3f &posOffset)
{
#if HAL_NAVEKF3_AVAILABLE
...
void AG_AHRS::writeExtNavData(const Vector3f &pos, const Quaternion &quat, float posErr, float angErr, uint32_t timeStamp_ms, uint16_t delay_ms, uint32_t resetTime_ms)
{
#if HAL_NAVEKF2_AVAILABLE
...
void AG_AHRS::writeDefaultAirSpeed(float airspeed, float uncertainty)
{
#if HAL_NAVEKF2_AVAILABLE
...
void AG_AHRS::writeExtNavVelData(const Vector3f &vel, float err, uint32_t timeStamp_ms, uint16_t delay_ms)
{
#if HAL_NAVEKF2_AVAILABLE
...
void AG_AHRS::getControlLimits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
 ...
 */
float AG_AHRS::getControlScaleZ(void) const
{
    if (active_EKF_type() == EKFType::NONE) {
...
bool AG_AHRS::getMagOffsets(uint8_t mag_idx, Vector3f &magOffsets) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
        return false;
...
void AG_AHRS::getCorrectedDeltaVelocityNED(Vector3f& ret, float& dt) const
{
    int8_t imu_idx = -1;
    Vector3f accel_bias;
    switch (active_EKF_type()) {
...
bool AG_AHRS::attitudes_consistent(char *failure_msg, const uint8_t failure_msg_len) const
{
...
uint32_t AG_AHRS::getLastYawResetAngle(float &yawAng)
{
    switch (active_EKF_type()) {
...
uint32_t AG_AHRS::getLastPosNorthEastReset(Vector2f &pos)
{
    switch (active_EKF_type()) {
...
// returns the time of the last reset or 0 if no reset has ever occurred
uint32_t AG_AHRS::getLastVelNorthEastReset(Vector2f &vel) const
{
    switch (active_EKF_type()) {
...
// returns the time of the last reset or 0 if no reset has ever occurred
uint32_t AG_AHRS::getLastPosDownReset(float &posDelta)
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        return 0;
...
bool AG_AHRS::resetHeightDatum(void)
{
    // support locked access functions to AHRS data
    WITH_SEMAPHORE(_rsem);
...
void AG_AHRS::send_ekf_status_report(GCS_MAVLINK &link) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
...
bool AG_AHRS::get_origin(EKFType type, Location &ret) const
{
    switch (type) {
    case EKFType::NONE:
 ...
 */
bool AG_AHRS::get_origin(Location &ret) const
{
...
bool AG_AHRS::get_hgt_ctrl_limit(float& limit) const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
void AG_AHRS::set_terrain_hgt_stable(bool stable)
{
...
bool AG_AHRS::get_innovations(Vector3f &velInnov, Vector3f &posInnov, Vector3f &magInnov, float &tasInnov, float &yawInnov) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
...
bool AG_AHRS::is_vibration_affected() const
{
    switch (ekf_type()) {
#if HAL_NAVEKF3_AVAILABLE
    case EKFType::THREE:
...
bool AG_AHRS::get_variances(float &velVar, float &posVar, float &hgtVar, Vector3f &magVar, float &tasVar) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
        // We are not using an EKF so no data
...
// get a source's velocity innovations.  source should be from 0 to 7 (see AG_NavEKF_Source::SourceXY)
// returns true on success and results are placed in innovations and variances arguments
bool AG_AHRS::get_vel_innovations_and_variances_for_source(uint8_t source, Vector3f &innovations, Vector3f &variances) const
{
    switch (ekf_type()) {
    case EKFType::NONE:
        // We are not using an EKF so no data
    ...
    case EKFType::THREE:
        // use EKF to get variance
        return EKF3.getVelInnovationsAndVariancesForSource((AG_NavEKF_Source::SourceXY)source, innovations, variances);
#endif
...
uint8_t AG_AHRS::get_active_airspeed_index() const
{
#if AP_AIRSPEED_ENABLED
...
uint8_t AG_AHRS::get_primary_IMU_index() const
{
    int8_t imu = -1;
    switch (active_EKF_type()) {
    case EKFType::NONE:
...
int8_t AG_AHRS::get_primary_core_index() const
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
#if AG_AHRS_SIM_ENABLED
...
uint8_t AG_AHRS::get_primary_accel_index(void) const
{
...
uint8_t AG_AHRS::get_primary_gyro_index(void) const
{
    if (ekf_type() != EKFType::NONE) {
        return get_primary_IMU_index();
    }
...
void AG_AHRS::check_lane_switch(void)
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
void AG_AHRS::request_yaw_reset(void)
{
    switch (active_EKF_type()) {
    case EKFType::NONE:
        break;
...
void AG_AHRS::set_posvelyaw_source_set(uint8_t source_set_idx)
{
#if HAL_NAVEKF3_AVAILABLE
...
uint8_t AG_AHRS::get_posvelyaw_source_set() const
{
#if HAL_NAVEKF3_AVAILABLE
...
bool AG_AHRS::using_noncompass_for_yaw(void) const
{
    switch (active_EKF_type()) {
#if HAL_NAVEKF2_AVAILABLE
    case EKFType::TWO:
...
bool AG_AHRS::using_extnav_for_yaw(void) const
{
    switch (active_EKF_type()) {
#if HAL_NAVEKF2_AVAILABLE
    case EKFType::TWO:
...
void AG_AHRS::set_alt_measurement_noise(float noise)
{
#if HAL_NAVEKF2_AVAILABLE
...
// check if non-compass sensor is providing yaw.  Allows compass pre-arm checks to be bypassed
const EKFGSF_yaw *AG_AHRS::get_yaw_estimator(void) const
{
    switch (active_EKF_type()) {
#if HAL_NAVEKF2_AVAILABLE
    case EKFType::TWO:
...
// singleton instance
AG_AHRS *AG_AHRS::_singleton;

namespace AP {

AG_AHRS &ahrs()
{
    return *AG_AHRS::get_singleton();
}
