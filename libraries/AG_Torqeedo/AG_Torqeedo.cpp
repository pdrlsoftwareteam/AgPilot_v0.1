...
// parameters
const AG_Param::GroupInfo AG_Torqeedo::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("TYPE", 1, AG_Torqeedo, _type, (int8_t)ConnectionType::TYPE_DISABLED, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("ONOFF_PIN", 2, AG_Torqeedo, _pin_onoff, -1),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("DE_PIN", 3, AG_Torqeedo, _pin_de, -1),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 4, AG_Torqeedo, _options, (int8_t)options::LOG),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("POWER", 5, AG_Torqeedo, _motor_power, 100),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("SLEW_TIME", 6, AG_Torqeedo, _slew_time, 2.0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("DIR_DELAY", 7, AG_Torqeedo, _dir_delay, 1.0),

    AP_GROUPEND
};

AG_Torqeedo::AG_Torqeedo()
{
    _singleton = this;
...
void AG_Torqeedo::init()
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_Torqeedo::thread_main, void), "torqeedo", 2048, AG_HAL::Scheduler::PRIORITY_RCOUT, 1)) {
        return;
    }
}

// initialise serial port and gpio pins (run from background thread)
bool AG_Torqeedo::init_internals()
{
    // find serial driver and initialise
    const AG_SerialManager &serial_manager = AP::serialmanager();
    _uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Torqeedo, 0);
    if (_uart == nullptr) {
        return false;
    }
...
bool AG_Torqeedo::enabled() const
{
...
bool AG_Torqeedo::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return true;
...
const char * AG_Torqeedo::map_master_error_code_to_string(uint8_t code) const
{
    switch (code) {
    case 2:
...
void AG_Torqeedo::report_error_codes()
{
...
bool AG_Torqeedo::get_batt_info(float &voltage, float &current_amps, float &temp_C, uint8_t &pct_remaining) const
{

    // use battery info from display_system_state if available (tiller connection)
    if ((AG_HAL::millis() - _display_system_state.last_update_ms) <= TORQEEDO_BATT_TIMEOUT_MS) {
    ...
    if ((AG_HAL::millis() - _motor_param.last_update_ms) <= TORQEEDO_BATT_TIMEOUT_MS) {
...
bool AG_Torqeedo::get_batt_capacity_Ah(uint16_t &amp_hours) const
{
...
bool AG_Torqeedo::parse_byte(uint8_t b)
{
    ...
    case ParseState::WAITING_FOR_FOOTER:
        if (b == TORQEEDO_PACKET_FOOTER) {
            ...
            _parse_success_count++;
            {
                ...
                WITH_SEMAPHORE(_last_healthy_sem);
                _last_received_ms = AG_HAL::millis();
            }
...
void AG_Torqeedo::parse_message()
{
    ...
    if ((_type == ConnectionType::TYPE_TILLER) && (msg_addr == MsgAddress::DISPLAY)) {
        ...
        case DisplayMsgId::SYSTEM_STATE :
            if (_received_buff_len == 30) {
                ...
                _display_system_state.last_update_ms = AG_HAL::millis();
    ...
    if ((_type == ConnectionType::TYPE_MOTOR) && (msg_addr == MsgAddress::BUS_MASTER)) {
        ...
        case MotorMsgId::PARAM:
            if (_received_buff_len == 15) {
                ...
                _motor_param.last_update_ms = AG_HAL::millis();
...
void AG_Torqeedo::check_for_send_end()
{
    if (_send_delay_us == 0) {
        // not sending
        return;
    ...
    if (AG_HAL::micros() - _send_start_us < _send_delay_us) {
...
uint32_t AG_Torqeedo::calc_send_delay_us(uint8_t num_bytes)
{
...
void AG_Torqeedo::set_expected_reply_msgid(uint8_t msg_id)
{
    _reply_msgid = msg_id;
    _reply_wait_start_ms = AG_HAL::millis();
}

// check for timeout waiting for reply message
void AG_Torqeedo::check_for_reply_timeout()
{
    // return immediately if not waiting for reply
    if (_reply_wait_start_ms == 0) {
        return;
    }
    if (AG_HAL::millis() - _reply_wait_start_ms > TORQEEDO_REPLY_TIMEOUT_MS) {
...
void AG_Torqeedo::set_reply_received()
{
    _reply_wait_start_ms = 0;
}
...
bool AG_Torqeedo::send_message(const uint8_t msg_contents[], uint8_t num_bytes)
{
...
bool AG_Torqeedo::add_byte_to_message(uint8_t byte_to_add, uint8_t msg_buff[], uint8_t msg_buff_size, uint8_t &num_bytes) const
{
...
void AG_Torqeedo::send_motor_speed_cmd()
{
    ...
    if (send_message(mot_speed_cmd_buff, ARRAY_SIZE(mot_speed_cmd_buff))) {
        ...
        WITH_SEMAPHORE(_last_healthy_sem);
        _last_send_motor_ms = AG_HAL::millis();
    }
}

// send request to motor to reply with a particular message
// msg_id can be INFO, STATUS or PARAM
void AG_Torqeedo::send_motor_msg_request(MotorMsgId msg_id)
{
    // prepare message
...
int16_t AG_Torqeedo::calc_motor_speed_limited(int16_t desired_motor_speed)
{
    const uint32_t now_ms = AG_HAL::millis();
...
void AG_Torqeedo::log_TRQD(bool force_logging)
{
...
// send ESC telemetry
void AG_Torqeedo::update_esc_telem(float rpm, float voltage, float current_amps, float esc_tempC, float motor_tempC)
{
    ...
    AG_ESC_Telem_Backend::TelemetryData telem_dat {};
    ...
    update_telem_data(telem_esc_index, telem_dat, AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE |
...
// get the AG_Torqeedo singleton
AG_Torqeedo *AG_Torqeedo::get_singleton()
{
    return _singleton;
}

AG_Torqeedo *AG_Torqeedo::_singleton = nullptr;

namespace AP {
AG_Torqeedo *torqeedo()
{
    return AG_Torqeedo::get_singleton();
}
};
