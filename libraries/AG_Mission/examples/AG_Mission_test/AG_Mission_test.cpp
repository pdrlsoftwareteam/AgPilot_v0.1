...
private:
    ...
    bool start_cmd(const AG_Mission::Mission_Command& cmd);
    bool verify_cmd(const AG_Mission::Mission_Command& cmd);
    ...
    AG_Mission mission{
            FUNCTOR_BIND_MEMBER(&MissionTest::start_cmd, bool, const AG_Mission::Mission_Command &),
            FUNCTOR_BIND_MEMBER(&MissionTest::verify_cmd, bool, const AG_Mission::Mission_Command &),
...
bool MissionTest::start_cmd(const AG_Mission::Mission_Command& cmd)
{
...
bool MissionTest::verify_cmd(const AG_Mission::Mission_Command& cmd)
{
    if (AG_Mission::is_nav_cmd(cmd)) {
        num_nav_cmd_runs++;
...
void MissionTest::run_resume_test()
{
    ...
    uint32_t start_time = AG_HAL::millis();
    while(AG_HAL::millis() - start_time < 5000) {
        mission.update();
    }
...
void MissionTest::run_set_current_cmd_while_stopped_test()
{
    ...
    uint32_t start_time = AG_HAL::millis();
    while(AG_HAL::millis() - start_time < 2000) {
        mission.update();
    }
    ...
    while(mission.state() != AG_Mission::MISSION_COMPLETE) {
        mission.update();
    }

    // pause for two seconds
    start_time = AG_HAL::millis();
    while(AG_HAL::millis() - start_time < 2000) {
        mission.update();
    }
...
{
    hal.console->printf("AG_Mission library test\n\n");
