...
const AG_Param::GroupInfo AG_Mission::var_info[] = {
    ...
    // @User: Advanced
    // @ReadOnly: True
    AP_GROUPINFO_FLAGS("TOTAL",  0, AG_Mission, _cmd_total, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
    ...
    // @User: Advanced
    AP_GROUPINFO("RESTART",  1, AG_Mission, _restart, AP_MISSION_RESTART_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS",  2, AG_Mission, _options, AP_MISSION_OPTIONS_DEFAULT),
...
// storage object
StorageAccess AG_Mission::_storage(StorageManager::StorageMission);

HAL_Semaphore AG_Mission::_rsem;
...
void AG_Mission::init()
{
    ...
    _last_change_time_ms = AG_HAL::millis();
}

/// start - resets current commands to point to the beginning of the mission
///     To-Do: should we validate the mission first and return true/false?
void AG_Mission::start()
{
...
bool AG_Mission::is_takeoff_next(uint16_t cmd_index)
{
    Mission_Command cmd = {};
...
bool AG_Mission::starts_with_takeoff_cmd()
{
...
*/
bool AG_Mission::continue_after_land_check_for_takeoff()
{
...
void AG_Mission::start_or_resume()
{
...
void AG_Mission::truncate(uint16_t index)
{
...
void AG_Mission::on_mission_timestamp_change()
{
    _jump_tag.age = 0;
}

bool AG_Mission::verify_command(const Mission_Command& cmd)
{
    switch (cmd.id) {
...
bool AG_Mission::start_command(const Mission_Command& cmd)
{
...
bool AG_Mission::add_cmd(Mission_Command& cmd)
{
...
bool AG_Mission::replace_cmd(uint16_t index, const Mission_Command& cmd)
{
    // sanity check index
...
bool AG_Mission::is_nav_cmd(const Mission_Command& cmd)
{
...
bool AG_Mission::get_next_nav_cmd(uint16_t start_index, Mission_Command& cmd)
{
...
int32_t AG_Mission::get_next_ground_course_cd(int32_t default_angle)
{
    Mission_Command cmd;
...
bool AG_Mission::set_current_cmd(uint16_t index, bool rewind)
{
...
bool AG_Mission::restart_current_nav_cmd()
{
...
bool AG_Mission::set_item(uint16_t index, mavlink_mission_item_int_t& src_packet)
{
    ...
    return AG_Mission::replace_cmd(index, cmd);
}

bool AG_Mission::get_item(uint16_t index, mavlink_mission_item_int_t& ret_packet) const
{
...
bool AG_Mission::read_cmd_from_storage(uint16_t index, Mission_Command& cmd) const
{
    ...
    if (stored_in_location(cmd.id)) {
        ...
        if (b1 == 0) {
            AG_HAL::panic("May not store location for 16-bit commands");
        }
#endif
...
bool AG_Mission::stored_in_location(uint16_t id)
{
    switch (id) {
...
bool AG_Mission::write_cmd_to_storage(uint16_t index, const Mission_Command& cmd)
{
    WITH_SEMAPHORE(_rsem);
...
void AG_Mission::write_home_to_storage()
{
...
MAV_MISSION_RESULT AG_Mission::sanity_check_params(const mavlink_mission_item_int_t& packet)
{
    uint8_t nan_mask;
...
// mavlink_int_to_mission_cmd - converts mavlink message to an AG_Mission::Mission_Command object which can be stored to eeprom
//  return MAV_MISSION_ACCEPTED on success, MAV_MISSION_RESULT error on failure
MAV_MISSION_RESULT AG_Mission::mavlink_int_to_mission_cmd(const mavlink_mission_item_int_t& packet, AG_Mission::Mission_Command& cmd)
{
...
MAV_MISSION_RESULT AG_Mission::convert_MISSION_ITEM_to_MISSION_ITEM_INT(const mavlink_mission_item_t &packet,
...
MAV_MISSION_RESULT AG_Mission::convert_MISSION_ITEM_INT_to_MISSION_ITEM(const mavlink_mission_item_int_t &item_int,
...
// mavlink_cmd_long_to_mission_cmd - converts a mavlink cmd long to an AG_Mission::Mission_Command object which can be stored to eeprom
// return MAV_MISSION_ACCEPTED on success, MAV_MISSION_RESULT error on failure
MAV_MISSION_RESULT AG_Mission::mavlink_cmd_long_to_mission_cmd(const mavlink_command_long_t& packet, AG_Mission::Mission_Command& cmd)
{
...
// mission_cmd_to_mavlink_int - converts an AG_Mission::Mission_Command object to a mavlink message which can be sent to the GCS
...
bool AG_Mission::mission_cmd_to_mavlink_int(const AG_Mission::Mission_Command& cmd, mavlink_mission_item_int_t& packet)
{
    // command's position in mission list and mavlink id
...
bool AG_Mission::advance_current_nav_cmd(uint16_t starting_index)
{
...
///     accounts for do-jump commands
void AG_Mission::advance_current_do_cmd()
{
...
bool AG_Mission::get_next_cmd(uint16_t start_index, Mission_Command& cmd, bool increment_jump_num_times_if_found, bool send_gcs_msg)
{
...
///     accounts for do_jump commands but never increments the jump's num_times_run (advance_current_nav_cmd is responsible for this)
bool AG_Mission::get_next_do_cmd(uint16_t start_index, Mission_Command& cmd)
{
    Mission_Command temp_cmd;
...
bool AG_Mission::jump_to_tag(const uint16_t tag)
{
...
uint16_t AG_Mission::get_index_of_jump_tag(const uint16_t tag) const
{
...
#if AP_SCRIPTING_ENABLED
bool AG_Mission::get_last_jump_tag(uint16_t &tag, uint16_t &age) const
{
    if (_jump_tag.age == 0) {
        return false;
    }
...
void AG_Mission::init_jump_tracking()
{
...
int16_t AG_Mission::get_jump_times_run(const Mission_Command& cmd)
{
...
void AG_Mission::increment_jump_times_run(Mission_Command& cmd, bool send_gcs_msg)
{
...
// check_eeprom_version - checks version of missions stored in eeprom matches this library
// command list will be cleared if they do not match
void AG_Mission::check_eeprom_version()
{
...
// be found.
uint16_t AG_Mission::get_landing_sequence_start()
{
...
bool AG_Mission::jump_to_abort_landing_sequence(void)
{
    Location current_loc;
...
bool AG_Mission::is_best_land_sequence(void)
{
...
bool AG_Mission::distance_to_landing(uint16_t index, float &tot_distance, Location prev_loc)
{
    Mission_Command temp_cmd;
    tot_distance = 0.0f;
...
bool AG_Mission::is_landing_type_cmd(uint16_t id) const
{
    switch (id) {
    case MAV_CMD_NAV_LAND:
    case MAV_CMD_NAV_VTOL_LAND:
...
bool AG_Mission::is_takeoff_type_cmd(uint16_t id) const
{
    switch (id) {
    case MAV_CMD_NAV_TAKEOFF:
    case MAV_CMD_NAV_VTOL_TAKEOFF:
...
const char *AG_Mission::Mission_Command::type() const
{
    switch (id) {
    case MAV_CMD_NAV_WAYPOINT:
        return "WP";
    ...
    default:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Mission command with ID %u has no string", id);
#endif
        return "?";
    }
}
 ...
 */
uint16_t AG_Mission::get_command_id(uint16_t index) const
{
    const uint16_t pos_in_storage = 4 + (index * AP_MISSION_EEPROM_COMMAND_SIZE);
    uint8_t b[3] {};
 ...
 */
bool AG_Mission::contains_item(MAV_CMD command) const
{
    const auto count = num_commands();
    for (uint16_t i = 1; i < count; i++) {
...
bool AG_Mission::cmd_has_location(const uint16_t command)
{
 ...
 */
bool AG_Mission::contains_terrain_alt_items(void)
{
...
bool AG_Mission::calculate_contains_terrain_alt_items(void) const
{
    const auto count = num_commands();
    for (uint16_t i = 1; i < count; i++) {
...
void AG_Mission::reset_wp_history(void)
{
...
void AG_Mission::update_exit_position(void)
{
...
bool AG_Mission::calc_rewind_pos(Mission_Command& rewind_cmd)
{
...
*/
void AG_Mission::format_conversion(uint8_t tag_byte, const Mission_Command &cmd, PackedContent &packed_content) const
{
...
// singleton instance
AG_Mission *AG_Mission::_singleton;
...
AG_Mission *mission()
{
    return AG_Mission::get_singleton();
}
