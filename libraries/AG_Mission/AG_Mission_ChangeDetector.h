/// @file    AG_Mission_ChangeDetector.h
/// @brief   Detects changes in the next few nav commands in the mission

/*
 *   The AG_Mission_ChangeDetector library:
...
/// @class    AG_Mission_ChangeDetector
...
private:
    ...
    struct {
        ...
        uint16_t curr_cmd_index;            // local copy of AG_Mission's current command index
        uint8_t cmd_count;                  // number of commands in the cmd array
        AG_Mission::Mission_Command cmd[mis_change_detect_cmd_max]; // local copy of the next few mission commands
    } mis_change_detect;
};
