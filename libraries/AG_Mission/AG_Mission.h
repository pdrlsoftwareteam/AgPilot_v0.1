...
public:
    ...
    // constructor
    AG_Mission(mission_cmd_fn_t cmd_start_fn, mission_cmd_fn_t cmd_verify_fn, mission_complete_fn_t mission_complete_fn) :
        ...
        _prev_nav_cmd_wp_index(AP_MISSION_CMD_INDEX_NONE)
    {
        ...
        if (_singleton != nullptr) {
            AG_HAL::panic("Mission must be singleton");
        }
#endif
        _singleton = this;
    ...
    static AG_Mission *get_singleton()
    {
        return _singleton;
    }

    /* Do not allow copies */
    CLASS_NO_COPY(AG_Mission);
    ...
    static MAV_MISSION_RESULT mavlink_int_to_mission_cmd(const mavlink_mission_item_int_t& packet, AG_Mission::Mission_Command& cmd);
    ...
    // return MAV_MISSION_ACCEPTED on success, MAV_MISSION_RESULT error on failure
    static MAV_MISSION_RESULT mavlink_cmd_long_to_mission_cmd(const mavlink_command_long_t& packet, AG_Mission::Mission_Command& cmd);
    ...
    static bool mission_cmd_to_mavlink_int(const AG_Mission::Mission_Command& cmd, mavlink_mission_item_int_t& packet);
    ...
    // get a reference to the AG_Mission semaphore, allowing an external caller to lock the
...
private:
    ...
    bool start_command_do_aux_function(const AG_Mission::Mission_Command& cmd);
    bool start_command_do_gripper(const AG_Mission::Mission_Command& cmd);
    bool start_command_do_servorelayevents(const AG_Mission::Mission_Command& cmd);
    bool start_command_camera(const AG_Mission::Mission_Command& cmd);
    bool start_command_parachute(const AG_Mission::Mission_Command& cmd);
    bool command_do_set_repeat_dist(const AG_Mission::Mission_Command& cmd);

    bool start_command_do_sprayer(const AG_Mission::Mission_Command& cmd);
    bool start_command_do_scripting(const AG_Mission::Mission_Command& cmd);
    bool start_command_do_gimbal_manager_pitchyaw(const AG_Mission::Mission_Command& cmd);
