...
void AP_ICEngine::update(void)
{
    ...
    case ICE_START_HEIGHT_DELAY: {
        ...
        if (!should_run) {
            ...
            if (height_pending) {
                ...
                gcs().send_text(AGPILOT_SEVERITY_INFO, "Starting height reached %.1f",
    ...
    case ICE_START_DELAY:
        if (!should_run) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Starting engine");
            state = ICE_STARTING;
        }
        break;
    ...
    case ICE_STARTING:
        if (!should_run) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Engine running");
            state = ICE_RUNNING;
        }
        break;
    ...
    case ICE_RUNNING:
        if (!should_run) {
            state = ICE_OFF;
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Stopped engine");
            ...
            if (!rpm.get_rpm(rpm_instance-1, rpm_value) ||
                ...
                state = ICE_START_DELAY;
                gcs().send_text(AGPILOT_SEVERITY_INFO, "Uncommanded engine stop");
            }
        }
        break;
    }
    ...
    if (rpm_instance > 0 && redline_rpm > 0 && rpm.get_rpm(rpm_instance-1, rpm_value)) {
        ...
        if (!redline.flag && filtered_rpm_value > redline_rpm) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Engine: Above redline RPM");
            redline.flag = true;
...
bool AP_ICEngine::engine_control(float start_control, float cold_start, float height_delay)
{
    ...
    if (state == ICE_RUNNING || state == ICE_START_DELAY || state == ICE_STARTING) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Engine: already running");
        return false;
    }
    RC_Channel *c = rc().channel(start_chan-1);
    if (c != nullptr && rc().has_valid_input()) {
        ...
        if (cvalue >= start_chan_min_pwm && cvalue <= RC_Channel::AUX_PWM_TRIGGER_LOW) {
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Engine: start control disabled");
            return false;
        }
    }
    if (height_delay > 0) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Takeoff height set to %.1fm", (double)height_delay);
        return true;
    }
    state = ICE_STARTING;
    return true;
