...
void AG_Generator_RichenPower::init()
{
    const AG_SerialManager &serial_manager = AP::serialmanager();

    uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Generator, 0);
    if (uart != nullptr) {
        const uint32_t baud = serial_manager.find_baudrate(AG_SerialManager::SerialProtocol_Generator, 0);
...
void AG_Generator_RichenPower::move_header_in_buffer(uint8_t initial_offset)
{
...
bool AG_Generator_RichenPower::get_reading()
{
...
// current RPM.
void AG_Generator_RichenPower::update_heat()
{
...
bool AG_Generator_RichenPower::generator_ok_to_run() const
{
...
constexpr float AG_Generator_RichenPower::heat_required_for_run()
{
...
void AG_Generator_RichenPower::check_maintenance_required()
{
    ...
    if (!AP::generator()->option_set(AG_Generator::Option::INHIBIT_MAINTENANCE_WARNINGS)) {
        const uint32_t now = AG_HAL::millis();
...
*/
void AG_Generator_RichenPower::update(void)
{
    if (uart == nullptr) {
        return;
    }
...
void AG_Generator_RichenPower::update_runstate()
{
...
void AG_Generator_RichenPower::Log_Write()
{
...
bool AG_Generator_RichenPower::pre_arm_check(char *failmsg, uint8_t failmsg_len) const
{
    if (uart == nullptr) {
...
void AG_Generator_RichenPower::update_frontend_readings(void)
{
...
bool AG_Generator_RichenPower::healthy() const
{
    const uint32_t now = AG_HAL::millis();
...
void AG_Generator_RichenPower::send_generator_status(const GCS_MAVLINK &channel)
{
    if (last_reading_ms == 0) {
        // nothing to report
        return;
...
bool AG_Generator_RichenPower::stop() 
{
    set_pilot_desired_runstate(RunState::STOP); 
    return true;
}

bool AG_Generator_RichenPower::idle()
{
    set_pilot_desired_runstate(RunState::IDLE);
    return true;
}

bool AG_Generator_RichenPower::run()
{
