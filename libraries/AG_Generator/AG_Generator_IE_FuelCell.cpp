...
#include "AG_Generator_IE_FuelCell.h"
...
void AG_Generator_IE_FuelCell::init()
{
    _uart = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_Generator, 0);

    if (_uart == nullptr) {
        gcs().send_text(MAV_SEVERITY_INFO, "Generator: No serial port found");
        return;
    }
    _uart->begin(AP::serialmanager().find_baudrate(AG_SerialManager::SerialProtocol_Generator, 0));
    _health_warn_last_ms = AG_HAL::millis();
}

// Update fuelcell, expected to be called at 20hz
void AG_Generator_IE_FuelCell::update()
{
    if (_uart == nullptr) {
        return;
    }
...
bool AG_Generator_IE_FuelCell::decode(char c)
{
    // Start of a string
    if (c == '<') {
...
// Check for arming
bool AG_Generator_IE_FuelCell::pre_arm_check(char *failmsg, uint8_t failmsg_len) const
{
...
const AG_Generator_IE_FuelCell::Lookup_State AG_Generator_IE_FuelCell::lookup_state[] = {
...
void AG_Generator_IE_FuelCell::check_status(const uint32_t now)
{
    // Check driver health
...
bool AG_Generator_IE_FuelCell::check_for_err_code_if_changed(char* msg_txt, uint8_t msg_len)
{
