...
class AG_ExpandingArrayGeneric
{
public:

    AG_ExpandingArrayGeneric(uint16_t element_size, uint16_t elements_per_chunk) :
        elem_size(element_size),
        chunk_size(elements_per_chunk)
    {}

    ~AG_ExpandingArrayGeneric(void);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_ExpandingArrayGeneric);
...
template <typename T>
class AG_ExpandingArray : public AG_ExpandingArrayGeneric
{
public:

    AG_ExpandingArray(uint16_t elements_per_chunk) :
        AG_ExpandingArrayGeneric(sizeof(T), elements_per_chunk)
    {}

    /* Do not allow copies */
    CLASS_NO_COPY(AG_ExpandingArray);
