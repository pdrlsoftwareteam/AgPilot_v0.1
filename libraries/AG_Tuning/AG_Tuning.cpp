...
const AG_Param::GroupInfo AG_Tuning::var_info[] = {
    // @Param: CHAN
    ...
    // @User: Standard
    AP_GROUPINFO("CHAN", 1, AG_Tuning, channel, 0),
    ...
    // @Range: 900 2100
    // @User: Standard
    AP_GROUPINFO("CHAN_MIN", 2, AG_Tuning, channel_min, 1000),
    ...
    // @Range: 900 2100
    // @User: Standard
    AP_GROUPINFO("CHAN_MAX", 3, AG_Tuning, channel_max, 2000),
    ...
    // @User: Standard
    AP_GROUPINFO("SELECTOR", 4, AG_Tuning, selector, 0),
    ...
    // @User: Standard
    AP_GROUPINFO("RANGE", 5, AG_Tuning, range, 2.0f),
    ...
    // @User: Standard
    AP_GROUPINFO("MODE_REVERT", 6, AG_Tuning, mode_revert, 1),
    ...
    // @Range: 0 1
    // @User: Standard
    AP_GROUPINFO("ERR_THRESH", 7, AG_Tuning, error_threshold, 0.15f),
...
*/
void AG_Tuning::check_selector_switch(void)
{
    if (selector == 0) {
    ...
    if (selector_in >= RC_Channel::AUX_PWM_TRIGGER_HIGH) {
        ...
        if (selector_start_ms == 0) {
            selector_start_ms = AG_HAL::millis();
        }
        uint32_t hold_time = AG_HAL::millis() - selector_start_ms;
        if (hold_time > 5000 && changed) {
            // save tune
            save_parameters();
            re_center();
            gcs().send_text(MAV_SEVERITY_INFO, "Tuning: Saved");
            AG_Notify::events.tune_save = 1;
            changed = false;
            need_revert = 0;
        }
 ...
 */
void AG_Tuning::check_input(uint8_t flightmode)
{
    ...
    if (mid_point_wait) {
        ...
        AG_Notify::events.tune_started = 1;
    }
    last_channel_value = chan_value;
...
/*
  log a tuning change
 */
void AG_Tuning::Log_Write_Parameter_Tuning(float value)
{
// @LoggerMessage: PRTN
 ...
 */
void AG_Tuning::save_parameters(void)
{
...
/*
  save parameters in the set
 */
void AG_Tuning::revert_parameters(void)
{
    uint8_t set = (uint8_t)parmset.get();
    if (set < set_base) {
        // single parameter tuning
 ...
 */
void AG_Tuning::next_parameter(void)
{
    uint8_t set = (uint8_t)parmset.get();
    if (set < set_base) {
    ...
    for (uint8_t i=0; tuning_sets[i].num_parms != 0; i++) {
        if (tuning_sets[i].set+set_base == set) {
            ...
            AG_Notify::events.tune_next = current_parm_index+1;
            break;
        }
    }
}
 ...
 */
const char *AG_Tuning::get_tuning_name(uint8_t parm)
{
