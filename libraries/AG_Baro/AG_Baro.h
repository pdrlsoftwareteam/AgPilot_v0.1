...
class AG_Baro_Backend;

class AG_Baro
{
    friend class AG_Baro_Backend;
    friend class AG_Baro_SITL; // for access to sensors[]

public:
    AG_Baro();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_Baro);

    // get singleton
    static AG_Baro *get_singleton(void) {
        return _singleton;
    }
    ...
    void handle_external(const AG_ExternalAHRS::baro_data_message_t &pkt);
#endif
...
private:
    // singleton
    static AG_Baro *_singleton;
    
    // how many drivers do we have?
    uint8_t _num_drivers;
    AG_Baro_Backend *drivers[BARO_MAX_DRIVERS];
    ...
    bool _add_backend(AG_Baro_Backend *backend);
