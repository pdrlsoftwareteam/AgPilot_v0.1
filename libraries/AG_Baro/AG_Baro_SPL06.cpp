...
AG_Baro_SPL06::AG_Baro_SPL06(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : AG_Baro_Backend(baro)
    , _dev(std::move(dev))
{
}

AG_Baro_Backend *AG_Baro_SPL06::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    ...
    AG_Baro_SPL06 *sensor = new AG_Baro_SPL06(baro, std::move(dev));
...
// protocol sensor(I2C and SPI), sometimes it takes one SPI operation to convert it to SPI mode after it starts up.
    ...
    // request 50Hz update
    _timer_counter = -1;
    _dev->register_periodic_callback(20 * AP_USEC_PER_MSEC, FUNCTOR_BIND_MEMBER(&AG_Baro_SPL06::_timer, void));

    return true;
}

int32_t AG_Baro_SPL06::raw_value_scale_factor(uint8_t oversampling)
{
...
void AG_Baro_SPL06::_timer(void)
{
    uint8_t buf[3];
...
void AG_Baro_SPL06::update(void)
{
    WITH_SEMAPHORE(_sem);
...
void AG_Baro_SPL06::_update_temperature(int32_t temp_raw)
{
...
// calculate pressure
void AG_Baro_SPL06::_update_pressure(int32_t press_raw)
{
