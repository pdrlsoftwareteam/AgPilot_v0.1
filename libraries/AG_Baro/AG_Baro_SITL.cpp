 ...
 */
AG_Baro_SITL::AG_Baro_SITL(AG_Baro &baro) :
    _sitl(AP::sitl()),
    _has_sample(false),
...
#endif
        set_bus_id(_instance, AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_SITL, 0, _instance, DEVTYPE_BARO_SITL));
        hal.scheduler->register_timer_process(FUNCTOR_BIND(this, &AG_Baro_SITL::_timer, void));
    }
}

// adjust for board temperature warmup on start-up
void AG_Baro_SITL::temperature_adjustment(float &p, float &T)
{
    const float tsec = AG_HAL::millis() * 0.001f;
...
void AG_Baro_SITL::_timer()
{
    ...
    AG_Baro::SimpleUnderWaterAtmosphere(-sim_alt * 0.001f, rho, delta, theta);
    float p = SSL_AIR_PRESSURE * delta;
    float T = KELVIN_TO_C(SSL_AIR_TEMPERATURE * theta);
#endif
...
bool AG_Baro_SITL::healthy(uint8_t instance) 
{
    return !_sitl->baro[instance].disable;
}

// Read the sensor
void AG_Baro_SITL::update(void)
{
    if (!_has_sample) {
        return;
    }
 ...
 */
float AG_Baro_SITL::wind_pressure_correction(void)
{
