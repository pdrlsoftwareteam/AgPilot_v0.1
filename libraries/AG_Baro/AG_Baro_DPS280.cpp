...
AG_Baro_DPS280::AG_Baro_DPS280(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> _dev)
    : AG_Baro_Backend(baro)
    , dev(std::move(_dev))
{
}

AG_Baro_Backend *AG_Baro_DPS280::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> _dev, bool _is_dps310)
{
    if (!_dev) {
        return nullptr;
    }

    AG_Baro_DPS280 *sensor = new AG_Baro_DPS280(baro, std::move(_dev));
    if (sensor) {
...
AG_Baro_Backend *AG_Baro_DPS310::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> _dev)
{
    // same as DPS280 but with is_dps310 set for temperature fix
    return AG_Baro_DPS280::probe(baro, std::move(_dev), true);
}

/*
  handle bit width for 16 bit config registers
 */
void AG_Baro_DPS280::fix_config_bits16(int16_t &v, uint8_t bits) const
{
 ...
 */
void AG_Baro_DPS280::fix_config_bits32(int32_t &v, uint8_t bits) const
{
 ...
 */
bool AG_Baro_DPS280::read_calibration(void)
{
    uint8_t buf[18];
...
void AG_Baro_DPS280::set_config_registers(void)
{
...
bool AG_Baro_DPS280::init()
{
    ...
    if (dev->bus_type() == AG_HAL::Device::BUS_TYPE_SPI) {
    ...
    dev->register_periodic_callback((1000 / 64) * AP_USEC_PER_MSEC, FUNCTOR_BIND_MEMBER(&AG_Baro_DPS280::timer, void));
 ...
 */
void AG_Baro_DPS280::calculate_PT(int32_t UT, int32_t UP, float &pressure, float &temperature)
{
 ...
 */
void AG_Baro_DPS280::check_health(void)
{
...
void AG_Baro_DPS280::timer(void)
{
    uint8_t buf[6];
    uint8_t ready;
...
void AG_Baro_DPS280::update(void)
{
    if (count == 0) {
        return;
    }
