...
#include "AG_Baro_SITL.h"
#include "AG_Baro_BMP085.h"
#include "AG_Baro_BMP280.h"
#include "AG_Baro_SPL06.h"
#include "AG_Baro_KellerLD.h"
#include "AG_Baro_MS5611.h"
#include "AG_Baro_ICM20789.h"
#include "AG_Baro_LPS2XH.h"
#include "AG_Baro_FBM320.h"
#include "AG_Baro_DPS280.h"
#include "AG_Baro_BMP388.h"
#include "AG_Baro_Dummy.h"
#include "AG_Baro_UAVCAN.h"
#include "AG_Baro_MSP.h"
#include "AG_Baro_ExternalAHRS.h"
#include "AG_Baro_ICP101XX.h"
#include "AG_Baro_ICP201XX.h"
...
const AG_Param::GroupInfo AG_Baro::var_info[] = {
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("1_GND_PRESS", 2, AG_Baro, sensors[0].ground_pressure, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO("_GND_TEMP", 3, AG_Baro, _user_ground_temperature, 0),

    // index 4 reserved for old AP_Int8 version in legacy FRAM
    //AP_GROUPINFO("ALT_OFFSET", 4, AG_Baro, _alt_offset, 0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("_ALT_OFFSET", 5, AG_Baro, _alt_offset, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_PRIMARY", 6, AG_Baro, _primary_baro, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_EXT_BUS", 7, AG_Baro, _ext_bus, HAL_BARO_EXTERNAL_BUS_DEFAULT),
    ...
    AP_GROUPINFO_FRAME("_SPEC_GRAV", 8, AG_Baro, _specific_gravity, 1.0, AP_PARAM_FRAME_SUB),
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("2_GND_PRESS", 9, AG_Baro, sensors[1].ground_pressure, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("3_GND_PRESS", 11, AG_Baro, sensors[2].ground_pressure, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
    ...
    // @Range: 0 100
    // @Increment: 1
    AP_GROUPINFO("_FLTR_RNG", 13, AG_Baro, _filter_range, HAL_BARO_FILTER_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("_PROBE_EXT", 14, AG_Baro, _baro_probe_ext, HAL_BARO_PROBE_EXT_DEFAULT),
#endif
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("1_DEVID", 15, AG_Baro, sensors[0].bus_id, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("2_DEVID", 16, AG_Baro, sensors[1].bus_id, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
#endif
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("3_DEVID", 17, AG_Baro, sensors[2].bus_id, 0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
#endif

#if HAL_BARO_WIND_COMP_ENABLED
    // @Group: 1_WCF_
    // @Path: AG_Baro_Wind.cpp
    AP_SUBGROUPINFO(sensors[0].wind_coeff, "1_WCF_", 18, AG_Baro, WindCoeff),

#if BARO_MAX_INSTANCES > 1
    // @Group: 2_WCF_
    // @Path: AG_Baro_Wind.cpp
    AP_SUBGROUPINFO(sensors[1].wind_coeff, "2_WCF_", 19, AG_Baro, WindCoeff),
#endif

#if BARO_MAX_INSTANCES > 2
    // @Group: 3_WCF_
    // @Path: AG_Baro_Wind.cpp
    AP_SUBGROUPINFO(sensors[2].wind_coeff, "3_WCF_", 20, AG_Baro, WindCoeff),
#endif
#ifndef HAL_BUILD_AP_PERIPH
    // @Param: _FIELD_ELV
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO("_FIELD_ELV", 22, AG_Baro, _field_elevation, 0),
#endif
#endif
    ...
    // @Range: 0 5000
    // @User: Advanced
    AP_GROUPINFO("_ALTERR_MAX", 23, AG_Baro, _alt_error_max, 2000),
    ...
    // @User: Advanced
    AP_GROUPINFO("_OPTIONS", 24, AG_Baro, _options, 0),
#endif
...
void AG_Baro::calibrate(bool save)
{
    ...
    for (uint8_t i = 0; i < 10; i++) {
        uint32_t tstart = AG_HAL::millis();
        do {
            update();
            if (AG_HAL::millis() - tstart > 500) {
                AG_BoardConfig::config_error("Baro: unable to calibrate");
            }
    ...
    AG_BoardConfig::config_error("Baro: all sensors uncalibrated");
}
...
*/
void AG_Baro::update_calibration()
{
...
float AG_Baro::get_altitude_difference(float base_pressure, float pressure) const
{
    float ret;
...
float AG_Baro::get_sealevel_pressure(float pressure) const
{
    float temp    = C_TO_KELVIN(get_ground_temperature());
    float p0_sealevel;
    // This is an exact calculation that is within +-2.5m of the standard
...
float AG_Baro::get_EAS2TAS(void)
{
...
float AG_Baro::get_air_density_ratio(void)
{
...
float AG_Baro::get_climb_rate(void)
{
...
float AG_Baro::get_ground_temperature(void) const
{
 ...
 */
void AG_Baro::set_external_temperature(float temperature)
{
    _external_temperature = temperature;
    _last_external_temperature_ms = AG_HAL::millis();
}

/*
  get the temperature in degrees C to be used for calibration purposes
 */
float AG_Baro::get_external_temperature(const uint8_t instance) const
{
    // if we have a recent external temperature then use it
    if (_last_external_temperature_ms != 0 && AG_HAL::millis() - _last_external_temperature_ms < 10000) {
...
bool AG_Baro::_add_backend(AG_Baro_Backend *backend)
{
    if (!backend) {
        return false;
    }
    if (_num_drivers >= BARO_MAX_DRIVERS) {
        AG_HAL::panic("Too many barometer drivers");
    }
 ...
 */
bool AG_Baro::_have_i2c_driver(uint8_t bus, uint8_t address) const
{
    for (int i=0; i<_num_drivers; ++i) {
        if (AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_I2C, bus, address, 0) ==
            AG_HAL::Device::change_bus_id(uint32_t(sensors[i].bus_id.get()), 0)) {
...
void AG_Baro::init(void)
{
    ...
    if (sitl == nullptr) {
        AG_HAL::panic("No SITL pointer");
    }
    for(uint8_t i = 0; i < sitl->baro_count; i++) {
        ADD_BACKEND(new AG_Baro_SITL(*this));
    }
#endif
    ...
    for (uint8_t i = 0; i < BARO_MAX_DRIVERS; i++) {
        ADD_BACKEND(AG_Baro_UAVCAN::probe(*this));
    }
#endif

#if AP_BARO_EXTERNALAHRS_ENABLED
    const int8_t serial_port = AP::externalAHRS().get_port(AG_ExternalAHRS::AvailableSensor::BARO);
    if (serial_port >= 0) {
        ADD_BACKEND(new AG_Baro_ExternalAHRS(*this, serial_port));
    }
#endif
...
#define GET_I2C_DEVICE(bus, address) _have_i2c_driver(bus, address)?nullptr:hal.i2c_mgr->get_device(bus, address)
    ...
    ADD_BACKEND(AG_Baro_MS56XX::probe(*this,
    ...
    case AG_BoardConfig::PX4_BOARD_PCNC1:
        ...
        ADD_BACKEND(AG_Baro_ICM20789::probe(*this,
	...
	ADD_BACKEND(AG_Baro_LPS2XH::probe_InvensenseIMU(*this,
...
#if AP_BARO_MS56XX_ENABLED
        ADD_BACKEND(AG_Baro_MS56XX::probe(*this,
                                          std::move(GET_I2C_DEVICE(_ext_bus, HAL_BARO_MS5837_I2C_ADDR)), AG_Baro_MS56XX::BARO_MS5837));
#endif
#if AP_BARO_KELLERLD_ENABLED
        ADD_BACKEND(AG_Baro_KellerLD::probe(*this,
    ...
    for (uint8_t i=0; i<8; i++) {
        if (msp_instance_mask & (1U<<i)) {
            ADD_BACKEND(new AG_Baro_MSP(*this, i));
        }
    }
#endif
...
#endif
    if (_num_drivers == 0 || _num_sensors == 0 || drivers[0] == nullptr) {
        AG_BoardConfig::config_error("Baro: unable to initialise driver");
    }
#endif
#ifdef HAL_BUILD_AP_PERIPH
 ...
 */
void AG_Baro::_probe_i2c_barometers(void)
{
    ...
    if (AG_BoardConfig::get_board_type() == AG_BoardConfig::PX4_BOARD_PIXHAWK2) {
    ...
    static const struct BaroProbeSpec {
        uint32_t bit;
        AG_Baro_Backend* (*probefn)(AG_Baro&, AG_HAL::OwnPtr<AG_HAL::Device>);
        uint8_t addr;
    } baroprobespec[] {
#if AP_BARO_BMP085_ENABLED
        { PROBE_BMP085, AG_Baro_BMP085::probe, HAL_BARO_BMP085_I2C_ADDR },
#endif
#if AP_BARO_BMP280_ENABLED
        { PROBE_BMP280, AG_Baro_BMP280::probe, HAL_BARO_BMP280_I2C_ADDR },
        { PROBE_BMP280, AG_Baro_BMP280::probe, HAL_BARO_BMP280_I2C_ADDR2 },
#endif
#if AP_BARO_SPL06_ENABLED
        { PROBE_SPL06, AG_Baro_SPL06::probe, HAL_BARO_SPL06_I2C_ADDR },
        { PROBE_SPL06, AG_Baro_SPL06::probe, HAL_BARO_SPL06_I2C_ADDR2 },
#endif
#if AP_BARO_BMP388_ENABLED
        { PROBE_BMP388, AG_Baro_BMP388::probe, HAL_BARO_BMP388_I2C_ADDR },
        { PROBE_BMP388, AG_Baro_BMP388::probe, HAL_BARO_BMP388_I2C_ADDR2 },
#endif
#if AP_BARO_MS56XX_ENABLED
        { PROBE_MS5611, AG_Baro_MS56XX::probe_5611, HAL_BARO_MS5611_I2C_ADDR },
        { PROBE_MS5611, AG_Baro_MS56XX::probe_5611, HAL_BARO_MS5611_I2C_ADDR2 },
        { PROBE_MS5607, AG_Baro_MS56XX::probe_5607, HAL_BARO_MS5607_I2C_ADDR },
        { PROBE_MS5637, AG_Baro_MS56XX::probe_5637, HAL_BARO_MS5637_I2C_ADDR },
#endif
#if AP_BARO_FBM320_ENABLED
        { PROBE_FBM320, AG_Baro_FBM320::probe, HAL_BARO_FBM320_I2C_ADDR },
        { PROBE_FBM320, AG_Baro_FBM320::probe, HAL_BARO_FBM320_I2C_ADDR2 },
#endif
#if AP_BARO_DPS280_ENABLED
        { PROBE_DPS280, AG_Baro_DPS280::probe_280, HAL_BARO_DPS280_I2C_ADDR },
        { PROBE_DPS280, AG_Baro_DPS280::probe_280, HAL_BARO_DPS280_I2C_ADDR2 },
#endif
#if AP_BARO_LPS2XH_ENABLED
        { PROBE_LPS25H, AG_Baro_LPS2XH::probe, HAL_BARO_LPS25H_I2C_ADDR },
#endif

#if APM_BUILD_TYPE(APM_BUILD_ArduSub)
#if AP_BARO_KELLERLD_ENABLED
        { PROBE_KELLER, AG_Baro_KellerLD::probe, HAL_BARO_KELLERLD_I2C_ADDR },
#endif
#if AP_BARO_MS56XX_ENABLED
        { PROBE_MS5837, AG_Baro_MS56XX::probe_5837, HAL_BARO_MS5837_I2C_ADDR },
#endif
 ...
 */
void AG_Baro::update_field_elevation(void)
{
...
/* register a new sensor, claiming a sensor slot. If we are out of
   slots it will panic
*/
uint8_t AG_Baro::register_sensor(void)
{
    if (_num_sensors >= BARO_MAX_INSTANCES) {
        AG_HAL::panic("Too many barometers");
    }
    return _num_sensors++;
}
 ...
 */
bool AG_Baro::all_healthy(void) const
{
     for (uint8_t i=0; i<_num_sensors; i++) {
         if (!healthy(i)) {
             return false;
...
// set a pressure correction from AG_TempCalibration
void AG_Baro::set_pressure_correction(uint8_t instance, float p_correction)
{
 ...
 */
void AG_Baro::handle_msp(const MSP::msp_baro_data_message_t &pkt)
{
    if (pkt.instance > 7) {
        return;
    }
 ...
 */
void AG_Baro::handle_external(const AG_ExternalAHRS::baro_data_message_t &pkt)
{
    for (uint8_t i=0; i<_num_drivers; i++) {
...
bool AG_Baro::arming_checks(size_t buflen, char *buffer) const
{
    if (!all_healthy()) {
    ...
    if (_alt_error_max > 0 && gps.status() >= AG_GPS::GPS_Status::GPS_OK_FIX_3D) {
