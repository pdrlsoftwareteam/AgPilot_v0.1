...
/*
  constructor
 */
AG_Baro_ICP101XX::AG_Baro_ICP101XX(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::I2CDevice> _dev)
    : AG_Baro_Backend(baro)
    , dev(std::move(_dev))
{
}

AG_Baro_Backend *AG_Baro_ICP101XX::probe(AG_Baro &baro,
                                         AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev)
{
    if (!dev) {
        return nullptr;
    }
    AG_Baro_ICP101XX *sensor = new AG_Baro_ICP101XX(baro, std::move(dev));
...
bool AG_Baro_ICP101XX::init()
{
    ...
    dev->register_periodic_callback(measure_interval/2, FUNCTOR_BIND_MEMBER(&AG_Baro_ICP101XX::timer, void));
...
bool AG_Baro_ICP101XX::read_measure_results(uint8_t *buf, uint8_t len)
{
	return dev->transfer(nullptr, 0, buf, len);
}

bool AG_Baro_ICP101XX::read_response(uint16_t cmd, uint8_t *buf, uint8_t len)
{
	uint8_t buff[2];
	buff[0] = (cmd >> 8) & 0xff;
	buff[1] = cmd & 0xff;
...
bool AG_Baro_ICP101XX::send_command(uint16_t cmd)
{
	uint8_t buf[2];
	buf[0] = (cmd >> 8) & 0xff;
	buf[1] = cmd & 0xff;
...
bool AG_Baro_ICP101XX::send_command(uint16_t cmd, uint8_t *data, uint8_t len)
{
	uint8_t buf[5];
	buf[0] = (cmd >> 8) & 0xff;
	buf[1] = cmd & 0xff;
...
int8_t AG_Baro_ICP101XX::cal_crc(uint8_t seed, uint8_t data)
{
	int8_t poly = 0x31;
	int8_t var2;
	uint8_t i;
...
bool AG_Baro_ICP101XX::read_calibration_data(void)
{
    // setup for OTP read
...
bool AG_Baro_ICP101XX::start_measure(uint16_t mode)
{
	/*
...
void AG_Baro_ICP101XX::calculate_conversion_constants(const float p_Pa[3], const float p_LUT[3],
...
*/
float AG_Baro_ICP101XX::get_pressure(uint32_t p_LSB, uint32_t T_LSB)
{
...
void AG_Baro_ICP101XX::convert_data(uint32_t Praw, uint32_t Traw)
{
    // temperature is easy
...
void AG_Baro_ICP101XX::timer(void)
{
    uint8_t d[9] {};
    if (read_measure_results(d, 9)) {
        ...
        if (AG_HAL::micros() - last_measure_us > measure_interval*3) {
            // lost a sample
            start_measure(CMD_MEAS_ULN);
            last_measure_us = AG_HAL::micros();
        }
