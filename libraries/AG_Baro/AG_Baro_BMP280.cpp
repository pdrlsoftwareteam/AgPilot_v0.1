...
AG_Baro_BMP280::AG_Baro_BMP280(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : AG_Baro_Backend(baro)
    , _dev(std::move(dev))
{
}

AG_Baro_Backend *AG_Baro_BMP280::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    ...
    AG_Baro_BMP280 *sensor = new AG_Baro_BMP280(baro, std::move(dev));
...
bool AG_Baro_BMP280::_init()
{
    ...
    // request 50Hz update
    _dev->register_periodic_callback(20 * AP_USEC_PER_MSEC, FUNCTOR_BIND_MEMBER(&AG_Baro_BMP280::_timer, void));
...
void AG_Baro_BMP280::_timer(void)
{
    uint8_t buf[6];
...
void AG_Baro_BMP280::update(void)
{
    WITH_SEMAPHORE(_sem);
...
void AG_Baro_BMP280::_update_temperature(int32_t temp_raw)
{
    int32_t var1, var2, t;
...
// calculate pressure
void AG_Baro_BMP280::_update_pressure(int32_t press_raw)
{
    int64_t var1, var2, p;
