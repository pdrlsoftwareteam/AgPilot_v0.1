...
AG_Baro_FBM320::AG_Baro_FBM320(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> _dev)
    : AG_Baro_Backend(baro)
    , dev(std::move(_dev))
{
}

AG_Baro_Backend *AG_Baro_FBM320::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> _dev)
{
    ...
    AG_Baro_FBM320 *sensor = new AG_Baro_FBM320(baro, std::move(_dev));
 ...
 */
bool AG_Baro_FBM320::read_calibration(void)
{
    uint8_t tmp[2];
    uint16_t R[10];
...
bool AG_Baro_FBM320::init()
{
    ...
    // request 50Hz update
    dev->register_periodic_callback(20 * AP_USEC_PER_MSEC, FUNCTOR_BIND_MEMBER(&AG_Baro_FBM320::timer, void));
 ...
 */
void AG_Baro_FBM320::calculate_PT(int32_t UT, int32_t UP, int32_t &pressure, int32_t &temperature)
{
...
void AG_Baro_FBM320::timer(void)
{
    uint8_t buf[3];
...
void AG_Baro_FBM320::update(void)
{
    if (count == 0) {
        return;
    }
