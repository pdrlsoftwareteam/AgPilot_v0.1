...
AG_Baro_LPS2XH::AG_Baro_LPS2XH(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : AG_Baro_Backend(baro)
    , _dev(std::move(dev))
{
}

AG_Baro_Backend *AG_Baro_LPS2XH::probe(AG_Baro &baro,
                                       AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    ...
    AG_Baro_LPS2XH *sensor = new AG_Baro_LPS2XH(baro, std::move(dev));
...
AG_Baro_Backend *AG_Baro_LPS2XH::probe_InvensenseIMU(AG_Baro &baro,
                                                     AG_HAL::OwnPtr<AG_HAL::Device> dev,
                                                     uint8_t imu_address)
{
    if (!dev) {
...
/*
  ...
  on the same i2c bus
*/
bool AG_Baro_LPS2XH::_imu_i2c_init(uint8_t imu_address)
{
...
bool AG_Baro_LPS2XH::_init()
{
    ...
    _dev->register_periodic_callback(CallTime, FUNCTOR_BIND_MEMBER(&AG_Baro_LPS2XH::_timer, void));

    return true;
}

//check ID
bool AG_Baro_LPS2XH::_check_whoami(void)
{
    uint8_t whoami;
...
void AG_Baro_LPS2XH::_timer(void)
{
    uint8_t status;
...
void AG_Baro_LPS2XH::update(void)
{
...
void AG_Baro_LPS2XH::_update_temperature(void)
{
    uint8_t pu8[2];
...
// calculate pressure
void AG_Baro_LPS2XH::_update_pressure(void)
{
    uint8_t pressure[3];
