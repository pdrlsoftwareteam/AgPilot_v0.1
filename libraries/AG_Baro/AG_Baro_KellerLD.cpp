...
AG_Baro_KellerLD::AG_Baro_KellerLD(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
...
AG_Baro_Backend *AG_Baro_KellerLD::probe(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    if (!dev) {
        return nullptr;
    }
    AG_Baro_KellerLD *sensor = new AG_Baro_KellerLD(baro, std::move(dev));
...
bool AG_Baro_KellerLD::transfer_with_delays(uint8_t *send, uint8_t sendlen, uint8_t *recv, uint8_t recvlen)
{
...
// AG_HAL::I2CDevice::set_split_transfers will not work with these
// sensors)
bool AG_Baro_KellerLD::read_measurement_limit(float *limit, uint8_t msb_addr, uint8_t lsb_addr)
{
    uint8_t data[3];
...
bool AG_Baro_KellerLD::read_cal()
{
...
bool AG_Baro_KellerLD::read_mode_type()
{
...
bool AG_Baro_KellerLD::_init()
{
    ...
    _dev->register_periodic_callback(20 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AG_Baro_KellerLD::_timer, void));
    return true;
}

// Read out most recent measurement from sensor hw
bool AG_Baro_KellerLD::_read()
{
    uint8_t data[5];
...
void AG_Baro_KellerLD::_timer(void)
{
    _read();
    _dev->transfer(&CMD_REQUEST_MEASUREMENT, 1, nullptr, 0);
}
...
void AG_Baro_KellerLD::_update_and_wrap_accumulator(uint16_t pressure, uint16_t temperature, uint8_t max_count)
{
