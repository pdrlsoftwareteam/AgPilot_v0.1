...
/*
  constructor
 */
AG_Baro_MS56XX::AG_Baro_MS56XX(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev, enum MS56XX_TYPE ms56xx_type)
...
AG_Baro_Backend *AG_Baro_MS56XX::probe(AG_Baro &baro,
                                       ...
                                       enum MS56XX_TYPE ms56xx_type)
{
    ...
    AG_Baro_MS56XX *sensor = new AG_Baro_MS56XX(baro, std::move(dev), ms56xx_type);
...
bool AG_Baro_MS56XX::_init()
{
     ...
     */
    if (_ms56xx_type == BARO_MS5611 && _frontend.option_enabled(AG_Baro::Options::TreatMS5611AsMS5607)) {
    ...
    if (_ms56xx_type == BARO_MS5837) {
        _frontend.set_type(_instance, AG_Baro::BARO_TYPE_WATER);
    }
    ...
    _dev->register_periodic_callback(10 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AG_Baro_MS56XX::_timer, void));
    return true;
}

uint16_t AG_Baro_MS56XX::_read_prom_word(uint8_t word)
{
...
uint32_t AG_Baro_MS56XX::_read_adc()
{
    uint8_t val[3];
...
bool AG_Baro_MS56XX::_read_prom_5611(uint16_t prom[8])
{
    /*
...
bool AG_Baro_MS56XX::_read_prom_5637(uint16_t prom[8])
{
    /*
 ...
 * pressure.
*/
void AG_Baro_MS56XX::_timer(void)
{
    uint8_t next_cmd;
    uint8_t next_state;
...
void AG_Baro_MS56XX::_update_and_wrap_accumulator(uint32_t *accum, uint32_t val,
...
void AG_Baro_MS56XX::_calculate_5611()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AG_Baro_MS56XX::_calculate_5607()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AG_Baro_MS56XX::_calculate_5637()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AG_Baro_MS56XX::_calculate_5837()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
    int32_t raw_pressure = _D1;
