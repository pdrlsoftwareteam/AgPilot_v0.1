...
AG_Baro_BMP085::AG_Baro_BMP085(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : AG_Baro_Backend(baro)
    , _dev(std::move(dev))
{ }

AG_Baro_Backend * AG_Baro_BMP085::probe(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    ...
    AG_Baro_BMP085 *sensor = new AG_Baro_BMP085(baro, std::move(dev));
...
bool AG_Baro_BMP085::_init()
{
    ...
    AG_HAL::Semaphore *sem = _dev->get_semaphore();
    ...
    _dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&AG_Baro_BMP085::_timer, void));
    return true;
}

uint16_t AG_Baro_BMP085::_read_prom_word(uint8_t word)
{
...
bool AG_Baro_BMP085::_read_prom(uint16_t *prom)
{
    bool all_zero = true;
 ...
 */
void AG_Baro_BMP085::_timer(void)
{
    if (!_data_ready()) {
        return;
    }
 ...
 */
void AG_Baro_BMP085::update(void)
{
    WITH_SEMAPHORE(_sem);
...
void AG_Baro_BMP085::_cmd_read_pressure()
{
    _dev->write_register(0xF4, 0x34 + (OVERSAMPLING << 6));
    _last_press_read_command_time = AG_HAL::millis();
}

// Read raw pressure values
bool AG_Baro_BMP085::_read_pressure()
{
    uint8_t buf[3];
    ...
    _last_press_read_command_time = AG_HAL::millis() + 1000;
    _dev->set_speed(AG_HAL::Device::SPEED_LOW);
    return false;
}

// Send Command to Read Temperature
void AG_Baro_BMP085::_cmd_read_temp()
{
    _dev->write_register(0xF4, 0x2E);
    _last_temp_read_command_time = AG_HAL::millis();
}

// Read raw temperature values
void AG_Baro_BMP085::_read_temp()
{
    uint8_t buf[2];
...
void AG_Baro_BMP085::_calculate()
{
...
bool AG_Baro_BMP085::_data_ready()
{
    if (BMP085_EOC >= 0) {
        return _eoc->read();
    }
    ...
    if (_state == 0) {
        return AG_HAL::millis() - _last_temp_read_command_time > 5u;
    }
    ...
    return AG_HAL::millis() - _last_press_read_command_time > conversion_time_msec;
}
