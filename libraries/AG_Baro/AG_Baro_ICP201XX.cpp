...
/*
  constructor
 */
AG_Baro_ICP201XX::AG_Baro_ICP201XX(AG_Baro &baro, AG_HAL::OwnPtr<AG_HAL::I2CDevice> _dev)
    : AG_Baro_Backend(baro)
    , dev(std::move(_dev))
{
}

AG_Baro_Backend *AG_Baro_ICP201XX::probe(AG_Baro &baro,
                                         AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev)
{
    if (!dev) {
        return nullptr;
    }
    AG_Baro_ICP201XX *sensor = new AG_Baro_ICP201XX(baro, std::move(dev));
...
bool AG_Baro_ICP201XX::init()
{
    ...
    dev->register_periodic_callback(CONVERSION_INTERVAL/2, FUNCTOR_BIND_MEMBER(&AG_Baro_ICP201XX::timer, void));
    return true;
...
void AG_Baro_ICP201XX::dummy_reg()
{
    do {
...
bool AG_Baro_ICP201XX::read_reg(uint8_t reg, uint8_t *buf, uint8_t len)
{
    bool ret;
...
bool AG_Baro_ICP201XX::read_reg(uint8_t reg, uint8_t *val)
{
    return read_reg(reg, val, 1);
}

bool AG_Baro_ICP201XX::write_reg(uint8_t reg, uint8_t val)
{
    bool ret;
...
void AG_Baro_ICP201XX::soft_reset()
{
...
bool AG_Baro_ICP201XX::mode_select(uint8_t mode)
{
...
bool AG_Baro_ICP201XX::read_otp_data(uint8_t addr, uint8_t cmd, uint8_t *val)
{
...
bool AG_Baro_ICP201XX::get_sensor_data(float *pressure, float *temperature)
{
...
bool AG_Baro_ICP201XX::boot_sequence()
{
    uint8_t reg_value = 0;
...
bool AG_Baro_ICP201XX::configure()
{
    uint8_t reg_value = 0;
...
void AG_Baro_ICP201XX::wait_read()
{
    /*
...
bool AG_Baro_ICP201XX::flush_fifo()
{
    uint8_t reg_value;
