...
AG_KEYSTORE* AG_KEYSTORE::m_pInstance = 0;
...
//const AG_Param::GroupInfo AG_KEYSTORE::var_info[] = {
//		// @Param: TYPE
...
//		// @RebootRequired: True
//		// @User: Advanced
//		AP_GROUPINFO("KEY_LEN",    0, AG_KEYSTORE, testParamKeystore, 16),
//
//		AP_GROUPEND
//};

AG_KEYSTORE* AG_KEYSTORE::getInstance()
{
	if (!m_pInstance)   // Only allow one instance of class to be generated.
		m_pInstance = new AG_KEYSTORE;
	return m_pInstance;
}

AG_KEYSTORE::AG_KEYSTORE()
{
...
uint16_t AG_KEYSTORE::getKeyLen(KEY_TYPE keytype)
{
	uint16_t keylen = 0;
	switch(keytype)
	{
...
void AG_KEYSTORE::transferKey(int index,unsigned char *keyptr,uint16_t keyLen,uint16_t validDataLen)
{
...
void AG_KEYSTORE::receivKey(unsigned char *keyptr,uint16_t validDataLen,uint8_t bufferIndex)
{
...
bool AG_KEYSTORE::verifyFirmwareSignature(uint8_t* firmwareSignature, uint8_t* firmwareHash)
{
	int err = 0;
...
AG_KEYSTORE &key_store()
{
	return *AG_KEYSTORE::getInstance();
}
