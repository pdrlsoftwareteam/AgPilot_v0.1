...
uint8_t AG_AccelCal::_num_clients = 0;
AG_AccelCal_Client* AG_AccelCal::_clients[AP_ACCELCAL_MAX_NUM_CLIENTS] {};

void AG_AccelCal::update()
{
    if (!get_calibrator(0)) {
        // no calibrators
        return;
...
void AG_AccelCal::start(GCS_MAVLINK *gcs)
{
...
void AG_AccelCal::collect_sample()
{
...
void AG_AccelCal::register_client(AG_AccelCal_Client* client) {
...
AccelCalibrator* AG_AccelCal::get_calibrator(uint8_t index) {
    AccelCalibrator* ret;
    for(uint8_t i=0; i<_num_clients; i++) {
...
void AG_AccelCal::update_status() {
    AccelCalibrator *cal;
...
bool AG_AccelCal::client_active(uint8_t client_num)
{
    return (bool)_clients[client_num]->_acal_get_calibrator(0);
}

#if HAL_GCS_ENABLED
void AG_AccelCal::handle_command_ack(const mavlink_command_ack_t &packet)
{
...
bool AG_AccelCal::gcs_vehicle_position(float position)
{
...
bool AG_AccelCal::running(void) const
{
