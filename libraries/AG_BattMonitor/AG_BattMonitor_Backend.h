...
public:
    ...
    AG_BattMonitor_Backend(AG_BattMonitor &mon, AG_BattMonitor::BattMonitor_State &mon_state, AG_BattMonitor_Params &params);

    // we declare a virtual destructor so that BattMonitor driver can
    // override with a custom destructor if need be
    virtual ~AG_BattMonitor_Backend(void) {}
    ...
    virtual AG_BattMonitor::Failsafe update_failsafes(void);
    ...
    bool option_is_set(const AG_BattMonitor_Params::Options option) const {
        return (uint16_t(_params._options.get()) & uint16_t(option)) != 0;
    }

protected:
    AG_BattMonitor                      &_mon;      // reference to front-end
    AG_BattMonitor::BattMonitor_State   &_state;    // reference to this instances state (held in the front-end)
    AG_BattMonitor_Params               &_params;   // reference to this instances parameters (held in the front-end)
    ...
    void update_consumed(AG_BattMonitor::BattMonitor_State &state, uint32_t dt_us);
