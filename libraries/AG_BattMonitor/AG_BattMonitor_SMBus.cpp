...
const AG_Param::GroupInfo AG_BattMonitor_SMBus::var_info[] = {
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("I2C_BUS", 10, AG_BattMonitor_SMBus, _bus, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("I2C_ADDR", 11, AG_BattMonitor_SMBus, _address, AP_BATTMONITOR_SMBUS_I2C_ADDR),
...
AG_BattMonitor_SMBus::AG_BattMonitor_SMBus(AG_BattMonitor &mon,
...
void AG_BattMonitor_SMBus::init(void)
{
    _dev = hal.i2c_mgr->get_device(_bus, _address, 100000, true, 20);
    
    if (_dev) {
        timer_handle = _dev->register_periodic_callback(100000, FUNCTOR_BIND_MEMBER(&AG_BattMonitor_SMBus::timer, void));
    }
}

// return true if cycle count can be provided and fills in cycles argument
bool AG_BattMonitor_SMBus::get_cycle_count(uint16_t &cycles) const
{
    if (!_has_cycle_count) {
        return false;
    }
...
void AG_BattMonitor_SMBus::read(void)
{
...
void AG_BattMonitor_SMBus::read_full_charge_capacity(void)
{
...
void AG_BattMonitor_SMBus::read_remaining_capacity(void)
{
...
void AG_BattMonitor_SMBus::read_temp(void)
{
    uint16_t data;
    ...
    _state.temperature_time = AG_HAL::millis();
...
void AG_BattMonitor_SMBus::read_serial_number(void)
{
...
void AG_BattMonitor_SMBus::read_cycle_count()
{
...
bool AG_BattMonitor_SMBus::read_word(uint8_t reg, uint16_t& data) const
{
...
uint8_t AG_BattMonitor_SMBus::read_block(uint8_t reg, uint8_t* data, uint8_t len) const
{
    // get length
    uint8_t bufflen;
...
uint8_t AG_BattMonitor_SMBus::get_PEC(const uint8_t i2c_addr, uint8_t cmd, bool reading, const uint8_t buff[], uint8_t len) const
{
