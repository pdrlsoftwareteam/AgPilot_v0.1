...
*/
AG_BattMonitor_Backend::AG_BattMonitor_Backend(AG_BattMonitor &mon, AG_BattMonitor::BattMonitor_State &mon_state,
...
bool AG_BattMonitor_Backend::capacity_remaining_pct(uint8_t &percentage) const
{
...
void AG_BattMonitor_Backend::update_resistance_estimate()
{
...
float AG_BattMonitor_Backend::voltage_resting_estimate() const
{
...
AG_BattMonitor::Failsafe AG_BattMonitor_Backend::update_failsafes(void)
{
    ...
    if (critical_voltage) {
        ...
        if (_state.critical_voltage_start_ms == 0) {
            ...
            return AG_BattMonitor::Failsafe::Critical;
        }
    } else {
    ...
    if (low_voltage) {
        ...
        if (_state.low_voltage_start_ms == 0) {
            ...
            return AG_BattMonitor::Failsafe::Low;
        }
    } else {
        // acceptable voltage so reset timer
...
bool AG_BattMonitor_Backend::arming_checks(char * buffer, size_t buflen) const
{
    bool low_voltage, low_capacity, critical_voltage, critical_capacity;
    check_failsafe_types(low_voltage, low_capacity, critical_voltage, critical_capacity);
...
void AG_BattMonitor_Backend::check_failsafe_types(bool &low_voltage, bool &low_capacity, bool &critical_voltage, bool &critical_capacity) const
{
    // use voltage or sag compensated voltage
    float voltage_used;
    switch (_params.failsafe_voltage_source()) {
        case AG_BattMonitor_Params::BattMonitor_LowVoltageSource_Raw:
        default:
            voltage_used = _state.voltage;
            break;
        case AG_BattMonitor_Params::BattMonitor_LowVoltageSource_SagCompensated:
...
/*
  ...
  for a full battery
*/
bool AG_BattMonitor_Backend::reset_remaining(float percentage)
{
 ...
 */
void AG_BattMonitor_Backend::update_consumed(AG_BattMonitor::BattMonitor_State &state, uint32_t dt_us)
{
