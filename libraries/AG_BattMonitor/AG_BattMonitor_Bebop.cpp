...
#include <AG_HAL_Linux/RCOutput_Bebop.h>
#include <AG_HAL_Linux/RCOutput_Disco.h>
...
void AG_BattMonitor_Bebop::init(void)
{
...
float AG_BattMonitor_Bebop::_filter_voltage(float vbat_raw)
{
...
float AG_BattMonitor_Bebop::_compute_compensation(const uint16_t *rpm,
                                                  float vbat_raw)
{
    float vbat, res;
    size_t i, j;
...
float AG_BattMonitor_Bebop::_compute_battery_percentage(float vbat)
{
    float percent = 0.0f;
    int i;
...
void AG_BattMonitor_Bebop::read(void)
{
    int ret;
    uint32_t tnow;
