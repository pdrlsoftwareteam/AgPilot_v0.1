...
#include "AG_BattMonitor_Params.h"
...
class AG_BattMonitor_Backend;
class AG_BattMonitor_Analog;
class AG_BattMonitor_SMBus;
class AG_BattMonitor_SMBus_Solo;
class AG_BattMonitor_SMBus_Generic;
class AG_BattMonitor_SMBus_Maxell;
class AG_BattMonitor_SMBus_Rotoye;
class AG_BattMonitor_UAVCAN;
class AG_BattMonitor_Generator;
class AG_BattMonitor_INA2XX;
class AG_BattMonitor_INA239;
class AG_BattMonitor_LTC2946;
class AG_BattMonitor_Torqeedo;
class AG_BattMonitor_FuelLevel_Analog;
class AG_BattMonitor_EFI;
class AG_BattMonitor_XKC_Y25_NPN;

class AG_BattMonitor
{
    friend class AG_BattMonitor_Backend;
    friend class AG_BattMonitor_Analog;
    friend class AG_BattMonitor_SMBus;
    friend class AG_BattMonitor_SMBus_Solo;
    friend class AG_BattMonitor_SMBus_Generic;
    friend class AG_BattMonitor_SMBus_Maxell;
    friend class AG_BattMonitor_SMBus_Rotoye;
    friend class AG_BattMonitor_UAVCAN;
    friend class AG_BattMonitor_Sum;
    friend class AG_BattMonitor_FuelFlow;
    friend class AG_BattMonitor_FuelLevel_PWM;
    friend class AG_BattMonitor_Generator;
    friend class AG_BattMonitor_EFI;
    friend class AG_BattMonitor_INA2XX;
    friend class AG_BattMonitor_INA239;
    friend class AG_BattMonitor_LTC2946;

    friend class AG_BattMonitor_Torqeedo;
    friend class AG_BattMonitor_FuelLevel_Analog;
    friend class AG_BattMonitor_Synthetic_Current;
    friend class AG_BattMonitor_XKC_Y25_NPN;
...
public:
    ...
    AG_BattMonitor(uint32_t log_battery_bit, battery_failsafe_handler_fn_t battery_failsafe_handler_fn, const int8_t *failsafe_priorities);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_BattMonitor);

    static AG_BattMonitor *get_singleton() {
        return _singleton;
    }
    ...
    struct BattMonitor_State {
        ...
        const struct AG_Param::GroupInfo *var_info;
    };

    static const struct AG_Param::GroupInfo *backend_var_info[AP_BATT_MONITOR_MAX_INSTANCES];
...
protected:
    ...
    /// parameters
    AG_BattMonitor_Params _params[AP_BATT_MONITOR_MAX_INSTANCES];

private:
    static AG_BattMonitor *_singleton;

    BattMonitor_State state[AP_BATT_MONITOR_MAX_INSTANCES];
    AG_BattMonitor_Backend *drivers[AP_BATT_MONITOR_MAX_INSTANCES];
