...
const AG_Param::GroupInfo AG_BattMonitor_UAVCAN::var_info[] = {
    ...
    // @Range: .1 10
    // @User: Advanced
    AP_GROUPINFO("CURR_MULT", 30, AG_BattMonitor_UAVCAN, _curr_mult, 1.0),
...
/// Constructor
AG_BattMonitor_UAVCAN::AG_BattMonitor_UAVCAN(AG_BattMonitor &mon, AG_BattMonitor::BattMonitor_State &mon_state, BattMonitor_UAVCAN_Type type, AG_BattMonitor_Params &params) :
    AG_BattMonitor_Backend(mon, mon_state, params),
    _type(type)
{
...
void AG_BattMonitor_UAVCAN::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (battinfo_listener_res < 0) {
        AG_BoardConfig::allocation_error("UAVCAN BatteryInfo subscriber start problem\n\r");
        return;
    }
    ...
    if (battinfo_aux_listener_res < 0) {
        AG_BoardConfig::allocation_error("UAVCAN BatteryInfoAux subscriber start problem");
        return;
    }
    ...
    if (mppt_stream_listener_res < 0) {
        AG_BoardConfig::allocation_error("UAVCAN Mppt::Stream subscriber start problem");
        return;
    }
    ...
    if (outputEnable_client[driver_index] == nullptr || outputEnable_client[driver_index]->init() < 0) {
        AG_BoardConfig::allocation_error("UAVCAN Mppt::outputEnable client start problem");
        return;
    }
    outputEnable_client[driver_index]->setCallback(trampoline_handleOutputEnable);
}

AG_BattMonitor_UAVCAN* AG_BattMonitor_UAVCAN::get_uavcan_backend(AG_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t battery_id)
{
    if (ap_uavcan == nullptr) {
        return nullptr;
    }
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        if (AP::battery().drivers[i] == nullptr ||
            AP::battery().get_type(i) != AG_BattMonitor::Type::UAVCAN_BatteryInfo) {
            continue;
        }
        AG_BattMonitor_UAVCAN* driver = (AG_BattMonitor_UAVCAN*)AP::battery().drivers[i];
    ...
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        if (AP::battery().drivers[i] != nullptr &&
            ...
            AG_BattMonitor_UAVCAN* batmon = (AG_BattMonitor_UAVCAN*)AP::battery().drivers[i];
...
void AG_BattMonitor_UAVCAN::handle_battery_info(const BattInfoCb &cb)
{
...
void AG_BattMonitor_UAVCAN::update_interim_state(const float voltage, const float current, const float temperature_K, const uint8_t soc)
{
    WITH_SEMAPHORE(_sem_battmon);
    ...
    if (!isnan(temperature_K) && temperature_K > 0) {
        ...
        _interim_state.temperature_time = AG_HAL::millis();
    }
    ...
    if (!_has_battery_info_aux ||
        _mppt.is_detected ||
        option_is_set(AG_BattMonitor_Params::Options::Ignore_UAVCAN_SoC)) {
...
void AG_BattMonitor_UAVCAN::handle_battery_info_aux(const BattInfoAuxCb &cb)
{
    WITH_SEMAPHORE(_sem_battmon);
...
void AG_BattMonitor_UAVCAN::handle_mppt_stream(const MpptStreamCb &cb)
{
    const bool use_input_value = option_is_set(AG_BattMonitor_Params::Options::MPPT_Use_Input_Value);
    ...
    if (!_mppt.is_detected) {
        ...
        // Boot/Power-up event
        if (option_is_set(AG_BattMonitor_Params::Options::MPPT_Power_On_At_Boot)) {
            mppt_set_powered_state(true);
        } else if (option_is_set(AG_BattMonitor_Params::Options::MPPT_Power_Off_At_Boot)) {
...
void AG_BattMonitor_UAVCAN::handle_battery_info_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const BattInfoCb &cb)
{
    AG_BattMonitor_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, cb.msg->battery_id);
    if (driver == nullptr) {
        return;
    }
...
void AG_BattMonitor_UAVCAN::handle_battery_info_aux_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const BattInfoAuxCb &cb)
{
    AG_BattMonitor_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, cb.msg->battery_id);
    if (driver == nullptr) {
        return;
...
void AG_BattMonitor_UAVCAN::handle_mppt_stream_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const MpptStreamCb &cb)
{
    AG_BattMonitor_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, node_id);
    if (driver == nullptr) {
        return;
    }
...
void AG_BattMonitor_UAVCAN::read()
{
    ...
    _has_temperature = (AG_HAL::millis() - _state.temperature_time) <= AP_BATT_MONITOR_TIMEOUT;
...
bool AG_BattMonitor_UAVCAN::use_CAN_SoC() const
{
    ...
    return !(option_is_set(AG_BattMonitor_Params::Options::Ignore_UAVCAN_SoC) ||
            _mppt.is_detected ||
            (_soc == 127));
}

/// capacity_remaining_pct - returns true if the percentage is valid and writes to percentage argument
bool AG_BattMonitor_UAVCAN::capacity_remaining_pct(uint8_t &percentage) const
{
    if (!use_CAN_SoC()) {
        return AG_BattMonitor_Backend::capacity_remaining_pct(percentage);
    }
...
bool AG_BattMonitor_UAVCAN::reset_remaining(float percentage)
{
    if (use_CAN_SoC()) {
    ...
    if (!AG_BattMonitor_Backend::reset_remaining(percentage)) {
...
bool AG_BattMonitor_UAVCAN::get_cycle_count(uint16_t &cycles) const
{
    if (_has_battery_info_aux) {
        cycles = _cycle_count;
        return true;
...
void AG_BattMonitor_UAVCAN::mppt_check_powered_state()
{
    if ((_mppt.powered_state_remote_ms != 0) && (AG_HAL::millis() - _mppt.powered_state_remote_ms >= 1000)) {
    ...
    if ((!_mppt.vehicle_armed_last && vehicle_armed) && option_is_set(AG_BattMonitor_Params::Options::MPPT_Power_On_At_Arm)) {
        // arm event
        mppt_set_powered_state(true);
    } else if ((_mppt.vehicle_armed_last && !vehicle_armed) && option_is_set(AG_BattMonitor_Params::Options::MPPT_Power_Off_At_Disarm)) {
        // disarm event
        mppt_set_powered_state(false);
    }
...
void AG_BattMonitor_UAVCAN::mppt_set_powered_state(bool power_on)
{
    ...
    _mppt.powered_state_remote_ms = AG_HAL::millis();
...
void trampoline_handleOutputEnable(const uavcan::ServiceCallResult<mppt::OutputEnable>& resp)
{
    ...
    for (uint8_t i = 0; i < can_num_drivers; i++) {
        AG_UAVCAN *uavcan = AG_UAVCAN::get_uavcan(i);
        if (uavcan == nullptr) {
            continue;
        }

        const uint8_t node_id = resp.getResponse().getSrcNodeID().get();
        AG_BattMonitor_UAVCAN* driver = AG_BattMonitor_UAVCAN::get_uavcan_backend(uavcan, node_id, node_id);
        if (driver == nullptr) {
            continue;
        }
...
void AG_BattMonitor_UAVCAN::handle_outputEnable_response(const uint8_t nodeId, const bool enabled)
{
    if (nodeId != _node_id) {
...
void AG_BattMonitor_UAVCAN::mppt_report_faults(const uint8_t instance, const uint8_t fault_flags)
{
    // handle recovery
    if (fault_flags == 0) {
...
const char* AG_BattMonitor_UAVCAN::mppt_fault_string(const MPPT_FaultFlags fault)
{
    switch (fault) {
...
uint32_t AG_BattMonitor_UAVCAN::get_mavlink_fault_bitmask() const
{
