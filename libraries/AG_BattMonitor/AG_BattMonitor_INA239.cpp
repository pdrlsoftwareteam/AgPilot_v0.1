...
AG_BattMonitor_INA239::AG_BattMonitor_INA239(AG_BattMonitor &mon,
...
void AG_BattMonitor_INA239::init(void)
{
    ...
    dev->register_periodic_callback(25000, FUNCTOR_BIND_MEMBER(&AG_BattMonitor_INA239::timer, void));
}

void AG_BattMonitor_INA239::configure(void)
{
...
void AG_BattMonitor_INA239::read(void)
{
...
*/
bool AG_BattMonitor_INA239::read_word(const uint8_t reg, int16_t& data) const
{
...
*/
bool AG_BattMonitor_INA239::write_word(const uint8_t reg, const uint16_t data) const
{
    const uint8_t b[3] { uint8_t(reg<<2), uint8_t(data >> 8), uint8_t(data&0xff) };
    return dev->transfer(b, sizeof(b), nullptr, 0);
}

void AG_BattMonitor_INA239::timer(void)
{
