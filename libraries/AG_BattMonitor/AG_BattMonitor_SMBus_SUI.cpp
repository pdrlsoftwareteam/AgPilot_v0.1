...
// Constructor
AG_BattMonitor_SMBus_SUI::AG_BattMonitor_SMBus_SUI(AG_BattMonitor &mon,
...
void AG_BattMonitor_SMBus_SUI::init(void)
{
    AG_BattMonitor_SMBus::init();
    if (_dev) {
        _dev->set_retries(2);
    }
...
void AG_BattMonitor_SMBus_SUI::timer()
{
...
bool AG_BattMonitor_SMBus_SUI::read_block_bare(uint8_t reg, uint8_t* data, uint8_t len) const
{
    // read bytes
...
void AG_BattMonitor_SMBus_SUI::read_cell_voltages()
{
    // read cell voltages
    ...
    last_volt_read_us = AG_HAL::micros();
}

/*
  update healthy flag
 */
void AG_BattMonitor_SMBus_SUI::update_health()
{
