...
#include <AG_EFI/AG_EFI_Currawong_ECU.h>

#include <stdio.h>

// Protocol files for the Velocity ESC
#include <AG_PiccoloCAN/piccolo_protocol/ESCVelocityProtocol.h>
#include <AG_PiccoloCAN/piccolo_protocol/ESCPackets.h>

// Protocol files for the CBS servo
#include <AG_PiccoloCAN/piccolo_protocol/ServoProtocol.h>
#include <AG_PiccoloCAN/piccolo_protocol/ServoPackets.h>

// Protocol files for the ECU
#include <AG_PiccoloCAN/piccolo_protocol/ECUProtocol.h>
...
const AG_Param::GroupInfo AG_PiccoloCAN::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("ESC_BM", 1, AG_PiccoloCAN, _esc_bm, 0xFFFF),
    ...
    // @User: Advanced
    // @Range: 1 500
    AP_GROUPINFO("ESC_RT", 2, AG_PiccoloCAN, _esc_hz, PICCOLO_MSG_RATE_HZ_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("SRV_BM", 3, AG_PiccoloCAN, _srv_bm, 0xFFFF),
    ...
    // @User: Advanced
    // @Range: 1 500
    AP_GROUPINFO("SRV_RT", 4, AG_PiccoloCAN, _srv_hz, PICCOLO_MSG_RATE_HZ_DEFAULT),
    ...
    // @Range: 0 255
    // @User: Advanced
    AP_GROUPINFO("ECU_ID", 5, AG_PiccoloCAN, _ecu_id, PICCOLO_CAN_ECU_ID_DEFAULT),
    ...
    // @User: Advanced
    // @Range: 1 500
    AP_GROUPINFO("ECU_RT", 6, AG_PiccoloCAN, _ecu_hz, PICCOLO_MSG_RATE_HZ_DEFAULT),
#endif
    AP_GROUPEND
};

AG_PiccoloCAN::AG_PiccoloCAN()
{
    AG_Param::setup_object_defaults(this, var_info);

    debug_can(AG_CANManager::LOG_INFO, "PiccoloCAN: constructed\n\r");
}

AG_PiccoloCAN *AG_PiccoloCAN::get_pcan(uint8_t driver_index)
{
    if (driver_index >= AP::can().get_num_drivers() ||
        AP::can().get_driver_type(driver_index) != AG_CANManager::Driver_Type_PiccoloCAN) {
        return nullptr;
    }

    return static_cast<AG_PiccoloCAN*>(AP::can().get_driver(driver_index));
}

bool AG_PiccoloCAN::add_interface(AG_HAL::CANIface* can_iface) {
    if (_can_iface != nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: Multiple Interface not supported\n\r");
        return false;
    }
    ...
    if (_can_iface == nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: CAN driver not found\n\r");
        return false;
    }

    if (!_can_iface->is_initialized()) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: Driver not initialized\n\r");
        return false;
    }

    if (!_can_iface->set_event_handle(&_event_handle)) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: Cannot add event handle\n\r");
        return false;
    }
    return true;
}
...
void AG_PiccoloCAN::init(uint8_t driver_index, bool enable_filters)
{
    _driver_index = driver_index;

    debug_can(AG_CANManager::LOG_DEBUG, "PiccoloCAN: starting init\n\r");

    if (_initialized) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: already initialized\n\r");
        return;
    }
    // start calls to loop in separate thread
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_PiccoloCAN::loop, void), _thread_name, 4096, AG_HAL::Scheduler::PRIORITY_MAIN, 1)) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: couldn't create thread\n\r");
        return;
    }
    ...
    debug_can(AG_CANManager::LOG_DEBUG, "PiccoloCAN: init done\n\r");
}

// loop to send output to CAN devices in background thread
void AG_PiccoloCAN::loop()
{
    AG_HAL::CANFrame txFrame {};
    AG_HAL::CANFrame rxFrame {};
    ...
    while (true) {
        ...
        if (!_initialized) {
            debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: not initialized\n\r");
...
#endif
        uint64_t timeout = AG_HAL::micros64() + 250ULL;
        ...
        while (read_frame(rxFrame, timeout)) {
            ...
            if ((rxFrame.id & AG_HAL::CANFrame::FlagEFF) == 0) {
                continue;
            }
...
bool AG_PiccoloCAN::write_frame(AG_HAL::CANFrame &out_frame, uint64_t timeout)
{
    if (!_initialized) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: Driver not initialized for write_frame\n\r");
        return false;
    }
    ...
    return (_can_iface->send(out_frame, timeout, AG_HAL::CANIface::AbortOnError) == 1);
}

// read frame on CAN bus, returns true on succses
bool AG_PiccoloCAN::read_frame(AG_HAL::CANFrame &recv_frame, uint64_t timeout)
{
    if (!_initialized) {
        debug_can(AG_CANManager::LOG_ERROR, "PiccoloCAN: Driver not initialized for read_frame\n\r");
        return false;
    }
...
void AG_PiccoloCAN::update()
{
    uint64_t timestamp = AG_HAL::micros64();
...
void AG_PiccoloCAN::send_esc_telemetry_mavlink(uint8_t mav_chan)
{
...
void AG_PiccoloCAN::send_servo_messages(void)
{
    AG_HAL::CANFrame txFrame {};

    uint64_t timeout = AG_HAL::micros64() + 1000ULL;
...
void AG_PiccoloCAN::send_esc_messages(void)
{
    AG_HAL::CANFrame txFrame {};
...
bool AG_PiccoloCAN::handle_servo_message(AG_HAL::CANFrame &frame)
{
    uint64_t timestamp = AG_HAL::micros64();
...
bool AG_PiccoloCAN::handle_esc_message(AG_HAL::CANFrame &frame)
{
    bool result = true;
    ...
    if (decodeESC_StatusAPacket(&frame, &esc.mode, &esc.status, &esc.setpoint, &esc.rpm)) {
        ...
        AG_ESC_Telem_Backend::TelemetryData telem {};
        ...
        update_telem_data(addr, telem,
            AG_ESC_Telem_Backend::TelemetryType::CURRENT
                ...
                | AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE);
        ...
        update_telem_data(addr, telem, AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE);
...
#if AG_EFI_CURRAWONG_ECU_ENABLED
void AG_PiccoloCAN::send_ecu_messages(void)
{
    AG_HAL::CANFrame txFrame {};

    const uint64_t timeout = AG_HAL::micros64() + 1000ULL;
...
bool AG_PiccoloCAN::handle_ecu_message(AG_HAL::CANFrame &frame)
{
    // Get the ecu instance
    AG_EFI_Currawong_ECU* ecu = AG_EFI_Currawong_ECU::get_instance();
    if (ecu != nullptr) {
 ...
 */
bool AG_PiccoloCAN::is_servo_channel_active(uint8_t chan)
{
 ...
 */
bool AG_PiccoloCAN::is_esc_channel_active(uint8_t chan)
{
 ...
 */
bool AG_PiccoloCAN::is_servo_present(uint8_t chan, uint64_t timeout_ms)
{
 ...
 */
bool AG_PiccoloCAN::is_esc_present(uint8_t chan, uint64_t timeout_ms)
{
 ...
 */
bool AG_PiccoloCAN::is_servo_enabled(uint8_t chan)
{
    if (chan >= PICCOLO_CAN_MAX_NUM_SERVO) {
        return false;
    }
 ...
 */
bool AG_PiccoloCAN::is_esc_enabled(uint8_t chan)
{
    if (chan >= PICCOLO_CAN_MAX_NUM_ESC) {
        return false;
    }
...
bool AG_PiccoloCAN::pre_arm_check(char* reason, uint8_t reason_len)
{
...
{
    AG_HAL::CANFrame* frame = (AG_HAL::CANFrame*) pkt;
...
void finishESCVelocityPacket(void* pkt, int size, uint32_t packetID)
{
    ...
    if (size > AG_HAL::CANFrame::MaxDataLen) {
        size = AG_HAL::CANFrame::MaxDataLen;
    }
    ...
    uint32_t id = (((uint8_t) AG_PiccoloCAN::MessageGroup::ACTUATOR) << 24) |       // CAN Group ID
                  ((packetID & 0xFF) << 16) |                                       // Message ID
                  (((uint8_t) AG_PiccoloCAN::ActuatorType::ESC) << 8);              // Actuator type

    // Extended frame format
    id |= AG_HAL::CANFrame::FlagEFF;
...
void finishServoPacket(void* pkt, int size, uint32_t packetID)
{
    ...
    uint32_t id = (((uint8_t) AG_PiccoloCAN::MessageGroup::ACTUATOR) << 24) |       // CAN Group ID
                  ((packetID & 0xFF) << 16) |                                       // Message ID
                  (((uint8_t) AG_PiccoloCAN::ActuatorType::SERVO) << 8);            // Actuator type
...
void finishECUPacket(void* pkt, int size, uint32_t packetID)
{
    ...
    uint32_t id = (((uint8_t) AG_PiccoloCAN::MessageGroup::ECU_IN) << 24) |       // CAN Group ID
                  ((packetID & 0xFF) << 16);                                       // Message ID
