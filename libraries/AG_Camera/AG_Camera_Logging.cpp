...
void AG_Camera_Backend::Write_CameraInfo(enum LogMessages msg, uint64_t timestamp_us)
{
    ...
    if (gps.status() >= AG_GPS::GPS_OK_FIX_3D) {
    ...
    const struct log_Camera pkt{
        ...
        time_us     : timestamp_us ? timestamp_us : AG_HAL::micros64(),
        instance    : _instance,
...
// Write a Camera packet
void AG_Camera_Backend::Write_Camera(uint64_t timestamp_us)
{
    Write_CameraInfo(LOG_CAMERA_MSG, timestamp_us);
}

// Write a Trigger packet
void AG_Camera_Backend::Write_Trigger()
{
