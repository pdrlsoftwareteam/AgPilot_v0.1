...
// Constructor
AG_Camera_Backend::AG_Camera_Backend(AG_Camera &frontend, AG_Camera_Params &params, uint8_t instance) :
    _frontend(frontend),
    _params(params),
    _instance(instance)
{}
...
bool AG_Camera_Backend::take_picture()
{
...
void AG_Camera_Backend::control(float session, float zoom_pos, float zoom_step, float focus_lock, float shooting_cmd, float cmd_id)
{
    // take picture
...
void AG_Camera_Backend::send_camera_feedback(mavlink_channel_t chan) const
{
    int32_t altitude = 0;
...
void AG_Camera_Backend::setup_feedback_callback()
{
    ...
    if (hal.gpio->attach_interrupt(_params.feedback_pin, FUNCTOR_BIND_MEMBER(&AG_Camera_Backend::feedback_pin_isr, void, uint8_t, bool, uint32_t),
                                   trigger_polarity?AG_HAL::GPIO::INTERRUPT_RISING:AG_HAL::GPIO::INTERRUPT_FALLING)) {
        isr_installed = true;
    } else {
        // install a 1kHz timer to check feedback pin
        hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&AG_Camera_Backend::feedback_pin_timer, void));
...
void AG_Camera_Backend::feedback_pin_isr(uint8_t pin, bool high, uint32_t timestamp_us)
{
...
void AG_Camera_Backend::check_feedback()
{
    if (feedback_trigger_logged_count != feedback_trigger_count) {
        ...
        // log camera message
        uint32_t tdiff = AG_HAL::micros() - timestamp32;
        uint64_t timestamp = AG_HAL::micros64();
        Write_Camera(timestamp - tdiff);
    }
}

void AG_Camera_Backend::prep_mavlink_msg_camera_feedback(uint64_t timestamp_us)
{
    const AG_AHRS &ahrs = AP::ahrs();
...
// log picture
void AG_Camera_Backend::log_picture()
{
