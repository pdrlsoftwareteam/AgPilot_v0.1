...
const AG_Param::GroupInfo AG_RunCam::var_info[] = {
    // @Param: TYPE
    ...
    AP_GROUPINFO_FLAGS("TYPE", 1, AG_RunCam, _cam_type, int(DeviceType::Disabled), AP_PARAM_FLAG_ENABLE),
    ...
    AP_GROUPINFO("FEATURES", 2, AG_RunCam, _features, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("BT_DELAY", 3, AG_RunCam, _boot_delay_ms, 7000),
    ...
    // @User: Advanced
    AP_GROUPINFO("BTN_DELAY", 4, AG_RunCam, _button_delay_ms, RUNCAM_DEFAULT_BUTTON_PRESS_DELAY),
    ...
    // @User: Advanced
    AP_GROUPINFO("MDE_DELAY", 5, AG_RunCam, _mode_delay_ms, 800),
    ...
    // @User: Advanced
    AP_GROUPINFO("CONTROL", 6, AG_RunCam, _cam_control_option, uint8_t(ControlOption::STICK_ROLL_RIGHT) | uint8_t(ControlOption::TWO_POS_SWITCH)),
...
// singleton instance
AG_RunCam *AG_RunCam::_singleton;

AG_RunCam::Request::Length AG_RunCam::Request::_expected_responses_length[RUNCAM_NUM_EXPECTED_RESPONSES] = {
...
AG_RunCam::Menu AG_RunCam::_menus[RUNCAM_MAX_DEVICE_TYPES] = {
...
AG_RunCam::AG_RunCam()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_RunCam must be singleton");
    }
    _singleton = this;
...
void AG_RunCam::init()
{
    AG_SerialManager *serial_manager = AG_SerialManager::get_singleton();
    if (serial_manager) {
        uart = serial_manager->find_serial(AG_SerialManager::SerialProtocol_RunCam, 0);
    }
    if (uart != nullptr) {
        /*
    ...
    _transition_start_ms = AG_HAL::millis();
...
bool AG_RunCam::simulate_camera_button(const ControlOperation operation, const uint32_t transition_timeout)
{
...
// start the video
void AG_RunCam::start_recording() {
...
void AG_RunCam::osd_option() {
    debug("osd option\n");
    _osd_option = OSDOption::OPTION;
}

// input update loop
void AG_RunCam::update()
{
...
bool AG_RunCam::pre_arm_check(char *failure_msg, const uint8_t failure_msg_len) const
{
...
void AG_RunCam::update_state_machine_armed()
{
...
void AG_RunCam::update_state_machine_disarmed()
{
    const uint32_t now = AG_HAL::millis();
...
void AG_RunCam::handle_initialized(Event ev)
{
    ...
    debug("device fully booted after %ums\n", unsigned(AG_HAL::millis()));
}

// handle the ready state
void AG_RunCam::handle_ready(Event ev)
{
    switch (ev) {
...
void AG_RunCam::handle_recording(Event ev)
{
    switch (ev) {
    case Event::ENTER_MENU:
    case Event::IN_MENU_ENTER:
...
void AG_RunCam::handle_in_menu(Event ev)
{
    if (has_5_key_OSD()) {
...
AG_RunCam::Event AG_RunCam::map_rc_input_to_event() const
{
...
void AG_RunCam::handle_2_key_simulation_process(Event ev)
{
...
void AG_RunCam::exit_2_key_osd_menu()
{
    _in_menu = _menu_enter_level;
...
void AG_RunCam::handle_5_key_simulation_process(Event ev) 
{
...
// handle a response
void AG_RunCam::handle_5_key_simulation_response(const Request& request)
{
...
AG_RunCam::ControlOperation AG_RunCam::start_recording_command() const {
...
AG_RunCam::ControlOperation AG_RunCam::stop_recording_command() const {
    if (DeviceType(_cam_type.get()) == DeviceType::Split4k || DeviceType(_cam_type.get()) == DeviceType::Hybrid || DeviceType(_cam_type.get()) == DeviceType::Run24k) {
        return ControlOperation::RCDEVICE_PROTOCOL_SIMULATE_POWER_BTN;
    } else {
...
void AG_RunCam::start_uart()
{
    // 8N1 communication
    uart->configure_parity(0);
    uart->set_stop_bits(1);
    uart->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
    uart->set_blocking_writes(false);   // updates run in the main thread
    uart->set_options(uart->get_options() | AG_HAL::UARTDriver::OPTION_NODMA_TX | AG_HAL::UARTDriver::OPTION_NODMA_RX);
    uart->begin(115200, 10, 10);
    uart->discard_input();
}

// get the device info (firmware version, protocol version and features)
void AG_RunCam::get_device_info()
{
    send_request_and_waiting_response(Command::RCDEVICE_PROTOCOL_COMMAND_GET_DEVICE_INFO, 0, RUNCAM_INIT_INTERVAL_MS * 4,
        UINT16_MAX, FUNCTOR_BIND_MEMBER(&AG_RunCam::parse_device_info, void, const Request&));
}

// map a Event to a SimulationOperation
AG_RunCam::SimulationOperation AG_RunCam::map_key_to_protocol_operation(const Event key) const
{
...
// send an event
void AG_RunCam::send_5_key_OSD_cable_simulation_event(const Event key, const uint32_t transition_timeout)
{
    ...
    switch (key) {
    case Event::ENTER_MENU:
        open_5_key_OSD_cable_connection(FUNCTOR_BIND_MEMBER(&AG_RunCam::handle_5_key_simulation_response, void, const Request&));
        break;
    case Event::EXIT_MENU:
        close_5_key_OSD_cable_connection(FUNCTOR_BIND_MEMBER(&AG_RunCam::handle_5_key_simulation_response, void, const Request&));
        break;
    case Event::IN_MENU_UP:
    case Event::IN_MENU_RIGHT:
    case Event::IN_MENU_DOWN:
    case Event::IN_MENU_ENTER:
    case Event::IN_MENU_EXIT:
        simulate_5_key_OSD_cable_button_press(map_key_to_protocol_operation(key), FUNCTOR_BIND_MEMBER(&AG_RunCam::handle_5_key_simulation_response, void, const Request&));
        break;
    case Event::BUTTON_RELEASE:
        simulate_5_key_OSD_cable_button_release(FUNCTOR_BIND_MEMBER(&AG_RunCam::handle_5_key_simulation_response, void, const Request&));
        break;
    case Event::STOP_RECORDING:
    case Event::START_RECORDING:
    case Event::NONE:
...
void AG_RunCam::open_5_key_OSD_cable_connection(parse_func_t parseFunc)
{
...
void AG_RunCam::close_5_key_OSD_cable_connection(parse_func_t parseFunc)
{
    send_request_and_waiting_response(Command::RCDEVICE_PROTOCOL_COMMAND_5KEY_CONNECTION,
...
void AG_RunCam::simulate_5_key_OSD_cable_button_press(const SimulationOperation operation, parse_func_t parseFunc)
{
...
void AG_RunCam::simulate_5_key_OSD_cable_button_release(parse_func_t parseFunc)
{
...
void AG_RunCam::send_request_and_waiting_response(Command commandID, uint8_t param,
...
void AG_RunCam::send_packet(Command command, uint8_t param)
{
    // is this device open?
    if (!uart) {
        return;
...
void AG_RunCam::parse_device_info(const Request& request)
{
...
bool AG_RunCam::camera_ready() const
{
...
void AG_RunCam::simulation_OSD_cable_failed(const Request& request)
{
    _waiting_device_response = false;
...
bool AG_RunCam::request_pending(uint32_t now)
{
...
AG_RunCam::Request::Request(AG_RunCam* device, Command commandID, uint8_t param,
    uint32_t timeout, uint16_t maxRetryTimes, parse_func_t parserFunc)
...
{
    _request_timestamp_ms = AG_HAL::millis();
    _expected_response_length = get_expected_response_length(commandID);
}

uint8_t AG_RunCam::Request::get_crc() const
{
    uint8_t crc = 0;
...
uint8_t AG_RunCam::Request::get_expected_response_length(const Command command) const
{
...
AG_RunCam *AP::runcam() {
    return AG_RunCam::get_singleton();
}
