...
#include "AG_Camera_Params.h"
#include "AG_Mount/AG_Mount_config.h"

#define AP_CAMERA_MAX_INSTANCES             2       // maximum number of camera backends

// declare backend classes
class AG_Camera_Backend;
class AG_Camera_Servo;
class AG_Camera_Relay;
class AG_Camera_SoloGimbal;
class AG_Camera_Mount;
...
class AG_Camera {
    ...
    friend class AG_Camera_Backend;
    friend class AG_Camera_Servo;
    friend class AG_Camera_Relay;
    friend class AG_Camera_SoloGimbal;
    friend class AG_Camera_Mount;
    friend class AG_Camera_MAVLink;

public:

    // constructor
    AG_Camera(uint32_t _log_camera_bit);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_Camera);

    // get singleton instance
    static AG_Camera *get_singleton() { return _singleton; }
...
protected:
    ...
    AG_Camera_Params _params[AP_CAMERA_MAX_INSTANCES];

private:

    static AG_Camera *_singleton;
    ...
    AG_Camera_Backend *get_instance(uint8_t instance);

    // perform any required parameter conversion
    void convert_params();

    AG_Camera_Backend *primary;         // primary camera backed
    bool _is_in_auto_mode;              // true if in AUTO mode
    uint32_t log_camera_bit;            // logging bit (from LOG_BITMASK) to enable camera logging
    AG_Camera_Backend *_backends[AP_CAMERA_MAX_INSTANCES];  // pointers to instantiated backends
};
