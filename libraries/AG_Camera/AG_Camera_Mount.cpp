#include "AG_Camera_Mount.h"

#if AP_CAMERA_MOUNT_ENABLED
#include <AG_Mount/AG_Mount.h>

extern const AG_HAL::HAL& hal;

// entry point to actually take a picture.  returns true on success
bool AG_Camera_Mount::trigger_pic()
{
    AG_Mount* mount = AP::mount();
    if (mount != nullptr) {
        mount->take_picture(0);
        return true;
    }
    return false;
}

// start/stop recording video.  returns true on success
// start_recording should be true to start recording, false to stop recording
bool AG_Camera_Mount::record_video(bool start_recording)
{
    AG_Mount* mount = AP::mount();
    if (mount != nullptr) {
        return mount->record_video(0, start_recording);
    }
    return false;
}

// zoom in, out or hold.  returns true on success
// zoom out = -1, hold = 0, zoom in = 1
bool AG_Camera_Mount::set_zoom_step(int8_t zoom_step)
{
    AG_Mount* mount = AP::mount();
    if (mount != nullptr) {
        return mount->set_zoom_step(0, zoom_step);
    }
    return false;
}

// focus in, out or hold.  returns true on success
// focus in = -1, focus hold = 0, focus out = 1
bool AG_Camera_Mount::set_manual_focus_step(int8_t focus_step)
{
    AG_Mount* mount = AP::mount();
    if (mount != nullptr) {
        return mount->set_manual_focus_step(0, focus_step);
    }
    return false;
}

// auto focus.  returns true on success
bool AG_Camera_Mount::set_auto_focus()
{
    AG_Mount* mount = AP::mount();
    if (mount != nullptr) {
        return mount->set_auto_focus(0);
    }
    return false;
}

#endif // AP_CAMERA_MOUNT_ENABLED
