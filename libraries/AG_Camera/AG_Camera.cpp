...
#include "AG_Camera_Servo.h"
#include "AG_Camera_Relay.h"
#include "AG_Camera_Mount.h"
#include "AG_Camera_MAVLink.h"
#include "AG_Camera_SoloGimbal.h"

const AG_Param::GroupInfo AG_Camera::var_info[] = {
    ...
    // @Units: deg
    // @Range: 0 180
    AP_GROUPINFO("_MAX_ROLL",  7, AG_Camera, _max_roll, 0),
    ...
    // @User: Standard
    AP_GROUPINFO("_AUTO_ONLY",  10, AG_Camera, _auto_mode_only, 0),

    // @Group: 1
    // @Path: AG_Camera_Params.cpp
    AP_SUBGROUPINFO(_params[0], "1", 12, AG_Camera, AG_Camera_Params),

#if AP_CAMERA_MAX_INSTANCES > 1
    // @Group: 2
    // @Path: AG_Camera_Params.cpp
    AP_SUBGROUPINFO(_params[1], "2", 13, AG_Camera, AG_Camera_Params),
#endif
...
AG_Camera::AG_Camera(uint32_t _log_camera_bit) :
...
void AG_Camera::set_trigger_distance(float distance_m)
{
    if (primary == nullptr) {
        return;
    }
...
void AG_Camera::cam_mode_toggle()
{
    if (primary == nullptr) {
        return;
    }
...
bool AG_Camera::record_video(bool start_recording)
{
    if (primary == nullptr) {
        return false;
    }
...
bool AG_Camera::set_zoom_step(int8_t zoom_step)
{
    if (primary == nullptr) {
        return false;
    }
...
bool AG_Camera::set_manual_focus_step(int8_t focus_step)
{
    if (primary == nullptr) {
        return false;
    }
...
void AG_Camera::init()
{
    ...
    for (uint8_t instance = 0; instance < AP_CAMERA_MAX_INSTANCES; instance++) {
        ...
        case CameraType::SERVO:
            _backends[instance] = new AG_Camera_Servo(*this, _params[instance], instance);
            break;
#endif
#if AP_CAMERA_RELAY_ENABLED
        case CameraType::RELAY:
            _backends[instance] = new AG_Camera_Relay(*this, _params[instance], instance);
            break;
#endif
#if AP_CAMERA_SOLOGIMBAL_ENABLED
        // check for GoPro in Solo camera
        case CameraType::SOLOGIMBAL:
            _backends[instance] = new AG_Camera_SoloGimbal(*this, _params[instance], instance);
            break;
#endif
#if AP_CAMERA_MOUNT_ENABLED
        // check for Mount camera
        case CameraType::MOUNT:
            _backends[instance] = new AG_Camera_Mount(*this, _params[instance], instance);
            break;
#endif
#if AP_CAMERA_MAVLINK_ENABLED
        // check for MAVLink enabled camera driver
        case CameraType::MAVLINK:
            _backends[instance] = new AG_Camera_MAVLink(*this, _params[instance], instance);
            break;
#endif
...
void AG_Camera::handle_message(mavlink_channel_t chan, const mavlink_message_t &msg)
{
...
MAV_RESULT AG_Camera::handle_command_long(const mavlink_command_long_t &packet)
{
...
// set camera trigger distance in a mission
void AG_Camera::set_trigger_distance(uint8_t instance, float distance_m)
{
...
// momentary switch to change camera between picture and video modes
void AG_Camera::cam_mode_toggle(uint8_t instance)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
// configure camera
void AG_Camera::configure(float shooting_mode, float shutter_speed, float aperture, float ISO, float exposure_type, float cmd_id, float engine_cutoff_time)
{
    if (primary == nullptr) {
        return;
    }
...
void AG_Camera::configure(uint8_t instance, float shooting_mode, float shutter_speed, float aperture, float ISO, float exposure_type, float cmd_id, float engine_cutoff_time)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
void AG_Camera::control(float session, float zoom_pos, float zoom_step, float focus_lock, float shooting_cmd, float cmd_id)
{
    if (primary == nullptr) {
        return;
    }
...
void AG_Camera::control(uint8_t instance, float session, float zoom_pos, float zoom_step, float focus_lock, float shooting_cmd, float cmd_id)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
 ...
 */
void AG_Camera::send_feedback(mavlink_channel_t chan) const
{
    // call each instance
    for (uint8_t instance = 0; instance < AP_CAMERA_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
...
void AG_Camera::take_picture(uint8_t instance)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return;
...
// start_recording should be true to start recording, false to stop recording
bool AG_Camera::record_video(uint8_t instance, bool start_recording)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
// zoom out = -1, hold = 0, zoom in = 1
bool AG_Camera::set_zoom_step(uint8_t instance, int8_t zoom_step)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
// focus in = -1, focus hold = 0, focus out = 1
bool AG_Camera::set_manual_focus_step(uint8_t instance, int8_t focus_step)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
bool AG_Camera::set_auto_focus(uint8_t instance)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
...
AG_Camera_Backend *AG_Camera::get_instance(uint8_t instance)
{
...
void AG_Camera::convert_params()
{
    ...
    int8_t cam1_type = 0;
    IGNORE_RETURN(AG_Param::get_param_by_index(this, 0, AP_PARAM_INT8, &cam_trigg_type));
    ...
    if (AG_Param::get_param_by_index(this, 1, AP_PARAM_INT8, &cam_duration) && (cam_duration > 0)) {
    ...
    if (AG_Param::get_param_by_index(this, 6, AP_PARAM_INT16, &cam_min_interval) && (cam_min_interval > 0)) {
        _params[0].interval_min.set_and_save(cam_min_interval * 0.001f);
    }

    // find Camera's top level key
    uint16_t k_param_camera_key;
    if (!AG_Param::find_top_level_key_by_pointer(this, k_param_camera_key)) {
        return;
    }

    // table parameters to convert without scaling
    static const AG_Param::ConversionInfo camera_param_conversion_info[] {
    ...
    for (uint8_t i=0; i<table_size; i++) {
        AG_Param::convert_old_parameter(&camera_param_conversion_info[i], 1.0f);
    }
}

// singleton instance
AG_Camera *AG_Camera::_singleton;

namespace AP {

AG_Camera *camera()
{
    return AG_Camera::get_singleton();
}
