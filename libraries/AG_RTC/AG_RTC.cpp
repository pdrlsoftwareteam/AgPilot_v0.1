...
const AG_Param::GroupInfo AG_RTC::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("_TYPES",  1, AG_RTC, allowed_types, 7),
    ...
    // @Range: -720 +840
    // @User: Advanced
    AP_GROUPINFO("_TZ_MIN",  2, AG_RTC, tz_min, 330),
    
    AP_GROUPEND
};

void AG_RTC::set_utc_usec(uint64_t time_utc_usec, source_type type)
{
...
bool AG_RTC::get_utc_usec(uint64_t &usec) const
{
    if (rtc_source_type == SOURCE_NONE) {
        return false;
    }
    usec = AG_HAL::micros64() + rtc_shift;
    return true;
}

bool AG_RTC::get_system_clock_utc(uint8_t &hour, uint8_t &min, uint8_t &sec, uint16_t &ms) const
{
...
bool AG_RTC::get_local_time(uint8_t &hour, uint8_t &min, uint8_t &sec, uint16_t &ms) const
{
...
bool AG_RTC::get_local_date_time(uint16_t &year, uint8_t &month, uint8_t &day,
...
bool AG_RTC::is_leap_year(uint16_t year) const
{
...
uint32_t AG_RTC::get_time_utc(int32_t hour, int32_t min, int32_t sec, int32_t ms)
{
 ...
 */
time_t AG_RTC::mktime(const struct tm *t)
{
    time_t epoch = 0;
    int n;
...
// singleton instance
AG_RTC *AG_RTC::_singleton;

namespace AP {

AG_RTC &rtc()
{
    return *AG_RTC::get_singleton();
}
