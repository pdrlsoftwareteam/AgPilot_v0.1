...
const AG_Param::GroupInfo AG_GyroFFT::var_info[] = {
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_GyroFFT, _enable, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_GROUPINFO("MINHZ", 2, AG_GyroFFT, _fft_min_hz, 50),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_GROUPINFO("MAXHZ", 3, AG_GyroFFT, _fft_max_hz, 450),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("SAMPLE_MODE", 4, AG_GyroFFT, _sample_mode, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("WINDOW_SIZE", 5, AG_GyroFFT, _window_size, FFT_DEFAULT_WINDOW_SIZE),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("WINDOW_OLAP", 6, AG_GyroFFT, _window_overlap, FFT_DEFAULT_WINDOW_OVERLAP),
    ...
    // @Range: 0 250
    // @User: Advanced
    AP_GROUPINFO("FREQ_HOVER", 7, AG_GyroFFT, _freq_hover_hz, 80.0f),
    ...
    // @Range: 0.01 0.9
    // @User: Advanced
    AP_GROUPINFO("THR_REF", 8, AG_GyroFFT, _throttle_ref, FFT_THR_REF_DEFAULT),
    ...
    // @Range: 0.0 100.0
    // @User: Advanced
    AP_GROUPINFO("SNR_REF", 9, AG_GyroFFT, _snr_threshold_db, FFT_SNR_DEFAULT),
    ...
    // @Range: 0 100
    // @User: Advanced
    AP_GROUPINFO("ATT_REF", 10, AG_GyroFFT, _attenuation_power_db, 15),
    ...
    // @Range: 0 200
    // @User: Advanced
    AP_GROUPINFO("BW_HOVER", 11, AG_GyroFFT, _bandwidth_hover_hz, 20),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("HMNC_FIT", 12, AG_GyroFFT, _harmonic_fit, FFT_HARMONIC_FIT_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("HMNC_PEAK", 13, AG_GyroFFT, _harmonic_peak, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("NUM_FRAMES", 14, AG_GyroFFT, _num_frames, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("OPTIONS", 15, AG_GyroFFT, _options, 0),
...
void AG_GyroFFT::init(uint16_t loop_rate_hz)
{
    ...
    if (_ins->get_raw_gyro_rate_hz() == 0) {
        AG_HAL::panic("AG_GyroFFT must be initialized after AG_InertialSensor");
    }
    ...
    if (_num_frames > 0) {
        _num_frames.set(constrain_int16(_num_frames, 2, AG_HAL::DSP::MAX_SLIDING_WINDOW_SIZE));
    }
    ...
    if (allocation_count * FFT_DEFAULT_WINDOW_SIZE > hal.util->available_memory() / 2) {
        gcs().send_text(MAV_SEVERITY_WARNING, "AG_GyroFFT: disabled, required %u bytes", (unsigned int)allocation_count * FFT_DEFAULT_WINDOW_SIZE);
        return;
    } else if (allocation_count * _window_size > hal.util->available_memory() / 2) {
        gcs().send_text(MAV_SEVERITY_WARNING, "AG_GyroFFT: req alloc %u bytes (free=%u)", (unsigned int)allocation_count * _window_size, (unsigned int)hal.util->available_memory());
    ...
    if (_sample_mode == 0) {
        ...
        for (uint8_t axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            if (!_downsampled_gyro_data[axis].set_size(_window_size + _samples_per_frame)) {
                gcs().send_text(MAV_SEVERITY_WARNING, "Failed to allocate window for AG_GyroFFT");
                return;
            }
        }
    }
...
uint16_t AG_GyroFFT::run_cycle()
{
    ...
    _thread_state._last_output_us[_update_axis] = AG_HAL::micros();
...
bool AG_GyroFFT::start_analysis() {
...
void AG_GyroFFT::update_parameters(bool force)
{
...
void AG_GyroFFT::update_thread(void)
{
    while (true) {
...
bool AG_GyroFFT::start_update_thread(void)
{
    WITH_SEMAPHORE(_sem);
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_GyroFFT::update_thread, void), "apm_fft", FFT_STACK_SIZE, AG_HAL::Scheduler::PRIORITY_IO, 0)) {
        AG_HAL::panic("Failed to start AG_GyroFFT update thread");
        return false;
    }
...
bool AG_GyroFFT::pre_arm_check(char *failure_msg, const uint8_t failure_msg_len)
{
    if (!analysis_enabled()) {
        return true;
    }
    ...
    // see timings AG_HAL_ChibiOS/DSP.cpp for per step timings on different hardware
#if defined(STM32H7)
...
bool AG_GyroFFT::prepare_for_arming()
{
    _calibrated = true;
    return true;
}
...
// called from main thread
void AG_GyroFFT::update_freq_hover(float dt, float throttle_out)
{
    if (!analysis_enabled()) {
        return;
    }
...
// called from main thread
void AG_GyroFFT::save_params_on_disarm()
{
    if (!analysis_enabled()) {
        return;
    }
    ...
    // notch tuning
void AG_GyroFFT::start_notch_tune()
{
    if (!analysis_enabled()) {
        return;
    }
...
float AG_GyroFFT::calculate_notch_frequency(float* freqs, uint16_t numpeaks, float harmonic_fit, uint8_t& harmonics)
{
...
void AG_GyroFFT::stop_notch_tune()
{
    if (!analysis_enabled()) {
        return;
    }
    ...
    if (is_zero(_avg_throttle_out) || is_zero(harmonic)) {
        ...
        AG_Notify::events.autotune_failed = true;
        return;
    }
    ...
    if (!_ins->setup_throttle_gyro_harmonic_notch(harmonic, (float)_fft_min_hz.get(), _avg_throttle_out, harmonics)) {
        ...
        AG_Notify::events.autotune_complete = true;
    }
}

// return the noise peak that is being tracked
// called from main thread
AG_GyroFFT::FrequencyPeak AG_GyroFFT::get_tracked_noise_peak() const
{
...
float AG_GyroFFT::get_weighted_freq_hz(FrequencyPeak peak) const
{
...
// called from main thread
float AG_GyroFFT::get_weighted_noise_center_freq_hz() const
{
    if (!analysis_enabled()) {
        return _fft_min_hz;
    }
...
// called from main thread
uint8_t AG_GyroFFT::get_weighted_noise_center_frequencies_hz(uint8_t num_freqs, float* freqs) const
{
    if (!analysis_enabled()) {
        freqs[0] = _fft_min_hz;
        return 1;
...
float AG_GyroFFT::has_noise_at_frequency_hz(float freq) const
{
    if (!analysis_enabled()) {
        return 0.0f;
    }
...
float AG_GyroFFT::calculate_weighted_freq_hz(const Vector3f& energy, const Vector3f& freq) const
{
...
void AG_GyroFFT::write_log_messages()
{
    if (!analysis_enabled()) {
        return;
    }
...
void AG_GyroFFT::log_noise_peak(uint8_t id, FrequencyPeak peak) const
{
...
// called from main thread
float AG_GyroFFT::get_weighted_noise_center_bandwidth_hz() const
{
    if (!analysis_enabled()) {
        return 0.0f;
    }
...
// called from FFT thread
void AG_GyroFFT::calculate_noise(bool calibrating, const EngineConfig& config)
{
    ...
    if (num_peaks > 0) {
        _thread_state._health_ms[_update_axis] = AG_HAL::millis();
    } else {
...
uint8_t AG_GyroFFT::calculate_tracking_peaks(float& weighted_center_freq_hz, bool calibrating, const EngineConfig& config)
{
    uint8_t num_peaks = 0;
...
uint8_t AG_GyroFFT::calculate_tracking_peaks(float& weighted_center_freq_hz, const FrequencyData& freqs, const EngineConfig& config)
{
    uint8_t num_peaks = 0;
...
// called from FFT thread
bool AG_GyroFFT::calculate_filtered_noise(FrequencyPeak target_peak, FrequencyPeak source_peak, const FrequencyData& freqs, const EngineConfig& config)
{
    ...
    AG_HAL::DSP::FrequencyPeakData* peak_data = &_state->_peak_data[source_peak];
...
void AG_GyroFFT::update_snr_values(const FrequencyData& freqs)
{
...
float AG_GyroFFT::MedianLowPassFilter3dFloat::apply(uint8_t axis, float sample)
{
...
AG_GyroFFT::FrequencyData::FrequencyData(const AG_GyroFFT& gyrofft, const EngineConfig& config)
{
...
// calculate noise frequencies from FFT data provided by the HAL subsystem
bool AG_GyroFFT::get_weighted_frequency(FrequencyPeak peak, float& weighted_peak_freq_hz, float& snr, const EngineConfig& config) const
{
    AG_HAL::DSP::FrequencyPeakData* peak_data = &_state->_peak_data[peak];
...
void AG_GyroFFT::find_distance_matrix(DistanceMatrix& distance_matrix, const FrequencyData& freqs, const EngineConfig& config) const
{
...
AG_GyroFFT::FrequencyPeak AG_GyroFFT::find_closest_peak(const FrequencyPeak target, const DistanceMatrix& distance_matrix, uint8_t ignore) const
{
...
// called from FFT thread
void AG_GyroFFT::update_ref_energy(uint16_t max_bin)
{
...
// called from main thread
float AG_GyroFFT::self_test_bin_frequencies()
{
...
// called from main thread
float AG_GyroFFT::self_test(float frequency, FloatBuffer& test_window)
{
    test_window.clear();
    for(uint16_t i = 0; i < _state->_window_size; i++) {
        if (!test_window.push(sinf(2.0f * M_PI * frequency * i / _fft_sampling_rate_hz) * ToRad(20) * 2000)) {
            AG_HAL::panic("Could not create FFT test window");
        }
    }
...
// singleton instance
AG_GyroFFT *AG_GyroFFT::_singleton;

namespace AP {

AG_GyroFFT *fft()
{
    return AG_GyroFFT::get_singleton();
}
