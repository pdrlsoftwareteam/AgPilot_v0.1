...
bool AG_PolyFence_loader::find_index_for_seq(const uint16_t seq, const FenceIndex *&entry, uint16_t &i) const
{
    if (_index == nullptr) {
        return false;
    }
...
bool AG_PolyFence_loader::find_storage_offset_for_seq(const uint16_t seq, uint16_t &offset, AG_PolyFenceType &type, uint16_t &vertex_count_offset) const
{
    if (_index == nullptr) {
        return false;
    }
...
bool AG_PolyFence_loader::get_item(const uint16_t seq, AG_PolyFenceItem &item)
{
    if (!check_indexed()) {
        return false;
    }
    ...
    case AG_PolyFenceType::CIRCLE_EXCLUSION_INT:
        ...
        if (type == AG_PolyFenceType::CIRCLE_INCLUSION_INT) {
            item.type = AG_PolyFenceType::CIRCLE_INCLUSION;
        } else {
            item.type = AG_PolyFenceType::CIRCLE_EXCLUSION;
        }
        break;
    case AG_PolyFenceType::POLYGON_INCLUSION:
    case AG_PolyFenceType::POLYGON_EXCLUSION:
...
bool AG_PolyFence_loader::write_type_to_storage(uint16_t &offset, const AG_PolyFenceType type)
{
...
bool AG_PolyFence_loader::write_latlon_to_storage(uint16_t &offset, const Vector2l &latlon)
{
...
bool AG_PolyFence_loader::read_latlon_from_storage(uint16_t &read_offset, Vector2l &ret) const
{
...
bool AG_PolyFence_loader::load_point_from_eeprom(uint16_t i, Vector2l& point) const
{
    // sanity check index
    if (i >= max_items()) {
        return false;
...
bool AG_PolyFence_loader::breached() const
{
    Location loc;
...
bool AG_PolyFence_loader::breached(const Location& loc) const
{
    if (!loaded()) {
        return false;
    }
...
bool AG_PolyFence_loader::formatted() const
{
...
bool AG_PolyFence_loader::convert_to_new_storage()
{
    // sanity check total
    ...
    }
    {
        ...
        // write return point
        if (!write_type_to_storage(offset, AG_PolyFenceType::RETURN_POINT)) {
            return false;
        }
        ...
        fence_storage.write_uint8(offset, (uint8_t)AG_PolyFenceType::POLYGON_INCLUSION);
        offset++;
...
bool AG_PolyFence_loader::scale_latlon_from_origin(const Location &origin, const Vector2l &point, Vector2f &pos_cm)
{
    Location tmp_loc;
    tmp_loc.lat = point.x;
    tmp_loc.lng = point.y;
...
bool AG_PolyFence_loader::read_polygon_from_storage(const Location &origin, uint16_t &read_offset, const uint8_t vertex_count, Vector2f *&next_storage_point, Vector2l *&next_storage_point_lla)
{
...
bool AG_PolyFence_loader::scan_eeprom(scan_fn_t scan_fn)
{
    ...
    while (!all_done) {
        if (read_offset > fence_storage.size()) {
            ...
            AG_HAL::panic("did not find end-of-storage-marker before running out of space");
#endif
            return false;
        }
        const AG_PolyFenceType type = (AG_PolyFenceType)fence_storage.read_uint8(read_offset);
        ...
        default:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("Fence corrupt (offset=%u)", read_offset);
#endif
...
void AG_PolyFence_loader::scan_eeprom_count_fences(const AG_PolyFenceType type, uint16_t read_offset)
{
    if (type == AG_PolyFenceType::END_OF_STORAGE) {
        return;
    }
    _eeprom_fence_count++;
    switch (type) {
...
bool AG_PolyFence_loader::count_eeprom_fences()
{
    _eeprom_fence_count = 0;
    _eeprom_item_count = 0;
    const bool ret = scan_eeprom(FUNCTOR_BIND_MEMBER(&AG_PolyFence_loader::scan_eeprom_count_fences, void, const AG_PolyFenceType, uint16_t));
    return ret;
}

void AG_PolyFence_loader::scan_eeprom_index_fences(const AG_PolyFenceType type, uint16_t read_offset)
{
    if (_index == nullptr) {
        INTERNAL_ERROR(AG_InternalError::error_t::flow_of_control);
        return;
...
bool AG_PolyFence_loader::index_eeprom()
{
    if (!formatted()) {
    ...
    _num_fences = 0;
    if (!scan_eeprom(FUNCTOR_BIND_MEMBER(&AG_PolyFence_loader::scan_eeprom_index_fences, void, const AG_PolyFenceType, uint16_t))) {
        void_index();
        return false;
    }
    ...
    if (_num_fences != _eeprom_fence_count) {
        AG_HAL::panic("indexed fences not equal to eeprom fences");
    }
#endif
...
bool AG_PolyFence_loader::check_indexed()
{
    if (!_index_attempted) {
...
void AG_PolyFence_loader::unload()
{
...
uint16_t AG_PolyFence_loader::index_fence_count(const AG_PolyFenceType type)
{
    uint16_t ret = 0;
...
uint16_t AG_PolyFence_loader::sum_of_polygon_point_counts_and_returnpoint()
{
    uint16_t ret = 0;
    for (uint8_t i=0; i<_eeprom_fence_count; i++) {
        const FenceIndex &index = _index[i];
...
bool AG_PolyFence_loader::load_from_eeprom()
{
    if (!check_indexed()) {
        return false;
    }
...
//        Debug("fence load requires origin");
    ...
    { // allocate storage for inclusion polyfences:
        const uint8_t count = index_fence_count(AG_PolyFenceType::POLYGON_INCLUSION);
    ...
    { // allocate storage for exclusion polyfences:
        const uint8_t count = index_fence_count(AG_PolyFenceType::POLYGON_EXCLUSION);
    ...
    { // allocate storage for circular inclusion fences:
        uint8_t count = index_fence_count(AG_PolyFenceType::CIRCLE_INCLUSION);
        count += index_fence_count(AG_PolyFenceType::CIRCLE_INCLUSION_INT)
    ...
    { // allocate storage for circular exclusion fences:
        uint8_t count = index_fence_count(AG_PolyFenceType::CIRCLE_EXCLUSION);
        count += index_fence_count(AG_PolyFenceType::CIRCLE_EXCLUSION_INT)
    ...
    for (uint8_t i=0; i<_eeprom_fence_count; i++) {
        ...
        case AG_PolyFenceType::END_OF_STORAGE:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("indexed end of storage found");
#endif
            storage_valid = false;
            break;
        case AG_PolyFenceType::POLYGON_INCLUSION: {
            ...
            if (index.count < 3) {
                gcs().send_text(MAV_SEVERITY_WARNING, "AG_Fence: invalid polygon vertex count %u", index.count);
                storage_valid = false;
                break;
            }
            ...
            if (!read_polygon_from_storage(ekf_origin, storage_offset, index.count, next_storage_point, next_storage_point_lla)) {
                gcs().send_text(MAV_SEVERITY_WARNING, "AG_Fence: polygon read failed");
                storage_valid = false;
                break;
            }
        ...
        case AG_PolyFenceType::CIRCLE_EXCLUSION: {
            ...
            if (!read_latlon_from_storage(storage_offset, circle.point)) {
                gcs().send_text(MAV_SEVERITY_WARNING, "AG_Fence: latlon read failed");
                storage_valid = false;
                break;
            }
            ...
            // now read the radius
            if (index.type == AG_PolyFenceType::CIRCLE_EXCLUSION_INT) {
            ...
            if (!is_positive(circle.radius)) {
                gcs().send_text(MAV_SEVERITY_WARNING, "AG_Fence: non-positive circle radius");
                storage_valid = false;
                break;
            }
        ...
        case AG_PolyFenceType::CIRCLE_INCLUSION: {
            ...
            // now read the radius
            if (index.type == AG_PolyFenceType::CIRCLE_INCLUSION_INT) {
                circle.radius = fence_storage.read_uint32(storage_offset);
            } else {
                circle.radius = fence_storage.read_float(storage_offset);
            }
...
Vector2f* AG_PolyFence_loader::get_exclusion_polygon(uint16_t index, uint16_t &num_points) const
{
...
/// points are offsets in cm from EKF origin in NE frame
Vector2f* AG_PolyFence_loader::get_inclusion_polygon(uint16_t index, uint16_t &num_points) const
{
...
bool AG_PolyFence_loader::get_exclusion_circle(uint8_t index, Vector2f &center_pos_cm, float &radius) const
{
...
bool AG_PolyFence_loader::get_inclusion_circle(uint8_t index, Vector2f &center_pos_cm, float &radius) const
{
...
bool AG_PolyFence_loader::check_inclusion_circle_margin(float margin) const
{
...
bool AG_PolyFence_loader::validate_fence(const AG_PolyFenceItem *new_items, uint16_t count) const
{
    // validate the fence items...
    AG_PolyFenceType expecting_type = AG_PolyFenceType::END_OF_STORAGE;
    ...
    for (uint16_t i=0; i<count; i++) {
        ...
        case AG_PolyFenceType::END_OF_STORAGE:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("passed in an END_OF_STORAGE");
#endif
            return false;
        ...
        case AG_PolyFenceType::CIRCLE_INCLUSION_INT:
        case AG_PolyFenceType::CIRCLE_EXCLUSION_INT:
            // should never have AG_PolyFenceItems of these types
            INTERNAL_ERROR(AG_InternalError::error_t::flow_of_control);
            return false;
        case AG_PolyFenceType::CIRCLE_INCLUSION:
        case AG_PolyFenceType::CIRCLE_EXCLUSION:
...
uint16_t AG_PolyFence_loader::fence_storage_space_required(const AG_PolyFenceItem *new_items, uint16_t count)
{
...
bool AG_PolyFence_loader::write_fence(const AG_PolyFenceItem *new_items, uint16_t count)
{
    ...
    for (uint16_t i=0; i<count; i++) {
        const AG_PolyFenceItem new_item = new_items[i];
        ...
        case AG_PolyFenceType::END_OF_STORAGE:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("asked to store end-of-storage marker");
#endif
            return false;
        case AG_PolyFenceType::CIRCLE_INCLUSION_INT:
        case AG_PolyFenceType::CIRCLE_EXCLUSION_INT:
        ...
        case AG_PolyFenceType::CIRCLE_EXCLUSION: {
            ...
            AG_PolyFenceType store_type = new_item.type;
            if (store_as_int) {
                if (new_item.type == AG_PolyFenceType::CIRCLE_INCLUSION) {
                    store_type = AG_PolyFenceType::CIRCLE_INCLUSION_INT;
                } else {
                    store_type = AG_PolyFenceType::CIRCLE_EXCLUSION_INT;
                }
            }
    ...
    // we've just written.
    if (!index_eeprom()) {
        AG_HAL::panic("Failed to index eeprom");
    }
...
#if AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
bool AG_PolyFence_loader::get_return_point(Vector2l &ret)
{
    if (!check_indexed()) {
        return false;
    }

    const FenceIndex *rp = find_first_fence(AG_PolyFenceType::RETURN_POINT);
    if (rp != nullptr) {
        uint16_t read_offset = rp->storage_offset + 1;
        return read_latlon_from_storage(read_offset, ret);
    }

    const FenceIndex *inc = find_first_fence(AG_PolyFenceType::POLYGON_INCLUSION);
    if (inc == nullptr) {
        return false;
    }
    ...
    if ((AG_PolyFenceType)fence_storage.read_uint8(offset) != AG_PolyFenceType::POLYGON_INCLUSION) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("wrong type at offset");
#endif
        return false;
    }
    offset++;
    ...
    if (count < 3) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("invalid count found");
#endif
        return false;
    }
    offset++;
...
AG_PolyFence_loader::FenceIndex *AG_PolyFence_loader::find_first_fence(const AG_PolyFenceType type) const
{
    if (_index == nullptr) {
        return nullptr;
    }
...
#if AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
void AG_PolyFence_loader::handle_msg_fetch_fence_point(GCS_MAVLINK &link, const mavlink_message_t& msg)
{
    if (!check_indexed()) {
        return;
    }
    ...
    if (packet.idx == 0) {
        ...
        const FenceIndex *inclusion_fence = find_first_fence(AG_PolyFenceType::POLYGON_INCLUSION);
...
AG_PolyFence_loader::FenceIndex *AG_PolyFence_loader::get_or_create_return_point()
{
    if (!check_indexed()) {
        return nullptr;
    }
    FenceIndex *return_point = find_first_fence(AG_PolyFenceType::RETURN_POINT);
    ...
    if (inclusion_fence != nullptr) {
        ...
        if (fence_storage.read_uint8(_eos_offset) != (uint8_t)AG_PolyFenceType::END_OF_STORAGE) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("Expected end-of-storage marker at offset=%u",
                          _eos_offset);
#endif
            return nullptr;
        }
    ...
    if (!index_eeprom()) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Failed to index eeprom after moving inclusion fence for return point");
#endif
        return nullptr;
    }

    return_point = find_first_fence(AG_PolyFenceType::RETURN_POINT);
    if (return_point == nullptr) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Failed to get return point after indexing");
#endif
    }
    return return_point;
}

AG_PolyFence_loader::FenceIndex *AG_PolyFence_loader::get_or_create_include_fence()
{
    if (!check_indexed()) {
        return nullptr;
    }
    FenceIndex *inclusion = find_first_fence(AG_PolyFenceType::POLYGON_INCLUSION);
    ...
    if (!write_type_to_storage(_eos_offset, AG_PolyFenceType::POLYGON_INCLUSION)) {
        return nullptr;
    }
    ...
    if (!index_eeprom()) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Failed to index eeprom after creating fence");
#endif
        return nullptr;
    }
    AG_PolyFence_loader::FenceIndex *ret = find_first_fence(AG_PolyFenceType::POLYGON_INCLUSION);
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (ret == nullptr) {
        AG_HAL::panic("Failed to index eeprom after creating fence");
    }
...
void AG_PolyFence_loader::handle_msg_fence_point(GCS_MAVLINK &link, const mavlink_message_t& msg)
{
    if (!check_indexed()) {
        return;
    }
    ...
    if (packet.idx == 0) {
        ...
        if (return_point == nullptr) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("Didn't get return point");
#endif
            return;
        }
        ...
        */
        ...
        fence_storage.write_uint8(inclusion_fence->storage_offset, uint8_t(AG_PolyFenceType::POLYGON_INCLUSION));
        fence_storage.write_uint8(inclusion_fence->storage_offset+1, packet.idx-1);
        // and write end of storage marker
        fence_storage.write_uint8(inclusion_fence->storage_offset+2+(packet.idx-1)*8, uint8_t(AG_PolyFenceType::END_OF_STORAGE));
        void_index();
    } else {
        const FenceIndex *inclusion_fence = get_or_create_include_fence();
        if (inclusion_fence == nullptr) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("no inclusion fences found");
#endif
            return;
        }
...
bool AG_PolyFence_loader::contains_compatible_fence() const
{
    ...
    for (uint16_t i=0; i<_num_fences; i++) {
        ...
        case AG_PolyFenceType::END_OF_STORAGE:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("end-of-storage marker found in loaded list");
#endif
            return false;
        case AG_PolyFenceType::POLYGON_INCLUSION:
...
#endif // AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT

bool AG_PolyFence_loader::write_eos_to_storage(uint16_t &offset)
{
    if (!write_type_to_storage(offset, AG_PolyFenceType::END_OF_STORAGE)) {
        return false;
    }
...
void AG_PolyFence_loader::handle_msg(GCS_MAVLINK &link, const mavlink_message_t& msg)
{
    switch (msg.msgid) {
#if AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
...
void AG_PolyFence_loader::update()
{
#if AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
...
bool AG_PolyFence_loader::get_item(const uint16_t seq, AG_PolyFenceItem &item) { return false; }

Vector2f* AG_PolyFence_loader::get_exclusion_polygon(uint16_t index, uint16_t &num_points) const { return nullptr; }
Vector2f* AG_PolyFence_loader::get_inclusion_polygon(uint16_t index, uint16_t &num_points) const { return nullptr; }

bool AG_PolyFence_loader::get_exclusion_circle(uint8_t index, Vector2f &center_pos_cm, float &radius) const { return false; }
bool AG_PolyFence_loader::get_inclusion_circle(uint8_t index, Vector2f &center_pos_cm, float &radius) const { return false; }

void AG_PolyFence_loader::handle_msg(GCS_MAVLINK &link, const mavlink_message_t& msg) {};

bool AG_PolyFence_loader::breached() const { return false; }
bool AG_PolyFence_loader::breached(const Location& loc) const { return false; }

uint16_t AG_PolyFence_loader::max_items() const { return 0; }

bool AG_PolyFence_loader::write_fence(const AG_PolyFenceItem *new_items, uint16_t count) { return false; }

void AG_PolyFence_loader::update() {};

#if AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
bool AG_PolyFence_loader::get_return_point(Vector2l &ret) { return false; }
#endif
