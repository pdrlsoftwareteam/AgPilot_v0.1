...
// and out of the polyfence loader.  It uses a AG_PolyFenceType to
...
#define AG_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT 1
...
public:
    ...
    AG_PolyFence_loader(AP_Int8 &total) :
        _total(total) {}

    /* Do not allow copies */
    CLASS_NO_COPY(AG_PolyFence_loader);
    ...
    bool get_item(const uint16_t seq, AG_PolyFenceItem &item) WARN_IF_UNUSED;
    ...
    bool write_fence(const AG_PolyFenceItem *new_items, uint16_t count)  WARN_IF_UNUSED;
...
private:
    ...
    uint16_t index_fence_count(const AG_PolyFenceType type);
    ...
    FenceIndex *find_first_fence(const AG_PolyFenceType type) const;
    ...
    bool find_storage_offset_for_seq(const uint16_t seq, uint16_t &offset, AG_PolyFenceType &type, uint16_t &vertex_count_offset) const WARN_IF_UNUSED;
    ...
    bool validate_fence(const AG_PolyFenceItem *new_items, uint16_t count) const WARN_IF_UNUSED;
    ...
    bool write_type_to_storage(uint16_t &offset, AG_PolyFenceType type) WARN_IF_UNUSED;
    ...
    FUNCTOR_TYPEDEF(scan_fn_t, void, const AG_PolyFenceType, uint16_t);
    ...
    void scan_eeprom_count_fences(const AG_PolyFenceType type, uint16_t read_offset);
    ...
    void scan_eeprom_index_fences(const AG_PolyFenceType type, uint16_t read_offset);
    ...
    uint16_t fence_storage_space_required(const AG_PolyFenceItem *new_items, uint16_t count);
};
