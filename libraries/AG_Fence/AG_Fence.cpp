...
// default boundaries
#define AC_FENCE_ALT_MAX_DEFAULT                    65.0f  // default max altitude is 100m
...
const AG_Param::GroupInfo AG_Fence::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("TYPE",        1,  AG_Fence,   _enabled_fences,  AC_FENCE_TYPE_DEFAULT),
    ...
    // @User: Standard
    AP_GROUPINFO("ACTION",      2,  AG_Fence,   _action,        AC_FENCE_ACTION_RTL_AND_LAND),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO_FRAME("ALT_MAX", 3, AG_Fence, _alt_max, AC_FENCE_ALT_MAX_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_SUB | AP_PARAM_FRAME_TRICOPTER | AP_PARAM_FRAME_PLANE),
    ...
    // @Range: 30 10000
    // @User: Standard
    AP_GROUPINFO("RADIUS",      4,  AG_Fence,   _circle_radius, AC_FENCE_CIRCLE_RADIUS_DEFAULT),
    ...
    // @Range: 1 10
    // @User: Standard
    AP_GROUPINFO("MARGIN",      5,  AG_Fence,   _margin, AC_FENCE_MARGIN_DEFAULT),
    ...
    // @Range: 1 20
    // @User: Standard
    AP_GROUPINFO("TOTAL",       6,  AG_Fence,   _total, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO_FRAME("ALT_MIN",     7,  AG_Fence,   _alt_min,       AC_FENCE_ALT_MIN_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_SUB | AP_PARAM_FRAME_TRICOPTER | AP_PARAM_FRAME_PLANE),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO_FRAME("RET_RALLY",   8,  AG_Fence,   _ret_rally,       0, AP_PARAM_FRAME_PLANE),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO_FRAME("RET_ALT",   9,  AG_Fence,   _ret_altitude,       0.0f, AP_PARAM_FRAME_PLANE),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO_FRAME("AUTOENABLE", 10, AG_Fence, _auto_enabled, static_cast<uint8_t>(AutoEnable::ALWAYS_DISABLED), AP_PARAM_FRAME_PLANE),
    ...
    // @User: Standard
    AP_GROUPINFO_FRAME("OPTIONS", 11, AG_Fence, _options, static_cast<uint16_t>(OPTIONS::DISABLE_MODE_CHANGE), AP_PARAM_FRAME_PLANE),
...
AG_Fence::AG_Fence()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("Fence must be singleton");
    }
#endif
    _singleton = this;
...
void AG_Fence::enable(bool value)
{
    if (_enabled && !value) {
...
*/
void AG_Fence::auto_enable_fence_after_takeoff(void)
{
    switch(auto_enabled()) {
        case AG_Fence::AutoEnable::ALWAYS_ENABLED:
        case AG_Fence::AutoEnable::ENABLE_DISABLE_FLOOR_ONLY:
            enable(true);
 ...
 */
void AG_Fence::auto_disable_fence_for_landing(void)
{
    switch (auto_enabled()) {
        case AG_Fence::AutoEnable::ALWAYS_ENABLED:
            enable(false);
...
uint8_t AG_Fence::get_enabled_fences() const
{
...
bool AG_Fence::pre_arm_check_polygon(const char* &fail_msg) const
{
...
bool AG_Fence::pre_arm_check_circle(const char* &fail_msg) const
{
    if (_circle_radius < 0) {
...
bool AG_Fence::pre_arm_check_alt(const char* &fail_msg) const
{
    if (_alt_max < 0.0f) {
...
bool AG_Fence::pre_arm_check(const char* &fail_msg) const
{
    fail_msg = nullptr;
...
bool AG_Fence::check_fence_alt_max()
{
...
/// fence; also may set up a fallback fence which, if breached, will
/// cause the altitude fence to be freshly breached
bool AG_Fence::check_fence_alt_min()
{
    // altitude fence check
...
// inclusions zones
bool AG_Fence::check_fence_polygon()
{
...
/// fence is breaced.
bool AG_Fence::check_fence_circle()
{
    if (!(_enabled_fences & AC_FENCE_TYPE_CIRCLE)) {
        // not enabled; no breach
        return false;
...
uint8_t AG_Fence::check()
{
    ...
    if (_manual_recovery_start_ms != 0) {
        ...
        if ((AG_HAL::millis() - _manual_recovery_start_ms) < AC_FENCE_MANUAL_RECOVERY_TIME_MIN) {
            return 0;
        }
...
bool AG_Fence::check_destination_within_fence(const Location& loc)
{
...
void AG_Fence::record_breach(uint8_t fence_type)
{
...
void AG_Fence::clear_breach(uint8_t fence_type)
{
    _breached_fences &= ~fence_type;
}
...
float AG_Fence::get_breach_distance(uint8_t fence_type) const
{
    float max = 0.0f;
...
void AG_Fence::manual_recovery_start()
{
    ...
    _manual_recovery_start_ms = AG_HAL::millis();
}

// methods for mavlink SYS_STATUS message (send_sys_status)
bool AG_Fence::sys_status_present() const
{
    return present();
}

bool AG_Fence::sys_status_enabled() const
{
...
bool AG_Fence::sys_status_failed() const
{
    if (!sys_status_present()) {
...
AG_PolyFence_loader &AG_Fence::polyfence()
{
    return _poly_loader;
}
const AG_PolyFence_loader &AG_Fence::polyfence() const
{
    return _poly_loader;
}
...
const AG_Param::GroupInfo AG_Fence::var_info[] = { AP_GROUPEND };
...
void AG_Fence::auto_enable_fence_after_takeoff() {};
void AG_Fence::auto_disable_fence_for_landing() {};

bool AG_Fence::present() const { return false; }

uint8_t AG_Fence::get_enabled_fences() const { return 0; }

bool AG_Fence::pre_arm_check(const char* &fail_msg) const  { return true; }

uint8_t AG_Fence::check() { return 0; }
bool AG_Fence::check_destination_within_fence(const Location& loc) { return true; }
float AG_Fence::get_breach_distance(uint8_t fence_type) const { return 0.0; }

void AG_Fence::manual_recovery_start() {}

bool AG_Fence::sys_status_present() const { return false; }
bool AG_Fence::sys_status_enabled() const { return false; }
bool AG_Fence::sys_status_failed() const { return false; }
