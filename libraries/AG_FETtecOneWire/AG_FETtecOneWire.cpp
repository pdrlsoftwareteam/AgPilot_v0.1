...
const AG_Param::GroupInfo AG_FETtecOneWire::var_info[] {
    ...
    // @RebootRequired: True
    // @User: Standard
    AP_GROUPINFO_FLAGS("MASK",  1, AG_FETtecOneWire, _motor_mask_parameter, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Bitmask: 0:SERVO1,1:SERVO2,2:SERVO3,3:SERVO4,4:SERVO5,5:SERVO6,6:SERVO7,7:SERVO8,8:SERVO9,9:SERVO10,10:SERVO11,11:SERVO12
    // @User: Standard
    AP_GROUPINFO("RVMASK",  2, AG_FETtecOneWire, _reverse_mask_parameter, 0),
    ...
    // @Range: 2 50
    // @User: Standard
    AP_GROUPINFO("POLES", 3, AG_FETtecOneWire, _pole_count_parameter, 14),
#endif

    AP_GROUPEND
};

AG_FETtecOneWire *AG_FETtecOneWire::_singleton;

AG_FETtecOneWire::AG_FETtecOneWire()
{
    AG_Param::setup_object_defaults(this, var_info);

#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_FETtecOneWire must be singleton");
    }
#endif
    _singleton = this;
}
...
*/
void AG_FETtecOneWire::init_uart()
{
    if (_uart != nullptr) {
        return;
    }
    const AG_SerialManager& serial_manager = AP::serialmanager();
    _uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_FETtecOneWire, 0);
    if (_uart == nullptr) {
...
*/
bool AG_FETtecOneWire::transmit(const uint8_t* bytes, const uint8_t length)
{
...
*/
bool AG_FETtecOneWire::transmit_config_request(const uint8_t* bytes, const uint8_t length)
{
...
void AG_FETtecOneWire::move_frame_source_in_receive_buffer(const uint8_t search_start_pos)
{
    uint8_t i;
...
void AG_FETtecOneWire::consume_bytes(const uint8_t n)
{
    if (n == 0) {
        return;
    }
...
bool AG_FETtecOneWire::buffer_contains_ok(const uint8_t length)
{
...
void AG_FETtecOneWire::handle_message(ESC &esc, const uint8_t length)
{
...
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            AG_HAL::panic("unexpected telemetry");
#endif
            return;
        }
...
#if HAL_WITH_ESC_TELEM
void AG_FETtecOneWire::handle_message_telem(ESC &esc)
{
    // the following two methods are coming from AG_ESC_Telem:
    ...
    esc.last_telem_us = AG_HAL::micros();
}
#endif  // HAL_WITH_ESC_TELEM

// reads data from the UART, calling handle_message on any message found
void AG_FETtecOneWire::read_data_from_uart()
{
    /*
    a frame looks like:
...
*/
void AG_FETtecOneWire::pack_fast_throttle_command(const uint16_t *motor_values, uint8_t *fast_throttle_command, const uint8_t length, const uint8_t esc_id_to_request_telem_from)
{
    // byte 1:
...
void AG_FETtecOneWire::escs_set_values(const uint16_t* motor_values)
{
...
bool AG_FETtecOneWire::pre_arm_check(char *failure_msg, const uint8_t failure_msg_len) const
{
...
void AG_FETtecOneWire::configure_escs()
{
    if (_uart->available()) {
...
*/
void AG_FETtecOneWire::beep(const uint8_t beep_frequency)
{
    for (uint8_t i=0; i<_esc_count; i++) {
        auto &esc = _escs[i];
        if (esc.state != ESCState::RUNNING) {
...
/**
    ...
    b = blue brightness
*/
void AG_FETtecOneWire::led_color(const uint8_t r, const uint8_t g, const uint8_t b)
{
    for (uint8_t i=0; i<_esc_count; i++) {
        auto &esc = _escs[i];
        if (esc.state != ESCState::RUNNING) {
