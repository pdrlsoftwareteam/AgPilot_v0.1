...
#endif
            gcs().send_text(AGPILOT_SEVERITY_WARNING, "Fence corrupt");
            return false;
        }
...
//        Debug("fence load requires origin");
    ...
    for (uint8_t i=0; i<_eeprom_fence_count; i++) {
        ...
        case AC_PolyFenceType::POLYGON_INCLUSION: {
            ...
            if (index.count < 3) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "AC_Fence: invalid polygon vertex count %u", index.count);
                storage_valid = false;
                break;
            }
            ...
            if (!read_polygon_from_storage(ekf_origin, storage_offset, index.count, next_storage_point, next_storage_point_lla)) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "AC_Fence: polygon read failed");
                storage_valid = false;
                break;
            }
        ...
        case AC_PolyFenceType::CIRCLE_EXCLUSION: {
            ...
            if (!read_latlon_from_storage(storage_offset, circle.point)) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "AC_Fence: latlon read failed");
                storage_valid = false;
                break;
            }
            ...
            if (!is_positive(circle.radius)) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "AC_Fence: non-positive circle radius");
                storage_valid = false;
                break;
            }
        ...
        case AC_PolyFenceType::RETURN_POINT:
            if (_loaded_return_point != nullptr) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "PolyFence: Multiple return points found");
                storage_valid = false;
                break;
            }
            ...
            if (!read_latlon_from_storage(storage_offset, *next_storage_point_lla)) {
                storage_valid = false;
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "PolyFence: latlon read failed");
                break;
            }
...
bool AC_PolyFence_loader::validate_fence(const AC_PolyFenceItem *new_items, uint16_t count) const
{
    ...
    for (uint16_t i=0; i<count; i++) {
        ...
        case AC_PolyFenceType::POLYGON_EXCLUSION:
            if (new_items[i].vertex_count < 3) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "Invalid vertex count (%u)", new_items[i].vertex_count);
                return false;
            }
            if (expected_type_count == 0) {
                ...
                if (new_items[i].type != expecting_type) {
                    gcs().send_text(AGPILOT_SEVERITY_WARNING, "Received incorrect vertex type (want=%u got=%u)", (unsigned)expecting_type, (unsigned)new_items[i].type);
                    return false;
                } else if (new_items[i].vertex_count != orig_expected_type_count) {
                    gcs().send_text(AGPILOT_SEVERITY_WARNING, "Unexpected vertex count want=%u got=%u\n", orig_expected_type_count, new_items[i].vertex_count);
                    return false;
                }
            }
            expected_type_count--;
        ...
        case AC_PolyFenceType::CIRCLE_EXCLUSION:
            if (expected_type_count) {
               gcs().send_text(AGPILOT_SEVERITY_WARNING, "Received incorrect type (want=%u got=%u)", (unsigned)expecting_type, (unsigned)new_items[i].type);
               return false;
            }
            if (!is_positive(new_items[i].radius)) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "Non-positive circle radius");
                return false;
            }
            validate_latlon = true;
            break;
        ...
        case AC_PolyFenceType::RETURN_POINT:
            ...
            if (seen_return_point) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "Multiple return points");
                return false;
            }
        ...
        if (validate_latlon) {
            if (!check_latlng(new_items[i].loc[0], new_items[i].loc[1])) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "Bad lat or lon");
                return false;
            }
        }
    }
    ...
    if (expected_type_count) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Incorrect item count");
        return false;
    }
...
bool AC_PolyFence_loader::write_fence(const AC_PolyFenceItem *new_items, uint16_t count)
{
    if (!validate_fence(new_items, count)) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "Fence validation failed");
        return false;
    }

    if (fence_storage_space_required(new_items, count) > fence_storage.size()) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "Fence exceeds storage size");
        return false;
    }
    ...
    gcs().send_text(AGPILOT_SEVERITY_DEBUG, "Fence Indexed OK");
#endif
...
#if AC_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
void AC_PolyFence_loader::handle_msg_fetch_fence_point(GCS_AGPILOTLINK &link, const mavlink_message_t& msg)
{
    if (!check_indexed()) {
        return;
    }
    if (!contains_compatible_fence()) {
        link.send_text(AGPILOT_SEVERITY_WARNING, "Vehicle contains advanced fences");
        return;
    }

    if (_total != 0 && _total < 5) {
        link.send_text(AGPILOT_SEVERITY_WARNING, "Invalid FENCE_TOTAL");
        return;
    }
    ...
    if (packet.idx >= _total) {
        link.send_text(AGPILOT_SEVERITY_WARNING, "Invalid fence point, index past total(%u >= %u)", packet.idx, _total.get());
        return;
    }
    ...
    if (packet.idx == 0) {
        ...
        if (get_return_point(ret)) {
            ...
            link.send_text(AGPILOT_SEVERITY_WARNING, "Failed to get return point");
        }
    } else {
        ...
        if (inclusion_fence == nullptr) {
            ...
            if (fencepoint_offset >= inclusion_fence->count) {
                ...
                if (!read_latlon_from_storage(storage_offset, bob)) {
                    link.send_text(AGPILOT_SEVERITY_WARNING, "Fence read failed");
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    ...
    link.send_message(AGPILOTLINK_MSG_ID_FENCE_POINT, (const char*)&ret_packet);
}
...
void AC_PolyFence_loader::handle_msg_fence_point(GCS_AGPILOTLINK &link, const mavlink_message_t& msg)
{
    if (!check_indexed()) {
        return;
    }
    ...
    if (packet.count != _total) {
        link.send_text(AGPILOT_SEVERITY_WARNING, "Invalid fence point, bad count (%u vs %u)", packet.count, _total.get());
        return;
    }
    ...
    if (!check_latlng(packet.lat, packet.lng)) {
        link.send_text(AGPILOT_SEVERITY_WARNING, "Invalid fence point, bad lat or lng");
        return;
    }
    ...
    if (packet.idx == 0) {
        ...
        if (!write_latlon_to_storage(offset, point)) {
            link.send_text(AGPILOT_SEVERITY_WARNING, "PolyFence: storage write failed");
            return;
        }
...
void AC_PolyFence_loader::handle_msg(GCS_AGPILOTLINK &link, const mavlink_message_t& msg)
{
    switch (msg.msgid) {
#if AC_POLYFENCE_FENCE_POINT_PROTOCOL_SUPPORT
