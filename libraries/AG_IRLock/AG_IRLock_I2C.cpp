 ...
 * AG_IRLock_I2C.cpp
 *
 * Based on AG_IRLock_PX4 by MLandes
 *
...
void AG_IRLock_I2C::init(int8_t bus)
{
    if (bus < 0) {
    ...
    dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&AG_IRLock_I2C::read_frames, void));
}
...
/*
   ...
   a frame
*/
bool AG_IRLock_I2C::sync_frame_start(void)
{
    uint32_t sync_word;
 ...
 */
void AG_IRLock_I2C::pixel_to_1M_plane(float pix_x, float pix_y, float &ret_x, float &ret_y)
{
...
*/
bool AG_IRLock_I2C::read_block(struct frame &irframe)
{
...
void AG_IRLock_I2C::read_frames(void)
{
    if (!sync_frame_start()) {
        return;
    }
...

    {
        ...
        /* convert to angles */
        _target_info.timestamp = AG_HAL::millis();
...
bool AG_IRLock_I2C::update()
{
    ...
    _flags.healthy = (AG_HAL::millis() - _last_read_ms < 100);
