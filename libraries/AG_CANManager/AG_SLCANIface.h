...
class CANIface: public AG_HAL::CANIface
{

    int16_t reportFrame(const AG_HAL::CANFrame& frame, uint64_t timestamp_usec);

    const char* processCommand(char* cmd);

    // pushes received frame into queue, false if failed
    bool push_Frame(AG_HAL::CANFrame &frame);
    ...
    AG_HAL::UARTDriver* _port; // UART interface port reference to be used for SLCAN iface

    ObjectBuffer<AG_HAL::CANIface::CanRxItem> rx_queue_; // Parsed Rx Frame queue
    ...
    uint8_t num_tries;
    AG_HAL::CANIface* _can_iface; // Can interface to be used for interaction by SLCAN interface
...
public:
    ...
    void set_can_iface(AG_HAL::CANIface* can_iface)
    {
    ...
    // Overriden methods
    bool set_event_handle(AG_HAL::EventHandle* evt_handle) override;
    ...
    bool select(bool &read, bool &write,
                const AG_HAL::CANFrame* const pending_tx,
                uint64_t blocking_deadline) override;
    int16_t send(const AG_HAL::CANFrame& frame, uint64_t tx_deadline,
                 AG_HAL::CANIface::CanIOFlags flags) override;

    int16_t receive(AG_HAL::CANFrame& out_frame, uint64_t& rx_time,
                    AG_HAL::CANIface::CanIOFlags& out_flags) override;
...
protected:
    ...
    bool add_to_rx_queue(const AG_HAL::CANIface::CanRxItem &frm) override {
