...
class CANTester : public AG_CANDriver
{
public:
    CANTester()
    {
...
private:
    ...
    void create_loopback_frame(loopback_stats_s &stats, AG_HAL::CANFrame& frame);
    void check_loopback_frame(loopback_stats_s &stats, AG_HAL::CANFrame& frame);
    ...
    bool write_frame(uint8_t iface, AG_HAL::CANFrame &out_frame, uint64_t timeout);

    // read frame on CAN bus, returns true on success
    bool read_frame(uint8_t iface, AG_HAL::CANFrame &recv_frame, uint64_t timeout, AG_HAL::CANIface::CanIOFlags &flags);

    void reset_frame(AG_HAL::CANFrame& frame);
