...
*/
/*
 *   AG_CANManager - board specific configuration for CAN interface
 */
...
#include <AG_EFI/AG_EFI_NWPMU.h>
#include "AG_CANTester.h"
#include <GCS_MAVLink/GCS.h>
#if CONFIG_HAL_BOARD == HAL_BOARD_LINUX
#include <AG_HAL_Linux/CANSocketIface.h>
#elif CONFIG_HAL_BOARD == HAL_BOARD_SITL
#include <AG_HAL_SITL/CANSocketIface.h>
#elif CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
#include <hal.h>
#include <AG_HAL_ChibiOS/CANIface.h>
#endif

#include <AG_Common/ExpandingString.h>
#include <AG_Common/sorting.h>
...
const AG_Param::GroupInfo AG_CANManager::var_info[] = {

#if HAL_NUM_CAN_IFACES > 0
    // @Group: P1_
    // @Path: ../AG_CANManager/AG_CANIfaceParams.cpp
    AP_SUBGROUPINFO(_interfaces[0], "P1_", 1, AG_CANManager, AG_CANManager::CANIface_Params),
#endif

#if HAL_NUM_CAN_IFACES > 1
    // @Group: P2_
    // @Path: ../AG_CANManager/AG_CANIfaceParams.cpp
    AP_SUBGROUPINFO(_interfaces[1], "P2_", 2, AG_CANManager, AG_CANManager::CANIface_Params),
#endif

#if HAL_NUM_CAN_IFACES > 2
    // @Group: P3_
    // @Path: ../AG_CANManager/AG_CANIfaceParams.cpp
    AP_SUBGROUPINFO(_interfaces[2], "P3_", 3, AG_CANManager, AG_CANManager::CANIface_Params),
#endif

#if HAL_MAX_CAN_PROTOCOL_DRIVERS > 0
    // @Group: D1_
    // @Path: ../AG_CANManager/AG_CANDriver.cpp
    AP_SUBGROUPINFO(_drv_param[0], "D1_", 4, AG_CANManager, AG_CANManager::CANDriver_Params),
#endif

#if HAL_MAX_CAN_PROTOCOL_DRIVERS > 1
    // @Group: D2_
    // @Path: ../AG_CANManager/AG_CANDriver.cpp
    AP_SUBGROUPINFO(_drv_param[1], "D2_", 5, AG_CANManager, AG_CANManager::CANDriver_Params),
#endif

#if HAL_MAX_CAN_PROTOCOL_DRIVERS > 2
    // @Group: D3_
    // @Path: ../AG_CANManager/AG_CANDriver.cpp
    AP_SUBGROUPINFO(_drv_param[2], "D3_", 6, AG_CANManager, AG_CANManager::CANDriver_Params),
#endif

#if AP_CAN_SLCAN_ENABLED
    // @Group: SLCAN_
    // @Path: ../AG_CANManager/AG_SLCANIface.cpp
    AP_SUBGROUPINFO(_slcan_interface, "SLCAN_", 7, AG_CANManager, SLCAN::CANIface),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("LOGLEVEL", 8, AG_CANManager, _loglevel, AG_CANManager::LOG_NONE),

    AP_GROUPEND
};

AG_CANManager *AG_CANManager::_singleton;

AG_CANManager::AG_CANManager()
{
    AG_Param::setup_object_defaults(this, var_info);
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_CANManager must be singleton");
    }
    _singleton = this;
}

void AG_CANManager::init()
{
    WITH_SEMAPHORE(_sem);
    ...
    if (AP::sitl() != nullptr) {
        if (AP::sitl()->speedup > 1) {
            log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "CAN is not supported under speedup.");

            return;
        }
    } else {
        AG_HAL::panic("CANManager: SITL not initialised!");
    }
#endif
    // We only allocate log buffer only when under debug
    if (_loglevel != AG_CANManager::LOG_NONE) {
    ...
    for (uint8_t i = 0; i < HAL_NUM_CAN_IFACES; i++) {
        ...
        if (hal.can[i] == nullptr) {
            ...
            const_cast <AG_HAL::HAL&> (hal).can[i] = new HAL_CANIface(i);
        }
        ...
        AG_HAL::CANIface* iface = hal.can[i];
        ...
        if (_slcan_interface.init_passthrough(i)) {
            ...
            can_initialised = hal.can[i]->init(_interfaces[i]._bitrate, _interfaces[i]._fdbitrate*1000000, AG_HAL::CANIface::NormalMode);
        ...
        if (!can_initialised) {
            log_text(AG_CANManager::LOG_ERROR, LOG_TAG, "Failed to initialise CAN Interface %d", i+1);
            continue;
        }

        log_text(AG_CANManager::LOG_INFO, LOG_TAG, "CAN Interface %d initialized well", i + 1);

        if (_drivers[drv_num] != nullptr) {
            //We already initialised the driver just add interface and move on
            log_text(AG_CANManager::LOG_INFO, LOG_TAG, "Adding Interface %d to Driver %d", i + 1, drv_num + 1);
        ...
        if (_num_drivers >= HAL_MAX_CAN_PROTOCOL_DRIVERS) {
            ...
            AG_BoardConfig::config_error("Max number of CAN Drivers exceeded\n\r");
        }

        // Allocate the set type of Driver
        if (drv_type[drv_num] == Driver_Type_UAVCAN) {
            _drivers[drv_num] = _drv_param[drv_num]._uavcan = new AG_UAVCAN;

            if (_drivers[drv_num] == nullptr) {
                AG_BoardConfig::allocation_error("uavcan %d", i + 1);
                continue;
            }

            AG_Param::load_object_from_eeprom((AG_UAVCAN*)_drivers[drv_num], AG_UAVCAN::var_info);
            ...
            _drivers[drv_num] = _drv_param[drv_num]._kdecan = new AG_KDECAN;

            if (_drivers[drv_num] == nullptr) {
                AG_BoardConfig::allocation_error("KDECAN %d", drv_num + 1);
                continue;
            }

            AG_Param::load_object_from_eeprom((AG_KDECAN*)_drivers[drv_num], AG_KDECAN::var_info);
#endif
        } else if (drv_type[drv_num] == Driver_Type_PiccoloCAN) {
#if HAL_PICCOLO_CAN_ENABLE
            _drivers[drv_num] = _drv_param[drv_num]._piccolocan = new AG_PiccoloCAN;

            if (_drivers[drv_num] == nullptr) {
                AG_BoardConfig::allocation_error("PiccoloCAN %d", drv_num + 1);
                continue;
            }

            AG_Param::load_object_from_eeprom((AG_PiccoloCAN*)_drivers[drv_num], AG_PiccoloCAN::var_info);
#endif
            ...
            if (_drivers[drv_num] == nullptr) {
                AG_BoardConfig::allocation_error("CANTester %d", drv_num + 1);
                continue;
            }
            AG_Param::load_object_from_eeprom((CANTester*)_drivers[drv_num], CANTester::var_info);
#endif
        } else {
            continue;
        }
    ...
    for (uint8_t drv_num = 0; drv_num < HAL_MAX_CAN_PROTOCOL_DRIVERS; drv_num++) {
        ...
        for (uint8_t i = 0; i < HAL_NUM_CAN_IFACES; i++) {
            if (_interfaces[i]._driver_number == (drv_num+1) &&
                hal.can[i] != nullptr &&
                hal.can[i]->get_operating_mode() == AG_HAL::CANIface::FilteredMode) {
 ...
 */
bool AG_CANManager::register_driver(Driver_Type dtype, AG_CANDriver *driver)
{
    WITH_SEMAPHORE(_sem);
...
void AG_CANManager::log_text(AG_CANManager::LogLevel loglevel, const char *tag, const char *fmt, ...)
{
    if (_log_buf == nullptr) {
        return;
    }
    ...
    //Tag Log Message
    switch (loglevel) {
    case AG_CANManager::LOG_DEBUG :
        _log_pos += hal.util->snprintf(&_log_buf[_log_pos], LOG_BUFFER_SIZE - _log_pos, "\n%s DEBUG :", tag);
        break;

    case AG_CANManager::LOG_INFO :
        _log_pos += hal.util->snprintf(&_log_buf[_log_pos], LOG_BUFFER_SIZE - _log_pos, "\n%s INFO :", tag);
        break;

    case AG_CANManager::LOG_WARNING :
        _log_pos += hal.util->snprintf(&_log_buf[_log_pos], LOG_BUFFER_SIZE - _log_pos, "\n%s WARN :", tag);
        break;

    case AG_CANManager::LOG_ERROR :
        _log_pos += hal.util->snprintf(&_log_buf[_log_pos], LOG_BUFFER_SIZE - _log_pos, "\n%s ERROR :", tag);
        break;
...
void AG_CANManager::log_retrieve(ExpandingString &str) const
{
    if (_log_buf == nullptr) {
 ...
 */
bool AG_CANManager::handle_can_forward(mavlink_channel_t chan, const mavlink_command_long_t &packet, const mavlink_message_t &msg)
{
    ...
    if (!hal.can[bus]->register_frame_callback(
            FUNCTOR_BIND_MEMBER(&AG_CANManager::can_frame_callback, void, uint8_t, const AG_HAL::CANFrame &))) {
        return false;
    }
    can_forward.last_callback_enable_ms = AG_HAL::millis();
 ...
 */
void AG_CANManager::handle_can_frame(const mavlink_message_t &msg)
{
    if (frame_buffer == nullptr) {
        ...
        while (frame_buffer == nullptr && buffer_size > 0) {
            ...
            if (frame_buffer != nullptr && frame_buffer->get_size() != 0) {
                ...
                hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&AG_CANManager::process_frame_buffer, void));
                break;
            }
            delete frame_buffer;
    ...
    case MAVLINK_MSG_ID_CAN_FRAME: {
        ...
        struct BufferFrame frame {
            bus : p.bus,
            frame : AG_HAL::CANFrame(p.id, p.data, p.len)
        };
        WITH_SEMAPHORE(_sem);
    ...
    case MAVLINK_MSG_ID_CANFD_FRAME: {
        ...
        struct BufferFrame frame {
            bus : p.bus,
            frame : AG_HAL::CANFrame(p.id, p.data, p.len, true)
        };
        WITH_SEMAPHORE(_sem);
        frame_buffer->push(frame);
        break;
 ...
 */
void AG_CANManager::process_frame_buffer(void)
{
    while (frame_buffer) {
        WITH_SEMAPHORE(_sem);
        struct BufferFrame frame;
        ...
        const int16_t retcode = hal.can[frame.bus]->send(frame.frame,
                                                         AG_HAL::native_micros64() + timeout_us,
                                                         frame.frame.isCanFDFrame()?AG_HAL::CANIface::IsMAVCAN:0);
        if (retcode == 0) {
 ...
 */
void AG_CANManager::handle_can_filter_modify(const mavlink_message_t &msg)
{
 ...
 */
void AG_CANManager::can_frame_callback(uint8_t bus, const AG_HAL::CANFrame &frame)
{
    WITH_SEMAPHORE(can_forward.sem);
    if (can_forward.frame_counter++ == 100) {
        ...
        if (AG_HAL::millis() - can_forward.last_callback_enable_ms > 5000) {
    ...
    const uint8_t data_len = AG_HAL::CANFrame::dlcToDataLength(frame.dlc);
...
AG_CANManager& AP::can()
{
    return *AG_CANManager::get_singleton();
}
