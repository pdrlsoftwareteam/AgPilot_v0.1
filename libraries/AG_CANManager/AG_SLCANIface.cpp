...
const AG_Param::GroupInfo SLCAN::CANIface::var_info[] = {
    // @Param: CPORT
...
bool SLCAN::CANIface::push_Frame(AG_HAL::CANFrame &frame)
{
    AG_HAL::CANIface::CanRxItem frm;
    frm.frame = frame;
    frm.flags = 0;
    frm.timestamp_us = AG_HAL::native_micros64();
...
bool SLCAN::CANIface::handle_FrameDataExt(const char* cmd, bool canfd)
{
    ...
    {
        const char* p = &cmd[10];
        const uint8_t dlen = AG_HAL::CANFrame::dlcToDataLength(f.dlc);
...
bool SLCAN::CANIface::handle_FDFrameDataExt(const char* cmd)
{
    ...
    f.id = f.FlagEFF |
           ...
           (hex2nibble(cmd[8]) <<  0);
    f.dlc = hex2nibble(cmd[9]);
    if (f.dlc > AG_HAL::CANFrame::dataLengthToDlc(AG_HAL::CANFrame::MaxDataLen)) {
        return false;
    }
    {
        const char* p = &cmd[10];
        for (unsigned i = 0; i < AG_HAL::CANFrame::dlcToDataLength(f.dlc); i++) {
            f.data[i] = (hex2nibble(*p) << 4) | hex2nibble(*(p + 1));
            p += 2;
        }
    }
...
bool SLCAN::CANIface::handle_FrameDataStd(const char* cmd)
{
    ...
    f.id = (hex2nibble(cmd[1]) << 8) |
           (hex2nibble(cmd[2]) << 4) |
           (hex2nibble(cmd[3]) << 0);
    if (cmd[4] < '0' || cmd[4] > ('0' + AG_HAL::CANFrame::NonFDCANMaxDataLen)) {
        return false;
    }
    f.dlc = cmd[4] - '0';
    if (f.dlc > AG_HAL::CANFrame::NonFDCANMaxDataLen) {
        return false;
    }
    {
        const char* p = &cmd[5];
...
bool SLCAN::CANIface::handle_FrameRTRExt(const char* cmd)
{
    ...
    f.id = f.FlagEFF | f.FlagRTR |
           ...
           (hex2nibble(cmd[7]) <<  4) |
           (hex2nibble(cmd[8]) <<  0);
    if (cmd[9] < '0' || cmd[9] > ('0' + AG_HAL::CANFrame::NonFDCANMaxDataLen)) {
        return false;
    }
    f.dlc = cmd[9] - '0';
...
bool SLCAN::CANIface::init_passthrough(uint8_t i)
{
    ...
    AP::can().log_text(AG_CANManager::LOG_INFO, LOG_TAG, "Setting SLCAN Passthrough for CAN%d\n", _slcan_can_port - 1);
#endif
    return true;
}
 ...
 */
int16_t SLCAN::CANIface::reportFrame(const AG_HAL::CANFrame& frame, uint64_t timestamp_usec)
{
    if (_port == nullptr) {
        return -1;
    }
    ...
    * Data
    */
    for (unsigned i = 0; i < AG_HAL::CANFrame::dlcToDataLength(frame.dlc); i++) {
...
void SLCAN::CANIface::update_slcan_port()
{
    ...
    if (_port == nullptr && !armed) {
         _port = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_SLCAN, 0);
        if (_port != nullptr) {
    ...
    if (_prev_ser_port != _slcan_ser_port) {
        if (!_slcan_start_req) {
            _slcan_start_req_time = AG_HAL::native_millis();
            _slcan_start_req = true;
        }
        if (((AG_HAL::native_millis() - _slcan_start_req_time) < ((uint32_t)_slcan_start_delay*1000))) {
            return;
        }
        ...
        _last_had_activity = AG_HAL::native_millis();
    }
    if (_port == nullptr) {
        return;
    }
    if (((AG_HAL::native_millis() - _last_had_activity) > ((uint32_t)_slcan_timeout*1000)) &&
...
bool SLCAN::CANIface::set_event_handle(AG_HAL::EventHandle* evt_handle)
{
...
bool SLCAN::CANIface::select(bool &read, bool &write, const AG_HAL::CANFrame* const pending_tx,
...
int16_t SLCAN::CANIface::send(const AG_HAL::CANFrame& frame, uint64_t tx_deadline, AG_HAL::CANIface::CanIOFlags flags)
{
    update_slcan_port();
    int16_t ret = 0;
    // When in passthrough mode select is handled through can iface
    ...
    reportFrame(frame, AG_HAL::native_micros64());
    return ret;
}

// receive method to read the frame recorded in the buffer
int16_t SLCAN::CANIface::receive(AG_HAL::CANFrame& out_frame, uint64_t& rx_time,
                                 AG_HAL::CANIface::CanIOFlags& out_flags)
{
    update_slcan_port();
    // When in passthrough mode select is handled through can iface
    if (_can_iface) {
        ...
        if (ret > 0) {
            ...
            reportFrame(out_frame, AG_HAL::native_micros64());
            return ret;
        } else if (ret < 0) {
            return ret;
        }
    ...
    if (rx_queue_.available()) {
        ...
        out_flags = frm.flags;
        _last_had_activity = AG_HAL::millis();
        ...
        if (_can_iface) {
            ...
            if (write && _can_iface->send(out_frame, AG_HAL::native_micros64() + 100000, out_flags) == 1) {
                    rx_queue_.pop();
                    num_tries = 0;
            } else if (num_tries > 8) {
                rx_queue_.pop();
