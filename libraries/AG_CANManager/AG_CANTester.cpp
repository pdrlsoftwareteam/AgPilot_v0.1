...
#include "AG_CANTester_KDECAN.h"
extern const AG_HAL::HAL& hal;

const AG_Param::GroupInfo CANTester::var_info[] = {
    // @Param: ID
...
bool CANTester::add_interface(AG_HAL::CANIface* can_iface)
{
    if (_num_ifaces >= HAL_NUM_CAN_IFACES) {
        debug_can(AG_CANManager::LOG_ERROR, "Max Number of CanIfaces exceeded");
        return false;
    }
...
void CANTester::init(uint8_t driver_index, bool enable_filters)
{
    ...
    debug_can(AG_CANManager::LOG_DEBUG, "starting init");
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&CANTester::main_thread, void), "can_tester", 4096, AG_HAL::Scheduler::PRIORITY_CAN, 1)) {
...
bool CANTester::write_frame(uint8_t iface, AG_HAL::CANFrame &out_frame, uint64_t timeout)
{
    ...
    bool ret = _can_ifaces[iface]->select(read_select, write_select, &out_frame, AG_HAL::native_micros64() + timeout);
    if (!ret || !write_select) {
        return false;
    }
    uint64_t deadline = AG_HAL::native_micros64() + 2000000;
    // hal.console->printf("%x TDEAD: %lu\n", out_frame.id, deadline);
    // send frame and return success
    return (_can_ifaces[iface]->send(out_frame, deadline, AG_HAL::CANIface::AbortOnError) == 1);
}

// read frame on CAN bus, returns true on success
bool CANTester::read_frame(uint8_t iface, AG_HAL::CANFrame &recv_frame, uint64_t timeout, AG_HAL::CANIface::CanIOFlags &flags)
{
    if (!_can_ifaces[iface]->set_event_handle(&_event_handle)) {
        debug_can(AG_CANManager::LOG_ERROR, "Cannot add event handle");
        return false;
    ...
    bool ret = _can_ifaces[iface]->select(read_select, write_select, nullptr, AG_HAL::native_micros64() + timeout);
    if (!ret || !read_select) {
        ...
        for (uint8_t i = 0; i < 2; i++) {
            ...
            while (true) {
                ...
                if (read_frame((i+1)%2, frame, 0, flags)) {
                    if (frame.id != ((13 | AG_HAL::CANFrame::FlagEFF) + i)) {
                        continue;
                    }
...
void CANTester::reset_frame(AG_HAL::CANFrame& frame)
{
    frame.id = 0;
...
void CANTester::create_loopback_frame(CANTester::loopback_stats_s &stats, AG_HAL::CANFrame& frame)
{
    frame.id = 13 | AG_HAL::CANFrame::FlagEFF;
    frame.dlc = 8;
...
void CANTester::check_loopback_frame(CANTester::loopback_stats_s &stats, AG_HAL::CANFrame& frame)
{
    if (frame.dlc != 8) {
        stats.bad_rx_data++;
    }
...
bool CANTester::test_busoff_recovery()
{
    ...
    uint64_t timestamp;
    AG_HAL::CANIface::CanIOFlags flags;
    AG_HAL::CANFrame bo_frame;
    bo_frame.id = (10 | AG_HAL::CANFrame::FlagEFF);
    memset(bo_frame.data, 0xA, sizeof(bo_frame.data));
    bo_frame.dlc =8;//AG_HAL::CANFrame::MaxDataLen;
    ...
    while (num_busoff_runs--) {
        ...
        _can_ifaces[0]->send(bo_frame, AG_HAL::native_micros64()+1000, 0);
        _can_ifaces[1]->receive(bo_frame, timestamp, flags);
        _can_ifaces[1]->send(bo_frame, AG_HAL::native_micros64()+1000, 0);
...
{
    AG_CANTester_KDECAN* kdecan_test = new AG_CANTester_KDECAN;
...
void handle_raw_command(const uavcan::ReceivedDataStructure<uavcan::equipment::esc::RawCommand>& msg)
{
    ...
    if (num_received == 0) {
        last_millis = AG_HAL::millis();
    }
    num_received++;
    // update rate every 50 packets
    if (num_received == 50) {
        uavcan_esc_command_rate = 50000/(AG_HAL::millis() - last_millis);
        num_received = 0;
    }
}
...
CANTester *CANTester::get_cantester(uint8_t driver_index)
{
    if (driver_index >= AP::can().get_num_drivers() ||
        AP::can().get_driver_type(driver_index) != AG_CANManager::Driver_Type_CANTester) {
        return nullptr;
    }
