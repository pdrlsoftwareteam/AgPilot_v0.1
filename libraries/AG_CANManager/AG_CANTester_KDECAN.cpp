...
void AG_CANTester_KDECAN::count_msg(uint32_t frame_id)
{
...
bool AG_CANTester_KDECAN::init(AG_HAL::CANIface* can_iface)
{
    _can_iface = can_iface;
    if (_can_iface == nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "CANIface is null, abort!");
        return false;
    }

    if (!_can_iface->is_initialized()) {
        debug_can(AG_CANManager::LOG_ERROR, "Can not initialised");
        return false;
    }

    if (!_can_iface->set_event_handle(&_event_handle)) {
        debug_can(AG_CANManager::LOG_ERROR, "Failed to set Event Handle");
        return false;
    }
    debug_can(AG_CANManager::LOG_ERROR, "init done");
    return true;
}

void AG_CANTester_KDECAN::loop(void)
{
    if (_can_iface == nullptr) {
        return;
    }
    ...
    bool select_ret = _can_iface->select(read_select, write_select, nullptr, AG_HAL::micros64() + 1000);
    ...
    if (select_ret && read_select) {
        ...
        if (res == 1) {
            uint32_t id = recv_frame.id & AG_HAL::CANFrame::MaskExtID;
            ...
            while (i < n) {
                ...
                case UPDATE_NODE_ID_OBJ_ADDR: {
                    if (_esc_info[i].enum_timeout_ms != 0
                        && _esc_info[i].enum_timeout_ms >= AG_HAL::millis()) {
                ...
                case START_ENUM_OBJ_ADDR: {
                    _esc_info[i].enum_timeout_ms = AG_HAL::millis() + be16toh_ptr(&recv_frame.data[0]);
                ...
                res_frame.id = (_esc_info[i].node_id << 16) | object_address | AG_HAL::CANFrame::FlagEFF;
                read_select = false;
                write_select = true;
                select_ret = _can_iface->select(read_select, write_select, &res_frame, AG_HAL::micros64() + 1000);
                if (!select_ret) {
                    break;
                }
                int16_t res2 = _can_iface->send(res_frame, AG_HAL::micros64() + 500000, 0);
                if (res2 == 1) {
                    i++;
                } else {
...
void AG_CANTester_KDECAN::print_stats(void)
{
    DEV_PRINTF("KDECANTester: TimeStamp: %u\n", (unsigned)AG_HAL::micros());
...
bool AG_CANTester_KDECAN::send_enumeration(uint8_t num)
{
    if (_esc_info[num].enum_timeout_ms == 0 ||
        AG_HAL::millis() > _esc_info[num].enum_timeout_ms) {
    ...
    while (true) {
        ...
        AG_HAL::CANFrame res_frame { (_esc_info[num].node_id << 16) | START_ENUM_OBJ_ADDR | AG_HAL::CANFrame::FlagEFF,
                                     (uint8_t*)&mcu,
                                     8 };
        int16_t res = _can_iface->send(res_frame, AG_HAL::micros64() + 1000, 0);
        if (res == 1) {
            return true;
        }
    }
