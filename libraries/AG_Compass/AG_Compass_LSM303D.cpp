...
AG_Compass_LSM303D::AG_Compass_LSM303D(AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : _dev(std::move(dev))
{
}

AG_Compass_Backend *AG_Compass_LSM303D::probe(AG_HAL::OwnPtr<AG_HAL::Device> dev,
                                              enum Rotation rotation)
{
    if (!dev) {
        return nullptr;
    }
    AG_Compass_LSM303D *sensor = new AG_Compass_LSM303D(std::move(dev));
...
uint8_t AG_Compass_LSM303D::_register_read(uint8_t reg)
{
    uint8_t val = 0;
...
bool AG_Compass_LSM303D::_block_read(uint8_t reg, uint8_t *buf, uint32_t size)
{
    reg |= DIR_READ | ADDR_INCREMENT;
    return _dev->read_registers(reg, buf, size);
}

void AG_Compass_LSM303D::_register_write(uint8_t reg, uint8_t val)
{
    _dev->write_register(reg, val);
}

void AG_Compass_LSM303D::_register_modify(uint8_t reg, uint8_t clearbits, uint8_t setbits)
{
    uint8_t val;
 ...
 * the accels.
 */
bool AG_Compass_LSM303D::_data_ready()
{
...
// Read Sensor data
bool AG_Compass_LSM303D::_read_sample()
{
    struct PACKED {
        uint8_t status;
        int16_t x;
...
bool AG_Compass_LSM303D::init(enum Rotation rotation)
{
    ...
    _dev->register_periodic_callback(11000, FUNCTOR_BIND_MEMBER(&AG_Compass_LSM303D::_update, void));

    return true;
}

bool AG_Compass_LSM303D::_hardware_init()
{
...
void AG_Compass_LSM303D::_update()
{
    if (!_read_sample()) {
        return;
    }
...
void AG_Compass_LSM303D::_disable_i2c()
{
...
bool AG_Compass_LSM303D::_mag_set_range(uint8_t max_ga)
{
    uint8_t setbits = 0;
...
bool AG_Compass_LSM303D::_mag_set_samplerate(uint16_t frequency)
{
    uint8_t setbits = 0;
