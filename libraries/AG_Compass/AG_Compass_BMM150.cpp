...
AG_Compass_Backend *AG_Compass_BMM150::probe(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev, bool force_external, enum Rotation rotation)
{
    if (!dev) {
        return nullptr;
    }
    AG_Compass_BMM150 *sensor = new AG_Compass_BMM150(std::move(dev), force_external, rotation);
...
AG_Compass_BMM150::AG_Compass_BMM150(AG_HAL::OwnPtr<AG_HAL::Device> dev, bool force_external, enum Rotation rotation)
    : _dev(std::move(dev)), _rotation(rotation), _force_external(force_external)
{
}

bool AG_Compass_BMM150::_load_trim_values()
{
    struct {
        int8_t dig_x1;
        int8_t dig_y1;
...
bool AG_Compass_BMM150::init()
{
    ...
    _dev->register_periodic_callback(MEASURE_TIME_USEC,
            FUNCTOR_BIND_MEMBER(&AG_Compass_BMM150::_update, void));

    _last_read_ms = AG_HAL::millis();
 ...
 */
int16_t AG_Compass_BMM150::_compensate_xy(int16_t xy, uint32_t rhall, int32_t txy1, int32_t txy2) const
{
...
int16_t AG_Compass_BMM150::_compensate_z(int16_t z, uint32_t rhall) const
{
...
void AG_Compass_BMM150::_update()
{
    le16_t data[4];
