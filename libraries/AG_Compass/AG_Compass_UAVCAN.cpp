...
AG_Compass_UAVCAN::DetectedModules AG_Compass_UAVCAN::_detected_modules[];
HAL_Semaphore AG_Compass_UAVCAN::_sem_registry;

AG_Compass_UAVCAN::AG_Compass_UAVCAN(AG_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t sensor_id, uint32_t devid)
...
void AG_Compass_UAVCAN::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (mag_listener_res < 0) {
        AG_HAL::panic("UAVCAN Mag subscriber start problem\n\r");
        return;
    }
...
AG_Compass_Backend* AG_Compass_UAVCAN::probe(uint8_t index)
{
    AG_Compass_UAVCAN* driver = nullptr;
    if (!_detected_modules[index].driver && _detected_modules[index].ap_uavcan) {
        WITH_SEMAPHORE(_sem_registry);
        // Register new Compass mode to a backend
        driver = new AG_Compass_UAVCAN(_detected_modules[index].ap_uavcan, _detected_modules[index].node_id, _detected_modules[index].sensor_id, _detected_modules[index].devid);
        if (driver) {
            if (!driver->init()) {
                delete driver;
                return nullptr;
...
bool AG_Compass_UAVCAN::init()
{
    ...
    AP::can().log_text(AG_CANManager::LOG_INFO, LOG_TAG,  "AG_Compass_UAVCAN loaded\n\r");
    return true;
}

AG_Compass_UAVCAN* AG_Compass_UAVCAN::get_uavcan_backend(AG_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t sensor_id)
{
    if (ap_uavcan == nullptr) {
        return nullptr;
    }
    ...
    if (!already_detected) {
        for (uint8_t i = 0; i < COMPASS_MAX_BACKEND; i++) {
            if (nullptr == _detected_modules[i].ap_uavcan) {
                ...
                _detected_modules[i].node_id = node_id;
                _detected_modules[i].sensor_id = sensor_id;
                _detected_modules[i].devid = AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_UAVCAN,
...
void AG_Compass_UAVCAN::handle_mag_msg(const Vector3f &mag)
{
...
void AG_Compass_UAVCAN::handle_magnetic_field(AG_UAVCAN* ap_uavcan, uint8_t node_id, const MagCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);

    Vector3f mag_vector;
    AG_Compass_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, 0);
    if (driver != nullptr) {
...
void AG_Compass_UAVCAN::handle_magnetic_field_2(AG_UAVCAN* ap_uavcan, uint8_t node_id, const Mag2Cb &cb)
{
    WITH_SEMAPHORE(_sem_registry);

    Vector3f mag_vector;
    uint8_t sensor_id = cb.msg->sensor_id;
    AG_Compass_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, sensor_id);
    if (driver != nullptr) {
        mag_vector[0] = cb.msg->magnetic_field_ga[0];
        mag_vector[1] = cb.msg->magnetic_field_ga[1];
        mag_vector[2] = cb.msg->magnetic_field_ga[2];
...
void AG_Compass_UAVCAN::read(void)
{
