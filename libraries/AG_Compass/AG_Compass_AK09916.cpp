...
#include <stdio.h>
#include <AG_InertialSensor/AG_InertialSensor_Invensensev2.h>
...
AG_Compass_AK09916::AG_Compass_AK09916(AP_AK09916_BusDriver *bus,
                                        bool force_external,
...
AG_Compass_AK09916::~AG_Compass_AK09916()
{
    delete _bus;
}

AG_Compass_Backend *AG_Compass_AK09916::probe(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev,
                                             bool force_external,
                                             enum Rotation rotation)
{
    if (!dev) {
    ...
    AG_Compass_AK09916 *sensor = new AG_Compass_AK09916(bus, force_external, rotation);
...
AG_Compass_Backend *AG_Compass_AK09916::probe_ICM20948(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev,
                                                     AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev_icm,
                                                     bool force_external,
                                                     enum Rotation rotation)
{
    if (!dev || !dev_icm) {
...
AG_Compass_Backend *AG_Compass_AK09916::probe_ICM20948(uint8_t inv2_instance,
                                                     enum Rotation rotation)
{  
    return probe_ICM20948_SPI(inv2_instance,rotation);
}

AG_Compass_Backend *AG_Compass_AK09916::probe_ICM20948_SPI(uint8_t inv2_instance,
                                                     enum Rotation rotation)
{
#if AP_INERTIALSENSOR_ENABLED
    AG_InertialSensor &ins = AP::ins();
    ...
    AG_Compass_AK09916 *sensor = new AG_Compass_AK09916(bus, false, rotation);
    if (!sensor || !sensor->init()) {
        delete sensor;
        return nullptr;
    }
...
AG_Compass_Backend *AG_Compass_AK09916::probe_ICM20948_I2C(uint8_t inv2_instance,
                                                     enum Rotation rotation)
{
    AG_InertialSensor &ins = AP::ins();
...
bool AG_Compass_AK09916::init()
{
    AG_HAL::Semaphore *bus_sem = _bus->get_semaphore();
    ...
    _bus->register_periodic_callback(10000, FUNCTOR_BIND_MEMBER(&AG_Compass_AK09916::_update, void));
...
void AG_Compass_AK09916::_make_adc_sensitivity_adjustment(Vector3f& field) const
{
...
void AG_Compass_AK09916::_update()
{
...
bool AG_Compass_AK09916::_check_id()
{
    for (int i = 0; i < 5; i++) {
        uint8_t deviceid = 0;
...
bool AG_Compass_AK09916::_setup_mode() {
    return _bus->register_write(REG_CNTL2, 0x08, true); //Continuous Mode 2
}

bool AG_Compass_AK09916::_reset()
{
    return _bus->register_write(REG_CNTL3, 0x01); //Soft Reset
}

/* AG_HAL::I2CDevice implementation of the AK09916 */
AP_AK09916_BusDriver_HALDevice::AP_AK09916_BusDriver_HALDevice(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev)
    : _dev(std::move(dev))
{
}
...
AG_HAL::Semaphore *AP_AK09916_BusDriver_HALDevice::get_semaphore()
{
    return _dev->get_semaphore();
}

AG_HAL::Device::PeriodicHandle AP_AK09916_BusDriver_HALDevice::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
    return _dev->register_periodic_callback(period_usec, cb);
}

/* AK09916 on an auxiliary bus of IMU driver */
AP_AK09916_BusDriver_Auxiliary::AP_AK09916_BusDriver_Auxiliary(AG_InertialSensor &ins, uint8_t backend_id,
...
AG_HAL::Semaphore *AP_AK09916_BusDriver_Auxiliary::get_semaphore()
{
...
AG_HAL::Device::PeriodicHandle AP_AK09916_BusDriver_Auxiliary::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
