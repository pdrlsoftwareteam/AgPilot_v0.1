...
AG_Compass_SITL::AG_Compass_SITL()
    : _sitl(AP::sitl())
{
    if (_sitl != nullptr) {
        ...
        // scale to 1
        AG_Param::set_default_by_name("COMPASS_SCALE", 1);
        AG_Param::set_default_by_name("COMPASS_SCALE2", 1);
        AG_Param::set_default_by_name("COMPASS_SCALE3", 1);

        // make first compass external
        set_external(_compass_instance[0], true);

        hal.scheduler->register_timer_process(FUNCTOR_BIND(this, &AG_Compass_SITL::_timer, void));
    }
}
...
*/
void AG_Compass_SITL::_setup_eliptical_correcion(uint8_t i)
{
...
void AG_Compass_SITL::_timer()
{
    // TODO: Refactor delay buffer with AG_Baro_SITL.
