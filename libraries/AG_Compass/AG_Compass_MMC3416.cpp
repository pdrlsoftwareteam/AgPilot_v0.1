...
AG_Compass_Backend *AG_Compass_MMC3416::probe(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev,
                                              bool force_external,
                                              enum Rotation rotation)
{
    ...
    AG_Compass_MMC3416 *sensor = new AG_Compass_MMC3416(std::move(dev), force_external, rotation);
...
AG_Compass_MMC3416::AG_Compass_MMC3416(AG_HAL::OwnPtr<AG_HAL::Device> _dev,
...
bool AG_Compass_MMC3416::init()
{
    ...
    dev->register_periodic_callback(10000,
                                    FUNCTOR_BIND_MEMBER(&AG_Compass_MMC3416::timer, void));
...
void AG_Compass_MMC3416::timer()
{
    ...
    case STATE_REFILL1:
        if (dev->write_register(REG_CONTROL0, REG_CONTROL0_REFILL)) {
            ...
            refill_start_ms = AG_HAL::millis();
        }
        break;

    case STATE_REFILL1_WAIT: {
        uint8_t status;
        if (AG_HAL::millis() - refill_start_ms > MIN_DELAY_SET_RESET &&
