...
void Compass::cal_update()
{
    ...
    for (Priority i(0); i<COMPASS_MAX_INSTANCES; i++) {
        ...
        if (_calibrator[i]->failed()) {
            AG_Notify::events.compass_cal_failed = 1;
        }
    ...
    AG_Notify::flags.compass_cal_running = running;
...
bool Compass::_start_calibration(uint8_t i, bool retry, float delay)
{
    ...
    if (!is_calibrating()) {
        AG_Notify::events.initiated_compass_cal = 1;
    }
    ...
    if (!_cal_thread_started) {
        ...
        if (!hal.scheduler->thread_create(FUNCTOR_BIND(this, &Compass::_update_calibration_trampoline, void), "compasscal", 2048, AG_HAL::Scheduler::PRIORITY_IO, 0)) {
...
bool Compass::_accept_calibration(uint8_t i)
{
    ...
    if (_cal_saved[prio] || cal_report.status == CompassCalibrator::Status::NOT_STARTED) {
        ...
        if (!is_calibrating()) {
            AG_Notify::events.compass_cal_saved = 1;
        }
        return true;
    } else {
        return false;
                                      ...
                                      float lat_deg, float lon_deg, bool force_use)
{
    ...
    float declination;
    float inclination;
    if (!AG_Declination::get_mag_field_ef(lat_deg, lon_deg, intensity, declination, inclination)) {
