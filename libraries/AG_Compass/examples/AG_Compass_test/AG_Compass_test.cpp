...
class DummyVehicle {
public:
    AG_AHRS ahrs;  // Need since https://github.com/ArduPilot/ardupilot/pull/10890
    AG_Baro baro; // Compass tries to set magnetic model based on location.
...
static void loop()
{
    ...
    // run read() at 10Hz
    if ((AG_HAL::micros() - timer) > 100000L) {
        timer = AG_HAL::micros();
        compass.read();
        const uint32_t read_time = AG_HAL::micros() - timer;
