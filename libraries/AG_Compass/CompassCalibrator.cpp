...
void CompassCalibrator::start(bool retry, float delay, uint16_t offset_max, uint8_t compass_idx, float tolerance)
{
    ...
    cal_settings.start_time_ms = AG_HAL::millis();
...
bool CompassCalibrator::set_status(CompassCalibrator::Status status)
{
    ...
    switch (status) {
        ...
        case Status::RUNNING_STEP_ONE:
            ...
            if (_attempt == 1 && (AG_HAL::millis()-_start_time_ms)*1.0e-3f < _delay_start_sec) {
                return false;
            }
...
bool CompassCalibrator::fix_radius(void)
{
    ...
    float declination;
    float inclination;
    AG_Declination::get_mag_field_ef(loc.lat * 1e-7f, loc.lng * 1e-7f, intensity, declination, inclination);
