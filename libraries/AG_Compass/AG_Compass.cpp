...
#include <AG_HAL_Linux/I2CDevice.h>
#endif
...
#include "AG_Compass_SITL.h"
#include "AG_Compass_AK8963.h"
#include "AG_Compass_Backend.h"
#include "AG_Compass_BMM150.h"
#include "AG_Compass_HMC5843.h"
#include "AG_Compass_IST8308.h"
#include "AG_Compass_IST8310.h"
#include "AG_Compass_LSM303D.h"
#include "AG_Compass_LSM9DS1.h"
#include "AG_Compass_LIS3MDL.h"
#include "AG_Compass_AK09916.h"
#include "AG_Compass_QMC5883L.h"
#if AP_COMPASS_UAVCAN_ENABLED
#include "AG_Compass_UAVCAN.h"
#endif
#include "AG_Compass_MMC3416.h"
#include "AG_Compass_MMC5xx3.h"
#include "AG_Compass_MAG3110.h"
#include "AG_Compass_RM3100.h"
#if AP_COMPASS_MSP_ENABLED
#include "AG_Compass_MSP.h"
#endif
...
const AG_Param::GroupInfo Compass::var_info[] = {
...
Compass::Compass(void)
{
    if (_singleton != nullptr) {
        ...
        AG_HAL::panic("Compass must be singleton");
#endif
        return;
    }
    _singleton = this;
...
void Compass::init()
{
    ...
    if (_priority_did_stored_list[Priority(0)] == 0) {
        ...
        if (AG_Param::find_top_level_key_by_pointer(this, k_param_compass)) {
            const AG_Param::ConversionInfo primary_compass_old_param = {k_param_compass, 12, AP_PARAM_INT8, ""};
            AP_Int8 value;
            value.set(0);
            bool primary_param_exists = AG_Param::find_old_parameter(&primary_compass_old_param, &value);
...
bool Compass::register_compass(int32_t dev_id, uint8_t& instance)
{
    ...
    if (_unreg_compass_count >= COMPASS_MAX_UNREG_DEV) {
        AG_HAL::panic("Too many compass instances");
    }
...
bool Compass::_add_backend(AG_Compass_Backend *backend)
{
    if (!backend) {
        return false;
    }
...
bool Compass::_have_i2c_driver(uint8_t bus, uint8_t address) const
{
    for (StateIndex i(0); i<COMPASS_MAX_INSTANCES; i++) {
        ...
        if (AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_I2C, bus, address, 0) ==
            AG_HAL::Device::change_bus_id(uint32_t(_state[i].dev_id.get()), 0)) {
...
void Compass::_probe_external_i2c_compasses(void)
{
    ...
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_HMC5843, AG_Compass_HMC5843::probe(GET_I2C_DEVICE(i, HAL_COMPASS_HMC5843_I2C_ADDR),
                    true, ROTATION_ROLL_180));
    }

#if !defined(HAL_SKIP_AUTO_INTERNAL_I2C_PROBE)
    if (AG_BoardConfig::get_board_type() != AG_BoardConfig::PX4_BOARD_MINDPXV2 &&
        AG_BoardConfig::get_board_type() != AG_BoardConfig::PX4_BOARD_AEROFC) {
        // internal i2c bus
    ...
    //external i2c bus
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_QMC5883L, AG_Compass_QMC5883L::probe(GET_I2C_DEVICE(i, HAL_COMPASS_QMC5883L_I2C_ADDR),
    ...
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_ICM20948, AG_Compass_AK09916::probe_ICM20948(GET_I2C_DEVICE(i, HAL_COMPASS_AK09916_I2C_ADDR),
...
#if !defined(HAL_SKIP_AUTO_INTERNAL_I2C_PROBE)
    FOREACH_I2C_INTERNAL(i) {
        ADD_BACKEND(DRIVER_LIS3MDL, AG_Compass_LIS3MDL::probe(GET_I2C_DEVICE(i, HAL_COMPASS_LIS3MDL_I2C_ADDR),
                    all_external, all_external?ROTATION_YAW_90:ROTATION_NONE));
    }

    // lis3mdl on bus 0 with alternate address
    FOREACH_I2C_INTERNAL(i) {
        ADD_BACKEND(DRIVER_LIS3MDL, AG_Compass_LIS3MDL::probe(GET_I2C_DEVICE(i, HAL_COMPASS_LIS3MDL_I2C_ADDR2),
                    all_external, all_external?ROTATION_YAW_90:ROTATION_NONE));
    }
#endif
    ...
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_AK09916, AG_Compass_AK09916::probe(GET_I2C_DEVICE(i, HAL_COMPASS_AK09916_I2C_ADDR),
    ...
    if (AG_BoardConfig::get_board_type() != AG_BoardConfig::PX4_BOARD_FMUV5 &&
        AG_BoardConfig::get_board_type() != AG_BoardConfig::PX4_BOARD_FMUV6) {
        enum Rotation default_rotation = AP_COMPASS_IST8310_DEFAULT_ROTATION;

        if (AG_BoardConfig::get_board_type() == AG_BoardConfig::PX4_BOARD_AEROFC) {
        ...
        for (uint8_t a=0; a<ARRAY_SIZE(ist8310_addr); a++) {
            FOREACH_I2C_EXTERNAL(i) {
                ADD_BACKEND(DRIVER_IST8310, AG_Compass_IST8310::probe(GET_I2C_DEVICE(i, ist8310_addr[a]),
    ...
    // external i2c bus
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_IST8308, AG_Compass_IST8308::probe(GET_I2C_DEVICE(i, HAL_COMPASS_IST8308_I2C_ADDR),
                    true, ROTATION_NONE));
    }
#if !defined(HAL_SKIP_AUTO_INTERNAL_I2C_PROBE)
    FOREACH_I2C_INTERNAL(i) {
    ...
    // external i2c bus
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_MMC3416, AG_Compass_MMC3416::probe(GET_I2C_DEVICE(i, HAL_COMPASS_MMC3416_I2C_ADDR),
                    true, ROTATION_NONE));
    }
#if !defined(HAL_SKIP_AUTO_INTERNAL_I2C_PROBE)
    FOREACH_I2C_INTERNAL(i) {
    ...
    FOREACH_I2C_EXTERNAL(i) {
        for (uint8_t j=0; j<ARRAY_SIZE(rm3100_addresses); j++) {
            ADD_BACKEND(DRIVER_RM3100, AG_Compass_RM3100::probe(GET_I2C_DEVICE(i, rm3100_addresses[j]), true, ROTATION_NONE));
        }
    }
    ...
    FOREACH_I2C_INTERNAL(i) {
        for (uint8_t j=0; j<ARRAY_SIZE(rm3100_addresses); j++) {
            ADD_BACKEND(DRIVER_RM3100, AG_Compass_RM3100::probe(GET_I2C_DEVICE(i, rm3100_addresses[j]), all_external, ROTATION_NONE));
        }
    }
#endif
    ...
    FOREACH_I2C_EXTERNAL(i) {
        for (uint8_t addr=BMM150_I2C_ADDR_MIN; addr <= BMM150_I2C_ADDR_MAX; addr++) {
            ADD_BACKEND(DRIVER_BMM150,
                        AG_Compass_BMM150::probe(GET_I2C_DEVICE(i, addr), true, ROTATION_NONE));
        }
    }
...
{
#if AP_COMPASS_EXTERNALAHRS_ENABLED
    const int8_t serial_port = AP::externalAHRS().get_port(AG_ExternalAHRS::AvailableSensor::COMPASS);
    if (serial_port >= 0) {
        ADD_BACKEND(DRIVER_EXTERNALAHRS, new AG_Compass_ExternalAHRS(serial_port));
    }
#endif
    ...
    ADD_BACKEND(DRIVER_SITL, new AG_Compass_SITL());
#endif
    ...
    for (uint8_t i=0; i<8; i++) {
        if (msp_instance_mask & (1U<<i)) {
            ADD_BACKEND(DRIVER_MSP, new AG_Compass_MSP(i));
        }
    }
#endif
    ...
    case AG_BoardConfig::PX4_BOARD_PCNC1:
        ...
        ADD_BACKEND(DRIVER_BMM150,
                    AG_Compass_BMM150::probe(GET_I2C_DEVICE(0, 0x10), false, ROTATION_NONE));
#endif
        break;
...
#if AP_COMPASS_HMC5843_ENABLED
        // external i2c bus
        ADD_BACKEND(DRIVER_HMC5843, AG_Compass_HMC5843::probe(GET_I2C_DEVICE(1, HAL_COMPASS_HMC5843_I2C_ADDR),
                    true, ROTATION_ROLL_180));

        // internal i2c bus
        ADD_BACKEND(DRIVER_HMC5843, AG_Compass_HMC5843::probe(GET_I2C_DEVICE(0, HAL_COMPASS_HMC5843_I2C_ADDR),
    ...
    case AG_BoardConfig::PX4_BOARD_PIXHAWK:
#if AP_COMPASS_HMC5843_ENABLED
        ADD_BACKEND(DRIVER_HMC5843, AG_Compass_HMC5843::probe(hal.spi->get_device(HAL_COMPASS_HMC5843_NAME),
                    false, ROTATION_PITCH_180));
#endif
#if AP_COMPASS_LSM303D_ENABLED
        ADD_BACKEND(DRIVER_LSM303D, AG_Compass_LSM303D::probe(hal.spi->get_device(HAL_INS_LSM9DS0_A_NAME), ROTATION_NONE));
#endif
        break;

    case AG_BoardConfig::PX4_BOARD_PIXHAWK2:
#if AP_COMPASS_LSM303D_ENABLED
        ADD_BACKEND(DRIVER_LSM303D, AG_Compass_LSM303D::probe(hal.spi->get_device(HAL_INS_LSM9DS0_EXT_A_NAME), ROTATION_YAW_270));
#endif
        ...
        ADD_BACKEND(DRIVER_AK8963, AG_Compass_AK8963::probe_mpu9250(1, ROTATION_YAW_270));
#endif
#if AP_COMPASS_AK09916_ENABLED && AP_COMPASS_ICM20948_ENABLED
        ADD_BACKEND(DRIVER_AK09916, AG_Compass_AK09916::probe_ICM20948(0, ROTATION_ROLL_180_YAW_90));
#endif
        break;
...
#if AP_COMPASS_IST8310_ENABLED
        FOREACH_I2C_EXTERNAL(i) {
            ADD_BACKEND(DRIVER_IST8310, AG_Compass_IST8310::probe(GET_I2C_DEVICE(i, HAL_COMPASS_IST8310_I2C_ADDR),
...
#if AP_COMPASS_AK8963_ENABLED
        ADD_BACKEND(DRIVER_AK8963, AG_Compass_AK8963::probe_mpu9250(1, ROTATION_NONE));
#endif
        break;

    case AG_BoardConfig::PX4_BOARD_PIXHAWK_PRO:
#if AP_COMPASS_AK8963_ENABLED
        ADD_BACKEND(DRIVER_AK8963, AG_Compass_AK8963::probe_mpu9250(0, ROTATION_ROLL_180_YAW_90));
#endif
#if AP_COMPASS_LIS3MDL_ENABLED
        ADD_BACKEND(DRIVER_LIS3MDL, AG_Compass_LIS3MDL::probe(hal.spi->get_device(HAL_COMPASS_LIS3MDL_NAME),
                    false, ROTATION_NONE));
#endif  // AP_COMPASS_LIS3MDL_ENABLED
        break;
    ...
    case AG_BoardConfig::PX4_BOARD_PHMINI:
#if AP_COMPASS_AK8963_ENABLED
        ADD_BACKEND(DRIVER_AK8963, AG_Compass_AK8963::probe_mpu9250(0, ROTATION_ROLL_180));
#endif
        break;
    ...
    case AG_BoardConfig::PX4_BOARD_PH2SLIM:
#if AP_COMPASS_AK8963_ENABLED
        ADD_BACKEND(DRIVER_AK8963, AG_Compass_AK8963::probe_mpu9250(0, ROTATION_YAW_270));
#endif
        break;
...
#endif
#if AP_COMPASS_LSM303D_ENABLED
        ADD_BACKEND(DRIVER_LSM303D, AG_Compass_LSM303D::probe(hal.spi->get_device(HAL_INS_LSM9DS0_A_NAME), ROTATION_PITCH_180_YAW_270));
#endif
        break;
    ...
    if (_driver_enabled(DRIVER_UAVCAN)) {
        for (uint8_t i=0; i<COMPASS_MAX_BACKEND; i++) {
            AG_Compass_Backend* _uavcan_backend = AG_Compass_UAVCAN::probe(i);
            if (_uavcan_backend) {
        ...
        for (Priority i(0); i<COMPASS_MAX_INSTANCES; i++) {
            if (AG_HAL::Device::devid_get_bus_type(_priority_did_list[i]) != AG_HAL::Device::BUS_TYPE_UAVCAN
            ...
            for (uint8_t j=0; j<COMPASS_MAX_INSTANCES; j++) {
                uint32_t detected_devid = AG_Compass_UAVCAN::get_detected_devid(j);
                ...
                AG_Compass_Backend* _uavcan_backend = AG_Compass_UAVCAN::probe(j);
                if (_uavcan_backend) {
                    _add_backend(_uavcan_backend);
...
bool Compass::is_replacement_mag(uint32_t devid) {
    ...
    if (devid == 0 || (AG_HAL::Device::devid_get_bus_type(devid) != AG_HAL::Device::BUS_TYPE_UAVCAN)) {
        return false;
    }
...
Compass::_detect_runtime(void)
{
    ...
    if ((AG_HAL::millis() - last_try) < 1000) {
        return;
    }
    last_try = AG_HAL::millis();
    if (_driver_enabled(DRIVER_UAVCAN)) {
        for (uint8_t i=0; i<COMPASS_MAX_BACKEND; i++) {
            AG_Compass_Backend* _uavcan_backend = AG_Compass_UAVCAN::probe(i);
            if (_uavcan_backend) {
...
void Compass::try_set_initial_location()
{
    ...
    _declination.set(radians(
                         AG_Declination::get_declination(
...
#if AP_COMPASS_EXTERNALAHRS_ENABLED
void Compass::handle_external(const AG_ExternalAHRS::mag_data_message_t &pkt)
{
