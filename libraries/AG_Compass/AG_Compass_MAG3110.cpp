...
/*
  ...
  into the AG_Compass_backend code, with a parameter to enable it.
 */
...
AG_Compass_MAG3110::AG_Compass_MAG3110(AG_HAL::OwnPtr<AG_HAL::Device> dev)
    : _dev(std::move(dev))
{
}

AG_Compass_Backend *AG_Compass_MAG3110::probe(AG_HAL::OwnPtr<AG_HAL::Device> dev,
                                              enum Rotation rotation)
{
    if (!dev) {
        return nullptr;
    }
    AG_Compass_MAG3110 *sensor = new AG_Compass_MAG3110(std::move(dev));
...
bool AG_Compass_MAG3110::init(enum Rotation rotation)
{
    ...
    // read at 75Hz
    _dev->register_periodic_callback(13333, FUNCTOR_BIND_MEMBER(&AG_Compass_MAG3110::_update, void)); 

    return true;
}

bool AG_Compass_MAG3110::_hardware_init()
{
...
// Read Sensor data
bool AG_Compass_MAG3110::_read_sample()
{
    {
        uint8_t status;
...
void AG_Compass_MAG3110::_update()
{
    if (!_read_sample()) {
        return;
    }
