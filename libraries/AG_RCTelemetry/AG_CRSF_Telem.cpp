...
const uint8_t AG_CRSF_Telem::PASSTHROUGH_STATUS_TEXT_FRAME_MAX_SIZE;
const uint8_t AG_CRSF_Telem::PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE;
const uint8_t AG_CRSF_Telem::CRSF_RX_DEVICE_PING_MAX_RETRY;

AG_CRSF_Telem *AG_CRSF_Telem::singleton;

AG_CRSF_Telem::AG_CRSF_Telem() : AG_RCTelemetry(0)
{
    singleton = this;
}

AG_CRSF_Telem::~AG_CRSF_Telem(void)
{
    singleton = nullptr;
}

bool AG_CRSF_Telem::init(void)
{
    // sanity check that we are using a UART for RC input
    if (!AP::serialmanager().have_serial(AG_SerialManager::SerialProtocol_RCIN, 0)
        && !AP::serialmanager().have_serial(AG_SerialManager::SerialProtocol_CRSF, 0)) {
        return false;
    }

    return AG_RCTelemetry::init();
}

/*
  setup ready for passthrough telem
 */
void AG_CRSF_Telem::setup_wfq_scheduler(void)
{
...
void AG_CRSF_Telem::setup_custom_telemetry()
{
    ...
    const int8_t frsky_port = AP::serialmanager().find_portnum(AG_SerialManager::SerialProtocol_FrSky_SPort_Passthrough,0);
    if (frsky_port != -1) {
    ...
    AG_Frsky_SPort_Passthrough* passthrough = AP::frsky_passthrough_telem();
    if (passthrough == nullptr) {
        return;
    }

    // setup the frsky scheduler for crossfire and elrs
    passthrough->disable_scheduler_entry(AG_Frsky_SPort_Passthrough::GPS_LAT);
    passthrough->disable_scheduler_entry(AG_Frsky_SPort_Passthrough::GPS_LON);
    passthrough->disable_scheduler_entry(AG_Frsky_SPort_Passthrough::TEXT);
    passthrough->set_scheduler_entry_min_period(AG_Frsky_SPort_Passthrough::ATTITUDE, 350); // 3Hz
...
void AG_CRSF_Telem::update_custom_telemetry_rates(AG_RCProtocol_CRSF::RFMode rf_mode)
{
...
bool AG_CRSF_Telem::process_rf_mode_changes()
{
    const AG_RCProtocol_CRSF::RFMode current_rf_mode = get_rf_mode();
    uint32_t now = AG_HAL::millis();

    // the presence of a uart indicates that we are using CRSF for RC control
    AG_RCProtocol_CRSF* crsf = AP::crsf();
    AG_HAL::UARTDriver* uart = nullptr;
    if (crsf != nullptr) {
        uart = crsf->get_UART();
    }
    if (uart == nullptr) {
...
uint8_t AG_CRSF_Telem::get_custom_telem_frame_id() const
{
    if (!_crsf_version.pending &&
        ((_crsf_version.major > 4 || (_crsf_version.major == 4 && _crsf_version.minor >= 6))
            || is_elrs())) {
        return AG_RCProtocol_CRSF::CRSF_FRAMETYPE_AP_CUSTOM_TELEM;
    }
    return AG_RCProtocol_CRSF::CRSF_FRAMETYPE_AP_CUSTOM_TELEM_LEGACY;
}

AG_RCProtocol_CRSF::RFMode AG_CRSF_Telem::get_rf_mode() const
{
    AG_RCProtocol_CRSF* crsf = AP::crsf();
    if (crsf == nullptr) {
        return AG_RCProtocol_CRSF::RFMode::RF_MODE_UNKNOWN;
    }

    if (!_crsf_version.pending && _crsf_version.use_rf_mode) {
        if (is_elrs()) {
            return static_cast<AG_RCProtocol_CRSF::RFMode>(uint8_t(AG_RCProtocol_CRSF::RFMode::ELRS_RF_MODE_4HZ) + crsf->get_link_status().rf_mode);
        }
        return static_cast<AG_RCProtocol_CRSF::RFMode>(crsf->get_link_status().rf_mode);
    } else if (is_tracer()) {
        return AG_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_250HZ;
    }
    ...
    */
    if (get_avg_packet_rate() < 40U) {
        ...
        return AG_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_50HZ;
    }
    if (get_avg_packet_rate() > 120U) {
        // DMA CRSF rf mode 2
        return AG_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_150HZ;
    }
...
bool AG_CRSF_Telem::is_high_speed_telemetry(const AG_RCProtocol_CRSF::RFMode rf_mode) const
{
    if (_crsf_version.protocol != AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_ELRS) {
        return rf_mode == AG_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_150HZ || rf_mode == AG_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_250HZ;
    }
    return get_telemetry_rate() > 30;
}

uint16_t AG_CRSF_Telem::get_telemetry_rate() const
{
    if (_crsf_version.protocol != AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_ELRS) {
...
void AG_CRSF_Telem::queue_message(MAV_SEVERITY severity, const char *text)
{
    ...
    AG_RCTelemetry::queue_message(severity, text);
}

/*
  disable telemetry entries that require a transmitter to be present
 */
void AG_CRSF_Telem::disable_tx_entries()
{
 ...
 */
void AG_CRSF_Telem::enable_tx_entries()
{
...
void AG_CRSF_Telem::enter_scheduler_params_mode()
{
...
void AG_CRSF_Telem::exit_scheduler_params_mode()
{
...
void AG_CRSF_Telem::adjust_packet_weight(bool queue_empty)
{
    ...
    */
    ...
    if (!_custom_telem.params_mode_active
        ...
        && _pending_request.frame_type != AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO
...
// WFQ scheduler
bool AG_CRSF_Telem::is_packet_ready(uint8_t idx, bool queue_empty)
{
...
// WFQ scheduler
void AG_CRSF_Telem::process_packet(uint8_t idx)
{
    // send packet
    switch (idx) {
        ...
        case PASSTHROUGH:
            if (is_high_speed_telemetry(_telem_rf_mode)) {
                ...
                const uint8_t size = is_elrs() ? 3 : AG_CRSF_Telem::PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE;
        ...
        case VERSION_PING:
            ...
            if (_crsf_version.retry_count++ > CRSF_RX_DEVICE_PING_MAX_RETRY) {
                ...
                calc_device_ping(AG_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER);
...
bool AG_CRSF_Telem::_process_frame(AG_RCProtocol_CRSF::FrameType frame_type, void* data) {
    switch (frame_type) {
    // this means we are connected to an RC receiver and can send telemetry
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_RC_CHANNELS_PACKED:
    // the EVO sends battery frames and we should send telemetry back to populate the OSD
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_BATTERY_SENSOR:
        _enable_telemetry = true;
        break;

#if AP_VIDEOTX_ENABLED
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_VTX:
        process_vtx_frame((VTXFrame*)data);
        break;

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_VTX_TELEM:
        process_vtx_telem_frame((VTXTelemetryFrame*)data);
        break;
#endif

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_PING:
        process_ping_frame((ParameterPingFrame*)data);
        break;

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_READ:
        process_param_read_frame((ParameterSettingsReadFrame*)data);
        break;

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_WRITE:
        process_param_write_frame((ParameterSettingsWriteFrame*)data);
        break;

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO:
        process_device_info_frame((ParameterDeviceInfoFrame*)data);
        break;

    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND:
...
#if AP_VIDEOTX_ENABLED
void AG_CRSF_Telem::process_vtx_frame(VTXFrame* vtx) {
    ...
    debug("VTX: SmartAudio: %d, Avail: %d, FreqMode: %d, Band: %d, Channel: %d, Freq: %d, PitMode: %d, Pwr: %d, Pit: %d",
        ...
        vtx->band, vtx->channel, vtx->is_in_user_frequency_mode ? vtx->user_frequency : AG_VideoTX::get_frequency_mhz(vtx->band, vtx->channel),
        vtx->is_in_pitmode, vtx->power, vtx->pitmode);
    AG_VideoTX& apvtx = AP::vtx();
    ...
    if (vtx->is_in_user_frequency_mode) {
        ...
        apvtx.set_frequency_mhz(AG_VideoTX::get_frequency_mhz(vtx->band, vtx->channel));
    }
    ...
    if (vtx->is_in_pitmode) {
        apvtx.set_options(apvtx.get_options() | uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
    } else {
        apvtx.set_options(apvtx.get_options() & ~uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
    }
...
void AG_CRSF_Telem::process_vtx_telem_frame(VTXTelemetryFrame* vtx)
{
    ...
    AG_VideoTX::VideoBand band;
    uint8_t channel;
    if (AG_VideoTX::get_band_and_channel(vtx->frequency, band, channel)) {
...
void AG_CRSF_Telem::process_ping_frame(ParameterPingFrame* ping)
{
    debug("process_ping_frame: %d -> %d", ping->origin, ping->destination);
    if (ping->destination != 0 && ping->destination != AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
...
// request for device info
void AG_CRSF_Telem::process_device_info_frame(ParameterDeviceInfoFrame* info)
{
    debug("process_device_info_frame: 0x%x -> 0x%x", info->origin, info->destination);
    if (info->destination != 0 && info->destination != AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        return; // request was not for us
    }

    // we are only interested in RC device info for firmware version detection
    if (info->origin != 0 && info->origin != AG_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER) {
        return;
    }
    /*
        Payload size is 58:
    ...
    */
    ...
    if (strncmp((char*)info->payload, "Tracer", 6) == 0) {
        _crsf_version.protocol = AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_TRACER;
        ...
        _crsf_version.protocol = AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_ELRS;
    }
...
void AG_CRSF_Telem::process_command_frame(CommandFrame* command)
{
    debug("process_command_frame: 0x%x -> 0x%x: 0x%x", command->origin, command->destination, command->payload[0]);
    if (command->destination != 0 && command->destination != AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        return; // request was not for us
    }

    // we are only interested in commands from the RX
    if (command->origin != 0 && command->origin != AG_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER) {
        return;
    }

    switch (command->payload[0]) {
        case AG_RCProtocol_CRSF::CRSF_COMMAND_GENERAL_CRSF_SPEED_PROPOSAL: {
...
void AG_CRSF_Telem::process_param_read_frame(ParameterSettingsReadFrame* read_frame)
{
    ...
    if (read_frame->destination != 0 && read_frame->destination != AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        return; // request was not for us
    }

    _param_request = *read_frame;
    _pending_request.frame_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_READ;
}

// process any changed settings and schedule for transmission
void AG_CRSF_Telem::update()
{
}

void AG_CRSF_Telem::process_pending_requests()
{
    // handle general parameter requests
    switch (_pending_request.frame_type) {
    // construct a response to a ping frame
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO:
        _custom_telem.params_mode_start_ms = AG_HAL::millis();
        calc_device_info();
        break;
...
#if AP_VIDEOTX_ENABLED
void AG_CRSF_Telem::update_vtx_params()
{
    AG_VideoTX& vtx = AP::vtx();
    ...
    const bool pitmode = vtx.get_configured_options() & uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE);
    ...
    if (_vtx_freq_change_pending || _vtx_power_change_pending || _vtx_options_change_pending) {
        ...
        _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND;
        _telem.ext.command.destination = AG_RCProtocol_CRSF::CRSF_ADDRESS_VTX;
        _telem.ext.command.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
        _telem.ext.command.command_id = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX;

        uint8_t len = 5;
        // prioritize option changes so that the pilot can get in and out of pitmode
        if (_vtx_options_change_pending) {
            _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX_PITMODE;
            if (vtx.get_configured_options() & uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE)) {
                _telem.ext.command.payload[1] = 1;
            } else {
                _telem.ext.command.payload[1] = 0;
            }
        } else if (_vtx_freq_change_pending && _vtx_freq_update) {
            _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX_FREQ;
            ...
            _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX_CHANNEL;
            ...
            _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX_POWER_DBM;
            _telem.ext.command.payload[1] = vtx.get_configured_power_dbm();
            _vtx_dbm_update = false;
        } else if (_vtx_power_change_pending) {
            _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_VTX_POWER;
...
#pragma GCC diagnostic ignored "-Wstringop-overflow"
        ...
        uint8_t crc = crc8_dvb(0, AG_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND, 0xBA);
...
void AG_CRSF_Telem::calc_parameter_ping()
{
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_PING;
    _telem.ext.ping.destination = AG_RCProtocol_CRSF::CRSF_ADDRESS_VTX;
    _telem.ext.ping.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    _telem_size = sizeof(ParameterPingFrame);
    _telem_pending = true;
}

// prepare qos data - mandatory frame that must be sent periodically
void AG_CRSF_Telem::calc_heartbeat()
{
    _telem.bcast.heartbeat.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    _telem_size = sizeof(HeartbeatFrame);
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_HEARTBEAT;
    _telem_pending = true;
}

// prepare battery data
void AG_CRSF_Telem::calc_battery()
{
    const AG_BattMonitor &_battery = AP::battery();
    ...
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_BATTERY_SENSOR;

    _telem_pending = true;
}

// prepare gps data
void AG_CRSF_Telem::calc_gps()
{
    ...
    _telem_size = sizeof(AG_CRSF_Telem::GPSFrame);
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_GPS;

    _telem_pending = true;
}

// prepare attitude data
void AG_CRSF_Telem::calc_attitude()
{
    ...
    _telem_size = sizeof(AG_CRSF_Telem::AttitudeFrame);
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_ATTITUDE;

    _telem_pending = true;
}

// prepare flight mode data
void AG_CRSF_Telem::calc_flight_mode()
{
    AG_Notify * notify = AG_Notify::get_singleton();
    if (notify) {
        // Note: snprintf() always terminates the string
        hal.util->snprintf(
            _telem.bcast.flightmode.flight_mode, 
            sizeof(AG_CRSF_Telem::FlightModeFrame), 
            "%s%s", 
        ...
        _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_FLIGHT_MODE;
        _telem_pending = true;
    }
}

// return device information about ArduPilot
void AG_CRSF_Telem::calc_device_info() {
#if !APM_BUILD_TYPE(APM_BUILD_UNKNOWN)
    _telem.ext.info.destination = _param_request.origin;
    _telem.ext.info.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;

    const AG_FWVersion &fwver = AP::fwversion();
    ...
    _telem.ext.info.payload[n++] = AG_OSD_ParamScreen::NUM_PARAMS * AG_OSD_NUM_PARAM_SCREENS; // param count
#else
    ...
    _telem_size = n + 2;
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO;
...
// send a device ping
void AG_CRSF_Telem::calc_device_ping(uint8_t destination) {
...
void AG_CRSF_Telem::calc_command_response() {
    _telem.ext.command.destination = AG_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER;
    _telem.ext.command.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    _telem.ext.command.command_id = AG_RCProtocol_CRSF::CRSF_COMMAND_GENERAL;
    _telem.ext.command.payload[0] = AG_RCProtocol_CRSF::CRSF_COMMAND_GENERAL_CRSF_SPEED_RESPONSE;
...
void AG_CRSF_Telem::calc_parameter() {
#if OSD_PARAM_ENABLED
    _telem.ext.param_entry.header.destination = _param_request.origin;
    _telem.ext.param_entry.header.origin = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    size_t idx = 0;
    ...
    if (_param_request.param_num == 0) {
        ...
        for (uint8_t i = 0; i < AG_OSD_ParamScreen::NUM_PARAMS * AG_OSD_NUM_PARAM_SCREENS; i++) {
            _telem.ext.param_entry.payload[idx++] = i + 1;
        }
        _telem.ext.param_entry.payload[idx] = 0xFF; // terminator

        _telem_size = sizeof(AG_CRSF_Telem::ParameterSettingsEntryHeader) + 1 + idx;
        _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY;
        _pending_request.frame_type = 0;
        _telem_pending = true;
        return;
    }
    ...
    AG_OSD_ParamSetting* param = osd->get_setting((_param_request.param_num - 1) / AG_OSD_ParamScreen::NUM_PARAMS,
        (_param_request.param_num - 1) % AG_OSD_ParamScreen::NUM_PARAMS);
...
void AG_CRSF_Telem::calc_text_selection(AG_OSD_ParamSetting* param, uint8_t chunk)
{
    const uint8_t CHUNK_SIZE = 56;
    const AG_OSD_ParamSetting::ParamMetadata* metadata = param->get_custom_metadata();
    ...
    _telem_size = sizeof(AG_CRSF_Telem::ParameterSettingsEntryHeader) + chunker.bytes_written();
    _telem_type = AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY;
...
void AG_CRSF_Telem::process_param_write_frame(ParameterSettingsWriteFrame* write_frame)
{
    debug("process_param_write_frame: %d -> %d", write_frame->origin, write_frame->destination);
    if (write_frame->destination != AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        return; // request was not for us
    }
#if OSD_PARAM_ENABLED
    AG_OSD* osd = AP::osd();
    ...
    AG_OSD_ParamSetting* param = osd->get_setting((write_frame->param_num - 1) / AG_OSD_ParamScreen::NUM_PARAMS,
        (write_frame->param_num - 1) % AG_OSD_ParamScreen::NUM_PARAMS);
...
void AG_CRSF_Telem::calc_status_text()
{
    ...
    _telem.bcast.custom_telem.status_text.sub_type = AG_RCProtocol_CRSF::CustomTelemSubTypeID::CRSF_AP_CUSTOM_TELEM_STATUS_TEXT;
    _telem.bcast.custom_telem.status_text.severity = _statustext.next.severity;
    // Note: snprintf() always terminates the string
    hal.util->snprintf(_telem.bcast.custom_telem.status_text.text, AG_CRSF_Telem::PASSTHROUGH_STATUS_TEXT_FRAME_MAX_SIZE, "%s", _statustext.next.text);
...
*/
void AG_CRSF_Telem::get_single_packet_passthrough_telem_data()
{
    _telem_pending = false;
    uint8_t packet_count;
    AG_Frsky_SPort::sport_packet_t packet;
    if (!AG_Frsky_Telem::get_telem_data(&packet, packet_count, 1)) {
        return;
    }
    _telem.bcast.custom_telem.single_packet_passthrough.sub_type = AG_RCProtocol_CRSF::CustomTelemSubTypeID::CRSF_AP_CUSTOM_TELEM_SINGLE_PACKET_PASSTHROUGH;
    ...
    _telem_size = sizeof(AG_CRSF_Telem::PassthroughSinglePacketFrame);
    _telem_type = get_custom_telem_frame_id();
    _telem_pending = true;
}
...
*/
void AG_CRSF_Telem::get_multi_packet_passthrough_telem_data(uint8_t size)
{
    size = MIN(size, AG_CRSF_Telem::PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE);
    _telem_pending = false;
    uint8_t count = 0;
    AG_Frsky_SPort::sport_packet_t buffer[AG_CRSF_Telem::PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE] {};
    // we request a PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE packet array, i.e. 9 packets
    if (!AG_Frsky_Telem::get_telem_data(buffer, count, size)) {
        return;
    }
    _telem.bcast.custom_telem.multi_packet_passthrough.sub_type = AG_RCProtocol_CRSF::CustomTelemSubTypeID::CRSF_AP_CUSTOM_TELEM_MULTI_PACKET_PASSTHROUGH;
    ...
    _telem_size = 2 + sizeof(AG_CRSF_Telem::PassthroughMultiPacketFrame::PassthroughTelemetryPacket)*count; //subtype + size + 6*count
    _telem_type = get_custom_telem_frame_id();
    _telem_pending = true;
}
 ...
 */
bool AG_CRSF_Telem::_get_telem_data(AG_RCProtocol_CRSF::Frame* data, bool is_tx_active)
{
    ...
    data->device_address = AG_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;  // sync byte
 ...
 */
bool AG_CRSF_Telem::process_frame(AG_RCProtocol_CRSF::FrameType frame_type, void* data)
{
    if (!get_singleton()) {
        return false;
    }
...
/*
  fetch data for an external transport, such as CRSF
 */
bool AG_CRSF_Telem::get_telem_data(AG_RCProtocol_CRSF::Frame* data, bool is_tx_active)
{
    if (!get_singleton()) {
        return false;
    }
...
AG_CRSF_Telem *AG_CRSF_Telem::get_singleton(void) {
    if (!singleton && !hal.util->get_soft_armed()) {
        // if telem data is requested when we are disarmed and don't
        // yet have a AG_CRSF_Telem object then try to allocate one
        new AG_CRSF_Telem();
...
namespace AP {
    AG_CRSF_Telem *crsf_telem() {
        return AG_CRSF_Telem::get_singleton();
    }
};
