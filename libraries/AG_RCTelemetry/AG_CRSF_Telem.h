...
#include "AG_RCTelemetry_config.h"

#if HAL_CRSF_TELEM_ENABLED

#include <AG_OSD/AG_OSD.h>
#include <AG_RCProtocol/AG_RCProtocol_CRSF.h>
#include "AG_RCTelemetry.h"
#include <AG_HAL/utility/sparse-endian.h>

class AG_OSD_ParamSetting;

class AG_CRSF_Telem : public AG_RCTelemetry {
public:
    AG_CRSF_Telem();
    ~AG_CRSF_Telem() override;

    /* Do not allow copies */
    CLASS_NO_COPY(AG_CRSF_Telem);

    // init - perform required initialisation
    virtual bool init() override;

    static AG_CRSF_Telem *get_singleton(void);
...
#error "Only supported on little-endian architectures"
    ...
    bool is_elrs() const { return _crsf_version.protocol == AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_ELRS; }
    // is the current protocol Tracer?
    bool is_tracer() const { return _crsf_version.protocol == AG_RCProtocol_CRSF::ProtocolType::PROTOCOL_TRACER; }

    // Process a frame from the CRSF protocol decoder
    static bool process_frame(AG_RCProtocol_CRSF::FrameType frame_type, void* data);
    // process any changed settings and schedule for transmission
    void update();
    // get next telemetry data for external consumers of SPort data
    static bool get_telem_data(AG_RCProtocol_CRSF::Frame* frame, bool is_tx_active);
...
private:
    ...
    void update_custom_telemetry_rates(const AG_RCProtocol_CRSF::RFMode rf_mode);
    ...
    void calc_text_selection( AG_OSD_ParamSetting* param, uint8_t chunk);
#endif
    ...
    AG_RCProtocol_CRSF::RFMode get_rf_mode() const;
    uint16_t get_telemetry_rate() const;
    bool is_high_speed_telemetry(const AG_RCProtocol_CRSF::RFMode rf_mode) const;
    ...
    bool _get_telem_data(AG_RCProtocol_CRSF::Frame* data, bool is_tx_active);
    bool _process_frame(AG_RCProtocol_CRSF::FrameType frame_type, void* data);

    TelemetryPayload _telem;
    uint8_t _telem_size;
    uint8_t _telem_type;
    AG_RCProtocol_CRSF::RFMode _telem_rf_mode;
    ...
    struct {
        uint8_t destination = AG_RCProtocol_CRSF::CRSF_ADDRESS_BROADCAST;
        uint8_t frame_type;
    } _pending_request;
    ...
    struct {
        ...
        uint8_t retry_count;
        bool use_rf_mode;
        AG_RCProtocol_CRSF::ProtocolType protocol;
        bool pending = true;
    } _crsf_version;
    ...
    static AG_CRSF_Telem *singleton;
};
