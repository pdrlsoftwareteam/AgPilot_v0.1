...
/*
   Spektrum Telemetry library, based on AG_Frsky_Telem.cpp
   See https://www.spektrumrc.com/ProdInfo/Files/SPM_Telemetry_Developers_Specs.pdf
*/

#include "AG_Spektrum_Telem.h"
...
AG_Spektrum_Telem *AG_Spektrum_Telem::singleton;

AG_Spektrum_Telem::AG_Spektrum_Telem() : AG_RCTelemetry(0)
{
    singleton = this;
}

AG_Spektrum_Telem::~AG_Spektrum_Telem(void)
{
    singleton = nullptr;
}

bool AG_Spektrum_Telem::init(void)
{
 ...
 */
void AG_Spektrum_Telem::setup_wfq_scheduler(void)
{
...
void AG_Spektrum_Telem::adjust_packet_weight(bool queue_empty)
{
    if (!queue_empty) {
...
// WFQ scheduler
bool AG_Spektrum_Telem::is_packet_ready(uint8_t idx, bool queue_empty)
{
...
// WFQ scheduler
void AG_Spektrum_Telem::process_packet(uint8_t idx)
{
    // send packet
    switch (idx) {
        case QOS: // QOS
...
bool AG_Spektrum_Telem::repeat_msg_chunk(void)
{
...
bool AG_Spektrum_Telem::get_next_msg_chunk(void)
{
    _msg_chunk.repeats++;
...
void AG_Spektrum_Telem::calc_qos()
{
...
void AG_Spektrum_Telem::calc_rpm()
{
...
void AG_Spektrum_Telem::send_msg_chunk(const MessageChunk& chunk)
{
...
void AG_Spektrum_Telem::calc_batt_volts(uint8_t instance)
{
    const AG_BattMonitor &_battery = AP::battery();
...
// prepare battery data - B/E but not supported by Spektrum
void AG_Spektrum_Telem::calc_batt_amps(uint8_t instance)
{
    const AG_BattMonitor &_battery = AP::battery();
...
void AG_Spektrum_Telem::calc_batt_mah()
{
    const AG_BattMonitor &_battery = AP::battery();
...
void AG_Spektrum_Telem::calc_temperature(uint8_t instance)
{
...
void AG_Spektrum_Telem::calc_altitude()
{
...
void AG_Spektrum_Telem::calc_airspeed()
{
    ...
    const AG_Airspeed *airspeed = AP::airspeed();
...
void AG_Spektrum_Telem::calc_attandmag(void)
{
...
void AG_Spektrum_Telem::calc_gps_location()
{
    ...
    if (AP::gps().status(0) >= AG_GPS::GPS_OK_FIX_3D) {
    ...
    if (AP::gps().status(0) > AG_GPS::NO_FIX) {
...
void AG_Spektrum_Telem::calc_gps_status()
{
...
void AG_Spektrum_Telem::calc_esc()
{
#ifdef HAVE_AP_BLHELI_SUPPORT
 ...
 */
bool AG_Spektrum_Telem::_get_telem_data(uint8_t* data)
{
    memset(&_telem, 0, 16);
    run_wfq_scheduler();
    if (!_telem_pending) {
 ...
 */
bool AG_Spektrum_Telem::get_telem_data(uint8_t* data)
{
    if (!singleton && !hal.util->get_soft_armed()) {
        // if telem data is requested when we are disarmed and don't
        // yet have a AG_Spektrum_Telem object then try to allocate one
        new AG_Spektrum_Telem();
...
namespace AP {
    AG_Spektrum_Telem *spektrum_telem() {
        return AG_Spektrum_Telem::get_singleton();
    }
};
