...
void AG_RCTelemetry::update_avg_packet_rate()
{
    uint32_t poll_now = AG_HAL::millis();
 ...
 */
uint8_t AG_RCTelemetry::run_wfq_scheduler(const bool use_shaper)
{
 ...
 */
bool AG_RCTelemetry::process_scheduler_entry(const uint8_t slot )
{
 ...
 */
void AG_RCTelemetry::queue_message(MAV_SEVERITY severity, const char *text)
{
 ...
 */
void AG_RCTelemetry::check_sensor_status_flags(void)
{
    uint32_t now = AG_HAL::millis();
 ...
 */
void AG_RCTelemetry::check_ekf_status(void)
{
    // get variances
    bool get_variance;
...
uint32_t AG_RCTelemetry::sensor_status_flags() const
{
    uint32_t present;
    uint32_t enabled;
    uint32_t health;
