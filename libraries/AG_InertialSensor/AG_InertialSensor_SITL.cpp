...
AG_InertialSensor_SITL::AG_InertialSensor_SITL(AG_InertialSensor &imu, const uint16_t sample_rates[]) :
...
/*
  detect the sensor
 */
AG_InertialSensor_Backend *AG_InertialSensor_SITL::detect(AG_InertialSensor &_imu, const uint16_t sample_rates[])
{
    AG_InertialSensor_SITL *sensor = new AG_InertialSensor_SITL(_imu, sample_rates);
    if (sensor == nullptr) {
        return nullptr;
    }
...
bool AG_InertialSensor_SITL::init_sensor(void)
{
    sitl = AP::sitl();
    if (sitl == nullptr) {
        return false;
...
float AG_InertialSensor_SITL::get_temperature(void)
{
    ...
    const float tsec = (AG_HAL::millis() - temp_start_ms) * 0.001f;
 ...
 */
void AG_InertialSensor_SITL::generate_accel()
{
    ...
    _notify_new_accel_raw_sample(accel_instance, accel_accum, AG_HAL::micros64());
 ...
 */
void AG_InertialSensor_SITL::generate_gyro()
{
    Vector3f gyro_accum;
    ...
    _notify_new_gyro_raw_sample(gyro_instance, gyro_accum, AG_HAL::micros64());
}

void AG_InertialSensor_SITL::timer_update(void)
{
...
float AG_InertialSensor_SITL::gyro_drift(void) const
{
    ...
    double minutes = fmod(AG_HAL::micros64() / 60.0e6, period);
    if (minutes < period/2) {
...
bool AG_InertialSensor_SITL::update(void) 
{
...
uint8_t AG_InertialSensor_SITL::bus_id = 0;

void AG_InertialSensor_SITL::start()
{
    if (!_imu.register_gyro(gyro_instance, gyro_sample_hz,
                            AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_SITL, bus_id, 1, DEVTYPE_SITL)) ||
        !_imu.register_accel(accel_instance, accel_sample_hz,
                             AG_HAL::Device::make_bus_id(AG_HAL::Device::BUS_TYPE_SITL, bus_id, 2, DEVTYPE_SITL))) {
        return;
    }
    bus_id++;
    hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&AG_InertialSensor_SITL::timer_update, void));
 ...
 */
#if AP_SIM_INS_FILE_ENABLED
void AG_InertialSensor_SITL::read_gyro_from_file()
{
    if (gyro_fd == -1) {
        char namebuf[32];
        ...
        if (gyro_fd == -1) {
            AG_HAL::panic("gyro data file %s not found", namebuf);
        }
    }
...
void AG_InertialSensor_SITL::read_gyro(const float* buf, uint8_t nsamples)
{
    Vector3f gyro_accum;
...
void AG_InertialSensor_SITL::write_gyro_to_file(const Vector3f& gyro)
{
    if (gyro_fd == -1) {
        char namebuf[32];
        snprintf(namebuf, 32, "/tmp/gyro%d.dat", gyro_instance);
...
void AG_InertialSensor_SITL::read_accel_from_file()
{
    if (accel_fd == -1) {
        char namebuf[32];
        ...
        if (accel_fd == -1) {
            AG_HAL::panic("accel data file %s not found", namebuf);
        }
    }
...
void AG_InertialSensor_SITL::read_accel(const float* buf, uint8_t nsamples)
{
    Vector3f accel_accum;
...
void AG_InertialSensor_SITL::write_accel_to_file(const Vector3f& accel)
{
