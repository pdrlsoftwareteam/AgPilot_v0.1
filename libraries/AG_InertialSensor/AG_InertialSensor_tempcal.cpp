...
const AG_Param::GroupInfo AG_InertialSensor_TCal::var_info[] = {
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_InertialSensor_TCal, enable,  float(Enable::Disabled), AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    // @Calibration: 1
    AP_GROUPINFO("TMIN", 2, AG_InertialSensor_TCal, temp_min,  0),
    ...
    // @User: Advanced
    // @Calibration: 1
    AP_GROUPINFO("TMAX", 3, AG_InertialSensor_TCal, temp_max,  70),
    ...
    AP_GROUPINFO("ACC1", 4, AG_InertialSensor_TCal, accel_coeff[0], 0),
    ...
    AP_GROUPINFO("ACC2", 5, AG_InertialSensor_TCal, accel_coeff[1], 0),
    ...
    AP_GROUPINFO("ACC3", 6, AG_InertialSensor_TCal, accel_coeff[2], 0),
    ...
    AP_GROUPINFO("GYR1", 7, AG_InertialSensor_TCal, gyro_coeff[0], 0),
    ...
    AP_GROUPINFO("GYR2", 8, AG_InertialSensor_TCal, gyro_coeff[1], 0),
    ...
    AP_GROUPINFO("GYR3", 9, AG_InertialSensor_TCal, gyro_coeff[2], 0),
 ...
 */
Vector3f AG_InertialSensor_TCal::polynomial_eval(float tdiff, const AP_Vector3f coeff[3]) const
{
 ...
 */
void AG_InertialSensor_TCal::correct_sensor(float temperature, float cal_temp, const AP_Vector3f coeff[3], Vector3f &v) const
{
...
void AG_InertialSensor_TCal::correct_accel(float temperature, float cal_temp, Vector3f &accel) const
{
    correct_sensor(temperature, cal_temp, accel_coeff, accel);
}

void AG_InertialSensor_TCal::correct_gyro(float temperature, float cal_temp, Vector3f &gyro) const
{
 ...
 */
void AG_InertialSensor_TCal::sitl_apply_accel(float temperature, Vector3f &accel) const
{
    const float tmid = 0.5*(temp_max+temp_min);
    accel += polynomial_eval(temperature - tmid, accel_coeff);
}

void AG_InertialSensor_TCal::sitl_apply_gyro(float temperature, Vector3f &gyro) const
{
    const float tmid = 0.5*(temp_max+temp_min);
    gyro += polynomial_eval(temperature - tmid, gyro_coeff);
}

AG_InertialSensor_TCal::Learn::Learn(AG_InertialSensor_TCal &_tcal, float _start_temp) :
 ...
 */
void AG_InertialSensor_TCal::Learn::add_sample(const Vector3f &sample, float temperature, struct LearnState &st)
{
 ...
 */
void AG_InertialSensor_TCal::update_accel_learning(const Vector3f &accel, float temperature)
{
    ...
    if (learn == nullptr && hal.scheduler->is_system_initialized()) {
        ...
        if (learn) {
            ...
            AG_Notify::events.initiated_temp_cal = 1;
        }
    }
    if (learn != nullptr) {
        AG_Notify::flags.temp_cal_running = true;
 ...
 */
void AG_InertialSensor_TCal::update_gyro_learning(const Vector3f &gyro, float temperature)
{
    if (enable != Enable::LearnCalibration) {
        return;
    }
...
/*
  reset calibration
 */
void AG_InertialSensor_TCal::Learn::reset(float temperature)
{
 ...
 */
void AG_InertialSensor_TCal::Learn::finish_calibration(float temperature)
{
    if (!save_calibration(temperature)) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "TCAL[%u]: failed fit", instance()+1);
        AG_Notify::events.temp_cal_failed = 1;
        tcal.enable.set_and_save_ifchanged(int8_t(AG_InertialSensor_TCal::Enable::Disabled));
        return;
    }
    GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "TCAL[%u]: completed calibration tmin=%.1f tmax=%.1f",
                  instance()+1,
                  tcal.temp_min.get(), tcal.temp_max.get());
    tcal.enable.set_and_save_ifchanged(int8_t(AG_InertialSensor_TCal::Enable::Enabled));
}

/*
  save calibration state
 */
bool AG_InertialSensor_TCal::Learn::save_calibration(float temperature)
{
...
uint8_t AG_InertialSensor_TCal::instance(void) const
{
...
*/
void AG_InertialSensor_TCal::get_persistent_params(ExpandingString &str) const
{
    if (enable != AG_InertialSensor_TCal::Enable::Enabled) {
        return;
    }
...
*/
void AG_InertialSensor::get_persistent_params(ExpandingString &str) const
{
