...
#include "AG_InertialSensor_Invensensev2_registers.h"
...
AG_InertialSensor_Invensensev2::AG_InertialSensor_Invensensev2(AG_InertialSensor &imu,
...
AG_InertialSensor_Invensensev2::~AG_InertialSensor_Invensensev2()
{
    if (_fifo_buffer != nullptr) {
        hal.util->free_type(_fifo_buffer, INV2_FIFO_BUFFER_LEN * INV2_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    }
    _dev->deregister_bankselect_callback();
    //delete _auxiliary_bus;
}

AG_InertialSensor_Backend *AG_InertialSensor_Invensensev2::probe(AG_InertialSensor &imu,
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    AG_InertialSensor_Invensensev2 *sensor =
        new AG_InertialSensor_Invensensev2(imu, std::move(dev), rotation);
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    sensor = new AG_InertialSensor_Invensensev2(imu, std::move(dev), rotation);
    if (!sensor || !sensor->_init()) {
        delete sensor;
        return nullptr;
    }
...
bool AG_InertialSensor_Invensensev2::_init()
{
...
void AG_InertialSensor_Invensensev2::_fifo_reset()
{
...
bool AG_InertialSensor_Invensensev2::_has_auxiliary_bus()
{
    return _dev->bus_type() != AG_HAL::Device::BUS_TYPE_I2C;
}

void AG_InertialSensor_Invensensev2::start()
{
    ...
    _fifo_buffer = (uint8_t *)hal.util->malloc_type(INV2_FIFO_BUFFER_LEN * INV2_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    if (_fifo_buffer == nullptr) {
        AG_HAL::panic("Invensense: Unable to allocate FIFO buffer");
    }

    // start the timer process to read samples
    _dev->register_periodic_callback(1265625UL / _gyro_backend_rate_hz, FUNCTOR_BIND_MEMBER(&AG_InertialSensor_Invensensev2::_poll_data, void));
}

// get a startup banner to output to the GCS
bool AG_InertialSensor_Invensensev2::get_output_banner(char* banner, uint8_t banner_len) {
    if (_fast_sampling) {
 ...
 */
bool AG_InertialSensor_Invensensev2::update()
{
 ...
 */
void AG_InertialSensor_Invensensev2::accumulate()
{
    // nothing to do
}

AuxiliaryBus *AG_InertialSensor_Invensensev2::get_auxiliary_bus()
{
    if (_auxiliary_bus) {
 ...
 * status register.
 */
bool AG_InertialSensor_Invensensev2::_data_ready()
{
    if (_drdy_pin) {
 ...
 */
void AG_InertialSensor_Invensensev2::_poll_data()
{
    _read_fifo();
}

bool AG_InertialSensor_Invensensev2::_accumulate(uint8_t *samples, uint8_t n_samples)
{
 ...
 */
bool AG_InertialSensor_Invensensev2::_accumulate_sensor_rate_sampling(uint8_t *samples, uint8_t n_samples)
{
    int32_t tsum = 0;
...
void AG_InertialSensor_Invensensev2::_read_fifo()
{
    uint8_t n_samples;
    uint16_t bytes_read;
...
*/
bool AG_InertialSensor_Invensensev2::_check_raw_temp(int16_t t2)
{
...
bool AG_InertialSensor_Invensensev2::_block_read(uint16_t reg, uint8_t *buf,
                                            uint32_t size)
{
    return _dev->read_bank_registers(GET_BANK(reg), GET_REG(reg), buf, size);
}

uint8_t AG_InertialSensor_Invensensev2::_register_read(uint16_t reg)
{
    uint8_t val = 0;
...
void AG_InertialSensor_Invensensev2::_register_write(uint16_t reg, uint8_t val, bool checked)
{
    _dev->write_bank_register(GET_BANK(reg), GET_REG(reg), val, checked);
}

bool AG_InertialSensor_Invensensev2::_select_bank(uint8_t bank)
{
 ...
 */
void AG_InertialSensor_Invensensev2::_set_filter_and_scaling(void)
{
 ...
 */
bool AG_InertialSensor_Invensensev2::_check_whoami(void)
{
...
bool AG_InertialSensor_Invensensev2::_hardware_init(void)
{
    WITH_SEMAPHORE(_dev->get_semaphore());

    // disabled setup of checked registers as it can't cope with bank switching
    _dev->setup_checked_registers(7, _dev->bus_type() == AG_HAL::Device::BUS_TYPE_I2C?200:20);
    _dev->setup_bankselect_callback(FUNCTOR_BIND_MEMBER(&AG_InertialSensor_Invensensev2::_select_bank, bool, uint8_t));
...
int AP_Invensensev2_AuxiliaryBusSlave::_set_passthrough(uint8_t reg, uint8_t size,
                                                  uint8_t *out)
{
    auto &backend = AG_InertialSensor_Invensensev2::from(_bus.get_backend());
    uint8_t addr;
...
AP_Invensensev2_AuxiliaryBus::AP_Invensensev2_AuxiliaryBus(AG_InertialSensor_Invensensev2 &backend, uint32_t devid)
    : AuxiliaryBus(backend, 4, devid)
{
}

AG_HAL::Semaphore *AP_Invensensev2_AuxiliaryBus::get_semaphore()
{
    return static_cast<AG_InertialSensor_Invensensev2&>(_ins_backend)._dev->get_semaphore();
}
...
{
    auto &backend = AG_InertialSensor_Invensensev2::from(_ins_backend);
...
AG_HAL::Device::PeriodicHandle AP_Invensensev2_AuxiliaryBus::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
    auto &backend = AG_InertialSensor_Invensensev2::from(_ins_backend);
