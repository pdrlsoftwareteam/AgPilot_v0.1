...
{
public:
    AG_InertialSensor_Backend(AG_InertialSensor &imu);
    AG_InertialSensor_Backend(const AG_InertialSensor_Backend &that) = delete;

    // we declare a virtual destructor so that drivers can
    // override with a custom destructor if need be.
    virtual ~AG_InertialSensor_Backend(void) {}
    ...
    virtual void handle_external(const AG_ExternalAHRS::ins_data_message_t &pkt) {}
#endif
...
protected:
    ...
    bool sensors_converging() const { return AG_HAL::millis() < HAL_INS_CONVERGANCE_MS; }
    ...
    */
    ...
    void log_register_change(uint32_t bus_id, const AG_HAL::Device::checkreg &reg) __RAMFUNC__;
