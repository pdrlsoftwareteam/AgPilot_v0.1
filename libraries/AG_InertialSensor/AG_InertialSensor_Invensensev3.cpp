...
AG_InertialSensor_Invensensev3::AG_InertialSensor_Invensensev3(AG_InertialSensor &imu,
...
AG_InertialSensor_Invensensev3::~AG_InertialSensor_Invensensev3()
{
    if (fifo_buffer != nullptr) {
        hal.util->free_type((void*)fifo_buffer, INV3_FIFO_BUFFER_LEN * INV3_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    }
}

AG_InertialSensor_Backend *AG_InertialSensor_Invensensev3::probe(AG_InertialSensor &imu,
                                                                 AG_HAL::OwnPtr<AG_HAL::Device> _dev,
                                                                 enum Rotation _rotation)
{
    if (!_dev) {
    ...
    AG_InertialSensor_Invensensev3 *sensor =
        new AG_InertialSensor_Invensensev3(imu, std::move(_dev), _rotation);
...
void AG_InertialSensor_Invensensev3::fifo_reset()
{
...
void AG_InertialSensor_Invensensev3::start()
{
    WITH_SEMAPHORE(dev->get_semaphore());

    // initially run the bus at low speed
    dev->set_speed(AG_HAL::Device::SPEED_LOW);
    ...
    dev->set_speed(AG_HAL::Device::SPEED_HIGH);
    ...
    fifo_buffer = (FIFOData *)hal.util->malloc_type(INV3_FIFO_BUFFER_LEN * INV3_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    if (fifo_buffer == nullptr) {
        AG_HAL::panic("Invensensev3: Unable to allocate FIFO buffer");
    }

    // start the timer process to read samples, using the fastest rate avilable
    periodic_handle = dev->register_periodic_callback(backend_period_us, FUNCTOR_BIND_MEMBER(&AG_InertialSensor_Invensensev3::read_fifo, void));
}

// get a startup banner to output to the GCS
bool AG_InertialSensor_Invensensev3::get_output_banner(char* banner, uint8_t banner_len) {
    if (fast_sampling) {
 ...
 */
bool AG_InertialSensor_Invensensev3::update()
{
 ...
 */
void AG_InertialSensor_Invensensev3::accumulate()
{
    // nothing to do
}

bool AG_InertialSensor_Invensensev3::accumulate_samples(const FIFOData *data, uint8_t n_samples)
{
 ...
 */
void AG_InertialSensor_Invensensev3::read_fifo()
{
...
bool AG_InertialSensor_Invensensev3::block_read(uint8_t reg, uint8_t *buf, uint32_t size)
{
    return dev->read_registers(reg, buf, size);
}

uint8_t AG_InertialSensor_Invensensev3::register_read(uint8_t reg)
{
    uint8_t val = 0;
...
void AG_InertialSensor_Invensensev3::register_write(uint8_t reg, uint8_t val, bool checked)
{
 ...
 */
uint8_t AG_InertialSensor_Invensensev3::register_read_bank(uint8_t bank, uint8_t reg)
{
    if (inv3_type == Invensensev3_Type::ICM42670) {
 ...
 */
void AG_InertialSensor_Invensensev3::register_write_bank(uint8_t bank, uint8_t reg, uint8_t val)
{
    if (inv3_type == Invensensev3_Type::ICM42670) {
        // the ICM42670 has a complex bank setup
...
uint16_t AG_InertialSensor_Invensensev3::calculate_fast_sampling_backend_rate(uint16_t base_odr, uint16_t max_odr) const
{
 ...
 */
void AG_InertialSensor_Invensensev3::set_filter_and_scaling(void)
{
    // 1KHz by default
    ...
    if (enable_fast_sampling(accel_instance) && get_fast_sampling_rate() > 1) {
        fast_sampling = dev->bus_type() == AG_HAL::Device::BUS_TYPE_SPI;
 ...
 */
void AG_InertialSensor_Invensensev3::set_filter_and_scaling_icm42670(void)
{
 ...
 */
void AG_InertialSensor_Invensensev3::set_filter_and_scaling_icm456xy(void)
{
 ...
 */
bool AG_InertialSensor_Invensensev3::check_whoami(void)
{
...
uint8_t AG_InertialSensor_Invensensev3::register_read_bank_icm456xy(uint16_t bank_addr, uint16_t reg)
{
    // combine addr
...
void AG_InertialSensor_Invensensev3::register_write_bank_icm456xy(uint16_t bank_addr, uint16_t reg, uint8_t val)
{
    // combine addr
    uint16_t addr = bank_addr + reg;
...
bool AG_InertialSensor_Invensensev3::hardware_init(void)
{
    WITH_SEMAPHORE(dev->get_semaphore());

    dev->setup_checked_registers(8, dev->bus_type() == AG_HAL::Device::BUS_TYPE_I2C?200:20);
