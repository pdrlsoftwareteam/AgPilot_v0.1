...
AG_InertialSensor_BMI160::AG_InertialSensor_BMI160(AG_InertialSensor &imu,
...
AG_InertialSensor_BMI160::probe(AG_InertialSensor &imu,
                                ...
                                enum Rotation rotation)
{
    ...
    auto sensor = new AG_InertialSensor_BMI160(imu, std::move(dev), rotation);
...
void AG_InertialSensor_BMI160::start()
{
    bool r;
    ...
    r = _configure_accel();
    if (!r) {
        AG_HAL::panic("BMI160: Unable to configure accelerometer");
    }

    r = _configure_gyro();
    if (!r) {
        AG_HAL::panic("BMI160: Unable to configure gyroscope");
    }

    r = _configure_fifo();
    if (!r) {
        AG_HAL::panic("BMI160: Unable to configure FIFO");
    }

    if (BMI160_INT1_GPIO >= 0) {
        r = _configure_int1_pin();
        if (!r) {
            AG_HAL::panic("BMI160: unable to configure INT1 pin");
        }
    }
    ...
    _dev->register_periodic_callback(1000,
        FUNCTOR_BIND_MEMBER(&AG_InertialSensor_BMI160::_poll_data, void));
}

bool AG_InertialSensor_BMI160::update()
{
...
void AG_InertialSensor_BMI160::_check_err_reg()
{
#ifdef BMI160_DEBUG
    uint8_t v;
    bool r;
    ...
    if (!r) {
        AG_HAL::panic("BMI160: couldn't read ERR_REG\n");
    }
    if (v) {
        AG_HAL::panic("BMI160: error detected on ERR_REG\n");
    }
#endif
}

bool AG_InertialSensor_BMI160::_configure_accel()
{
    bool r;
...
bool AG_InertialSensor_BMI160::_configure_gyro()
{
    bool r;
...
bool AG_InertialSensor_BMI160::_configure_int1_pin()
{
    bool r;
...
bool AG_InertialSensor_BMI160::_configure_fifo()
{
    bool r;
...
void AG_InertialSensor_BMI160::_read_fifo()
{
...
void AG_InertialSensor_BMI160::_poll_data()
{
    _read_fifo();
}

bool AG_InertialSensor_BMI160::_hardware_init()
{
    bool ret = false;
...
bool AG_InertialSensor_BMI160::_init()
{
    bool ret = false;
