...
AG_InertialSensor_Backend::AG_InertialSensor_Backend(AG_InertialSensor &imu) :
    _imu(imu)
{
}
 ...
 */
void AG_InertialSensor_Backend::notify_accel_fifo_reset(uint8_t instance)
{
...
/*
  notify of a FIFO reset so we don't use bad data to update observed sensor rate
 */
void AG_InertialSensor_Backend::notify_gyro_fifo_reset(uint8_t instance)
{
...
void AG_InertialSensor_Backend::_set_accel_oversampling(uint8_t instance, uint8_t n)
{
    _imu._accel_over_sampling[instance] = n;
}

// set the amount of oversamping a gyro is doing
void AG_InertialSensor_Backend::_set_gyro_oversampling(uint8_t instance, uint8_t n)
{
...
*/
void AG_InertialSensor_Backend::_update_sensor_rate(uint16_t &count, uint32_t &start_us, float &rate_hz) const
{
...
void AG_InertialSensor_Backend::_rotate_and_correct_accel(uint8_t instance, Vector3f &accel) 
{
    /*
...
void AG_InertialSensor_Backend::_rotate_and_correct_gyro(uint8_t instance, Vector3f &gyro) 
{
    // rotate for sensor orientation
 ...
 */
void AG_InertialSensor_Backend::_publish_gyro(uint8_t instance, const Vector3f &gyro) /* front end */
{
...
void AG_InertialSensor_Backend::save_gyro_window(const uint8_t instance, const Vector3f &gyro, uint8_t phase)
{
#if HAL_WITH_DSP
 ...
 */
void AG_InertialSensor_Backend::apply_gyro_filters(const uint8_t instance, const Vector3f &gyro)
{
...
void AG_InertialSensor_Backend::_notify_new_gyro_raw_sample(uint8_t instance,
                                                            const Vector3f &gyro,
                                                            uint64_t sample_us)
{
    if ((1U<<instance) & _imu.imu_kill_mask) {
    ...
    if (sample_us != 0 && _imu._gyro_last_sample_us[instance] != 0) {
        ...
        _imu._gyro_last_sample_us[instance] = AG_HAL::micros64();
        sample_us = _imu._gyro_last_sample_us[instance];
    }

#if AP_MODULE_SUPPORTED
    // call gyro_sample hook if any
    AG_Module::call_hook_gyro_sample(instance, dt, gyro);
#endif
 ...
 */
void AG_InertialSensor_Backend::_notify_new_delta_angle(uint8_t instance, const Vector3f &dangle)
{
    if ((1U<<instance) & _imu.imu_kill_mask) {
        return;
    }
...
void AG_InertialSensor_Backend::log_gyro_raw(uint8_t instance, const uint64_t sample_us, const Vector3f &gyro)
{
    ...
    if (should_log_imu_raw()) {
        ...
        if (!_imu.batchsampler.doing_sensor_rate_logging()) {
            _imu.batchsampler.sample(instance, AG_InertialSensor::IMU_SENSOR_TYPE_GYRO, sample_us, gyro);
        }
#endif
    }
#endif
 ...
 */
void AG_InertialSensor_Backend::_publish_accel(uint8_t instance, const Vector3f &accel) /* front end */
{
    if ((1U<<instance) & _imu.imu_kill_mask) {
        return;
    }
...
void AG_InertialSensor_Backend::_notify_new_accel_raw_sample(uint8_t instance,
                                                             ...
                                                             bool fsync_set)
{
    ...
    if (sample_us != 0 && _imu._accel_last_sample_us[instance] != 0) {
        ...
        _imu._accel_last_sample_us[instance] = AG_HAL::micros64();
        sample_us = _imu._accel_last_sample_us[instance];
    }

#if AP_MODULE_SUPPORTED
    // call accel_sample hook if any
    AG_Module::call_hook_accel_sample(instance, dt, accel, fsync_set);
#endif    
 ...
 */
void AG_InertialSensor_Backend::_notify_new_delta_velocity(uint8_t instance, const Vector3f &dvel)
{
    if ((1U<<instance) & _imu.imu_kill_mask) {
        return;
    }
...
#if AP_MODULE_SUPPORTED
    // call accel_sample hook if any
    AG_Module::call_hook_accel_sample(instance, dt, accel, false);
#endif    
...
void AG_InertialSensor_Backend::_notify_new_accel_sensor_rate_sample(uint8_t instance, const Vector3f &accel)
{
#if AP_INERTIALSENSOR_BATCHSAMPLER_ENABLED
    if (!_imu.batchsampler.doing_sensor_rate_logging()) {
        return;
    ...
    _imu.batchsampler.sample(instance, AG_InertialSensor::IMU_SENSOR_TYPE_ACCEL, AG_HAL::micros64(), accel);
#endif
}

void AG_InertialSensor_Backend::_notify_new_gyro_sensor_rate_sample(uint8_t instance, const Vector3f &gyro)
{
#if AP_INERTIALSENSOR_BATCHSAMPLER_ENABLED
    if (!_imu.batchsampler.doing_sensor_rate_logging()) {
        return;
    }
    _imu.batchsampler.sample(instance, AG_InertialSensor::IMU_SENSOR_TYPE_GYRO, AG_HAL::micros64(), gyro);
#endif
}

void AG_InertialSensor_Backend::log_accel_raw(uint8_t instance, const uint64_t sample_us, const Vector3f &accel)
{
#if HAL_LOGGING_ENABLED
    AG_Logger *logger = AG_Logger::get_singleton();
    if (logger == nullptr) {
...
#if AP_INERTIALSENSOR_BATCHSAMPLER_ENABLED
        if (!_imu.batchsampler.doing_sensor_rate_logging()) {
            _imu.batchsampler.sample(instance, AG_InertialSensor::IMU_SENSOR_TYPE_ACCEL, sample_us, accel);
        }
#endif
    }
#endif
...
void AG_InertialSensor_Backend::_set_accel_max_abs_offset(uint8_t instance,
                                                          float max_offset)
{
...
void AG_InertialSensor_Backend::_inc_accel_error_count(uint8_t instance)
{
    _imu._accel_error_count[instance]++;
}

// increment gyro error_count
void AG_InertialSensor_Backend::_inc_gyro_error_count(uint8_t instance)
{
 ...
 */
void AG_InertialSensor_Backend::_publish_temperature(uint8_t instance, float temperature) /* front end */
{
    if ((1U<<instance) & _imu.imu_kill_mask) {
        return;
    }
    ...
    if (instance == AP_HEATER_IMU_INSTANCE) {
        AG_BoardConfig *bc = AP::boardConfig();
        if (bc) {
 ...
 */
void AG_InertialSensor_Backend::update_gyro(uint8_t instance) /* front end */
{    
    WITH_SEMAPHORE(_sem);
 ...
 */
void AG_InertialSensor_Backend::update_accel(uint8_t instance) /* front end */
{    
    WITH_SEMAPHORE(_sem);
...
bool AG_InertialSensor_Backend::should_log_imu_raw() const
{
...
void AG_InertialSensor_Backend::log_register_change(uint32_t bus_id, const AG_HAL::Device::checkreg &reg)
{
#if HAL_LOGGING_ENABLED
