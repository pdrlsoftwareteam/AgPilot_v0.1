...
AG_InertialSensor_SCHA63T::AG_InertialSensor_SCHA63T(AG_InertialSensor &imu,
        AG_HAL::OwnPtr<AG_HAL::Device> _dev_uno,
        AG_HAL::OwnPtr<AG_HAL::Device> _dev_due,
...
AG_InertialSensor_Backend* AG_InertialSensor_SCHA63T::probe(AG_InertialSensor &imu,
                                 AG_HAL::OwnPtr<AG_HAL::SPIDevice> dev_uno,
                                 AG_HAL::OwnPtr<AG_HAL::SPIDevice> dev_due,
                                 enum Rotation rotation)
{
    if (!dev_uno || !dev_due) {
        return nullptr;
    }
    auto sensor = new AG_InertialSensor_SCHA63T(imu, std::move(dev_uno), std::move(dev_due), rotation);
...
void AG_InertialSensor_SCHA63T::start()
{
    ...
    if (enable_fast_sampling(accel_instance) && get_fast_sampling_rate() > 1) {
        bool fast_sampling = dev_uno->bus_type() == AG_HAL::Device::BUS_TYPE_SPI;
        if (fast_sampling) {
    ...
    // setup callbacks
    dev_uno->register_periodic_callback(backend_period_us, FUNCTOR_BIND_MEMBER(&AG_InertialSensor_SCHA63T::read_accel, void));
    dev_due->register_periodic_callback(backend_period_us, FUNCTOR_BIND_MEMBER(&AG_InertialSensor_SCHA63T::read_gyro, void));
}

/*
  probe and initialise accelerometer
 */
bool AG_InertialSensor_SCHA63T::init()
{
...
bool AG_InertialSensor_SCHA63T::check_startup()
{
    uint8_t val[4] {};
...
/*
  read accel fifo
 */
void AG_InertialSensor_SCHA63T::read_accel()
{
...
/*
  read gyro fifo
 */
void AG_InertialSensor_SCHA63T::read_gyro()
{
...
void AG_InertialSensor_SCHA63T::set_temperature(uint8_t instance, uint16_t temper)
{
...
bool AG_InertialSensor_SCHA63T::update()
{
...
bool AG_InertialSensor_SCHA63T::read_register(uint8_t uno_due, reg_scha63t reg_addr, uint8_t* val)
{
    bool ret = false;
    uint8_t cmd[4];
    uint8_t bCrc;
...
bool AG_InertialSensor_SCHA63T::write_register(uint8_t uno_due, reg_scha63t reg_addr, uint16_t val)
{
    bool ret = false;
    uint8_t res[4];
    uint8_t cmd[4];
