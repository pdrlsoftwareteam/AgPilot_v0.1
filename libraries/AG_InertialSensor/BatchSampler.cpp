...
const AG_Param::GroupInfo AG_InertialSensor::BatchSampler::var_info[] = {
    // @Param: BAT_CNT
    ...
    // @Increment: 32
    // @RebootRequired: True
    AP_GROUPINFO("BAT_CNT",  1, AG_InertialSensor::BatchSampler, _required_count,   1024),
    ...
    // @RebootRequired: True
    AP_GROUPINFO("BAT_MASK",  2, AG_InertialSensor::BatchSampler, _sensor_mask,   DEFAULT_IMU_LOG_BAT_MASK),
    ...
    // @User: Advanced
    AP_GROUPINFO("BAT_OPT",  3, AG_InertialSensor::BatchSampler, _batch_options_mask, 0),

    // @Param: BAT_LGIN
    // @DisplayName: logging interval
    // @Description: Interval between pushing samples to the AG_Logger log
    // @Units: ms
    // @Increment: 10
    AP_GROUPINFO("BAT_LGIN", 4, AG_InertialSensor::BatchSampler, push_interval_ms,   20),

    // @Param: BAT_LGCT
    // @DisplayName: logging count
    // @Description: Number of samples to push to count every @PREFIX@BAT_LGIN
    // @Increment: 1
    AP_GROUPINFO("BAT_LGCT", 5, AG_InertialSensor::BatchSampler, samples_per_msg,   32),
...
void AG_InertialSensor::BatchSampler::init()
{
    if (_sensor_mask == 0) {
        return;
    }
...
void AG_InertialSensor::BatchSampler::periodic()
{
    if (_sensor_mask == 0) {
        return;
    }
...
void AG_InertialSensor::BatchSampler::update_doing_sensor_rate_logging()
{
...
void AG_InertialSensor::BatchSampler::rotate_to_next_sensor()
{
    if (_sensor_mask == 0) {
...
void AG_InertialSensor::BatchSampler::push_data_to_log()
{
    if (!initialised) {
        return;
    }
    ...
    if (AG_HAL::millis() - last_sent_ms < (uint16_t)push_interval_ms) {
        // avoid flooding AG_Logger's buffer
        return;
    }
...
bool AG_InertialSensor::BatchSampler::should_log(uint8_t _instance, IMU_SENSOR_TYPE _type)
{
    if (_sensor_mask == 0) {
        return false;
    }
...
void AG_InertialSensor::BatchSampler::sample(uint8_t _instance, AG_InertialSensor::IMU_SENSOR_TYPE _type, uint64_t sample_us, const Vector3f &_sample)
{
