...
AG_InertialSensor_ADIS1647x::AG_InertialSensor_ADIS1647x(AG_InertialSensor &imu,
...
AG_InertialSensor_ADIS1647x::probe(AG_InertialSensor &imu,
                                   ...
                                   uint8_t drdy_gpio)
{
    ...
    auto sensor = new AG_InertialSensor_ADIS1647x(imu, std::move(dev), rotation, drdy_gpio);
...
void AG_InertialSensor_ADIS1647x::start()
{
     ...
     */
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_InertialSensor_ADIS1647x::loop, void),
                                      "ADIS1647x",
                                      1024, AG_HAL::Scheduler::PRIORITY_BOOST, 1)) {
        AG_HAL::panic("Failed to create ADIS1647x thread");
    }
}
...
/*
  check product ID
 */
bool AG_InertialSensor_ADIS1647x::check_product_id(uint16_t &prod_id)
{
...
bool AG_InertialSensor_ADIS1647x::init()
{
 ...
 */
uint16_t AG_InertialSensor_ADIS1647x::read_reg16(uint8_t regnum) const
{
 ...
 */
bool AG_InertialSensor_ADIS1647x::write_reg16(uint8_t regnum, uint16_t value, bool confirm) const
{
 ...
 */
void AG_InertialSensor_ADIS1647x::read_sensor16(void)
{
    struct adis_data {
        uint8_t cmd[2];
        uint16_t diag_stat;
 ...
 */
void AG_InertialSensor_ADIS1647x::read_sensor32(void)
{
    struct adis_data {
        uint8_t cmd[2];
        uint16_t diag_stat;
 ...
 */
void AG_InertialSensor_ADIS1647x::read_sensor32_delta(void)
{
    struct adis_data {
        uint8_t cmd[2];
        uint16_t diag_stat;
...
/*
  sensor read loop
 */
void AG_InertialSensor_ADIS1647x::loop(void)
{
    while (true) {
        uint32_t tstart = AG_HAL::micros();
        ...
        if (drdy_pin != 0) {
            ...
            DEBUG_SET_PIN(0, 1);
            wait_ok = hal.gpio->wait_pin(drdy_pin, AG_HAL::GPIO::INTERRUPT_RISING, 2100);
            DEBUG_SET_PIN(0, 0);
        }
        if (opmode == OpMode::Delta32) {
            read_sensor32_delta();
        ...
        uint32_t dt = AG_HAL::micros() - tstart;
        if (dt < period_us) {
...
bool AG_InertialSensor_ADIS1647x::update()
{
