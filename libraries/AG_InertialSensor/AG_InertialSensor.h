...
class AG_InertialSensor_Backend;
class AuxiliaryBus;
class AG_AHRS;

/*
  forward declare AG_Logger class. We can't include logger.h
  because of mutual dependencies
 */
class AG_Logger;

/* AG_InertialSensor is an abstraction for gyro and accel measurements
 ...
 */
class AG_InertialSensor : AG_AccelCal_Client
{
    friend class AG_InertialSensor_Backend;

public:
    AG_InertialSensor();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_InertialSensor);

    static AG_InertialSensor *get_singleton();
    ...
    AG_InertialSensor_Params params[INS_AUX_INSTANCES];
#endif
    ...
    AG_AccelCal* get_acal() const { return _acal; }
    ...
    class BatchSampler {
    public:
        BatchSampler(const AG_InertialSensor &imu) :
        ...
        // Parameters controlling pushing data to AG_Logger:
    ...
    private:
        ...
        AG_InertialSensor::IMU_SENSOR_TYPE type;
        uint16_t isb_seqnum;
        int16_t *data_x;
        int16_t *data_y;
        int16_t *data_z;
    ...
    void handle_external(const AG_ExternalAHRS::ins_data_message_t &pkt);
#endif
...
private:
    ...
    bool _add_backend(AG_InertialSensor_Backend *backend);
    void _start_backends();
    AG_InertialSensor_Backend *_find_backend(int16_t backend_id, uint8_t instance);
    ...
    // backend objects
    AG_InertialSensor_Backend *_backends[INS_MAX_BACKENDS];
    ...
    #define INS_PARAM_WRAPPER(var) \
        ...
        inline decltype(var##_old_param[0])& var(uint8_t i) const { \
            return const_cast<AG_InertialSensor*>(this)->var(i); \
        }
#else
    #define INS_PARAM_WRAPPER(var) \
        inline decltype(var##_old_param[0])& var(uint8_t i) { \
    ...
    static AG_InertialSensor *_singleton;
    AG_AccelCal* _acal;
...
#if INS_AUX_INSTANCES
    uint8_t tcal_instance(const AG_InertialSensor_TCal &tc) const {
...
#endif
private:
    AG_InertialSensor_TCal tcal_old_param[INS_MAX_INSTANCES - INS_AUX_INSTANCES];
