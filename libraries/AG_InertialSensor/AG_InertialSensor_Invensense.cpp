...
#include "AG_InertialSensor_Invensense_registers.h"
...
AG_InertialSensor_Invensense::AG_InertialSensor_Invensense(AG_InertialSensor &imu,
...
AG_InertialSensor_Invensense::~AG_InertialSensor_Invensense()
{
    if (_fifo_buffer != nullptr) {
        hal.util->free_type(_fifo_buffer, MPU_FIFO_BUFFER_LEN * MPU_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    }
    delete _auxiliary_bus;
}

AG_InertialSensor_Backend *AG_InertialSensor_Invensense::probe(AG_InertialSensor &imu,
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    AG_InertialSensor_Invensense *sensor =
        new AG_InertialSensor_Invensense(imu, std::move(dev), rotation);
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    sensor = new AG_InertialSensor_Invensense(imu, std::move(dev), rotation);
    if (!sensor || !sensor->_init()) {
        delete sensor;
        return nullptr;
    }
...
bool AG_InertialSensor_Invensense::_init()
{
...
void AG_InertialSensor_Invensense::_fifo_reset(bool log_error)
{
    ...
    if (log_error &&
        ...
        if (reset_count == 10) {
            ...
            INTERNAL_ERROR(AG_InternalError::error_t::imu_reset);
            reset_count = 0;
        }
    } else if (log_error &&
        !hal.scheduler->in_expected_delay() &&
    ...
    _dev->set_speed(AG_HAL::Device::SPEED_LOW);
    ...
    _dev->set_speed(AG_HAL::Device::SPEED_HIGH);
...
void AG_InertialSensor_Invensense::_fast_fifo_reset()
{
    fast_reset_count++;
...
bool AG_InertialSensor_Invensense::_has_auxiliary_bus()
{
    return _dev->bus_type() != AG_HAL::Device::BUS_TYPE_I2C;
}

void AG_InertialSensor_Invensense::start()
{
    ...
    _fifo_buffer = (uint8_t *)hal.util->malloc_type(MPU_FIFO_BUFFER_LEN * MPU_SAMPLE_SIZE, AG_HAL::Util::MEM_DMA_SAFE);
    if (_fifo_buffer == nullptr) {
        AG_HAL::panic("Invensense: Unable to allocate FIFO buffer");
    }

    // start the timer process to read samples, using the fastest rate avilable
    _dev->register_periodic_callback(1000000UL / _gyro_backend_rate_hz, FUNCTOR_BIND_MEMBER(&AG_InertialSensor_Invensense::_poll_data, void));
}

// get a startup banner to output to the GCS
bool AG_InertialSensor_Invensense::get_output_banner(char* banner, uint8_t banner_len) {
    if (_fast_sampling) {
 ...
 */
bool AG_InertialSensor_Invensense::update() /* front end */
{
 ...
 */
void AG_InertialSensor_Invensense::accumulate()
{
    // nothing to do
}

AuxiliaryBus *AG_InertialSensor_Invensense::get_auxiliary_bus()
{
    if (_auxiliary_bus) {
 ...
 * status register.
 */
bool AG_InertialSensor_Invensense::_data_ready()
{
    if (_drdy_pin) {
 ...
 */
void AG_InertialSensor_Invensense::_poll_data()
{
    _read_fifo();
...
*/
void AG_InertialSensor_Invensense::_check_register_change(void)
{
    if (_mpu_type != Invensense_ICM20602) {
        return;
    }
...
bool AG_InertialSensor_Invensense::_accumulate(uint8_t *samples, uint8_t n_samples)
{
 ...
 */
bool AG_InertialSensor_Invensense::_accumulate_sensor_rate_sampling(uint8_t *samples, uint8_t n_samples)
{
    int32_t tsum = 0;
...
void AG_InertialSensor_Invensense::_read_fifo()
{
    uint8_t n_samples;
    uint16_t bytes_read;
     ...
     */
    if (_dev->bus_type() == AG_HAL::Device::BUS_TYPE_I2C) {
        if (n_samples > 4) {
            need_reset = true;
            n_samples = 4;
        }
...
*/
bool AG_InertialSensor_Invensense::_check_raw_temp(int16_t t2)
{
...
bool AG_InertialSensor_Invensense::_block_read(uint8_t reg, uint8_t *buf,
                                            uint32_t size)
{
    return _dev->read_registers(reg, buf, size);
}

uint8_t AG_InertialSensor_Invensense::_register_read(uint8_t reg)
{
    uint8_t val = 0;
...
void AG_InertialSensor_Invensense::_register_write(uint8_t reg, uint8_t val, bool checked)
{
 ...
 */
void AG_InertialSensor_Invensense::_set_filter_register(void)
{
    uint8_t config;
    ...
    if (enable_fast_sampling(_accel_instance)) {
        _fast_sampling = _dev->bus_type() == AG_HAL::Device::BUS_TYPE_SPI;
        if (_fast_sampling) {
 ...
 */
bool AG_InertialSensor_Invensense::_check_whoami(void)
{
...
bool AG_InertialSensor_Invensense::_hardware_init(void)
{
    WITH_SEMAPHORE(_dev->get_semaphore());

    // setup for register checking. We check much less often on I2C
    // where the cost of the checks is higher
    _dev->setup_checked_registers(13, _dev->bus_type() == AG_HAL::Device::BUS_TYPE_I2C?200:20);
    ...
    for (tries = 0; tries < 5; tries++) {
        ...
        if (_dev->bus_type() == AG_HAL::Device::BUS_TYPE_SPI) {
        ...
        /* bus-dependent initialization */
        if ((_dev->bus_type() == AG_HAL::Device::BUS_TYPE_I2C) && (_mpu_type == Invensense_MPU9250 || _mpu_type == Invensense_ICM20789)) {
...
int AP_Invensense_AuxiliaryBusSlave::_set_passthrough(uint8_t reg, uint8_t size,
                                                  uint8_t *out)
{
    auto &backend = AG_InertialSensor_Invensense::from(_bus.get_backend());
    uint8_t addr;
...
AP_Invensense_AuxiliaryBus::AP_Invensense_AuxiliaryBus(AG_InertialSensor_Invensense &backend, uint32_t devid)
    : AuxiliaryBus(backend, 4, devid)
{
}

AG_HAL::Semaphore *AP_Invensense_AuxiliaryBus::get_semaphore()
{
    return static_cast<AG_InertialSensor_Invensense&>(_ins_backend)._dev->get_semaphore();
}
...
{
    auto &backend = AG_InertialSensor_Invensense::from(_ins_backend);

    if (backend._mpu_type == AG_InertialSensor_Invensense::Invensense_ICM20789) {
...
AG_HAL::Device::PeriodicHandle AP_Invensense_AuxiliaryBus::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
    auto &backend = AG_InertialSensor_Invensense::from(_ins_backend);
