...
AG_Follow *AG_Follow::_singleton;

// table of user settable parameters
const AG_Param::GroupInfo AG_Follow::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("_ENABLE", 1, AG_Follow, _enabled, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 0 255
    // @User: Standard
    AP_GROUPINFO("_SYSID", 3, AG_Follow, _sysid, 0),
    ...
    // @Range: 1 1000
    // @User: Standard
    AP_GROUPINFO("_DIST_MAX", 5, AG_Follow, _dist_max, 100),
    ...
    // @User: Standard
    AP_GROUPINFO("_OFS_TYPE", 6, AG_Follow, _offset_type, AP_FOLLOW_OFFSET_TYPE_NED),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("_OFS", 7, AG_Follow, _offset, 0),
    ...
    // @User: Standard
    AP_GROUPINFO("_YAW_BEHAVE", 8, AG_Follow, _yaw_behave, 1),
#endif
    ...
    // @Increment: 0.01
    // @User: Standard
    AP_SUBGROUPINFO(_p_pos, "_POS_", 9, AG_Follow, AG_P),
    ...
    // @User: Standard
    AP_GROUPINFO("_ALT_TYPE", 10, AG_Follow, _alt_type, AP_FOLLOW_ALT_TYPE_DEFAULT),
#endif
...
void AG_Follow::clear_offsets_if_required()
{
...
bool AG_Follow::get_target_location_and_velocity(Location &loc, Vector3f &vel_ned) const
{
    ...
    // check for timeout
    if ((_last_location_update_ms == 0) || (AG_HAL::millis() - _last_location_update_ms > AP_FOLLOW_TIMEOUT_MS)) {
        return false;
    }

    // calculate time since last actual position update
    const float dt = (AG_HAL::millis() - _last_location_update_ms) * 0.001f;
...
bool AG_Follow::get_target_dist_and_vel_ned(Vector3f &dist_ned, Vector3f &dist_with_offs, Vector3f &vel_ned)
{
    // get our location
...
bool AG_Follow::get_target_heading_deg(float &heading) const
{
    // exit immediately if not enabled
    if (!_enabled) {
        return false;
    ...
    // check for timeout
    if ((_last_heading_update_ms == 0) || (AG_HAL::millis() - _last_heading_update_ms > AP_FOLLOW_TIMEOUT_MS)) {
        return false;
    }
...
void AG_Follow::handle_msg(const mavlink_message_t &msg)
{
    // exit immediately if not enabled
    if (!_enabled) {
        return;
    ...
    if (_sysid != 0 && msg.sysid != _sysid) {
        if (_automatic_sysid) {
            ...
            if ((_last_location_update_ms == 0) || (AG_HAL::millis() - _last_location_update_ms > AP_FOLLOW_SYSID_TIMEOUT_MS)) {
                _sysid.set(0);
            }
        }
        return;
    ...
    case MAVLINK_MSG_ID_GLOBAL_POSITION_INT: {
        ...
        _last_location_update_ms = _jitter.correct_offboard_timestamp_msec(packet.time_boot_ms, AG_HAL::millis());
    ...
    case MAVLINK_MSG_ID_FOLLOW_TARGET: {
        ...
        // get a local timestamp with correction for transport jitter
        _last_location_update_ms = _jitter.correct_offboard_timestamp_msec(packet.timestamp, AG_HAL::millis());
...
bool AG_Follow::get_velocity_ned(Vector3f &vel_ned, float dt) const
{
...
void AG_Follow::init_offsets_if_required(const Vector3f &dist_vec_ned)
{
...
bool AG_Follow::get_offsets_ned(Vector3f &offset) const
{
...
Vector3f AG_Follow::rotate_vector(const Vector3f &vec, float angle_deg) const
{
...
void AG_Follow::clear_dist_and_bearing_to_target()
{
...
bool AG_Follow::get_target_location_and_velocity_ofs(Location &loc, Vector3f &vel_ned) const
{
    Vector3f ofs;
...
bool AG_Follow::have_target(void) const
{
    if (!_enabled) {
        return false;
    }
...
AG_Follow &follow()
{
    return *AG_Follow::get_singleton();
}
