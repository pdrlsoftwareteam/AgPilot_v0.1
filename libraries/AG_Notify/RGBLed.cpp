...
uint32_t RGBLed::get_colour_sequence(void) const
{
    ...
    if (AG_Notify::flags.save_trim ||
        AG_Notify::flags.esc_calibration ||
        AG_Notify::flags.compass_cal_running ||
    ...
    if (AG_Notify::flags.failsafe_radio ||
        AG_Notify::flags.failsafe_gcs ||
        AG_Notify::flags.failsafe_battery ||
        AG_Notify::flags.ekf_bad ||
        AG_Notify::flags.gps_glitching ||
        AG_Notify::flags.leak_detected) {

        if (AG_Notify::flags.leak_detected) {
            // purple if leak detected
            return sequence_failsafe_leak;
        } else if (AG_Notify::flags.ekf_bad) {
            // red on if ekf bad
            return sequence_failsafe_ekf;
        } else if (AG_Notify::flags.gps_glitching) {
            // blue on gps glitch
    ...
    if (AG_Notify::flags.armed) {
        ...
        if (AG_Notify::flags.gps_status >= AG_GPS::GPS_OK_FIX_3D) {
    ...
    if (AG_Notify::flags.gps_status >= AG_GPS::GPS_OK_FIX_3D_DGPS && AG_Notify::flags.pre_arm_gps_check) {
        return sequence_disarmed_good_dgps;
    }

    if (AG_Notify::flags.gps_status >= AG_GPS::GPS_OK_FIX_3D && AG_Notify::flags.pre_arm_gps_check) {
...
void RGBLed::update()
{
    ...
    uint8_t step = (AG_HAL::millis()/100) % 10;
...
void RGBLed::handle_led_control(const mavlink_message_t &msg)
{
    ...
    _led_override.start_ms = AG_HAL::millis();
...
void RGBLed::update_override(void)
{
    ...
    uint32_t cycle = (AG_HAL::millis() - _led_override.start_ms) % ms_per_cycle;
