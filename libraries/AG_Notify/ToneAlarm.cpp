...
bool AP_ToneAlarm::init()
{
    ...
    flags.armed = AG_Notify::flags.armed;
    flags.failsafe_battery = AG_Notify::flags.failsafe_battery;
...
{
    uint32_t tnow_ms = AG_HAL::millis();
...
void AP_ToneAlarm::update()
{
    ...
    if (AG_Notify::flags.powering_off) {
        ...
        flags.powering_off = AG_Notify::flags.powering_off;
        return;
    }

    if (AG_Notify::flags.compass_cal_running != flags.compass_cal_running) {
    ...
    flags.compass_cal_running = AG_Notify::flags.compass_cal_running;

    if (AG_Notify::events.compass_cal_canceled) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEU_FEEDBACK);
        return;
    }

    if (AG_Notify::events.initiated_compass_cal) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEU_FEEDBACK);
        return;
    }

    if (AG_Notify::events.compass_cal_saved ||
        AG_Notify::events.temp_cal_saved) {
        play_tone(AP_NOTIFY_TONE_QUIET_READY_OR_FINISHED);
        return;
    }

    if (AG_Notify::events.compass_cal_failed ||
        AG_Notify::events.temp_cal_failed) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEG_FEEDBACK);
        return;
    }

    if (AG_Notify::events.initiated_temp_cal) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEU_FEEDBACK);
        return;
    }

    if (AG_Notify::flags.temp_cal_running != flags.temp_cal_running) {
        if (AG_Notify::flags.temp_cal_running) {
            play_tone(AP_NOTIFY_TONE_QUIET_CALIBRATING_CTS);
            play_tone(AP_NOTIFY_TONE_QUIET_POS_FEEDBACK);
        } else {
            if (_cont_tone_playing == AP_NOTIFY_TONE_QUIET_CALIBRATING_CTS) {
    ...
    if (AG_Notify::events.user_mode_change_failed) {
        if (AG_Notify::flags.armed) {
            play_tone(AP_NOTIFY_TONE_LOUD_NEG_FEEDBACK);
        } else {
            play_tone(AP_NOTIFY_TONE_QUIET_NEG_FEEDBACK);
    ...
    if (AG_Notify::events.failsafe_mode_change) {
        play_tone(AP_NOTIFY_TONE_LOUD_ATTENTION_NEEDED);
    }

    // notify the user when arming fails
    if (AG_Notify::events.arming_failed) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEG_FEEDBACK);
    }

    // notify the user when RC contact is lost
    if (flags.failsafe_radio != AG_Notify::flags.failsafe_radio) {
        flags.failsafe_radio = AG_Notify::flags.failsafe_radio;
    ...
    if (flags.failsafe_gcs != AG_Notify::flags.failsafe_gcs) {
        flags.failsafe_gcs = AG_Notify::flags.failsafe_gcs;
    ...
    if (flags.pre_arm_check != AG_Notify::flags.pre_arm_check) {
        flags.pre_arm_check = AG_Notify::flags.pre_arm_check;
    ...
    if (flags.armed != AG_Notify::flags.armed) {
        flags.armed = AG_Notify::flags.armed;
        if (flags.armed) {
            // arming tune
    ...
    if (flags.failsafe_battery != AG_Notify::flags.failsafe_battery) {
        flags.failsafe_battery = AG_Notify::flags.failsafe_battery;
    ...
    if (flags.parachute_release != AG_Notify::flags.parachute_release) {
        flags.parachute_release = AG_Notify::flags.parachute_release;
    ...
    // lost vehicle tone
    if (flags.vehicle_lost != AG_Notify::flags.vehicle_lost) {
        flags.vehicle_lost = AG_Notify::flags.vehicle_lost;
    ...
    if (flags.waiting_for_throw != AG_Notify::flags.waiting_for_throw) {
        flags.waiting_for_throw = AG_Notify::flags.waiting_for_throw;
    ...
    if (flags.leak_detected != AG_Notify::flags.leak_detected) {
        flags.leak_detected = AG_Notify::flags.leak_detected;
        if (flags.leak_detected) {
    ...
    if (AG_Notify::events.tune_started) {
        play_tone(AP_NOTIFY_TONE_TUNING_START);
        AG_Notify::events.tune_started = 0;
    }
    if (AG_Notify::events.tune_next) {
        // signify which parameter in the set is starting
        play_tone(AP_NOTIFY_TONE_LOUD_1 + (AG_Notify::events.tune_next-1));
        AG_Notify::events.tune_next = 0;
    }
    if (AG_Notify::events.tune_save) {
        play_tone(AP_NOTIFY_TONE_TUNING_SAVE);
        AG_Notify::events.tune_save = 0;
    }
    if (AG_Notify::events.tune_error) {
        play_tone(AP_NOTIFY_TONE_TUNING_ERROR);
        AG_Notify::events.tune_error = 0;
    }

    // notify the user when ekf failsafe is triggered
    if (flags.failsafe_ekf != AG_Notify::flags.failsafe_ekf) {
        flags.failsafe_ekf = AG_Notify::flags.failsafe_ekf;
 ...
 */
void AG_Notify::handle_play_tune(const mavlink_message_t &msg)
{
