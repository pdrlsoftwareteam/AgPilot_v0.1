...
bool Buzzer::init()
{
    ...
    _flags.armed = AG_Notify::flags.armed;
    _flags.failsafe_battery = AG_Notify::flags.failsafe_battery;
    return true;
}
...
void Buzzer::update_pattern_to_play()
{
    ...
    if (AG_HAL::millis() - _pattern_start_time < _pattern_start_interval_time_ms) {
        // do not interrupt playing patterns / enforce minumum separation
        return;
    }

    // check if armed status has changed
    if (_flags.armed != AG_Notify::flags.armed) {
        _flags.armed = AG_Notify::flags.armed;
        if (_flags.armed) {
    ...
    // check ekf bad
    if (_flags.ekf_bad != AG_Notify::flags.ekf_bad) {
        _flags.ekf_bad = AG_Notify::flags.ekf_bad;
        if (_flags.ekf_bad) {
...
void Buzzer::update_playing_pattern()
{
    ...
    const uint32_t delta = AG_HAL::millis() - _pattern_start_time;
    if (delta >= 3200) {
...
{
    _pattern = pattern;
    _pattern_start_time = AG_HAL::millis();
}
