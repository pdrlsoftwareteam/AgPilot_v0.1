...
AG_Notify *AG_Notify::_singleton;
...
const AG_Param::GroupInfo AG_Notify::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_BRIGHT", 0, AG_Notify, _rgb_led_brightness, NOTIFY_LED_BRIGHT_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_TYPES", 1, AG_Notify, _buzzer_type, BUILD_DEFAULT_BUZZER_TYPE),
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_OVERRIDE", 2, AG_Notify, _rgb_led_override, NOTIFY_LED_OVERRIDE_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("DISPLAY_TYPE", 3, AG_Notify, _display_type, 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("OREO_THEME", 4, AG_Notify, _oreo_theme, 0),
#endif
    ...
    // @Values: -1:Disabled
    // @User: Advanced
    AP_GROUPINFO("BUZZ_PIN", 5, AG_Notify, _buzzer_pin, HAL_BUZZER_PIN),
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_TYPES", 6, AG_Notify, _led_type, BUILD_DEFAULT_LED_TYPE),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_ON_LVL", 7, AG_Notify, _buzzer_level, HAL_BUZZER_ON),
    ...
    // @Range: 0 100
    // @Units: %
    AP_GROUPINFO("BUZZ_VOLUME", 8, AG_Notify, _buzzer_volume, 100),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("LED_LEN", 9, AG_Notify, _led_len, NOTIFY_LED_LEN_DEFAULT),
...
AG_Notify::AG_Notify()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_Notify must be singleton");
    }
    _singleton = this;
}

// static flags, to allow for direct class update from device drivers
struct AG_Notify::notify_flags_and_values_type AG_Notify::flags;
struct AG_Notify::notify_events_type AG_Notify::events;

NotifyDevice *AG_Notify::_devices[CONFIG_NOTIFY_DEVICES_MAX];
uint8_t AG_Notify::_num_devices;

void AG_Notify::add_backend_helper(NotifyDevice *backend)
{
...
void AG_Notify::add_backends(void)
{
    if (_num_devices != 0) {
        return;
    }
...
void AG_Notify::update(void)
{
    ...
    //reset the events
    memset(&AG_Notify::events, 0, sizeof(AG_Notify::events));
}

#if AP_NOTIFY_MAVLINK_LED_CONTROL_SUPPORT_ENABLED
// handle a LED_CONTROL message
void AG_Notify::handle_led_control(const mavlink_message_t &msg)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AG_Notify::handle_rgb(uint8_t r, uint8_t g, uint8_t b, uint8_t rate_hz)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AG_Notify::handle_rgb_id(uint8_t r, uint8_t g, uint8_t b, uint8_t id)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AG_Notify::play_tune(const char *tune)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
AG_Notify &notify()
{
    return *AG_Notify::get_singleton();
}
