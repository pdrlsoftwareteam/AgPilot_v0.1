...
void Display::update_gps(uint8_t r)
{
    ...
    snprintf(msg, DISPLAY_MESSAGE_SIZE, "GPS:%-5s Sats:%2u", fixname, (unsigned)AG_Notify::flags.gps_num_sats) ;
...
void Display::update_gps_sats(uint8_t r)
{
    ...
    draw_char(COLUMN(8), ROW(r), (AG_Notify::flags.gps_num_sats / 10) + '0');
    draw_char(COLUMN(9), ROW(r), (AG_Notify::flags.gps_num_sats % 10) + '0');
}
...
void Display::update_text(uint8_t r)
{
    ...
    const bool text_is_valid = AG_HAL::millis() - pNotify->_send_text_updated_millis < _send_text_valid_millis;
    if (!text_is_valid) {
        update_text_empty(r);
        return;
    }
