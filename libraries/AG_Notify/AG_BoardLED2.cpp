...
void AG_BoardLED2::update(void)
{
    ...
    if (AG_Notify::flags.save_trim || AG_Notify::flags.esc_calibration) {
    ...
    if(AG_Notify::events.autotune_complete){
        switch(save_trim_counter) {
        case 0:
    ...
    if(AG_Notify::events.autotune_failed){
        switch(save_trim_counter) {
        case 0:
    ...
    if(!led_a_used) {
        if (AG_Notify::flags.armed) {
            if(AG_Notify::flags.failsafe_battery){//   blink slowly (around 2Hz)
                if ((counter2 & 0x7) == 0) {
                    hal.gpio->toggle(HAL_GPIO_A_LED_PIN);
                }
            }else if(AG_Notify::flags.failsafe_radio || AG_Notify::flags.failsafe_gcs){//   blink fast (around 4Hz)
                if ((counter2 & 0x3) == 0) {
                    hal.gpio->toggle(HAL_GPIO_A_LED_PIN);
                }
            } else {
    ...
    // gps light
    switch (AG_Notify::flags.gps_status) {
        case 0:
        case 1:
        ...
        default: // show number of sats 
            ...
            uint16_t sats = AG_Notify::flags.gps_num_sats;
...
#else
bool AG_BoardLED2::init(void) {return true;}
void AG_BoardLED2::update(void) {return;}
#endif
