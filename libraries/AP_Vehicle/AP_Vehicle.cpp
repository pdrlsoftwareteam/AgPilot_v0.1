...
const AP_Scheduler::Task AP_Vehicle::scheduler_tasks[] = {
		...
		SCHED_TASK(check_task,      3.3,   75, 254),
...
void AP_Vehicle::check_task(void)
{
	static int chunk_offset = 0;
	static bool progress = 0;

	if(!AP_PDRL_COMMANDER::getInstance()->start)
	{
		chunk_offset = 0;
		progress = 0;
		return;
	...
	if(AP_PDRL_COMMANDER::getInstance()->start && !progress)
	{
		...
		{
			gcs().send_text(MAV_SEVERITY_INFO,"backup failed at : %d",chunk_offset);
