 ...
 */
const struct ap_secure_data *AG_CheckFirmware::find_public_keys(void)
{
 ...
 */
bool AG_CheckFirmware::all_zero_keys(const struct ap_secure_data *sec_data)
{
 ...
 */
AG_CheckFirmware::bl_data *AG_CheckFirmware::read_bootloader(void)
{
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
...
#if HAL_GCS_ENABLED
uint8_t AG_CheckFirmware::session_key[8];
 ...
 */
bool AG_CheckFirmware::write_bootloader(const struct bl_data *bld)
{
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
    const uint32_t flash_addr = hal.flash->getpageaddr(0);
 ...
 */
bool AG_CheckFirmware::check_signature(const mavlink_secure_command_t &pkt)
{
 ...
 */
bool AG_CheckFirmware::set_public_keys(uint8_t key_idx, uint8_t num_keys, const uint8_t *key_data)
{
 ...
 */
void AG_CheckFirmware::handle_secure_command(mavlink_channel_t chan, const mavlink_secure_command_t &pkt)
{
 ...
 */
void AG_CheckFirmware::handle_msg(mavlink_channel_t chan, const mavlink_message_t &msg)
{
    switch (msg.msgid) {
 ...
 */
bool AG_CheckFirmware::check_signed_bootloader(const uint8_t *fw, uint32_t fw_size)
{
    const struct ap_secure_data *sec_data = find_public_keys();
