...
AG_AutoTune::AG_AutoTune()
{
}

// autotune_init - should be called when autotune mode is selected
bool AG_AutoTune::init_internals(bool _use_poshold,
                                 AG_AttitudeControl *_attitude_control,
                                 AG_PosControl *_pos_control,
                                 AG_AHRS_View *_ahrs_view,
                                 AG_InertialNav *_inertial_nav)
{
    ...
    motors = AG_Motors::get_singleton();
    ...
    case TUNING:
        ...
        step_start_time_ms = AG_HAL::millis();
...
// initialise position controller
bool AG_AutoTune::init_position_controller(void)
{
...
void AG_AutoTune::send_step_string()
{
    if (pilot_override) {
...
const char *AG_AutoTune::type_string() const
{
    switch (tune_type) {
    case RD_UP:
        return "Rate D Up";
...
const char *AG_AutoTune::axis_string() const
{
    switch (axis) {
    case ROLL:
        return "Roll";
...
bool AG_AutoTune::currently_level()
{
...
void AG_AutoTune::control_attitude()
{
        ...
        if (counter >= AUTOTUNE_SUCCESS_COUNT) {
            ...
            if (tune_type == TUNE_COMPLETE) {
                ...
                if (complete) {
                    ...
                    AG_Notify::events.autotune_next_axis = true;
...
void AG_AutoTune::backup_gains_and_initialise()
{
 ...
 */
void AG_AutoTune::load_gains(enum GainType gain_type)
{
    switch (gain_type) {
    case GAIN_ORIGINAL:
        load_orig_gains();
...
void AG_AutoTune::update_gcs(uint8_t message_id) const
{
    switch (message_id) {
 ...
 */
bool AG_AutoTune::position_ok(void)
{
...
void AG_AutoTune::get_poshold_attitude(float &roll_cd_out, float &pitch_cd_out, float &yaw_cd_out)
{
...
void AG_AutoTune::next_tune_type(TuneType &curr_tune_type, bool reset)
{
    if (reset) {
        set_tune_sequence();
        tune_seq_curr = 0;
