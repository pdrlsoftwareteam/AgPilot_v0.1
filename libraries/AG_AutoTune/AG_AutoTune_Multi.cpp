...
const AG_Param::GroupInfo AG_AutoTune_Multi::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("AXES", 1, AG_AutoTune_Multi, axis_bitmask,  7),  // AUTOTUNE_AXIS_BITMASK_DEFAULT
    ...
    // @Range: 0.05 0.10
    // @User: Standard
    AP_GROUPINFO("AGGR", 2, AG_AutoTune_Multi, aggressiveness, 0.1f),
    ...
    // @Range: 0.001 0.006
    // @User: Standard
    AP_GROUPINFO("MIN_D", 3, AG_AutoTune_Multi, min_d,  0.001f),

    AP_GROUPEND
};

// constructor
AG_AutoTune_Multi::AG_AutoTune_Multi()
{
    tune_seq[0] = TUNE_COMPLETE;
    AG_Param::setup_object_defaults(this, var_info);
}

void AG_AutoTune_Multi::do_gcs_announcements()
{
...
void AG_AutoTune_Multi::test_init()
{
    twitch_test_init();
}

void AG_AutoTune_Multi::test_run(AxisType test_axis, const float dir_sign)
{
...
void AG_AutoTune_Multi::backup_gains_and_initialise()
{
    AG_AutoTune::backup_gains_and_initialise();
...
void AG_AutoTune_Multi::load_orig_gains()
{
...
void AG_AutoTune_Multi::load_tuned_gains()
{
...
void AG_AutoTune_Multi::load_intra_test_gains()
{
...
void AG_AutoTune_Multi::load_test_gains()
{
    switch (axis) {
    case ROLL:
        attitude_control->get_rate_roll_pid().kP(tune_roll_rp);
...
void AG_AutoTune_Multi::save_tuning_gains()
{
...
void AG_AutoTune_Multi::report_final_gains(AxisType test_axis) const
{
    switch (test_axis) {
        case ROLL:
...
void AG_AutoTune_Multi::report_axis_gains(const char* axis_string, float rate_P, float rate_I, float rate_D, float angle_P, float max_accel) const
{
...
void AG_AutoTune_Multi::twitching_test_rate(float rate, float rate_target_max, float &meas_rate_min, float &meas_rate_max)
{
    const uint32_t now = AG_HAL::millis();
...
// twitching_test_rate - twitching tests
// update min and max and test for end conditions
void AG_AutoTune_Multi::twitching_abort_rate(float angle, float rate, float angle_max, float meas_rate_min)
{
    if (angle >= angle_max) {
...
// update min and max and test for end conditions
void AG_AutoTune_Multi::twitching_test_angle(float angle, float rate, float angle_target_max, float &meas_angle_min, float &meas_angle_max, float &meas_rate_min, float &meas_rate_max)
{
    const uint32_t now = AG_HAL::millis();
...
void AG_AutoTune_Multi::twitching_measure_acceleration(float &rate_of_change, float rate_measurement, float &rate_measurement_max) const
{
    if (rate_measurement_max < rate_measurement) {
        rate_measurement_max = rate_measurement;
        rate_of_change = (1000.0f*rate_measurement_max)/(AG_HAL::millis() - step_start_time_ms);
    }
}

// update gains for the rate p up tune type
void AG_AutoTune_Multi::updating_rate_p_up_all(AxisType test_axis)
{
    switch (test_axis) {
    case ROLL:
...
void AG_AutoTune_Multi::updating_rate_d_up_all(AxisType test_axis)
{
    switch (test_axis) {
    case ROLL:
...
void AG_AutoTune_Multi::updating_rate_d_down_all(AxisType test_axis)
{
    switch (test_axis) {
    case ROLL:
...
void AG_AutoTune_Multi::updating_angle_p_up_all(AxisType test_axis)
{
    switch (test_axis) {
    case ROLL:
...
void AG_AutoTune_Multi::updating_angle_p_down_all(AxisType test_axis)
{
    switch (test_axis) {
    case ROLL:
...
void AG_AutoTune_Multi::set_gains_post_tune(AxisType test_axis)
{
    switch (tune_type) {
    case RD_UP:
        break;
...
void AG_AutoTune_Multi::updating_rate_d_up(float &tune_d, float tune_d_min, float tune_d_max, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
...
// optimize D term while keeping the maximum just below the target by adjusting P
void AG_AutoTune_Multi::updating_rate_d_down(float &tune_d, float tune_d_min, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
    if (meas_rate_max > rate_target) {
        // if maximum measurement was higher than target
        // reduce P gain (which should reduce maximum)
...
void AG_AutoTune_Multi::updating_rate_p_up_d_down(float &tune_d, float tune_d_min, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
...
void AG_AutoTune_Multi::updating_angle_p_down(float &tune_p, float tune_p_min, float tune_p_step_ratio, float angle_target, float meas_angle_max, float meas_rate_min, float meas_rate_max)
{
...
void AG_AutoTune_Multi::updating_angle_p_up(float &tune_p, float tune_p_max, float tune_p_step_ratio, float angle_target, float meas_angle_max, float meas_rate_min, float meas_rate_max)
{
    if ((meas_angle_max > angle_target*(1+0.5f*aggressiveness)) ||
...
void AG_AutoTune_Multi::Log_AutoTune()
{
...
void AG_AutoTune_Multi::Log_AutoTuneDetails()
{
...
void AG_AutoTune_Multi::Log_Write_AutoTune(uint8_t _axis, uint8_t tune_step, float meas_target, float meas_min, float meas_max, float new_gain_rp, float new_gain_rd, float new_gain_sp, float new_ddt)
{
    AP::logger().Write(
        "ATUN",
...
// Write an Autotune data packet
void AG_AutoTune_Multi::Log_Write_AutoTuneDetails(float angle_cd, float rate_cds)
{
    // @LoggerMessage: ATDE
...
void AG_AutoTune_Multi::twitch_test_init()
{
...
//run twitch test
void AG_AutoTune_Multi::twitch_test_run(AxisType test_axis, const float dir_sign)
{
    // disable rate limits
...
uint32_t AG_AutoTune_Multi::get_testing_step_timeout_ms() const
{
