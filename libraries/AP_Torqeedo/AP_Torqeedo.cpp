...
void AP_Torqeedo::report_error_codes()
{
    ...
    if (_display_system_state.flags.set_throttle_stop) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s zero throttle required", msg_prefix);
    }
    if (_display_system_state.flags.temp_warning) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s high temp", msg_prefix);
    }
    if (_display_system_state.flags.batt_nearly_empty) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s batt nearly empty", msg_prefix);
    }
    if (_display_system_state.master_error_code > 0) {
        ...
        if (error_string != nullptr) {
            gcs().send_text(
                AGPILOT_SEVERITY_CRITICAL, "%s err:%u %s",
                msg_prefix,
                _display_system_state.master_error_code,
                error_string);
        } else {
            gcs().send_text(
                AGPILOT_SEVERITY_CRITICAL, "%s err:%u",
                msg_prefix,
                _display_system_state.master_error_code);
        }
    }
    ...
    if (_motor_status.error_flags.overcurrent) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s overcurrent", msg_prefix);
    }
    if (_motor_status.error_flags.blocked) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s prop blocked", msg_prefix);
    }
    if (_motor_status.error_flags.overvoltage_static || _motor_status.error_flags.overvoltage_current) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s high voltage", msg_prefix);
    }
    if (_motor_status.error_flags.undervoltage_static || _motor_status.error_flags.undervoltage_current) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s low voltage", msg_prefix);
    }
    ...
    if (_motor_status.error_flags.timeout_rs485) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s comm timeout", msg_prefix);
    }
    if (_motor_status.error_flags.temp_sensor_error) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s temp sensor err", msg_prefix);
    }
    if (_motor_status.error_flags.tilt) {
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "%s tilted", msg_prefix);
    }
    ...
    if (!now_errored && prev_errored) {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "%s OK", msg_prefix);
    }
    ...
    if ((_type == ConnectionType::TYPE_TILLER) && (msg_addr == MsgAddress::DISPLAY)) {
        ...
        case DisplayMsgId::SYSTEM_STATE :
            if (_received_buff_len == 30) {
                ...
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(AGPILOT_SEVERITY_INFO,"TRST F:%u Err:%u MV:%4.1f MC:%4.1f P:%u MT:%d B%%:%d BV:%4.1f BC:%4.1f",
        ...
        case DisplayMsgId::SYSTEM_SETUP:
            if (_received_buff_len == 13) {
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(AGPILOT_SEVERITY_INFO,"TRSE:%u F:%u Mot:%u/%u Bat:%u/%u/%u%%",
    ...
    if ((_type == ConnectionType::TYPE_MOTOR) && (msg_addr == MsgAddress::BUS_MASTER)) {
        ...
        case MotorMsgId::PARAM:
            if (_received_buff_len == 15) {
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(AGPILOT_SEVERITY_INFO, "TRMP: rpm:%d p:%u V:%4.1f C:%4.1f PT:%4.1f MT:%4.1f",
        ...
        case MotorMsgId::STATUS:
            if (_received_buff_len == 6) {
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(AGPILOT_SEVERITY_INFO,"TRMS S:%d Err:%d",
                                   _motor_status.status_flags_value,
                                   _motor_status.error_flags_value);
                }
...
void AP_Torqeedo::log_TRQD(bool force_logging)
{
    ...
    if ((_options & options::DEBUG_TO_GCS) != 0) {
        gcs().send_text(AGPILOT_SEVERITY_INFO,"TRQD h:%u dspd:%d spd:%d succ:%ld err:%ld",
                (unsigned)health,
