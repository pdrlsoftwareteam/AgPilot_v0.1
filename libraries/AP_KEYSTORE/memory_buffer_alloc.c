...
static void debug_header( memory_header *hdr )
{
    ...
    fprintf stderr, "HDR:  PTR(%10zu), PREV(%10zu), NEXT(%10zu), "
                              "ALLOC(%zu), SIZE(%10zu)\n",
                      (size_t) hdr, (size_t) hdr->prev, (size_t) hdr->next,
                      hdr->alloc, hdr->size );
    fprintf stderr, "      FPREV(%10zu), FNEXT(%10zu)\n",
...
static int verify_chain( void )
{
    ...
    {
#if defined(MBEDTLS_MEMORY_DEBUG)
        fprintf stderr, "FATAL: verification of first header "
                                  "failed\n" );
#endif
        return( 1 );
    }
    ...
    {
#if defined(MBEDTLS_MEMORY_DEBUG)
        fprintf stderr, "FATAL: verification failed: "
                                  "first->prev != NULL\n" );
#endif
        return( 1 );
    }
    ...
    while( cur != NULL )
    {
        ...
        {
#if defined(MBEDTLS_MEMORY_DEBUG)
            fprintf stderr, "FATAL: verification of header "
                                      "failed\n" );
#endif
            return( 1 );
        }
...
static void *buffer_alloc_calloc( size_t n, size_t size )
{
    ...
    {
#if defined(MBEDTLS_MEMORY_DEBUG)
        fprintf stderr, "FATAL: block in free_list but allocated "
                                  "data\n" );
#endif
        mbedtls_exit( 1 );
    }
...
static void buffer_alloc_free( void *ptr )
{
    ...
    {
#if defined(MBEDTLS_MEMORY_DEBUG)
        fprintf stderr, "FATAL: mbedtls_free() outside of managed "
                                  "space\n" );
#endif
        mbedtls_exit( 1 );
    }
    ...
    {
#if defined(MBEDTLS_MEMORY_DEBUG)
        fprintf stderr, "FATAL: mbedtls_free() on unallocated "
                                  "data\n" );
#endif
        mbedtls_exit( 1 );
    }
