...
void mbedtls_strerror( int ret, char *buf, size_t buflen )
{
    ...
    if( use_ret == -(MBEDTLS_ERR_ARIA_INVALID_KEY_LENGTH) )
        snprintf(buf, buflen, "ARIA - Invalid key length" );
    if( use_ret == -(MBEDTLS_ERR_ARIA_INVALID_INPUT_LENGTH) )
        snprintf(buf, buflen, "ARIA - Invalid data input length" );
    if( use_ret == -(MBEDTLS_ERR_ARIA_FEATURE_UNAVAILABLE) )
        snprintf(buf, buflen, "ARIA - Feature not available. For example, an unsupported ARIA key size" );
    if( use_ret == -(MBEDTLS_ERR_ARIA_HW_ACCEL_FAILED) )
        snprintf(buf, buflen, "ARIA - ARIA hardware accelerator failed" );
    ...
    if( use_ret == -(MBEDTLS_ERR_HKDF_BAD_INPUT_DATA) )
        snprintf(buf, buflen, "HKDF - Bad input parameters to function" );
    ...
    if( use_ret == -(MBEDTLS_ERR_MD2_HW_ACCEL_FAILED) )
        snprintf(buf, buflen, "MD2 - MD2 hardware accelerator failed" );
#endif /* MBEDTLS_MD2_C */

#if defined(MBEDTLS_MD4_C)
    if( use_ret == -(MBEDTLS_ERR_MD4_HW_ACCEL_FAILED) )
        snprintf(buf, buflen, "MD4 - MD4 hardware accelerator failed" );
#endif /* MBEDTLS_MD4_C */
