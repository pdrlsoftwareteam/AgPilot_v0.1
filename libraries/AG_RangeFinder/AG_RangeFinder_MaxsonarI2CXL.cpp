...
/*
 *       AG_RangeFinder_MaxsonarI2CXL.cpp - Arduino Library for MaxBotix I2C XL sonar
...
AG_RangeFinder_MaxsonarI2CXL::AG_RangeFinder_MaxsonarI2CXL(RangeFinder::RangeFinder_State &_state,
...
/*
   ...
   there.
*/
AG_RangeFinder_Backend *AG_RangeFinder_MaxsonarI2CXL::detect(RangeFinder::RangeFinder_State &_state,
																AG_RangeFinder_Params &_params,
                                                             AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev)
{
    if (!dev) {
    ...
    AG_RangeFinder_MaxsonarI2CXL *sensor
        = new AG_RangeFinder_MaxsonarI2CXL(_state, _params, std::move(dev));
    if (!sensor) {
        return nullptr;
    }
...
/*
  initialise sensor
 */
bool AG_RangeFinder_MaxsonarI2CXL::_init(void)
{
    ...
    _dev->register_periodic_callback(100000,
                                     FUNCTOR_BIND_MEMBER(&AG_RangeFinder_MaxsonarI2CXL::_timer, void));

    return true;
}

// start_reading() - ask sensor to make a range reading
bool AG_RangeFinder_MaxsonarI2CXL::start_reading()
{
...
bool AG_RangeFinder_MaxsonarI2CXL::get_reading(uint16_t &reading_cm)
{
    be16_t val;
...
*/
void AG_RangeFinder_MaxsonarI2CXL::_timer(void)
{
    uint16_t d;
    if (get_reading(d)) {
        WITH_SEMAPHORE(_sem);
...
*/
void AG_RangeFinder_MaxsonarI2CXL::update(void)
{
    WITH_SEMAPHORE(_sem);
    if (new_distance) {
        ...
        new_distance = false;
        update_status();
    } else if (AG_HAL::millis() - state.last_reading_ms > 300) {
