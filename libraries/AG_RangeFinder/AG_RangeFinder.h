...
class AG_RangeFinder_Backend;

class RangeFinder
{
    friend class AG_RangeFinder_Backend;
    //UAVCAN drivers are initialised in the Backend, hence list of drivers is needed there.
    friend class AG_RangeFinder_UAVCAN;
public:
    RangeFinder();
...
//        PX4    = 4, // no longer used, but may be in some user's parameters
    ...
    static const struct AG_Param::GroupInfo *backend_var_info[RANGEFINDER_MAX_INSTANCES];
    ...
    */
    ...
    AG_RangeFinder_Backend *find_instance(enum Rotation orientation) const;

    AG_RangeFinder_Backend *get_backend(uint8_t id) const;
...
protected:
    AG_RangeFinder_Params params[RANGEFINDER_MAX_INSTANCES];

private:
    static RangeFinder *_singleton;

    RangeFinder_State state[RANGEFINDER_MAX_INSTANCES];
    AG_RangeFinder_Backend *drivers[RANGEFINDER_MAX_INSTANCES];
    ...
    bool _add_backend(AG_RangeFinder_Backend *driver, uint8_t instance, uint8_t serial_instance=0);
