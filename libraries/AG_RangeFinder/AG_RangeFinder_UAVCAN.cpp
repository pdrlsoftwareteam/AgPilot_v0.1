...
void AG_RangeFinder_UAVCAN::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (measurement_listener_res < 0) {
        AG_HAL::panic("UAVCAN RangeFinder subscriber start problem\n\r");
        return;
    }
}

//Method to find the backend relating to the node id
AG_RangeFinder_UAVCAN* AG_RangeFinder_UAVCAN::get_uavcan_backend(AG_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t address, bool create_new)
{
    if (ap_uavcan == nullptr) {
        return nullptr;
    }
    AG_RangeFinder_UAVCAN* driver = nullptr;
    ...
    for (uint8_t i = 0; i < RANGEFINDER_MAX_INSTANCES; i++) {
        if ((RangeFinder::Type)frontend.params[i].type.get() == RangeFinder::Type::UAVCAN &&
            ...
            driver = (AG_RangeFinder_UAVCAN*)frontend.drivers[i];
        }
    ...
    if (create_new) {
        for (uint8_t i = 0; i < RANGEFINDER_MAX_INSTANCES; i++) {
            if ((RangeFinder::Type)frontend.params[i].type.get() == RangeFinder::Type::UAVCAN &&
                ...
                frontend.drivers[i] = new AG_RangeFinder_UAVCAN(frontend.state[i], frontend.params[i]);
                driver = (AG_RangeFinder_UAVCAN*)frontend.drivers[i];
                if (driver == nullptr) {
                    break;
                }
...
void AG_RangeFinder_UAVCAN::update()
{
    WITH_SEMAPHORE(_sem);
    if ((AG_HAL::millis() - _last_reading_ms) > 500) {
...
void AG_RangeFinder_UAVCAN::handle_measurement(AG_UAVCAN* ap_uavcan, uint8_t node_id, const MeasurementCb &cb)
{
    //fetch the matching uavcan driver, node id and sensor id backend instance
    AG_RangeFinder_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id, cb.msg->sensor_id, true);
    if (driver == nullptr) {
        return;
    }
    ...
    switch (cb.msg->reading_type) {
        case uavcan::equipment::range_sensor::Measurement::READING_TYPE_VALID_RANGE:
        {
            ...
            driver->_last_reading_ms = AG_HAL::millis();
