...
*/
void AG_RangeFinder_MAVLink::handle_msg(const mavlink_message_t &msg)
{
...
int16_t AG_RangeFinder_MAVLink::max_distance_cm() const
{
...
int16_t AG_RangeFinder_MAVLink::min_distance_cm() const
{
    if (_max_distance_cm == 0 && _min_distance_cm == 0) {
        // we assume if both of these are zero that we ignore both
...
*/
void AG_RangeFinder_MAVLink::update(void)
{
    //Time out on incoming data; if we don't get new
    //data in 500ms, dump it
    if (AG_HAL::millis() - state.last_reading_ms > AP_RANGEFINDER_MAVLINK_TIMEOUT_MS) {
