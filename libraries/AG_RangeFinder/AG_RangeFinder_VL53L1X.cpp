...
AG_RangeFinder_VL53L1X::AG_RangeFinder_VL53L1X(RangeFinder::RangeFinder_State &_state, AG_RangeFinder_Params &_params, AG_HAL::OwnPtr<AG_HAL::I2CDevice> _dev)
...
/*
   ...
   there.
*/
AG_RangeFinder_Backend *AG_RangeFinder_VL53L1X::detect(RangeFinder::RangeFinder_State &_state, AG_RangeFinder_Params &_params, AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev, DistanceMode mode)
{
    if (!dev) {
        return nullptr;
    }

    AG_RangeFinder_VL53L1X *sensor
        = new AG_RangeFinder_VL53L1X(_state, _params, std::move(dev));
...
bool AG_RangeFinder_VL53L1X::check_id(void)
{
    uint8_t v1, v2;
...
bool AG_RangeFinder_VL53L1X::reset(void) {
...
/*
  initialise sensor
 */
bool AG_RangeFinder_VL53L1X::init(DistanceMode mode)
{
    ...
    dev->register_periodic_callback(MEASUREMENT_TIME_MS * 1000,
                                    FUNCTOR_BIND_MEMBER(&AG_RangeFinder_VL53L1X::timer, void));
...
bool AG_RangeFinder_VL53L1X::setDistanceMode(DistanceMode distance_mode)
{
...
bool AG_RangeFinder_VL53L1X::setMeasurementTimingBudget(uint32_t budget_us)
{
...
// based on VL53L1_SetMeasurementTimingBudgetMicroSeconds()
bool AG_RangeFinder_VL53L1X::getMeasurementTimingBudget(uint32_t &budget)
{
...
bool AG_RangeFinder_VL53L1X::startContinuous(uint32_t period_ms)
{
...
uint32_t AG_RangeFinder_VL53L1X::decodeTimeout(uint16_t reg_val)
{
...
uint16_t AG_RangeFinder_VL53L1X::encodeTimeout(uint32_t timeout_mclks)
{
...
uint32_t AG_RangeFinder_VL53L1X::timeoutMclksToMicroseconds(uint32_t timeout_mclks, uint32_t macro_period_us)
{
...
uint32_t AG_RangeFinder_VL53L1X::timeoutMicrosecondsToMclks(uint32_t timeout_us, uint32_t macro_period_us)
{
...
uint32_t AG_RangeFinder_VL53L1X::calcMacroPeriod(uint8_t vcsel_period) const
{
...
bool AG_RangeFinder_VL53L1X::setupManualCalibration(void)
{
...
bool AG_RangeFinder_VL53L1X::dataReady(void)
{
...
bool AG_RangeFinder_VL53L1X::get_reading(uint16_t &reading_mm)
{
    uint8_t tries = 10;
    while (!dataReady()) {
        tries--;
...
bool AG_RangeFinder_VL53L1X::read_register(uint16_t reg, uint8_t &value)
{
    uint8_t b[2] = { uint8_t(reg >> 8), uint8_t(reg & 0xFF) };
    return dev->transfer(b, 2, &value, 1);
}

bool AG_RangeFinder_VL53L1X::read_register16(uint16_t reg, uint16_t & value)
{
    uint16_t v = 0;
    uint8_t b[2] = { uint8_t(reg >> 8), uint8_t(reg & 0xFF) };
...
bool AG_RangeFinder_VL53L1X::write_register(uint16_t reg, uint8_t value)
{
    uint8_t b[3] = { uint8_t(reg >> 8), uint8_t(reg & 0xFF), value };
    return dev->transfer(b, 3, nullptr, 0);
}

bool AG_RangeFinder_VL53L1X::write_register16(uint16_t reg, uint16_t value)
{
    uint8_t b[4] = { uint8_t(reg >> 8), uint8_t(reg & 0xFF), uint8_t(value >> 8), uint8_t(value & 0xFF) };
    return dev->transfer(b, 4, nullptr, 0);
}

bool AG_RangeFinder_VL53L1X::write_register32(uint16_t reg, uint32_t value)
{
...
*/
void AG_RangeFinder_VL53L1X::timer(void)
{
    uint16_t range_mm;
...
*/
void AG_RangeFinder_VL53L1X::update(void)
{
    WITH_SEMAPHORE(_sem);
    if (counter > 0) {
        ...
        sum_mm = 0;
        counter = 0;
    } else if (AG_HAL::millis() - state.last_reading_ms > 200) {
