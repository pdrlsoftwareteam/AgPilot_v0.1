...
const AG_Param::GroupInfo AGP_Avoidance::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE", 1, AGP_Avoidance, _enabled, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    AP_GROUPINFO("F_ACTION",    2, AGP_Avoidance, _fail_action, AP_AVOIDANCE_FAIL_ACTION_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("W_ACTION",    3, AGP_Avoidance, _warn_action, MAV_COLLISION_ACTION_REPORT),
    ...
    // @User: Advanced
    AP_GROUPINFO("F_RCVRY",     4, AGP_Avoidance, _fail_recovery, uint8_t(AP_AVOIDANCE_RECOVERY_DEFAULT)),

    // @Param: OBS_MAX
    // @DisplayName: Maximum number of obstacles to track
    // @Description: Maximum number of obstacles to track
    // @User: Advanced
    AP_GROUPINFO("OBS_MAX",     5, AGP_Avoidance, _obstacles_max, 20),
    ...
    // @Units: s
    // @User: Advanced
    AP_GROUPINFO("W_TIME",      6, AGP_Avoidance, _warn_time_horizon, AP_AVOIDANCE_WARN_TIME_DEFAULT),
    ...
    // @Units: s
    // @User: Advanced
    AP_GROUPINFO("F_TIME",      7, AGP_Avoidance, _fail_time_horizon, AP_AVOIDANCE_FAIL_TIME_DEFAULT),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("W_DIST_XY",   8, AGP_Avoidance, _warn_distance_xy, AP_AVOIDANCE_WARN_DISTANCE_XY_DEFAULT),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("F_DIST_XY",   9, AGP_Avoidance, _fail_distance_xy, AP_AVOIDANCE_FAIL_DISTANCE_XY_DEFAULT),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("W_DIST_Z",    10, AGP_Avoidance, _warn_distance_z, AP_AVOIDANCE_WARN_DISTANCE_Z_DEFAULT),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("F_DIST_Z",    11, AGP_Avoidance, _fail_distance_z, AP_AVOIDANCE_FAIL_DISTANCE_Z_DEFAULT),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("F_ALT_MIN",    12, AGP_Avoidance, _fail_altitude_minimum, 0),

    AP_GROUPEND
};

AGP_Avoidance::AGP_Avoidance(AG_ADSB &adsb) :
    _adsb(adsb)
{
    AG_Param::setup_object_defaults(this, var_info);
    if (_singleton != nullptr) {
        AG_HAL::panic("AGP_Avoidance must be singleton");
    }
    _singleton = this;
}
 ...
 */
void AGP_Avoidance::init(void)
{
    debug("ADSB initialisation: %d obstacles", _obstacles_max.get());
    if (_obstacles == nullptr) {
        _obstacles = new AGP_Avoidance::Obstacle[_obstacles_max];
 ...
 */
void AGP_Avoidance::deinit(void)
{
    if (_obstacles != nullptr) {
        delete [] _obstacles;
        _obstacles = nullptr;
...
bool AGP_Avoidance::check_startup()
{
    if (!_enabled) {
        if (_obstacles != nullptr) {
            deinit();
...
void AGP_Avoidance::add_obstacle(const uint32_t obstacle_timestamp_ms,
...
uint32_t AGP_Avoidance::src_id_for_adsb_vehicle(const AG_ADSB::adsb_vehicle_t &vehicle) const
{
    // TODO: need to include squawk code and callsign
    return vehicle.info.ICAO_address;
}

void AGP_Avoidance::get_adsb_samples()
{
...
void AGP_Avoidance::update_threat_level(const Location &my_loc,
                                       const Vector3f &my_vel,
                                       AGP_Avoidance::Obstacle &obstacle)
{
    ...
    const uint32_t obstacle_age = AG_HAL::millis() - obstacle.timestamp_ms;
...
MAV_COLLISION_THREAT_LEVEL AGP_Avoidance::current_threat_level() const {
    if (_obstacles == nullptr) {
...
void AGP_Avoidance::send_collision_all(const AGP_Avoidance::Obstacle &threat, MAV_COLLISION_ACTION behaviour) const
{
...
void AGP_Avoidance::handle_threat_gcs_notify(AGP_Avoidance::Obstacle *threat)
{
    if (threat == nullptr) {
        return;
    }
...
bool AGP_Avoidance::obstacle_is_more_serious_threat(const AGP_Avoidance::Obstacle &obstacle) const
{
    if (_current_most_serious_threat == -1) {
        // any threat is more of a threat than no threat
        return true;
    }
    const AGP_Avoidance::Obstacle &current = _obstacles[_current_most_serious_threat];
...
void AGP_Avoidance::check_for_threats()
{
    ...
    for (uint8_t i=0; i<_obstacle_count; i++) {
        ...
        AGP_Avoidance::Obstacle &obstacle = _obstacles[i];
        const uint32_t obstacle_age = AG_HAL::millis() - obstacle.timestamp_ms;
...
AGP_Avoidance::Obstacle *AGP_Avoidance::most_serious_threat()
{
...
void AGP_Avoidance::handle_avoidance_local(AGP_Avoidance::Obstacle *threat)
{
...
void AGP_Avoidance::handle_msg(const mavlink_message_t &msg)
{
    if (!check_startup()) {
    ...
    // inform AGP_Avoidance we have a new player
...
bool AGP_Avoidance::get_vector_perpendicular(const AGP_Avoidance::Obstacle *obstacle, Vector3f &vec_neu) const
{
    if (obstacle == nullptr) {
        // why where we called?!
        return false;
...
// v1 is NED
Vector3f AGP_Avoidance::perpendicular_xyz(const Location &p1, const Vector3f &v1, const Location &p2)
{
...
// v1 is NED
Vector2f AGP_Avoidance::perpendicular_xy(const Location &p1, const Vector3f &v1, const Location &p2)
{
...
// singleton instance
AGP_Avoidance *AGP_Avoidance::_singleton;

namespace AP {

AGP_Avoidance *ap_avoidance()
{
    return AGP_Avoidance::get_singleton();
}
