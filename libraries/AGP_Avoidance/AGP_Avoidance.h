...
/*
 ...
 - produce messages for GCS if a collision risk is detected
  ...
  based on AG_ADSB,  Tom Pittenger, November 2015
*/
...
public:
    ...
    // constructor
    AGP_Avoidance(class AG_ADSB &adsb);

    /* Do not allow copies */
    CLASS_NO_COPY(AGP_Avoidance);

    // get singleton instance
    static AGP_Avoidance *get_singleton() {
        return _singleton;
    }
...
protected:
    ...
    void handle_avoidance_local(AGP_Avoidance::Obstacle *threat);
    ...
    virtual MAV_COLLISION_ACTION handle_avoidance(const AGP_Avoidance::Obstacle *obstacle, MAV_COLLISION_ACTION requested_action) = 0;
    ...
    void handle_threat_gcs_notify(AGP_Avoidance::Obstacle *threat);

    AGP_Avoidance::Obstacle *most_serious_threat();
    ...
    bool get_destination_perpendicular(const AGP_Avoidance::Obstacle *obstacle, Vector3f &newdest_neu, const float wp_speed_xy, const float wp_speed_z, const uint8_t _minimum_avoid_height);

    // get unit vector away from the nearest obstacle
    bool get_vector_perpendicular(const AGP_Avoidance::Obstacle *obstacle, Vector3f &vec_neu) const;
...
private:
    ...
    void send_collision_all(const AGP_Avoidance::Obstacle &threat, MAV_COLLISION_ACTION behaviour) const;
    ...
    uint32_t src_id_for_adsb_vehicle(const AG_ADSB::adsb_vehicle_t &vehicle) const;
    ...
    // calls into the AG_ADSB library to retrieve vehicle data
    ...
    bool obstacle_is_more_serious_threat(const AGP_Avoidance::Obstacle &obstacle) const;

    // internal variables
    AGP_Avoidance::Obstacle *_obstacles;
    ...
    static AGP_Avoidance *_singleton;
};
