...
// singleton instance
AG_MotorsUGV *AG_MotorsUGV::_singleton;

// parameters for the motor class
const AG_Param::GroupInfo AG_MotorsUGV::var_info[] = {
    // @Param: PWM_TYPE
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("PWM_TYPE", 1, AG_MotorsUGV, _pwm_type, PWM_TYPE_NORMAL),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("PWM_FREQ", 2, AG_MotorsUGV, _pwm_freq, 16),
    ...
    // @User: Advanced
    AP_GROUPINFO("SAFE_DISARM", 3, AG_MotorsUGV, _disarm_disable_pwm, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("THR_MIN", 5, AG_MotorsUGV, _throttle_min, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("THR_MAX", 6, AG_MotorsUGV, _throttle_max, 100),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("SLEWRATE", 8, AG_MotorsUGV, _slew_rate, 100),
    ...
    // @Range: -1.0 1.0
    // @User: Advanced
    AP_GROUPINFO("THST_EXPO", 9, AG_MotorsUGV, _thrust_curve_expo, 0.0f),
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO("SPD_SCA_BASE", 11, AG_MotorsUGV, _speed_scale_base, 1.0f),
    ...
    // @Range: 0.2 1.0
    // @User: Advanced
    AP_GROUPINFO("STR_THR_MIX", 12, AG_MotorsUGV, _steering_throttle_mix, 0.5f),
    ...
    // @Range: 0 90
    // @User: Standard
    AP_GROUPINFO("VEC_ANGLEMAX", 13, AG_MotorsUGV, _vector_angle_max, 0.0f),
    ...
    // @Range: 1.0 10.0
    // @User: Advanced
    AP_GROUPINFO("THST_ASYM", 14, AG_MotorsUGV, _thrust_asymmetry, 1.0f),

    AP_GROUPEND
};

AG_MotorsUGV::AG_MotorsUGV(AG_ServoRelayEvents &relayEvents, AG_WheelRateControl& rate_controller) :
...
void AG_MotorsUGV::init(uint8_t frtype)
{
...
void AG_MotorsUGV::setup_safety_output()
{
...
void AG_MotorsUGV::setup_servo_output()
{
...
void AG_MotorsUGV::set_steering(float steering, bool apply_scaling)
{
    _steering = steering;
...
void AG_MotorsUGV::set_throttle(float throttle)
{
...
float AG_MotorsUGV::get_slew_limited_throttle(float throttle, float dt) const
{
    if (_slew_rate <= 0) {
        return throttle;
    }
 ...
 */
bool AG_MotorsUGV::have_skid_steering() const
{
...
bool AG_MotorsUGV::has_sail() const
{
...
void AG_MotorsUGV::output(bool armed, float ground_speed, float dt)
{
...
bool AG_MotorsUGV::output_test_pct(motor_test_order motor_seq, float pct)
{
...
bool AG_MotorsUGV::output_test_pwm(motor_test_order motor_seq, float pwm)
{
    // check if the motor_seq is valid
...
bool AG_MotorsUGV::pre_arm_check(bool report) const
{
...
// sanity check parameters
void AG_MotorsUGV::sanity_check_parameters()
{
...
void AG_MotorsUGV::setup_pwm_type()
{
    _motor_mask = 0;
    ...
    case PWM_TYPE_ONESHOT:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_ONESHOT);
        break;
    case PWM_TYPE_ONESHOT125:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_ONESHOT125);
        break;
    case PWM_TYPE_BRUSHED_WITH_RELAY:
    case PWM_TYPE_BRUSHED_BIPOLAR:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_BRUSHED);
        hal.rcout->set_freq(_motor_mask, uint16_t(_pwm_freq * 1000));
        break;
    case PWM_TYPE_DSHOT150:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_DSHOT150);
        break;
    case PWM_TYPE_DSHOT300:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_DSHOT300);
        break;
    case PWM_TYPE_DSHOT600:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_DSHOT600);
        break;
    case PWM_TYPE_DSHOT1200:
        hal.rcout->set_output_mode(_motor_mask, AG_HAL::RCOutput::MODE_PWM_DSHOT1200);
        break;
    default:
        // do nothing
        break;
...
void AG_MotorsUGV::add_omni_motor(int8_t motor_num, float throttle_factor, float steering_factor, float lateral_factor)
{
...
void AG_MotorsUGV::add_omni_motor_num(int8_t motor_num)
{
...
void AG_MotorsUGV::clear_omni_motors(int8_t motor_num)
{
    // ensure valid motor number is provided
    if (motor_num >= 0 && motor_num < AP_MOTORS_NUM_MOTORS_MAX) {
...
void AG_MotorsUGV::output_regular(bool armed, float ground_speed, float steering, float throttle)
{
...
void AG_MotorsUGV::output_skid_steering(bool armed, float steering, float throttle, float dt)
{
    if (!have_skid_steering()) {
        return;
    }
...
// output for omni frames
void AG_MotorsUGV::output_omni(bool armed, float steering, float throttle, float lateral)
{
...
void AG_MotorsUGV::output_throttle(SRV_Channel::Aux_servo_function_t function, float throttle, float dt)
{
...
void AG_MotorsUGV::slew_limit_throttle(float dt)
{
...
void AG_MotorsUGV::set_limits_from_input(bool armed, float steering, float throttle)
{
...
float AG_MotorsUGV::get_scaled_throttle(float throttle) const
{
...
float AG_MotorsUGV::get_rate_controlled_throttle(SRV_Channel::Aux_servo_function_t function, float throttle, float dt)
{
    // require non-zero dt
    if (!is_positive(dt)) {
        return throttle;
...
bool AG_MotorsUGV::active() const
{
...
bool AG_MotorsUGV::is_digital_pwm_type() const
{
    switch (_pwm_type) {
        case PWM_TYPE_DSHOT150:
        case PWM_TYPE_DSHOT300:
...
namespace AP {
    AG_MotorsUGV *motors_ugv()
    {
        return AG_MotorsUGV::get_singleton();
    }
}
