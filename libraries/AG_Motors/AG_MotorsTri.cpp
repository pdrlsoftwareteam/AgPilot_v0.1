...
#include "AG_MotorsTri.h"

extern const AG_HAL::HAL& hal;

// init
void AG_MotorsTri::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AG_MotorsTri::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
    // check for reverse tricopter
    if (frame_type == MOTOR_FRAME_TYPE_PLUSREV) {
        _pitch_reversed = true;
...
void AG_MotorsTri::set_update_rate(uint16_t speed_hz)
{
...
void AG_MotorsTri::output_to_motors()
{
    switch (_spool_state) {
...
uint32_t AG_MotorsTri::get_motor_mask()
{
...
void AG_MotorsTri::output_armed_stabilizing()
{
...
void AG_MotorsTri::_output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
*/
void AG_MotorsTri::thrust_compensation(void)
{
 ...
 */
void AG_MotorsTri::output_motor_mask(float thrust, uint16_t mask, float rudder_dt)
{
    // normal multicopter output
    AG_MotorsMulticopter::output_motor_mask(thrust, mask, rudder_dt);

    // and override yaw servo
    rc_write_angle(AP_MOTORS_CH_TRI_YAW, 0);
}

float AG_MotorsTri::get_roll_factor(uint8_t i)
{
    float ret = 0.0f;
