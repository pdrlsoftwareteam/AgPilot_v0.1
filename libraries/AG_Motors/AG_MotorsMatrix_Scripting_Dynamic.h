...
class AG_MotorsMatrix_Scripting_Dynamic : public AG_MotorsMatrix {
public:

    // Constructor
    AG_MotorsMatrix_Scripting_Dynamic(uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
        AG_MotorsMatrix(speed_hz)
    {
        if (_singleton != nullptr) {
            AG_HAL::panic("AG_MotorsMatrix_Scripting_Dynamic must be singleton");
        }
        _singleton = this;
    };

    // get singleton instance
    static AG_MotorsMatrix_Scripting_Dynamic *get_singleton() {
        return _singleton;
    }
...
private:
    ...
    static AG_MotorsMatrix_Scripting_Dynamic *_singleton;
};
