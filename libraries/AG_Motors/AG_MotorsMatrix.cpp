...
#include "AG_MotorsMatrix.h"
#include <AG_Vehicle/AG_Vehicle_Type.h>

extern const AG_HAL::HAL& hal;

// init
void AG_MotorsMatrix::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
bool AG_MotorsMatrix::init(uint8_t expected_num_motors)
{
...
bool AG_MotorsMatrix::set_throttle_factor(int8_t motor_num, float throttle_factor)
{
    if ((_active_frame_class != MOTOR_FRAME_SCRIPTING_MATRIX) ) {
        // not the correct class
        return false;
...
void AG_MotorsMatrix::set_update_rate(uint16_t speed_hz)
{
...
void AG_MotorsMatrix::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AG_MotorsMatrix::output_to_motors()
{
    int8_t i;
...
uint32_t AG_MotorsMatrix::get_motor_mask()
{
    ...
    // add parent's mask
    mask |= AG_MotorsMulticopter::get_motor_mask();
...
void AG_MotorsMatrix::output_armed_stabilizing()
{
...
void AG_MotorsMatrix::check_for_failed_motor(float throttle_thrust_best_plus_adj)
{
...
void AG_MotorsMatrix::_output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
//  pwm value is an actual pwm value that will be output, normally in the range of 1000 ~ 2000
bool AG_MotorsMatrix::output_test_num(uint8_t output_channel, int16_t pwm)
{
    if (!armed()) {
        return false;
    }
...
// add_motor
void AG_MotorsMatrix::add_motor_raw(int8_t motor_num, float roll_fac, float pitch_fac, float yaw_fac, uint8_t testing_order, float throttle_factor)
{
    if (initialised_ok()) {
...
void AG_MotorsMatrix::add_motor(int8_t motor_num, float angle_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor(motor_num, angle_degrees, angle_degrees, yaw_factor, testing_order);
}

// add_motor using position and prop direction. Roll and Pitch factors can differ (for asymmetrical frames)
void AG_MotorsMatrix::add_motor(int8_t motor_num, float roll_factor_in_degrees, float pitch_factor_in_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor_raw(
        motor_num,
...
void AG_MotorsMatrix::remove_motor(int8_t motor_num)
{
    // ensure valid motor number is provided
    if (motor_num >= 0 && motor_num < AP_MOTORS_MAX_NUM_MOTORS) {
...
void AG_MotorsMatrix::add_motors(const struct MotorDef *motors, uint8_t num_motors)
{
...
void AG_MotorsMatrix::add_motors_raw(const struct MotorDefRaw *motors, uint8_t num_motors)
{
    for (uint8_t i=0; i<num_motors; i++) {
...
bool AG_MotorsMatrix::setup_quad_matrix(motor_frame_type frame_type)
{
    ...
    case MOTOR_FRAME_TYPE_PLUS: {
        ...
        static const AG_MotorsMatrix::MotorDef motors[] {
    ...
    case MOTOR_FRAME_TYPE_Y4:
        ...
        static const AG_MotorsMatrix::MotorDefRaw motors[] {
...
bool AG_MotorsMatrix::setup_hexa_matrix(motor_frame_type frame_type)
{
...
bool AG_MotorsMatrix::setup_octa_matrix(motor_frame_type frame_type)
{
...
bool AG_MotorsMatrix::setup_octaquad_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_OCTOROTOR;
...
bool AG_MotorsMatrix::setup_dodecahexa_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_DODECAROTOR;
...
bool AG_MotorsMatrix::setup_y6_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_HEXAROTOR;
...
bool AG_MotorsMatrix::setup_deca_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_DECAROTOR;
...
void AG_MotorsMatrix::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AG_MotorsMatrix::normalise_rpy_factors()
{
    float roll_fac = 0.0f;
...
*/
void AG_MotorsMatrix::thrust_compensation(void)
{
...
*/
void AG_MotorsMatrix::disable_yaw_torque(void)
{
    for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        _yaw_factor[i] = 0;
    }
...
// singleton instance
AG_MotorsMatrix *AG_MotorsMatrix::_singleton;
