...
#include "AG_MotorsMatrix_Scripting_Dynamic.h"
...
bool AG_MotorsMatrix_Scripting_Dynamic::add_motor(uint8_t motor_num, uint8_t testing_order)
{
    if (initialised_ok()) {
...
void AG_MotorsMatrix_Scripting_Dynamic::load_factors(const factor_table &new_table)
{
    WITH_SEMAPHORE(_sem);
    had_table = true;
...
bool AG_MotorsMatrix_Scripting_Dynamic::init(uint8_t expected_num_motors)
{
    WITH_SEMAPHORE(_sem);
...
void AG_MotorsMatrix_Scripting_Dynamic::output_to_motors()
{
    WITH_SEMAPHORE(_sem);

    // call the base class ouput function
    AG_MotorsMatrix::output_to_motors();
}

// singleton instance
AG_MotorsMatrix_Scripting_Dynamic *AG_MotorsMatrix_Scripting_Dynamic::_singleton;
