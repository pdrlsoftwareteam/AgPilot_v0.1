...
// singleton instance
AG_Motors *AG_Motors::_singleton;

// Constructor
AG_Motors::AG_Motors(uint16_t speed_hz) :
    _speed_hz(speed_hz),
    _throttle_filter(),
    _throttle_slew(),
...
void AG_Motors::get_frame_and_type_string(char *buffer, uint8_t buflen) const
{
...
void AG_Motors::armed(bool arm)
{
    if (_armed != arm) {
        _armed = arm;
        AG_Notify::flags.armed = arm;
        if (!arm) {
...
void AG_Motors::set_desired_spool_state(DesiredSpoolState spool)
{
...
void AG_Motors::set_radio_passthrough(float roll_input, float pitch_input, float throttle_input, float yaw_input)
{
 ...
 */
void AG_Motors::rc_write(uint8_t chan, uint16_t pwm)
{
 ...
 */
void AG_Motors::rc_write_angle(uint8_t chan, int16_t angle_cd)
{
    SRV_Channel::Aux_servo_function_t function = SRV_Channels::get_motor_function(chan);
 ...
 */
void AG_Motors::rc_set_freq(uint32_t motor_mask, uint16_t freq_hz)
{
    if (freq_hz > 50) {
    ...
    case PWM_TYPE_ONESHOT:
        if (freq_hz > 50 && mask != 0) {
            hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_ONESHOT);
        }
        break;
    case PWM_TYPE_ONESHOT125:
        if (freq_hz > 50 && mask != 0) {
            hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_ONESHOT125);
        }
        break;
    case PWM_TYPE_BRUSHED:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_BRUSHED);
        break;
    case PWM_TYPE_DSHOT150:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_DSHOT150);
        break;
    case PWM_TYPE_DSHOT300:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_DSHOT300);
        break;
    case PWM_TYPE_DSHOT600:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_DSHOT600);
        break;
    case PWM_TYPE_DSHOT1200:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_DSHOT1200);
        break;
    case PWM_TYPE_PWM_RANGE:
        ...
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_NORMAL);
        break;
    default:
        hal.rcout->set_output_mode(mask, AG_HAL::RCOutput::MODE_PWM_NORMAL);
        break;
...
/*
  ...
  motor number
 */
uint32_t AG_Motors::motor_mask_to_srv_channel_mask(uint32_t mask) const
{
    uint32_t mask2 = 0;
 ...
 */
void AG_Motors::add_motor_num(int8_t motor_num)
{
...
void AG_Motors::set_limit_flag_pitch_roll_yaw(bool flag)
{
    limit.roll = flag;
    limit.pitch = flag;
    limit.yaw = flag;
...
#if AP_SCRIPTING_ENABLED
void AG_Motors::set_external_limits(bool roll, bool pitch, bool yaw, bool throttle_lower, bool throttle_upper)
{
...
bool AG_Motors::is_digital_pwm_type() const
{
    switch (_pwm_type) {
        case PWM_TYPE_DSHOT150:
        case PWM_TYPE_DSHOT300:
...
const char* AG_Motors::get_frame_string() const
{
#if AP_SCRIPTING_ENABLED
...
void AG_Motors::set_frame_string(const char * str) {
    if (custom_frame_string != nullptr) {
        return;
    }
...
void AG_Motors::output_test_seq(uint8_t motor_seq, int16_t pwm)
{
    if (armed() && _interlock) {
...
bool AG_Motors::arming_checks(size_t buflen, char *buffer) const
{
    if (!initialised_ok()) {
...
namespace AP {
    AG_Motors *motors()
    {
        return AG_Motors::get_singleton();
    }
}
