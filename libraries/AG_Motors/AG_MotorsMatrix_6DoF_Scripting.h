...
class AG_MotorsMatrix_6DoF_Scripting : public AG_MotorsMatrix {
public:

    /// Constructor
    AG_MotorsMatrix_6DoF_Scripting(uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
        AG_MotorsMatrix(speed_hz)
    {
        if (_singleton != nullptr) {
            AG_HAL::panic("AG_MotorsMatrix 6DoF must be singleton");
        }
        _singleton = this;
    };

    // get singleton instance
    static AG_MotorsMatrix_6DoF_Scripting *get_singleton() {
        return _singleton;
    }
...
private:
    static AG_MotorsMatrix_6DoF_Scripting *_singleton;
