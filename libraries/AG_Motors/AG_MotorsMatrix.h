...
/// @class      AG_MotorsMatrix
class AG_MotorsMatrix : public AG_MotorsMulticopter {
public:

    /// Constructor
    AG_MotorsMatrix(uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
        AG_MotorsMulticopter(speed_hz)
    {
        if (_singleton != nullptr) {
            AG_HAL::panic("AG_MotorsMatrix must be singleton");
        }
        _singleton = this;
    };

    // get singleton instance
    static AG_MotorsMatrix *get_singleton() {
        return _singleton;
    }
...
private:
    ...
    static AG_MotorsMatrix *_singleton;
};
