...
/*
 *       AG_Motors6DOF.cpp - ArduSub motors library
 */

#include <AG_BattMonitor/AG_BattMonitor.h>
#include <AG_HAL/AG_HAL.h>
#include "AG_Motors6DOF.h"

extern const AG_HAL::HAL& hal;

// parameters for the motor class
const AG_Param::GroupInfo AG_Motors6DOF::var_info[] = {
    AP_NESTEDGROUPINFO(AG_MotorsMulticopter, 0),
    // @Param: 1_DIRECTION
    ...
    // @User: Standard
    AP_GROUPINFO("1_DIRECTION", 1, AG_Motors6DOF, _motor_reverse[0], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("2_DIRECTION", 2, AG_Motors6DOF, _motor_reverse[1], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("3_DIRECTION", 3, AG_Motors6DOF, _motor_reverse[2], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("4_DIRECTION", 4, AG_Motors6DOF, _motor_reverse[3], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("5_DIRECTION", 5, AG_Motors6DOF, _motor_reverse[4], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("6_DIRECTION", 6, AG_Motors6DOF, _motor_reverse[5], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("7_DIRECTION", 7, AG_Motors6DOF, _motor_reverse[6], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("8_DIRECTION", 8, AG_Motors6DOF, _motor_reverse[7], 1),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("FV_CPLNG_K", 9, AG_Motors6DOF, _forwardVerticalCouplingFactor, 1.0),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("9_DIRECTION", 10, AG_Motors6DOF, _motor_reverse[8], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("10_DIRECTION", 11, AG_Motors6DOF, _motor_reverse[9], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("11_DIRECTION", 12, AG_Motors6DOF, _motor_reverse[10], 1),
    ...
    // @Values: 1:normal,-1:reverse
    // @User: Standard
    AP_GROUPINFO("12_DIRECTION", 13, AG_Motors6DOF, _motor_reverse[11], 1),

    AP_GROUPEND
};

void AG_Motors6DOF::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AG_Motors6DOF::add_motor_raw_6dof(int8_t motor_num, float roll_fac, float pitch_fac, float yaw_fac, float throttle_fac, float forward_fac, float lat_fac, uint8_t testing_order)
{
...
int16_t AG_Motors6DOF::calc_thrust_to_pwm(float thrust_in) const
{
    return constrain_int16(1500 + thrust_in * 400, get_pwm_output_min(), get_pwm_output_max());
}

void AG_Motors6DOF::output_to_motors()
{
    int8_t i;
...
float AG_Motors6DOF::get_current_limit_max_throttle()
{
    return 1.0f;
}
...
void AG_Motors6DOF::output_armed_stabilizing()
{
...
// TODO pull code that is common to output_armed_not_stabilizing into helper functions
// ToDo calculate headroom for rpy to be added for stabilization during full throttle/forward/lateral commands
void AG_Motors6DOF::output_armed_stabilizing_vectored()
{
    uint8_t i;                          // general purpose counter
    float   roll_thrust;                // roll thrust input value, +/- 1.0
    float   pitch_thrust;               // pitch thrust input value, +/- 1.0
...
void AG_Motors6DOF::output_armed_stabilizing_vectored_6dof()
{
    uint8_t i;                          // general purpose counter
    float   roll_thrust;                // roll thrust input value, +/- 1.0
    float   pitch_thrust;               // pitch thrust input value, +/- 1.0
...
Vector3f AG_Motors6DOF::get_motor_angular_factors(int motor_number) {
...
bool AG_Motors6DOF::motor_is_enabled(int motor_number) {
    if (motor_number < 0 || motor_number >= AP_MOTORS_MAX_NUM_MOTORS) {
        return false;
    }
...
bool AG_Motors6DOF::set_reversed(int motor_number, bool reversed) {
    if (motor_number < 0 || motor_number >= AP_MOTORS_MAX_NUM_MOTORS) {
        return false;
    }
    if (reversed) {
