 ...
 *      Author: Owner
 */
#include "AG_PDRL_Commander.h"
#include "AG_LIBNPNT/PdrlBootPlugin.h"
...
void AP_PDRL_COMMANDER::sendIsPubKeyPresent()
{
	...
	//	m_AG_Direct_Flash_Access->readFromInflash(droneIDBuffer,30,DRONE_ID_ADDRESS);
...
//		keyStore->setGenarateKeyFlagForReboot();
	...
	case COMMAND_SET_FLIGHT_START_TIME:
	{
		...
		memcpy(&m_AG_NPNT_data_helper->flightEndTime,packet.command_buff,sizeof(uint8_t));//*dataPtr;
		m_AG_NPNT_data_helper->flightStartTime *= (uint64_t)1000000;
	}
	break;
	...
	case COMMAND_SET_FLIGHT_END_TIME:
	{
		// uint64_t *dataPtr= (uint64_t*)packet.command_buff;
		memcpy(&m_AG_NPNT_data_helper->flightEndTime,packet.command_buff,sizeof(uint8_t));//*dataPtr;
		m_AG_NPNT_data_helper->flightEndTime *= (uint64_t)1000000;
	}
	break;
...
//		gcs().send_text(MAV_SEVERITY_ERROR, "Sent flag status");
	...
	case COMMAND_SET_PERMISSION_ARTIFACTS:
	{
		...
		m_AG_NPNT_data_helper->setParam(m_AG_NPNT_data_helper->permissionArticaftID,(char*)packet.command_buff);
		AP_PDRL_Logger::getInstance()->setLogFileName(m_AG_NPNT_data_helper->permissionArticaftID);
	}
	break;
