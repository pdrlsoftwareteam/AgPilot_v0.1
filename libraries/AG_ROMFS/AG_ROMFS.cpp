...
#ifdef HAL_HAVE_AG_ROMFS_EMBEDDED_H
#include <ap_romfs_embedded.h>
#else
const AG_ROMFS::embedded_file AG_ROMFS::files[] = {};
#endif

/*
  find an embedded file
*/
const uint8_t *AG_ROMFS::find_file(const char *name, uint32_t &size, uint32_t &crc)
{
...
/*
  ...
  be null
*/
const uint8_t *AG_ROMFS::find_decompress(const char *name, uint32_t &size)
{
...
// free returned data
void AG_ROMFS::free(const uint8_t *data)
{
...
/*
  ...
  files found
*/
const char *AG_ROMFS::dir_list(const char *dirname, uint16_t &ofs)
{
