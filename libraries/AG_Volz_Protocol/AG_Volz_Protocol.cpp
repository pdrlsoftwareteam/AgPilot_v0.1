...
const AG_Param::GroupInfo AG_Volz_Protocol::var_info[] = {
    // @Param: MASK
    ...
    // @User: Standard
    AP_GROUPINFO("MASK",  1, AG_Volz_Protocol, bitmask, 0),

    AP_GROUPEND
};

// constructor
AG_Volz_Protocol::AG_Volz_Protocol(void)
{
    // set defaults from the parameter table
    AG_Param::setup_object_defaults(this, var_info);
}

void AG_Volz_Protocol::init(void)
{
    AG_SerialManager &serial_manager = AP::serialmanager();
    port = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Volz,0);
    update_volz_bitmask(bitmask);
}

void AG_Volz_Protocol::update()
{
    if (!initialised) {
        initialised = true;
        init();
...
void AG_Volz_Protocol::send_command(uint8_t data[VOLZ_DATA_FRAME_SIZE])
{
    uint8_t i,j;
    uint16_t crc = 0xFFFF;
...
void AG_Volz_Protocol::update_volz_bitmask(uint32_t new_bitmask)
{
    uint8_t count = 0;
