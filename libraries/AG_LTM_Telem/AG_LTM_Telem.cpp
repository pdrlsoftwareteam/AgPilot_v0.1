...
void AG_LTM_Telem::init()
{
    ...
    if ((_port = serial_manager.find_serial(
            AG_SerialManager::SerialProtocol_LTM_Telem, 0))) {
        ...
        hal.scheduler->register_io_process(
                FUNCTOR_BIND_MEMBER(&AG_LTM_Telem::tick, void));
    }
}

void AG_LTM_Telem::send_LTM(uint8_t lt_packet[], uint8_t lt_packet_size)
{
...
// GPS frame
void AG_LTM_Telem::send_Gframe(void)
{
...
// Sensors frame
void AG_LTM_Telem::send_Sframe(void)
{
    ...
    const AG_Airspeed *aspeed = AP::airspeed();
    if (aspeed && aspeed->enabled()) {
        airspeed = (uint8_t) roundf(aspeed->get_airspeed());
    }
#endif

    const uint8_t flightmode = AG_Notify::flags.flight_mode; // flight mode
    ...
    const uint8_t armstat = AG_Notify::flags.armed;                                     // 0: disarmed, 1: armed
    const uint8_t failsafe = AG_Notify::flags.failsafe_radio;                           // 0: normal,   1: failsafe
...
// Attitude frame
void AG_LTM_Telem::send_Aframe(void)
{
    int16_t pitch;
    int16_t roll;
    int16_t heading;
...
// send LTM
void AG_LTM_Telem::generate_LTM(void)
{
