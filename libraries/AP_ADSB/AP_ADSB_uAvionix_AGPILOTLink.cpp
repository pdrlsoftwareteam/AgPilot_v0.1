...
// detect if an port is configured as AGPILOTLink
bool AP_ADSB_uAvionix_AGPILOTLink::detect()
{
    // this actually requires SerialProtocol_AGPILOTLink or SerialProtocol_AGPILOTLink2 but
    // we can't have a running system with that, so its safe to assume it's already defined
    return true;
}

void AP_ADSB_uAvionix_AGPILOTLink::update()
{
    ...
    if (_frontend.out_state.chan_last_ms > 0 && now - _frontend.out_state.chan_last_ms > ADSB_CHAN_TIMEOUT_MS) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_ERROR, "ADSB: Transceiver heartbeat timed out");
    } else if (_frontend.out_state.chan >= 0 && !_frontend._my_loc.is_zero() && _frontend.out_state.chan < AGPILOTLINK_COMM_NUM_BUFFERS) {
        const mavlink_channel_t chan = (mavlink_channel_t)(AGPILOTLINK_COMM_0 + _frontend.out_state.chan);
...
void AP_ADSB_uAvionix_AGPILOTLink::send_dynamic_out(const mavlink_channel_t chan) const
{
...
/*
 * To expand functionality in their HW, uAvionix has extended a few of the unused AGPILOTLink bits to pack in more new features
 ...
 */
uint32_t AP_ADSB_uAvionix_AGPILOTLink::encode_icao(const uint32_t icao_id) const
{
    // utilize the upper unused 8bits of the icao with special flags.
    // This encoding is required for uAvionix devices that break the AGPILOTLink spec.
 ...
 */
uint8_t AP_ADSB_uAvionix_AGPILOTLink::get_encoded_callsign_null_char()
{
...
/*
 * populate and send AGPILOTLINK_MSG_UAVIONIX_ADSB_OUT_CFG
 */
void AP_ADSB_uAvionix_AGPILOTLink::send_configure(const mavlink_channel_t chan)
{
    // AGPILOTLink spec says the 9 byte callsign field is 8 byte string with 9th byte as null.
    ...
    if (_frontend.out_state.cfg.was_set_externally) {
        ...
        callsign[AGPILOTLINK_MSG_UAVIONIX_ADSB_OUT_CFG_FIELD_CALLSIGN_LEN-1] = (int8_t)get_encoded_callsign_null_char();
