...
#include "math_test.h"
#include <AG_Math/AG_GeodesicGrid.h>
...
public:
    ...
    Vector3f v;
    /**
     * Expected section if when AG_GeodesicGrid::section() is called with
     ...
     * AG_GeodesicGrid::section() with inclusive set as true expects a return
    ...
    void test_triangles_indexes(const TestParam &p) {
        if (p.section >= 0) {
            int expected_triangle =
                    p.section / AG_GeodesicGrid::NUM_SUBTRIANGLES;
            int triangle = AG_GeodesicGrid::_triangle_index(p.v, false);
            ASSERT_EQ(expected_triangle, triangle);

            int expected_subtriangle =
                    p.section % AG_GeodesicGrid::NUM_SUBTRIANGLES;
            int subtriangle =
                    AG_GeodesicGrid::_subtriangle_index(triangle, p.v, false);
            ASSERT_EQ(expected_subtriangle, subtriangle);
        } else {
            int triangle = AG_GeodesicGrid::_triangle_index(p.v, false);
            if (triangle >= 0) {
                int subtriangle = AG_GeodesicGrid::_subtriangle_index(triangle,
                                                                      p.v,
                                                                      false);
...
TEST_P(GeodesicGridTest, Sections)
{
    ...
    EXPECT_EQ(p.section, AG_GeodesicGrid::section(p.v));

    if (p.section < 0) {
        int s = AG_GeodesicGrid::section(p.v, true);
        int i;
...
static std::vector<TestParam> general_vectors = []()
{
    ...
    for (int i = 0; i < 20 * AG_GeodesicGrid::NUM_SUBTRIANGLES; i++) {
        Vector3f a, b, c;
        TestParam p;
 ...
 * implementation*)
 *
 * See AG_GeodesicGrid.h for the notation on the comments below.
 */
