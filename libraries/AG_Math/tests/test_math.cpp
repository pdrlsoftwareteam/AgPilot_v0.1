...
#define TEST_ROTATION(rotation, _x, _y, _z) { \
    ...
    Vector3F v {1, 1, 1};
    EXPECT_EXIT(v.rotate(ROTATION_CUSTOM_OLD), testing::KilledBySignal(SIGABRT), "AG_InternalError::error_t::bad_rotation");
    EXPECT_EXIT(v.rotate(ROTATION_MAX), testing::KilledBySignal(SIGABRT), "AG_InternalError::error_t::bad_rotation");
#endif
}
...
#elif CONFIG_HAL_BOARD == HAL_BOARD_SITL
    EXPECT_EXIT(constrain_float(nanf("0x4152"), 1.0f, 1.0f), testing::KilledBySignal(SIGABRT), "AG_InternalError::error_t::cnstring_nan");
    EXPECT_EXIT(constrain_value(nanf("0x4152"), 1.0f, 1.0f), testing::KilledBySignal(SIGABRT), "AG_InternalError::error_t::cnstring_nan");
#endif
}
