...
class AG_ADSB {
public:
    friend class AG_ADSB_Backend;
    friend class AG_ADSB_uAvionix_MAVLink;
    friend class AG_ADSB_uAvionix_UCP;
    friend class AG_ADSB_Sagetech;
    friend class AG_ADSB_Sagetech_MXS;

    // constructor
    AG_ADSB();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_ADSB);

    // get singleton instance
    static AG_ADSB *get_singleton(void) {
        return _singleton;
    }
    ...
    AG_ADSB::Type get_type(uint8_t instance) const;

private:
    static AG_ADSB *_singleton;
    ...
    AG_ADSB_Backend *_backend[ADSB_MAX_INSTANCES];
};
