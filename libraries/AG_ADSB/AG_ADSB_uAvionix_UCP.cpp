...
#define AG_ADSB_UAVIONIX_HEALTH_TIMEOUT_MS                     (5000UL)

#define AG_ADSB_UAVIONIX_GCS_LOST_COMMS_LONG_TIMEOUT_MINUTES    (15UL)
#define AG_ADSB_UAVIONIX_GCS_LOST_COMMS_LONG_TIMEOUT_MS         (1000UL * 60UL * AG_ADSB_UAVIONIX_GCS_LOST_COMMS_LONG_TIMEOUT_MINUTES)

#define AG_ADSB_UAVIONIX_DETECT_GROUNDSTATE                     0
#define AG_ADSB_UAVIONIX_EMERGENCY_STATUS_ON_LOST_LINK          0

// detect if any port is configured as uAvionix_UCP
bool AG_ADSB_uAvionix_UCP::detect()
{
    return AP::serialmanager().have_serial(AG_SerialManager::SerialProtocol_ADSB, 0);
}


// Init, called once after class is constructed
bool AG_ADSB_uAvionix_UCP::init()
{
    _port = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_ADSB, 0);
    if (_port == nullptr) {
        return false;
    }
...
void AG_ADSB_uAvionix_UCP::update()
{
    if (_port == nullptr) {
        return;
    }
   ...
   if (now_ms - run_state.last_packet_Transponder_Control_ms >= 1000) {
    ...
    if ((now_ms - run_state.last_packet_GPS_ms >= 200) && (_frontend._options & uint32_t(AG_ADSB::AdsbOption::Ping200X_Send_GPS)) != 0) {
...
void AG_ADSB_uAvionix_UCP::handle_msg(const GDL90_RX_MESSAGE &msg)
{
    ...
    case GDL90_ID_HEARTBEAT: {
        ...
        run_state.last_packet_Transponder_Heartbeat_ms = AG_HAL::millis();
...
#if AG_ADSB_UAVIONIX_UCP_CAPTURE_ALL_RX_PACKETS
    case GDL90_ID_OWNSHIP_REPORT:
        ...
        run_state.last_packet_Transponder_Ownship_ms = AG_HAL::millis();
    ...
    case GDL90_ID_TRANSPONDER_STATUS:
        ...
        run_state.last_packet_Transponder_Status_ms = AG_HAL::millis();
        gcs().send_message(MSG_UAVIONIX_ADSB_OUT_STATUS);
        break;
#endif // AG_ADSB_UAVIONIX_UCP_CAPTURE_ALL_RX_PACKETS
...
const char* AG_ADSB_uAvionix_UCP::get_hardware_name(const uint8_t hwId)
{
    switch(hwId) {
...
void AG_ADSB_uAvionix_UCP::send_Transponder_Control()
{
...
#elif AG_ADSB_UAVIONIX_DETECT_GROUNDSTATE
    ...
    if (((_frontend._options & uint32_t(AG_ADSB::AdsbOption::Squawk_7400_FS_RC)) && notify.flags.failsafe_radio) ||
        ((_frontend._options & uint32_t(AG_ADSB::AdsbOption::Squawk_7400_FS_GCS)) && notify.flags.failsafe_gcs)) {
        msg.squawkCode = 7400;
    } else {
        msg.squawkCode = _frontend.out_state.ctrl.squawkCode;
    }

#if AG_ADSB_UAVIONIX_EMERGENCY_STATUS_ON_LOST_LINK
    const uint32_t last_gcs_ms = gcs().sysid_myggcs_last_seen_time_ms();
    const bool gcs_lost_comms = (last_gcs_ms != 0) && (AG_HAL::millis() - last_gcs_ms > AG_ADSB_UAVIONIX_GCS_LOST_COMMS_LONG_TIMEOUT_MS);
...
void AG_ADSB_uAvionix_UCP::send_GPS_Data()
{
...
bool AG_ADSB_uAvionix_UCP::hostTransmit(uint8_t *buffer, uint16_t length)
{
...
bool AG_ADSB_uAvionix_UCP::request_msg(const GDL90_MESSAGE_ID msg_id)
{
...
uint16_t AG_ADSB_uAvionix_UCP::gdl90Transmit(GDL90_TX_MESSAGE &message, const uint16_t length)
{
...
bool AG_ADSB_uAvionix_UCP::parseByte(const uint8_t data, GDL90_RX_MESSAGE &msg, GDL90_RX_STATUS &status)
{
    switch (status.state)
    {
    case GDL90_RX_IDLE:
