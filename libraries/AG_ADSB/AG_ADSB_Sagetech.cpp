...
void AG_ADSB_Sagetech::send_packet(const MsgType_XP type)
{
    switch (type) {
...
void AG_ADSB_Sagetech::request_packet(const MsgType_XP type)
{
...
void AG_ADSB_Sagetech::handle_packet_XP(const Packet_XP &msg)
{
    switch (msg.type) {
    case MsgType_XP::ACK:
        handle_ack(msg);
...
void AG_ADSB_Sagetech::handle_ack(const Packet_XP &msg)
{
    // ACK received!
...
void AG_ADSB_Sagetech::handle_adsb_in_msg(const Packet_XP &msg)
{
...
bool AG_ADSB_Sagetech::parse_byte_XP(const uint8_t data)
{
...
uint16_t AG_ADSB_Sagetech::checksum_generate_XP(Packet_XP &msg) const
{
    uint8_t sum = 0;
...
bool AG_ADSB_Sagetech::checksum_verify_XP(Packet_XP &msg) const
{
...
void AG_ADSB_Sagetech::checksum_assign_XP(Packet_XP &msg)
{
    const uint16_t checksum = checksum_generate_XP(msg);
...
void AG_ADSB_Sagetech::send_msg(Packet_XP &msg)
{
...
void AG_ADSB_Sagetech::send_msg_Installation()
{
    Packet_XP pkt {};
...
//    // convert a decimal 123456 to 0x123456
    ...
    const uint32_t icao_hex = AG_ADSB::convert_base_to_decimal(16, _frontend.out_state.cfg.ICAO_id_param);
...
void AG_ADSB_Sagetech::send_msg_PreFlight()
{
    Packet_XP pkt {};
...
void AG_ADSB_Sagetech::send_msg_Operating()
{
    Packet_XP pkt {};
    ...
    const uint16_t squawk = AG_ADSB::convert_base_to_decimal(8, last_operating_squawk);
...
void AG_ADSB_Sagetech::send_msg_GPS()
{
    Packet_XP pkt {};
    ...
    hemisphere |= (AP::gps().status() < AG_GPS::GPS_OK_FIX_2D) ? 0x80 : 0;  // isInvalid
