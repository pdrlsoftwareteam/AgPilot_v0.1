...
void AG_ADSB_Sagetech_MXS::update() 
{
    if (_port == nullptr) {
        return;
    }
    ...
    if (!mxs_state.init) {
        if (!last.packet_initialize_ms || (now_ms - last.packet_initialize_ms >= SAGETECH_INSTALL_MSG_RATE)) {
            ...
            if (_frontend._options & uint32_t(AG_ADSB::AdsbOption::SagteTech_MXS_External_Config)) {
...
void AG_ADSB_Sagetech_MXS::handle_packet(const Packet &msg) 
{
...
bool AG_ADSB_Sagetech_MXS::parse_byte(const uint8_t data) 
{
...
void AG_ADSB_Sagetech_MXS::msg_write(const uint8_t *data, const uint16_t len) const
{
    if (_port != nullptr) {
...
void AG_ADSB_Sagetech_MXS::auto_config_operating()
{
// Configure the Default Operation Message Data
    mxs_state.op.squawk = AG_ADSB::convert_base_to_decimal(8, _frontend.out_state.cfg.squawk_octal);
...
void AG_ADSB_Sagetech_MXS::auto_config_installation()
{
...
void AG_ADSB_Sagetech_MXS::auto_config_flightid()
{
...
void AG_ADSB_Sagetech_MXS::handle_ack(const sg_ack_t ack)
{
...
void AG_ADSB_Sagetech_MXS::handle_svr(const sg_svr_t svr) 
{
    ...
    vehicle.last_update_ms = AG_HAL::millis();
    _frontend.handle_adsb_vehicle(vehicle);
}

void AG_ADSB_Sagetech_MXS::handle_msr(const sg_msr_t msr) 
{
    AG_ADSB::adsb_vehicle_t vehicle;
...
void AG_ADSB_Sagetech_MXS::send_data_req(const sg_datatype_t dataReqType) 
{
...
void AG_ADSB_Sagetech_MXS::send_install_msg()
{
...
void AG_ADSB_Sagetech_MXS::send_flight_id_msg()
{
...
void AG_ADSB_Sagetech_MXS::send_operating_msg()
{
    ...
    mxs_state.op.squawk = AG_ADSB::convert_base_to_decimal(8, last.operating_squawk);
...
void AG_ADSB_Sagetech_MXS::send_gps_msg()
{
    sg_gps_t gps {};
    const AG_GPS &ap_gps = AP::gps();
    ...
    gps.gpsValid = (AP::gps().status() < AG_GPS::GPS_OK_FIX_2D) ? false : true;  // If the status is not OK, gpsValid is false.
...
void AG_ADSB_Sagetech_MXS::send_targetreq_msg() 
{
...
sg_emitter_t AG_ADSB_Sagetech_MXS::convert_emitter_type_to_sg(const uint8_t emitterType) const
{
...
uint8_t AG_ADSB_Sagetech_MXS::convert_sg_emitter_type_to_adsb(const sg_emitter_t sgEmitterType) const
{
...
sg_airspeed_t AG_ADSB_Sagetech_MXS::convert_airspeed_knots_to_sg(const float maxAirSpeed) const
{
