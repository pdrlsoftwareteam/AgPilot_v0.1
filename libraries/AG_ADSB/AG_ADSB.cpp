...
#if HAL_ADSB_ENABLED
#include "AG_ADSB_uAvionix_MAVLink.h"
#include "AG_ADSB_uAvionix_UCP.h"
#include "AG_ADSB_Sagetech.h"
#include "AG_ADSB_Sagetech_MXS.h"
...
AG_ADSB *AG_ADSB::_singleton;

// table of user settable parameters
const AG_Param::GroupInfo AG_ADSB::var_info[] = {
    // @Param: TYPE
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("TYPE",     0, AG_ADSB, _type[0],    0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("LIST_MAX",   2, AG_ADSB, in_state.list_size_param, ADSB_VEHICLE_LIST_SIZE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("LIST_RADIUS",   3, AG_ADSB, in_state.list_radius, ADSB_LIST_RADIUS_DEFAULT),
    ...
    // @Range: -1 16777215
    // @User: Advanced
    AP_GROUPINFO("ICAO_ID",   4, AG_ADSB, out_state.cfg.ICAO_id_param, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("EMIT_TYPE",   5, AG_ADSB, out_state.cfg.emitterType, ADSB_EMITTER_TYPE_UAV),
    ...
    // @User: Advanced
    AP_GROUPINFO("LEN_WIDTH",   6, AG_ADSB, out_state.cfg.lengthWidth, UAVIONIX_ADSB_OUT_CFG_AIRCRAFT_SIZE_L15M_W23M),
    ...
    // @User: Advanced
    AP_GROUPINFO("OFFSET_LAT",   7, AG_ADSB, out_state.cfg.gpsOffsetLat, UAVIONIX_ADSB_OUT_CFG_GPS_OFFSET_LAT_RIGHT_0M),
    ...
    // @User: Advanced
    AP_GROUPINFO("OFFSET_LON",   8, AG_ADSB, out_state.cfg.gpsOffsetLon, UAVIONIX_ADSB_OUT_CFG_GPS_OFFSET_LON_APPLIED_BY_SENSOR),
    ...
    // @Bitmask: 0:Rx,1:Tx
    // @User: Advanced
    AP_GROUPINFO("RF_SELECT",   9, AG_ADSB, out_state.cfg.rfSelect, UAVIONIX_ADSB_OUT_RF_SELECT_RX_ENABLED),
    ...
    // @Units: octal
    // @User: Advanced
    AP_GROUPINFO("SQUAWK",  10, AG_ADSB, out_state.cfg.squawk_octal_param, ADSB_SQUAWK_OCTAL_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("RF_CAPABLE",  11, AG_ADSB, out_state.cfg.rf_capable, 0),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("LIST_ALT",   12, AG_ADSB, in_state.list_altitude, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("ICAO_SPECL",  13, AG_ADSB, _special_ICAO_target, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("LOG",  14, AG_ADSB, _log, 1),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS",  15, AG_ADSB, _options, 0),
...
AG_ADSB::AG_ADSB()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_ADSB must be singleton");
    }
#endif
    _singleton = this;
...
void AG_ADSB::detect_instance(uint8_t instance)
{
    ...
    case Type::uAvionix_MAVLink:
        ...
        if (AG_ADSB_uAvionix_MAVLink::detect()) {
            _backend[instance] = new AG_ADSB_uAvionix_MAVLink(*this, instance);
        }
#endif
        break;
    ...
    case Type::uAvionix_UCP:
        ...
        if (AG_ADSB_uAvionix_UCP::detect()) {
            _backend[instance] = new AG_ADSB_uAvionix_UCP(*this, instance);
        }
#endif
        break;
    ...
    case Type::Sagetech:
        ...
        if (AG_ADSB_Sagetech::detect()) {
            _backend[instance] = new AG_ADSB_Sagetech(*this, instance);
        }
#endif
        break;
    ...
    case Type::Sagetech_MXS:
        ...
        if (AG_ADSB_Sagetech_MXS::detect()) {
            _backend[instance] = new AG_ADSB_Sagetech_MXS(*this, instance);
        }
#endif
        break;
    }
...
AG_ADSB::Type AG_ADSB::get_type(uint8_t instance) const
{
...
bool AG_ADSB::is_valid_callsign(uint16_t octal)
{
 ...
 */
void AG_ADSB::determine_furthest_aircraft(void)
{
 ...
 */
Location AG_ADSB::get_location(const adsb_vehicle_t &vehicle) const
{
 ...
 */
void AG_ADSB::delete_vehicle(const uint16_t index)
{
 ...
 */
bool AG_ADSB::find_index(const adsb_vehicle_t &vehicle, uint16_t *index) const
{
 ...
 */
void AG_ADSB::handle_adsb_vehicle(const adsb_vehicle_t &vehicle)
{
    if (!check_startup()) {
        return;
    }
    ...
    const Location vehicle_loc = AG_ADSB::get_location(vehicle);
 ...
 */
void AG_ADSB::set_vehicle(const uint16_t index, const adsb_vehicle_t &vehicle)
{
...
void AG_ADSB::send_adsb_vehicle(const mavlink_channel_t chan)
{
 ...
 */
void AG_ADSB::handle_out_cfg(const mavlink_uavionix_adsb_out_cfg_t &packet)
{
 ...
 */
void AG_ADSB::handle_out_control(const mavlink_uavionix_adsb_out_control_t &packet)
{
 ...
 */
void AG_ADSB::handle_transceiver_report(const mavlink_channel_t chan, const mavlink_uavionix_adsb_transceiver_health_report_t &packet)
{
    if (out_state.chan != chan) {
        gcs().send_text(MAV_SEVERITY_DEBUG, "ADSB: Found transceiver on channel %d", chan);
    }

    out_state.chan_last_ms = AG_HAL::millis();
    out_state.chan = chan;
 ...
 */
void AG_ADSB::send_adsb_out_status(const mavlink_channel_t chan) const
{
    for (uint8_t i=0; i < ADSB_MAX_INSTANCES; i++) {
        if (_type[i] == (int8_t)(AG_ADSB::Type::uAvionix_UCP) || _type[i] == (int8_t)(AG_ADSB::Type::Sagetech_MXS)) {
...
*/
uint32_t AG_ADSB::genICAO(const Location &loc) const
{
...
void AG_ADSB::set_callsign(const char* str, const bool append_icao)
{
...
void AG_ADSB::push_sample(const adsb_vehicle_t &vehicle)
{
    _samples.push(vehicle);
}

bool AG_ADSB::next_sample(adsb_vehicle_t &vehicle)
{
    return _samples.pop(vehicle);
}

void AG_ADSB::handle_message(const mavlink_channel_t chan, const mavlink_message_t &msg)
{
    switch (msg.msgid) {
        case MAVLINK_MSG_ID_ADSB_VEHICLE: {
            adsb_vehicle_t vehicle {};
            mavlink_msg_adsb_vehicle_decode(&msg, &vehicle.info);
            vehicle.last_update_ms = AG_HAL::millis() - uint32_t(vehicle.info.tslc * 1000U);
...
bool AG_ADSB::get_vehicle_by_ICAO(const uint32_t icao, adsb_vehicle_t &vehicle) const
{
 ...
 */
void AG_ADSB::write_log(const adsb_vehicle_t &vehicle) const
{
    switch (_log) {
...
*/
uint32_t AG_ADSB::convert_base_to_decimal(const uint8_t baseIn, uint32_t inputNumber)
{
