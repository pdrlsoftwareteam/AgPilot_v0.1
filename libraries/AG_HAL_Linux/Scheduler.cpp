...
void Scheduler::init_realtime()
{
    ...
    if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &param) == -1) {
        AG_HAL::panic("Scheduler: failed to set scheduling parameters: %s",
                      strerror(errno));
    }
}
...
void Scheduler::init_cpu_affinity()
{
    ...
    if (sched_setaffinity(0, sizeof(_cpu_affinity), &_cpu_affinity) != 0) {
        AG_HAL::panic("Failed to set affinity for main process: %m");
    }
}
...
void Scheduler::init()
{
    ...
    if (ret) {
        AG_HAL::panic("Scheduler: Failed to initialise barrier object: %s",
                      strerror(ret));
    }
...
void Scheduler::delay(uint16_t ms)
{
    ...
    uint64_t start = AG_HAL::millis64();

    while ((AG_HAL::millis64() - start) < ms) {
...
{
    if (_initialized) {
        AG_HAL::panic("PANIC: scheduler::set_system_initialized called more than once");
    }
