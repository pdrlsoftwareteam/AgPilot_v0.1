...
void OpticalFlow_Onboard::init()
{
    ...
    if (device_path == nullptr ||
        ...
        AG_HAL::panic("OpticalFlow_Onboard: couldn't open "
                      "video device");
    }
    ...
    if (!_camerasensor->set_format(HAL_OPTFLOW_ONBOARD_SENSOR_WIDTH,
        ...
        AG_HAL::panic("OpticalFlow_Onboard: couldn't set subdev fmt\n");
    }
    ...
    if (!_videoin->set_format(&_width, &_height, &_format, &_bytesperline,
                              &_sizeimage)) {
        AG_HAL::panic("OpticalFlow_Onboard: couldn't set video format");
    }

    if (_format != V4L2_PIX_FMT_NV12 && _format != V4L2_PIX_FMT_GREY &&
        _format != V4L2_PIX_FMT_YUYV) {
        AG_HAL::panic("OpticalFlow_Onboard: format not supported\n");
    }
    ...
    if (!_videoin->allocate_buffers(nbufs)) {
        AG_HAL::panic("OpticalFlow_Onboard: couldn't allocate video buffers");
    }
    ...
    if (ret != 0) {
        AG_HAL::panic("OpticalFlow_Onboard: failed to init mutex");
    }

    ret = pthread_attr_init(&attr);
    if (ret != 0) {
        AG_HAL::panic("OpticalFlow_Onboard: failed to init attr");
    }
    ...
    if (ret != 0) {
        AG_HAL::panic("OpticalFlow_Onboard: failed to create thread");
    }
...
bool OpticalFlow_Onboard::read(AG_HAL::OpticalFlow::Data_Frame& frame)
{
    bool ret;
...
void OpticalFlow_Onboard::_run_optflow()
{
    ...
    if (_format == V4L2_PIX_FMT_YUYV) {
        ...
        if (!convert_buffer) {
            AG_HAL::panic("OpticalFlow_Onboard: couldn't allocate conversion buffer\n");
        }
    }
    ...
    if (_shrink_by_software || _crop_by_software) {
        ...
        if (!output_buffer) {
            ...
            AG_HAL::panic("OpticalFlow_Onboard: couldn't allocate crop buffer\n");
        }
    }
    ...
    while(true) {
        ...
        if (!_videoin->get_frame(video_frame)) {
            ...
            AG_HAL::panic("OpticalFlow_Onboard: couldn't get frame\n");
        }
