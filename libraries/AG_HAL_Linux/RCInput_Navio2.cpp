...
void RCInput_Navio2::init()
{
    for (size_t i = 0; i < ARRAY_SIZE(channels); i++) {
        ...
        if (channels[i] < 0) {
            AG_HAL::panic("[RCInput_Navio2]: failed to open channels\n");
        }
    }
}
...
{
    if (AG_HAL::micros() - _last_timestamp < 10000) {
        return;
    }
...
int RCInput_Navio2::open_channel(int channel)
{
    ...
    if (asprintf(&channel_path, "%s/ch%d", RCIN_SYSFS_PATH, channel) == -1) {
        AG_HAL::panic("[RCInput_Navio2]: not enough memory\n");
    }
