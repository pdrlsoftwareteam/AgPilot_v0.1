...
bool Thread::start(const char *name, int policy, int prio)
{
    ...
    if (geteuid() == 0) {
        if ((r = pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED)) != 0 ||
            ...
            AG_HAL::panic("Failed to set attributes for thread '%s': %s",
                          name, strerror(r));
        }
    }
    ...
    if (r != 0) {
        AG_HAL::panic("Failed to create thread '%s': %s",
                      name, strerror(r));
    }
...
bool PeriodicThread::_run()
{
    ...
    uint64_t next_run_usec = AG_HAL::micros64() + _period_usec;

    while (!_should_exit) {
        uint64_t dt = next_run_usec - AG_HAL::micros64();
        if (dt > _period_usec) {
            // we've lost sync - restart
            next_run_usec = AG_HAL::micros64();
        } else {
