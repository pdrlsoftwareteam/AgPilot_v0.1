...
AG_HAL::Semaphore *I2CDevice::get_semaphore()
{
    return &_bus.sem;
}

AG_HAL::Device::PeriodicHandle I2CDevice::register_periodic_callback(
...
AG_HAL::OwnPtr<AG_HAL::I2CDevice>
I2CDeviceManager::get_device(std::vector<const char *> devpaths, uint8_t address)
{
    ...
    d = opendir(dirname);
    if (!d) {
        AG_HAL::panic("Could not get list of I2C buses");
    }
    ...
    for (de = readdir(d); de; de = readdir(d)) {
        ...
        if (t != std::end(devpaths)) {
            ...
            if (sscanf(de->d_name, "i2c-%u", &n) != 1) {
                AG_HAL::panic("I2CDevice: can't parse %s", de->d_name);
            }
            if (n > UINT8_MAX) {
                AG_HAL::panic("I2CDevice: bus with number n=%u higher than %u",
                              n, UINT8_MAX);
            }
                             ...
                             uint32_t timeout_ms)
{
    ...
    AG_HAL::OwnPtr<I2CBus> b{new I2CBus()};
    if (!b) {
        return nullptr;
    }
...
{
    auto dev = AG_HAL::OwnPtr<AG_HAL::I2CDevice>(new I2CDevice(b, address));
    if (!dev) {
        return nullptr;
    }
    b.ref++;
