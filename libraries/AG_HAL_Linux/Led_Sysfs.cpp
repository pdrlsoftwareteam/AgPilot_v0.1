...
bool Led_Sysfs::init()
{
    ...
    if (asprintf(&br_path, "/sys/class/leds/%s/brightness", _led_name) == -1) {
        AG_HAL::panic("LinuxLed_Sysfs : Couldn't allocate brightness path");
    }
    ...
    if (asprintf(&max_br_path, "/sys/class/leds/%s/max_brightness", _led_name) == -1) {
        AG_HAL::panic("LinuxLed_Sysfs : Couldn't allocate max_brightness path");
    }

    if (Util::from(hal.util)->read_file(max_br_path, "%u", &_max_brightness) < 0) {
        AG_HAL::panic("LinuxLed_Sysfs : Unable to read max_brightness in %s",
                      max_br_path);
    }
