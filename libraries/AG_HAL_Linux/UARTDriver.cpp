...
*/
AG_HAL::OwnPtr<SerialDevice> UARTDriver::_parseDevicePath(const char *arg)
{
    struct stat st;

    if (stat(arg, &st) == 0 && S_ISCHR(st.st_mode)) {
        return AG_HAL::OwnPtr<SerialDevice>(new UARTDevice(arg));
    ...
    AG_HAL::OwnPtr<SerialDevice> device = nullptr;
    ...
    if (strcmp(protocol, "udp") == 0 || strcmp(protocol, "udpin") == 0) {
        ...
        if (strcmp(protocol, "udp") == 0) {
            ...
            if (bcast) {
                AG_HAL::panic("Can't combine udpin with bcast");
            }
