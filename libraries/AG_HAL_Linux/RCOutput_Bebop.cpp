...
RCOutput_Bebop::RCOutput_Bebop(AG_HAL::OwnPtr<AG_HAL::I2CDevice> dev)
    : _dev(std::move(dev))
...
void RCOutput_Bebop::_run_rcout()
{
    ...
    if (!_get_info(&info)) {
        AG_HAL::panic("failed to get BLDC info");
    }
    ...
    if (hw_version == UTIL_HARDWARE_BEBOP) {
        ...
        AG_HAL::panic("failed to get hw version %s", strerror(hw_version));
    } else {
        AG_HAL::panic("unsupported hw version %d", hw_version);
    }
