...
class UARTDriver : public AG_HAL::UARTDriver {
public:
    UARTDriver(bool default_console);

    static UARTDriver *from(AG_HAL::UARTDriver *uart) {
...
private:
    AG_HAL::OwnPtr<SerialDevice> _device;
    ...
    AG_HAL::OwnPtr<SerialDevice> _parseDevicePath(const char *arg);
