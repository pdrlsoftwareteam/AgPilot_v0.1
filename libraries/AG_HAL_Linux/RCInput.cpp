...
#include <AG_HAL/utility/sumd.h>
#include <AG_HAL/utility/st24.h>
#include <AG_HAL/utility/srxl.h>
...
void RCInput::_process_dsm_pulse(uint16_t width_s0, uint16_t width_s1)
{
    ...
    if (bits_s0 - nbits > 10) {
        if (dsm_state.bit_ofs == 16*10) {
            ...
            uint16_t values[8];
            uint16_t num_values=0;
            if (dsm_decode(AG_HAL::micros64(), bytes, values, &num_values, 8) &&
                num_values >= MIN_NUM_CHANNELS) {
                for (i=0; i<num_values; i++) {
                    _pwm_values[i] = values[i];
                }
...
bool RCInput::add_dsm_input(const uint8_t *bytes, size_t nbytes)
{
	...
	if (dsm.partial_frame_count == dsm_frame_size) {
             ...
             */
            if (dsm_decode(AG_HAL::micros64(), dsm.frame, values, &num_values, 16) &&
                num_values >= MIN_NUM_CHANNELS &&
                nbytes == 0) {
