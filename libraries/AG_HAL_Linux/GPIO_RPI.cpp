...
bool GPIO_RPI::openMemoryDevice()
{
    ...
    if (_system_memory_device < 0) {
        AG_HAL::panic("Can't open %s", GPIO_RPI::_system_memory_device_path);
        return false;
    }
...
void GPIO_RPI::init()
{
    ...
    if(rpi_version == LINUX_BOARD_TYPE::RPI_ZERO_1) {
        ...
        AG_HAL::panic("Unknown rpi_version, cannot locate peripheral base address");
        return;
    }

    if (!openMemoryDevice()) {
        AG_HAL::panic("Failed to initialize memory device.");
        return;
    }
    ...
    if (!_gpio) {
        AG_HAL::panic("Failed to get GPIO memory map.");
    }
...
AG_HAL::DigitalSource* GPIO_RPI::channel(uint16_t n)
{
