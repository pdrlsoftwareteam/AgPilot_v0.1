...
void SPIBus::open(uint16_t subdev)
{
    ...
    if (fd[subdev] < 0) {
        AG_HAL::panic("SPI: unable to open SPI bus %s: %s",
                      path, strerror(errno));
    }
}
    ...
    , _desc(device_desc)
{
    ...
    if (_desc.cs_pin != SPI_CS_KERNEL) {
        ...
        if (!_cs) {
            AG_HAL::panic("Unable to instantiate cs pin");
        }
...
AG_HAL::Device::PeriodicHandle SPIDevice::register_periodic_callback(
    uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
    TimerPollable *p = _bus.thread.add_timer(cb, &_bus, period_usec);
    if (!p) {
        AG_HAL::panic("Could not create periodic callback");
    }
    ...
    return static_cast<AG_HAL::Device::PeriodicHandle>(p);
}

bool SPIDevice::adjust_periodic_callback(
    AG_HAL::Device::PeriodicHandle h, uint32_t period_usec)
{
...
SPIDeviceManager::get_device(const char *name)
{
    ...
    AG_HAL::OwnPtr<SPIBus> b{new SPIBus(desc->bus)};
    if (!b) {
        return nullptr;
    }
...
SPIDeviceManager::_create_device(SPIBus &b, SPIDesc &desc) const
{
    ...
    auto dev = AG_HAL::OwnPtr<AG_HAL::SPIDevice>(new SPIDevice(b, desc));
    if (!dev) {
        return nullptr;
    }
