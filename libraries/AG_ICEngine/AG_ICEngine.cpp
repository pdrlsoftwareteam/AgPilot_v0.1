...
const AG_Param::GroupInfo AG_ICEngine::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE", 0, AG_ICEngine, enable, 0, AP_PARAM_FLAG_ENABLE),
    ...
    AP_GROUPINFO("START_CHAN", 1, AG_ICEngine, start_chan, 0),
    ...
    // @Units: s
    // @Range: 0.1 5
    AP_GROUPINFO("STARTER_TIME", 2, AG_ICEngine, starter_time, 3),
    ...
    // @Units: s
    // @Range: 1 10
    AP_GROUPINFO("START_DELAY", 3, AG_ICEngine, starter_delay, 2),
    ...
    // @User: Standard
    // @Range: 100 100000
    AP_GROUPINFO("RPM_THRESH", 4, AG_ICEngine, rpm_threshold, 100),
    ...
    // @User: Standard
    // @Range: 1000 2000
    AP_GROUPINFO("PWM_IGN_ON", 5, AG_ICEngine, pwm_ignition_on, 2000),
    ...
    // @User: Standard
    // @Range: 1000 2000
    AP_GROUPINFO("PWM_IGN_OFF", 6, AG_ICEngine, pwm_ignition_off, 1000),
    ...
    // @User: Standard
    // @Range: 1000 2000
    AP_GROUPINFO("PWM_STRT_ON", 7, AG_ICEngine, pwm_starter_on, 2000),
    ...
    // @User: Standard
    // @Range: 1000 2000
    AP_GROUPINFO("PWM_STRT_OFF", 8, AG_ICEngine, pwm_starter_off, 1000),
    ...
    AP_GROUPINFO("RPM_CHAN",  9, AG_ICEngine, rpm_instance, 0),
    ...
    // @User: Standard
    // @Range: 0 100
    AP_GROUPINFO("START_PCT", 10, AG_ICEngine, start_percent, 5),
    ...
    // @User: Standard
    // @Range: 0 100
    AP_GROUPINFO("IDLE_PCT", 11, AG_ICEngine, idle_percent, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("IDLE_RPM", 12, AG_ICEngine, idle_rpm, -1),
    ...
    AP_GROUPINFO("IDLE_DB", 13, AG_ICEngine, idle_db, 50),
    ...
    AP_GROUPINFO("IDLE_SLEW", 14, AG_ICEngine, idle_slew, 1),
    ...
    AP_GROUPINFO("OPTIONS", 15, AG_ICEngine, options, 0),
    ...
    // @User: Standard
    // @Range: 0 1300
    AP_GROUPINFO("STARTCHN_MIN", 16, AG_ICEngine, start_chan_min_pwm, 0),
    ...
    // @Range: 0 2000000
    // @Units: RPM
    AP_GROUPINFO("REDLINE_RPM", 17, AG_ICEngine, redline_rpm, 0),
...
// constructor
AG_ICEngine::AG_ICEngine(const AG_RPM &_rpm) :
    rpm(_rpm)
{
    AG_Param::setup_object_defaults(this, var_info);

    if (_singleton != nullptr) {
        AG_HAL::panic("AG_ICEngine must be singleton");
    }
    _singleton = this;
...
void AG_ICEngine::update(void)
{
    ...
    if (option_set(Options::DISABLE_IGNITION_RC_FAILSAFE) && AG_Notify::flags.failsafe_radio) {
 ...
 */
bool AG_ICEngine::throttle_override(float &percentage, const float base_throttle)
{
    if (!enable) {
        return false;
    }
...
*/
bool AG_ICEngine::engine_control(float start_control, float cold_start, float height_delay)
{
    if (start_control <= 0) {
        state = ICE_OFF;
        return true;
...
*/
void AG_ICEngine::update_idle_governor(int8_t &min_throttle)
{
    if (!enable) {
        return;
    }
    ...
    if (get_state() != AG_ICEngine::ICE_RUNNING || idle_rpm < 0) {
        return;
    }
...
AG_ICEngine *AG_ICEngine::_singleton;
namespace AP {
AG_ICEngine *ice() {
        return AG_ICEngine::get_singleton();
    }
}
