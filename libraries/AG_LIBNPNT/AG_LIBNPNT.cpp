 ...
 *      Author: Owner
 */
#include "AG_LIBNPNT.h"
#include <AG_KEYSTORE/AG_KEYSTORE.h>
#include <GCS_MAVLink/GCS.h>
#include <AG_PDRL_Commander/AG_NPNT_data_helper.h>
#define  TODO 0

AG_LIBNPNT* AG_LIBNPNT::m_pInstance = NULL;

AG_LIBNPNT* AG_LIBNPNT::getInstance()
{
	if (!m_pInstance)   // Only allow one instance of class to be generated.
		m_pInstance = new AG_LIBNPNT;
	//	m_AG_NPNT_data_helper = AG_NPNT_data_helper::getInstance();
	return m_pInstance;
}

AG_LIBNPNT::AG_LIBNPNT()
{
...
char* AG_LIBNPNT::getStringBetweenTags(char* s,char* PATTERN1,char* PATTERN2)
{
	char *target = NULL;
	char *start, *end;
...
int16_t AG_LIBNPNT::extract_public_key_from_xml_artefact(char *buffer)
{
#if OPENSSL
...
int16_t AG_LIBNPNT::verifyValidGeofence()
{
#if TODO
	int listNotMatch = 0;
...
int16_t AG_LIBNPNT::load_artifact(char * buffer,uint16_t dataLen)
{
	...
	if (!npnt_handle.params.permissionArtifactID) {
		...
		memcpy((void*)npnt_handle.params.permissionArtifactID,(void*)AG_NPNT_data_helper::getInstance()->permissionArticaftID,37);
	}
...
int AG_LIBNPNT::authenticateSinglePA(char* paBuffer,uint16_t dataLen)
{
...
bool AG_LIBNPNT::getIsPAValid(bool silent)
{
...
void AG_LIBNPNT::setIsPAValid(LIB_NPNT_STATUS val)
{
	libNpntStatus = val;
}

void AG_LIBNPNT::showMessageTimebreach()
{
	gcs().send_text(MAV_SEVERITY_ERROR, "NPNT Time breach");
	setIsPAValid(STATUS_NPNT_INV_TIME);
}

void AG_LIBNPNT::showMessageFencebreach()
{
	gcs().send_text(MAV_SEVERITY_ERROR, "NPNT Fence breach");
}

void AG_LIBNPNT::sendPAvalidationResponse(mavlink_channel_t chan)
{
...
void AG_LIBNPNT::receiveHash(unsigned char *hashPtr,uint16_t validDataLen)
{
	memcpy(firmwareHash,hashPtr,validDataLen);
}

void AG_LIBNPNT::receiveSignature(unsigned char *signaturePtr,uint16_t validDataLen,uint8_t bufferIndex)
{
