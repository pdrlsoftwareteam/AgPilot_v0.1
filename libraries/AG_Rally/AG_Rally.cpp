...
// storage object
StorageAccess AG_Rally::_storage(StorageManager::StorageRally);
...
const AG_Param::GroupInfo AG_Rally::var_info[] = {
    // @Param: TOTAL
    // @DisplayName: Rally Total
    // @Description: Number of rally points currently loaded
    // @User: Advanced
    AP_GROUPINFO("TOTAL", 0, AG_Rally, _rally_point_total_count, 0),
    ...
    // @Units: km
    // @Increment: 0.1
    AP_GROUPINFO("LIMIT_KM", 1, AG_Rally, _rally_limit_km, RALLY_LIMIT_KM_DEFAULT),
    ...
    AP_GROUPINFO("INCL_HOME", 2, AG_Rally, _rally_incl_home, RALLY_INCLUDE_HOME_DEFAULT),
...
AG_Rally::AG_Rally()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("Rally must be singleton");
    }
#endif
    _singleton = this;
...
bool AG_Rally::get_rally_point_with_index(uint8_t i, RallyLocation &ret) const
{
...
void AG_Rally::truncate(uint8_t num)
{
...
bool AG_Rally::append(const RallyLocation &loc)
{
...
bool AG_Rally::set_rally_point_with_index(uint8_t i, const RallyLocation &rallyLoc)
{
    if (i >= (uint8_t) _rally_point_total_count) {
        return false;
    }
...
Location AG_Rally::rally_location_to_location(const RallyLocation &rally_loc) const
{
...
bool AG_Rally::find_nearest_rally_point(const Location &current_loc, RallyLocation &return_loc) const
{
    float min_dis = -1;
...
Location AG_Rally::calc_best_rally_or_home_location(const Location &current_loc, float rtl_home_alt_amsl_cm) const
{
...
// singleton instance
AG_Rally *AG_Rally::_singleton;

namespace AP {

AG_Rally *rally()
{
    return AG_Rally::get_singleton();
}
