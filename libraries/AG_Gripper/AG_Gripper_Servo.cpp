#include <AG_Gripper/AG_Gripper_Servo.h>
...
void AG_Gripper_Servo::release()
{
    ...
    config.state = AG_Gripper::STATE_RELEASING;
...
bool AG_Gripper_Servo::has_state_pwm(const uint16_t pwm) const
{
    ...
    if (AG_HAL::millis() - _last_grab_or_release < SERVO_ACTUATION_TIME) {
        // servo still moving....
        return false;
    }
    return true;
...
bool AG_Gripper_Servo::released() const
{
    return (config.state == AG_Gripper::STATE_RELEASED);
}

bool AG_Gripper_Servo::grabbed() const
{
    return (config.state == AG_Gripper::STATE_GRABBED);
}

// type-specific periodic updates:
void AG_Gripper_Servo::update_gripper()
{
    // Check for successful grabbed or released
    if (config.state == AG_Gripper::STATE_GRABBING && has_state_pwm(config.grab_pwm)) {
        config.state = AG_Gripper::STATE_GRABBED;
    } else if (config.state == AG_Gripper::STATE_RELEASING && has_state_pwm(config.release_pwm)) {
        config.state = AG_Gripper::STATE_RELEASED;
    }
}

bool AG_Gripper_Servo::valid() const
{
    if (!AG_Gripper_Backend::valid()) {
        return false;
    }
