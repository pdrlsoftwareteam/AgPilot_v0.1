 ...
 *              Peter Barker - moved into AG_Gripper_EPM
 */
...
AG_Gripper_EPM::AG_Gripper_EPM(struct AG_Gripper::Backend_Config &_config) :
    AG_Gripper_Backend(_config) { }

void AG_Gripper_EPM::init_gripper()
{
#ifdef UAVCAN_NODE_FILE
...
void AG_Gripper_EPM::update_gripper()
{
    // move EPM PWM output back to neutral after the last grab or release
    if (AG_HAL::millis() - _last_grab_or_release > EPM_RETURN_TO_NEUTRAL_MS) {
        if (config.state == AG_Gripper::STATE_GRABBING) {
            neutral();
            config.state = AG_Gripper::STATE_GRABBED;
        } else if (config.state == AG_Gripper::STATE_RELEASING) {
            neutral();
    ...
    if (config.state == AG_Gripper::STATE_GRABBED &&
        ...
        (AG_HAL::millis() - _last_grab_or_release > ((uint32_t)config.regrab_interval * 1000))) {
        grab();
    }
}

UAVCANCommand AG_Gripper_EPM::make_uavcan_command(uint16_t command) const
{
    UAVCANCommand cmd;
...
bool AG_Gripper_EPM::released() const
{
...
bool AG_Gripper_EPM::grabbed() const
{
