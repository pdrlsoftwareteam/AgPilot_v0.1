...
const AG_Param::GroupInfo AG_OpenDroneID::var_info[] = {

    // @Param: ENABLE
    // @DisplayName: Enable ODID subsystem
    // @Description: Enable ODID subsystem
    // @Values: 0:Disabled,1:Enabled
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_OpenDroneID, _enable, 0, AP_PARAM_FLAG_ENABLE),
    ...
    AP_GROUPINFO("MAVPORT", 2, AG_OpenDroneID, _mav_port, -1),
    ...
    AP_GROUPINFO("CANDRIVER", 3, AG_OpenDroneID, _can_driver, 0),
    ...
    AP_GROUPINFO("OPTIONS", 4, AG_OpenDroneID, _options, 0),
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("BARO_ACC", 5, AG_OpenDroneID, _baro_accuracy, -1.0),
...
// constructor
AG_OpenDroneID::AG_OpenDroneID()
{
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton != nullptr) {
        AG_HAL::panic("OpenDroneID must be singleton");
    }
#endif
    _singleton = this;
...
void AG_OpenDroneID::load_UAS_ID_from_persistent_memory()
{
...
void AG_OpenDroneID::set_basic_id() {
...
void AG_OpenDroneID::get_persistent_params(ExpandingString &str) const
{
...
bool AG_OpenDroneID::pre_arm_check(char* failmsg, uint8_t failmsg_len)
{
    WITH_SEMAPHORE(_sem);
...
void AG_OpenDroneID::send_dynamic_out()
{
...
void AG_OpenDroneID::send_static_out()
{
    const uint32_t now_ms = AG_HAL::millis();
...
void AG_OpenDroneID::send_location_message()
{
    auto &ahrs = AP::ahrs();
    const auto &barometer = AP::baro();
    const auto &gps = AP::gps();

    const AG_GPS::GPS_Status gps_status = gps.status();
    const bool got_bad_gps_fix = (gps_status < AG_GPS::GPS_Status::GPS_OK_FIX_3D);
    const bool armed = hal.util->get_soft_armed();
...
void AG_OpenDroneID::send_basic_id_message()
{
...
void AG_OpenDroneID::send_self_id_message()
{
...
void AG_OpenDroneID::send_system_update_message()
{
    need_send_system |= dronecan_send_all;
    // note that packet is filled in by the GCS
    if (_chan != MAV_CHAN_INVALID) {
...
void AG_OpenDroneID::send_operator_id_message()
{
...
*/
MAV_ODID_HOR_ACC AG_OpenDroneID::create_enum_horizontal_accuracy(float accuracy) const
{
...
* @return Enum value representing the accuracy
*/
MAV_ODID_VER_ACC AG_OpenDroneID::create_enum_vertical_accuracy(float accuracy) const
{
    // Out of bounds return UKNOWN flag
...
* @return Enum value representing the accuracy
*/
MAV_ODID_SPEED_ACC AG_OpenDroneID::create_enum_speed_accuracy(float accuracy) const
{
    // Out of bounds return UKNOWN flag
...
* @return Enum value representing the accuracy
*/
MAV_ODID_TIME_ACC AG_OpenDroneID::create_enum_timestamp_accuracy(float accuracy) const
{
    // Out of bounds return UKNOWN flag
...
uint16_t AG_OpenDroneID::create_speed_horizontal(uint16_t speed) const
{
...
// make sure value is within limits of remote ID standard
int16_t AG_OpenDroneID::create_speed_vertical(int16_t speed) const
{
...
// make sure value is within limits of remote ID standard
float AG_OpenDroneID::create_altitude(float altitude) const
{
...
// make sure value is within limits of remote ID standard
float AG_OpenDroneID::create_location_timestamp(float timestamp) const
{
...
void AG_OpenDroneID::handle_msg(mavlink_channel_t chan, const mavlink_message_t &msg)
{
    if (!_initialised) {
        return;
    }
    ...
    case MAVLINK_MSG_ID_OPEN_DRONE_ID_ARM_STATUS: {
        if (chan == _chan) {
            ...
            last_arm_status_ms = AG_HAL::millis();
        }
        break;
    }
    ...
    case MAVLINK_MSG_ID_OPEN_DRONE_ID_SYSTEM_UPDATE: {
        ...
        last_system_update_ms = AG_HAL::millis();
        if (last_system_ms != 0) {
...
// singleton instance
AG_OpenDroneID *AG_OpenDroneID::_singleton;
...
{
    return *AG_OpenDroneID::get_singleton();
}
