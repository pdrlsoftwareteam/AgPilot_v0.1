...
class AG_RCProtocol_CRSF : public AG_RCProtocol_Backend {
public:
    AG_RCProtocol_CRSF(AG_RCProtocol &_frontend);
    virtual ~AG_RCProtocol_CRSF();
    ...
    bool is_rx_active() const override {
        ...
        return AG_HAL::micros() < _last_rx_frame_time_us + CRSF_RX_TIMEOUT;
    }
    ...
    bool is_tx_active() const {
        ...
        return AG_HAL::micros() < _last_tx_frame_time_us + CRSF_TX_TIMEOUT;
    }

    // get singleton instance
    static AG_RCProtocol_CRSF* get_singleton() {
        return _singleton;
    }
...
#error "Only supported on little-endian architectures"
    ...
    // this will be used by AG_CRSF_Telem to access link status data
    // from within AG_RCProtocol_CRSF thread so no need for cross-thread synch
...
private:
    ...
    static AG_RCProtocol_CRSF* _singleton;
    ...
    void start_uart();
    AG_HAL::UARTDriver* get_current_UART() { return (_uart ? _uart : get_available_UART()); }
