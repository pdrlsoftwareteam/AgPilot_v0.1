...
// constructor
AG_RCProtocol_FPort2::AG_RCProtocol_FPort2(AG_RCProtocol &_frontend, bool _inverted) :
    AG_RCProtocol_Backend(_frontend),
    inverted(_inverted)
{}

// decode a full FPort2 control frame
void AG_RCProtocol_FPort2::decode_control(const FPort2_Frame &frame)
{
...
*/
void AG_RCProtocol_FPort2::decode_downlink(const FPort2_Frame &frame)
{
    ...
    AG_HAL::UARTDriver *uart = get_UART();
    if (!uart) {
        return;
    }
    ...
    if (frame.downlink.prim == FPORT2_PRIM_READ || frame.downlink.prim == FPORT2_PRIM_WRITE) {
        AG_Frsky_Telem::set_telem_data(frame.downlink.prim, frame.downlink.appid, le32toh_ptr(frame.downlink.data));
    }
    ...
    if (frame.type != FRAME_TYPE_FC) {
        ...
        */
        if (!telem_data.available) {
            ...
            if (!AG_Frsky_Telem::get_telem_data(&telem_data.packet, packet_count, 1)) {
 ...
 */
void AG_RCProtocol_FPort2::process_pulse(uint32_t width_s0, uint32_t width_s1)
{
    if (have_UART()) {
...
// support byte input
void AG_RCProtocol_FPort2::_process_byte(uint32_t timestamp_us, uint8_t b)
{
    ...
    if (byte_input.control_len > 2 && byte_input.ofs == byte_input.control_len) {
        if (!byte_input.is_downlink) {
            log_data(AG_RCProtocol::FPORT2, timestamp_us, byte_input.buf, byte_input.ofs);
            if (check_checksum()) {
...
// check checksum byte
bool AG_RCProtocol_FPort2::check_checksum(void)
{
    return crc_sum8(&byte_input.buf[1], byte_input.control_len-1) == 0;
}

// support byte input
void AG_RCProtocol_FPort2::process_byte(uint8_t b, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
    _process_byte(AG_HAL::micros(), b);
}
