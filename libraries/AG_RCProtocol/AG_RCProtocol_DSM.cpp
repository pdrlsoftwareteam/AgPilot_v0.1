...
#include <AG_VideoTX/AG_VideoTX_config.h>
...
void AG_RCProtocol_DSM::process_pulse(uint32_t width_s0, uint32_t width_s1)
{
    uint8_t b;
 ...
 */
bool AG_RCProtocol_DSM::dsm_decode_channel(uint16_t raw, unsigned shift, unsigned *channel, unsigned *value)
{
 ...
 */
void AG_RCProtocol_DSM::dsm_guess_format(bool reset, const uint8_t dsm_frame[16], unsigned frame_channels)
{
 ...
 *
 */
bool AG_RCProtocol_DSM::dsm_decode(uint32_t frame_time_ms, const uint8_t dsm_frame[16],
                                   uint16_t *values, uint16_t *num_values, uint16_t max_values)
{
    ...
    if (haveVtxControl) {
        AG_RCProtocol_SRXL2::configure_vtx(
 ...
 */
void AG_RCProtocol_DSM::start_bind(void)
{
 ...
 */
void AG_RCProtocol_DSM::update(void)
{
 ...
 */
bool AG_RCProtocol_DSM::dsm_parse_byte(uint32_t frame_time_ms, uint8_t b, uint16_t *values,
                                       uint16_t *num_values, uint16_t max_channels)
{
    ...
    case DSM_DECODE_STATE_SYNC: {
         ...
         * decode it.
         */
        log_data(AG_RCProtocol::DSM, frame_time_ms * 1000, byte_input.buf, byte_input.ofs);
...
// support byte input
void AG_RCProtocol_DSM::_process_byte(uint32_t timestamp_ms, uint8_t b)
{
...
// support byte input
void AG_RCProtocol_DSM::process_byte(uint8_t b, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
    _process_byte(AG_HAL::millis(), b);
}
