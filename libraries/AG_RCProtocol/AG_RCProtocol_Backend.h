...
class AG_RCProtocol_Backend {
    friend class AP_RCProtcol;

public:
    AG_RCProtocol_Backend(AG_RCProtocol &_frontend);
    virtual ~AG_RCProtocol_Backend() {}
    ...
    AG_HAL::UARTDriver *get_UART(void) const {
    ...
    AG_HAL::UARTDriver *get_available_UART(void) const {
...
#error "Only supported on little-endian architectures"
    ...
    AG_RCProtocol &frontend;

    void log_data(AG_RCProtocol::rcprotocol_t prot, uint32_t timestamp, const uint8_t *data, uint8_t len) const;
