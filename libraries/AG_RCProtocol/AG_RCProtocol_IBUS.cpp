...
// constructor
AG_RCProtocol_IBUS::AG_RCProtocol_IBUS(AG_RCProtocol &_frontend) :
    AG_RCProtocol_Backend(_frontend)
{}

// decode a full IBUS frame
bool AG_RCProtocol_IBUS::ibus_decode(const uint8_t frame[IBUS_FRAME_SIZE], uint16_t *values, bool *ibus_failsafe)
{
    uint32_t chksum = 96;
 ...
 */
void AG_RCProtocol_IBUS::process_pulse(uint32_t w0, uint32_t w1)
{
    uint8_t b;
...
// support byte input
void AG_RCProtocol_IBUS::_process_byte(uint32_t timestamp_us, uint8_t b)
{
    ...
    if (byte_input.ofs == sizeof(byte_input.buf)) {
        ...
        log_data(AG_RCProtocol::IBUS, timestamp_us, byte_input.buf, byte_input.ofs);
...
// support byte input
void AG_RCProtocol_IBUS::process_byte(uint8_t b, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
