...
AG_RCProtocol_Backend::AG_RCProtocol_Backend(AG_RCProtocol &_frontend) :
    frontend(_frontend),
    rc_input_count(0),
...
bool AG_RCProtocol_Backend::new_input()
{
...
uint8_t AG_RCProtocol_Backend::num_channels() const
{
    return _num_channels;
}

uint16_t AG_RCProtocol_Backend::read(uint8_t chan)
{
    return _pwm_values[chan];
}

void AG_RCProtocol_Backend::read(uint16_t *pwm, uint8_t n)
{
 ...
 */
void AG_RCProtocol_Backend::add_input(uint8_t num_values, uint16_t *values, bool in_failsafe, int16_t _rssi, int16_t _rx_link_quality)
{
    ...
    // failsafed is sorted out in AG_IOMCU.cpp
    in_failsafe = false;
#else
...
*/
void AG_RCProtocol_Backend::decode_11bit_channels(const uint8_t* data, uint8_t nchannels, uint16_t *values, uint16_t mult, uint16_t div, uint16_t offset)
{
 ...
 */
void AG_RCProtocol_Backend::log_data(AG_RCProtocol::rcprotocol_t prot, uint32_t timestamp, const uint8_t *data, uint8_t len) const
{
