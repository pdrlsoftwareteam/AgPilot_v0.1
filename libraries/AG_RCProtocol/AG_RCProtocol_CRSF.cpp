...
{
    switch(byte) {
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_GPS:
        return "GPS";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_BATTERY_SENSOR:
        return "BATTERY";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_HEARTBEAT:
        return "HEARTBEAT";
    ...
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND:
        return "COMMAND";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_ATTITUDE:
        return "ATTITUDE";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_FLIGHT_MODE:
        return "FLIGHT_MODE";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO:
        return "DEVICE_INFO";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_READ:
        return "PARAM_READ";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY:
        return "SETTINGS_ENTRY";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_LINK_STATISTICS:
        return "LINK_STATS";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_RC_CHANNELS_PACKED:
        return "RC";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_SUBSET_RC_CHANNELS_PACKED:
        return "RCv3";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_RC_CHANNELS_PACKED_11BIT:
        return "RCv3_11BIT";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_LINK_STATISTICS_RX:
        return "LINK_STATSv3_RX";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_LINK_STATISTICS_TX:
        return "LINK_STATSv3_TX";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_WRITE:
        return "UNKNOWN";
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_AP_CUSTOM_TELEM_LEGACY:
    case AG_RCProtocol_CRSF::CRSF_FRAMETYPE_AP_CUSTOM_TELEM:
        switch (subtype) {
        case AG_RCProtocol_CRSF::CRSF_AP_CUSTOM_TELEM_SINGLE_PACKET_PASSTHROUGH:
            return "AP_CUSTOM_SINGLE";
        case AG_RCProtocol_CRSF::CRSF_AP_CUSTOM_TELEM_STATUS_TEXT:
            return "AP_CUSTOM_TEXT";
        case AG_RCProtocol_CRSF::CRSF_AP_CUSTOM_TELEM_MULTI_PACKET_PASSTHROUGH:
...
const uint16_t AG_RCProtocol_CRSF::RF_MODE_RATES[RFMode::RF_MODE_MAX_MODES] = {
    4, 50, 150, 250,    // CRSF
    4, 25, 50, 100, 150, 200, 250, 500  // ELRS
};

AG_RCProtocol_CRSF* AG_RCProtocol_CRSF::_singleton;

AG_RCProtocol_CRSF::AG_RCProtocol_CRSF(AG_RCProtocol &_frontend) : AG_RCProtocol_Backend(_frontend)
{
#if !APM_BUILD_TYPE(APM_BUILD_UNKNOWN)
    if (_singleton != nullptr) {
        AG_HAL::panic("Duplicate CRSF handler");
    }
    ...
    _uart = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_CRSF, 0);
    if (_uart) {
        start_uart();
    }
#endif
...
AG_RCProtocol_CRSF::~AG_RCProtocol_CRSF() {
    _singleton = nullptr;
}

// get the protocol string
const char* AG_RCProtocol_CRSF::get_protocol_string(ProtocolType protocol) const {
...
uint16_t AG_RCProtocol_CRSF::get_link_rate(ProtocolType protocol) const {
    if (protocol == ProtocolType::PROTOCOL_ELRS) {
...
void AG_RCProtocol_CRSF::_process_byte(uint32_t timestamp_us, uint8_t byte)
{
    ...
    if (_frame_ofs == _frame.length + CSRF_HEADER_LEN) {
        log_data(AG_RCProtocol::CRSF, timestamp_us, (const uint8_t*)&_frame, _frame_ofs - CSRF_HEADER_LEN);
...
void AG_RCProtocol_CRSF::update(void)
{
    ...
    if (_uart) {
        ...
        for (uint8_t i = 0; i < n; i++) {
            ...
            if (b >= 0) {
                process_byte(AG_HAL::micros(), uint8_t(b));
            }
        }
    }
...
void AG_RCProtocol_CRSF::write_frame(Frame* frame)
{
    AG_HAL::UARTDriver *uart = get_current_UART();
...
bool AG_RCProtocol_CRSF::decode_crsf_packet()
{
#ifdef CRSF_DEBUG
    ...
    if (AG_CRSF_Telem::process_frame(FrameType(_frame.type), (uint8_t*)&_frame.payload)) {
        process_telemetry();
    }
...
*/
void AG_RCProtocol_CRSF::decode_variable_bit_channels(const uint8_t* payload, uint8_t frame_length, uint8_t nchannels, uint16_t *values)
{
...
// send out telemetry
bool AG_RCProtocol_CRSF::process_telemetry(bool check_constraint)
{
    ...
    if (!telem_available) {
#if HAL_CRSF_TELEM_ENABLED && !APM_BUILD_TYPE(APM_BUILD_iofirmware)
        if (AG_CRSF_Telem::get_telem_data(&_telemetry_frame, is_tx_active())) {
...
void AG_RCProtocol_CRSF::process_link_stats_frame(const void* data)
{
...
void AG_RCProtocol_CRSF::process_link_stats_rx_frame(const void* data)
{
...
void AG_RCProtocol_CRSF::process_link_stats_tx_frame(const void* data)
{
...
void AG_RCProtocol_CRSF::process_byte(uint8_t byte, uint32_t baudrate)
{
    ...
    _process_byte(AG_HAL::micros(), byte);
}

// start the uart if we have one
void AG_RCProtocol_CRSF::start_uart()
{
    _uart->configure_parity(0);
    _uart->set_stop_bits(1);
    _uart->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
    _uart->set_blocking_writes(false);
    _uart->set_options(_uart->get_options() & ~AG_HAL::UARTDriver::OPTION_RXINV);
    _uart->begin(get_bootstrap_baud_rate());
}

// change the baudrate of the protocol if we are able
bool AG_RCProtocol_CRSF::change_baud_rate(uint32_t baudrate)
{
    AG_HAL::UARTDriver* uart = get_available_UART();
    if (uart == nullptr) {
        return false;
    }
#if !defined(STM32H7)
...
void AG_RCProtocol_CRSF::process_handshake(uint32_t baudrate)
{
    AG_HAL::UARTDriver *uart = get_current_UART();
    ...
    if (uart == nullptr
        ...
        || (get_rc_protocols_mask() & ((1U<<(uint8_t(AG_RCProtocol::CRSF)+1))+1)) == 0) {
        return;
    }
...
int16_t AG_RCProtocol_CRSF::derive_scaled_lq_value(uint8_t uplink_lq)
{
    return int16_t(roundf(constrain_float(uplink_lq*2.5f,0,255)));
}

namespace AP {
    AG_RCProtocol_CRSF* crsf() {
        return AG_RCProtocol_CRSF::get_singleton();
    }
};
