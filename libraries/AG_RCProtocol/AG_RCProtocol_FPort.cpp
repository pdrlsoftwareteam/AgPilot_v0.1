...
// constructor
AG_RCProtocol_FPort::AG_RCProtocol_FPort(AG_RCProtocol &_frontend, bool _inverted) :
    AG_RCProtocol_Backend(_frontend),
    inverted(_inverted)
{}

// decode a full FPort control frame
void AG_RCProtocol_FPort::decode_control(const FPort_Frame &frame)
{
...
*/
void AG_RCProtocol_FPort::decode_downlink(const FPort_Frame &frame)
{
 ...
 */
void AG_RCProtocol_FPort::process_pulse(uint32_t width_s0, uint32_t width_s1)
{
    if (have_UART()) {
...
// support byte input
void AG_RCProtocol_FPort::_process_byte(uint32_t timestamp_us, uint8_t b)
{
    ...
    if (frame->type == FPORT_TYPE_CONTROL && byte_input.ofs == FRAME_LEN_CONTROL + 4) {
        log_data(AG_RCProtocol::FPORT, timestamp_us, byte_input.buf, byte_input.ofs);
        if (check_checksum()) {
...
// check checksum byte
bool AG_RCProtocol_FPort::check_checksum(void)
{
...
// support byte input
void AG_RCProtocol_FPort::process_byte(uint8_t b, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
