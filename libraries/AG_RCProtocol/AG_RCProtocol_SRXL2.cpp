...
AG_RCProtocol_SRXL2* AG_RCProtocol_SRXL2::_singleton;

AG_RCProtocol_SRXL2::AG_RCProtocol_SRXL2(AG_RCProtocol &_frontend) : AG_RCProtocol_Backend(_frontend)
{
#if !APM_BUILD_TYPE(APM_BUILD_UNKNOWN)
    if (_singleton != nullptr) {
        AG_HAL::panic("Duplicate SRXL2 handler\n");
    }
...
void AG_RCProtocol_SRXL2::_bootstrap(uint8_t device_id)
{
    ...
    if (!srxlInitDevice(device_id, SRXL_DEVICE_PRIORITY, SRXL_DEVICE_INFO, device_id)) {
        AG_HAL::panic("Failed to initialize SRXL2 device\n");
    }
    ...
    if (!srxlInitBus(0, 0, SRXL_SUPPORTED_BAUD_RATES)) {
        AG_HAL::panic("Failed to initialize SRXL2 bus\n");
    }
...
AG_RCProtocol_SRXL2::~AG_RCProtocol_SRXL2() {
    _singleton = nullptr;
}

void AG_RCProtocol_SRXL2::_process_byte(uint32_t timestamp_us, uint8_t byte)
{
    ...
    case STATE_COLLECT: // receive all bytes. After reception decode frame and provide rc channel information to FMU
        ...
        if (_buflen == _frame_len_full) {
            log_data(AG_RCProtocol::SRXL2, timestamp_us, _buffer, _buflen);
...
void AG_RCProtocol_SRXL2::update(void)
{
...
void AG_RCProtocol_SRXL2::capture_scaled_input(const uint8_t *values_p, bool in_failsafe, int16_t new_rssi)
{
    AG_RCProtocol_SRXL2* srxl2 = AG_RCProtocol_SRXL2::get_singleton();
...
void AG_RCProtocol_SRXL2::_capture_scaled_input(const uint8_t *values_p, bool in_failsafe, int16_t new_rssi)
{
...
void AG_RCProtocol_SRXL2::start_bind(void)
{
    srxlEnterBind(DSMX_11MS, true);
}

// process a byte provided by a uart
void AG_RCProtocol_SRXL2::process_byte(uint8_t byte, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
...
// handshake
void AG_RCProtocol_SRXL2::process_handshake(uint32_t baudrate)
{
    // only bootstrap if only SRXL2 is enabled
    if (baudrate != 115200 || (get_rc_protocols_mask() & ~(1U<<(uint8_t(AG_RCProtocol::SRXL2)+1)))) {
...
void AG_RCProtocol_SRXL2::send_on_uart(uint8_t* pBuffer, uint8_t length)
{
    AG_RCProtocol_SRXL2* srxl2 = AG_RCProtocol_SRXL2::get_singleton();
...
void AG_RCProtocol_SRXL2::configure_vtx(uint8_t band, uint8_t channel, uint8_t power, uint8_t pitmode)
{
    ...
    case VTX_BAND_FATSHARK:
        vtx.set_configured_band(AG_VideoTX::VideoBand::FATSHARK);
        break;
    case VTX_BAND_RACEBAND:
        vtx.set_configured_band(AG_VideoTX::VideoBand::RACEBAND);
        break;
    case VTX_BAND_E_BAND:
        vtx.set_configured_band(AG_VideoTX::VideoBand::BAND_E);
        break;
    case VTX_BAND_B_BAND:
        vtx.set_configured_band(AG_VideoTX::VideoBand::BAND_B);
        break;
    case VTX_BAND_A_BAND:
        vtx.set_configured_band(AG_VideoTX::VideoBand::BAND_A);
        break;
    default:
        break;
    }
    ...
    if (pitmode) {
        vtx.set_configured_options(vtx.get_options() | uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
    } else {
        vtx.set_configured_options(vtx.get_options() & ~uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
    }
...
// send data to the uart
void AG_RCProtocol_SRXL2::_send_on_uart(uint8_t* pBuffer, uint8_t length)
{
...
void AG_RCProtocol_SRXL2::change_baud_rate(uint32_t baudrate)
{
    AG_RCProtocol_SRXL2* srxl2 = AG_RCProtocol_SRXL2::get_singleton();
...
// change the uart baud rate
void AG_RCProtocol_SRXL2::_change_baud_rate(uint32_t baudrate)
{
    AG_HAL::UARTDriver* uart = get_available_UART();
    if (uart != nullptr) {
        uart->begin(baudrate);
...
{
    AG_RCProtocol_SRXL2::change_baud_rate(baudRate);
...
{
    AG_RCProtocol_SRXL2::send_on_uart(pBuffer, length);
}
...
void srxlFillTelemetry(SrxlTelemetryData* pTelemetryData)
{
    ...
    AG_Spektrum_Telem::get_telem_data(pTelemetryData->raw);
#endif
}
...
{
    if (isFailsafe) {
        AG_RCProtocol_SRXL2::capture_scaled_input((const uint8_t *)pChannelData->values, true, pChannelData->rssi);
    } else {
        AG_RCProtocol_SRXL2::capture_scaled_input((const uint8_t *)srxlChData.values, false, srxlChData.rssi);
    }
}
...
{
#if AP_VIDEOTX_ENABLED
    AG_RCProtocol_SRXL2::configure_vtx(pVtxData->band, pVtxData->channel, pVtxData->power, pVtxData->pit);
#endif
}
