...
void AG_RCProtocol_SRXL::process_pulse(uint32_t width_s0, uint32_t width_s1)
{
    uint8_t b;
 ...
 * @retval 0 success
 */
int AG_RCProtocol_SRXL::srxl_channels_get_v1v2(uint16_t max_values, uint8_t *num_values, uint16_t *values, bool *failsafe_state)
{
    uint8_t loop;
 ...
 * @retval 0 success
 */
int AG_RCProtocol_SRXL::srxl_channels_get_v5(uint16_t max_values, uint8_t *num_values, uint16_t *values, bool *failsafe_state)
{
...
void AG_RCProtocol_SRXL::_process_byte(uint32_t timestamp_us, uint8_t byte)
{
    ...
    case STATE_COLLECT: /* receive all bytes. After reception decode frame and provide rc channel information to FMU   */
        ...
        if (buflen == frame_len_full) {
            log_data(AG_RCProtocol::SRXL, timestamp_us, buffer, buflen);
            /* CRC check here */
 ...
 */
void AG_RCProtocol_SRXL::process_byte(uint8_t byte, uint32_t baudrate)
{
    if (baudrate != 115200) {
        return;
    }
