...
class AG_RCProtocol_SRXL2 : public AG_RCProtocol_Backend {
public:
    AG_RCProtocol_SRXL2(AG_RCProtocol &_frontend);
    virtual ~AG_RCProtocol_SRXL2();
    void process_byte(uint8_t byte, uint32_t baudrate) override;
    void process_handshake(uint32_t baudrate) override;
    void start_bind(void) override;
    void update(void) override;
    // get singleton instance
    static AG_RCProtocol_SRXL2* get_singleton() {
        return _singleton;
    }
...
private:
    ...
    static AG_RCProtocol_SRXL2* _singleton;
