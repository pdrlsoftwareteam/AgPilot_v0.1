...
#endif
const char* _storage_bkp_directory = "/PDRL/FIRM_BKP";
...
int Storage::save_mainFw_direct(int offset_kb)
{
	...
	if ((offset_kb / 8) == 0) {
	    ...
	    logFileFd = AP::FS().open(fname, O_WRONLY | O_APPEND);
	...
	if (logFileFd >= 0) {
		...
		for (uint8_t* p = flashStartAddr; p < flashEndAddr; p += BLOCK_SIZE) {
...
int Storage::save_FramBkp()
{
	...
	ret = AP::FS().mkdir(_storage_bkp_directory);
	if (ret == -1 && errno != EEXIST) {
		return -1;
	}
	...
	char* fname = nullptr;
	unsigned curr_bak = 0;
	ret = asprintf(&fname, "%s/last_storage_bak", _storage_bkp_directory);
	if (fname == nullptr && (ret <= 0)) {
		return -1;
	}
	int fd = AP::FS().open(fname, O_RDONLY);
