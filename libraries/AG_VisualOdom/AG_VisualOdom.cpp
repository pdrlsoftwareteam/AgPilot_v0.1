...
#include "AG_VisualOdom.h"

#if HAL_VISUALODOM_ENABLED

#include "AG_VisualOdom_Backend.h"
#include "AG_VisualOdom_MAV.h"
#include "AG_VisualOdom_IntelT265.h"
...
const AG_Param::GroupInfo AG_VisualOdom::var_info[] = {
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("_TYPE", 0, AG_VisualOdom, _type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("_POS", 1, AG_VisualOdom, _pos_offset, 0.0f),
    ...
    // @User: Advanced
    AP_GROUPINFO("_ORIENT", 2, AG_VisualOdom, _orientation, ROTATION_NONE),
    ...
    // @User: Advanced
    AP_GROUPINFO("_SCALE", 3, AG_VisualOdom, _pos_scale, 1.0f),
    ...
    // @Range: 0 250
    // @User: Advanced
    AP_GROUPINFO("_DELAY_MS", 4, AG_VisualOdom, _delay_ms, 10),
    ...
    // @Range: 0.05 5.0
    // @User: Advanced
    AP_GROUPINFO("_VEL_M_NSE", 5, AG_VisualOdom, _vel_noise, 0.1),
    ...
    // @Range: 0.1 10.0
    // @User: Advanced
    AP_GROUPINFO("_POS_M_NSE", 6, AG_VisualOdom, _pos_noise, 0.2f),
    ...
    // @Range: 0.05 1.0
    // @User: Advanced
    AP_GROUPINFO("_YAW_M_NSE", 7, AG_VisualOdom, _yaw_noise, 0.2f),

    AP_GROUPEND
};

AG_VisualOdom::AG_VisualOdom()
{
    AG_Param::setup_object_defaults(this, var_info);
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton != nullptr) {
        AG_HAL::panic("must be singleton");
    }
#endif
    _singleton = this;
}
...
void AG_VisualOdom::init()
{
    ...
    case VisualOdom_Type::MAV:
        _driver = new AG_VisualOdom_MAV(*this);
        break;
    case VisualOdom_Type::IntelT265:
    case VisualOdom_Type::VOXL:
        _driver = new AG_VisualOdom_IntelT265(*this);
        break;
    }
}

// return true if sensor is enabled
bool AG_VisualOdom::enabled() const
{
    return ((_type != VisualOdom_Type::None));
}

// return true if sensor is basically healthy (we are receiving data)
bool AG_VisualOdom::healthy() const
{
    if (!enabled()) {
        return false;
    }
...
void AG_VisualOdom::handle_vision_position_delta_msg(const mavlink_message_t &msg)
{
...
void AG_VisualOdom::handle_vision_position_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, float roll, float pitch, float yaw, float posErr, float angErr, uint8_t reset_counter)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
// general purpose method to consume position estimate data and send to EKF
void AG_VisualOdom::handle_vision_position_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, const Quaternion &attitude, float posErr, float angErr, uint8_t reset_counter)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
void AG_VisualOdom::handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, uint8_t reset_counter)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
void AG_VisualOdom::request_align_yaw_to_ahrs()
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
void AG_VisualOdom::align_position_to_ahrs(bool align_xy, bool align_z)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
bool AG_VisualOdom::pre_arm_check(char *failure_msg, uint8_t failure_msg_len) const
{
    // exit immediately if not enabled
    if (!enabled()) {
        return true;
...
// singleton instance
AG_VisualOdom *AG_VisualOdom::_singleton;

namespace AP {

AG_VisualOdom *visualodom()
{
    return AG_VisualOdom::get_singleton();
}
