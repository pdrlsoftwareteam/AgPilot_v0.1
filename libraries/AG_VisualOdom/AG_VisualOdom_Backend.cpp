...
*/
AG_VisualOdom_Backend::AG_VisualOdom_Backend(AG_VisualOdom &frontend) :
    _frontend(frontend)
{
}

// return true if sensor is basically healthy (we are receiving data)
bool AG_VisualOdom_Backend::healthy() const
{
    // healthy if we have received sensor messages within the past 300ms
    return ((AG_HAL::millis() - _last_update_ms) < AP_VISUALODOM_TIMEOUT_MS);
}

// consume vision_position_delta mavlink messages
void AG_VisualOdom_Backend::handle_vision_position_delta_msg(const mavlink_message_t &msg)
{
    // decode message
...
uint32_t AG_VisualOdom_Backend::get_reset_timestamp_ms(uint8_t reset_counter)
{
    ...
    if (reset_counter != _last_reset_counter) {
        ...
        _reset_timestamp_ms = AG_HAL::millis();
    }
