...
const AG_Param::GroupInfo AG_RobotisServo::var_info[] = {
    ...
    // @Range: 0 4095
    // @User: Standard
    AP_GROUPINFO("POSMIN",  1, AG_RobotisServo, pos_min, 0),
    ...
    // @Range: 0 4095
    // @User: Standard
    AP_GROUPINFO("POSMAX",  2, AG_RobotisServo, pos_max, 4095),
    
    AP_GROUPEND
};

// constructor
AG_RobotisServo::AG_RobotisServo(void)
{
    // set defaults from the parameter table
    AG_Param::setup_object_defaults(this, var_info);
}

void AG_RobotisServo::init(void)
{
    AG_SerialManager &serial_manager = AP::serialmanager();
    port = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Robotis,0);
    if (port) {
        baudrate = serial_manager.find_baudrate(AG_SerialManager::SerialProtocol_Robotis, 0);
...
*/
void AG_RobotisServo::add_stuffing(uint8_t *packet)
{
 ...
 */
void AG_RobotisServo::send_packet(uint8_t *txpacket)
{
 ...
 */
void AG_RobotisServo::detect_servos(void)
{
    uint8_t txpacket[10] {};
 ...
 */
void AG_RobotisServo::configure_servos(void)
{
 ...
 */
void AG_RobotisServo::send_command(uint8_t id, uint16_t reg, uint32_t value, uint8_t len)
{
    uint8_t txpacket[16] {};
...
/*
  read response bytes
 */
void AG_RobotisServo::read_bytes(void)
{
 ...
 */
void AG_RobotisServo::process_packet(const uint8_t *pkt, uint8_t length)
{
