...
void NavEKF2_core::controlMagYawReset()
{
    ...
    if (magYawResetRequest || magStateResetRequest || extNavYawResetRequest) {
        ...
        if (extNavYawResetRequest) {
            ...
            if (!yawAlignComplete) {
                GCS_SEND_TEXT(AGPILOT_SEVERITY_INFO, "EKF2 IMU%u ext nav yaw alignment complete",(unsigned)imu_index);
            }
            ...
            if (magYawResetRequest) {
                ...
                // send initial alignment status to console
                if (!yawAlignComplete) {
                    GCS_SEND_TEXT(AGPILOT_SEVERITY_INFO, "EKF2 IMU%u MAG%u initial yaw alignment complete",(unsigned)imu_index, (unsigned)magSelectIndex);
                }

                // send in-flight yaw alignment status to console
                if (finalResetRequest) {
                    GCS_SEND_TEXT(AGPILOT_SEVERITY_INFO, "EKF2 IMU%u MAG%u in-flight yaw alignment complete",(unsigned)imu_index, (unsigned)magSelectIndex);
                } else if (interimResetRequest) {
                    GCS_SEND_TEXT(AGPILOT_SEVERITY_WARNING, "EKF2 IMU%u MAG%u ground mag anomaly, yaw re-aligned",(unsigned)imu_index, (unsigned)magSelectIndex);
                }
...
void NavEKF2_core::realignYawGPS()
{
    if ((sq(gpsDataDelayed.vel.x) + sq(gpsDataDelayed.vel.y)) > 25.0f) {
        ...
        if (badMagYaw) {
            ...
            GCS_SEND_TEXT(AGPILOT_SEVERITY_INFO, "EKF2 IMU%u yaw aligned to GPS velocity",(unsigned)imu_index);
...
bool NavEKF2_core::EKFGSF_resetMainFilterYaw()
{
    ...
    if (yawEstimator->getYawData(yawEKFGSF, yawVarianceEKFGSF) &&
        ...
        if (!use_compass()) {
            GCS_SEND_TEXT(AGPILOT_SEVERITY_INFO, "EKF2 IMU%u yaw aligned using GPS",(unsigned)imu_index);
        } else {
            GCS_SEND_TEXT(AGPILOT_SEVERITY_WARNING, "EKF2 IMU%u emergency yaw reset",(unsigned)imu_index);
        }
