...
class AG_Proximity_Backend;

class AG_Proximity
{
public:
    friend class AG_Proximity_Backend;
    friend class AG_Proximity_DroneCAN;

    AG_Proximity();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_Proximity);
    ...
    // method called by vehicle to have AG_Proximity write onboard log messages
    void log();
    ...
    static AG_Proximity *get_singleton(void) { return _singleton; };

    // 3D boundary
    AG_Proximity_Boundary_3D boundary;
...
protected:
    ...
    AG_Proximity_Params params[PROXIMITY_MAX_INSTANCES];

private:
    static AG_Proximity *_singleton;
    Proximity_State state[PROXIMITY_MAX_INSTANCES];
    AG_Proximity_Backend *drivers[PROXIMITY_MAX_INSTANCES];
