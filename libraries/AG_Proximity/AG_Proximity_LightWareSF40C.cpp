...
void AG_Proximity_LightWareSF40C::update(void)
{
    if (_uart == nullptr) {
        return;
    }
    ...
    if ((_last_distance_received_ms == 0) || ((AG_HAL::millis() - _last_distance_received_ms) > PROXIMITY_SF40C_TIMEOUT_MS)) {
        set_status(AG_Proximity::Status::NoData);
    } else {
        set_status(AG_Proximity::Status::Good);
    }
}

// initialise sensor
void AG_Proximity_LightWareSF40C::initialise()
{
...
void AG_Proximity_LightWareSF40C::restart_sensor()
{
...
void AG_Proximity_LightWareSF40C::send_message(MessageID msgid, bool write, const uint8_t *payload, uint16_t payload_len)
{
...
// request motor state
void AG_Proximity_LightWareSF40C::request_motor_state()
{
    send_message(MessageID::MOTOR_STATE, false, (const uint8_t *)nullptr, 0);
}

// request start of streaming of distances
void AG_Proximity_LightWareSF40C::request_stream_start()
{
    // request output rate
...
void AG_Proximity_LightWareSF40C::request_token()
{
    // request token
...
void AG_Proximity_LightWareSF40C::request_reset()
{
    // send reset request
...
void AG_Proximity_LightWareSF40C::process_replies()
{
    if (_uart == nullptr) {
        return;
    }
...
void AG_Proximity_LightWareSF40C::parse_byte(uint8_t b)
{
    // check that payload buffer is large enough
    static_assert(ARRAY_SIZE(_msg.payload) == PROXIMITY_SF40C_PAYLOAD_LEN_MAX, "AG_Proximity_LightWareSF40C: check _msg.payload array size");
...
void AG_Proximity_LightWareSF40C::process_message()
{
    // process payload
    switch (_msg.msgid) {
    ...
    case MessageID::DISTANCE_OUTPUT: {
        _last_distance_received_ms = AG_HAL::millis();
        ...
        for (uint16_t i = 0; i < point_count; i++) {
            ...
            const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(angle_deg);
...
uint32_t AG_Proximity_LightWareSF40C::buff_to_uint32(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3) const
{
    uint32_t leval = (uint32_t)b0 | (uint32_t)b1 << 8 | (uint32_t)b2 << 16 | (uint32_t)b3 << 24;
    return leval;
}

uint16_t AG_Proximity_LightWareSF40C::buff_to_uint16(uint8_t b0, uint8_t b1) const
{
