...
void AG_Proximity_TeraRangerTowerEvo::update(void)
{
    if (_uart == nullptr) {
        return;
    }
    if (_last_request_sent_ms == 0) {
        _last_request_sent_ms = AG_HAL::millis();
    }
    ...
    if ((_last_distance_received_ms == 0) || (AG_HAL::millis() - _last_distance_received_ms > PROXIMITY_TRTOWER_TIMEOUT_MS)) {
...
float AG_Proximity_TeraRangerTowerEvo::distance_max() const
{
    return 60.0f;
}
float AG_Proximity_TeraRangerTowerEvo::distance_min() const
{
    return 0.50f;
}

void AG_Proximity_TeraRangerTowerEvo::initialise_modes()
{
    if((AG_HAL::millis() - _last_request_sent_ms) < _mode_request_delay) {
        return;
    }
...
void AG_Proximity_TeraRangerTowerEvo::set_mode(const uint8_t *c, int length)
{
...
bool AG_Proximity_TeraRangerTowerEvo::read_sensor_data()
{
    if (_uart == nullptr) {
        return false;
    }
...
// process reply
void AG_Proximity_TeraRangerTowerEvo::update_sector_data(int16_t angle_deg, uint16_t distance_mm)
{
