...
void AG_Proximity_RangeFinder::update(void)
{
    ...
    for (uint8_t i=0; i < rngfnd->num_sensors(); i++) {
        AG_RangeFinder_Backend *sensor = rngfnd->get_backend(i);
        if (sensor == nullptr) {
            continue;
        }
        if (sensor->has_data()) {
            ...
            if (sensor->orientation() <= ROTATION_YAW_315) {
                ...
                const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(angle);
                // distance in meters
...
bool AG_Proximity_RangeFinder::get_upward_distance(float &distance) const
{
    if ((AG_HAL::millis() - _last_upward_update_ms <= PROXIMITY_RANGEFIDER_TIMEOUT_MS) &&
