...
void AG_Proximity_MAV::update(void)
{
    // check for timeout and set health status
    if ((_last_update_ms == 0 || (AG_HAL::millis() - _last_update_ms > PROXIMITY_MAV_TIMEOUT_MS)) &&
        (_last_upward_update_ms == 0 || (AG_HAL::millis() - _last_upward_update_ms > PROXIMITY_MAV_TIMEOUT_MS))) {
...
bool AG_Proximity_MAV::get_upward_distance(float &distance) const
{
    if ((_last_upward_update_ms != 0) && (AG_HAL::millis() - _last_upward_update_ms <= PROXIMITY_MAV_TIMEOUT_MS)) {
...
void AG_Proximity_MAV::handle_msg(const mavlink_message_t &msg)
{   
    switch (msg.msgid) {
...
void AG_Proximity_MAV::handle_distance_sensor_msg(const mavlink_message_t &msg)
{
    ...
    if (packet.orientation <= MAV_SENSOR_ROTATION_YAW_315) {
        ...
        const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(yaw_angle_deg);
    ...
    if (packet.orientation == MAV_SENSOR_ROTATION_PITCH_90) {
        ...
        _last_upward_update_ms = AG_HAL::millis();
    }
    return;
}

// handle mavlink OBSTACLE_DISTANCE messages
void AG_Proximity_MAV::handle_obstacle_distance_msg(const mavlink_message_t &msg)
{
    ...
    AG_Proximity_Boundary_3D::Face face;
    ...
    for (uint8_t j = 0; j < total_distances; j++) {
        ...
        AG_Proximity_Boundary_3D::Face latest_face = frontend.boundary.get_face(mid_angle);
        if (latest_face != face) {
            // store previous face
...
void AG_Proximity_MAV::handle_obstacle_distance_3d_msg(const mavlink_message_t &msg)
{
    ...
    const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(pitch, yaw);
