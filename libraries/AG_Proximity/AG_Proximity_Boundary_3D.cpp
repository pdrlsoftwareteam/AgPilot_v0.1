...
#include "AG_Proximity_Boundary_3D.h"
...
*/
AG_Proximity_Boundary_3D::AG_Proximity_Boundary_3D() 
{
...
void AG_Proximity_Boundary_3D::init()
{
...
AG_Proximity_Boundary_3D::Face AG_Proximity_Boundary_3D::get_face(float pitch, float yaw) const
{
...
void AG_Proximity_Boundary_3D::set_face_attributes(const Face &face, float pitch, float angle, float distance, uint8_t prx_instance)
{
    if (!face.valid()) {
        return;
    }
...
void AG_Proximity_Boundary_3D::apply_filter_freq(float cutoff_freq)
{
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
...
void AG_Proximity_Boundary_3D::set_filtered_distance(const Face &face, float distance)
{
    if (!face.valid()) {
        return;
    }
...
void AG_Proximity_Boundary_3D::update_boundary(const Face &face)
{
    // sanity check
    if (!face.valid()) {
        return;
...
void AG_Proximity_Boundary_3D::reset()
{
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
...
void AG_Proximity_Boundary_3D::reset_face(const Face &face, uint8_t prx_instance)
{
    if (!face.valid()) {
        return;
    }
...
void AG_Proximity_Boundary_3D::check_face_timeout()
{
        ...
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
            if (_distance_valid[layer][sector]) {
                if ((now_ms - _last_update_ms[layer][sector]) > PROXIMITY_FACE_RESET_MS) {
                    ...
                    _distance_valid[layer][sector] = false;
                    update_boundary(AG_Proximity_Boundary_3D::Face{layer, sector});
                }
            }
        }
    }
...
bool AG_Proximity_Boundary_3D::get_distance(const Face &face, float &distance) const
{
    if (!face.valid()) {
        return false;
    }
...
uint8_t AG_Proximity_Boundary_3D::get_obstacle_count() const
{
...
bool AG_Proximity_Boundary_3D::convert_obstacle_num_to_face(uint8_t obstacle_num, Face& face) const
{
...
bool AG_Proximity_Boundary_3D::get_obstacle(uint8_t obstacle_num, Vector3f& vec_to_obstacle) const
{
    Face face;
...
// False is returned if the obstacle_num provided does not produce a valid obstacle
bool AG_Proximity_Boundary_3D::closest_point_from_segment_to_obstacle(uint8_t obstacle_num, const Vector3f& seg_start, const Vector3f& seg_end, Vector3f& closest_point) const
{
    Face face;
    if (!convert_obstacle_num_to_face(obstacle_num, face)) {
        // not a valid a face
...
bool AG_Proximity_Boundary_3D::get_closest_object(float& angle_deg, float &distance) const
{
...
uint8_t AG_Proximity_Boundary_3D::get_horizontal_object_count() const
{
...
bool AG_Proximity_Boundary_3D::get_horizontal_object_angle_and_distance(uint8_t object_number, float &angle_deg, float &distance) const
{
...
// returns false if no angle or distance could be returned for some reason
bool AG_Proximity_Boundary_3D::get_obstacle_info(uint8_t obstacle_num, float &angle_deg, float &pitch_deg, float &distance) const
{
    // obstacle num is just "flattened layers, and sectors"
    const uint8_t layer = obstacle_num / PROXIMITY_NUM_SECTORS;
    const uint8_t sector = obstacle_num % PROXIMITY_NUM_SECTORS;
...
bool AG_Proximity_Boundary_3D::get_filtered_distance(const Face &face, float &distance) const
{
    if (!face.valid()) {
        return false;
    }
...
bool AG_Proximity_Boundary_3D::get_layer_distances(uint8_t layer_number, float dist_max, Proximity_Distance_Array &prx_dist_array, Proximity_Distance_Array &prx_filt_dist_array) const
{
    ...
    for (uint8_t i=0; i<PROXIMITY_MAX_DIRECTION; i++) {
        ...
        const AG_Proximity_Boundary_3D::Face face(layer_number, i);
        if (!face.valid()) {
            return false;
        }
...
void AG_Proximity_Temp_Boundary::reset()
{
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
...
void AG_Proximity_Temp_Boundary::add_distance(const AG_Proximity_Boundary_3D::Face &face, float pitch, float yaw, float distance)
{
...
// prx_instance should be set to the proximity sensor's backend instance number
void AG_Proximity_Temp_Boundary::update_3D_boundary(uint8_t prx_instance, AG_Proximity_Boundary_3D &boundary)
{
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
            if (_distances[layer][sector] < FLT_MAX) {
                AG_Proximity_Boundary_3D::Face face{layer, sector};
