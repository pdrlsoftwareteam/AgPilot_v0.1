...
*/
AG_Proximity_SITL::AG_Proximity_SITL(AG_Proximity &_frontend,
...
{
    ap_var_type ptype;
    fence_alt_max = (AP_Float *)AG_Param::find("FENCE_ALT_MAX", &ptype);
    if (fence_alt_max == nullptr || ptype != AP_PARAM_FLOAT) {
        AG_HAL::panic("Proximity_SITL: Failed to find FENCE_ALT_MAX");
    }
}

// update the state of the sensor
void AG_Proximity_SITL::update(void)
{
    ...
    if (AP::fence()->polyfence().inclusion_boundary_available()) {
        ...
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
            ...
            AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(yaw_angle_deg);
...
bool AG_Proximity_SITL::get_distance_to_fence(float angle_deg, float &distance) const
{
#if AP_FENCE_ENABLED
    if (!AP::fence()->polyfence().inclusion_boundary_available()) {
        return false;
...
float AG_Proximity_SITL::distance_max() const
{
    return PROXIMITY_MAX_RANGE;
}
float AG_Proximity_SITL::distance_min() const
{
    return 0.0f;
}

// get distance upwards in meters. returns true on success
bool AG_Proximity_SITL::get_upward_distance(float &distance) const
{
