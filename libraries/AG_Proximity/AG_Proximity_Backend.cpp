...
*/
AG_Proximity_Backend::AG_Proximity_Backend(AG_Proximity& _frontend, AG_Proximity::Proximity_State& _state, AG_Proximity_Params& _params) :
        frontend(_frontend),
        state(_state),
        params(_params)
{
    _backend_type = (AG_Proximity::Type )_params.type.get();
}
...
void AG_Proximity_Backend::set_status(AG_Proximity::Status status)
{
    state.status = status;
}

// correct an angle (in degrees) based on the orientation and yaw correction parameters
float AG_Proximity_Backend::correct_angle_for_orientation(float angle_degrees) const
{
...
bool AG_Proximity_Backend::ignore_reading(float pitch, float yaw, float distance_m, bool check_for_ign_area) const
{
...
bool AG_Proximity_Backend::database_prepare_for_push(Vector3f &current_pos, Matrix3f &body_to_ned)
{
...
void AG_Proximity_Backend::database_push(float angle, float pitch, float distance)
{
    Vector3f current_pos;
    Matrix3f body_to_ned;

    if (database_prepare_for_push(current_pos, body_to_ned)) {
        database_push(angle, pitch, distance, AG_HAL::millis(), current_pos, body_to_ned);
    }
}

// update Object Avoidance database with Earth-frame point
// pitch can be optionally provided if needed
void AG_Proximity_Backend::database_push(float angle, float pitch, float distance, uint32_t timestamp_ms, const Vector3f &current_pos, const Matrix3f &body_to_ned)
{
