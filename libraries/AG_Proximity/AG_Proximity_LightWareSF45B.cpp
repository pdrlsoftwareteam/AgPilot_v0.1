...
void AG_Proximity_LightWareSF45B::update(void)
{
    if (_uart == nullptr) {
        return;
    }
    ...
    if ((_last_distance_received_ms == 0) || ((AG_HAL::millis() - _last_distance_received_ms) > PROXIMITY_SF45B_TIMEOUT_MS)) {
...
// initialise sensor
void AG_Proximity_LightWareSF45B::initialise()
{
...
void AG_Proximity_LightWareSF45B::request_stream_start()
{
    // request output rate
...
void AG_Proximity_LightWareSF45B::process_replies()
{
    if (_uart == nullptr) {
        return;
    }
...
void AG_Proximity_LightWareSF45B::process_message()
{
    // process payload
...
uint8_t AG_Proximity_LightWareSF45B::convert_angle_to_minisector(float angle_deg) const
{
