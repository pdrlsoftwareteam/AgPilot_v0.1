...
#include "AG_Proximity_LightWareSerial.h"
...
void AG_Proximity_LightWareSerial::send_message(uint8_t msgid, bool write, const uint8_t *payload, uint16_t payload_len)
{
...
bool AG_Proximity_LightWareSerial::parse_byte(uint8_t b)
{
    // check that payload buffer is large enough
    static_assert(ARRAY_SIZE(_msg.payload) == PROXIMITY_LIGHTWARE_PAYLOAD_LEN_MAX, "AG_Proximity_LightWareSerial: check _msg.payload array size");
