...
public:
	...
	AG_Proximity_Backend(AG_Proximity &_frontend, AG_Proximity::Proximity_State &_state, AG_Proximity_Params &_params);

    // we declare a virtual destructor so that Proximity drivers can
    // override with a custom destructor if need be
    virtual ~AG_Proximity_Backend(void) {}
...
protected:
    ...
    void set_status(AG_Proximity::Status status);
    ...
    AG_Proximity::Type _backend_type;

    AG_Proximity &frontend;
    AG_Proximity::Proximity_State &state;   // reference to this instances state
    AG_Proximity_Params &params;            // parameters for this backend
};
