...
void AG_Proximity_TeraRangerTower::update(void)
{
    if (_uart == nullptr) {
        return;
    }
...
float AG_Proximity_TeraRangerTower::distance_max() const
{
    return 4.5f;
}
float AG_Proximity_TeraRangerTower::distance_min() const
{
    return 0.20f;
}

// check for replies from sensor, returns true if at least one message was processed
bool AG_Proximity_TeraRangerTower::read_sensor_data()
{
    if (_uart == nullptr) {
        return false;
    }
...
// process reply
void AG_Proximity_TeraRangerTower::update_sector_data(int16_t angle_deg, uint16_t distance_mm)
{
