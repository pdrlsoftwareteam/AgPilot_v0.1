...
ObjectBuffer_TS<AG_Proximity_DroneCAN::ObstacleItem> AG_Proximity_DroneCAN::items(50);
...
void AG_Proximity_DroneCAN::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (measurement_listener == nullptr) {
        AG_BoardConfig::allocation_error("DroneCAN_PRX");
    }
    ...
    if (measurement_listener_res < 0) {
        AG_BoardConfig::allocation_error("DroneCAN Proximity subscriber start problem\n\r");
        return;
    }
}

//Method to find the backend relating to the node id
AG_Proximity_DroneCAN* AG_Proximity_DroneCAN::get_uavcan_backend(AG_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t address, bool create_new)
{
    if (ap_uavcan == nullptr) {
        return nullptr;
    }

    AG_Proximity *prx = AP::proximity();
    if (prx == nullptr) {
        return nullptr;
    }

    AG_Proximity_DroneCAN* driver = nullptr;
    //Scan through the proximity type params to find DroneCAN with matching address.
    for (uint8_t i = 0; i < PROXIMITY_MAX_INSTANCES; i++) {
        if ((AG_Proximity::Type)prx->params[i].type.get() == AG_Proximity::Type::DroneCAN &&
            prx->params[i].address == address) {
            driver = (AG_Proximity_DroneCAN*)prx->drivers[i];
        }
        //Double check if the driver was initialised as DroneCAN Type
        if (driver != nullptr && (driver->_backend_type == AG_Proximity::Type::DroneCAN)) {
    ...
    if (create_new) {
        for (uint8_t i = 0; i < PROXIMITY_MAX_INSTANCES; i++) {
            if ((AG_Proximity::Type)prx->params[i].type.get() == AG_Proximity::Type::DroneCAN &&
                ...
                prx->drivers[i] = new AG_Proximity_DroneCAN(*prx, prx->state[i], prx->params[i]);
                driver = (AG_Proximity_DroneCAN*)prx->drivers[i];
                if (driver == nullptr) {
                    break;
                }
...
void AG_Proximity_DroneCAN::update(void)
{
    // check for timeout and set health status
    if ((_last_update_ms == 0 || (AG_HAL::millis() - _last_update_ms > PROXIMITY_TIMEOUT_MS))) {
        set_status(AG_Proximity::Status::NoData);
    } else {
        set_status(_status);
    }

    if (_status == AG_Proximity::Status::Good) {
        ObstacleItem object_item;
        WITH_SEMAPHORE(_sem);
        while (items.pop(object_item)) {
            const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(object_item.pitch_deg, object_item.yaw_deg);
...
float AG_Proximity_DroneCAN::distance_max() const
{
...
float AG_Proximity_DroneCAN::distance_min() const
{
    return params.min_m;
}

//Proximity message handler
void AG_Proximity_DroneCAN::handle_measurement(AG_UAVCAN* ap_uavcan, uint8_t node_id, const MeasurementCb &cb)
{
    //fetch the matching DroneCAN driver, node id and sensor id backend instance
    AG_Proximity_DroneCAN* driver = get_uavcan_backend(ap_uavcan, node_id, cb.msg->sensor_id, true);
    if (driver == nullptr) {
        return;
    }
    ...
    switch (cb.msg->reading_type) {
        case ardupilot::equipment::proximity_sensor::Proximity::READING_TYPE_GOOD: {
            ...
            driver->_last_update_ms = AG_HAL::millis();
            driver->_status = AG_Proximity::Status::Good;
        ...
        case ardupilot::equipment::proximity_sensor::Proximity::READING_TYPE_NOT_CONNECTED: {
            driver->_last_update_ms = AG_HAL::millis();
            driver->_status = AG_Proximity::Status::NotConnected;
            break;
        }
        case ardupilot::equipment::proximity_sensor::Proximity::READING_TYPE_NO_DATA: {
            driver->_last_update_ms = AG_HAL::millis();
            driver->_status = AG_Proximity::Status::NoData;
            break;
        }
        default:
            break;
