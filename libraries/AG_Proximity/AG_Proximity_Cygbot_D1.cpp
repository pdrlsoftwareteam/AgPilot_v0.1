...
void AG_Proximity_Cygbot_D1::update()
{
    if (!_initialized) {
        send_sensor_start();
        _temp_boundary.reset();
        _initialized = true;
        _last_init_ms = AG_HAL::millis();
    }

    if ((AG_HAL::millis() - _last_init_ms) < CYGBOT_INIT_TIMEOUT_MS) {
        // just initialized
    ...
    if (AG_HAL::millis() - _last_distance_received_ms < CYGBOT_TIMEOUT_MS) {
...
void AG_Proximity_Cygbot_D1::send_sensor_start()
{
...
void AG_Proximity_Cygbot_D1::read_sensor_data()
{
...
bool AG_Proximity_Cygbot_D1::parse_byte(uint8_t data)
{
    switch (_parse_state) {
    case Header1:
...
void AG_Proximity_Cygbot_D1::parse_payload()
{
    ...
    for (uint16_t i = 2; i < _msg.payload_len; i += 2) {
        ...
        if (distance_m > distance_min() && distance_m < distance_max()) {
            ...
            const AG_Proximity_Boundary_3D::Face face = frontend.boundary.get_face(corrected_angle);
...
// Checksum
uint8_t AG_Proximity_Cygbot_D1::calc_checksum(uint8_t *buff, int buffSize)
{
...
void AG_Proximity_Cygbot_D1::reset()
{
