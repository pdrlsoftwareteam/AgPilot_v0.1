...
void AG_Proximity_RPLidarA2::update(void)
{
    if (_uart == nullptr) {
        return;
    }
    ...
    if ((_last_distance_received_ms == 0) || (AG_HAL::millis() - _last_distance_received_ms > COMM_ACTIVITY_TIMEOUT_MS)) {
...
float AG_Proximity_RPLidarA2::distance_max() const
{
...
float AG_Proximity_RPLidarA2::distance_min() const
{
    return 0.20f;  //20cm min range RPLIDAR2
}

bool AG_Proximity_RPLidarA2::initialise()
{
    if (!_initialised) {
...
void AG_Proximity_RPLidarA2::reset_rplidar()
{
    if (_uart == nullptr) {
        return;
    }
    ...
    _last_reset_ms =  AG_HAL::millis();
    _rp_state = rp_resetted;

}

// set Lidar into SCAN mode
void AG_Proximity_RPLidarA2::set_scan_mode()
{
    if (_uart == nullptr) {
        return;
    }
    ...
    _uart->write(tx_buffer, 2);
    _last_request_ms = AG_HAL::millis();
    Debug(1, "LIDAR SCAN MODE ACTIVATED");
    _rp_state = rp_responding;
}

// send request for sensor health
void AG_Proximity_RPLidarA2::send_request_for_health()                                    //not called yet
{
    if (_uart == nullptr) {
        return;
    }
...
void AG_Proximity_RPLidarA2::get_readings()
{
    if (_uart == nullptr) {
        return;
    }
            ...
            case rp_measurements:
                if (_sync_error) {
                    ...
                    if ((c & 0x03) == 0x01) {
                        ...
                        if (AG_HAL::millis() - _last_distance_received_ms > RESYNC_TIMEOUT) {
                            reset_rplidar();
                        }
                        break;
                    }
...
void AG_Proximity_RPLidarA2::parse_response_descriptor()
{
...
void AG_Proximity_RPLidarA2::parse_response_data()
{
