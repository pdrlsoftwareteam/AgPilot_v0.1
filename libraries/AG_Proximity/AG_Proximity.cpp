...
#include "AG_Proximity_RPLidarA2.h"
#include "AG_Proximity_TeraRangerTower.h"
#include "AG_Proximity_TeraRangerTowerEvo.h"
#include "AG_Proximity_RangeFinder.h"
#include "AG_Proximity_MAV.h"
#include "AG_Proximity_LightWareSF40C.h"
#include "AG_Proximity_LightWareSF45B.h"
#include "AG_Proximity_SITL.h"
#include "AG_Proximity_AirSimSITL.h"
#include "AG_Proximity_Cygbot_D1.h"
#include "AG_Proximity_DroneCAN.h"
...
const AG_Param::GroupInfo AG_Proximity::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO_FRAME("_IGN_GND", 16, AG_Proximity, _ign_gnd_enable, 0, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),
    ...
    // @Values: 0:Off, 1:On
    // @User: Advanced
    AP_GROUPINFO("_LOG_RAW", 17, AG_Proximity, _raw_log_enable, 0),
    ...
    // @Range: 0 20
    // @User: Advanced
    AP_GROUPINFO("_FILT", 18, AG_Proximity, _filt_freq, 0.25f),
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO_FRAME("_ALT_MIN", 25, AG_Proximity, _alt_min, 1.0f, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),

    // @Group: 1
    // @Path: AG_Proximity_Params.cpp
    AP_SUBGROUPINFO(params[0], "1", 21, AG_Proximity, AG_Proximity_Params),

#if PROXIMITY_MAX_INSTANCES > 1
    // @Group: 2
    // @Path: AG_Proximity_Params.cpp
    AP_SUBGROUPINFO(params[1], "2", 22, AG_Proximity, AG_Proximity_Params),
#endif

#if PROXIMITY_MAX_INSTANCES > 2
    // @Group: 3
    // @Path: AG_Proximity_Params.cpp
    AP_SUBGROUPINFO(params[2], "3", 23, AG_Proximity, AG_Proximity_Params),
#endif

#if PROXIMITY_MAX_INSTANCES > 3
    // @Group: 4
    // @Path: AG_Proximity_Params.cpp
    AP_SUBGROUPINFO(params[3], "4", 24, AG_Proximity, AG_Proximity_Params),
#endif

    AP_GROUPEND
};

AG_Proximity::AG_Proximity()
{
    AG_Param::setup_object_defaults(this, var_info);
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_Proximity must be singleton");
    }
...
void AG_Proximity::init()
{
    ...
    for (uint8_t instance=0; instance<PROXIMITY_MAX_INSTANCES; instance++) {
        ...
        case Type::None:
            break;
        case Type::RPLidarA2:
            if (AG_Proximity_RPLidarA2::detect(serial_instance)) {
                state[instance].instance = instance;
                drivers[instance] = new AG_Proximity_RPLidarA2(*this, state[instance], params[instance], serial_instance);
                serial_instance++;
            }
            break;
        case Type::MAV:
            state[instance].instance = instance;
            drivers[instance] = new AG_Proximity_MAV(*this, state[instance], params[instance]);
            break;

        case Type::TRTOWER:
            if (AG_Proximity_TeraRangerTower::detect(serial_instance)) {
                state[instance].instance = instance;
                drivers[instance] = new AG_Proximity_TeraRangerTower(*this, state[instance], params[instance], serial_instance);
                serial_instance++;
            }
            break;
        case Type::TRTOWEREVO:
            if (AG_Proximity_TeraRangerTowerEvo::detect(serial_instance)) {
                state[instance].instance = instance;
                drivers[instance] = new AG_Proximity_TeraRangerTowerEvo(*this, state[instance], params[instance], serial_instance);
                serial_instance++;
            }
            break;
        ...
        case Type::RangeFinder:
            state[instance].instance = instance;
            drivers[instance] = new AG_Proximity_RangeFinder(*this, state[instance], params[instance]);
            break;

        case Type::SF40C:
            if (AG_Proximity_LightWareSF40C::detect(serial_instance)) {
                state[instance].instance = instance;
                drivers[instance] = new AG_Proximity_LightWareSF40C(*this, state[instance], params[instance], serial_instance);
                serial_instance++;
            }
            break;

        case Type::SF45B:
            if (AG_Proximity_LightWareSF45B::detect(serial_instance)) {
                state[instance].instance = instance;
                drivers[instance] = new AG_Proximity_LightWareSF45B(*this, state[instance], params[instance], serial_instance);
                serial_instance++;
            }
            break;
        ...
        if (AG_Proximity_Cygbot_D1::detect(serial_instance)) {
            state[instance].instance = instance;
            drivers[instance] = new AG_Proximity_Cygbot_D1(*this, state[instance], params[instance], serial_instance);
            serial_instance++;
        }
            break;
# endif
        ...
        case Type::SITL:
            state[instance].instance = instance;
            drivers[instance] = new AG_Proximity_SITL(*this, state[instance], params[instance]);
            break;

        case Type::AirSimSITL:
            state[instance].instance = instance;
            drivers[instance] = new AG_Proximity_AirSimSITL(*this, state[instance], params[instance]);
            break;
#endif
        }
...
AG_Proximity::Type AG_Proximity::get_type(uint8_t instance) const
{
...
AG_Proximity::Status AG_Proximity::get_instance_status(uint8_t instance) const
{
...
AG_Proximity::Status AG_Proximity::get_status() const
{
    for (uint8_t i=0; i<num_instances; i++) {
...
// prearm checks
bool AG_Proximity::prearm_healthy(char *failure_msg, const uint8_t failure_msg_len) const
{
    for (uint8_t i=0; i<num_instances; i++) {
...
float AG_Proximity::distance_max() const
{
    float dist_max = 0;
...
float AG_Proximity::distance_min() const
{
    float dist_min = 0;
...
bool AG_Proximity::get_horizontal_distances(Proximity_Distance_Array &prx_dist_array) const
{
...
uint8_t AG_Proximity::get_obstacle_count() const
{
    return boundary.get_obstacle_count();
}

// get vector to obstacle based on obstacle_num passed, used in GPS based Simple Avoidance
bool AG_Proximity::get_obstacle(uint8_t obstacle_num, Vector3f& vec_to_obstacle) const
{
...
bool AG_Proximity::closest_point_from_segment_to_obstacle(uint8_t obstacle_num, const Vector3f& seg_start, const Vector3f& seg_end, Vector3f& closest_point) const
{
...
bool AG_Proximity::get_closest_object(float& angle_deg, float &distance) const
{
    return boundary.get_closest_object(angle_deg, distance);
}

// get number of objects, angle and distance - used for non-GPS avoidance
uint8_t AG_Proximity::get_object_count() const
{
    return boundary.get_horizontal_object_count();
}

// get number of objects, angle and distance - used for non-GPS avoidance
bool AG_Proximity::get_object_angle_and_distance(uint8_t object_number, float& angle_deg, float &distance) const
{
...
bool AG_Proximity::get_obstacle_info(uint8_t obstacle_num, float &angle_deg, float &pitch, float &distance) const
{
    return boundary.get_obstacle_info(obstacle_num, angle_deg, pitch, distance);
}

// handle mavlink messages
void AG_Proximity::handle_msg(const mavlink_message_t &msg)
{
    for (uint8_t i=0; i<num_instances; i++) {
        if (valid_instance(i)) {
...
bool AG_Proximity::sensor_present() const
{
    return get_status() != Status::NotConnected;
}

bool AG_Proximity::sensor_enabled() const
{
    // check atleast one sensor is enabled
    return (num_instances > 0);
}

bool AG_Proximity::sensor_failed() const
{
    return get_status() != Status::Good;
}

// get distance in meters upwards, returns true on success
bool AG_Proximity::get_upward_distance(uint8_t instance, float &distance) const
{
    if (!valid_instance(instance)) {
        return false;
    }
...
bool AG_Proximity::get_upward_distance(float &distance) const
{
    // get upward distance from backend
    for (uint8_t i=0; i<num_instances; i++) {
...
bool AG_Proximity::valid_instance(uint8_t i) const
{
...
AG_Proximity *AG_Proximity::_singleton;

namespace AP {

AG_Proximity *proximity()
{
    return AG_Proximity::get_singleton();
}
