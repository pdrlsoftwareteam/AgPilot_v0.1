...
void AG_Proximity::set_rangefinder_alt(bool use, bool healthy, float alt_cm)
{
    _rangefinder_state.use = use;
    _rangefinder_state.healthy = healthy;
    _rangefinder_state.alt_cm = alt_cm;
    _rangefinder_state.last_downward_update_ms = AG_HAL::millis();
}

// get alt from rangefinder in meters
bool AG_Proximity::get_rangefinder_alt(float &alt_m) const
{
    ...
    const uint32_t dt = AG_HAL::millis() - _rangefinder_state.last_downward_update_ms;
...
bool AG_Proximity::check_obstacle_near_ground(float pitch, float yaw, float distance) const
{
