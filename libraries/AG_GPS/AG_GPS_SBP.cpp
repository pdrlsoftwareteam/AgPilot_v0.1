...
AG_GPS_SBP::AG_GPS_SBP(AG_GPS &_gps, AG_GPS::GPS_State &_state,
    ...
    AG_GPS_Backend(_gps, _state, _port)
{
    ...
    state.last_gps_time_ms = last_heatbeat_received_ms = AG_HAL::millis();
...
void
AG_GPS_SBP::inject_data(const uint8_t *data, uint16_t len)
{
...
void
AG_GPS_SBP::_sbp_process_message() {
    switch (parser_state.msg_type) {
        case SBP_HEARTBEAT_MSGTYPE:
            last_heatbeat_received_ms = AG_HAL::millis();
            break;
...
bool
AG_GPS_SBP::_attempt_state_update()
{
...
bool
AG_GPS_SBP::_detect(struct SBP_detect_state &state, uint8_t data)
{
...
void
AG_GPS_SBP::logging_log_full_update()
{
...
void
AG_GPS_SBP::logging_log_raw_sbp(uint16_t msg_type,
        uint16_t sender_id,
        uint8_t msg_len,
        uint8_t *msg_buff) {
    if (!should_log()) {
