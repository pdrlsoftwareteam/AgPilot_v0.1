...
//
const uint8_t AG_GPS_SIRF::_initialisation_blob[] = {
...
AG_GPS_SIRF::AG_GPS_SIRF(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_HAL::UARTDriver *_port) :
...
void
AG_GPS_SIRF::_accumulate(uint8_t val)
{
...
/*
  detect a SIRF GPS
 */
bool AG_GPS_SIRF::_detect(struct SIRF_detect_state &state, uint8_t data)
{
    switch (state.step) {
    case 1:
        if (PREAMBLE2 == data) {
