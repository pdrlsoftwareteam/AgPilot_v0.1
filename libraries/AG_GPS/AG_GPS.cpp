...
#include "AG_GPS_NOVA.h"
#include "AG_GPS_ERB.h"
#include "AG_GPS_GSOF.h"
#include "AG_GPS_NMEA.h"
#include "AG_GPS_SBF.h"
#include "AG_GPS_SBP.h"
#include "AG_GPS_SBP2.h"
#include "AG_GPS_SIRF.h"
#include "AG_GPS_UBLOX.h"
#include "AG_GPS_MAV.h"
#include "AG_GPS_MSP.h"
#include "AG_GPS_ExternalAHRS.h"
#include "GPS_Backend.h"
#if HAL_SIM_GPS_ENABLED
#include "AG_GPS_SITL.h"
#endif

#if HAL_ENABLE_LIBUAVCAN_DRIVERS
#include <AG_CANManager/AG_CANManager.h>
#include <AG_UAVCAN/AG_UAVCAN.h>
#include "AG_GPS_UAVCAN.h"
#endif
...
const uint32_t AG_GPS::_baudrates[] = {9600U, 115200U, 4800U, 19200U, 38400U, 57600U, 230400U, 460800U};

// initialisation blobs to send to the GPS to try to get it into the
// right mode.
const char AG_GPS::_initialisation_blob[] =
#if AG_GPS_UBLOX_ENABLED
    UBLOX_SET_BINARY_230400
#endif
#if AG_GPS_SIRF_ENABLED
    SIRF_SET_BINARY
#endif
#if AG_GPS_NMEA_UNICORE_ENABLED
    NMEA_UNICORE_SETUP
#endif
    ""   // to compile we need *some_initialiser if all backends compiled out
    ;

AG_GPS *AG_GPS::_singleton;

// table of user settable parameters
const AG_Param::GroupInfo AG_GPS::var_info[] = {
    // @Param: _TYPE
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("_TYPE",    0, AG_GPS, _type[0], HAL_GPS_TYPE_DEFAULT),
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("_TYPE2",   1, AG_GPS, _type[1], 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("_NAVFILTER", 2, AG_GPS, _navfilter, GPS_ENGINE_AIRBORNE_4G),
    ...
    // @User: Advanced
    AP_GROUPINFO("_AUTO_SWITCH", 3, AG_GPS, _auto_switch, (int8_t)GPSAutoSwitch::USE_BEST),
#endif
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("_MIN_DGPS", 4, AG_GPS, _min_dgps, 100),
    ...
    // @User: Advanced
    AP_GROUPINFO("_SBAS_MODE", 5, AG_GPS, _sbas_mode, 2),
    ...
    // @Units: deg
    // @User: Advanced
    AP_GROUPINFO("_MIN_ELEV", 6, AG_GPS, _min_elevation, -100),
    ...
    // @User: Advanced
    AP_GROUPINFO("_INJECT_TO",   7, AG_GPS, _inject_to, GPS_RTK_INJECT_TO_ALL),

#if AG_GPS_SBP2_ENABLED || AG_GPS_SBP_ENABLED
    // @Param: _SBP_LOGMASK
    ...
    // @User: Advanced
    AP_GROUPINFO("_SBP_LOGMASK", 8, AG_GPS, _sbp_logmask, -256),
#endif //AG_GPS_SBP2_ENABLED || AG_GPS_SBP_ENABLED
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("_RAW_DATA", 9, AG_GPS, _raw_data, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_GNSS_MODE", 10, AG_GPS, _gnss_mode[0], 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_SAVE_CFG", 11, AG_GPS, _save_config, 2),
    ...
    // @Bitmask: 0:GPS,1:SBAS,2:Galileo,3:Beidou,4:IMES,5:QZSS,6:GLONASS
    // @User: Advanced
    AP_GROUPINFO("_GNSS_MODE2", 12, AG_GPS, _gnss_mode[1], 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("_AUTO_CONFIG", 13, AG_GPS, _auto_config, 1),
    ...
    // @Range: 50 200
    // @User: Advanced
    AP_GROUPINFO("_RATE_MS", 14, AG_GPS, _rate_ms[0], 200),
    ...
    // @Range: 50 200
    // @User: Advanced
    AP_GROUPINFO("_RATE_MS2", 15, AG_GPS, _rate_ms[1], 200),
#endif
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("_POS1", 16, AG_GPS, _antenna_offset[0], 0.0f),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("_POS2", 17, AG_GPS, _antenna_offset[1], 0.0f),
#endif
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("_DELAY_MS", 18, AG_GPS, _delay_ms[0], 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("_DELAY_MS2", 19, AG_GPS, _delay_ms[1], 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("_BLEND_MASK", 20, AG_GPS, _blend_mask, 5),
    ...
    // @Range: 5.0 30.0
    // @User: Advanced
    AP_GROUPINFO("_BLEND_TC", 21, AG_GPS, _blend_tc, 10.0f),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("_DRV_OPTIONS", 22, AG_GPS, _driver_options, 0),

#if AG_GPS_SBF_ENABLED
    // @Param: _COM_PORT
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("_COM_PORT", 23, AG_GPS, _com_port[0], HAL_GPS_COM_PORT_DEFAULT),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("_COM_PORT2", 24, AG_GPS, _com_port[1], HAL_GPS_COM_PORT_DEFAULT),
#endif
#endif //AG_GPS_SBF_ENABLED

#if GPS_MOVING_BASELINE

    // @Group: _MB1_
    // @Path: MovingBase.cpp
    AP_SUBGROUPINFO(mb_params[0], "_MB1_", 25, AG_GPS, MovingBase),

#if GPS_MAX_RECEIVERS > 1
    // @Group: _MB2_
    // @Path: MovingBase.cpp
    AP_SUBGROUPINFO(mb_params[1], "_MB2_", 26, AG_GPS, MovingBase),
    ...
    // @User: Advanced
    AP_GROUPINFO("_PRIMARY", 27, AG_GPS, _primary, 0),
#endif
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("_CAN_NODEID1", 28, AG_GPS, _node_id[0], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("_CAN_NODEID2", 29, AG_GPS, _node_id[1], 0),
#endif // GPS_MAX_RECEIVERS > 1
    // @Param: 1_CAN_OVRIDE
    ...
    // @User: Advanced
    AP_GROUPINFO("1_CAN_OVRIDE", 30, AG_GPS, _override_node_id[0], 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("2_CAN_OVRIDE", 31, AG_GPS, _override_node_id[1], 0),
#endif // GPS_MAX_RECEIVERS > 1
...
AG_GPS::AG_GPS()
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_GPS must be singleton");
    }
    _singleton = this;
}

// return true if a specific type of GPS uses a UART
bool AG_GPS::needs_uart(GPS_Type type) const
{
    switch (type) {
    case GPS_TYPE_NONE:
    case GPS_TYPE_HIL:
...
void AG_GPS::init(const AG_SerialManager& serial_manager)
{
    ...
    for (uint8_t i=0; i<GPS_MAX_RECEIVERS; i++) {
        if (needs_uart((GPS_Type)_type[i].get())) {
            _port[i] = serial_manager.find_serial(AG_SerialManager::SerialProtocol_GPS, uart_idx);
            uart_idx++;
        }
    }
...
uint8_t AG_GPS::num_sensors(void) const
{
...
bool AG_GPS::speed_accuracy(uint8_t instance, float &sacc) const
{
...
bool AG_GPS::horizontal_accuracy(uint8_t instance, float &hacc) const
{
...
bool AG_GPS::vertical_accuracy(uint8_t instance, float &vacc) const
{
 ...
 */
uint64_t AG_GPS::istate_time_to_epoch_ms(uint16_t gps_week, uint32_t gps_ms)
{
 ...
 */
uint64_t AG_GPS::time_epoch_usec(uint8_t instance) const
{
    ...
    if (istate.last_corrected_gps_time_us != 0) {
        ...
        return (fix_time_ms*1000ULL) + (AG_HAL::micros64() - istate.last_corrected_gps_time_us);
    } else {
        fix_time_ms = istate_time_to_epoch_ms(istate.time_week, istate.time_week_ms);
        return (fix_time_ms + (AG_HAL::millis() - istate.last_gps_time_ms)) * 1000ULL;
    }
}
 ...
 */
uint64_t AG_GPS::last_message_epoch_usec(uint8_t instance) const
{
    const GPS_State &istate = state[instance];
 ...
 */
void AG_GPS::send_blob_start(uint8_t instance, const char *_blob, uint16_t size)
{
...
/*
  ...
  parameters.
 */
void AG_GPS::send_blob_start(uint8_t instance)
{
#if AG_GPS_UBLOX_ENABLED
...
#endif // AG_GPS_UBLOX_ENABLED

#if GPS_MOVING_BASELINE && AG_GPS_UBLOX_ENABLED
    ...
    case GPS_TYPE_SBF:
#endif //AG_GPS_SBF_ENABLED
#if AG_GPS_GSOF_ENABLED
    case GPS_TYPE_GSOF:
#endif //AG_GPS_GSOF_ENABLED
#if AG_GPS_NOVA_ENABLED
    case GPS_TYPE_NOVA:
#endif //AG_GPS_NOVA_ENABLED
#if HAL_SIM_GPS_ENABLED
    case GPS_TYPE_SITL:
...
/*
  ...
  UART transmit buffer
 */
void AG_GPS::send_blob_update(uint8_t instance)
{
 ...
 */
void AG_GPS::detect_instance(uint8_t instance)
{
    ...
    AG_GPS_Backend *new_gps = _detect_instance(instance);
    if (new_gps == nullptr) {
        return;
    }
 ...
 */
AG_GPS_Backend *AG_GPS::_detect_instance(uint8_t instance)
{
    ...
    case GPS_TYPE_MAV:
        ...
        return new AG_GPS_MAV(*this, state[instance], nullptr);
#endif //AG_GPS_MAV_ENABLED
...
#if HAL_ENABLE_LIBUAVCAN_DRIVERS
        dstate->auto_detected_baud = false; // specified, not detected
        return AG_GPS_UAVCAN::probe(*this, state[instance]);
#endif
        return nullptr; // We don't do anything here if UAVCAN is not supported
#if HAL_MSP_GPS_ENABLED
    case GPS_TYPE_MSP:
        dstate->auto_detected_baud = false; // specified, not detected
        return new AG_GPS_MSP(*this, state[instance], nullptr);
#endif
#if HAL_EXTERNAL_AHRS_ENABLED
    case GPS_TYPE_EXTERNAL_AHRS:
        dstate->auto_detected_baud = false; // specified, not detected
        return new AG_GPS_ExternalAHRS(*this, state[instance], nullptr);
#endif
    default:
        break;
    }
    ...
    if (now - dstate->last_baud_change_ms > GPS_BAUD_TIME_MS) {
        ...
        _port[instance]->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
    ...
    case GPS_TYPE_SBF:
        return new AG_GPS_SBF(*this, state[instance], _port[instance]);
#endif //AG_GPS_SBF_ENABLED
#if AG_GPS_GSOF_ENABLED
    case GPS_TYPE_GSOF:
        return new AG_GPS_GSOF(*this, state[instance], _port[instance]);
#endif //AG_GPS_GSOF_ENABLED
#if AG_GPS_NOVA_ENABLED
    case GPS_TYPE_NOVA:
        return new AG_GPS_NOVA(*this, state[instance], _port[instance]);
#endif //AG_GPS_NOVA_ENABLED

#if HAL_SIM_GPS_ENABLED
    case GPS_TYPE_SITL:
        return new AG_GPS_SITL(*this, state[instance], _port[instance]);
#endif  // HAL_SIM_GPS_ENABLED
    ...
    while (bytecount-- > 0) {
        ...
        if ((_type[instance] == GPS_TYPE_AUTO ||
            ...
            AG_GPS_UBLOX::_detect(dstate->ublox_detect_state, data)) {
            return new AG_GPS_UBLOX(*this, state[instance], _port[instance], GPS_ROLE_NORMAL);
        }
        ...
        if ((_type[instance] == GPS_TYPE_UBLOX_RTK_BASE ||
            ...
            return new AG_GPS_UBLOX(*this, state[instance], _port[instance], role);
        }
#endif  // AG_GPS_UBLOX_ENABLED
#if AG_GPS_SBP2_ENABLED
        if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SBP) &&
                 AG_GPS_SBP2::_detect(dstate->sbp2_detect_state, data)) {
            return new AG_GPS_SBP2(*this, state[instance], _port[instance]);
        }
#endif //AG_GPS_SBP2_ENABLED
#if AG_GPS_SBP_ENABLED
        if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SBP) &&
                 AG_GPS_SBP::_detect(dstate->sbp_detect_state, data)) {
            return new AG_GPS_SBP(*this, state[instance], _port[instance]);
        }
#endif //AG_GPS_SBP_ENABLED
#if AG_GPS_SIRF_ENABLED
        if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SIRF) &&
                 AG_GPS_SIRF::_detect(dstate->sirf_detect_state, data)) {
            return new AG_GPS_SIRF(*this, state[instance], _port[instance]);
        }
#endif
#if AG_GPS_ERB_ENABLED
        if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_ERB) &&
                 AG_GPS_ERB::_detect(dstate->erb_detect_state, data)) {
            return new AG_GPS_ERB(*this, state[instance], _port[instance]);
        }
#endif // AG_GPS_ERB_ENABLED
#if AG_GPS_NMEA_ENABLED
        if ((_type[instance] == GPS_TYPE_NMEA ||
                   ...
                   AG_GPS_NMEA::_detect(dstate->nmea_detect_state, data)) {
            return new AG_GPS_NMEA(*this, state[instance], _port[instance]);
        }
#endif //AG_GPS_NMEA_ENABLED
    }

    return nullptr;
}

AG_GPS::GPS_Status AG_GPS::highest_supported_status(uint8_t instance) const
{
    if (instance < GPS_MAX_RECEIVERS && drivers[instance] != nullptr) {
        return drivers[instance]->highest_supported_status();
    }
    return AG_GPS::GPS_OK_FIX_3D;
}

bool AG_GPS::should_log() const
{
 ...
 */
void AG_GPS::update_instance(uint8_t instance)
{
    ...
    if (state[instance].status >= GPS_OK_FIX_3D) {
        ...
        if (now != 0) {
            AP::rtc().set_utc_usec(now, AG_RTC::SOURCE_GPS);
        }
    }
#else
...
#if GPS_MOVING_BASELINE
void AG_GPS::get_RelPosHeading(uint32_t &timestamp, float &relPosHeading, float &relPosLength, float &relPosD, float &accHeading)
{
    for (uint8_t i=0; i< GPS_MAX_RECEIVERS; i++) {
        if (drivers[i] &&
            state[i].relposheading_ts != 0 &&
            AG_HAL::millis() - state[i].relposheading_ts < 500) {
...
bool AG_GPS::get_RTCMV3(const uint8_t *&bytes, uint16_t &len)
{
    for (uint8_t i=0; i< GPS_MAX_RECEIVERS; i++) {
...
*/
void AG_GPS::inject_MBL_data(uint8_t* data, uint16_t length)
{
    for (uint8_t i=0; i< GPS_MAX_RECEIVERS; i++) {
...
void AG_GPS::update(void)
{
    ...
    AG_Notify::flags.gps_status = state[primary_instance].status;
    AG_Notify::flags.gps_num_sats = state[primary_instance].num_sats;
#endif
}
 ...
 */
#if GPS_MAX_RECEIVERS > 1
void AG_GPS::update_primary(void)
{
#if defined(GPS_BLENDED_INSTANCE)
...
void AG_GPS::handle_gps_inject(const mavlink_message_t &msg)
{
 ...
 */
void AG_GPS::handle_msg(const mavlink_message_t &msg)
{
    switch (msg.msgid) {
...
#if HAL_MSP_GPS_ENABLED
void AG_GPS::handle_msp(const MSP::msp_gps_data_message_t &pkt)
{
...
#if HAL_EXTERNAL_AHRS_ENABLED
void AG_GPS::handle_external(const AG_ExternalAHRS::gps_data_message_t &pkt)
{
    for (uint8_t i=0; i<num_instances; i++) {
 ...
 */
void AG_GPS::lock_port(uint8_t instance, bool lock)
{
...
void AG_GPS::inject_data(const uint8_t *data, uint16_t len)
{
...
void AG_GPS::inject_data(uint8_t instance, const uint8_t *data, uint16_t len)
{
    if (instance < GPS_MAX_RECEIVERS && drivers[instance] != nullptr) {
 ...
 */
uint16_t AG_GPS::gps_yaw_cdeg(uint8_t instance) const
{
...
void AG_GPS::send_mavlink_gps_raw(mavlink_channel_t chan)
{
...
#if GPS_MAX_RECEIVERS > 1
void AG_GPS::send_mavlink_gps2_raw(mavlink_channel_t chan)
{
...
void AG_GPS::send_mavlink_gps_rtk(mavlink_channel_t chan, uint8_t inst)
{
...
bool AG_GPS::first_unconfigured_gps(uint8_t &instance) const
{
...
void AG_GPS::broadcast_first_configuration_failure_reason(void) const
{
...
bool AG_GPS::all_consistent(float &distance) const
{
...
bool AG_GPS::blend_health_check() const
{
 ...
 */
void AG_GPS::handle_gps_rtcm_fragment(uint8_t flags, const uint8_t *data, uint8_t len)
{
    if ((flags & 1) == 0) {
 ...
 */
void AG_GPS::handle_gps_rtcm_data(const mavlink_message_t &msg)
{
 ...
 */
bool AG_GPS::get_lag(uint8_t instance, float &lag_sec) const
{
...
const Vector3f &AG_GPS::get_antenna_offset(uint8_t instance) const
{
    if (instance >= GPS_MAX_INSTANCES) {
...
*/
uint16_t AG_GPS::get_rate_ms(uint8_t instance) const
{
    // sanity check
...
*/
bool AG_GPS::calc_blend_weights(void)
{
...
*/
void AG_GPS::calc_blended_state(void)
{
...
bool AG_GPS::is_healthy(uint8_t instance) const
{
    if (instance >= GPS_MAX_INSTANCES) {
        return false;
    }
...
bool AG_GPS::prepare_for_arming(void) {
...
bool AG_GPS::backends_healthy(char failure_msg[], uint16_t failure_msg_len) {
    for (uint8_t i = 0; i < GPS_MAX_RECEIVERS; i++) {
#if HAL_ENABLE_LIBUAVCAN_DRIVERS
        if (_type[i] == GPS_TYPE_UAVCAN ||
            _type[i] == GPS_TYPE_UAVCAN_RTK_BASE ||
            _type[i] == GPS_TYPE_UAVCAN_RTK_ROVER) {
            if (!AG_GPS_UAVCAN::backends_healthy(failure_msg, failure_msg_len)) {
                return false;
            }
        }
#endif
        if (_type[i] == GPS_TYPE_UBLOX_RTK_ROVER ||
            _type[i] == GPS_TYPE_UAVCAN_RTK_ROVER) {
            if (AG_HAL::millis() - state[i].gps_yaw_time_ms > 15000) {
...
bool AG_GPS::logging_failed(void) const {
    if (!logging_enabled()) {
        return false;
    }
...
uint32_t AG_GPS::get_itow(uint8_t instance) const
{
    if (instance >= GPS_MAX_RECEIVERS || drivers[instance] == nullptr) {
        return 0;
    }
...
bool AG_GPS::get_error_codes(uint8_t instance, uint32_t &error_codes) const
{
    if (instance >= GPS_MAX_RECEIVERS || drivers[instance] == nullptr) {
        return false;
    }
...
bool AG_GPS::get_undulation(uint8_t instance, float &undulation) const
{
...
// Logging support:
// Write an GPS packet
void AG_GPS::Write_GPS(uint8_t i)
{
    const uint64_t time_us = AG_HAL::micros64();
...
/*
  get GPS based yaw
 */
bool AG_GPS::gps_yaw_deg(uint8_t instance, float &yaw_deg, float &accuracy_deg, uint32_t &time_ms) const
{
#if GPS_MAX_RECEIVERS > 1
    if (instance < GPS_MAX_RECEIVERS &&
