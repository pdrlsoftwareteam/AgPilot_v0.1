...
AG_GPS_UBLOX::AG_GPS_UBLOX(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_HAL::UARTDriver *_port, AG_GPS::GPS_Role _role) :
    AG_GPS_Backend(_gps, _state, _port),
    _next_message(STEP_PVT),
    _ublox_port(255),
    _unconfigured_messages(CONFIG_ALL),
    _hardware_generation(UBLOX_UNKNOWN_HARDWARE_GENERATION),
    next_fix(AG_GPS::NO_FIX),
    noReceivedHdop(true),
    role(_role)
{
    ...
    if (role == AG_GPS::GPS_ROLE_MB_BASE && !mb_use_uart2()) {
    ...
    if (role == AG_GPS::GPS_ROLE_MB_ROVER) {
        _unconfigured_messages |= CONFIG_RTK_MOVBASE;
 ...
 */
const AG_GPS_UBLOX::config_list AG_GPS_UBLOX::config_MB_Base_uart1[] {
...
const AG_GPS_UBLOX::config_list AG_GPS_UBLOX::config_MB_Base_uart2[] {
 ...
 */
const AG_GPS_UBLOX::config_list AG_GPS_UBLOX::config_MB_Rover_uart1[] {
 { ConfigKey::CFG_UART2OUTPROT_RTCM3X, 0},
 { ConfigKey::CFG_UART1INPROT_RTCM3X, 1},
...
const AG_GPS_UBLOX::config_list AG_GPS_UBLOX::config_MB_Rover_uart2[] {
 { ConfigKey::CFG_UART2_ENABLED, 1},
 { ConfigKey::CFG_UART2_BAUDRATE, 460800},
 { ConfigKey::CFG_UART2OUTPROT_RTCM3X, 0},
 ...
 */
const AG_GPS_UBLOX::config_list AG_GPS_UBLOX::config_M10[] {
...
void
AG_GPS_UBLOX::_request_next_config(void)
{
    // don't request config if we shouldn't configure the GPS
    if (gps._auto_config == AG_GPS::GPS_AUTO_CONFIG_DISABLE) {
        return;
    }
    ...
    // check AG_GPS_UBLOX.h for the enum that controls the order.
    ...
    case STEP_POLL_SVINFO:
        ...
        break;
    case STEP_POLL_SBAS:
        if (gps._sbas_mode != AG_GPS::SBAS_Mode::DoNotChange) {
...
void
AG_GPS_UBLOX::_verify_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate) {
...
void
AG_GPS_UBLOX::_request_port(void)
{
...
//
bool
AG_GPS_UBLOX::read(void)
{
    bool parsed = false;
    uint32_t millis_now = AG_HAL::millis();
...
#if AG_GPS_DEBUG_LOGGING_ENABLED
        log_data(&data, 1);
#endif
...
// Private Methods /////////////////////////////////////////////////////////////
void AG_GPS_UBLOX::log_mon_hw(void)
{
...
void AG_GPS_UBLOX::log_mon_hw2(void)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
#if UBLOX_TIM_TM2_LOGGING
void AG_GPS_UBLOX::log_tim_tm2(void)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
#if UBLOX_RXM_RAW_LOGGING
void AG_GPS_UBLOX::log_rxm_raw(const struct ubx_rxm_raw &raw)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
void AG_GPS_UBLOX::log_rxm_rawx(const struct ubx_rxm_rawx &raw)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
void AG_GPS_UBLOX::unexpected_message(void)
{
...
uint8_t AG_GPS_UBLOX::config_key_size(ConfigKey key) const
{
    // step over the value
 ...
 */
int8_t AG_GPS_UBLOX::find_active_config_index(ConfigKey key) const
{
...
//      }
    ...
    if (_class == CLASS_CFG) {
	    ...
	    Debug("Got settings %u min_elev %d drLimit %u\n", 
            ...
            if (gps._navfilter != AG_GPS::GPS_ENGINE_NONE &&
...
#ifdef HAL_GPIO_PPS
            hal.gpio->attach_interrupt(HAL_GPIO_PPS, FUNCTOR_BIND_MEMBER(&AG_GPS_UBLOX::pps_interrupt, void, uint8_t, bool, uint32_t), AG_HAL::GPIO::INTERRUPT_FALLING);
#endif
    ...
    case MSG_STATUS:
        ...
        if (_buffer.status.fix_status & NAV_STATUS_FIX_VALID) {
            if( (_buffer.status.fix_type == AG_GPS_UBLOX::FIX_3D) &&
                (_buffer.status.fix_status & AG_GPS_UBLOX::NAV_STATUS_DGPS_USED)) {
                next_fix = AG_GPS::GPS_OK_FIX_3D_DGPS;
            }else if( _buffer.status.fix_type == AG_GPS_UBLOX::FIX_3D) {
                next_fix = AG_GPS::GPS_OK_FIX_3D;
            }else if (_buffer.status.fix_type == AG_GPS_UBLOX::FIX_2D) {
                next_fix = AG_GPS::GPS_OK_FIX_2D;
            }else{
                next_fix = AG_GPS::NO_FIX;
                state.status = AG_GPS::NO_FIX;
            }
        }else{
            next_fix = AG_GPS::NO_FIX;
            state.status = AG_GPS::NO_FIX;
...
#if UBLOX_FAKE_3DLOCK
        state.status = AG_GPS::GPS_OK_FIX_3D;
    ...
    case MSG_SOL:
        ...
        if (_buffer.solution.fix_status & NAV_STATUS_FIX_VALID) {
            if( (_buffer.solution.fix_type == AG_GPS_UBLOX::FIX_3D) &&
                (_buffer.solution.fix_status & AG_GPS_UBLOX::NAV_STATUS_DGPS_USED)) {
                next_fix = AG_GPS::GPS_OK_FIX_3D_DGPS;
            }else if( _buffer.solution.fix_type == AG_GPS_UBLOX::FIX_3D) {
                next_fix = AG_GPS::GPS_OK_FIX_3D;
            }else if (_buffer.solution.fix_type == AG_GPS_UBLOX::FIX_2D) {
                next_fix = AG_GPS::GPS_OK_FIX_2D;
            }else{
                next_fix = AG_GPS::NO_FIX;
                state.status = AG_GPS::NO_FIX;
        ...
        if (next_fix >= AG_GPS::GPS_OK_FIX_2D) {
            state.last_gps_time_ms = AG_HAL::millis();
        ...
        state.time_week_ms = AG_HAL::millis() + 3*60*60*1000 + 37000;
        state.last_gps_time_ms = AG_HAL::millis();
        state.hdop = 130;
#endif
        break;
    ...
    case MSG_RELPOSNED:
        {
            ...
            if (((_buffer.relposned.flags & valid_mask) == valid_mask) &&
                ...
                if (calculate_moving_base_yaw(_buffer.relposned.relPosHeading * 1e-5,
                    ...
                    _last_relposned_ms = AG_HAL::millis();
                }
                ...
                state.relposheading_ts = AG_HAL::millis();
            } else {
    ...
    case MSG_PVT:
        ...
        switch (_buffer.pvt.fix_type)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                state.status = AG_GPS::GPS_OK_FIX_2D;
                break;
            case 3:
                state.status = AG_GPS::GPS_OK_FIX_3D;
                if (_buffer.pvt.flags & 0b00000010)  // diffsoln
                    state.status = AG_GPS::GPS_OK_FIX_3D_DGPS;
                if (_buffer.pvt.flags & 0b01000000)  // carrsoln - float
                    state.status = AG_GPS::GPS_OK_FIX_3D_RTK_FLOAT;
                if (_buffer.pvt.flags & 0b10000000)  // carrsoln - fixed
                    state.status = AG_GPS::GPS_OK_FIX_3D_RTK_FIXED;
                break;
            case 4:
                GCS_SEND_TEXT(MAV_SEVERITY_INFO,
    ...
    if (state.have_gps_yaw) {
        ...
        // consistent view
        if (AG_HAL::millis() - _last_relposned_ms > 400) {
...
#ifdef HAL_GPIO_PPS
void
AG_GPS_UBLOX::pps_interrupt(uint8_t pin, bool high, uint32_t timestamp_us)
{
    _last_pps_time_us = timestamp_us;
}

void
AG_GPS_UBLOX::set_pps_desired_freq(uint8_t freq)
{
    _pps_freq = freq;
    _unconfigured_messages |= CONFIG_TP5;
}
 ...
 */
void
AG_GPS_UBLOX::_update_checksum(uint8_t *data, uint16_t len, uint8_t &ck_a, uint8_t &ck_b)
{
    while (len--) {
        ck_a += *data;
        ck_b += ck_a;
 ...
 */
bool
AG_GPS_UBLOX::_send_message(uint8_t msg_class, uint8_t msg_id, const void *msg, uint16_t size)
{
 ...
 */
bool
AG_GPS_UBLOX::_request_message_rate(uint8_t msg_class, uint8_t msg_id)
{
 ...
 */
bool
AG_GPS_UBLOX::_configure_message_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate)
{
 ...
 */
bool
AG_GPS_UBLOX::_configure_valset(ConfigKey key, const void *value, uint8_t layers)
{
    if (!supports_F9_config()) {
        return false;
    }
 ...
 */
bool
AG_GPS_UBLOX::_configure_valget(ConfigKey key)
{
    if (!supports_F9_config()) {
        return false;
    }
 ...
 */
bool
AG_GPS_UBLOX::_configure_config_set(const config_list *list, uint8_t count, uint32_t unconfig_bit, uint8_t layers)
{
...
AG_GPS_UBLOX::_save_cfg()
{
    ...
    _last_cfg_sent_time = AG_HAL::millis();
    _num_cfg_save_tries++;
    GCS_SEND_TEXT(MAV_SEVERITY_INFO,
 ...
 */
bool
AG_GPS_UBLOX::_detect(struct UBLOX_detect_state &state, uint8_t data)
{
reset:
	switch (state.step) {
        case 1:
...
void
AG_GPS_UBLOX::_request_version(void)
{
    _send_message(CLASS_MON, MSG_MON_VER, nullptr, 0);
}

void
AG_GPS_UBLOX::_request_uniqid(void)
{
...
void
AG_GPS_UBLOX::_configure_rate(void)
{
...
void
AG_GPS_UBLOX::broadcast_configuration_failure_reason(void) const {
 ...
 */
bool AG_GPS_UBLOX::get_lag(float &lag_sec) const
{
    switch (_hardware_generation) {
    case UBLOX_UNKNOWN_HARDWARE_GENERATION:
        ...
        return gps._auto_config == AG_GPS::GPS_AUTO_CONFIG_DISABLE;
    case UBLOX_5:
    case UBLOX_6:
    default:
        lag_sec = 0.22f;
...
void AG_GPS_UBLOX::Write_AG_Logger_Log_Startup_messages() const
{
#if HAL_LOGGING_ENABLED
    AG_GPS_Backend::Write_AG_Logger_Log_Startup_messages();
...
void AG_GPS_UBLOX::_check_new_itow(uint32_t itow)
{
    check_new_itow(itow, _payload_length + sizeof(ubx_header) + 2);
}

// support for retrieving RTCMv3 data from a moving baseline base
bool AG_GPS_UBLOX::get_RTCMV3(const uint8_t *&bytes, uint16_t &len)
{
#if GPS_MOVING_BASELINE
    if (rtcm3_parser) {
...
void AG_GPS_UBLOX::clear_RTCMV3(void)
{
#if GPS_MOVING_BASELINE
    if (rtcm3_parser) {
...
bool AG_GPS_UBLOX::is_healthy(void) const
{
    ...
    if (role == AG_GPS::GPS_ROLE_MB_BASE && rtcm3_parser == nullptr && !mb_use_uart2()) {
...
bool AG_GPS_UBLOX::supports_F9_config(void) const
{
