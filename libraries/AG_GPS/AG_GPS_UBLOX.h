...
class AG_GPS_UBLOX : public AG_GPS_Backend
{
public:
    AG_GPS_UBLOX(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_HAL::UARTDriver *_port, AG_GPS::GPS_Role role);
    ~AG_GPS_UBLOX() override;

    // Methods
    bool read() override;

    AG_GPS::GPS_Status highest_supported_status(void) override { return AG_GPS::GPS_OK_FIX_3D_RTK_FIXED; }
    ...
    void Write_AG_Logger_Log_Startup_messages() const override;
...
private:
    ...
    bool mb_use_uart2(void) const {
        return option_set(AG_GPS::DriverOptions::UBX_MBUseUart2)?true:false;
    }
#endif
