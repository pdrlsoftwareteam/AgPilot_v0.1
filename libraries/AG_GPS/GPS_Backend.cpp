...
AG_GPS_Backend::AG_GPS_Backend(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_HAL::UARTDriver *_port) :
    port(_port),
    gps(_gps),
    state(_state)
{
...
int32_t AG_GPS_Backend::swap_int32(int32_t v) const
{
...
int16_t AG_GPS_Backend::swap_int16(int16_t v) const
{
    const uint8_t *b = (const uint8_t *)&v;
    union {
        int16_t v;
 ...
 */
void AG_GPS_Backend::make_gps_time(uint32_t bcd_date, uint32_t bcd_milliseconds)
{
    struct tm tm {};
 ...
 */
uint32_t AG_GPS_Backend::get_last_itow_ms(void) const
{
    if (!_have_itow) {
 ...
 */
void AG_GPS_Backend::fill_3d_velocity(void)
{
...
/*
  fill in 3D velocity for a GPS that doesn't give vertical velocity numbers
 */
void AG_GPS_Backend::velocity_to_speed_course(AG_GPS::GPS_State &s)
{
...
void
AG_GPS_Backend::inject_data(const uint8_t *data, uint16_t len)
{
...
void AG_GPS_Backend::_detection_message(char *buffer, const uint8_t buflen) const
{
    const uint8_t instance = state.instance;
    const struct AG_GPS::detect_state dstate = gps.detect_state[instance];
...
void AG_GPS_Backend::broadcast_gps_type() const
{
...
void AG_GPS_Backend::Write_AG_Logger_Log_Startup_messages() const
{
...
bool AG_GPS_Backend::should_log() const
{
    return gps.should_log();
}


void AG_GPS_Backend::send_mavlink_gps_rtk(mavlink_channel_t chan)
{
#if HAL_GCS_ENABLED
    const uint8_t instance = state.instance;
    // send status
...
*/
void AG_GPS_Backend::set_uart_timestamp(uint16_t nbytes)
{
    if (port) {
...
void AG_GPS_Backend::check_new_itow(uint32_t itow, uint32_t msg_length)
{
    if (itow != _last_itow_ms) {
        ...
        uint64_t uart_us;
        if (_last_pps_time_us != 0 && (state.status >= AG_GPS::GPS_OK_FIX_2D)) {
        ...
        if (state.status >= AG_GPS::GPS_OK_FIX_2D) {
...
bool AG_GPS_Backend::calculate_moving_base_yaw(float reported_heading_deg, const float reported_distance, const float reported_D) {
    return calculate_moving_base_yaw(state, reported_heading_deg, reported_distance, reported_D);
}

bool AG_GPS_Backend::calculate_moving_base_yaw(AG_GPS::GPS_State &interim_state, const float reported_heading_deg, const float reported_distance, const float reported_D) {
 ...
 */
void AG_GPS_Backend::set_alt_amsl_cm(AG_GPS::GPS_State &_state, int32_t alt_amsl_cm)
{
    if (option_set(AG_GPS::HeightEllipsoid) && _state.have_undulation) {
...
*/
void AG_GPS_Backend::log_data(const uint8_t *data, uint16_t length)
{
    if (state.instance < 2) {
    ...
    if (!log_thread_created) {
        ...
        hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_GPS_Backend::logging_start, void), "gps_log", 4096, AG_HAL::Scheduler::PRIORITY_IO, 0);
    }
}

AG_GPS_Backend::loginfo AG_GPS_Backend::logging[2];
bool AG_GPS_Backend::log_thread_created;

// logging loop, needs to be static to allow for re-alloc of GPS backends
void AG_GPS_Backend::logging_loop(void)
{
    while (true) {
...
void AG_GPS_Backend::logging_start(void)
{
    logging_loop();
}
#endif // AG_GPS_DEBUG_LOGGING_ENABLED
