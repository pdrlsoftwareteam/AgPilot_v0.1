...
constexpr const char *AG_GPS_SBF::portIdentifiers[];
constexpr const char* AG_GPS_SBF::_initialisation_blob[];
constexpr const char* AG_GPS_SBF::sbas_on_blob[];

AG_GPS_SBF::AG_GPS_SBF(AG_GPS &_gps, AG_GPS::GPS_State &_state,
                       AG_HAL::UARTDriver *_port) :
    AG_GPS_Backend(_gps, _state, _port)
{
    sbf_msg.sbf_state = sbf_msg_parser_t::PREAMBLE1;

    _config_last_ack_time = AG_HAL::millis();
    ...
    if (option_set(AG_GPS::DriverOptions::SBF_UseBaseForYaw)) {
        state.gps_yaw_configured = true;
    }
}

AG_GPS_SBF::~AG_GPS_SBF (void) {
    free(config_string);
}
                        ...
                        switch (config_step) {
                            case Config_State::Baud_Rate:
                                if (asprintf(&config_string, "scs,COM%d,baud%d,bits8,No,bit1,%s\n",
                                             ...
                                             230400,
                                             port->get_flow_control() != AG_HAL::UARTDriver::flow_control::FLOW_CONTROL_ENABLE ? "none" : "RTS|CTS") == -1) {
                            ...
                            case Config_State::SBAS:
                                switch ((AG_GPS::SBAS_Mode)gps._sbas_mode) {
                                    case AG_GPS::SBAS_Mode::Disabled:
                                        if (asprintf(&config_string, "%s\n", sbas_off) == -1) {
                                            config_string = nullptr;
                                        }
                                        break;
                                    case AG_GPS::SBAS_Mode::Enabled:
                                        if (asprintf(&config_string, "%s\n", sbas_on_blob[_init_blob_index]) == -1) {
                                            config_string = nullptr;
                                        }
                                        break;
                                    case AG_GPS::SBAS_Mode::DoNotChange:
                                        config_string = nullptr;
...
bool AG_GPS_SBF::logging_healthy(void) const
{
                    ...
                    if (config_string != nullptr) {
                        if (!strncmp(config_string, (char *)(sbf_msg.data.bytes + 2),
                            ...
                            switch (config_step) {
                                ...
                                case Config_State::SBAS:
                                    _init_blob_index++;
                                    if ((gps._sbas_mode == AG_GPS::SBAS_Mode::Disabled)
...
bool
AG_GPS_SBF::process_message(void)
{
...
void AG_GPS_SBF::broadcast_configuration_failure_reason(void) const
{
    if (gps._auto_config != AG_GPS::GPS_AUTO_CONFIG_DISABLE &&
        config_step != Config_State::Complete) {
