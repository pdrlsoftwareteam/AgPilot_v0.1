...
#include "AG_GPS.h"
#include "AG_GPS_config.h"

#ifndef AG_GPS_DEBUG_LOGGING_ENABLED
// enable this to log all bytes from the GPS. Also needs a call to
// log_data() in each backend
#define AG_GPS_DEBUG_LOGGING_ENABLED 0
#endif
...
class AG_GPS_Backend
{
public:
    AG_GPS_Backend(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_HAL::UARTDriver *_port);

    // we declare a virtual destructor so that GPS drivers can
    // override with a custom destructor if need be.
    virtual ~AG_GPS_Backend(void) {}
    ...
    virtual AG_GPS::GPS_Status highest_supported_status(void) { return AG_GPS::GPS_OK_FIX_3D; }
    ...
    virtual void handle_external(const AG_ExternalAHRS::gps_data_message_t &pkt) { return; }
#endif
    ...
    virtual void Write_AG_Logger_Log_Startup_messages() const;
    ...
    bool option_set(const AG_GPS::DriverOptions option) const {
        return gps.option_set(option);
    }

protected:
    AG_HAL::UARTDriver *port;           ///< UART we are attached to
    AG_GPS &gps;                        ///< access to frontend (for parameters)
    AG_GPS::GPS_State &state;           ///< public state for this instance
     ...
     */
    void velocity_to_speed_course(AG_GPS::GPS_State &s);
    ...
    */
    ...
    bool calculate_moving_base_yaw(AG_GPS::GPS_State &interim_state, const float reported_heading_deg, const float reported_distance, const float reported_D);
#endif //GPS_MOVING_BASELINE

    // get GPS type, for subtype config
    AG_GPS::GPS_Type get_type() const {
    ...
    void set_alt_amsl_cm(AG_GPS::GPS_State &_state, int32_t alt_amsl_cm);
