...
#ifndef AG_GPS_NMEA_CONFIG_PERIOD_MS
// how often we send board specific config commands
#define AG_GPS_NMEA_CONFIG_PERIOD_MS 15000U
#endif
 ...
 */
bool AG_GPS_NMEA::_decode(char c)
{
    _sentence_length++;
...
int32_t AG_GPS_NMEA::_parse_decimal_100(const char *p)
{
    char *endptr = nullptr;
 ...
 */
uint32_t AG_GPS_NMEA::_parse_degrees()
{
    char *p, *q;
    uint8_t deg = 0, min = 0;
    float frac_min = 0;
 ...
 */
bool AG_GPS_NMEA::_have_new_message()
{
    if (_last_RMC_ms == 0 ||
        _last_GGA_ms == 0) {
        return false;
...
#endif // AG_GPS_NMEA_UNICORE_ENABLED
...
bool AG_GPS_NMEA::_term_complete()
{
    ...
    if (_is_checksum_term) {
        ...
        if (crc_ok) {
            ...
            case _GPS_SENTENCE_RMC:
                ...
                if (state.status >= AG_GPS::GPS_OK_FIX_3D) {
 ...
 */
void AG_GPS_NMEA::parse_agrica_field(uint16_t term_number, const char *term)
{
    auto &ag = _agrica;
 ...
 */
void AG_GPS_NMEA::parse_uniheadinga_field(uint16_t term_number, const char *term)
{
    const uint8_t hdr_align = 8;
    if (term_number < hdr_align) {
        // discard header;
...
void AG_GPS_NMEA::parse_versiona_field(uint16_t term_number, const char *term)
{
 ...
 */
bool
AG_GPS_NMEA::_detect(struct NMEA_detect_state &state, uint8_t data)
{
	switch (state.step) {
	case 0:
		state.ck = 0;
 ...
 */
void AG_GPS_NMEA::send_config(void)
{
    const auto type = get_type();
    _expect_agrica = (type == AG_GPS::GPS_TYPE_UNICORE_NMEA ||
                      type == AG_GPS::GPS_TYPE_UNICORE_MOVINGBASE_NMEA);
    if (gps._auto_config == AG_GPS::GPS_AUTO_CONFIG_DISABLE) {
        // not doing auto-config
        return;
    }
    uint32_t now_ms = AG_HAL::millis();
    if (now_ms - last_config_ms < AG_GPS_NMEA_CONFIG_PERIOD_MS) {
        return;
    }
    ...
    switch (get_type()) {
#if AG_GPS_NMEA_UNICORE_ENABLED
    case AG_GPS::GPS_TYPE_UNICORE_MOVINGBASE_NMEA:
    ...
    case AG_GPS::GPS_TYPE_UNICORE_NMEA: {
        port->printf("\r\nAGRICA %.3f\r\n" \
                     "MODE MOVINGBASE\r\n" \
                     "GNGGA %.3f\r\n" \
                     "GNRMC %.3f\r\n",
...
#ifdef AG_GPS_NMEA_CUSTOM_CONFIG_STRING
    // allow for custom config strings, useful for peripherals
    port->printf("%s\r\n", AG_GPS_NMEA_CUSTOM_CONFIG_STRING);
#endif
}
 ...
 */
bool AG_GPS_NMEA::is_healthy(void) const
{
    switch (get_type()) {
#if AG_GPS_NMEA_UNICORE_ENABLED
    case AG_GPS::GPS_TYPE_UNICORE_MOVINGBASE_NMEA:
...
bool AG_GPS_NMEA::get_lag(float &lag_sec) const
{
    switch (get_type()) {
#if AG_GPS_NMEA_UNICORE_ENABLED
    case AG_GPS::GPS_TYPE_UNICORE_MOVINGBASE_NMEA:
...
void AG_GPS_NMEA::Write_AG_Logger_Log_Startup_messages() const
{
