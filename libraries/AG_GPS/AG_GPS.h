...
class AG_GPS
{
    friend class AG_GPS_ERB;
    friend class AG_GPS_GSOF;
    friend class AG_GPS_MAV;
    friend class AG_GPS_MSP;
    friend class AG_GPS_ExternalAHRS;
    friend class AG_GPS_NMEA;
    friend class AG_GPS_NOVA;
    friend class AG_GPS_PX4;
    friend class AG_GPS_SBF;
    friend class AG_GPS_SBP;
    friend class AG_GPS_SBP2;
    friend class AG_GPS_SIRF;
    friend class AG_GPS_UBLOX;
    friend class AG_GPS_Backend;
    friend class AG_GPS_UAVCAN;

public:
    AG_GPS();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_GPS);

    static AG_GPS *get_singleton() {
        return _singleton;
    }
    ...
    void init(const class AG_SerialManager& serial_manager);
    ...
    void handle_external(const AG_ExternalAHRS::gps_data_message_t &pkt);
#endif
    ...
    char status_onechar(void) const {
        ...
        case AG_GPS::NO_FIX:
            return '-';
        case AG_GPS::GPS_OK_FIX_2D:
            return '2';
        case AG_GPS::GPS_OK_FIX_3D:
            return '3';
        case AG_GPS::GPS_OK_FIX_3D_DGPS:
            return '4';
        case AG_GPS::GPS_OK_FIX_3D_RTK_FLOAT:
            return '5';
        case AG_GPS::GPS_OK_FIX_3D_RTK_FIXED:
            return '6';
        }
    ...
    void Write_AG_Logger_Log_Startup_messages();
...
private:
    ...
    AG_GPS_Backend *drivers[GPS_MAX_RECEIVERS];
    AG_HAL::UARTDriver *_port[GPS_MAX_RECEIVERS];
    ...
    AG_GPS_Backend *_detect_instance(uint8_t instance);
