...
bool AG_GPS_ExternalAHRS::read(void)
{
    if (new_data) {
        new_data = false;
        return true;
...
void AG_GPS_ExternalAHRS::handle_external(const AG_ExternalAHRS::gps_data_message_t &pkt)
{
    ...
    if (pkt.fix_type == 0) {
        ...
        state.status = (AG_GPS::GPS_Status)pkt.fix_type;
    }
 ...
 */
bool AG_GPS_ExternalAHRS::get_lag(float &lag_sec) const
{
    // fixed assumed lag
    lag_sec = 0.11;
    return true;
