...
AG_GPS_GSOF::AG_GPS_GSOF(AG_GPS &_gps, AG_GPS::GPS_State &_state,
...
bool
AG_GPS_GSOF::parse(uint8_t temp)
{
...
void
AG_GPS_GSOF::requestBaud(uint8_t portindex)
{
...
void
AG_GPS_GSOF::requestGSOF(uint8_t messagetype, uint8_t portindex)
{
...
double
AG_GPS_GSOF::SwapDouble(uint8_t* src, uint32_t pos)
{
    union {
        double d;
...
float
AG_GPS_GSOF::SwapFloat(uint8_t* src, uint32_t pos)
{
    union {
        float f;
...
uint32_t
AG_GPS_GSOF::SwapUint32(uint8_t* src, uint32_t pos)
{
    union {
        uint32_t u;
...
uint16_t
AG_GPS_GSOF::SwapUint16(uint8_t* src, uint32_t pos)
{
    union {
        uint16_t u;
...
bool
AG_GPS_GSOF::process_message(void)
{
