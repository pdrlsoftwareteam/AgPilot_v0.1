...
AG_GPS_SBP2::AG_GPS_SBP2(AG_GPS &_gps, AG_GPS::GPS_State &_state,
...
void
AG_GPS_SBP2::inject_data(const uint8_t *data, uint16_t len)
{
    if (port->txspace() > len) {
        last_injected_data_ms = AG_HAL::millis();
        port->write(data, len);
    } else {
...
void
AG_GPS_SBP2::_sbp_process_message() {
    ...
    switch (parser_state.msg_type) {
        case SBP_HEARTBEAT_MSGTYPE:
            ...
            last_heartbeat_received_ms = AG_HAL::millis();
            break;
...
int32_t 
AG_GPS_SBP2::distMod(int32_t tow1_ms, int32_t tow2_ms, int32_t mod) {
    return MIN(abs(tow1_ms - tow2_ms), mod - abs(tow1_ms - tow2_ms));
}

bool
AG_GPS_SBP2::_attempt_state_update()
{
...
bool
AG_GPS_SBP2::_detect(struct SBP2_detect_state &state, uint8_t data)
{
...
#if HAL_LOGGING_ENABLED
void
AG_GPS_SBP2::logging_log_full_update()
{
    if (!should_log()) {
      return;
    }
...
void
AG_GPS_SBP2::logging_log_raw_sbp(uint16_t msg_type,
        uint16_t sender_id,
        uint8_t msg_len,
        uint8_t *msg_buff) {
    if (!should_log()) {
    ...
    uint64_t time_us = AG_HAL::micros64();
    uint8_t pages = 1;
