...
AG_GPS_ERB::_parse_gps(void)
{
    ...
    case MSG_STAT:
        ...
        if (_buffer.stat.fix_status & STAT_FIX_VALID) {
            if (_buffer.stat.fix_type == AG_GPS_ERB::FIX_FIX) {
                next_fix = AG_GPS::GPS_OK_FIX_3D_RTK_FIXED;
            } else if (_buffer.stat.fix_type == AG_GPS_ERB::FIX_FLOAT) {
                next_fix = AG_GPS::GPS_OK_FIX_3D_RTK_FLOAT;
            } else if (_buffer.stat.fix_type == AG_GPS_ERB::FIX_SINGLE) {
        ...
        if (next_fix >= AG_GPS::GPS_OK_FIX_3D) {
 ...
 */
bool
AG_GPS_ERB::_detect(struct ERB_detect_state &state, uint8_t data)
{
reset:
    switch (state.step) {
        case 1:
