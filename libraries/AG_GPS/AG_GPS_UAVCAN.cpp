...
#define NATIVE_TIME_OFFSET (AG_HAL::micros64() - AG_HAL::native_micros64())
#else
#define NATIVE_TIME_OFFSET 0
#endif
AG_GPS_UAVCAN::DetectedModules AG_GPS_UAVCAN::_detected_modules[];
HAL_Semaphore AG_GPS_UAVCAN::_sem_registry;

// Member Methods
AG_GPS_UAVCAN::AG_GPS_UAVCAN(AG_GPS &_gps, AG_GPS::GPS_State &_state, AG_GPS::GPS_Role _role) :
    AG_GPS_Backend(_gps, _state, nullptr),
    interim_state(_state),
    role(_role)
{
    param_int_cb = FUNCTOR_BIND_MEMBER(&AG_GPS_UAVCAN::handle_param_get_set_response_int, bool, AG_UAVCAN*, const uint8_t, const char*, int32_t &);
    param_float_cb = FUNCTOR_BIND_MEMBER(&AG_GPS_UAVCAN::handle_param_get_set_response_float, bool, AG_UAVCAN*, const uint8_t, const char*, float &);
    param_save_cb = FUNCTOR_BIND_MEMBER(&AG_GPS_UAVCAN::handle_param_save_response, void, AG_UAVCAN*, const uint8_t, bool);
}

AG_GPS_UAVCAN::~AG_GPS_UAVCAN()
{
...
void AG_GPS_UAVCAN::subscribe_msgs(AG_UAVCAN* ap_uavcan)
{
    ...
    if (gnss_aux == nullptr) {
        AG_BoardConfig::allocation_error("gnss_aux");
    }
    ...
    if (gnss_heading == nullptr) {
        AG_BoardConfig::allocation_error("gnss_heading");
    }
    ...
    if (gnss_status == nullptr) {
        AG_BoardConfig::allocation_error("gnss_status");
    }
    ...
    if (gnss_moving_baseline == nullptr) {
        AG_BoardConfig::allocation_error("gnss_moving_baseline");
    }
    ...
    if (gnss_relposheading == nullptr) {
        AG_BoardConfig::allocation_error("gnss_relposheading");
    }
...
AG_GPS_Backend* AG_GPS_UAVCAN::probe(AG_GPS &_gps, AG_GPS::GPS_State &_state)
{
    WITH_SEMAPHORE(_sem_registry);
    int8_t found_match = -1, last_match = -1;
    AG_GPS_UAVCAN* backend = nullptr;
    ...
    switch (_gps.get_type(_state.instance)) {
        case AG_GPS::GPS_TYPE_UAVCAN:
            backend = new AG_GPS_UAVCAN(_gps, _state, AG_GPS::GPS_ROLE_NORMAL);
            break;
#if GPS_MOVING_BASELINE
        case AG_GPS::GPS_TYPE_UAVCAN_RTK_BASE:
            backend = new AG_GPS_UAVCAN(_gps, _state, AG_GPS::GPS_ROLE_MB_BASE);
            break;
        case AG_GPS::GPS_TYPE_UAVCAN_RTK_ROVER:
            backend = new AG_GPS_UAVCAN(_gps, _state, AG_GPS::GPS_ROLE_MB_ROVER);
            break;
#endif
        default:
            return NULL;
...
#if GPS_MOVING_BASELINE
        if (backend->role == AG_GPS::GPS_ROLE_MB_BASE) {
...
bool AG_GPS_UAVCAN::backends_healthy(char failure_msg[], uint16_t failure_msg_len)
{
    for (uint8_t i = 0; i < GPS_MAX_RECEIVERS; i++) {
...
AG_GPS_UAVCAN* AG_GPS_UAVCAN::get_uavcan_backend(AG_UAVCAN* ap_uavcan, uint8_t node_id)
{
    if (ap_uavcan == nullptr) {
        return nullptr;
    }
 ...
 */
void AG_GPS_UAVCAN::handle_velocity(const float vx, const float vy, const float vz)
{
    if (!uavcan::isNaN(vx)) {
...
void AG_GPS_UAVCAN::handle_fix2_msg(const Fix2Cb &cb)
{
    bool process = false;
    seen_fix2 = true;
    ...
    if (cb.msg->status == uavcan::equipment::gnss::Fix2::STATUS_NO_FIX) {
        interim_state.status = AG_GPS::GPS_Status::NO_FIX;
    } else {
        if (cb.msg->status == uavcan::equipment::gnss::Fix2::STATUS_TIME_ONLY) {
            ...
            interim_state.status = AG_GPS::GPS_Status::GPS_OK_FIX_2D;
            process = true;
        } else if (cb.msg->status == uavcan::equipment::gnss::Fix2::STATUS_3D_FIX) {
            interim_state.status = AG_GPS::GPS_Status::GPS_OK_FIX_3D;
            process = true;
        }
    ...
    if (next_toggle != last_toggle) {
        chVTSet(&timeout_vt, chTimeUS2I(next_toggle - AG_HAL::micros64()), handle_timeout, nullptr);
    ...
    _new_data = true;
    if (!seen_message) {
        if (interim_state.status == AG_GPS::GPS_Status::NO_GPS) {
...
void AG_GPS_UAVCAN::handle_aux_msg(const AuxCb &cb)
{
    WITH_SEMAPHORE(sem);
...
void AG_GPS_UAVCAN::handle_heading_msg(const HeadingCb &cb)
{
#if GPS_MOVING_BASELINE
    ...
    if (interim_state.have_gps_yaw) {
        interim_state.gps_yaw_time_ms = AG_HAL::millis();
    }
...
void AG_GPS_UAVCAN::handle_status_msg(const StatusCb &cb)
{
    WITH_SEMAPHORE(sem);
...
/*
  ...
  */
void AG_GPS_UAVCAN::handle_moving_baseline_msg(const MovingBaselineDataCb &cb, uint8_t node_id)
{
    WITH_SEMAPHORE(sem);
    if (role != AG_GPS::GPS_ROLE_MB_BASE) {
...
*/
void AG_GPS_UAVCAN::handle_relposheading_msg(const RelPosHeadingCb &cb, uint8_t node_id)
{
    WITH_SEMAPHORE(sem);
...
bool AG_GPS_UAVCAN::get_RTCMV3(const uint8_t *&bytes, uint16_t &len)
{
    WITH_SEMAPHORE(sem);
    if (rtcm3_parser != nullptr) {
...
void AG_GPS_UAVCAN::clear_RTCMV3(void)
{
    WITH_SEMAPHORE(sem);
    if (rtcm3_parser != nullptr) {
...
void AG_GPS_UAVCAN::handle_fix2_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const Fix2Cb &cb)
{
    WITH_SEMAPHORE(_sem_registry);

    AG_GPS_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id);
    if (driver != nullptr) {
        driver->handle_fix2_msg(cb);
    }
}

void AG_GPS_UAVCAN::handle_aux_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const AuxCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);
...
void AG_GPS_UAVCAN::handle_heading_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const HeadingCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);
...
void AG_GPS_UAVCAN::handle_status_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const StatusCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);
...
void AG_GPS_UAVCAN::handle_moving_baseline_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const MovingBaselineDataCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);
    AG_GPS_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id);
    if (driver != nullptr) {
...
void AG_GPS_UAVCAN::handle_relposheading_msg_trampoline(AG_UAVCAN* ap_uavcan, uint8_t node_id, const RelPosHeadingCb &cb)
{
    WITH_SEMAPHORE(_sem_registry);
    AG_GPS_UAVCAN* driver = get_uavcan_backend(ap_uavcan, node_id);
    if (driver != nullptr) {
...
bool AG_GPS_UAVCAN::do_config()
{
    AG_UAVCAN *ap_uavcan = _detected_modules[_detected_module].ap_uavcan;
    if (ap_uavcan == nullptr) {
        return false;
    }
    ...
    switch(cfg_step) {
        ...
        case STEP_SET_MB_CAN_TX:
            if (role != AG_GPS::GPS_Role::GPS_ROLE_NORMAL) {
...
bool AG_GPS_UAVCAN::read(void)
{
    if (gps._auto_config >= AG_GPS::GPS_AUTO_CONFIG_ENABLE_ALL) {
        if (!do_config()) {
            return false;
        }
    }
    ...
    if (!seen_message) {
        ...
        state.status = AG_GPS::GPS_Status::NO_GPS;
    }

    return false;
}

bool AG_GPS_UAVCAN::is_healthy(void) const
{
...
bool AG_GPS_UAVCAN::logging_healthy(void) const
{
...
bool AG_GPS_UAVCAN::is_configured(void) const
{
 ...
 */
void AG_GPS_UAVCAN::inject_data(const uint8_t *data, uint16_t len)
{
...
*/
bool AG_GPS_UAVCAN::handle_param_get_set_response_int(AG_UAVCAN* ap_uavcan, uint8_t node_id, const char* name, int32_t &value)
{
    Debug("AG_GPS_UAVCAN: param set/get response from %d %s %ld\n", node_id, name, value);
    if (strcmp(name, "GPS_TYPE") == 0 && cfg_step == STEP_SET_TYPE) {
        if (role == AG_GPS::GPS_ROLE_MB_BASE && value != AG_GPS::GPS_TYPE_UBLOX_RTK_BASE) {
            value = (int32_t)AG_GPS::GPS_TYPE_UBLOX_RTK_BASE;
            requires_save_and_reboot = true;
            return true;
        } else if (role == AG_GPS::GPS_ROLE_MB_ROVER && value != AG_GPS::GPS_TYPE_UBLOX_RTK_ROVER) {
            value = (int32_t)AG_GPS::GPS_TYPE_UBLOX_RTK_ROVER;
            requires_save_and_reboot = true;
            return true;
        } else {
            cfg_step++;
    ...
    if (strcmp(name, "GPS_MB_ONLY_PORT") == 0 && cfg_step == STEP_SET_MB_CAN_TX) {
        if (option_set(AG_GPS::UAVCAN_MBUseDedicatedBus) && !value) {
            ...
            requires_save_and_reboot = true;
            return true;
        } else if (!option_set(AG_GPS::UAVCAN_MBUseDedicatedBus) && value) {
...
bool AG_GPS_UAVCAN::handle_param_get_set_response_float(AG_UAVCAN* ap_uavcan, uint8_t node_id, const char* name, float &value)
{
    Debug("AG_GPS_UAVCAN: param set/get response from %d %s %f\n", node_id, name, value);
    return false;
}

void AG_GPS_UAVCAN::handle_param_save_response(AG_UAVCAN* ap_uavcan, const uint8_t node_id, bool success)
{
    Debug("AG_GPS_UAVCAN: param save response from %d %s\n", node_id, success ? "success" : "failure");
    ...
    Debug("AG_GPS_UAVCAN: sending reboot command %d\n", node_id);
    ap_uavcan->send_reboot_request(node_id);
}

#if AP_DRONECAN_SEND_GPS
bool AG_GPS_UAVCAN::instance_exists(const AG_UAVCAN* ap_uavcan)
{
