...
AG_GPS_NOVA::AG_GPS_NOVA(AG_GPS &_gps, AG_GPS::GPS_State &_state,
...
const char* const AG_GPS_NOVA::_initialisation_blob[4] {
    "\r\n\r\nunlogall\r\n", // cleanup enviroment
...
bool
AG_GPS_NOVA::parse(uint8_t temp)
{
...
bool
AG_GPS_NOVA::process_message(void)
{
...
uint32_t AG_GPS_NOVA::CRC32Value(uint32_t icrc)
{
    int i;
    uint32_t crc = icrc;
    for ( i = 8 ; i > 0; i-- ) {
...
uint32_t AG_GPS_NOVA::CalculateBlockCRC32(uint32_t length, uint8_t *buffer, uint32_t crc)
{
    while ( length-- != 0 ) {
