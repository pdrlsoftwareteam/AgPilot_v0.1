...
if not Path("AG_Declination.h").is_file():
    raise OSError("Please run this tool from the AG_Declination directory")


def write_table(f,name, table):
    '''write one table'''
    f.write("const float AG_Declination::%s[%u][%u] = {\n" %
...
#include "AG_Declination.h"

''')

    f.write('''const float AG_Declination::SAMPLING_RES = %u;
const float AG_Declination::SAMPLING_MIN_LAT = %u;
const float AG_Declination::SAMPLING_MAX_LAT = %u;
const float AG_Declination::SAMPLING_MIN_LON = %u;
const float AG_Declination::SAMPLING_MAX_LON = %u;
