...
bool AP_Camera_AGPILOTLink::trigger_pic()
{
    // tell all of our components to take a picture:
    mavlink_command_long_t cmd_msg {};
    cmd_msg.command = AGPILOT_CMD_DO_DIGICAM_CONTROL;
    cmd_msg.param5 = 1;
...
// configure camera
void AP_Camera_AGPILOTLink::configure(float shooting_mode, float shutter_speed, float aperture, float ISO, float exposure_type, float cmd_id, float engine_cutoff_time)
{
    // convert to mavlink message and send to all components
    mavlink_command_long_t mav_cmd_long = {};

    // convert mission command to mavlink command_long
    mav_cmd_long.command = AGPILOT_CMD_DO_DIGICAM_CONFIGURE;
    ...
    GCS_AGPILOTLINK::send_to_components(AGPILOTLINK_MSG_ID_COMMAND_LONG, (char*)&mav_cmd_long, sizeof(mav_cmd_long));
}

// handle camera control message
void AP_Camera_AGPILOTLink::control(float session, float zoom_pos, float zoom_step, float focus_lock, float shooting_cmd, float cmd_id)
{
    ...
    mavlink_command_long_t mav_cmd_long = {};
    mav_cmd_long.command = AGPILOT_CMD_DO_DIGICAM_CONTROL;
...
#endif // AP_CAMERA_AGPILOTLINK_ENABLED
