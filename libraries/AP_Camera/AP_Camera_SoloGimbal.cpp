...
bool AP_Camera_SoloGimbal::trigger_pic()
{
    if (gopro_status != GOPRO_HEARTBEAT_STATUS_CONNECTED) {
        gcs().send_text(AGPILOT_SEVERITY_ERROR, "GoPro Not Available");
        return false;
    }
    ...
    if (gopro_capture_mode == GOPRO_CAPTURE_MODE_PHOTO) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro Photo Trigger");
        mavlink_msg_gopro_set_request_send(heartbeat_channel, mavlink_system.sysid, AGPILOT_COMP_ID_GIMBAL,GOPRO_COMMAND_SHUTTER,gopro_shutter_start);

    } else if (gopro_capture_mode == GOPRO_CAPTURE_MODE_VIDEO) {
        if (gopro_is_recording) {
            // GoPro is recording, so stop recording
            gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro Recording Stop");
            mavlink_msg_gopro_set_request_send(heartbeat_channel, mavlink_system.sysid, AGPILOT_COMP_ID_GIMBAL,GOPRO_COMMAND_SHUTTER,gopro_shutter_stop);
        } else {
            // GoPro is not recording, so start recording
            gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro Recording Start");
            mavlink_msg_gopro_set_request_send(heartbeat_channel, mavlink_system.sysid, AGPILOT_COMP_ID_GIMBAL,GOPRO_COMMAND_SHUTTER,gopro_shutter_start);
        }
    } else {
        gcs().send_text(AGPILOT_SEVERITY_ERROR, "GoPro Unsupported Capture Mode");
        return false;
    }
    ...
    switch(gopro_capture_mode) {
        case GOPRO_CAPTURE_MODE_VIDEO:
            if (gopro_is_recording) {
                ...
                gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro recording, can't change modes");
            } else {
                // Change to camera mode
                gopro_capture_mode_values[0] = GOPRO_CAPTURE_MODE_PHOTO;
                mavlink_msg_gopro_set_request_send(heartbeat_channel, mavlink_system.sysid, AGPILOT_COMP_ID_GIMBAL,GOPRO_COMMAND_CAPTURE_MODE,gopro_capture_mode_values);
                gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro changing to mode photo");
            }
            break;
        ...
        default:
            ...
            mavlink_msg_gopro_set_request_send(heartbeat_channel, mavlink_system.sysid, AGPILOT_COMP_ID_GIMBAL,GOPRO_COMMAND_CAPTURE_MODE,gopro_capture_mode_values);
            gcs().send_text(AGPILOT_SEVERITY_INFO, "GoPro changing to mode video");
            break;
    }
}
