...
#include <AG_PID/AG_P.h>            // P library
#include <AG_PID/AG_PID.h>          // PID library
#include <AG_PID/AG_P_1D.h>         // P library (1-axis)
#include <AG_PID/AG_P_2D.h>         // P library (2-axis)
#include <AG_PID/AG_PI_2D.h>        // PI library (2-axis)
#include <AG_PID/AG_PID_Basic.h>    // PID library (1-axis)
#include <AG_PID/AG_PID_2D.h>       // PID library (2-axis)
#include <AG_InertialNav/AG_InertialNav.h>  // Inertial Navigation library
#include "AG_AttitudeControl.h"     // Attitude control library
...
public:
    ...
    /// Constructor
    AG_PosControl(AG_AHRS_View& ahrs, const AG_InertialNav& inav,
                  const class AG_Motors& motors, AG_AttitudeControl& attitude_control);
    ...
    */
    ...
    /// get pid controllers
    AG_P_2D& get_pos_xy_p() { return _p_pos_xy; }
    AG_P_1D& get_pos_z_p() { return _p_pos_z; }
    AG_PID_2D& get_vel_xy_pid() { return _pid_vel_xy; }
    AG_PID_Basic& get_vel_z_pid() { return _pid_vel_z; }
    AG_PID& get_accel_z_pid() { return _pid_accel_z; }
...
protected:
    ...
    AG_AHRS_View&           _ahrs;
    const AG_InertialNav&   _inav;
    const class AG_Motors&  _motors;
    AG_AttitudeControl&     _attitude_control;
    ...
    AG_P_2D         _p_pos_xy;          // XY axis position controller to convert distance error to desired velocity
    AG_P_1D         _p_pos_z;           // Z axis position controller to convert altitude error to desired climb rate
    AG_PID_2D       _pid_vel_xy;        // XY axis velocity controller to convert velocity error to desired acceleration
    AG_PID_Basic    _pid_vel_z;         // Z axis velocity controller to convert climb rate error to desired acceleration
    AG_PID          _pid_accel_z;       // Z axis acceleration controller to convert desired acceleration to throttle output
