...
#include <AG_Vehicle/AG_MultiCopter.h>
...
public:
    AG_AttitudeControl( AG_AHRS_View &ahrs,
                        const AG_MultiCopter &aparm,
                        AG_Motors& motors) :
    ...
    static AG_AttitudeControl *get_singleton(void) {
        return _singleton;
    }

    // Empty destructor to suppress compiler warning
    virtual ~AG_AttitudeControl() {}
    ...
    // pid accessors
    AG_P& get_angle_roll_p() { return _p_angle_roll; }
    AG_P& get_angle_pitch_p() { return _p_angle_pitch; }
    AG_P& get_angle_yaw_p() { return _p_angle_yaw; }
    virtual AG_PID& get_rate_roll_pid() = 0;
    virtual AG_PID& get_rate_pitch_pid() = 0;
    virtual AG_PID& get_rate_yaw_pid() = 0;
    ...
    // Used by child class AG_AttitudeControl_TS to change behaviour for tailsitter quadplanes
    ...
    // (implemented only in AG_AttitudeControl_TS for tailsitter quadplanes)
...
protected:
    ...
    const AG_AHRS_View&  _ahrs;
    const AG_MultiCopter &_aparm;
    AG_Motors&          _motors;

    static AG_AttitudeControl *_singleton;
