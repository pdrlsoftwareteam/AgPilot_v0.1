...
const AG_Param::GroupInfo AG_WeatherVane::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_WeatherVane, _direction, WVANE_PARAM_ENABLED, AP_PARAM_FLAG_ENABLE),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("GAIN", 2, AG_WeatherVane, _gain, WVANE_PARAM_GAIN_DEFAULT),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("ANG_MIN", 3, AG_WeatherVane, _min_dz_ang_deg, 1.0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("HGT_MIN", 4, AG_WeatherVane, _min_height, 0.0),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("SPD_MAX", 5, AG_WeatherVane, _max_vel_xy, WVANE_PARAM_SPD_MAX_DEFAULT),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("VELZ_MAX", 6, AG_WeatherVane, _max_vel_z, WVANE_PARAM_VELZ_MAX_DEFAULT),
    ...
    // @User: Standard
    AP_GROUPINFO("TAKEOFF", 7, AG_WeatherVane, _takeoff_direction, -1),
    ...
    // @Values: -1:No override,0:Disabled,1:Nose into wind,2:Nose or tail into wind,3:Side into wind,4:tail into wind
    // @User: Standard
    AP_GROUPINFO("LAND", 8, AG_WeatherVane, _landing_direction, -1),
    ...
    // @User: Standard
    AP_GROUPINFO("OPTIONS", 9, AG_WeatherVane, _options, 0),
...
// Constructor
AG_WeatherVane::AG_WeatherVane(void)
{
    AG_Param::setup_object_defaults(this, var_info);
}

bool AG_WeatherVane::get_yaw_out(float &yaw_output, const int16_t pilot_yaw, const float hgt, const float roll_cdeg, const float pitch_cdeg, const bool is_takeoff, const bool is_landing)
{
...
void AG_WeatherVane::reset(void)
{
    last_output = 0;
    active_msg_sent = false;
    first_activate_ms = 0;
    last_check_ms = AG_HAL::millis();
}
