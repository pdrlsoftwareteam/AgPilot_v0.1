...
void AP_Tuning::check_selector_switch(void)
{
    ...
    if (selector_in >= RC_Channel::AUX_PWM_TRIGGER_HIGH) {
        ...
        if (hold_time > 5000 && changed) {
            ...
            save_parameters();
            re_center();
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Tuning: Saved");
        ...
        if (selector_start_ms != 0) {
            ...
            if (hold_time < 200) {
                ...
                // re-center the value
                re_center();
                gcs().send_text(AGPILOT_SEVERITY_INFO, "Tuning: recentered %s", get_tuning_name(current_parm));
...
void AP_Tuning::check_input(uint8_t flightmode)
{
    ...
    if (flightmode != last_flightmode) {
        if (need_revert != 0 && mode_revert != 0) {
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Tuning: reverted");
            revert_parameters();
            re_center();
        }
    ...
    if (mid_point_wait) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Tuning: mid-point %s", get_tuning_name(current_parm));
...
void AP_Tuning::next_parameter(void)
{
    ...
    for (uint8_t i=0; tuning_sets[i].num_parms != 0; i++) {
        if (tuning_sets[i].set+set_base == set) {
            ...
            re_center();
            gcs().send_text(AGPILOT_SEVERITY_INFO, "Tuning: started %s", get_tuning_name(current_parm));
