...
const AG_Param::GroupInfo AG_KDECAN::var_info[] = {
    // @Param: NPOLE
    // @DisplayName: Number of motor poles
    // @Description: Sets the number of motor poles to calculate the correct RPM value
    AP_GROUPINFO("NPOLE", 1, AG_KDECAN, _num_poles, DEFAULT_NUM_POLES),

    AP_GROUPEND
};

const uint16_t AG_KDECAN::SET_PWM_MIN_INTERVAL_US;

AG_KDECAN::AG_KDECAN()
{
    AG_Param::setup_object_defaults(this, var_info);

    debug_can(AG_CANManager::LOG_INFO, "constructed");
}

AG_KDECAN *AG_KDECAN::get_kdecan(uint8_t driver_index)
{
    if (driver_index >= AP::can().get_num_drivers() ||
        AP::can().get_driver_type(driver_index) != AG_CANManager::Driver_Type_KDECAN) {
        return nullptr;
    }
    return static_cast<AG_KDECAN*>(AP::can().get_driver(driver_index));
}

bool AG_KDECAN::add_interface(AG_HAL::CANIface* can_iface) {

    if (_can_iface != nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "Multiple Interface not supported");
        return false;
    }
    ...
    if (_can_iface == nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "CAN driver not found");
        return false;
    }

    if (!_can_iface->is_initialized()) {
        debug_can(AG_CANManager::LOG_ERROR, "Driver not initialized");
        return false;
    }

    if (!_can_iface->set_event_handle(&_event_handle)) {
        debug_can(AG_CANManager::LOG_ERROR, "Cannot add event handle");
        return false;
    }
    return true;
}

void AG_KDECAN::init(uint8_t driver_index, bool enable_filters)
{
    _driver_index = driver_index;

    debug_can(AG_CANManager::LOG_INFO, "starting init");

    if (_initialized) {
        debug_can(AG_CANManager::LOG_ERROR, "already initialized");
        return;
    }

    if (_can_iface == nullptr) {
        debug_can(AG_CANManager::LOG_ERROR, "Interface not found");
        return;
    }
    ...
    AG_HAL::CANFrame frame { (id.value | AG_HAL::CANFrame::FlagEFF), nullptr, 0 };

    if(!_can_iface->send(frame, AG_HAL::micros() + 1000000, 0)) {
        debug_can(AG_CANManager::LOG_DEBUG, "couldn't send discovery message");
        return;
    }

    debug_can(AG_CANManager::LOG_DEBUG, "discovery message sent");

    uint32_t start = AG_HAL::millis();

    // wait 1 second for answers
    while (AG_HAL::millis() - start < 1000) {
        AG_HAL::CANFrame esc_id_frame {};
        uint64_t rx_time;
        AG_HAL::CANIface::CanIOFlags flags = 0;
        ...
        id.value = esc_id_frame.id & AG_HAL::CANFrame::MaskExtID;
        ...
        debug_can(AG_CANManager::LOG_DEBUG, "found ESC id %u", id.source_id);
    }

    snprintf(_thread_name, sizeof(_thread_name), "kdecan_%u", driver_index);

    // start thread for receiving and sending CAN frames
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_KDECAN::loop, void), _thread_name, 4096, AG_HAL::Scheduler::PRIORITY_CAN, 0)) {
        debug_can(AG_CANManager::LOG_ERROR, "couldn't create thread");
        return;
    }

    _initialized = true;

    debug_can(AG_CANManager::LOG_DEBUG, "init done");
...
void AG_KDECAN::loop()
{
    ...
    AG_HAL::CANFrame empty_frame { (0 | AG_HAL::CANFrame::FlagEFF), nullptr, 0 };

    while (true) {
        if (!_initialized) {
            debug_can(AG_CANManager::LOG_ERROR, "not initialized");
        ...
        if (_enum_sem.take(1)) {
            ...
            debug_can(AG_CANManager::LOG_DEBUG, "failed to get enumeration semaphore on loop");
        }
        ...
        if (enumeration_state != ENUMERATION_STOPPED) {
            ...
            switch (enumeration_state) {
                case ENUMERATION_START: {
                    ...
                    AG_HAL::CANFrame frame { (id.value | AG_HAL::CANFrame::FlagEFF), (uint8_t*) &data, sizeof(data) };
                    ...
                    if (select_ret && write_select) {
                        ...
                        if (res == 1) {
                            ...
                            debug_can(AG_CANManager::LOG_ERROR, "strange buffer full when starting ESC enumeration");
                            break;
                        } else {
                            debug_can(AG_CANManager::LOG_ERROR, "error sending message to start ESC enumeration, result %d", res);
                            break;
                        }
                    } else {
                        break;
                ...
                case ENUMERATION_RUNNING: {
                    ...
                    if (select_ret && read_select) {
                        ...
                        if (res == 1) {
                            frame_id_t id { .value = recv_frame.id & AG_HAL::CANFrame::MaskExtID };
                            if (rx_time < enumeration_start) {
                                // old message
                                debug_can(AG_CANManager::LOG_DEBUG, "Received old message from ESC id %u", id.source_id);
                                break;
                            }
                            ...
                            while (AG_HAL::micros64() - enumeration_start < ENUMERATION_TIMEOUT_MS * 1000) {
                                // wait for write space to be available
                                id = { { .object_address = UPDATE_NODE_ID_OBJ_ADDR,
                                            ...
                                            .priority = 0,
                                            .unused = 0 } };
                                AG_HAL::CANFrame send_frame { (id.value | AG_HAL::CANFrame::FlagEFF), (uint8_t*) &recv_frame.data, recv_frame.dlc };
                                read_select = false;
                                write_select = true;
                                ...
                                if (select_ret && write_select) {
                                    ...
                                    if (res == 1) {
                                        ...
                                        debug_can(AG_CANManager::LOG_ERROR, "strange buffer full when setting ESC node ID");
                                    } else {
                                        debug_can(AG_CANManager::LOG_ERROR, "error sending message to set ESC node ID, result %d", res);
                                    }
                                }
                            }
                        } else if (res == 0) {
                            debug_can(AG_CANManager::LOG_ERROR, "strange failed read when getting ESC enumeration message");
                        } else {
                            debug_can(AG_CANManager::LOG_ERROR, "error receiving ESC enumeration message, result %d", res);
                        }
                    }
                    break;
                }
                case ENUMERATION_STOP: {
                    ...
                    if (select_ret && write_select) {
                        ...
                        if (res == 1) {
                            ...
                            debug_can(AG_CANManager::LOG_ERROR, "strange buffer full when stop ESC enumeration");
                        } else {
                            debug_can(AG_CANManager::LOG_ERROR, "error sending message to stop ESC enumeration, result %d", res);
                        }
                    }
                    break;
                }
                ...
                default:
                    debug_can(AG_CANManager::LOG_DEBUG, "something wrong happened, shouldn't be here, enumeration state: %u", enumeration_state);
                    break;
            }
        ...
        if (select_ret && read_select) {
            ...
            if (res == 1) {
                ...
                frame_id_t id { .value = frame.id & AG_HAL::CANFrame::MaskExtID };
        ...
        if (select_ret && try_write && write_select) {
            ...
            if (sending_esc_num > 0) {
                ...
                if (now - pwm_last_sent > SET_PWM_TIMEOUT_US) {
                    debug_can(AG_CANManager::LOG_DEBUG, "timed-out after sending frame to ESC with ID %d", sending_esc_num - 1);
                    sending_esc_num = 0;
                }
            }
            ...
            if (sending_esc_num == 0 && new_output) {
                if (!_rc_out_sem.take(1)) {
                    debug_can(AG_CANManager::LOG_ERROR, "failed to get PWM semaphore on read");
                    continue;
                }
            ...
            if (sending_esc_num > 0 ||
                ...
                for (uint8_t esc_num = sending_esc_num; esc_num < _esc_max_node_id; esc_num++) {
                    ...
                    AG_HAL::CANFrame frame { (id.value | AG_HAL::CANFrame::FlagEFF), (uint8_t*) &kde_pwm, sizeof(kde_pwm) };
                    ...
                    if (res == 1) {
                        ...
                        debug_can(AG_CANManager::LOG_ERROR, "error sending message to ESC with ID %d, result %d", esc_num + ESC_NODE_ID_FIRST, res);
                    }
                ...
                if (res == 1) {
                    ...
                    debug_can(AG_CANManager::LOG_ERROR, "strange buffer full when sending message requesting telemetry");
                } else {
                    debug_can(AG_CANManager::LOG_ERROR, "error sending message requesting telemetry, result %d", res);
                }
            }
        }
    }
...
void AG_KDECAN::update()
{
    if (_rc_out_sem.take(1)) {
        ...
        debug_can(AG_CANManager::LOG_DEBUG, "failed to get PWM semaphore on write");
    }
}

bool AG_KDECAN::pre_arm_check(char* reason, uint8_t reason_len)
{
    if (!_enum_sem.take(1)) {
    ...
    AG_Motors *motors = AG_Motors::get_singleton();
...
bool AG_KDECAN::run_enumeration(bool start_stop)
{
    if (!_enum_sem.take(1)) {
        debug_can(AG_CANManager::LOG_DEBUG, "failed to get enumeration semaphore on write");
        return false;
    }
