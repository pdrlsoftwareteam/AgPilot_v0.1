...
#define OPTFLOW_AGPILOT_TIMEOUT_SEC 0.5f

// detect the device
AP_OpticalFlow_AGPILOT *AP_OpticalFlow_AGPILOT::detect(AP_OpticalFlow &_frontend)
{
    // we assume mavlink messages will be sent into this driver
    AP_OpticalFlow_AGPILOT *sensor = new AP_OpticalFlow_AGPILOT(_frontend);
    return sensor;
}

// read latest values from sensor and fill in x,y and totals.
void AP_OpticalFlow_AGPILOT::update(void)
{
    ...
    // sanity check dt
    if (is_positive(dt) && (dt < OPTFLOW_AGPILOT_TIMEOUT_SEC)) {
...
void AP_OpticalFlow_AGPILOT::handle_msg(const mavlink_message_t &msg)
{
...
#endif  // AP_OPTICALFLOW_AGPILOT_ENABLED
