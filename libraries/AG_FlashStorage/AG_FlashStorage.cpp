...
#include <AG_FlashStorage/AG_FlashStorage.h>
...
// constructor.
AG_FlashStorage::AG_FlashStorage(uint8_t *_mem_buffer,
...
// initialise storage
bool AG_FlashStorage::init(void)
{
    debug("running init()\n");
...
bool AG_FlashStorage::switch_full_sector(void)
{
    debug("running switch_full_sector()\n");

    if (in_switch_full_sector) {
        INTERNAL_ERROR(AG_InternalError::error_t::switch_full_sector_recursion);
        return false;
    }
...
// happens.
bool AG_FlashStorage::protected_switch_full_sector(void)
{
    // clear any write error
    write_error = false;
    reserved_space = 0;
...
bool AG_FlashStorage::write(uint16_t offset, uint16_t length)
{
    if (write_error) {
        return false;
    }
 ...
 */
bool AG_FlashStorage::load_sector(uint8_t sector)
{
...
/*
  erase one sector
 */
bool AG_FlashStorage::erase_sector(uint8_t sector, bool mark_available)
{
...
/*
  erase both sectors
 */
bool AG_FlashStorage::erase_all(void)
{
    write_error = false;
 ...
 */
bool AG_FlashStorage::write_all()
{
...
bool AG_FlashStorage::all_zero(uint16_t ofs, uint16_t size)
{
    while (size--) {
        if (mem_buffer[ofs++] != 0) {
            return false;
...
bool AG_FlashStorage::switch_sectors(void)
{
    if (reserved_space != 0) {
 ...
 */
bool AG_FlashStorage::re_initialise(void)
{
    if (!flash_erase_ok()) {
        return false;
    }
 ...
 */
bool AG_FlashStorage::sector_header::signature_ok(void) const
{
...
AG_FlashStorage::SectorState AG_FlashStorage::sector_header::get_state(void) const
{
...
void AG_FlashStorage::sector_header::set_state(SectorState state)
{
