//
// Unit tests for the AG_Math rotations code
//
...
class FlashTest : public AG_HAL::HAL::Callbacks {
public:
...
private:
    ...
    uint8_t mem_buffer[AG_FlashStorage::storage_size];
    uint8_t mem_mirror[AG_FlashStorage::storage_size];
    ...
    AG_FlashStorage storage{mem_buffer,
            flash_sector_size,
...
{
    if (sector > 1) {
        AG_HAL::panic("FATAL: write to sector %u\n", (unsigned)sector);
    }
    if (offset + length > flash_sector_size) {
        AG_HAL::panic("FATAL: write to sector %u at offset %u length %u\n",
                      (unsigned)sector,
                      (unsigned)offset,
                      (unsigned)length);
    }
    ...
    if ((offset & 1) || (length & 1)) {
        AG_HAL::panic("FATAL: invalid write at %u:%u len=%u\n",
                      (unsigned)sector,
                      (unsigned)offset,
                      (unsigned)length);
    }
    ...
    for (uint16_t i=0; i<len16; i++) {
        ...
        if (v & !v2) {
            AG_HAL::panic("FATAL: invalid write16 at %u:%u 0x%04x 0x%04x\n",
                          (unsigned)sector,
                          unsigned(offset+i),
                          b[i],
                          data[i]);
...
{
    if (sector > 1) {
        AG_HAL::panic("FATAL: read from sector %u\n", (unsigned)sector);
    }
    if (offset + length > flash_sector_size) {
        AG_HAL::panic("FATAL: read from sector %u at offset %u length %u\n",
                      (unsigned)sector,
                      (unsigned)offset,
                      (unsigned)length);
    }
...
{
    if (sector > 1) {
        AG_HAL::panic("FATAL: erase sector %u\n", (unsigned)sector);
    }
...
{
    hal.console->printf("AG_FlashStorage test\n");
}
...
void FlashTest::loop(void)
{
    ...
    if (!storage.init()) {
        AG_HAL::panic("Failed first init()");
    }
    ...
    for (uint32_t i=0; i<5000000; i++) {
        ...
        if (erase_ok) {
            if (memcmp(mem_buffer, mem_mirror, sizeof(mem_buffer)) != 0) {
                AG_HAL::panic("FATAL: data mis-match at i=%u", (unsigned)i);
            }
        }
    }
    ...
    if (memcmp(mem_buffer, mem_mirror, sizeof(mem_buffer)) != 0) {
        AG_HAL::panic("FATAL: data mis-match before re-init");
    }
    ...
    if (!storage.init()) {
        AG_HAL::panic("Failed second init()");
    }

    if (memcmp(mem_buffer, mem_mirror, sizeof(mem_buffer)) != 0) {
        AG_HAL::panic("FATAL: data mis-match");
    }
    while (true) {
...
AG_HAL_MAIN_CALLBACKS(&flashtest);
