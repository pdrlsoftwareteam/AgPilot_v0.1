#include "AG_CSVReader.h"

#include <AG_Common/AG_Common.h>

#include <stdio.h>

AG_CSVReader::RetCode AG_CSVReader::handle_unquoted_term(uint8_t c)
{
    if (c == separator) {
...
AG_CSVReader::RetCode AG_CSVReader::handle_quoted_term(uint8_t c)
{
    if (c == '"') {
...
AG_CSVReader::RetCode AG_CSVReader::feed(uint8_t c)
{
    if (term_len == 0) {
        return RetCode::ERROR;
    }
