...
#define AG_PID_TFILT_HZ_DEFAULT  0.0f   // default input filter frequency
#define AG_PID_EFILT_HZ_DEFAULT  0.0f   // default input filter frequency
#define AG_PID_DFILT_HZ_DEFAULT  20.0f   // default input filter frequency
#define AG_PID_RESET_TC          0.16f   // Time constant for integrator reset decay to zero

#include "AG_PIDInfo.h"
...
public:
    ...
    // Constructor for PID
    AG_PID(float initial_p, float initial_i, float initial_d, float initial_ff, float initial_imax, float initial_filt_T_hz, float initial_filt_E_hz, float initial_filt_D_hz,
           float initial_srmax=0, float initial_srtau=1.0);

    CLASS_NO_COPY(AG_PID);
    ...
    const AG_PIDInfo& get_pid_info(void) const { return _pid_info; }
    ...
    // single user of AG_PID by adding the parameter in the param
