...
const AG_Param::GroupInfo AG_PI_2D::var_info[] = {
    // @Param: P
    // @DisplayName: PI Proportional Gain
    // @Description: P Gain which produces an output value that is proportional to the current error value
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("P",    0, AG_PI_2D, _kp, default_kp),

    // @Param: I
    // @DisplayName: PI Integral Gain
    // @Description: I Gain which produces an output that is proportional to both the magnitude and the duration of the error
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("I",    1, AG_PI_2D, _ki, default_ki),

    // @Param: IMAX
    // @DisplayName: PI Integral Maximum
    // @Description: The maximum/minimum value that the I term can output
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("IMAX", 2, AG_PI_2D, _imax, default_imax),

    // @Param: FILT_HZ
    // @DisplayName: PI Input filter frequency in Hz
    // @Description: Input filter frequency in Hz
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FILT_HZ", 3, AG_PI_2D, _filt_hz, default_filt_hz),

    AP_GROUPEND
};

// Constructor
AG_PI_2D::AG_PI_2D(float initial_p, float initial_i, float initial_imax, float initial_filt_hz, float dt) :
    _dt(dt),
...
void AG_PI_2D::filt_hz(float hz)
{
    _filt_hz.set(fabsf(hz));

    // sanity check _filt_hz
    _filt_hz.set(MAX(_filt_hz, AG_PI_2D_FILT_HZ_MIN));
...
void AG_PI_2D::set_input(const Vector2f &input)
{
...
Vector2f AG_PI_2D::get_p() const
{
    return (_input * _kp);
}

Vector2f AG_PI_2D::get_i()
{
...
Vector2f AG_PI_2D::get_i_shrink()
{
    if (!is_zero(_ki) && !is_zero(_dt)) {
...
Vector2f AG_PI_2D::get_pi()
{
    return get_p() + get_i();
}

void AG_PI_2D::reset_I()
{
    _integrator.zero();
}

void AG_PI_2D::load_gains()
{
    _kp.load();
    _ki.load();
    _imax.load();
...
void AG_PI_2D::operator() (float p, float i, float imaxval, float input_filt_hz, float dt)
{
    _kp.set(p);
    _ki.set(i);
