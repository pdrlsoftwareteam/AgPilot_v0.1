...
void setup()
{
    hal.console->printf("ArduPilot AG_PID library test\n");
...
void loop()
{
    // setup (unfortunately must be done here as we cannot create a global AG_PID object)
    AG_PID pid(TEST_P, TEST_I, TEST_D, 0.0f, TEST_IMAX * 100.0f, 0.0f, 0.0f, TEST_FILTER);
    AG_HELI_PID heli_pid(TEST_P, TEST_I, TEST_D, TEST_INITIAL_FF, TEST_IMAX * 100, 0.0f, 0.0f, TEST_FILTER);
