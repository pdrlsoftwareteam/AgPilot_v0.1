...
public:
    ...
    // Constructor for PID
    AG_PID_2D(float initial_kP, float initial_kI, float initial_kD, float initial_kFF, float initial_imax, float initial_filt_hz, float initial_filt_d_hz);

    CLASS_NO_COPY(AG_PID_2D);
    ...
    const AG_PIDInfo& get_pid_info_x(void) const { return _pid_info_x; }
    const AG_PIDInfo& get_pid_info_y(void) const { return _pid_info_y; }
...
protected:
    ...
    AG_PIDInfo _pid_info_x;
    AG_PIDInfo _pid_info_y;
