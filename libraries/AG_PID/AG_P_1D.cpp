...
const AG_Param::GroupInfo AG_P_1D::var_info[] = {
    // @Param: P
    // @DisplayName: P Proportional Gain
    // @Description: P Gain which produces an output value that is proportional to the current error value
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("P",    0, AG_P_1D, _kp, default_kp),
    AP_GROUPEND
};

// Constructor
AG_P_1D::AG_P_1D(float initial_p) :
...
float AG_P_1D::update_all(float &target, float measurement)
{
...
void AG_P_1D::set_limits(float output_min, float output_max, float D_Out_max, float D2_Out_max)
{
    _D1_max = 0.0f;
    _error_min = 0.0f;
    _error_max = 0.0f;
...
void AG_P_1D::set_error_limits(float error_min, float error_max)
{
