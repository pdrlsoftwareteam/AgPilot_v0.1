...
const AG_Param::GroupInfo AG_P_2D::var_info[] = {
    // @Param: P
    // @DisplayName: Proportional Gain
    // @Description: P Gain which produces an output value that is proportional to the current error value
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("P",    0, AG_P_2D, _kp, default_kp),
    AP_GROUPEND
};

// Constructor
AG_P_2D::AG_P_2D(float initial_p) :
...
Vector2f AG_P_2D::update_all(postype_t &target_x, postype_t &target_y, const Vector2f &measurement)
{
...
void AG_P_2D::set_limits(float output_max, float D_Out_max, float D2_Out_max)
{
    _D1_max = 0.0f;
    _error_max = 0.0f;
...
void AG_P_2D::set_error_max(float error_max)
{
