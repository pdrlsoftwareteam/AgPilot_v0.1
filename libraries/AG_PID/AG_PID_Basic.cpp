/// @file	AG_PID_Basic.cpp
/// @brief	Generic PID algorithm

#include <AG_Math/AG_Math.h>
#include <AG_InternalError/AG_InternalError.h>
#include "AG_PID_Basic.h"

#define AG_PID_Basic_FILT_E_HZ_MIN     0.01f   // minimum input filter frequency
#define AG_PID_Basic_FILT_D_HZ_MIN     0.005f  // minimum input filter frequency

const AG_Param::GroupInfo AG_PID_Basic::var_info[] = {
    // @Param: P
    // @DisplayName: PID Proportional Gain
    // @Description: P Gain which produces an output value that is proportional to the current error value
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("P",    0, AG_PID_Basic, _kp, default_kp),

    // @Param: I
    // @DisplayName: PID Integral Gain
    // @Description: I Gain which produces an output that is proportional to both the magnitude and the duration of the error
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("I",    1, AG_PID_Basic, _ki, default_ki),

    // @Param: IMAX
    // @DisplayName: PID Integral Maximum
    // @Description: The maximum/minimum value that the I term can output
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("IMAX", 2, AG_PID_Basic, _kimax, default_kimax),

    // @Param: FLTE
    // @DisplayName: PID Error filter frequency in Hz
    // @Description: Error filter frequency in Hz
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FLTE", 3, AG_PID_Basic, _filt_E_hz, default_filt_E_hz),

    // @Param: D
    // @DisplayName: PID Derivative Gain
    // @Description: D Gain which produces an output that is proportional to the rate of change of the error
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("D",    4, AG_PID_Basic, _kd, default_kd),

    // @Param: FLTD
    // @DisplayName: D term filter frequency in Hz
    // @Description: D term filter frequency in Hz
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FLTD", 5, AG_PID_Basic, _filt_D_hz, default_filt_D_hz),
    ...
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FF",   6, AG_PID_Basic, _kff, default_kff),

    AP_GROUPEND
};

// Constructor
AG_PID_Basic::AG_PID_Basic(float initial_p, float initial_i, float initial_d, float initial_ff, float initial_imax, float initial_filt_E_hz, float initial_filt_D_hz) :
...
float AG_PID_Basic::update_all(float target, float measurement, float dt, bool limit)
{
...
float AG_PID_Basic::update_all(float target, float measurement, float dt, bool limit_neg, bool limit_pos)
{
...
void AG_PID_Basic::update_i(float dt, bool limit_neg, bool limit_pos)
{
    if (!is_zero(_ki)) {
...
void AG_PID_Basic::reset_I()
{
    _integrator = 0.0; 
}

// save_gains - save gains to eeprom
void AG_PID_Basic::save_gains()
{
    _kp.save();
    _ki.save();
    _kd.save();
...
float AG_PID_Basic::get_filt_E_alpha(float dt) const
{
    return calc_lowpass_alpha_dt(dt, _filt_E_hz);
}

// get_filt_D_alpha - get the derivative filter alpha
float AG_PID_Basic::get_filt_D_alpha(float dt) const
{
    return calc_lowpass_alpha_dt(dt, _filt_D_hz);
}

void AG_PID_Basic::set_integrator(float target, float measurement, float i)
{
    set_integrator(target - measurement, i);
}

void AG_PID_Basic::set_integrator(float error, float i)
{
    set_integrator(i - error * _kp);
}

void AG_PID_Basic::set_integrator(float i)
{
