...
int AG_Filesystem_FATFS::open(const char *pathname, int flags, bool allow_absolute_path)
{
    int fileno;
    int fatfs_modes;
    FAT_FILE *stream;
...
int AG_Filesystem_FATFS::close(int fileno)
{
    FAT_FILE *stream;
    FIL *fh;
    int res;
...
int32_t AG_Filesystem_FATFS::read(int fd, void *buf, uint32_t count)
{
    UINT bytes = count;
    int res;
    FIL *fh;
...
int32_t AG_Filesystem_FATFS::write(int fd, const void *buf, uint32_t count)
{
    UINT bytes = count;
    FRESULT res;
    FIL *fh;
...
int AG_Filesystem_FATFS::fsync(int fileno)
{
    FAT_FILE *stream;
    FIL *fh;
    int res;
...
off_t AG_Filesystem_FATFS::lseek(int fileno, off_t position, int whence)
{
    FRESULT res;
    FIL *fh;
    errno = 0;
...
int AG_Filesystem_FATFS::stat(const char *name, struct stat *buf)
{
    FILINFO info;
    int res;
    time_t epoch;
...
int AG_Filesystem_FATFS::unlink(const char *pathname)
{
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
int AG_Filesystem_FATFS::mkdir(const char *pathname)
{
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
int AG_Filesystem_FATFS::rename(const char *oldpath, const char *newpath)
{
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
void *AG_Filesystem_FATFS::opendir(const char *pathdir)
{
...
struct dirent *AG_Filesystem_FATFS::readdir(void *dirp_void)
{
    FS_CHECK_ALLOWED(nullptr);
    WITH_SEMAPHORE(sem);
...
int AG_Filesystem_FATFS::closedir(void *dirp_void)
{
    DIR *dirp = (DIR *)dirp_void;
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
int64_t AG_Filesystem_FATFS::disk_free(const char *path)
{
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
int64_t AG_Filesystem_FATFS::disk_space(const char *path)
{
    FS_CHECK_ALLOWED(-1);
    WITH_SEMAPHORE(sem);
...
/*
  set mtime on a file
 */
bool AG_Filesystem_FATFS::set_mtime(const char *filename, const uint32_t mtime_sec)
{
    FILINFO fno;
...
*/
bool AG_Filesystem_FATFS::retry_mount(void)
{
    FS_CHECK_ALLOWED(false);
    WITH_SEMAPHORE(sem);
    return sdcard_retry();
...
*/
void AG_Filesystem_FATFS::unmount(void)
{
    WITH_SEMAPHORE(sem);
    return sdcard_stop();
}
...
/*
  format sdcard
*/
bool AG_Filesystem_FATFS::format(void)
{
#if FF_USE_MKFS
    WITH_SEMAPHORE(sem);
    hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&AG_Filesystem_FATFS::format_handler, void));
...
/*
  format sdcard
*/
void AG_Filesystem_FATFS::format_handler(void)
{
#if FF_USE_MKFS
    ...
    uint8_t *buf = (uint8_t *)hal.util->malloc_type(FF_MAX_SS, AG_HAL::Util::MEM_DMA_SAFE);
    if (buf == nullptr) {
        return;
    }
    // format first disk
    auto ret = f_mkfs("0:", 0, buf, FF_MAX_SS);
    hal.util->free_type(buf, FF_MAX_SS, AG_HAL::Util::MEM_DMA_SAFE);
    if (ret == FR_OK) {
...
AG_Filesystem_Backend::FormatStatus AG_Filesystem_FATFS::get_format_status(void) const
{
