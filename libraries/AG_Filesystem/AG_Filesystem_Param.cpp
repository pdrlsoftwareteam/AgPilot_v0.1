...
int AG_Filesystem_Param::open(const char *fname, int flags, bool allow_absolute_path)
{
...
int AG_Filesystem_Param::close(int fd)
{
...
/*
  packed format:
    ...
    uint8_t type:4;         // AG_Param type NONE=0, INT8=1, INT16=2, INT32=3, FLOAT=4
 ...
 */
uint8_t AG_Filesystem_Param::pack_param(const struct rfile &r, struct cursor &c, uint8_t *buf)
{
    ...
    if (c.token_ofs == 0) {
        c.idx = 0;
        ap = AG_Param::first(&c.token, &ptype, &default_val);
        uint16_t idx = 0;
        while (idx < r.start && ap) {
            ap = AG_Param::next_scalar(&c.token, &ptype, &default_val);
            idx++;
        }
    } else {
        c.idx++;
    ...
    if (ap == nullptr || (r.count && c.idx >= r.count)) {
        if (r.count == 0 && c.idx != AG_Param::count_parameters()) {
...
#endif
    const uint8_t type_len = AG_Param::type_size(ptype);
 ...
 */
bool AG_Filesystem_Param::token_seek(const struct rfile &r, const uint32_t data_ofs, struct cursor &c)
{
    if (data_ofs == 0) {
        memset(&c, 0, sizeof(c));
        return true;
...
int32_t AG_Filesystem_Param::read(int fd, void *buf, uint32_t count)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
    ...
    if (r.file_ofs < sizeof(struct header)) {
        struct header hdr;
        hdr.total_params = AG_Param::count_parameters();
...
int32_t AG_Filesystem_Param::lseek(int fd, int32_t offset, int seek_from)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
int AG_Filesystem_Param::stat(const char *name, struct stat *stbuf)
{
    ...
    stbuf->st_size = AG_Param::count_parameters() * 12;
    return 0;
}
 ...
 */
bool AG_Filesystem_Param::check_file_name(const char *name)
{
 ...
 */
int32_t AG_Filesystem_Param::write(int fd, const void *buf, uint32_t count)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
 ...
 */
bool AG_Filesystem_Param::param_upload_parse(const rfile &r, bool &need_retry)
{
    need_retry = false;
    ...
    for (uint16_t i=0; i<hdr.num_params; i++) {
        ...
        AG_Param *p = AG_Param::find(name, &ptype2, &flags2);
        if (p == nullptr) {
...
/*
  parse incoming parameters
 */
bool AG_Filesystem_Param::finish_upload(const rfile &r)
{
    uint8_t loops = 0;
    while (loops++ < 4) {
        bool need_retry;
