...
int AG_Filesystem_Posix::open(const char *fname, int flags, bool allow_absolute_paths)
{
    FS_CHECK_ALLOWED(-1);
...
int AG_Filesystem_Posix::close(int fd)
{
    FS_CHECK_ALLOWED(-1);
    return ::close(fd);
}

int32_t AG_Filesystem_Posix::read(int fd, void *buf, uint32_t count)
{
    FS_CHECK_ALLOWED(-1);
    return ::read(fd, buf, count);
}

int32_t AG_Filesystem_Posix::write(int fd, const void *buf, uint32_t count)
{
    FS_CHECK_ALLOWED(-1);
    return ::write(fd, buf, count);
}

int AG_Filesystem_Posix::fsync(int fd)
{
    FS_CHECK_ALLOWED(-1);
    return ::fsync(fd);
}

int32_t AG_Filesystem_Posix::lseek(int fd, int32_t offset, int seek_from)
{
    FS_CHECK_ALLOWED(-1);
    return ::lseek(fd, offset, seek_from);
}

int AG_Filesystem_Posix::stat(const char *pathname, struct stat *stbuf)
{
    FS_CHECK_ALLOWED(-1);
...
int AG_Filesystem_Posix::unlink(const char *pathname)
{
    FS_CHECK_ALLOWED(-1);
    pathname = map_filename(pathname);
...
int AG_Filesystem_Posix::mkdir(const char *pathname)
{
    FS_CHECK_ALLOWED(-1);
    pathname = map_filename(pathname);
...
void *AG_Filesystem_Posix::opendir(const char *pathname)
{
...
struct dirent *AG_Filesystem_Posix::readdir(void *dirp)
{
    FS_CHECK_ALLOWED(nullptr);
    return ::readdir((DIR *)dirp);
}

int AG_Filesystem_Posix::closedir(void *dirp)
{
    FS_CHECK_ALLOWED(-1);
    return ::closedir((DIR *)dirp);
}

int AG_Filesystem_Posix::rename(const char *oldpath, const char *newpath)
{
    FS_CHECK_ALLOWED(-1);
...
int64_t AG_Filesystem_Posix::disk_free(const char *path)
{
    FS_CHECK_ALLOWED(-1);
...
int64_t AG_Filesystem_Posix::disk_space(const char *path)
{
    FS_CHECK_ALLOWED(-1);
    path = map_filename(path);
    struct statfs stats;
...
/*
  set mtime on a file
 */
bool AG_Filesystem_Posix::set_mtime(const char *filename, const uint32_t mtime_sec)
{
