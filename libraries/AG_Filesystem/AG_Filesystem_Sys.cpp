...
int8_t AG_Filesystem_Sys::file_in_sysfs(const char *fname) {
...
int AG_Filesystem_Sys::open(const char *fname, int flags, bool allow_absolute_paths)
{
...
int AG_Filesystem_Sys::close(int fd)
{
...
int32_t AG_Filesystem_Sys::read(int fd, void *buf, uint32_t count)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
int32_t AG_Filesystem_Sys::lseek(int fd, int32_t offset, int seek_from)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
void *AG_Filesystem_Sys::opendir(const char *pathname)
{
...
struct dirent *AG_Filesystem_Sys::readdir(void *dirp)
{
...
int AG_Filesystem_Sys::closedir(void *dirp)
{
    if (dirp == nullptr) {
        errno = EINVAL;
        return -1;
...
int AG_Filesystem_Sys::stat(const char *pathname, struct stat *stbuf)
{
