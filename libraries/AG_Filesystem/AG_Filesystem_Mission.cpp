...
int AG_Filesystem_Mission::open(const char *fname, int flags, bool allow_absolute_paths)
{
...
int AG_Filesystem_Mission::close(int fd)
{
 ...
 */
int32_t AG_Filesystem_Mission::read(int fd, void *buf, uint32_t count)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
int32_t AG_Filesystem_Mission::lseek(int fd, int32_t offset, int seek_from)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
int AG_Filesystem_Mission::stat(const char *name, struct stat *stbuf)
{
    enum MAV_MISSION_TYPE mtype;
 ...
 */
bool AG_Filesystem_Mission::check_file_name(const char *name, enum MAV_MISSION_TYPE &mtype)
{
...
/*
  get one item
 */
bool AG_Filesystem_Mission::get_item(uint32_t idx, enum MAV_MISSION_TYPE mtype, mavlink_mission_item_int_t &item) const
{
    switch (mtype) {
...
// get number of items
uint32_t AG_Filesystem_Mission::get_num_items(enum MAV_MISSION_TYPE mtype) const
{
    switch (mtype) {
    case MAV_MISSION_TYPE_MISSION: {
        auto *mission = AP::mission();
 ...
 */
int32_t AG_Filesystem_Mission::write(int fd, const void *buf, uint32_t count)
{
    if (fd < 0 || fd >= max_open_file || !file[fd].open) {
        errno = EBADF;
        return -1;
...
bool AG_Filesystem_Mission::all_zero(const uint8_t *b, uint8_t len) const
{
    while (len--) {
        if (*b++ != 0) {
            return false;
 ...
 */
bool AG_Filesystem_Mission::finish_upload(const rfile &r)
{
    ...
    for (uint32_t i=0; i<nitems; i++) {
        ...
        const MAV_MISSION_RESULT res = AG_Mission::mavlink_int_to_mission_cmd(m, cmd);
