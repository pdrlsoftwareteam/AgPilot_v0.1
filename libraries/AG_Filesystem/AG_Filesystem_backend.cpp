...
*/
FileData *AG_Filesystem_Backend::load_file(const char *filename)
{
    struct stat st;
...
*/
void AG_Filesystem_Backend::unload_file(FileData *fd)
{
    if (fd->data != nullptr) {
...
bool AG_Filesystem_Backend::file_op_allowed(void) const
{
...
{
    if (backend != nullptr) {
        ((AG_Filesystem_Backend *)backend)->unload_file(this);
    }
}
