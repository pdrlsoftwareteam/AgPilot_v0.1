/* 
   AP_Logger Remote(via AGPILOTLink) logging
*/
...
// initialisation
void AP_Logger_AGPILOTLink::Init()
{
    _blocks = nullptr;
...
bool AP_Logger_AGPILOTLink::logging_failed() const
{
    return !_sending_to_client;
}

uint32_t AP_Logger_AGPILOTLink::bufferspace_available() {
    return (_blockcount_free * 200 + remaining_space_in_current_block());
}

uint8_t AP_Logger_AGPILOTLink::remaining_space_in_current_block() const {
    // note that _current_block *could* be NULL ATM.
    return (AGPILOTLINK_MSG_REMOTE_LOG_DATA_BLOCK_FIELD_DATA_LEN - _latest_block_len);
}

void AP_Logger_AGPILOTLink::enqueue_block(dm_block_queue_t &queue, struct dm_block *block)
{
...
struct AP_Logger_AGPILOTLink::dm_block *AP_Logger_AGPILOTLink::dequeue_seqno(AP_Logger_AGPILOTLink::dm_block_queue_t &queue, uint32_t seqno)
{
...
bool AP_Logger_AGPILOTLink::free_seqno_from_queue(uint32_t seqno, dm_block_queue_t &queue)
{
...
bool AP_Logger_AGPILOTLink::WritesOK() const
{
...
bool AP_Logger_AGPILOTLink::_WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical)
{
    ...
    while (copied < size) {
        ...
        if (_latest_block_len == AGPILOTLINK_MSG_REMOTE_LOG_DATA_BLOCK_FIELD_DATA_LEN) {
...
//Get a free block
struct AP_Logger_AGPILOTLink::dm_block *AP_Logger_AGPILOTLink::next_block()
{
    AP_Logger_AGPILOTLink::dm_block *ret = _blocks_free;
    if (ret != nullptr) {
...
void AP_Logger_AGPILOTLink::free_all_blocks()
{
...
void AP_Logger_AGPILOTLink::stop_logging()
{
    if (_sending_to_client) {
...
void AP_Logger_AGPILOTLink::handle_ack(const GCS_AGPILOTLINK &link,
                                   ...
                                   uint32_t seqno)
{
    ...
    if(seqno == AGPILOT_REMOTE_LOG_DATA_BLOCK_STOP) {
        Debug("Received stop-logging packet");
        stop_logging();
        return;
    }
    if(seqno == AGPILOT_REMOTE_LOG_DATA_BLOCK_START) {
        if (!_sending_to_client) {
...
void AP_Logger_AGPILOTLink::remote_log_block_status_msg(const GCS_AGPILOTLINK &link,
                                                    const mavlink_message_t& msg)
{
    ...
    switch ((AGPILOT_REMOTE_LOG_DATA_BLOCK_STATUSES)packet.status) {
        case AGPILOT_REMOTE_LOG_DATA_BLOCK_NACK:
            handle_retry(packet.seqno);
            break;
        case AGPILOT_REMOTE_LOG_DATA_BLOCK_ACK:
            handle_ack(link, msg, packet.seqno);
            break;
        // we apparently have to handle an END enum entry, just drop it so we catch future additions
        case AGPILOT_REMOTE_LOG_DATA_BLOCK_STATUSES_ENUM_END:
            break;
    }
    semaphore.give();
}

void AP_Logger_AGPILOTLink::handle_retry(uint32_t seqno)
{
...
void AP_Logger_AGPILOTLink::stats_init() {
    _dropped = 0;
    stats.resends = 0;
    stats_reset();
}
void AP_Logger_AGPILOTLink::stats_reset() {
    stats.state_free = 0;
...
void AP_Logger_AGPILOTLink::Write_logger_AGPILOT(AP_Logger_AGPILOTLink &logger_mav)
{
    if (logger_mav.stats.collection_count == 0) {
        return;
    }
    const struct log_AGPILOT_Stats pkt{
        LOG_PACKET_HEADER_INIT(LOG_AGPILOT_STATS),
...
void AP_Logger_AGPILOTLink::stats_log()
{
    if (!_initialised) {
        return;
    }
...
uint8_t AP_Logger_AGPILOTLink::stack_size(struct dm_block *stack)
{
    uint8_t ret = 0;
...
uint8_t AP_Logger_AGPILOTLink::queue_size(dm_block_queue_t queue)
{
    return stack_size(queue.oldest);
}

void AP_Logger_AGPILOTLink::stats_collect()
{
    if (!_initialised) {
        return;
    }
...
*/
bool AP_Logger_AGPILOTLink::send_log_blocks_from_queue(dm_block_queue_t &queue)
{
    ...
    while (queue.oldest != nullptr) {
        ...
        queue.sent_count++;
        struct AP_Logger_AGPILOTLink::dm_block *tmp = dequeue_seqno(queue,queue.oldest->seqno);
...
void AP_Logger_AGPILOTLink::push_log_blocks()
{
    if (!_initialised || !_sending_to_client) {
        return;
    }
...
void AP_Logger_AGPILOTLink::do_resends(uint32_t now)
{
    if (!_initialised || !_sending_to_client) {
        return;
    }
...
// appropriately!
void AP_Logger_AGPILOTLink::periodic_10Hz(const uint32_t now)
{
    do_resends(now);
    stats_collect();
}
void AP_Logger_AGPILOTLink::periodic_1Hz()
{
...
bool AP_Logger_AGPILOTLink::send_log_block(struct dm_block &block)
{
    if (!_initialised) {
       return false;
    }
    ...
    static_assert(AGPILOTLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN <= min_payload_space,
...
#if DF_AGPILOTLINK_DISABLE_INTERRUPTS
