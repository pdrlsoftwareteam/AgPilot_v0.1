/* 
   AP_Logger logging - AGPILOTLink variant

   - transfers blocks of the open log file to a client using AGPILOTLink
 */
#pragma once
...
#define DF_AGPILOTLINK_DISABLE_INTERRUPTS 0

class AP_Logger_AGPILOTLink : public AP_Logger_Backend
{
public:
    // constructor
    AP_Logger_AGPILOTLink(AP_Logger &front, LoggerMessageWriter_DFLogStart *writer) :
    ...
    static AP_Logger_Backend  *probe(AP_Logger &front,
        ...
        return new AP_Logger_AGPILOTLink(front, ls);
    }
    ...
    void remote_log_block_status_msg(const GCS_AGPILOTLINK &link, const mavlink_message_t& msg) override;
...
private:
    ...
    struct dm_block {
        uint32_t seqno;
        uint8_t buf[AGPILOTLINK_MSG_REMOTE_LOG_DATA_BLOCK_FIELD_DATA_LEN];
        uint32_t last_sent;
        struct dm_block *next;
    };
    bool send_log_block(struct dm_block &block);
    void handle_ack(const GCS_AGPILOTLINK &link, const mavlink_message_t &msg, uint32_t seqno);
    ...
    void Write_logger_AGPILOT(AP_Logger_AGPILOTLink &logger);
...
#endif // HAL_LOGGING_AGPILOTLINK_ENABLED
