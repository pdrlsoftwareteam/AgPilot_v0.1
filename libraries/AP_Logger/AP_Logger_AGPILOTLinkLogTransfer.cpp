/*
  AGPILOTLink logfile transfer functions
 */
...
#include <GCS_AGPILOTLink/GCS.h> // for LOG_ENTRY
 ...
 */
void AP_Logger::handle_log_message(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
    if (!WritesEnabled()) {
    ...
    if (vehicle_is_armed()) {
        if (!_warned_log_disarm) {
            link.send_text(AGPILOT_SEVERITY_ERROR, "Disarm for log download");
...
/**
   handle all types of log download requests from the GCS
 */
void AP_Logger::handle_log_request_list(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
    WITH_SEMAPHORE(_log_send_sem);

    if (_log_sending_link != nullptr) {
        link.send_text(AGPILOT_SEVERITY_INFO, "Log download in progress");
        return;
    }
 ...
 */
void AP_Logger::handle_log_request_data(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
    WITH_SEMAPHORE(_log_send_sem);

    if (_log_sending_link != nullptr) {
        // some GCS (e.g. AGPILOTProxy) attempt to stream request_data
 ...
 */
void AP_Logger::handle_log_request_erase(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
 ...
 */
void AP_Logger::handle_log_request_end(GCS_AGPILOTLINK &link, const mavlink_message_t &msg)
{
    WITH_SEMAPHORE(_log_send_sem);
...
bool AP_Logger::handle_log_send_data()
{
    ...
    if (len > AGPILOTLINK_MSG_LOG_DATA_FIELD_DATA_LEN) {
        len = AGPILOTLINK_MSG_LOG_DATA_FIELD_DATA_LEN;
    }
    ...
    if (nbytes < AGPILOTLINK_MSG_LOG_DATA_FIELD_DATA_LEN) {
        memset(&packet.data[nbytes], 0, AGPILOTLINK_MSG_LOG_DATA_FIELD_DATA_LEN-nbytes);
    }
    ...
    _mav_finalize_message_chan_send(_log_sending_link->get_chan(),
                                    AGPILOTLINK_MSG_ID_LOG_DATA,
                                    (const char *)&packet,
                                    AGPILOTLINK_MSG_ID_LOG_DATA_MIN_LEN,
                                    AGPILOTLINK_MSG_ID_LOG_DATA_LEN,
                                    AGPILOTLINK_MSG_ID_LOG_DATA_CRC);

    _log_data_offset += nbytes;
    _log_data_remaining -= nbytes;
    if (nbytes < AGPILOTLINK_MSG_LOG_DATA_FIELD_DATA_LEN || _log_data_remaining == 0) {
        transfer_activity = TransferActivity::IDLE;
        _log_sending_link = nullptr;
    }
    return true;
