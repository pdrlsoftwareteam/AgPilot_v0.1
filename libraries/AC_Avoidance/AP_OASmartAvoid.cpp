...
// parameter defaults
const float OA_SMARTRULER_LOOKAHEAD_DEFAULT = 15.0f;
const float OA_SMARTRULER_RATIO_DEFAULT = 1.5f;
const int16_t OA_SMARTRULER_ANGLE_DEFAULT = 75;
const int16_t OA_SMARTRULER_TYPE_DEFAULT = 1;

const int16_t OA_SMARTRULER_BEARING_INC_XY = 5;            // check every 5 degrees around vehicle
const int16_t OA_SMARTRULER_BEARING_INC_VERTICAL = 90;
const float OA_SMARTRULER_LOOKAHEAD_STEP2_RATIO = 1.0f; // step2's lookahead length as a ratio of step1's lookahead length
const float OA_SMARTRULER_LOOKAHEAD_STEP2_MIN = 2.0f;   // step2 checks at least this many meters past step1's location
const float OA_SMARTRULER_LOOKAHEAD_PAST_DEST = 2.0f;   // lookahead length will be at least this many meters past the destination
const float OA_SMARTRULER_LOW_SPEED_SQUARED = (0.2f * 0.2f);    // when ground course is below this speed squared, vehicle's heading will be used

#define VERTICAL_ENABLED APM_BUILD_COPTER_OR_HELI

const AP_Param::GroupInfo AP_OASmartAvoid::var_info[] = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("LOOKAHEAD", 1, AP_OASmartAvoid, _lookahead, OA_SMARTRULER_LOOKAHEAD_DEFAULT),

    // @Param: CONT_RATIO
    // @DisplayName: Obstacle Avoidance margin ratio for SmartRuler to change bearing significantly
    // @Description:  SmartRuler will avoid changing bearing unless ratio of previous margin from obstacle (or fence) to present calculated margin is atleast this much.
    // @Range: 1.1 2
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("CONT_RATIO", 2, AP_OASmartAvoid, _smart_ratio, OA_SMARTRULER_RATIO_DEFAULT),

    // @Param: CONT_ANGLE
    // @DisplayName: SmartRuler's bearing change resistance threshold angle
    // @Description:  SmartRuler will resist changing current bearing if the change in bearing is over this angle
    // @Range: 20 180
    // @Increment: 5
    // @User: Standard
    AP_GROUPINFO("CONT_ANGLE", 3, AP_OASmartAvoid, _smart_angle, OA_SMARTRULER_ANGLE_DEFAULT),

    // @Param{Copter}: TYPE
    // @DisplayName: Type of SmartRuler
    // @Description: SmartRuler will search for clear path along the direction defined by this parameter
    // @Values: 1:Horizontal search, 2:Vertical search
    // @User: Standard
    AP_GROUPINFO_FRAME("TYPE", 4, AP_OASmartAvoid, _smart_type, OA_SMARTRULER_TYPE_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_HELI | AP_PARAM_FRAME_TRICOPTER),

    AP_GROUPEND
};

AP_OASmartAvoid::AP_OASmartAvoid()
{
...
// smart_type is set to the type of SmartRuler used
bool AP_OASmartAvoid::update(const Location& current_loc, const Location& destination, const Vector2f &ground_speed_vec, Location &origin_new, Location &destination_new, OASmartType &smart_type, bool proximity_only)
{
    // smart ruler always sets origin to current_loc
    origin_new = current_loc;

    // init smart_type returned
    smart_type = OASmartType::OA_SMART_DISABLED;
    ...
    const float lookahead_step1_dist = MIN(_current_lookahead, distance_to_dest + OA_SMARTRULER_LOOKAHEAD_PAST_DEST);

    // calculate lookahead dist for step2
    const float lookahead_step2_dist = _current_lookahead * OA_SMARTRULER_LOOKAHEAD_STEP2_RATIO;

    // get ground course
    float ground_course_deg;
    if (ground_speed_vec.length_squared() < OA_SMARTRULER_LOW_SPEED_SQUARED) {
    ...
    bool ret;
    switch (get_type()) {
        case OASmartType::OA_SMART_VERTICAL:
        #if VERTICAL_ENABLED
            ...
            smart_type = OASmartType::OA_SMART_VERTICAL;
            break;
        #endif

        case OASmartType::OA_SMART_HORIZONTAL:
        default:
            ...
            smart_type = OASmartType::OA_SMART_HORIZONTAL;
    }
...
bool AP_OASmartAvoid::search_xy_path(const Location& current_loc, const Location& destination, float ground_course_deg, Location &destination_new, float lookahead_step1_dist, float lookahead_step2_dist, float bearing_to_dest, float distance_to_dest, bool proximity_only)
{
    // check OA_BEARING_INC definition allows checking in all directions
    static_assert(360 % OA_SMARTRULER_BEARING_INC_XY == 0, "check 360 is a multiple of OA_BEARING_INC");

    // search in OA_SMARTRULER_BEARING_INC degree increments around the vehicle alternating left
    ...
    for (uint8_t i = 0; i <= (170 / OA_SMARTRULER_BEARING_INC_XY); i++) {
        for (uint8_t bdir = 0; bdir <= 1; bdir++) {
            ...
            const float bearing_delta = i * OA_SMARTRULER_BEARING_INC_XY * (bdir == 0 ? -1.0f : 1.0f);
            ...
            if (margin > _margin_max) {
                ...
                float distance2 = constrain_float(lookahead_step2_dist, OA_SMARTRULER_LOOKAHEAD_STEP2_MIN, test_loc.get_distance(destination));
                for (uint8_t j = 0; j < ARRAY_SIZE(test_bearings); j++) {
                    ...
                    if (margin2 > _margin_max) {
                        ...
                        Write_OASmartAvoid((uint8_t)OASmartType::OA_SMART_HORIZONTAL, active, bearing_to_dest, 0.0f, ignore_bearing_change, final_margin, destination, destination_new);
                        return active;
                    }
                }
            }
    ...
    // log results
    Write_OASmartAvoid((uint8_t)OASmartType::OA_SMART_HORIZONTAL, true, chosen_bearing, 0.0f, false, best_margin, destination, destination_new);

    return true;
}

// Search for path in the vertical directions
bool AP_OASmartAvoid::search_vertical_path(const Location &current_loc, const Location &destination, Location &destination_new, float lookahead_step1_dist, float lookahead_step2_dist, float bearing_to_dest, float distance_to_dest, bool proximity_only)
{
    // check OA_BEARING_INC_VERTICAL definition allows checking in all directions
    static_assert(360 % OA_SMARTRULER_BEARING_INC_VERTICAL == 0, "check 360 is a multiple of OA_BEARING_INC_VERTICAL");
    float best_pitch = 0.0f;
    bool  have_best_pitch = false;
    float best_margin = -FLT_MAX;
    float best_margin_pitch = best_pitch;
    const uint8_t angular_limit = 180 / OA_SMARTRULER_BEARING_INC_VERTICAL;
    ...
    for (uint8_t i = 0; i <= angular_limit; i++) {
        for (uint8_t bdir = 0; bdir <= 1; bdir++) {
            ...
            // bearing that we are probing
            const float pitch_delta = i * OA_SMARTRULER_BEARING_INC_VERTICAL * (bdir == 0 ? 1.0f : -1.0f);
            ...
            if (margin > _margin_max) {
                ...
                for (uint8_t j = 0; j < ARRAY_SIZE(test_pitch_step2); j++) {
                    ...
                    if (margin2 > _margin_max) {
                        ...
                        bool active = (i != 0 || j != 0);
                        if (!active) {
                            // do a sub test for proximity obstacles to confirm if we should really turn of SmartRuler
                        ...
                        Write_OASmartAvoid((uint8_t)OASmartType::OA_SMART_VERTICAL, active, bearing_to_dest, pitch_delta, false, margin, destination, destination_new);
                        return active;
                    }
                }
            }
    ...
    // log results
    Write_OASmartAvoid((uint8_t)OASmartType::OA_SMART_VERTICAL, true, bearing_to_dest, chosen_pitch,false, best_margin, destination, destination_new);

    return true;
}

AP_OASmartAvoid::OASmartType AP_OASmartAvoid::get_type() const
{
    switch (_smart_type) {
        case (uint8_t)OASmartType::OA_SMART_VERTICAL:
        #if VERTICAL_ENABLED
            return OASmartType::OA_SMART_VERTICAL;
        #endif

        case (uint8_t)OASmartType::OA_SMART_HORIZONTAL:
        default:
            return OASmartType::OA_SMART_HORIZONTAL;
    }
...
/*
This function is called when SmartRuler has found a bearing which is obstacles free at atleast lookahead_step1_dist and  then lookahead_step2_dist from the present location
In many situations, this new bearing can be either left or right of the obstacle, and SmartRuler can have a tough time deciding between the two.
...
Therefore, this method attempts to avoid changing direction of the vehicle by more than _smart_angle degrees,
unless the new margin is atleast _smart_ratio times better than the margin with previously calculated bearing.
We return true if we have resisted the change and will follow the last calculated bearing.
*/
bool AP_OASmartAvoid::resist_bearing_change(const Location &destination, const Location &current_loc, bool active, float bearing_test, float lookahead_step1_dist, float margin, Location &prev_dest, float &prev_bearing, float &final_bearing, float &final_margin, bool proximity_only) const
{
    ...
    if (active && !dest_change && is_positive(_smart_ratio)) {
        // check the change in bearing between freshly calculated and previous stored SmartRuler bearing
        if ((fabsf(wrap_180(prev_bearing-bearing_test)) > _smart_angle) && (!is_equal(prev_bearing,FLT_MAX))) {
            ...
            if (margin < (_smart_ratio * previous_bearing_margin)) {
        ...
        // reset stored bearing if SmartRuler is not active or if WP has changed for unnecessary resistance to path change
        prev_bearing = FLT_MAX;
    }
    if (!resisted_change) {
        // we are not resisting the change, hence store SmartRuler's presently calculated bearing for future iterations
...
float AP_OASmartAvoid::calc_avoidance_margin(const Location &start, const Location &end, bool proximity_only) const
{
    ...
    if (get_type() == OASmartType::OA_SMART_VERTICAL) {
...
bool AP_OASmartAvoid::calc_margin_from_circular_fence(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
...
// on success returns true and updates margin
bool AP_OASmartAvoid::calc_margin_from_alt_fence(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
    // exit immediately if polygon fence is not enabled
    const AC_Fence *fence = AC_Fence::get_singleton();
...
// on success returns true and updates margin
bool AP_OASmartAvoid::calc_margin_from_inclusion_and_exclusion_polygons(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
...
// on success returns true and updates margin
bool AP_OASmartAvoid::calc_margin_from_inclusion_and_exclusion_circles(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
...
// on success returns true and updates margin
bool AP_OASmartAvoid::calc_margin_from_object_database(const Location &start, const Location &end, float &margin) const
{
