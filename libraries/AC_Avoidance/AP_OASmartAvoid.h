...
public:
    ...
    bool update(const Location& current_loc, const Location& destination, const Vector2f &ground_speed_vec, Location &origin_new, Location &destination_new, OASmartType &smart_type, bool proximity_only);

    static const struct AP_Param::GroupInfo var_info[];

private:

    // return type of SmartRuler in use
    OASmartType get_type() const;
    ...
    // determine if SmartRuler should accept the new bearing or try and resist it. Returns true if bearing is not changed
    ...
    // Logging function
    void Write_OASmartAvoid(const uint8_t type, const bool active, const float target_yaw, const float target_pitch, const bool resist_chg, const float margin, const Location &final_dest, const Location &oa_dest) const;

    // OA common parameters
    float _margin_max;              // object avoidance will ignore objects more than this many meters from vehicle

    // SmartRuler parameters
    AP_Float _lookahead;            // object avoidance will look this many meters ahead of vehicle
    AP_Float _smart_ratio;          // object avoidance will avoid major directional change if change in margin ratio is less than this param
    AP_Int16 _smart_angle;          // object avoidance will try avoding change in direction over this much angle
    AP_Int8  _smart_type;           // Type of SmartRuler to run
