...
void AP_WindVane_Analog::calibrate()
{
    ...
    if (_cal_start_ms == 0) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "WindVane: Calibration started, rotate wind vane");
    }
    ...
    if ((AP_HAL::millis() - _cal_start_ms) > 30000) {
        ...
        if (volt_diff >= WINDVANE_CALIBRATION_VOLT_DIFF_MIN) {
            ...
            gcs().send_text(AGPILOT_SEVERITY_INFO, "WindVane: Calibration complete (volt min:%.1f max:%1.f)",
            (double)_cal_volt_min,
            (double)_cal_volt_max);
        } else {
             gcs().send_text(AGPILOT_SEVERITY_INFO, "WindVane: Calibration failed (volt diff %.1f below %.1f)",
            (double)volt_diff,
