...
#include <AG_RCProtocol/AG_RCProtocol.h>
#include <AG_MSP/AG_MSP.h>
#include "AG_SerialManager.h"
...
const AG_Param::GroupInfo AG_SerialManager::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("0_BAUD",  0, AG_SerialManager, state[0].baud, AP_SERIALMANAGER_CONSOLE_BAUD/1000),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("0_PROTOCOL",  11, AG_SerialManager, state[0].protocol, SerialProtocol_MAVLink2),
#endif
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("1_PROTOCOL",  1, AG_SerialManager, state[1].protocol, SerialProtocol_MAVLink2),
    ...
    // @Values: 1:1200,2:2400,4:4800,9:9600,19:19200,38:38400,57:57600,111:111100,115:115200,230:230400,256:256000,460:460800,500:500000,921:921600,1500:1500000,2000:2000000
    // @User: Standard
    AP_GROUPINFO("1_BAUD", 2, AG_SerialManager, state[1].baud, AP_SERIALMANAGER_MAVLINK_BAUD/1000),
#endif
    ...
    AP_GROUPINFO("2_PROTOCOL",  3, AG_SerialManager, state[2].protocol, SERIAL2_PROTOCOL),
    ...
    AP_GROUPINFO("2_BAUD", 4, AG_SerialManager, state[2].baud, AP_SERIALMANAGER_MAVLINK_BAUD/1000),
#endif
    ...
    AP_GROUPINFO("3_PROTOCOL",  5, AG_SerialManager, state[3].protocol, SERIAL3_PROTOCOL),
    ...
    AP_GROUPINFO("3_BAUD", 6, AG_SerialManager, state[3].baud, AP_SERIALMANAGER_GPS_BAUD/1000),
#endif
    ...
    AP_GROUPINFO("4_PROTOCOL",  7, AG_SerialManager, state[4].protocol, SERIAL4_PROTOCOL),
    ...
    AP_GROUPINFO("4_BAUD", 8, AG_SerialManager, state[4].baud, AP_SERIALMANAGER_GPS_BAUD/1000),
#endif
    ...
    AP_GROUPINFO("5_PROTOCOL",  9, AG_SerialManager, state[5].protocol, SERIAL5_PROTOCOL),
    ...
    AP_GROUPINFO("5_BAUD", 10, AG_SerialManager, state[5].baud, SERIAL5_BAUD),
#endif
    ...
    AP_GROUPINFO("6_PROTOCOL",  12, AG_SerialManager, state[6].protocol, SERIAL6_PROTOCOL),
    ...
    AP_GROUPINFO("6_BAUD", 13, AG_SerialManager, state[6].baud, SERIAL6_BAUD),
#endif
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("1_OPTIONS",  14, AG_SerialManager, state[1].options, 0),
#endif
    ...
    AP_GROUPINFO("2_OPTIONS",  15, AG_SerialManager, state[2].options, 0),
#endif
    ...
    AP_GROUPINFO("3_OPTIONS",  16, AG_SerialManager, state[3].options, 0),
#endif
    ...
    AP_GROUPINFO("4_OPTIONS",  17, AG_SerialManager, state[4].options, 0),
#endif
    ...
    AP_GROUPINFO("5_OPTIONS",  18, AG_SerialManager, state[5].options, 0),
#endif
    ...
    AP_GROUPINFO("6_OPTIONS",  19, AG_SerialManager, state[6].options, 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("_PASS1",  20, AG_SerialManager, passthru_port1, 0),
    ...
    // @Values: -1:Disabled,0:Serial0,1:Serial1,2:Serial2,3:Serial3,4:Serial4,5:Serial5,6:Serial6
    // @User: Advanced
    AP_GROUPINFO("_PASS2",  21, AG_SerialManager, passthru_port2, -1),
    ...
    // @Units: s
    // @User: Advanced
    AP_GROUPINFO("_PASSTIMO",  22, AG_SerialManager, passthru_timeout, 15),
    ...
    AP_GROUPINFO("7_PROTOCOL",  23, AG_SerialManager, state[7].protocol, SERIAL7_PROTOCOL),
    ...
    AP_GROUPINFO("7_BAUD", 24, AG_SerialManager, state[7].baud, SERIAL7_BAUD),

    // @Param: 7_OPTIONS
    // @CopyFieldsFrom: SERIAL1_OPTIONS
    // @DisplayName: Serial7 options
    AP_GROUPINFO("7_OPTIONS",  25, AG_SerialManager, state[7].options, 0),
#endif
    ...
    AP_GROUPINFO("8_PROTOCOL",  26, AG_SerialManager, state[8].protocol, SERIAL8_PROTOCOL),
    ...
    AP_GROUPINFO("8_BAUD", 27, AG_SerialManager, state[8].baud, SERIAL8_BAUD),

    // @Param: 8_OPTIONS
    // @CopyFieldsFrom: SERIAL1_OPTIONS
    // @DisplayName: Serial8 options
    AP_GROUPINFO("8_OPTIONS",  28, AG_SerialManager, state[8].options, 0),
#endif
    ...
    AP_GROUPINFO("9_PROTOCOL",  29, AG_SerialManager, state[9].protocol, SERIAL9_PROTOCOL),

    // @Param: 9_BAUD
    // @CopyFieldsFrom: SERIAL1_BAUD
    // @DisplayName: Serial 9 Baud Rate
    // @Description: The baud rate used for Serial8. Most stm32-based boards can support rates of up to 1500. If you setup a rate you cannot support and then can't connect to your board you should load a firmware from a different vehicle type. That will reset all your parameters to defaults.
    AP_GROUPINFO("9_BAUD", 30, AG_SerialManager, state[9].baud, SERIAL9_BAUD),

    // @Param: 9_OPTIONS
    // @CopyFieldsFrom: SERIAL1_OPTIONS
    // @DisplayName: Serial9 options
    AP_GROUPINFO("9_OPTIONS",  31, AG_SerialManager, state[9].options, 0),
#endif
...
// singleton instance
AG_SerialManager *AG_SerialManager::_singleton;

// Constructor
AG_SerialManager::AG_SerialManager()
{
    _singleton = this;
...
void AG_SerialManager::init_console()
{
        ...
        if (uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_MSP_DisplayPort:
                    ...
                    uart->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    // Note init is handled by AG_MSP
                    break;
#endif
                default:
                    uart->begin(state[i].baudrate());
...
const AG_SerialManager::UARTState *AG_SerialManager::find_protocol_instance(enum SerialProtocol protocol, uint8_t instance) const
{
...
AG_HAL::UARTDriver *AG_SerialManager::find_serial(enum SerialProtocol protocol, uint8_t instance) const
{
    ...
    AG_HAL::UARTDriver *port = hal.serial(serial_idx);
    if (port) {
...
bool AG_SerialManager::have_serial(enum SerialProtocol protocol, uint8_t instance) const
{
...
uint32_t AG_SerialManager::find_baudrate(enum SerialProtocol protocol, uint8_t instance) const
{
    const struct UARTState *_state = find_protocol_instance(protocol, instance);
    if (_state == nullptr) {
        return 0;
...
int8_t AG_SerialManager::find_portnum(enum SerialProtocol protocol, uint8_t instance) const
{
    const struct UARTState *_state = find_protocol_instance(protocol, instance);
    if (_state == nullptr) {
        return -1;
...
AG_HAL::UARTDriver *AG_SerialManager::get_serial_by_id(uint8_t id)
{
...
void AG_SerialManager::set_blocking_writes_all(bool blocking)
{
 ...
 *  reliable.
 */
uint32_t AG_SerialManager::map_baudrate(int32_t rate)
{
    if (rate <= 0) {
        rate = 57;
    }
...
bool AG_SerialManager::protocol_match(enum SerialProtocol protocol1, enum SerialProtocol protocol2) const
{
...
void AG_SerialManager::set_options(uint16_t i)
{
...
bool AG_SerialManager::get_passthru(AG_HAL::UARTDriver *&port1, AG_HAL::UARTDriver *&port2, uint8_t &timeout_s,
...
void AG_SerialManager::disable_passthru(void)
{
    passthru_port2.set_and_notify(-1);
}

// accessor for AP_Periph to set baudrate and type
void AG_SerialManager::set_protocol_and_baud(uint8_t sernum, enum SerialProtocol protocol, uint32_t baudrate)
{
...
AG_SerialManager &serialmanager()
{
    return *AG_SerialManager::get_singleton();
}
