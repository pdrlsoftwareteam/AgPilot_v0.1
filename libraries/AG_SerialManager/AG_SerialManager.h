...
class AG_SerialManager {
public:
    AG_SerialManager();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_SerialManager);
    ...
    static AG_SerialManager *get_singleton(void) {
        return _singleton;
    }
    ...
    AG_HAL::UARTDriver *find_serial(enum SerialProtocol protocol, uint8_t instance) const;
    ...
    bool get_passthru(AG_HAL::UARTDriver *&port1, AG_HAL::UARTDriver *&port2, uint8_t &timeout_s,
    ...
    // get Serial Port
    AG_HAL::UARTDriver *get_serial_by_id(uint8_t id);
    ...
    public:
        ...
        uint32_t baudrate() const {
            return AG_SerialManager::map_baudrate(baud);
        }
        AG_SerialManager::SerialProtocol get_protocol() const {
            return AG_SerialManager::SerialProtocol(protocol.get());
        }
    private:
        AP_Int32 baud;
        AP_Int16 options;
...
private:
    static AG_SerialManager *_singleton;
