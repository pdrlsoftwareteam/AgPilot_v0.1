...
const AG_Param::GroupInfo AG_Scheduler::var_info[] = {
    // @Param: DEBUG
    ...
    // @User: Advanced
    AP_GROUPINFO("DEBUG",    0, AG_Scheduler, _debug, 0),
    ...
    // @RebootRequired: True
    // @User: Advanced
    AP_GROUPINFO("LOOP_RATE",  1, AG_Scheduler, _loop_rate_hz, SCHEDULER_DEFAULT_LOOP_RATE),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS",  2, AG_Scheduler, _options, 0),
...
AG_Scheduler::AG_Scheduler()
{
    if (_singleton) {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        AG_HAL::panic("Too many schedulers");
#endif
        return;
    }
    _singleton = this;
...
/*
 * Get the AG_Scheduler singleton
 */
AG_Scheduler *AG_Scheduler::_singleton;
AG_Scheduler *AG_Scheduler::get_singleton()
{
    return _singleton;
}

// initialise the scheduler
void AG_Scheduler::init(const AG_Scheduler::Task *tasks, uint8_t num_tasks, uint32_t log_performance_bit)
{
 ...
 */
void AG_Scheduler::run(uint32_t time_available)
{
    uint32_t run_started_usec = AG_HAL::micros();
    ...
    for (uint8_t i=0; i<_num_tasks; i++) {
        ...
        const AG_Scheduler::Task &task = run_vehicle_task ? _vehicle_tasks[vehicle_tasks_offset] : _common_tasks[common_tasks_offset];
        if (run_vehicle_task) {
 ...
 */
uint16_t AG_Scheduler::time_available_usec(void) const
{
    uint32_t dt = AG_HAL::micros() - _task_time_started;
 ...
 */
float AG_Scheduler::load_average()
{
...
void AG_Scheduler::loop()
{
    ...
    const uint32_t sample_time_us = AG_HAL::micros();
...
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    // move result of AG_HAL::micros() forward:
...
void AG_Scheduler::update_logging()
{
    if (debug_flags()) {
...
void AG_Scheduler::Log_Write_Performance()
{
...
void AG_Scheduler::task_info(ExpandingString &str)
{
...
{
    return *AG_Scheduler::get_singleton();
}
