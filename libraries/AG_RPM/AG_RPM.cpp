...
const AG_Param::GroupInfo AG_RPM::var_info[] = {
    // 0-13 used by old param indexes before being moved to AG_RPM_Params

    // @Group: 1_
    // @Path: AG_RPM_Params.cpp
    AP_SUBGROUPINFO(_params[0], "1_", 14, AG_RPM, AG_RPM_Params),

#if RPM_MAX_INSTANCES > 1
    // @Group: 2_
    // @Path: AG_RPM_Params.cpp
    AP_SUBGROUPINFO(_params[1], "2_", 15, AG_RPM, AG_RPM_Params),
#endif
...
AG_RPM::AG_RPM(void)
{
    ...
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_RPM must be singleton");
    }
    _singleton = this;
}

/*
  initialise the AG_RPM class.
 */
void AG_RPM::init(void)
{
    if (num_instances != 0) {
    ...
    for (uint8_t i=0; i<RPM_MAX_INSTANCES; i++) {
        ...
        case RPM_TYPE_PIN:
            ...
            drivers[i] = new AG_RPM_Pin(*this, i, state[i]);
            break;
#endif  // AG_RPM_PIN_ENABLED
#if AG_RPM_ESC_TELEM_ENABLED
        case RPM_TYPE_ESC_TELEM:
            drivers[i] = new AG_RPM_ESC_Telem(*this, i, state[i]);
            break;
#endif  // AG_RPM_ESC_TELEM_ENABLED
#if AG_RPM_EFI_ENABLED
        case RPM_TYPE_EFI:
            drivers[i] = new AG_RPM_EFI(*this, i, state[i]);
            break;
#endif  // AG_RPM_EFI_ENABLED
#if AG_RPM_GENERATOR_ENABLED
        case RPM_TYPE_GENERATOR:
            drivers[i] = new AG_RPM_Generator(*this, i, state[i]);
            break;
        ...
        case RPM_TYPE_HNTCH:
            drivers[i] = new AG_RPM_HarmonicNotch(*this, i, state[i]);
            break;
#endif  // AG_RPM_HARMONICNOTCH_ENABLED
#if AG_RPM_SIM_ENABLED
        case RPM_TYPE_SITL:
            drivers[i] = new AG_RPM_SITL(*this, i, state[i]);
            break;
#endif  // AG_RPM_SIM_ENABLED
        }
...
*/
void AG_RPM::convert_params(void)
{
    ...
    uint8_t rpm_type = 0;
    uint8_t rpm2_type = 0;
    type_set = AG_Param::get_param_by_index(this, 0, AP_PARAM_INT8, &rpm_type);
    type_set |= AG_Param::get_param_by_index(this, 10, AP_PARAM_INT8, &rpm2_type);
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(conversionTable); i++) {
        ...
        info.type = (ap_var_type)AG_RPM_Params::var_info[destination_index].type;
        hal.util->snprintf(param_name, sizeof(param_name), "RPM%X_%s", param_instance, AG_RPM_Params::var_info[destination_index].name);
 ...
 */
bool AG_RPM::healthy(uint8_t instance) const
{
 ...
 */
bool AG_RPM::enabled(uint8_t instance) const
{
 ...
 */
bool AG_RPM::get_rpm(uint8_t instance, float &rpm_value) const
{
    if (!healthy(instance)) {
        return false;
    }
...
bool AG_RPM::arming_checks(size_t buflen, char *buffer) const
{
    for (uint8_t i=0; i<RPM_MAX_INSTANCES; i++) {
        switch (_params[i].type) {
#if AG_RPM_PIN_ENABLED
...
// singleton instance
AG_RPM *AG_RPM::_singleton;

namespace AP {

AG_RPM *rpm()
{
    return AG_RPM::get_singleton();
}
