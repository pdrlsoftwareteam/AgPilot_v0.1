...
AG_RPM_Pin::IrqState AG_RPM_Pin::irq_state[RPM_MAX_INSTANCES];

/* 
   open the sensor in constructor
*/
AG_RPM_Pin::AG_RPM_Pin(AG_RPM &_ap_rpm, uint8_t instance, AG_RPM::RPM_State &_state) :
	AG_RPM_Backend(_ap_rpm, instance, _state)
{
}
 ...
 */
void AG_RPM_Pin::irq_handler(uint8_t pin, bool pin_state, uint32_t timestamp)
{
...
void AG_RPM_Pin::update(void)
{
    if (last_pin != get_pin()) {
        ...
        if (last_pin > 0) {
            ...
            if (hal.gpio->attach_interrupt(
                    last_pin,
                    FUNCTOR_BIND_MEMBER(&AG_RPM_Pin::irq_handler, void, uint8_t, bool, uint32_t),
    ...
    if (AG_HAL::millis() - state.last_reading_ms > 1000) {
