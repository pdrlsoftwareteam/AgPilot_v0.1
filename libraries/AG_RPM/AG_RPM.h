...
#include "AG_RPM_Params.h"

// Maximum number of RPM measurement instances available on this platform
#define RPM_MAX_INSTANCES 2

class AG_RPM_Backend;

class AG_RPM
{
    friend class AG_RPM_Backend;

public:
    AG_RPM();

    CLASS_NO_COPY(AG_RPM);  /* Do not allow copies */
    ...
    AG_RPM_Params _params[RPM_MAX_INSTANCES];
    ...
    static AG_RPM *get_singleton() { return _singleton; }
...
private:
    ...
    static AG_RPM *_singleton;

    RPM_State state[RPM_MAX_INSTANCES];
    AG_RPM_Backend *drivers[RPM_MAX_INSTANCES];
