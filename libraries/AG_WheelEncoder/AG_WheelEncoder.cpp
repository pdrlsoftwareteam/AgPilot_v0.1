...
const AG_Param::GroupInfo AG_WheelEncoder::var_info[] = {
    // @Param: _TYPE
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("_TYPE", 0, AG_WheelEncoder, _type[0], 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("_CPR",     1, AG_WheelEncoder, _counts_per_revolution[0], WHEELENCODER_CPR_DEFAULT),
    ...
    // @Increment: 0.001
    // @User: Standard
    AP_GROUPINFO("_RADIUS",  2, AG_WheelEncoder, _wheel_radius[0], WHEELENCODER_RADIUS_DEFAULT),
    ...
    // @Increment: 0.01
    // @User: Standard
    AP_GROUPINFO("_POS",     3, AG_WheelEncoder, _pos_offset[0], 0.0f),
    ...
    // @User: Standard
    AP_GROUPINFO("_PINA",    4, AG_WheelEncoder, _pina[0], -1),
    ...
    // @Values: -1:Disabled,50:AUX1,51:AUX2,52:AUX3,53:AUX4,54:AUX5,55:AUX6
    // @User: Standard
    AP_GROUPINFO("_PINB",    5, AG_WheelEncoder, _pinb[0], -1),
    ...
    // @Values: 0:None,1:Quadrature,10:SITL Quadrature
    // @User: Standard
    AP_GROUPINFO("2_TYPE",   6, AG_WheelEncoder, _type[1], 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("2_CPR",     7, AG_WheelEncoder, _counts_per_revolution[1], WHEELENCODER_CPR_DEFAULT),
    ...
    // @Increment: 0.001
    // @User: Standard
    AP_GROUPINFO("2_RADIUS", 8, AG_WheelEncoder, _wheel_radius[1], WHEELENCODER_RADIUS_DEFAULT),
    ...
    // @Increment: 0.01
    // @User: Standard
    AP_GROUPINFO("2_POS",    9, AG_WheelEncoder, _pos_offset[1], 0.0f),
    ...
    // @Values: -1:Disabled,50:AUX1,51:AUX2,52:AUX3,53:AUX4,54:AUX5,55:AUX6
    // @User: Standard
    AP_GROUPINFO("2_PINA",   10, AG_WheelEncoder, _pina[1], 53),
    ...
    // @Values: -1:Disabled,50:AUX1,51:AUX2,52:AUX3,53:AUX4,54:AUX5,55:AUX6
    // @User: Standard
    AP_GROUPINFO("2_PINB",   11, AG_WheelEncoder, _pinb[1], 52),
#endif

    AP_GROUPEND
};

AG_WheelEncoder::AG_WheelEncoder(void)
{
    _singleton = this;
...
// initialise the AG_WheelEncoder class.
void AG_WheelEncoder::init(void)
{
    if (num_instances != 0) {
    ...
    for (uint8_t i=0; i<WHEELENCODER_MAX_INSTANCES; i++) {
        ...
        case WheelEncoder_TYPE_QUADRATURE:
            ...
            drivers[i] = new AG_WheelEncoder_Quadrature(*this, i, state[i]);
#endif
            break;

        case WheelEncoder_TYPE_SITL_QUADRATURE:
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
            drivers[i] = new AG_WheelEncoder_SITL_Quadrature(*this, i, state[i]);
#endif
            break;
...
void AG_WheelEncoder::update(void)
{
...
void AG_WheelEncoder::Log_Write() const
{
...
bool AG_WheelEncoder::healthy(uint8_t instance) const
{
...
bool AG_WheelEncoder::enabled(uint8_t instance) const
{
    if (instance >= num_instances) {
        return false;
    }
...
uint16_t AG_WheelEncoder::get_counts_per_revolution(uint8_t instance) const
{
...
float AG_WheelEncoder::get_wheel_radius(uint8_t instance) const
{
    // for invalid instances return zero vector
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
        return 0.0f;
...
const Vector3f &AG_WheelEncoder::get_pos_offset(uint8_t instance) const
{
    // for invalid instances return zero vector
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
...
float AG_WheelEncoder::get_delta_angle(uint8_t instance) const
{
...
float AG_WheelEncoder::get_distance(uint8_t instance) const
{
    // for invalid instances return zero
...
float AG_WheelEncoder::get_rate(uint8_t instance) const
{
    // for invalid instances return zero
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
        return 0.0f;
...
uint32_t AG_WheelEncoder::get_total_count(uint8_t instance) const
{
    // for invalid instances return zero
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
        return 0;
...
// get the total distance traveled in meters
uint32_t AG_WheelEncoder::get_error_count(uint8_t instance) const
{
    // for invalid instances return zero
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
        return 0;
...
float AG_WheelEncoder::get_signal_quality(uint8_t instance) const
{
    // protect against divide by zero
...
uint32_t AG_WheelEncoder::get_last_reading_ms(uint8_t instance) const
{
    // for invalid instances return zero
    if (instance >= WHEELENCODER_MAX_INSTANCES) {
        return 0;
...
// singleton instance
AG_WheelEncoder *AG_WheelEncoder::_singleton;

namespace AP {

AG_WheelEncoder *wheelencoder()
{
    return AG_WheelEncoder::get_singleton();
}
