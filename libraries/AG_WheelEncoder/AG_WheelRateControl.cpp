#include <AG_WheelEncoder/AG_WheelRateControl.h>

extern const AG_HAL::HAL& hal;

const AG_Param::GroupInfo AG_WheelRateControl::var_info[] = {
    // @Param: _ENABLE
    // @DisplayName: Wheel rate control enable/disable
    // @Description: Enable or disable wheel rate control
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    AP_GROUPINFO_FLAGS("_ENABLE", 1, AG_WheelRateControl, _enabled, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 0 200
    // @User: Standard
    AP_GROUPINFO("_RATE_MAX", 2, AG_WheelRateControl, _rate_max, AP_WHEEL_RATE_MAX_DEFAULT),
    ...
    AP_SUBGROUPINFO(_rate_pid0, "_RATE_", 3, AG_WheelRateControl, AG_PID),
    ...
    AP_SUBGROUPINFO(_rate_pid1, "2_RATE_", 4, AG_WheelRateControl, AG_PID),

    AP_GROUPEND
};

AG_WheelRateControl::AG_WheelRateControl(const AG_WheelEncoder &wheel_encoder_ref) :
...
bool AG_WheelRateControl::enabled(uint8_t instance)
{
...
float AG_WheelRateControl::get_rate_controlled_throttle(uint8_t instance, float desired_rate_pct, float dt)
{
    if (!enabled(instance)) {
        return 0;
    }
    ...
    AG_PID& rate_pid = (instance == 0) ? _rate_pid0 : _rate_pid1;
...
AG_PID& AG_WheelRateControl::get_pid(uint8_t instance)
{
    if (instance == 0) {
        return _rate_pid0;
    } else {
