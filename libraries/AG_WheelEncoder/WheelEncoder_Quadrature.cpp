...
void AG_WheelEncoder_Quadrature::update_pin(uint8_t &pin,
                                            uint8_t new_pin,
                                            uint8_t &pin_value)
{
    if (new_pin == pin) {
    ...
    if (new_pin != (uint8_t)-1) {
        ...
        if (!hal.gpio->attach_interrupt(
                pin,
                FUNCTOR_BIND_MEMBER(&AG_WheelEncoder_Quadrature::irq_handler,
                                    void,
                                    uint8_t,
                                    bool,
                                    uint32_t),
                AG_HAL::GPIO::INTERRUPT_BOTH)) {
...
void AG_WheelEncoder_Quadrature::update(void)
{
...
uint8_t AG_WheelEncoder_Quadrature::pin_ab_to_phase(bool pin_a, bool pin_b)
{
    if (!pin_a) {
        if (!pin_b) {
            // A = 0, B = 0
...
void AG_WheelEncoder_Quadrature::update_phase_and_error_count()
{
...
void AG_WheelEncoder_Quadrature::irq_handler(uint8_t pin,
                                             bool pin_value,
                                             uint32_t timestamp)
{
    // sanity check
