...
class AG_WheelEncoder
{
public:
    friend class AG_WheelEncoder_Backend;
    friend class AG_WheelEncoder_Quadrature;
    friend class AG_WheelEncoder_SITL_Quadrature;

    AG_WheelEncoder(void);

    /* Do not allow copies */
    CLASS_NO_COPY(AG_WheelEncoder);

    // get singleton instance
    static AG_WheelEncoder *get_singleton() {
        return _singleton;
    }
...
protected:
    ...
    AG_WheelEncoder_Backend *drivers[WHEELENCODER_MAX_INSTANCES];
    uint8_t num_instances;
    Vector3f pos_offset_zero;   // allows returning position offsets of zero for invalid requests

private:

    static AG_WheelEncoder *_singleton;
};
