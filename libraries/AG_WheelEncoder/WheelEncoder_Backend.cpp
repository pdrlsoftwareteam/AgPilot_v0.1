...
AG_WheelEncoder_Backend::AG_WheelEncoder_Backend(AG_WheelEncoder &frontend, uint8_t instance, AG_WheelEncoder::WheelEncoder_State &state) :
        _frontend(frontend),
        _state(state) 
{
...
int8_t AG_WheelEncoder_Backend::get_pin_a() const
{
...
// return pin.  returns -1 if pin is not defined for this instance
int8_t AG_WheelEncoder_Backend::get_pin_b() const
{
    if (_state.instance >= WHEELENCODER_MAX_INSTANCES) {
        return -1;
    }
...
void AG_WheelEncoder_Backend::copy_state_to_frontend(int32_t distance_count, uint32_t total_count, uint32_t error_count, uint32_t last_reading_ms)
{
