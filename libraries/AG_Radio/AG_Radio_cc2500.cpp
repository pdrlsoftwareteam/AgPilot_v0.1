...
AG_Radio_cc2500 *AG_Radio_cc2500::radio_singleton;
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
thread_t *AG_Radio_cc2500::_irq_handler_ctx;
virtual_timer_t AG_Radio_cc2500::timeout_vt;
uint32_t AG_Radio_cc2500::irq_time_us;
#endif
...
/*
  constructor
 */
AG_Radio_cc2500::AG_Radio_cc2500(AG_Radio &_radio) :
...
/*
  initialise radio
 */
bool AG_Radio_cc2500::init(void)
{
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
    if (_irq_handler_ctx != nullptr) {
        AG_HAL::panic("AG_Radio_cc2500: double instantiation of irq_handler\n");
    }
...
/*
  reset radio
 */
bool AG_Radio_cc2500::reset(void)
{
    if (!cc2500.lock_bus()) {
        return false;
    }
 ...
 */
const AG_Radio::stats &AG_Radio_cc2500::get_stats(void)
{
    return stats;
}
 ...
 */
uint16_t AG_Radio_cc2500::read(uint8_t chan)
{
 ...
 */
void AG_Radio_cc2500::update(void)
{
    check_fw_ack();
}
 ...
 */
uint8_t AG_Radio_cc2500::num_channels(void)
{
 ...
 */
uint32_t AG_Radio_cc2500::last_recv_us(void)
{
    return packet_timer;
}
 ...
 */
bool AG_Radio_cc2500::send(const uint8_t *pkt, uint16_t len)
{
    // disabled for now
    return false;
}

const AG_Radio_cc2500::config AG_Radio_cc2500::radio_config_GFSK[] = {
    /*
...
const AG_Radio_cc2500::config AG_Radio_cc2500::radio_config[] = {
 ...
 */
bool AG_Radio_cc2500::probe(void)
{
 ...
 */
void AG_Radio_cc2500::radio_init(void)
{
    ...
    if (get_protocol() == AG_Radio::PROTOCOL_CC2500_GFSK) {
    ...
    packet_timer = AG_HAL::micros();
    chanskip = 1;
    nextChannel(1);

    // set default autobind power to suit the cc2500
    AG_Param::set_default_by_name("BRD_RADIO_ABLVL", 90);

    chVTSet(&timeout_vt, chTimeMS2I(INTER_PACKET_INITIAL_MS), trigger_timeout_event, nullptr);

}

void AG_Radio_cc2500::trigger_irq_radio_event()
{
    //we are called from ISR context
    chSysLockFromISR();
    irq_time_us = AG_HAL::micros();
    chEvtSignalI(_irq_handler_ctx, EVT_IRQ);
    chSysUnlockFromISR();
}

void AG_Radio_cc2500::trigger_timeout_event(void *arg)
{
    (void)arg;
    //we are called from ISR context
    chSysLockFromISR();
...
void AG_Radio_cc2500::start_recv_bind(void)
{
    chan_count = 0;
    packet_timer = AG_HAL::micros();
...
void AG_Radio_cc2500::handle_data_packet(mavlink_channel_t chan, const mavlink_data96_t &m)
{
    uint32_t ofs=0;
 ...
 */
bool AG_Radio_cc2500::handle_D16_packet(const uint8_t *packet)
{
    if (packet[0] != 0x1D) {
        return false;
    }
...
/*
  handle a SRT packet
 */
bool AG_Radio_cc2500::handle_SRT_packet(const uint8_t *packet)
{
 ...
 */
bool AG_Radio_cc2500::have_channel(uint8_t channel, uint8_t count, uint8_t loop)
{
    uint8_t i;
    for (i=0; i<count; i++) {
 ...
 */
void AG_Radio_cc2500::setup_hopping_table_SRT(void)
{
    uint8_t val;
 ...
 */
bool AG_Radio_cc2500::handle_autobind_packet(const uint8_t *packet, uint8_t lqi)
{
 ...
 */
uint8_t AG_Radio_cc2500::map_RSSI_to_dBm(uint8_t rssi_raw)
{
    float rssi_dbm;
    if (rssi_raw >= 128) {
...
// main IRQ handler
void AG_Radio_cc2500::irq_handler(void)
{
    uint8_t ccLen;
    bool matched = false;
    do {
    ...
    case STATE_DATA: {
        ...
        if (ok) {
            ...
            if (get_telem_enable()) {
                ...
                if (ccLen == 32 || get_protocol() == AG_Radio::PROTOCOL_D16) {
                    send_D16_telemetry();
                } else {
                    if (have_tx_pps) {
 ...
 */
void AG_Radio_cc2500::set_fcc_channel(void)
{
...
// handle timeout IRQ
void AG_Radio_cc2500::irq_timeout(void)
{
    ...
    case STATE_SEARCH: {
        ...
        if (stats.recv_packets == 0 &&
            get_autobind_time() != 0 &&
            get_factory_test() == 0 &&
            (AG_HAL::micros() - packet_timer) > get_autobind_time() * 1000UL*1000UL &&
            (search_count & 1) == 0) {
...
void AG_Radio_cc2500::irq_handler_thd(void *arg)
{
    (void)arg;
    while (true) {
        ...
        case EVT_BIND:
            ...
            radio_singleton->packet_timer = AG_HAL::micros();
...
void AG_Radio_cc2500::initTuneRx(void)
{
    cc2500.WriteReg(CC2500_19_FOCCFG, 0x14);
    timeTunedMs = AG_HAL::millis();
    bindOffset = -126;
    best_lqi = 255;
...
void AG_Radio_cc2500::initialiseData(uint8_t adr)
{
...
void AG_Radio_cc2500::initGetBind(void)
{
    setChannelRX(0);
 ...
 */
bool AG_Radio_cc2500::check_best_LQI(void)
{
    if (best_lqi >= 50) {
        return false;
    }
 ...
 */
bool AG_Radio_cc2500::tuneRx(uint8_t ccLen, uint8_t *packet)
{
    if (bindOffset >= 126) {
 ...
 */
bool AG_Radio_cc2500::getBindData(uint8_t ccLen, uint8_t *packet)
{
...
void AG_Radio_cc2500::setChannel(uint8_t channel)
{
    cc2500.Strobe(CC2500_SIDLE);
...
void AG_Radio_cc2500::setChannelRX(uint8_t channel)
{
    setChannel(channel);
    cc2500.Strobe(CC2500_SFRX);
    cc2500.Strobe(CC2500_SRX);
...
void AG_Radio_cc2500::nextChannel(uint8_t skip)
{
    channr = (channr + skip) % listLength;
    setChannelRX(bindHopData[channr]);
}

void AG_Radio_cc2500::parse_frSkyX(const uint8_t *packet)
{
    uint16_t c[8];
...
uint16_t AG_Radio_cc2500::calc_crc(const uint8_t *data, uint8_t len)
{
    uint16_t crc = 0;
...
bool AG_Radio_cc2500::check_crc(uint8_t ccLen, uint8_t *packet)
{
...
/*
  save bind info
 */
void AG_Radio_cc2500::save_bind_info(void)
{
...
/*
  load bind info
 */
bool AG_Radio_cc2500::load_bind_info(void)
{
    // access to storage for bind information
    StorageAccess bind_storage(StorageManager::StorageBindInfo);
    struct bind_info info;
 ...
 */
void AG_Radio_cc2500::send_D16_telemetry(void)
{
    uint8_t frame[15];
 ...
 */
void AG_Radio_cc2500::send_SRT_telemetry(void)
{
 ...
 */
void AG_Radio_cc2500::check_fw_ack(void)
{
 ...
 */
void AG_Radio_cc2500::map_stick_mode(uint16_t *channels)
{
 ...
 */
void AG_Radio_cc2500::check_double_bind(void)
{
