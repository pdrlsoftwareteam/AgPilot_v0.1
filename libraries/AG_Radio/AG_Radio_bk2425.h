...
/*
  AG_Radio implementation for CC2500 2.4GHz radio.
...
class AG_Radio_beken : public AG_Radio_backend
{
public:
    // Override base class functions
    AG_Radio_beken(AG_Radio &radio); // Normal constructore
    ...
    const AG_Radio::stats &get_stats(void) override; // get radio statistics structure
...
private:
    ...
    static AG_Radio_beken *radio_singleton; // Singleton pointer to the Beken radio instance
    ...
    // Class data
    AG_HAL::OwnPtr<AG_HAL::SPIDevice> dev; // Low level support of SPI device
    HAL_Semaphore sem;  // semaphore between ISR and main thread to protect fwupload

    AG_Radio::stats stats; // Radio stats (live) for the current time-period
    AG_Radio::stats last_stats; // Radio stats (snapshot) for the previous time-period
