...
#define DebugPrintf(level, fmt, args...)   do { if (AG_Radio_beken::radio_singleton && ((level) <= AG_Radio_beken::radio_singleton->get_debug_level())) { printf(fmt, ##args); }} while (0)
...
AG_Radio_beken *AG_Radio_beken::radio_singleton;
thread_t *AG_Radio_beken::_irq_handler_ctx;
virtual_timer_t AG_Radio_beken::timeout_vt;
// See variable definitions in AG_Radio_bk2425.h for comments
uint32_t AG_Radio_beken::isr_irq_time_us;
uint32_t AG_Radio_beken::isr_timeout_time_us;
uint32_t AG_Radio_beken::next_switch_us;
uint32_t AG_Radio_beken::bind_time_ms;
SyncTiming AG_Radio_beken::synctm; // Let the IRQ see the interpacket timing
...
/*
  constructor
 */
AG_Radio_beken::AG_Radio_beken(AG_Radio &_radio) :
    AG_Radio_backend(_radio),
    beken(hal.spi->get_device("beken")) // trace this later - its on libraries/AG_HAL_ChibiOS/SPIDevice.cpp:92
{
...
/*
  initialise radio
 */
bool AG_Radio_beken::init(void)
{
    if (_irq_handler_ctx != nullptr) {
        AG_HAL::panic("AG_Radio_beken: double instantiation of irq_handler\n");
    }
...
/*
  reset radio
 */
bool AG_Radio_beken::reset(void)
{
    if (!beken.lock_bus()) {
        return false;
    }
 ...
 */
const AG_Radio::stats &AG_Radio_beken::get_stats(void)
{
    return stats;
}
 ...
 */
uint16_t AG_Radio_beken::read(uint8_t chan)
{
 ...
 */
void AG_Radio_beken::update(void)
{
    check_fw_ack();
}
 ...
 */
uint8_t AG_Radio_beken::num_channels(void)
{
 ...
 */
uint32_t AG_Radio_beken::last_recv_us(void)
{
 ...
 */
bool AG_Radio_beken::send(const uint8_t *pkt, uint16_t len)
{
    // disabled for now
    return false;
}

// Borrow the CRC32 algorithm from AG_HAL_SITL
 ...
 */
void AG_Radio_beken::radio_init(void)
{
    ...
    hal.gpio->attach_interrupt(HAL_GPIO_RADIO_IRQ, trigger_irq_radio_event, AG_HAL::GPIO::INTERRUPT_FALLING);
    ...
    isr_irq_time_us = isr_timeout_time_us = AG_HAL::micros();
...
// ----------------------------------------------------------------------------
void AG_Radio_beken::trigger_irq_radio_event()
{
    //we are called from ISR context
    //  DEBUG2_HIGH();
    chSysLockFromISR();
    isr_irq_time_us = AG_HAL::micros();
...
// ----------------------------------------------------------------------------
void AG_Radio_beken::trigger_timeout_event(void *arg)
{
    (void)arg;
    //we are called from ISR context
    //  DEBUG2_HIGH();
    //  DEBUG2_LOW();
    //  DEBUG2_HIGH();
    isr_timeout_time_us = AG_HAL::micros();
    chSysLockFromISR();
...
void AG_Radio_beken::start_recv_bind(void)
{
    chan_count = 0;
    synctm.packet_timer = AG_HAL::micros();
    radio_singleton->bind_time_ms = AG_HAL::millis();
...
void AG_Radio_beken::handle_data_packet(mavlink_channel_t chan, const mavlink_data96_t &m)
{
...
void AG_Radio_beken::update_SRT_telemetry(void)
{
    t_status.flags = 0;
...
bool AG_Radio_beken::UpdateTxData(void)
{
...
void AG_Radio_beken::check_fw_ack(void)
{
...
void AG_Radio_beken::map_stick_mode(void)
{
...
uint8_t AG_Radio_beken::ProcessBindPacket(const packetFormatRx * rx)
{
...
uint8_t AG_Radio_beken::ProcessPacket(const uint8_t* packet, uint8_t rxaddr)
{
    uint8_t result = 0;
    ...
    case BK_PKT_TYPE_CTRL_LOST:
        ...
        if (rxaddr == 0) {
            ...
            synctm.packet_timer = AG_HAL::micros(); // This is essential for letting the channels update
            if (!already_bound) {
    ...
    case BK_PKT_TYPE_BIND_MANUAL: // Sent by the tx for a few seconds after power-up when a button is held down
        if (rxaddr == 1) {
            ...
            //          if (uint32_t(AG_HAL::millis() - bind_time_ms) > 1000ul * 60u) // Have we pressed the button to bind recently? One minute timeout
            //              break; // Do not bind
            result = ProcessBindPacket(rx);
        }
        break;
...
void AG_Radio_beken::UpdateFccScan(void)
{
    // Support scan mode
...
// ----------------------------------------------------------------------------
// main IRQ handler
void AG_Radio_beken::irq_handler(uint32_t when)
{
...
void AG_Radio_beken::irq_timeout(uint32_t when)
{
    DEBUG1_LOW();
    DEBUG1_HIGH();
    DEBUG1_LOW();
    ...
    if (beken.bkReady) { // We are not reinitialising the chip in the main thread
        ...
        if (beken.fcc.fcc_mode) {
            ...
            if (dt > 50*d) { // We have lost sync (missed 50 packets) so slow down the channel hopping until we resync
                ...
                //          DebugPrintf(2, "c%d ", AG_HAL::micros() - next_switch_us);
                DebugPrintf(2, "c");
...
void AG_Radio_beken::irq_handler_thd(void *arg)
{
    (void) arg;
    while (true) {
        DEBUG1_LOW();
...
void AG_Radio_beken::setChannel(uint8_t channel)
{
...
void AG_Radio_beken::nextChannel(uint8_t skip)
{
    if (skip) {
        syncch.NextChannel();
    }
...
/*
  save bind info
 */
void AG_Radio_beken::save_bind_info(void)
{
...
/*
  load bind info
 */
bool AG_Radio_beken::load_bind_info(void)
{
    // access to storage for bind information
    StorageAccess bind_storage(StorageManager::StorageBindInfo);
    struct bind_info info;
...
// ----------------------------------------------------------------------------
void AG_Radio_beken::BadDroneId(void)
{
