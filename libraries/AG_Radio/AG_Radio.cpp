...
const AG_Param::GroupInfo AG_Radio::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("_TYPE",  1, AG_Radio, radio_type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    AP_GROUPINFO("_PROT",  2, AG_Radio, protocol, PROTOCOL_AUTO),
    ...
    // @Range: 0 4
    // @User: Advanced
    AP_GROUPINFO("_DEBUG",  3, AG_Radio, debug_level, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_DISCRC",  4, AG_Radio, disable_crc, 0),
    ...
    // @Range: 0 16
    // @User: Advanced
    AP_GROUPINFO("_SIGCH",  5, AG_Radio, rssi_chan, 0),
    ...
    // @Range: 0 16
    // @User: Advanced
    AP_GROUPINFO("_PPSCH",  6, AG_Radio, pps_chan, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_TELEM",  7, AG_Radio, telem_enable, 0),
    ...
    // @Range: 1 8
    // @User: Advanced
    AP_GROUPINFO("_TXPOW",  8, AG_Radio, transmit_power, 8),
    ...
    // @User: Advanced
    AP_GROUPINFO("_FCCTST", 9, AG_Radio, fcc_test, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("_STKMD", 10, AG_Radio, stick_mode, 2),
    ...
    // @User: Advanced
    AP_GROUPINFO("_TESTCH", 11, AG_Radio, factory_test, 0),
    ...
    // @Range: 0 16
    // @User: Advanced
    AP_GROUPINFO("_TSIGCH", 12, AG_Radio, tx_rssi_chan, 0),
    ...
    // @Range: 0 16
    // @User: Advanced
    AP_GROUPINFO("_TPPSCH", 13, AG_Radio, tx_pps_chan, 0),
    ...
    // @Range: 1 8
    // @User: Advanced
    AP_GROUPINFO("_TXMAX",  14, AG_Radio, tx_max_power, 8),
    ...
    // @Range: 0 40
    // @User: Advanced
    AP_GROUPINFO("_BZOFS",  15, AG_Radio, tx_buzzer_adjust, 25),
    ...
    // @Range: 0 120
    // @User: Advanced
    AP_GROUPINFO("_ABTIME",  16, AG_Radio, auto_bind_time, 0),
    ...
    // @Range: 0 31
    // @User: Advanced
    AP_GROUPINFO("_ABLVL",  17, AG_Radio, auto_bind_rssi, 0),

    AP_GROUPEND
};

AG_Radio *AG_Radio::_singleton;

// constructor
AG_Radio::AG_Radio(void)
{
    AG_Param::setup_object_defaults(this, var_info);
    if (_singleton != nullptr) {
        AG_HAL::panic("Multiple AG_Radio declarations");
    }
    _singleton = this;
}

bool AG_Radio::init(void)
{
    switch (radio_type) {
#if (not defined AP_RADIO_CYRF6936 || AP_RADIO_CYRF6936)
    case RADIO_TYPE_CYRF6936:
        driver = new AG_Radio_cypress(*this);
        break;
#endif
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
#if (not defined AP_RADIO_CC2500 || AP_RADIO_CC2500)
    case RADIO_TYPE_CC2500:
        driver = new AG_Radio_cc2500(*this);
        break;
#endif
#endif
    ...
    case RADIO_TYPE_BK2425:
        driver = new AG_Radio_beken(*this);
        break;
#endif
#endif
#if CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_CHIBIOS_SKYVIPER_F412
...
bool AG_Radio::send(const uint8_t *pkt, uint16_t len)
{
    if (!driver) {
        return false;
    }
...
void AG_Radio::start_recv_bind(void)
{
    if (!driver) {
        return;
    }
...
const AG_Radio::stats &AG_Radio::get_stats(void)
{
    return driver->get_stats();
}

uint8_t AG_Radio::num_channels(void)
{
    if (!driver) {
        return 0;
    }
...
uint16_t AG_Radio::read(uint8_t chan)
{
    if (!driver) {
        return 0;
    }
...
uint32_t AG_Radio::last_recv_us(void)
{
    if (!driver) {
        return 0;
    }
...
void AG_Radio::handle_data_packet(mavlink_channel_t chan, const mavlink_data96_t &m)
{
    if (driver) {
...
void AG_Radio::play_tune(const char *tune_str)
{
...
uint32_t AG_Radio::get_tx_version(void)
{
    if (driver) {
...
void AG_Radio::set_wifi_channel(uint8_t channel)
{
    if (driver) {
...
void AG_Radio::change_txmode(void)
{
    if (stick_mode == 2) {
