...
const AG_Param::GroupInfo AG_OADatabase::var_info[] = {
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("SIZE", 1, AG_OADatabase, _database_size_param, AP_OADATABASE_SIZE_DEFAULT),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("EXPIRE", 2, AG_OADatabase, _database_expiry_seconds, AP_OADATABASE_TIMEOUT_SECONDS_DEFAULT),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("QUEUE_SIZE", 3, AG_OADatabase, _queue_size_param, AP_OADATABASE_QUEUE_SIZE_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("OUTPUT", 4, AG_OADatabase, _output_level, (float)OA_DbOutputLevel::OUTPUT_LEVEL_SEND_HIGH),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("BEAM_WIDTH", 5, AG_OADatabase, _beam_width, 5.0f),
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO("RADIUS_MIN", 6, AG_OADatabase, _radius_min, 0.01f),
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO("DIST_MAX", 7, AG_OADatabase, _dist_max, 0.0f),
    ...
    // @Range: 0 4
    // @User: Advanced
    AP_GROUPINFO_FRAME("ALT_MIN", 8, AG_OADatabase, _min_alt, 0.0f, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),

    AP_GROUPEND
};

AG_OADatabase::AG_OADatabase()
{
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_OADatabase must be singleton");
    }
    _singleton = this;
...
void AG_OADatabase::queue_push(const Vector3f &pos, uint32_t timestamp_ms, float distance)
{
    if (!healthy()) {
        return;
    }
    ...
    const OA_DbItem item = {pos, timestamp_ms, MAX(_radius_min, distance * dist_to_radius_scalar), 0, AG_OADatabase::OA_DbItemImportance::Normal};
    {
...
void AG_OADatabase::init_database()
{
...
uint8_t AG_OADatabase::get_send_to_gcs_flags(const OA_DbItemImportance importance)
{
    switch (importance) {
...
bool AG_OADatabase::process_queue()
{
    if (!healthy()) {
        return false;
    }
...
void AG_OADatabase::database_item_add(const OA_DbItem &item)
{
...
void AG_OADatabase::database_item_remove(const uint16_t index)
{
...
void AG_OADatabase::database_item_refresh(const uint16_t index, const uint32_t timestamp_ms, const float radius)
{
    if (index >= _database.count) {
        // index out of range
        return;
...
void AG_OADatabase::database_items_remove_all_expired()
{
...
bool AG_OADatabase::is_close_to_item_in_database(const uint16_t index, const OA_DbItem &item) const
{
    if (index >= _database.count) {
        // index out of range
        return false;
...
void AG_OADatabase::send_adsb_vehicle(mavlink_channel_t chan, uint16_t interval_ms)
{
    // ensure database's send_to_gcs field is large enough
    static_assert(MAVLINK_COMM_NUM_BUFFERS <= sizeof(OA_DbItem::send_to_gcs) * 8,
                  "AG_OADatabase's OA_DBItem.send_to_gcs bitmask must be large enough to hold MAVLINK_COMM_NUM_BUFFERS");
...
// singleton instance
AG_OADatabase *AG_OADatabase::_singleton;

namespace AP {
AG_OADatabase *oadatabase()
{
    return AG_OADatabase::get_singleton();
}
