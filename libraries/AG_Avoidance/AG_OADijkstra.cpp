...
/// Constructor
AG_OADijkstra::AG_OADijkstra(AP_Int16 &options) :
        _options(options),
...
AG_OADijkstra::AG_OADijkstra_State AG_OADijkstra::update(const Location &current_loc, const Location &destination, Location& origin_new, Location& destination_new)
{
    ...
    if (_polyfence_visgraph_ok && (_log_num_points < total_numpoints()) && (_options & AG_OAGathPlanner::OARecoveryOptions::OA_OPTION_LOG_DIJKSTRA_POINTS) ) {
        Vector2f vis_point;
...
bool AG_OADijkstra::some_fences_enabled() const
{
    const AG_Fence *fence = AG_Fence::get_singleton();
    if (fence == nullptr) {
        return false;
    }
...
const char* AG_OADijkstra::get_error_msg(AG_OADijkstra_Error error_id) const
{
    switch (error_id) {
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_NONE:
        return "no error";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_OUT_OF_MEMORY:
        return "out of memory";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_OVERLAPPING_POLYGON_POINTS:
        return "overlapping polygon points";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_FAILED_TO_BUILD_INNER_POLYGON:
        return "failed to build inner polygon";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_OVERLAPPING_POLYGON_LINES:
        return "overlapping polygon lines";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_FENCE_DISABLED:
        return "fence disabled";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_TOO_MANY_FENCE_POINTS:
        return "too many fence points";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_NO_POSITION_ESTIMATE:
        return "no position estimate";
        break;
    case AG_OADijkstra_Error::DIJKSTRA_ERROR_COULD_NOT_FIND_PATH:
...
void AG_OADijkstra::report_error(AG_OADijkstra_Error error_id)
{
    // report errors to GCS every 5 seconds
    uint32_t now_ms = AG_HAL::millis();
    if ((error_id != AG_OADijkstra_Error::DIJKSTRA_ERROR_NONE) &&
...
bool AG_OADijkstra::check_inclusion_polygon_updated() const
{
...
bool AG_OADijkstra::create_inclusion_polygon_with_margin(float margin_cm, AG_OADijkstra_Error &err_id)
{
    const AG_Fence *fence = AG_Fence::get_singleton();

    if (fence == nullptr) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_FENCE_DISABLED;
        return false;
    }
    ...
    for (uint8_t i = 0; i < num_inclusion_polygons; i++) {
        ...
        for (uint16_t j = 0; j < num_points; j++) {
            ...
            if (before_pt.is_zero() || after_pt.is_zero() || (before_pt == after_pt)) {
                err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_OVERLAPPING_POLYGON_POINTS;
                return false;
            }
            ...
            if (Polygon_outside(temp_point, boundary, num_points)) {
                ...
                if (Polygon_outside(temp_point, boundary, num_points)) {
                    ...
                    err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_OVERLAPPING_POLYGON_LINES;
                    return false;
                }
            }
            ...
            if (!_inclusion_polygon_pts.expand_to_hold(_inclusion_polygon_numpoints + new_points + 1)) {
                err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_OUT_OF_MEMORY;
                return false;
            }
            // add point
...
bool AG_OADijkstra::check_exclusion_polygon_updated() const
{
    const AG_Fence *fence = AG_Fence::get_singleton();
    if (fence == nullptr) {
        return false;
...
// returns true on success.  returns false on failure and err_id is updated
bool AG_OADijkstra::create_exclusion_polygon_with_margin(float margin_cm, AG_OADijkstra_Error &err_id)
{
    const AG_Fence *fence = AG_Fence::get_singleton();
...
// returns true if changed
bool AG_OADijkstra::check_exclusion_circle_updated() const
{
...
// returns true on success.  returns false on failure and err_id is updated
bool AG_OADijkstra::create_exclusion_circle_with_margin(float margin_cm, AG_OADijkstra_Error &err_id)
{
    // exit immediately if fence is not enabled
    const AG_Fence *fence = AG_Fence::get_singleton();
    if (fence == nullptr) {
...
uint16_t AG_OADijkstra::total_numpoints() const
{
...
bool AG_OADijkstra::get_point(uint16_t index, Vector2f &point) const
{
    // sanity check index
...
bool AG_OADijkstra::intersects_fence(const Vector2f &seg_start, const Vector2f &seg_end) const
{
...
bool AG_OADijkstra::create_fence_visgraph(AG_OADijkstra_Error &err_id)
{
    // exit immediately if fence is not enabled
    const AG_Fence *fence = AG_Fence::get_singleton();
    if (fence == nullptr) {
    ...
    if (total_numpoints() >= OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_TOO_MANY_FENCE_POINTS;
        return false;
    }
            ...
            for (uint8_t j = i + 1; j < total_numpoints(); j++) {
                ...
                if (get_point(j, end_seg)) {
                    ...
                    if (!intersects_fence(start_seg, end_seg)) {
                        if (!_fence_visgraph.add_item({AG_OAVisGraph::OATYPE_INTERMEDIATE_POINT, i},
                                                      {AG_OAVisGraph::OATYPE_INTERMEDIATE_POINT, j},
...
bool AG_OADijkstra::update_visgraph(AG_OAVisGraph& visgraph, const AG_OAVisGraph::OAItemID& oaid, const Vector2f &position, bool add_extra_position, Vector2f extra_position)
{
    ...
    for (uint8_t i = 0; i < total_numpoints(); i++) {
        ...
        if (get_point(i, seg_end)) {
            if (!intersects_fence(position, seg_end)) {
                ...
                if (!visgraph.add_item(oaid, {AG_OAVisGraph::OATYPE_INTERMEDIATE_POINT, i}, (position - seg_end).length())) {
                    return false;
                }
            }
        }
    ...
    if (add_extra_position) {
        if (!intersects_fence(position, extra_position)) {
            if (!visgraph.add_item(oaid, {AG_OAVisGraph::OATYPE_DESTINATION, 0}, (position - extra_position).length())) {
                return false;
            }
        }
    }
...
void AG_OADijkstra::update_visible_node_distances(node_index curr_node_idx)
{
    // sanity check
    ...
    const AG_OAVisGraph* visgraphs[] = {&_fence_visgraph, &_destination_visgraph};
    for (uint8_t v=0; v<ARRAY_SIZE(visgraphs); v++) {

        // skip if empty
        const AG_OAVisGraph &curr_visgraph = *visgraphs[v];
        ...
        for (uint16_t i = 0; i < curr_visgraph.num_items(); i++) {
            const AG_OAVisGraph::VisGraphItem &item = curr_visgraph[i];
            ...
            if ((curr_node.id == item.id1) || (curr_node.id == item.id2)) {
                AG_OAVisGraph::OAItemID matching_id = (curr_node.id == item.id1) ? item.id2 : item.id1;
...
bool AG_OADijkstra::find_node_from_id(const AG_OAVisGraph::OAItemID &id, node_index &node_idx) const
{
    switch (id.id_type) {
    case AG_OAVisGraph::OATYPE_SOURCE:
        ...
        break;
    case AG_OAVisGraph::OATYPE_DESTINATION:
        ...
        break;
    case AG_OAVisGraph::OATYPE_INTERMEDIATE_POINT:
...
bool AG_OADijkstra::find_closest_node_idx(node_index &node_idx) const
{
...
bool AG_OADijkstra::calc_shortest_path(const Location &origin, const Location &destination, AG_OADijkstra_Error &err_id)
{
    ...
    if (!origin.get_vector_xy_from_origin_NE(_path_source) || !destination.get_vector_xy_from_origin_NE(_path_destination)) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_NO_POSITION_ESTIMATE;
        return false;
    }

    // create visgraphs of origin and destination to fence points
    if (!update_visgraph(_source_visgraph, {AG_OAVisGraph::OATYPE_SOURCE, 0}, _path_source, true, _path_destination)) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_OUT_OF_MEMORY;
        return false;
    }
    if (!update_visgraph(_destination_visgraph, {AG_OAVisGraph::OATYPE_DESTINATION, 0}, _path_destination)) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_OUT_OF_MEMORY;
        return false;
    }
    ...
    _short_path_data[0] = {{AG_OAVisGraph::OATYPE_SOURCE, 0}, false, 0, 0};
    _short_path_data[1] = {{AG_OAVisGraph::OATYPE_DESTINATION, 0}, false, OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX, FLT_MAX};
    ...
    for (uint8_t i=0; i<total_numpoints(); i++) {
        _short_path_data[_short_path_data_numpoints++] = {{AG_OAVisGraph::OATYPE_INTERMEDIATE_POINT, i}, false, OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX, FLT_MAX};
    }
    ...
    for (uint16_t i = 0; i < _source_visgraph.num_items(); i++) {
        ...
        if (find_node_from_id(_source_visgraph[i].id2, node_idx)) {
            ...
            err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_COULD_NOT_FIND_PATH;
            return false;
        }
    }
    ...
    while (find_closest_node_idx(current_node_idx)) {
        ...
        if (find_node_from_id({AG_OAVisGraph::OATYPE_DESTINATION,0}, dest_node) && current_node_idx == dest_node) {
    ...
    bool success = false;
    node_index nidx;
    if (!find_node_from_id({AG_OAVisGraph::OATYPE_DESTINATION,0}, nidx)) {
        err_id = AG_OADijkstra_Error::DIJKSTRA_ERROR_COULD_NOT_FIND_PATH;
        return false;
    }
    _path_numpoints = 0;
    while (true) {
        ...
        if ((_short_path_data[nidx].distance_from_idx == OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX) ||
            ...
            if (_short_path_data[nidx].id.id_type == AG_OAVisGraph::OATYPE_SOURCE) {
                success = true;
                break;
            } else {
...
bool AG_OADijkstra::get_shortest_path_point(uint8_t point_num, Vector2f& pos)
{
    ...
    // get id from path
    AG_OAVisGraph::OAItemID id = _path[_path_numpoints - point_num - 1];

    return convert_node_to_point(id, pos);
}

// find the position of a node as an offset (in cm) from the ekf origin
bool AG_OADijkstra::convert_node_to_point(const AG_OAVisGraph::OAItemID& id, Vector2f& pos) const
{
