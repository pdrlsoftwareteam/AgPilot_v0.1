...
#include "AG_Avoid.h"
#include <AG_AHRS/AG_AHRS.h>     // AHRS library
#include <AG_Fence/AG_Fence.h>         // Failsafe fence library
...
#include <stdio.h>
 ...
 # define AP_AVOID_BEHAVE_DEFAULT AG_Avoid::BehaviourType::BEHAVIOR_STOP
#else
 # define AP_AVOID_BEHAVE_DEFAULT AG_Avoid::BehaviourType::BEHAVIOR_SLIDE
#endif
...
const AG_Param::GroupInfo AG_Avoid::var_info[] = {

	    AP_GROUPINFO("FLAG", 10, AG_Avoid, _man_flag, false),
		AP_GROUPINFO("ALT", 11, AG_Avoid, _min_alt_avoid, 300.0),
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("ENABLE", 1,  AG_Avoid, _enabled, AC_AVOID_DEFAULT, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 0 4500
    // @User: Standard
    AP_GROUPINFO_FRAME("ANGLE_MAX", 2,  AG_Avoid, _angle_max, 1000, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),
    ...
    // @Range: 1 30
    // @User: Standard
    AP_GROUPINFO_FRAME("DIST_MAX", 3,  AG_Avoid, _dist_max, AC_AVOID_NONGPS_DIST_MAX_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),
    ...
    // @Range: 1 10
    // @User: Standard
    AP_GROUPINFO("MARGIN", 4, AG_Avoid, _margin, 2.0f),
    ...
    // @User: Standard
    AP_GROUPINFO_FRAME("BEHAVE", 5, AG_Avoid, _behavior, AP_AVOID_BEHAVE_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER | AP_PARAM_FRAME_ROVER),
    ...
    // @Range: 0 2
    // @User: Standard
    AP_GROUPINFO("BACKUP_SPD", 6, AG_Avoid, _backup_speed_max, 0.75f),
    ...
    // @Range: 0 6
    // @User: Standard
    AP_GROUPINFO_FRAME("ALT_MIN", 7, AG_Avoid, _alt_min, 0.0f, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),
    ...
    // @Range: 0 9
    // @User: Standard
    AP_GROUPINFO("ACCEL_MAX", 8, AG_Avoid, _accel_max, 3.0f),
    ...
    // @Range: 0 2
    // @User: Standard
    AP_GROUPINFO("BACKUP_DZ", 9, AG_Avoid, _backup_deadzone, 0.10f),
...
*/
void AG_Avoid::adjust_velocity_fence(float kP, float accel_cmss, Vector3f &desired_vel_cms, Vector3f &backup_vel, float kP_z, float accel_cmss_z, float dt)
{   
...
*/
void AG_Avoid::adjust_velocity(Vector3f &desired_vel_cms, bool &backing_up, float kP, float accel_cmss, float kP_z, float accel_cmss_z, float dt)
{
	float _curr_alt;
    ...
    if (desired_vel_cms_original != desired_vel_cms) {
        _last_limit_time = AG_HAL::millis();
    }
...
*/
void AG_Avoid::limit_accel(const Vector3f &original_vel, Vector3f &modified_vel, float dt)
{
    ...
    if (AG_HAL::millis() - _last_limit_time > AC_AVOID_ACCEL_TIMEOUT_MS) {
...
void AG_Avoid::adjust_speed(float kP, float accel, float heading, float &speed, float dt)
{
...
void AG_Avoid::adjust_velocity_z(float kP, float accel_cmss, float& climb_rate_cms, float& backup_speed, float dt)
{
...
void AG_Avoid::adjust_roll_pitch(float &roll, float &pitch, float veh_angle_max)
{
 ...
 */
void AG_Avoid::limit_velocity_2D(float kP, float accel_cmss, Vector2f &desired_vel_cms, const Vector2f& limit_direction, float limit_distance_cm, float dt)
{
 ...
 */
void AG_Avoid::limit_velocity_3D(float kP, float accel_cmss, Vector3f &desired_vel_cms, const Vector3f& obstacle_vector, float margin_cm, float kP_z, float accel_cmss_z, float dt)
{  
 ...
 */
void AG_Avoid::calc_backup_velocity_2D(float kP, float accel_cmss, Vector2f &quad1_back_vel_cms, Vector2f &quad2_back_vel_cms, Vector2f &quad3_back_vel_cms, Vector2f &quad4_back_vel_cms, float back_distance_cm, Vector2f limit_direction, float dt)
{      
...
*/
void AG_Avoid::find_max_quadrant_velocity(Vector2f &desired_vel, Vector2f &quad1_vel, Vector2f &quad2_vel, Vector2f &quad3_vel, Vector2f &quad4_vel) 
{   
...
*/
void AG_Avoid::find_max_quadrant_velocity_3D(Vector3f &desired_vel, Vector2f &quad1_vel, Vector2f &quad2_vel, Vector2f &quad3_vel, Vector2f &quad4_vel, float &max_z_vel, float &min_z_vel)
{   
 ...
 */
float AG_Avoid::get_max_speed(float kP, float accel_cmss, float distance_cm, float dt) const
{
    if (is_zero(kP)) {
 ...
 */
void AG_Avoid::adjust_velocity_circle_fence(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, float dt)
{
    AG_Fence *fence = AP::fence();
    if (fence == nullptr) {
        return;
 ...
 */
void AG_Avoid::adjust_velocity_inclusion_and_exclusion_polygons(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, float dt)
{
 ...
 */
void AG_Avoid::adjust_velocity_inclusion_circles(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, float dt)
{
    const AG_Fence *fence = AP::fence();
    if (fence == nullptr) {
        return;
 ...
 */
void AG_Avoid::adjust_velocity_exclusion_circles(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, float dt)
{
    const AG_Fence *fence = AP::fence();
    if (fence == nullptr) {
        return;
    ...
    Vector2f stopping_offset;
    if (!is_zero(desired_speed)) {
        if ((AG_Avoid::BehaviourType)_behavior.get() == BEHAVIOR_STOP) {
            stopping_offset = desired_vel_cms*((2.0f + margin_cm + get_stopping_distance(kP, accel_cmss, desired_speed))/desired_speed);
        }
    }
 ...
 */
void AG_Avoid::adjust_velocity_beacon_fence(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, float dt)
{
    AG_Beacon *_beacon = AP::beacon();
 ...
 */
void AG_Avoid::adjust_velocity_proximity(float kP, float accel_cmss, Vector3f &desired_vel_cms, Vector3f &backup_vel, float kP_z, float accel_cmss_z, float dt)
{
#if HAL_PROXIMITY_ENABLED
    ...
    AG_Proximity &_proximity = *proximity;
    // check for status of the sensor
    if (_proximity.get_status() != AG_Proximity::Status::Good) {
        return;
    }
 ...
 */
void AG_Avoid::adjust_velocity_polygon(float kP, float accel_cmss, Vector2f &desired_vel_cms, Vector2f &backup_vel, const Vector2f* boundary, uint16_t num_points, float margin, float dt, bool stay_inside)
{
 ...
 *
 * Implementation copied from AG_PosControl.
 */
float AG_Avoid::get_stopping_distance(float kP, float accel_cmss, float speed_cms) const
{
...
float AG_Avoid::distance_to_lean_pct(float dist_m)
{
...
void AG_Avoid::get_proximity_roll_pitch_pct(float &roll_positive, float &roll_negative, float &pitch_positive, float &pitch_negative)
{
#if HAL_PROXIMITY_ENABLED
    AG_Proximity *proximity = AP::proximity();
...
// singleton instance
AG_Avoid *AG_Avoid::_singleton;

namespace AP {

AG_Avoid *ac_avoid()
{
    return AG_Avoid::get_singleton();
}
