...
#include <AG_Avoidance/AG_OADatabase.h>
...
const AG_Param::GroupInfo AG_OABendyRuler::var_info[] = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("LOOKAHEAD", 1, AG_OABendyRuler, _lookahead, OA_BENDYRULER_LOOKAHEAD_DEFAULT),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("CONT_RATIO", 2, AG_OABendyRuler, _bendy_ratio, OA_BENDYRULER_RATIO_DEFAULT),
    ...
    // @Increment: 5
    // @User: Standard
    AP_GROUPINFO("CONT_ANGLE", 3, AG_OABendyRuler, _bendy_angle, OA_BENDYRULER_ANGLE_DEFAULT),
    ...
    // @User: Standard
    AP_GROUPINFO_FRAME("TYPE", 4, AG_OABendyRuler, _bendy_type, OA_BENDYRULER_TYPE_DEFAULT, AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_TRICOPTER),

    AP_GROUPEND
};

AG_OABendyRuler::AG_OABendyRuler() 
{ 
...
bool AG_OABendyRuler::update(const Location& current_loc, const Location& destination, const Vector2f &ground_speed_vec, Location &origin_new, Location &destination_new, OABendyType &bendy_type, bool proximity_only)
{
...
bool AG_OABendyRuler::search_xy_path(const Location& current_loc, const Location& destination, float ground_course_deg, Location &destination_new, float lookahead_step1_dist, float lookahead_step2_dist, float bearing_to_dest, float distance_to_dest, bool proximity_only) 
{
...
bool AG_OABendyRuler::search_vertical_path(const Location &current_loc, const Location &destination, Location &destination_new, float lookahead_step1_dist, float lookahead_step2_dist, float bearing_to_dest, float distance_to_dest, bool proximity_only)
{
...
AG_OABendyRuler::OABendyType AG_OABendyRuler::get_type() const
{
    switch (_bendy_type) {
...
*/
bool AG_OABendyRuler::resist_bearing_change(const Location &destination, const Location &current_loc, bool active, float bearing_test, float lookahead_step1_dist, float margin, Location &prev_dest, float &prev_bearing, float &final_bearing, float &final_margin, bool proximity_only) const
{      
...
float AG_OABendyRuler::calc_avoidance_margin(const Location &start, const Location &end, bool proximity_only) const
{
...
bool AG_OABendyRuler::calc_margin_from_circular_fence(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
...
// on success returns true and updates margin
bool AG_OABendyRuler::calc_margin_from_alt_fence(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
    // exit immediately if polygon fence is not enabled
    const AG_Fence *fence = AG_Fence::get_singleton();
...
// on success returns true and updates margin
bool AG_OABendyRuler::calc_margin_from_inclusion_and_exclusion_polygons(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
    const AG_Fence *fence = AG_Fence::get_singleton();
    if (fence == nullptr) {
...
// on success returns true and updates margin
bool AG_OABendyRuler::calc_margin_from_inclusion_and_exclusion_circles(const Location &start, const Location &end, float &margin) const
{
#if AP_FENCE_ENABLED
...
// on success returns true and updates margin
bool AG_OABendyRuler::calc_margin_from_object_database(const Location &start, const Location &end, float &margin) const
{
    // exit immediately if db is empty
    AG_OADatabase *oaDb = AP::oadatabase();
    ...
    for (uint16_t i=0; i<oaDb->database_count(); i++) {
        const AG_OADatabase::OA_DbItem& item = oaDb->get_item(i);
