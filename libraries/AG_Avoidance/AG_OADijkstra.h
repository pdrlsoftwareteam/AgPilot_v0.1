...
public:
    ...
    AG_OADijkstra(AP_Int16 &options);

    CLASS_NO_COPY(AG_OADijkstra);  /* Do not allow copies */
    ...
    enum AG_OADijkstra_State : uint8_t {
    ...
    AG_OADijkstra_State update(const Location &current_loc, const Location &destination, Location& origin_new, Location& destination_new);
...
private:
    ...
    enum class AG_OADijkstra_Error : uint8_t {
    ...
    const char* get_error_msg(AG_OADijkstra_Error error_id) const;

    // report error to ground station
    void report_error(AG_OADijkstra_Error error_id);
    ...
    bool create_inclusion_polygon_with_margin(float margin_cm, AG_OADijkstra_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_exclusion_polygon_with_margin(float margin_cm, AG_OADijkstra_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_exclusion_circle_with_margin(float margin_cm, AG_OADijkstra_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_fence_visgraph(AG_OADijkstra_Error &err_id);
    ...
    bool calc_shortest_path(const Location &origin, const Location &destination, AG_OADijkstra_Error &err_id);
    ...
    AG_ExpandingArray<Vector2f> _inclusion_polygon_pts; // array of nodes corresponding to inclusion polygon points plus a margin
    uint8_t _inclusion_polygon_numpoints;   // number of points held in above array
    uint32_t _inclusion_polygon_update_ms;  // system time of boundary update from AG_Fence (used to detect changes to polygon fence)

    // exclusion polygon related variables
    AG_ExpandingArray<Vector2f> _exclusion_polygon_pts; // array of nodes corresponding to exclusion polygon points plus a margin
    ...
    AG_ExpandingArray<Vector2f> _exclusion_circle_pts; // array of nodes surrounding exclusion circles plus a margin
    ...
    // visibility graphs
    AG_OAVisGraph _fence_visgraph;          // holds distances between all inclusion/exclusion fence points (with margin)
    AG_OAVisGraph _source_visgraph;         // holds distances from source point to all other nodes
    AG_OAVisGraph _destination_visgraph;    // holds distances from the destination to all other nodes
    ...
    bool update_visgraph(AG_OAVisGraph& visgraph, const AG_OAVisGraph::OAItemID& oaid, const Vector2f &position, bool add_extra_position = false, Vector2f extra_position = Vector2f(0,0));

    typedef uint8_t node_index;         // indices into short path data
    struct ShortPathNode {
        AG_OAVisGraph::OAItemID id;     // unique id for node (combination of type and id number)
    ...
    AG_ExpandingArray<ShortPathNode> _short_path_data;
    ...
    bool find_node_from_id(const AG_OAVisGraph::OAItemID &id, node_index &node_idx) const;
    ...
    AG_ExpandingArray<AG_OAVisGraph::OAItemID> _path;   // ids of points on return path in reverse order (i.e. destination is first element)
    ...
    bool convert_node_to_point(const AG_OAVisGraph::OAItemID& id, Vector2f& pos) const;

    AG_OADijkstra_Error _error_last_id;                 // last error id sent to GCS
    ...
    // refernce to AG_OAGathPlanner options param
    AP_Int16 &_options;
};
