 ...
 */
class AG_OAGathPlanner {

public:
    AG_OAGathPlanner();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_OAGathPlanner);

    // get singleton instance
    static AG_OAGathPlanner *get_singleton() {
        return _singleton;
    }

    // perform any required initialisation
    void init();
    uint8_t get_autoFlag()
    {
    	return _auto_flag;
    }
...
private:
    ...
    OAPathPlannerUsed map_bendytype_to_pathplannerused(AG_OABendyRuler::OABendyType bendy_type);
...
private:
    ...
    AG_OABendyRuler *_oabendyruler; // Bendy Ruler algorithm
    AG_OADijkstra *_oadijkstra;     // Dijkstra's algorithm
    AG_OADatabase _oadatabase;      // Database of dynamic objects to avoid
    uint32_t avoidance_latest_ms;   // last time Dijkstra's or BendyRuler algorithms ran

    bool proximity_only = true;
    static AG_OAGathPlanner *_singleton;
};
