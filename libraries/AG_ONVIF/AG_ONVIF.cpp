...
#include "AG_ONVIF.h"
#if ENABLE_ONVIF
#include <AG_ONVIF/MediaBinding.nsmap>
...
#if 0
#define DEBUG_PRINT(fmt,args...) do {GCS_SEND_TEXT(MAV_SEVERITY_INFO ,"AG_ONVIF:" fmt "\n", ## args); } while(0)
#else
#define DEBUG_PRINT(fmt,args...)
#endif

#define ERROR_PRINT(fmt,args...) do {GCS_SEND_TEXT(MAV_SEVERITY_ERROR , "AG_ONVIF:" fmt "\n", ## args); } while(0)
...
AG_ONVIF *AG_ONVIF::_singleton;
static AG_ONVIF onvif;
...
AG_ONVIF::AG_ONVIF()
{
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_ONVIF must be singleton");
    }
    _singleton = this;
}

// Start ONVIF client with username, password and service host url
bool AG_ONVIF::start(const char *user, const char *pass, const char *hostname)
{
    if (!initialised) {
        soap = soap_new1(SOAP_XML_CANONICAL | SOAP_C_UTFSTRING);
        if (soap == nullptr) {
            ERROR_PRINT("AG_ONVIF: Failed to allocate soap");
            return false;
        }
        ...
        if (proxy_device == nullptr ||
            proxy_media == nullptr ||
            proxy_ptz == nullptr) {
            ERROR_PRINT("AG_ONVIF: Failed to allocate gSOAP Proxy objects.");
            return false;
        }
        initialised = true;
    }
...
bool AG_ONVIF::probe_onvif_server()
{
    {
...
bool AG_ONVIF::rand_nonce(char *nonce, size_t noncelen)
{
    if (noncelen <= 4) {
        // invalid size
        return false;
...
// between limits
bool AG_ONVIF::set_absolutemove(float x, float y, float z)
{
