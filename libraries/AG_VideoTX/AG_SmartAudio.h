...
public:
    ...
    static AG_SmartAudio *get_singleton(void)
    {
        return _singleton;
    }

    /* Do not allow copies */
    CLASS_NO_COPY(AG_SmartAudio);
...
private:
    // serial interface
    AG_HAL::UARTDriver *_port;                  // UART used to send data to SmartAudio VTX

    //Pointer to singleton
    static AG_SmartAudio* _singleton;
    ...
    bool ignore_crc() const { return AP::vtx().has_option(AG_VideoTX::VideoOptions::VTX_SA_IGNORE_CRC); }
    ...
    bool get_readings(AG_VideoTX *vtx_dest);
