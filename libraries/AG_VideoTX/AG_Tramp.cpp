...
AG_Tramp *AG_Tramp::singleton;

// Calculate tramp protocol checksum of provided buffer
uint8_t AG_Tramp::checksum(uint8_t *buf)
{
    uint8_t cksum = 0;
...
void AG_Tramp::send_command(uint8_t cmd, uint16_t param)
{
    if (port == nullptr) {
        return;
    }
...
char AG_Tramp::handle_response(void)
{
    ...
    case 'v': {
        ...
        if (freq != 0) {
            ...
            if (pit_mode) {
                vtx.set_options(vtx.get_options() | uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
            } else {
                vtx.set_options(vtx.get_options() & ~uint8_t(AG_VideoTX::VideoOptions::VTX_PITMODE));
            }
...
void AG_Tramp::reset_receiver(void)
{
...
char AG_Tramp::receive_response()
{
    if (port == nullptr) {
        return 0;
    }
...
void AG_Tramp::send_query(uint8_t cmd)
{
...
void AG_Tramp::set_status(TrampStatus _status)
{
    status = _status;
#ifdef TRAMP_DEBUG
    switch (status) {
...
void AG_Tramp::process_requests()
{
    if (port == nullptr) {
        return;
    }
    ...
    case TrampStatus::TRAMP_STATUS_ONLINE_MONITOR_FREQPWRPIT: {
        ...
        if (retry_count > 0 && ((now - last_time_us) >= TRAMP_MIN_REQUEST_PERIOD_US)) {
            ...
            if (!is_race_lock_enabled() && vtx.update_frequency()) {
                ...
                send_command('I', vtx.has_option(AG_VideoTX::VideoOptions::VTX_PITMODE) ? 0 : 1);
...
bool AG_Tramp::is_device_ready()
{
    return status >= TrampStatus::TRAMP_STATUS_ONLINE_MONITOR_FREQPWRPIT;
}

void AG_Tramp::set_frequency(uint16_t freq)
{
    uint8_t freqValid;
...
bool AG_Tramp::init(void)
{
    ...
    // init uart
    port = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_Tramp, 0);
    if (port != nullptr) {
        port->configure_parity(0);
        port->set_stop_bits(1);
        port->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
        port->set_options((port->get_options() & ~AG_HAL::UARTDriver::OPTION_RXINV));
