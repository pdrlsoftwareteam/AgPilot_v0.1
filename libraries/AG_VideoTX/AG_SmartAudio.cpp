...
AG_SmartAudio::AG_SmartAudio()
{
    _singleton = this;
}

AG_SmartAudio *AG_SmartAudio::_singleton;

// initialization start making a request settings to the vtx
bool AG_SmartAudio::init()
{
    ...
    // init uart
    _port = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_SmartAudio, 0);
    if (_port!=nullptr) {
        _port->configure_parity(0);
        _port->set_stop_bits(AP::vtx().has_option(AG_VideoTX::VideoOptions::VTX_SA_ONE_STOP_BIT) ? 1 : 2);
        _port->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
        _port->set_options((_port->get_options() & ~AG_HAL::UARTDriver::OPTION_RXINV)
            | AG_HAL::UARTDriver::OPTION_HDPLEX | AG_HAL::UARTDriver::OPTION_PULLDOWN_TX | AG_HAL::UARTDriver::OPTION_PULLDOWN_RX);
        if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_SmartAudio::loop, void),
                                          "SmartAudio",
                                          768, AG_HAL::Scheduler::PRIORITY_IO, -1)) {
            return false;
        }
...
void AG_SmartAudio::update_vtx_params()
{
    AG_VideoTX& vtx = AP::vtx();
    ...
    if (_vtx_freq_change_pending || _vtx_power_change_pending || _vtx_options_change_pending) {
        ...
        if (pitMode != vtx.get_pitmode() && _protocol_version >= SMARTAUDIO_SPEC_PROTOCOL_v2) {
            if (vtx.get_pitmode()) {
                ...
                // turn it off
                mode = 0x04 | ((opts & uint8_t(AG_VideoTX::VideoOptions::VTX_UNLOCKED)) << 2);
            } else {
                debug("Turning ON pitmode");
                // turn it on (in range pitmode flag)
                mode = 0x01 | ((opts & uint8_t(AG_VideoTX::VideoOptions::VTX_UNLOCKED)) << 2);
            }
        } else {
            mode = ((opts & uint8_t(AG_VideoTX::VideoOptions::VTX_UNLOCKED)) << 2);
            if (pitMode) {
                mode |= 0x01;
            }
        }
 ...
 */
void AG_SmartAudio::send_request(const Frame& requestFrame, uint8_t size)
{
    AG_VideoTX &vtx = AP::vtx();
    ...
    // write request
    if (vtx.has_option(AG_VideoTX::VideoOptions::VTX_PULLDOWN)) {
 ...
 **/
bool AG_SmartAudio::read_response(uint8_t *response_buffer)
{
...
void AG_SmartAudio::push_command_only_frame(uint8_t cmd_id)
{
    Packet command;
...
void AG_SmartAudio::push_uint8_command_frame(uint8_t cmd_id, uint8_t data)
{
    Packet command;
...
void AG_SmartAudio::push_uint16_command_frame(uint8_t cmd_id, uint16_t data)
{
    Packet command;
 ...
 * */
void AG_SmartAudio::request_settings()
{
    debug("request_settings()");
    push_command_only_frame(SMARTAUDIO_CMD_GET_SETTINGS);
}

void AG_SmartAudio::set_operation_mode(uint8_t mode)
{
...
/**
     ...
     */
void AG_SmartAudio::set_frequency(uint16_t frequency, bool isPitModeFreq)
{
...
void AG_SmartAudio::set_channel(uint8_t channel)
{
 ...
 * */
void AG_SmartAudio::request_pit_mode_frequency()
{
...
void AG_SmartAudio::set_power(uint8_t power_level)
{
...
void AG_SmartAudio::set_band_channel(const uint8_t band, const uint8_t channel)
{
...
void AG_SmartAudio::unpack_frequency(AG_SmartAudio::Settings *settings, const uint16_t frequency)
{
...
// SmartAudio v1/v2
void AG_SmartAudio::unpack_settings(Settings *settings, const SettingsResponseFrame *frame)
{
...
// SmartAudio v2.1
void AG_SmartAudio::unpack_settings(Settings *settings, const SettingsExtendedResponseFrame *frame)
{
...
#ifdef SA_DEBUG
void AG_SmartAudio::print_bytes_to_hex_string(const char* msg, const uint8_t buf[], uint8_t len)
{
...
void AG_SmartAudio::print_settings(const Settings* settings)
{
...
void AG_SmartAudio::update_vtx_settings(const Settings& settings)
{
    AG_VideoTX& vtx = AP::vtx();
    ...
    if (settings.version == SMARTAUDIO_SPEC_PROTOCOL_v21) {
        ...
        vtx.set_power_level(settings.power, AG_VideoTX::PowerActive::Active);
...
bool  AG_SmartAudio::parse_response_buffer(const uint8_t *buffer)
{
    ...
    case SMARTAUDIO_RSP_SET_POWER: {
        ...
        case SMARTAUDIO_SPEC_PROTOCOL_v21:
            if (vtx.get_configured_power_dbm() != power) {
                vtx.update_power_dbm(vtx.get_configured_power_dbm(), AG_VideoTX::PowerActive::Inactive);
            }
...
void AG_SmartAudio::update_baud_rate()
{
