...
AG_VideoTX *AG_VideoTX::singleton;

const AG_Param::GroupInfo AG_VideoTX::var_info[] = {

    // @Param: ENABLE
    // @DisplayName: Is the Video Transmitter enabled or not
    // @Description: Toggles the Video Transmitter on and off
    // @Values: 0:Disable,1:Enable
    AP_GROUPINFO_FLAGS("ENABLE", 1, AG_VideoTX, _enabled, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 1 1000
    AP_GROUPINFO("POWER",    2, AG_VideoTX, _power_mw, 0),
    ...
    // @User: Standard
    // @Range: 0 7
    AP_GROUPINFO("CHANNEL",  3, AG_VideoTX, _channel, 0),
    ...
    AP_GROUPINFO("BAND",  4, AG_VideoTX, _band, 0),
    ...
    // @ReadOnly: True
    // @Range: 5000 6000
    AP_GROUPINFO("FREQ",  5, AG_VideoTX, _frequency_mhz, 0),
    ...
    AP_GROUPINFO("OPTIONS",  6, AG_VideoTX, _options, 0),
    ...
    // @Range: 25 1000
    AP_GROUPINFO("MAX_POWER", 7, AG_VideoTX, _max_power_mw, 800),
...
const char * AG_VideoTX::band_names[] = {"A","B","E","F","R","L"};

const uint16_t AG_VideoTX::VIDEO_CHANNELS[AG_VideoTX::MAX_BANDS][VTX_MAX_CHANNELS] =
{
...
AG_VideoTX::PowerLevel AG_VideoTX::_power_levels[VTX_MAX_POWER_LEVELS] = {
    // level, mw, dbm, dac
...
AG_VideoTX::AG_VideoTX()
{
    if (singleton) {
        AG_HAL::panic("Too many VTXs");
        return;
    }
    singleton = this;
...
AG_VideoTX::~AG_VideoTX(void)
{
    singleton = nullptr;
}

bool AG_VideoTX::init(void)
{
    if (_initialized) {
        return false;
    }
...
bool AG_VideoTX::get_band_and_channel(uint16_t freq, VideoBand& band, uint8_t& channel)
{
    for (uint8_t i = 0; i < AG_VideoTX::MAX_BANDS; i++) {
...
void AG_VideoTX::set_configured_power_mw(uint16_t power)
{
    _power_mw.set_and_save_ifchanged(power);
}

uint8_t AG_VideoTX::find_current_power() const
{
    for (uint8_t i = 0; i < VTX_MAX_POWER_LEVELS; i++) {
        if (_power_mw == _power_levels[i].mw) {
            return i;
...
void AG_VideoTX::set_power_dbm(uint8_t power, PowerActive active)
{
...
uint8_t AG_VideoTX::update_power_dbm(uint8_t power, PowerActive active)
{
    for (uint8_t i = 0; i < VTX_MAX_POWER_LEVELS; i++) {
        if (power == _power_levels[i].dbm) {
...
void AG_VideoTX::update_all_power_dbm(uint8_t nlevels, const uint8_t power[])
{
...
// set the power in mw
void AG_VideoTX::set_power_mw(uint16_t power)
{
    for (uint8_t i = 0; i < VTX_MAX_POWER_LEVELS; i++) {
...
// set the power "level"
void AG_VideoTX::set_power_level(uint8_t level, PowerActive active)
{
...
// set the power dac
void AG_VideoTX::set_power_dac(uint16_t power, PowerActive active)
{
...
void AG_VideoTX::set_enabled(bool enabled)
{
...
void AG_VideoTX::set_power_is_current()
{
    set_power_dbm(get_configured_power_dbm());
}

void AG_VideoTX::set_freq_is_current()
{
    _current_frequency = _frequency_mhz;
    _current_band = _band;
    _current_channel = _channel;
...
bool AG_VideoTX::update_options() const
{
    if (!_defaults_set) {
        return false;
    }
...
bool AG_VideoTX::update_power() const {
...
bool AG_VideoTX::have_params_changed() const
{
    return _enabled
        && (update_power()
        || update_band()
...
void AG_VideoTX::update_configured_frequency()
{
...
void AG_VideoTX::update_configured_channel_and_band()
{
    VideoBand band;
    uint8_t channel;
...
void AG_VideoTX::announce_vtx_settings() const
{
...
void AG_VideoTX::change_power(int8_t position)
{
...
namespace AP {
    AG_VideoTX& vtx() {
        return *AG_VideoTX::get_singleton();
    }
};
