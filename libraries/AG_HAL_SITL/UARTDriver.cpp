...
#if HAL_GCS_ENABLED
#include <AG_HAL/utility/packetise.h>
#endif
...
void UARTDriver::begin(uint32_t baud, uint16_t rxSpace, uint16_t txSpace)
{
    if (_portNumber >= ARRAY_SIZE(_sitlState->_uart_path)) {
        AG_HAL::panic("port number out of range; you may need to extend _sitlState->_uart_path");
    }
    ...
    if (strcmp(path, "GPS1") == 0) {
        ...
        if (strcmp(devtype, "tcp") == 0) {
            ...
            if (args2 == nullptr) {
                AG_HAL::panic("Invalid tcp client path: %s", path);
            }
            ...
            if (_fd == -1) {
                AG_HAL::panic("Failed to open (%s): %m", args1);
            }
            _connected = true;
            ...
            AG_HAL::panic("Invalid device path: %s", path);
        }
        free(s);
    }
...
void UARTDriver::flush(void)
{
    ...
    while (AG_HAL::millis() - start_ms < 1000) {
...
void UARTDriver::_uart_start_connection(void)
{
    ...
    if (!_connected) {
        ...
        if (_fd == -1) {
            ...
            if (AG_HAL::millis() - last_error_print_ms > 5000) {
                ::printf("Failed to open (%s): %s\n", _uart_path, strerror(errno));
                last_error_print_ms = AG_HAL::millis();
            }
            return;
        }
    ...
    if (_fd == -1) {
        AG_HAL::panic("Unable to open UART %s", _uart_path);
    }
...
uint16_t UARTDriver::read_from_async_csv(uint8_t *buffer, uint16_t space)
{
    ...
    const uint32_t micros = AG_HAL::micros();
    if (micros < 5000000) {
    ...
    for (i=0; i<space; i++) {
        ...
        while (!logic_async_csv.loaded) {
            ...
            switch (retcode) {
            case AG_CSVReader::RetCode::OK:
                continue;
            case AG_CSVReader::RetCode::ERROR:
                AG_HAL::panic("Malformed CSV?");
            case AG_CSVReader::RetCode::TERM_DONE:
            case AG_CSVReader::RetCode::VECTOR_DONE:
                ...
                case 3:  // framing error
                    break;
                case 4:
                    AG_HAL::panic("Too many terms in CSV, want (name,type,start_time,duration,data");
                }
                logic_async_csv.terms_seen++;
                if (retcode != AG_CSVReader::RetCode::VECTOR_DONE) {
                    break;
                }

                // we've handled the last term, now handle the vector:
                if (logic_async_csv.terms_seen != 4) {
                    AG_HAL::panic("Incorrect number off terms in CSV, want (Time [s],Value,Parity Error,Framing Error)");
                }
...
void UARTDriver::_timer_tick(void)
{
    ...
    if (nread > 0) {
        ...
        _receive_timestamp = AG_HAL::micros64();
    }
}
