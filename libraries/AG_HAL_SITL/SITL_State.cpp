...
void SITL_State::_set_param_default(const char *parm)
{
    ...
    *p = 0;
    ...
    AG_Param *vp = AG_Param::find(pdup, &var_type);
    if (vp == nullptr) {
...
void SITL_State::_fdm_input_step(void)
{
    ...
    if (AG_HAL::millis() - last_pwm_input >= 20 && _sitl != nullptr && _sitl->rc_fail != SITL::SIM::SITL_RCFail_NoPulses) {
        last_pwm_input = AG_HAL::millis();
        new_rc_input = true;
    }
...
void SITL_State::wait_clock(uint64_t wait_time_usec)
{
    ...
    while (AG_HAL::micros64() < wait_time_usec) {
...
SITL::SerialDevice *SITL_State::create_serial_sim(const char *name, const char *arg)
{
    if (streq(name, "vicon")) {
        if (vicon != nullptr) {
            AG_HAL::panic("Only one vicon system at a time");
        }
        ...
        if (teraranger_serial != nullptr) {
            AG_HAL::panic("Only one teraranger_serial at a time");
        }
        ...
        if (lightwareserial != nullptr) {
            AG_HAL::panic("Only one lightwareserial at a time");
        }
        ...
        if (rf_mavlink != nullptr) {
            AG_HAL::panic("Only one rf_mavlink at a time");
        }
        ...
        if (frsky_d != nullptr) {
            AG_HAL::panic("Only one frsky_d at a time");
        }
        frsky_d = new SITL::Frsky_D();
        return frsky_d;
    // } else if (streq(name, "frsky-SPort")) {
    //     if (frsky_sport != nullptr) {
    //         AG_HAL::panic("Only one frsky_sport at a time");
    //     }
    ...
    //         AG_HAL::panic("Only one frsky_sport passthrough at a time");
    //     }
    ...
    //     return frsky_sportpassthrough;
        ...
        if (crsf != nullptr) {
            AG_HAL::panic("Only one crsf at a time");
        }
        crsf = new SITL::CRSF();
        return crsf;
#endif
        ...
        if (rplidara2 != nullptr) {
            AG_HAL::panic("Only one rplidara2 at a time");
        }
        ...
        if (terarangertower != nullptr) {
            AG_HAL::panic("Only one terarangertower at a time");
        }
        ...
        if (sf45b != nullptr) {
            AG_HAL::panic("Only one sf45b at a time");
        }
        ...
        if (ais != nullptr) {
            AG_HAL::panic("Only one AIS at a time");
        }
        ais = new SITL::AIS();
        return ais;
#endif
        ...
        if (p == nullptr) {
            AG_HAL::panic("Need a GPS number (e.g. sim:gps:1)");
        }
        uint8_t x = atoi(p+1);
        if (x <= 0 || x > ARRAY_SIZE(gps)) {
            AG_HAL::panic("Bad GPS number %u", x);
        }
        gps[x-1] = new SITL::GPS(x-1);
        return gps[x-1];
    }

    AG_HAL::panic("unknown simulated device: %s", name);
}
...
void SITL_State::_fdm_input_local(void)
{
    ...
    if (_sitl) {
        ...
        hal.scheduler->stop_clock(AG_HAL::micros64()+100);
    }
            ...
            if (_sitl->fetteconewireesc_sim.enabled()) {
                ...
                for (uint8_t i=0; i<ARRAY_SIZE(input.servos); i++) {
                    if (input.servos[i] != 0 && input.servos[i] < 1000) {
                        AG_HAL::panic("Bad input servo value (%u)", input.servos[i]);
                    }
                }
            }
        }
...
void SITL_State::set_height_agl(void)
{
    ...
    if (_sitl != nullptr &&
        ...
        // get height above terrain from AG_Terrain. This assumes
        // AG_Terrain is working
        float terrain_height_amsl;
        Location location;
        location.lat = _sitl->state.latitude*1.0e7;
        location.lng = _sitl->state.longitude*1.0e7;

        AG_Terrain *_terrain = AG_Terrain::get_singleton();
        if (_terrain != nullptr &&
