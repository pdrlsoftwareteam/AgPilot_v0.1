...
#include <AG_HAL_SITL/I2CDevice.h>
#include "Scheduler.h"
...
AG_HAL::Proc Scheduler::_failsafe = nullptr;

AG_HAL::MemberProc Scheduler::_timer_proc[SITL_SCHEDULER_MAX_TIMER_PROCS] = {nullptr};
uint8_t Scheduler::_num_timer_procs = 0;
bool Scheduler::_in_timer_proc = false;

AG_HAL::MemberProc Scheduler::_io_proc[SITL_SCHEDULER_MAX_TIMER_PROCS] = {nullptr};
...
void Scheduler::delay_microseconds(uint16_t usec)
{
    ...
    do {
        uint64_t dtime = AG_HAL::micros64() - start;
        if (dtime >= usec) {
            break;
        }
...
bool Scheduler::thread_create(AG_HAL::MemberProc proc, const char *name, uint32_t stack_size, priority_base base, int8_t priority)
{
    ...
    a->f = (AG_HAL::MemberProc *)malloc(sizeof(proc));
    if (!a->f) {
        goto failed;
    }
    ...
    if (pthread_attr_setstack(&a->attr, a->stack, alloc_stack) != 0) {
        AG_HAL::panic("Failed to set stack of size %u for thread %s", alloc_stack, name);
    }
#endif
    ...
    for (struct thread_attr *p=threads; p; p=p->next) {
        ...
        for (uint8_t i=0; i<ncheck; i++) {
            if (p->stack_min[i] != stackfill) {
                AG_HAL::panic("stack overflow in thread %s\n", p->name);
            }
        }
    }
}
