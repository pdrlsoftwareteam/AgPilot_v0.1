...
public:
    ...
    int16_t receive(AG_HAL::CANFrame& out_frame, uint64_t& out_timestamp_us,
    ...
    bool set_event_handle(AG_HAL::EventHandle* handle) override;
    ...
    class CANSocketEventSource : public AG_HAL::EventSource {
...
private:
    ...
    int _write(const AG_HAL::CANFrame& frame) const;

    int _read(AG_HAL::CANFrame& frame, uint64_t& ts_usec, bool& loopback) const;

    int _readfd(AG_HAL::CANFrame& frame, uint64_t& ts_usec, bool& loopback) const;

    void _incrementNumFramesInSocketTxQueue();

    void _confirmSentFrame();

    bool _wasInPendingLoopbackSet(const AG_HAL::CANFrame& frame);
    ...
    AG_HAL::EventHandle *_evt_handle;
     ...
     */
    struct bus_stats : public AG_HAL::CANIface::bus_stats_t {
        uint32_t tx_full;
