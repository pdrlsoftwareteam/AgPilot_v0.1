...
static void _sig_fpe(int signum)
{
    ...
    AG_HAL::dump_stack_trace();
    AG_HAL::dump_core_file();
    abort();
}
...
void SITL_State::_parse_command_line(int argc, char * const argv[])
{
    ...
    struct AG_Param::defaults_table_struct temp_cmdline_param{};
    ...
    while (!is_replay && (opt = gopt.getoption()) != -1) {
        ...
        case 'w':
            ...
            break;
        case 'u':
            AG_Param::set_hide_disabled_groups(false);
            break;
        case 's':
