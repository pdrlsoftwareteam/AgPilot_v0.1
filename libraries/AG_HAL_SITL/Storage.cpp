...
//        _save_backup();  // FIXME
        ...
        AG_HAL::panic("Unable to init RAMTRON storage");
#endif
    }
#endif
...
{
    if (!_flash.init()) {
        AG_HAL::panic("unable to init flash storage");
    }
}
...
static bool sitl_flash_write(uint32_t addr, const uint8_t *data, uint32_t length)
{
    ...
    if (pread(flash_fd, old, length, addr) != (ssize_t)length) {
        AG_HAL::panic("Failed to read flash.dat at %u len=%u", unsigned(addr), unsigned(length));
    }
    ...
    if ((length % HAL_FLASH_MIN_WRITE_SIZE) != 0 || (addr % HAL_FLASH_MIN_WRITE_SIZE) != 0) {
        AG_HAL::panic("Attempt to write flash at %u length %u\n", addr, length);
    }
    ...
    for (uint32_t i=0; i<length; i+= 32) {
        ...
        for (uint32_t j=0; j<32; j++) {
            if (old[i+j] != 0xFF) {
                AG_HAL::panic("Attempt to write modified flash at %u length %u\n", addr+i+j, length);
            }
        }
    }
#endif
    ...
    for (uint32_t i=0; i<length; i++) {
        ...
        if (data[i] & ~old[i]) {
            AG_HAL::panic("Attempt to set flash byte 0x%02x from 0x%02x at %u\n", data[i], old[i], addr+i);
        }
#else
...
bool Storage::healthy(void)
{
    ...
    return AG_HAL::millis() - _last_empty_ms < 2000;
}
