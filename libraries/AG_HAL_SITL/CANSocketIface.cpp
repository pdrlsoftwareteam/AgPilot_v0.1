...
#define Debug(fmt, args...) do { AP::can().log_text(AG_CANManager::LOG_DEBUG, "CANLinuxIface", fmt, ##args); } while (0)
#else
...
static can_frame makeSocketCanFrame(const AG_HAL::CANFrame& uavcan_frame)
{
    can_frame sockcan_frame { uavcan_frame.id& AG_HAL::CANFrame::MaskExtID, uavcan_frame.dlc, { } };
...
static canfd_frame makeSocketCanFDFrame(const AG_HAL::CANFrame& uavcan_frame)
{
    canfd_frame sockcan_frame { uavcan_frame.id& AG_HAL::CANFrame::MaskExtID, AG_HAL::CANFrame::dlcToDataLength(uavcan_frame.dlc), CANFD_BRS, 0, 0, { } };
    std::copy(uavcan_frame.data, uavcan_frame.data + AG_HAL::CANFrame::dlcToDataLength(uavcan_frame.dlc), sockcan_frame.data);
    if (uavcan_frame.isExtended()) {
        sockcan_frame.can_id |= CAN_EFF_FLAG;
    }
    if (uavcan_frame.isErrorFrame()) {
...
static AG_HAL::CANFrame makeCanFrame(const can_frame& sockcan_frame)
{
    AG_HAL::CANFrame can_frame(sockcan_frame.can_id & CAN_EFF_MASK, sockcan_frame.data, sockcan_frame.can_dlc);
    if (sockcan_frame.can_id & CAN_EFF_FLAG) {
        can_frame.id |= AG_HAL::CANFrame::FlagEFF;
    }
    if (sockcan_frame.can_id & CAN_ERR_FLAG) {
        can_frame.id |= AG_HAL::CANFrame::FlagERR;
    }
    if (sockcan_frame.can_id & CAN_RTR_FLAG) {
        can_frame.id |= AG_HAL::CANFrame::FlagRTR;
    }
    return can_frame;
}

static AG_HAL::CANFrame makeCanFDFrame(const canfd_frame& sockcan_frame)
{
    AG_HAL::CANFrame can_frame(sockcan_frame.can_id & CAN_EFF_MASK, sockcan_frame.data, sockcan_frame.len);
    if (sockcan_frame.can_id & CAN_EFF_FLAG) {
        can_frame.id |= AG_HAL::CANFrame::FlagEFF;
    }
    if (sockcan_frame.can_id & CAN_ERR_FLAG) {
...
int16_t CANIface::send(const AG_HAL::CANFrame& frame, const uint64_t tx_deadline,
                       const CANIface::CanIOFlags flags)
{
    ...
    return AG_HAL::CANIface::send(frame, tx_deadline, flags);
}

int16_t CANIface::receive(AG_HAL::CANFrame& out_frame, uint64_t& out_timestamp_us,
                          CANIface::CanIOFlags& out_flags)
{
    ...
    (void)_rx_queue.pop();
    return AG_HAL::CANIface::receive(out_frame, out_timestamp_us, out_flags);
}
                              ...
                              const std::uint16_t num_configs)
{
    ...
    for (unsigned i = 0; i < num_configs; i++) {
        ...
        _hw_filters_container[i].can_id   = fc.id   & AG_HAL::CANFrame::MaskExtID;
        _hw_filters_container[i].can_mask = fc.mask & AG_HAL::CANFrame::MaskExtID;
        if (fc.id & AG_HAL::CANFrame::FlagEFF) {
            _hw_filters_container[i].can_id |= CAN_EFF_FLAG;
        }
        if (fc.id & AG_HAL::CANFrame::FlagRTR) {
            _hw_filters_container[i].can_id |= CAN_RTR_FLAG;
        }
        if (fc.mask & AG_HAL::CANFrame::FlagEFF) {
            _hw_filters_container[i].can_mask |= CAN_EFF_FLAG;
        }
        if (fc.mask & AG_HAL::CANFrame::FlagRTR) {
...
void CANIface::_pollWrite()
{
    while (_hasReadyTx()) {
        ...
        uint64_t curr_time = AG_HAL::native_micros64();
...
bool CANIface::_pollRead()
{
    ...
    while (iterations_count < CAN_MAX_POLL_ITERATIONS_COUNT)
    {
        iterations_count++;
        CanRxItem rx;
        rx.timestamp_us = AG_HAL::native_micros64();  // Monotonic timestamp is not required to be precise (unlike UTC)
        bool loopback = false;
        int res;
...
int CANIface::_write(const AG_HAL::CANFrame& frame) const
{
    if (_fd < 0) {
        return -1;
    }
...
int CANIface::_read(AG_HAL::CANFrame& frame, uint64_t& timestamp_us, bool& loopback) const
{
    if (_fd < 0) {
        return -1;
    }
     ...
     * Timestamp
     */
    timestamp_us = AG_HAL::native_micros64();
    return 1;
}

int CANIface::_readfd(AG_HAL::CANFrame& frame, uint64_t& timestamp_us, bool& loopback) const
{
    if (_fd < 0) {
        return -1;
    }
...
bool CANIface::_wasInPendingLoopbackSet(const AG_HAL::CANFrame& frame)
{
...
bool CANIface::select(bool &read_select, bool &write_select,
                        const AG_HAL::CANFrame* const pending_tx, uint64_t blocking_deadline)
{
    ...
    if (need_block) {
        ...
        if (_evt_handle != nullptr && blocking_deadline > AG_HAL::native_micros64()) {
            _evt_handle->wait(blocking_deadline - AG_HAL::native_micros64());
        }
    }
...
bool CANIface::set_event_handle(AG_HAL::EventHandle* handle) {
    _evt_handle = handle;
...
bool CANIface::CANSocketEventSource::wait(uint64_t duration, AG_HAL::EventHandle* evt_handle)
{
