...
HAL_SITL& hal_sitl = (HAL_SITL&)AG_HAL::get_HAL();
...
void HAL_SITL::run(int argc, char * const argv[], Callbacks* callbacks) const
{
    ...
    if (getenv("SITL_WATCHDOG_RESET")) {
        const AG_HAL::Util::PersistentData &pd = util->persistent_data;
    ...
    uint32_t last_watchdog_save = AG_HAL::millis();
...
{
    execv(new_argv[0], new_argv);
    AG_HAL::panic("PANIC: REBOOT FAILED: %s", strerror(errno));
}
