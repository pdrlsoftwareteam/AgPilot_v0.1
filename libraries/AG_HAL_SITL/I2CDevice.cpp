...
AG_HAL::Device::PeriodicHandle I2CBus::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
                             ...
                             uint32_t timeout_ms)
{
    if (bus >= ARRAY_SIZE(buses)) {
        return AG_HAL::OwnPtr<AG_HAL::I2CDevice>(nullptr);
    }
    auto dev = AG_HAL::OwnPtr<AG_HAL::I2CDevice>(new I2CDevice(buses[bus], address));
    return dev;
}
...
AG_HAL::Device::PeriodicHandle I2CDevice::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
