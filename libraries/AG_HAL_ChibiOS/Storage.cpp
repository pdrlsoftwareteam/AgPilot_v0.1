...
#define HAL_STORAGE_FILE "/PDRL/" SKETCHNAME ".stg"
#endif
...
// allow for devices with no FRAM chip to fall through to other storage
    ...
    if (_initialisedType != StorageBackend::None) {
        ...
        AG_HAL::panic("Unable to init Storage backend");
    }
}
...
void Storage::_save_backup(void)
{
    ...
    uint32_t start_millis = AG_HAL::millis();
    while(!AP::FS().retry_mount() && (AG_HAL::millis() - start_millis) < 1000) {
...
#else
    AG_HAL::panic("Unable to init storage");
#endif
}
...
bool Storage::healthy(void)
{
    ...
    if (_initialisedType == StorageBackend::SDCard) {
        return log_fd != -1 || AG_HAL::millis() - _last_empty_ms < 30000U;
    }
#endif
    return ((_initialisedType != StorageBackend::None) &&
            (AG_HAL::millis() - _last_empty_ms < 2000u));
}
