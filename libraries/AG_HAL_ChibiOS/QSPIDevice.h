...
class QSPIDevice : public AG_HAL::QSPIDevice
{
public:
    static QSPIDevice *from(AG_HAL::QSPIDevice *dev)
    {
...
class QSPIDeviceManager : public AG_HAL::QSPIDeviceManager
{
public:
    friend class QSPIDevice;

    static QSPIDeviceManager *from(AG_HAL::QSPIDeviceManager *qspi_mgr)
    {
        return static_cast<QSPIDeviceManager*>(qspi_mgr);
    }

    AG_HAL::OwnPtr<AG_HAL::QSPIDevice> get_device(const char *name) override;
private:
