...
#ifndef AG_HAL_SHARED_DMA_ENABLED
#define AG_HAL_SHARED_DMA_ENABLED 0
#endif
...
def write_IMU_config(f):
    ...
    for dev in imu_list:
        ...
        if aux_devid != -1:
            f.write(
            '#define HAL_INS_PROBE%u %s ADD_BACKEND_AUX(AG_InertialSensor_%s::probe(*this,%s),%d)\n'
            % (n, wrapper, driver, ','.join(dev[1:]), aux_devid))
        elif instance != -1:
            f.write(
            '#define HAL_INS_PROBE%u %s ADD_BACKEND_INSTANCE(AG_InertialSensor_%s::probe(*this,%s),%d)\n'
            % (n, wrapper, driver, ','.join(dev[1:]), instance))
        elif dev[-1].startswith("BOARD_MATCH("):
            f.write(
                '#define HAL_INS_PROBE%u %s ADD_BACKEND_BOARD_MATCH(%s, AG_InertialSensor_%s::probe(*this,%s))\n'
                % (n, wrapper, dev[-1], driver, ','.join(dev[1:-1])))
        else:
            f.write(
                '#define HAL_INS_PROBE%u %s ADD_BACKEND(AG_InertialSensor_%s::probe(*this,%s))\n'
...
def write_MAG_config(f):
    ...
    for dev in compass_list:
        ...
        f.write(
            '#define HAL_MAG_PROBE%u %s ADD_BACKEND(DRIVER_%s, AG_Compass_%s::%s(%s))\n'
...
def write_BARO_config(f):
    ...
    for dev in baro_list:
        ...
        args = ['*this'] + dev[1:]
        f.write(
            '#define HAL_BARO_PROBE%u %s ADD_BACKEND(AG_Baro_%s::%s(%s))\n'
...
def write_AIRSPEED_config(f):
    ...
    for dev in airspeed_list:
        ...
        f.write(
            '#define HAL_AIRSPEED_PROBE%u %s ADD_BACKEND(AG_Airspeed_%s::probe(%s))\n'
...
''')
    ...
    if len(romfs) > 0:
        f.write('#define HAL_HAVE_AG_ROMFS_EMBEDDED_H 1\n')
 ...
 */
#define AG_GPS_BACKEND_DEFAULT_ENABLED 0

#ifndef AG_GPS_ERB_ENABLED
#define AG_GPS_ERB_ENABLED 0
#endif

#ifndef AG_GPS_GSOF_ENABLED
#define AG_GPS_GSOF_ENABLED defined(HAL_PERIPH_ENABLE_GPS)
#endif

#ifndef AG_GPS_NMEA_ENABLED
#define AG_GPS_NMEA_ENABLED 0
#endif

#ifndef AG_GPS_SBF_ENABLED
#define AG_GPS_SBF_ENABLED defined(HAL_PERIPH_ENABLE_GPS)
#endif

#ifndef AG_GPS_SBP_ENABLED
#define AG_GPS_SBP_ENABLED 0
#endif

#ifndef AG_GPS_SBP2_ENABLED
#define AG_GPS_SBP2_ENABLED 0
#endif

#ifndef AG_GPS_SIRF_ENABLED
#define AG_GPS_SIRF_ENABLED 0
#endif

#ifndef AG_GPS_MAV_ENABLED
#define AG_GPS_MAV_ENABLED 0
#endif

#ifndef AG_GPS_NOVA_ENABLED
#define AG_GPS_NOVA_ENABLED defined(HAL_PERIPH_ENABLE_GPS)
#endif
