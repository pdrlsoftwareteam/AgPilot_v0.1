...
AG_HAL::Device::PeriodicHandle DeviceBus::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb, AG_HAL::Device *_hal_device)
{
    if (!thread_started) {
        thread_started = true;
        ...
        case AG_HAL::Device::BUS_TYPE_I2C:
            snprintf(name, name_len, "I2C%u",
                     hal_device->bus_num());
            break;

        case AG_HAL::Device::BUS_TYPE_SPI:
        ...
        if (thread_ctx == nullptr) {
            AG_HAL::panic("Failed to create bus thread %s", name);
        }
    }
 ...
 */
bool DeviceBus::adjust_timer(AG_HAL::Device::PeriodicHandle h, uint32_t period_usec)
{
