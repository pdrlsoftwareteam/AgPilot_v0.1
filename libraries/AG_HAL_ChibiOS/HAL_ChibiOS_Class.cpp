...
#include <AG_HAL_Empty/AG_HAL_Empty_Private.h>
#include <AG_HAL_ChibiOS/AG_HAL_ChibiOS_Private.h>
...
#if AP_SIM_ENABLED
static AG_HAL::SIMState xsimstate;
#endif
...
static AG_HAL::HAL::Callbacks* g_callbacks;
...
static void main_loop()
{
    ...
    if (AG_BoardConfig::unlock_flash()) {
        stm32_flash_unprotect_flash();
    } else {
        stm32_flash_protect_flash(false, AG_BoardConfig::protect_flash());
        stm32_flash_protect_flash(true, AG_BoardConfig::protect_bootloader());
    }
#endif
    ...
    if (AG_BoardConfig::watchdog_enabled()) {
        stm32_watchdog_init();
    }

    if (hal.util->was_watchdog_reset()) {
        INTERNAL_ERROR(AG_InternalError::error_t::watchdog_reset);
    }
#endif // IOMCU_FW
...
const AG_HAL::HAL& AG_HAL::get_HAL() {
