...
#include <AG_HAL_ChibiOS/UARTDriver.h>
#include <AG_HAL_ChibiOS/AnalogIn.h>
#include <AG_HAL_ChibiOS/Storage.h>
#include <AG_HAL_ChibiOS/RCOutput.h>
#include <AG_HAL_ChibiOS/RCInput.h>
...
{
    uint64_t start = AG_HAL::micros64();

    while ((AG_HAL::micros64() - start)/1000 < ms) {
...
void Scheduler::register_timer_process(AG_HAL::MemberProc proc)
{
...
void Scheduler::register_io_process(AG_HAL::MemberProc proc)
{
...
void Scheduler::register_timer_failsafe(AG_HAL::Proc failsafe, uint32_t period_us)
{
    _failsafe = failsafe;
}
...
void Scheduler::_monitor_thread(void *arg)
{
    ...
    bool using_watchdog = AG_BoardConfig::watchdog_enabled();
#if HAL_LOGGING_ENABLED
    ...
    while (true) {
        ...
        if (loop_delay >= 500 && !sched->in_expected_delay()) {
            ...
            INTERNAL_ERROR(AG_InternalError::error_t::main_loop_stuck);
        }
...
#if HAL_LOGGING_ENABLED
    uint32_t last_sd_start_ms = AG_HAL::millis();
#endif
...
{
    if (_initialized) {
        AG_HAL::panic("PANIC: scheduler::set_system_initialized called"
                      "more than once");
    }
    _initialized = true;
}
...
{
    AG_HAL::MemberProc *t = (AG_HAL::MemberProc *)ctx;
    (*t)();
    free(t);
}
...
/*
  create a new thread
*/
bool Scheduler::thread_create(AG_HAL::MemberProc proc, const char *name, uint32_t stack_size, priority_base base, int8_t priority)
{
    // take a copy of the MemberProc, it is freed after thread exits
    AG_HAL::MemberProc *tproc = (AG_HAL::MemberProc *)malloc(sizeof(proc));
    if (!tproc) {
        return false;
    }
    *tproc = proc;
...
{
    stm32_watchdog_pat();
    last_watchdog_pat_ms = AG_HAL::millis();
...
void Scheduler::check_stack_free(void)
{
    ...
    if (stack_free(&__main_stack_base__) < min_stack) {
        ...
        AP::internalerror().error(AG_InternalError::error_t::stack_overflow, 0xFFFF);
    }
    ...
    for (thread_t *tp = chRegFirstThread(); tp; tp = chRegNextThread(tp)) {
        if (stack_free(tp->wabase) < min_stack) {
            ...
            AP::internalerror().error(AG_InternalError::error_t::stack_overflow, tp->realprio);
        }
    }
}
