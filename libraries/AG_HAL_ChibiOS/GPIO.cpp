...
static struct gpio_entry {
    ...
    uint8_t pwm_num;
    ioline_t pal_line;
    AG_HAL::GPIO::irq_handler_fn_t fn; // callback for GPIO interface
    bool is_input;
    uint8_t mode;
...
{
#if HAL_WITH_IO_MCU
    if (AG_BoardConfig::io_enabled() && iomcu.valid_GPIO_pin(pin)) {
...
AG_HAL::DigitalSource* GPIO::channel(uint16_t pin)
{
#if HAL_WITH_IO_MCU
    if (AG_BoardConfig::io_enabled() && iomcu.valid_GPIO_pin(pin)) {
 ...
 */
bool GPIO::_attach_interrupt(ioline_t line, AG_HAL::Proc p, uint8_t mode)
{
...
#endif
            INTERNAL_ERROR(AG_InternalError::error_t::gpio_isr);
        }
