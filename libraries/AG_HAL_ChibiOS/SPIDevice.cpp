    ...
    , device_desc(_device_desc)
{
    ...
    set_speed(AG_HAL::Device::SPEED_LOW);

    asprintf(&pname, "SPI:%s:%u:%u",
             device_desc.name,
             (unsigned)bus.bus, (unsigned)device_desc.device);
    AG_HAL::SPIDevice::setup_bankselect_callback(device_desc.bank_select_cb);
    AG_HAL::SPIDevice::set_register_rw_callback(device_desc.register_rw_cb);
...
bool SPIDevice::set_speed(AG_HAL::Device::Speed speed)
{
    switch (speed) {
    case AG_HAL::Device::SPEED_HIGH:
        freq_flag = freq_flag_high;
        break;
    case AG_HAL::Device::SPEED_LOW:
...
bool SPIDevice::do_transfer(const uint8_t *send, uint8_t *recv, uint32_t len)
{
    ...
    if (msg == MSG_TIMEOUT) {
        ...
        if (!hal.scheduler->in_expected_delay()) {
            INTERNAL_ERROR(AG_InternalError::error_t::spi_fail);
        }
...
AG_HAL::Semaphore *SPIDevice::get_semaphore()
{
    return &bus.semaphore;
}


AG_HAL::Device::PeriodicHandle SPIDevice::register_periodic_callback(uint32_t period_usec, AG_HAL::Device::PeriodicCb cb)
{
    return bus.register_periodic_callback(period_usec, cb, this);
}

bool SPIDevice::adjust_periodic_callback(AG_HAL::Device::PeriodicHandle h, uint32_t period_usec)
{
 ...
 */
AG_HAL::OwnPtr<AG_HAL::SPIDevice>
SPIDeviceManager::get_device(const char *name)
{
    ...
    if (i == ARRAY_SIZE(device_table)) {
        return AG_HAL::OwnPtr<AG_HAL::SPIDevice>(nullptr);
    }
    ...
    return AG_HAL::OwnPtr<AG_HAL::SPIDevice>(new SPIDevice(*busp, desc));
}

void SPIDeviceManager::set_register_rw_callback(const char* name, AG_HAL::Device::RegisterRWCb cb)
{
    /* Find the bus description in the table */
    uint8_t i;
    for (i = 0; i<ARRAY_SIZE(device_table); i++) {
...
void SPIDevice::test_clock_freq(void)
{
    ...
    uint16_t len = 1024;
    uint8_t *buf1 = (uint8_t *)hal.util->malloc_type(len, AG_HAL::Util::MEM_DMA_SAFE);
    uint8_t *buf2 = (uint8_t *)hal.util->malloc_type(len, AG_HAL::Util::MEM_DMA_SAFE);
    ...
    hal.util->free_type(buf1, len, AG_HAL::Util::MEM_DMA_SAFE);
    hal.util->free_type(buf2, len, AG_HAL::Util::MEM_DMA_SAFE);
}
