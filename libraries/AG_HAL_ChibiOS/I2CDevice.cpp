                         ...
                         uint8_t *recv, uint32_t recv_len)
{
    ...
    for(uint8_t i=0 ; i <= _retries; i++) {
        ...
        if (I2CD[bus.busnum].i2c->errors & I2C_ISR_LIMIT) {
            INTERNAL_ERROR(AG_InternalError::error_t::i2c_isr);
            break;
        }
...
*/
bool I2CDevice::adjust_periodic_callback(AG_HAL::Device::PeriodicHandle h, uint32_t period_usec)
{
                             ...
                             uint32_t timeout_ms)
{
    ...
    auto dev = AG_HAL::OwnPtr<AG_HAL::I2CDevice>(new I2CDevice(bus, address, bus_clock, use_smbus, timeout_ms));
    return dev;
}
