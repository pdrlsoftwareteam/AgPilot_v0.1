...
void RCOutput::init()
{
    ...
    safety_state = AG_HAL::Util::SAFETY_DISARMED;
...
bool RCOutput::start_led_thread(void)
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&RCOutput::led_thread, void), "led", LED_THD_WA_SIZE, AG_HAL::Scheduler::PRIORITY_LED, 0)) {
        return false;
    }
...
void RCOutput::led_thread()
{
    ...
    while (true) {
        ...
        led_timer_tick(LED_OUTPUT_PERIOD_US + AG_HAL::micros64());
    }
}
...
void RCOutput::rcout_thread()
{
    ...
    while (true) {
        ...
        if (AG_HAL::millis() > 5000 && output_masks) {
            output_masks = false;
...
#endif // AG_HAL_SHARED_DMA_ENABLED
...
void RCOutput::write(uint8_t chan, uint16_t period_us)
{
    ...
    if (safety_state == AG_HAL::Util::SAFETY_DISARMED && !(safety_mask & (1U<<chan))) {
...
void RCOutput::push_local(void)
{
    ...
    bool safety_on = hal.util->safety_switch_state() == AG_HAL::Util::SAFETY_DISARMED;
    for (auto &group : pwm_group_list) {
        if (serial_group) {
            continue;
        }
                               ...
                               uint32_t pulse_time_us, bool is_dshot)
{
    ...
    if (!group.dma_buffer || buffer_length != group.dma_buffer_len) {
        if (group.dma_buffer) {
            hal.util->free_type(group.dma_buffer, group.dma_buffer_len, AG_HAL::Util::MEM_DMA_SAFE);
            group.dma_buffer_len = 0;
        }
        group.dma_buffer = (uint32_t *)hal.util->malloc_type(buffer_length, AG_HAL::Util::MEM_DMA_SAFE);
        if (!group.dma_buffer) {
            group.dma_handle->unlock();
...
void RCOutput::trigger_groups(void)
{
    ...
    */
    min_pulse_trigger_us = AG_HAL::micros64() + trigger_widest_pulse + 50;
...
void RCOutput::dshot_send(pwm_group &group, uint64_t time_out_us)
{
    ...
    if (AG_HAL::micros64() + group.dshot_pulse_time_us > time_out_us) {
        group.dma_handle->unlock();
        return;
    }
...
#ifndef DISABLE_DSHOT
    if (irq.waiter || (group.dshot_state != DshotState::IDLE && group.dshot_state != DshotState::RECV_COMPLETE)
        || AG_HAL::micros64() - group.last_dmar_send_us < (group.dshot_pulse_time_us + 50)) {
        // doing serial output or DMAR input, don't send DShot pulses
        return false;
    }
...
void RCOutput::send_pulses_DMAR(pwm_group &group, uint32_t buffer_length)
{
    ...
    group.last_dmar_send_us = AG_HAL::micros64();
#endif //#ifndef DISABLE_DSHOT
}
...
uint16_t RCOutput::serial_read_bytes(uint8_t *buf, uint16_t len)
{
    ...
    if (!((GPIO *)hal.gpio)->_attach_interrupt(line, serial_bit_irq, AG_HAL::GPIO::INTERRUPT_BOTH)) {
#if RCOU_SERIAL_TIMING_DEBUG
 ...
 */
AG_HAL::Util::safety_state RCOutput::_safety_switch_state(void)
{
#if HAL_WITH_IO_MCU
    if (AG_BoardConfig::io_enabled()) {
...
#endif
    safety_state = AG_HAL::Util::SAFETY_ARMED;
}
...
void RCOutput::safety_update(void)
{
    ...
    const uint16_t led_pattern = safety_state==AG_HAL::Util::SAFETY_DISARMED?0x5500:0xFFFF;
