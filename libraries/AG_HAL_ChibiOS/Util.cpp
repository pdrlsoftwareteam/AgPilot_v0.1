...
void* Util::malloc_type(size_t size, AG_HAL::Util::Memory_Type mem_type)
{
    if (mem_type == AG_HAL::Util::MEM_DMA_SAFE) {
        return malloc_dma(size);
    } else if (mem_type == AG_HAL::Util::MEM_FAST) {
...
void Util::free_type(void *ptr, size_t size, AG_HAL::Util::Memory_Type mem_type)
{
    if (ptr != NULL) {
        free(ptr);
    }
...
/*
  realloc implementation thanks to wolfssl, used by AG_Scripting
 */
...
void Util::toneAlarm_set_buzzer_tone(float frequency, float volume, uint32_t duration_ms)
{
    ...
    if (!(_toneAlarm_types & AG_Notify::Notify_Buzz_DShot) || get_soft_armed() || hal.rcout->get_dshot_esc_type() != RCOutput::DSHOT_ESC_BLHELI) {
        return;
    }
...
Util::FlashBootloader Util::flash_bootloader()
{
    ...
    const uint8_t *fw = AG_ROMFS::find_decompress(fw_name, fw_size);
    if (!fw) {
    ...
    if (!AG_CheckFirmware::check_signed_bootloader(fw, fw_size)) {
...
bool Util::get_persistent_params(ExpandingString &str) const
{
    ...
    const auto *ins = AG_InertialSensor::get_singleton();
    if (ins) {
        ins->get_persistent_params(str);
    }
#endif
#if AP_OPENDRONEID_ENABLED
    const auto *odid = AG_OpenDroneID::get_singleton();
    if (odid) {
...
/*
  apply persistent parameters from the bootloader sector to AG_Param
 */
    ...
    for (char *p = strtok_r(s, "\n", &saveptr);
        ...
        if (eq) {
            *eq = 0;
            ...
            if (AG_Param::set_default_by_name(pname, value)) {
                count++;
                /*
                  ...
                  AG_InertialSensor determines if a calibration has
                ...
                if (strncmp(pname, "INS_ACC", 7) == 0 &&
                    ...
                    AP_Int32 *ap = (AP_Int32 *)AG_Param::find(pname, &ptype);
...
/**
 * This method will generate random values with set size. It will fall back to AG_Math's get_random16()
