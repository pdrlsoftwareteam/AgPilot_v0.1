...
MMCDriver MMCD1;
static AG_HAL::OwnPtr<AG_HAL::SPIDevice> device;
...
/*
  ...
  AG_BoardConfig initialisation. The parameter BRD_SD_SLOWDOWN
...
bool sdcard_init()
{
    ...
    uint8_t sd_slowdown = AG_BoardConfig::get_sdcard_slowdown();
#else
    ...
    device = AG_HAL::get_HAL().spi->get_device("sdcard");
    if (!device) {
...
void spiStartHook(SPIDriver *spip, const SPIConfig *config)
{
    device->set_speed(config == &lowspeed ?
        AG_HAL::Device::SPEED_LOW : AG_HAL::Device::SPEED_HIGH);
}
