...
const AG_Param::GroupInfo AG_Stats::var_info[] = {
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_BOOTCNT",    0, AG_Stats, params.bootcount, 0),
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_FLTTIME",    1, AG_Stats, params.flttime, 0),
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_RUNTIME",    2, AG_Stats, params.runtime, 0),
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_RESET",    3, AG_Stats, params.reset, 1),

    AP_GROUPEND
};

AG_Stats *AG_Stats::_singleton;

// constructor
AG_Stats::AG_Stats(void)
{
    AG_Param::setup_object_defaults(this, var_info);
    _singleton = this;
}

void AG_Stats::copy_variables_from_parameters()
{
...
void AG_Stats::update_flighttime()
{
    if (_flying_ms) {
        WITH_SEMAPHORE(sem);
...
void AG_Stats::set_flying(const bool is_flying)
{
    if (is_flying) {
        if (!_flying_ms) {
            _flying_ms = AG_HAL::millis();
        }
    } else {
        update_flighttime();
        _flying_ms = 0;
 ...
 */
uint32_t AG_Stats::get_flight_time_s(void)
{
    update_flighttime();
    return flttime - flttime_boot;
}

AG_Stats *AP::stats(void)
{
    return AG_Stats::get_singleton();
}
