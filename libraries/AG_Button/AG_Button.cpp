...
AG_Button *AG_Button::_singleton;

const AG_Param::GroupInfo AG_Button::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLE", 0, AG_Button, enable, 0, AP_PARAM_FLAG_ENABLE),
    ...
    AP_GROUPINFO("PIN1",  1, AG_Button, pin[0], -1),
    ...
    // @User: Standard
    // @Values: -1:Disabled,50:AUXOUT1,51:AUXOUT2,52:AUXOUT3,53:AUXOUT4,54:AUXOUT5,55:AUXOUT6
    AP_GROUPINFO("PIN2",  2, AG_Button, pin[1], -1),
    ...
    // @User: Standard
    // @Values: -1:Disabled,50:AUXOUT1,51:AUXOUT2,52:AUXOUT3,53:AUXOUT4,54:AUXOUT5,55:AUXOUT6
    AP_GROUPINFO("PIN3",  3, AG_Button, pin[2], -1),
    ...
    // @User: Standard
    // @Values: -1:Disabled,50:AUXOUT1,51:AUXOUT2,52:AUXOUT3,53:AUXOUT4,54:AUXOUT5,55:AUXOUT6
    AP_GROUPINFO("PIN4",  4, AG_Button, pin[3], -1),
    ...
    // @User: Standard
    // @Range: 0 3600
    AP_GROUPINFO("REPORT_SEND", 5, AG_Button, report_send_time, 10),
    ...
    AP_GROUPINFO("OPTIONS1",  6, AG_Button, options[0], 0),
    ...
    // @User: Standard
    // @Bitmask: 0:PWM Input,1:InvertInput
    AP_GROUPINFO("OPTIONS2",  7, AG_Button, options[1], 0),
    ...
    // @Bitmask: 0:PWM Input,1:InvertInput
    AP_GROUPINFO("OPTIONS3",  8, AG_Button, options[2], 0),
    ...
    // @User: Standard
    // @Bitmask: 0:PWM Input,1:InvertInput
    AP_GROUPINFO("OPTIONS4",  9, AG_Button, options[3], 0),
    ...
    // @User: Standard
    AP_GROUPINFO("FUNC1",  10, AG_Button, pin_func[0], (uint16_t)RC_Channel::AUX_FUNC::DO_NOTHING),

    // @Param: FUNC2
    // @CopyFieldsFrom: BTN_FUNC1
    // @DisplayName: Button Pin 2 RC Channel function
    AP_GROUPINFO("FUNC2",  11, AG_Button, pin_func[1], (uint16_t)RC_Channel::AUX_FUNC::DO_NOTHING),

    // @Param: FUNC3
    // @CopyFieldsFrom: BTN_FUNC1
    // @DisplayName: Button Pin 3 RC Channel function
    AP_GROUPINFO("FUNC3",  12, AG_Button, pin_func[2], (uint16_t)RC_Channel::AUX_FUNC::DO_NOTHING),

    // @Param: FUNC4
    // @CopyFieldsFrom: BTN_FUNC1
    // @DisplayName: Button Pin 4 RC Channel function
    AP_GROUPINFO("FUNC4",  13, AG_Button, pin_func[3], (uint16_t)RC_Channel::AUX_FUNC::DO_NOTHING),
...
// constructor
AG_Button::AG_Button(void)
{
    AG_Param::setup_object_defaults(this, var_info);

    if (_singleton != nullptr) {
        AG_HAL::panic("AG_Button must be singleton");
    }
    _singleton = this;
}
...
void AG_Button::update(void)
{
    ...
    if (!initialised) {
        ...
        hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&AG_Button::timer_update, void));        
    }
    ...
    const uint64_t now_ms = AG_HAL::millis64();
    ...
    if (last_debounce_ms != 0 &&
        (AG_HAL::millis() - last_report_ms) > AP_BUTTON_REPORT_PERIOD_MS &&
        (AG_HAL::millis64() - last_debounce_ms) < report_send_time*1000ULL) {
...
void AG_Button::run_aux_functions(bool force)
{
...
// used by scripting
bool AG_Button::get_button_state(uint8_t number)
{
...
/*
  get current mask
 */
uint8_t AG_Button::get_mask(void)
{
    uint8_t mask = 0;
    for (uint8_t i=0; i<AP_BUTTON_NUM_PINS; i++) {
        if (pin[i] == -1) {
 ...
 */
void AG_Button::timer_update(void)
{
    if (!enable) {
        return;
    }
 ...
 */
void AG_Button::send_report(void) const
{
 ...
 */
void AG_Button::setup_pins(void)
{
    for (uint8_t i=0; i<AP_BUTTON_NUM_PINS; i++) {
        if (is_pwm_input(i)) {
...
bool AG_Button::arming_checks(size_t buflen, char *buffer) const
{
    if (!enable) {
        return true;
    }
...
AG_Button &button()
{
    return *AG_Button::get_singleton();
}
