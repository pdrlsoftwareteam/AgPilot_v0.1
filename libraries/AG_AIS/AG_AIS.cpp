...
#define AG_AIS_DUMMY_METHODS_ENABLED ((AG_AIS_ENABLED == 2) && !APM_BUILD_TYPE(APM_BUILD_Rover))

#if !AG_AIS_DUMMY_METHODS_ENABLED
...
const AG_Param::GroupInfo AG_AIS::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("TYPE", 1, AG_AIS, _type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 1 100
    // @User: Advanced
    AP_GROUPINFO("LIST_MAX", 2, AG_AIS, _max_list, 25),
    ...
    // @Range: 1 2000
    // @User: Advanced
    AP_GROUPINFO("TIME_OUT", 3, AG_AIS, _time_out, 600),
    ...
    // @User: Advanced
    AP_GROUPINFO("LOGGING", 4, AG_AIS, _log_options, AIS_OPTIONS_LOG_UNSUPPORTED_RAW | AIS_OPTIONS_LOG_DECODED),
...
AG_AIS::AG_AIS()
{
    if (_singleton != nullptr) {
        AG_HAL::panic("AIS must be singleton");
    }
    _singleton = this;
...
void AG_AIS::init()
{
    ...
    _uart = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_AIS, 0);
    if (_uart == nullptr) {
        return;
    }

    _uart->begin(AP::serialmanager().find_baudrate(AG_SerialManager::SerialProtocol_AIS, 0));
}
...
void AG_AIS::send(mavlink_channel_t chan)
{
    if (!enabled()) {
        return;
    }
...
void AG_AIS::buffer_shift(uint8_t i)
{
...
bool AG_AIS::get_vessel_index(uint32_t mmsi, uint16_t &index, uint32_t lat, uint32_t lon)
{
    const uint16_t list_size = _list.max_items();
...
void AG_AIS::clear_list_item(uint16_t index)
{
...
bool AG_AIS::payload_decode(const char *payload)
{
...
bool AG_AIS::decode_position_report(const char *payload, uint8_t type)
{
    ...
    _list[index].last_update_ms = AG_HAL::millis();

    return true;
}

bool AG_AIS::decode_base_station_report(const char *payload)
{
    if (strlen(payload) != 28) {
        return false;
    }
...
bool AG_AIS::decode_static_and_voyage_data(const char *payload)
{
...
void AG_AIS::get_char(const char *payload, char *array, uint16_t low, uint16_t high)
{
...
uint32_t AG_AIS::get_bits(const char *payload, uint16_t low, uint16_t high)
{
...
int32_t AG_AIS::get_bits_signed(const char *payload, uint16_t low, uint16_t high)
{
...
uint8_t AG_AIS::payload_char_decode(const char c)
{
    uint8_t value = c;
    value -= 48;
    if (value > 40) {
...
void AG_AIS::log_raw(const AIVDM *msg)
{
...
bool AG_AIS::decode_latest_term()
{
...
const AG_Param::GroupInfo AG_AIS::var_info[] = { AP_GROUPEND };
AG_AIS::AG_AIS() {};

bool AG_AIS::enabled() const { return false; }

void AG_AIS::init() {};
void AG_AIS::update() {};
void AG_AIS::send(mavlink_channel_t chan) {};

AG_AIS *AG_AIS::get_singleton() { return nullptr; }

#endif // AG_AIS_DUMMY_METHODS_ENABLED

AG_AIS *AG_AIS::_singleton;

#endif  // AG_AIS_ENABLED
