#include <AG_Winch/AG_Winch_Daiwa.h>
...
bool AG_Winch_Daiwa::healthy() const
{
    // healthy if we have received data within 3 seconds
    return (AG_HAL::millis() - data_update_ms < 3000);
}

void AG_Winch_Daiwa::init()
{
    // call superclass init
    AG_Winch_Backend::init();

    // initialise serial connection to winch
    const AG_SerialManager &serial_manager = AP::serialmanager();
    uart = serial_manager.find_serial(AG_SerialManager::SerialProtocol_Winch, 0);
}

void AG_Winch_Daiwa::update()
{
...
void AG_Winch_Daiwa::send_status(const GCS_MAVLINK &channel)
{
...
void AG_Winch_Daiwa::read_data_from_winch()
{
...
void AG_Winch_Daiwa::control_winch()
{
    ...
    if (config.control_mode == AG_Winch::ControlMode::RELAXED) {
    ...
    if ((config.control_mode == AG_Winch::ControlMode::POSITION) && healthy()) {
