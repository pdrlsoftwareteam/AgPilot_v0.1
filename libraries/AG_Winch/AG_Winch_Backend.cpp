#include <AG_Winch/AG_Winch_Backend.h>
...
void AG_Winch_Backend::read_pilot_desired_rate()
{
    ...
    if (!rc().has_valid_input() && (config.control_mode == AG_Winch::ControlMode::RATE_FROM_RC)) {
    ...
    if (config.control_mode != AG_Winch::ControlMode::RATE_FROM_RC) {
        if (abs(radio_in - previous_radio_in) > rc_input->get_dead_zone()) {
            config.control_mode = AG_Winch::ControlMode::RATE_FROM_RC;
        }
    }
...
float AG_Winch_Backend::get_rate_limited_by_accel(float rate, float dt)
{
