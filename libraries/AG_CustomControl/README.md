...
1. Generate a copy of `AG_CustomControl_Empty.cpp` and `AG_CustomControl_Empty.h` within `AG_CustomControl` folder. The folder tree would look like this,
```
AG_CustomControl.cpp
AG_CustomControl.h
AG_CustomControl_Backend.h
AG_CustomControl_Empty.cpp
AG_CustomControl_Empty.h
AG_CustomControl_Empty - Copy.cpp
AG_CustomControl_Empty - Copy.h
.
.
.
```
...
AG_CustomControl_Empty.cpp
AG_CustomControl_Empty.h
AG_CustomControl_XYZ.cpp
AG_CustomControl_XYZ.h
.
.
.
```

3. Change every class name, function definition etc. from `AG_CustomControl_Empty` to `AG_CustomControl_XYZ` inside `AG_CustomControl_XYZ.cpp` and `AG_CustomControl_XYZ.h` files. This would look like this for the header file
```C++
#pragma once

#include "AG_CustomControl_Backend.h"

class AG_CustomControl_XYZ : public AG_CustomControl_Backend {
public:
    AG_CustomControl_XYZ(AG_CustomControl &frontend, AG_AHRS_View*& ahrs, AG_AttitudeControl_Multi*& atti_control, AG_MotorsMulticopter*& motors, float dt);
...
5. Add a new backend header in `AG_CustomControl.cpp` file. Place it under other backend includes.
```C++
#include "AG_CustomControl_Backend.h"
#include "AG_CustomControl_Empty.h"
#include "AG_CustomControl_PID.h"
#include "AG_CustomControl_XYZ.h"
``` 

6. Add new backend parameter in `AG_CustomControl.cpp` file. Increment `_backend`, `_backend_var_info` array index by one and also increment backend parameter prefix and parameter table index by one. Place it under the other backend's parameters.
```C++
.
.
.
    ...
    AP_SUBGROUPVARPTR(_backend, "3_", 8, AG_CustomControl, _backend_var_info[2]),

    AP_GROUPEND
};
```

7. Allow creating new backend class in `AG_CustomControl.cpp` file inside `init` function.
```C++
.
.
.
case CustomControlType::CONT_PID:
    _backend = new AG_CustomControl_PID(*this, _ahrs, _atti_control, _motors, _dt);
    _backend_var_info[get_type()] = AG_CustomControl_PID::var_info;
    break;
case CustomControlType::CONT_XYZ:
    _backend = new AG_CustomControl_XYZ(*this, _ahrs, _atti_control, _motors, _dt);
    _backend_var_info[get_type()] = AG_CustomControl_XYZ::var_info;
    break;
default:
            return;
}
...
8. This is the bare minimum to compile and run your custom controller. You can add controller related code to `AG_CustomControl_XYZ` file without changing  anything else. 
...
```C++
AG_CustomControl_PID::AG_CustomControl_PID(AG_CustomControl &frontend, AG_AHRS_View*& ahrs, AG_AttitudeControl_Multi*& atti_control, AG_MotorsMulticopter*& motors, float dt) :
    AG_CustomControl_Backend(frontend, ahrs, atti_control, motors, dt),
...
11. Add your controller inside `update` function of `AG_CustomControl_XYZ.cpp` file. This function returns a 3-dimensional vector consisting of roll pitch yaw mixer input, respectively.

12. Add reset functionality inside `reset` function of `AG_CustomControl_XYZ.cpp` file. It is user's responsibility to add proper controller resetting functionality. This highly depends on the controller and it should not be copy-pasted from another backend without testing it in SITL.
