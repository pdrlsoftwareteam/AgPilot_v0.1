...
const AG_Param::GroupInfo AG_CustomControl_PID::var_info[] = {
    // @Param: ANG_RLL_P
    ...
    // @User: Standard
    AP_SUBGROUPINFO(_p_angle_roll2, "ANG_RLL_", 1, AG_CustomControl_PID, AG_P),
    ...
    // @Range{Sub}: 0.0 12.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_angle_pitch2, "ANG_PIT_", 2, AG_CustomControl_PID, AG_P),
    ...
    // @Range{Sub}: 0.0 6.000
    // @User: Standard    
    AP_SUBGROUPINFO(_p_angle_yaw2, "ANG_YAW_", 3, AG_CustomControl_PID, AG_P),
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_atti_rate_roll, "RAT_RLL_", 4, AG_CustomControl_PID, AG_PID),
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_atti_rate_pitch, "RAT_PIT_", 5, AG_CustomControl_PID, AG_PID),
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_atti_rate_yaw, "RAT_YAW_", 6, AG_CustomControl_PID, AG_PID),

    AP_GROUPEND
};

AG_CustomControl_PID::AG_CustomControl_PID(AG_CustomControl& frontend, AG_AHRS_View*& ahrs, AG_AttitudeControl_Multi*& att_control, AG_MotorsMulticopter*& motors, float dt) :
...
Vector3f AG_CustomControl_PID::update()
{
...
// Note that integrator are not reset correctly as it is done in reset_main_att_controller inside AG_CustomControl.cpp
// This is done intentionally to demonstrate switch over performance of two exact controller with different reset handling.
void AG_CustomControl_PID::reset(void)
{
