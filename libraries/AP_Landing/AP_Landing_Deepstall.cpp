...
bool AP_Landing_Deepstall::override_servos(void)
{
    ...
    if (elevator == nullptr) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "Deepstall: Unable to find the elevator channels");
        request_go_around();
        return false;
    }
...
#ifdef DEBUG_PRINTS
    // TODO: Send this information via a AGPILOTLink packet
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Arc: %3.8f %3.8f",
                                     (double)(arc.lat / 1e7),(double)( arc.lng / 1e7));
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Loiter en: %3.8f %3.8f",
                                     (double)(arc_entry.lat / 1e7), (double)(arc_entry.lng / 1e7));
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Loiter ex: %3.8f %3.8f",
                                     (double)(arc_exit.lat / 1e7), (double)(arc_exit.lng / 1e7));
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Extended: %3.8f %3.8f",
                                     (double)(extended_approach.lat / 1e7), (double)(extended_approach.lng / 1e7));
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Extended by: %f (%f)", (double)approach_extension_m,
                                     (double)expected_travel_distance);
    gcs().send_text(AGPILOT_SEVERITY_INFO, "Target Heading: %3.1f", (double)target_heading_deg);
#endif // DEBUG_PRINTS
...
float AP_Landing_Deepstall::predict_travel_distance(const Vector3f wind, const float height, const bool print)
{
    ...
    if(print) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "Deepstall: Entry: %0.1f (m) Travel: %0.1f (m)",
...
float AP_Landing_Deepstall::update_steering()
{
    ...
    if ((!landing.ahrs.get_location(current_loc) || !landing.ahrs.healthy()) && !hold_level) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_CRITICAL, "Deepstall: Invalid data from AHRS. Holding level");
        hold_level = true;
    }
...
#ifdef DEBUG_PRINTS
    gcs().send_text(AGPILOT_SEVERITY_INFO, "x: %f e: %f r: %f d: %f",
