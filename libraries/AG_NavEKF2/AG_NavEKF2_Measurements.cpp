...
void NavEKF2_core::readGpsData()
{
    ...
    if (gps.last_message_time_ms(gps.primary_sensor()) - lastTimeGpsReceived_ms > 70) {
        if (gps.status() >= AG_DAL_GPS::GPS_OK_FIX_3D) {
            ...
            if (gpsGoodToAlign && !have_table_earth_field) {
                ...
                if (compass.have_scale_factor(magSelectIndex) &&
                    ...
                    table_earth_field_ga = AG_Declination::get_earth_field_ga(gpsloc).toftype();
                    table_declination = radians(AG_Declination::get_declination(gpsloc.lat*1.0e-7,
                                                                                gpsloc.lng*1.0e-7));
