...
const AG_Param::GroupInfo NavEKF2::var_info[] = {
...
{
    AP::dal().start_frame(AG_DAL::FrameType::InitialiseFilterEKF2);
    ...
    if (core == nullptr) {
        ...
        core = (NavEKF2_core*)AP::dal().malloc_type(sizeof(NavEKF2_core)*num_cores, AG_DAL::MEM_FAST);
        if (core == nullptr) {
            initFailure = InitFailures::NO_MEM;
            core_malloc_failed = true;
            num_cores = 0;
        ...
        for (uint8_t i=0; i<7; i++) {
            if (_imuMask & (1U<<i)) {
                if(!core[num_cores].setup_core(i, num_cores)) {
                    ...
                    hal.util->free_type(core, sizeof(NavEKF2_core)*num_cores, AG_HAL::Util::MEM_FAST);
                    core = nullptr;
...
{
    AP::dal().start_frame(AG_DAL::FrameType::UpdateFilterEKF2);
    ...
    for (uint8_t i=0; i<num_cores; i++) {
        ...
        if (core[i].getFramesSincePredict() < (_framesPerPrediction+3) &&
            AP::dal().ekf_low_time_remaining(AG_DAL::EKFType::EKF2, i)) {
...
{
    AP::dal().log_event2(AG_DAL::Event::checkLaneSwitch);
...
{
    AP::dal().log_event2(AG_DAL::Event::resetGyroBias);
...
{
    AP::dal().log_event2(AG_DAL::Event::resetHeightDatum);
...
{
    if (val) {
        AP::dal().log_event2(AG_DAL::Event::setTerrainHgtStable);
    } else {
        AP::dal().log_event2(AG_DAL::Event::unsetTerrainHgtStable);
    }
    if (core) {
        for (uint8_t i=0; i<num_cores; i++) {
 ...
 *
 * Sensor offsets are pulled directly from the AG_VisualOdom library
 *
*/
...
{
    AP::dal().log_event2(AG_DAL::Event::requestYawReset);
