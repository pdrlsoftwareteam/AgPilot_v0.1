...
/*
   Set the distance based on a AGPILOTLINK message
*/
void AP_RangeFinder_AGPILOTLink::handle_msg(const mavlink_message_t &msg)
{
    ...
    if (packet.orientation == orientation()) {
        ...
        sensor_type = (AGPILOT_DISTANCE_SENSOR)packet.type;
    }
}

int16_t AP_RangeFinder_AGPILOTLink::max_distance_cm() const
{
...
int16_t AP_RangeFinder_AGPILOTLink::min_distance_cm() const
{
    if (_max_distance_cm == 0 && _min_distance_cm == 0) {
        // we assume if both of these are zero that we ignore both
...
*/
void AP_RangeFinder_AGPILOTLink::update(void)
{
    //Time out on incoming data; if we don't get new
    //data in 500ms, dump it
    if (AP_HAL::millis() - state.last_reading_ms > AP_RANGEFINDER_AGPILOTLINK_TIMEOUT_MS) {
