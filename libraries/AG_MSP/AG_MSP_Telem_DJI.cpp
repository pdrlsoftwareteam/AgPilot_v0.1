...
bool AG_MSP_Telem_DJI::init_uart()
{
    if (_msp_port.uart != nullptr)  {
        _msp_port.uart->set_flow_control(AG_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
...
bool AG_MSP_Telem_DJI::is_scheduler_enabled() const
{
    const AG_MSP *msp = AP::msp();
    return msp && msp->is_option_enabled(AG_MSP::Option::TELEMETRY_MODE);
}

void AG_MSP_Telem_DJI::hide_osd_items(void)
{
    const AG_MSP *msp = AP::msp();
    if (msp == nullptr) {
        return;
    ...
    AG_MSP_Telem_Backend::hide_osd_items();
...
uint32_t AG_MSP_Telem_DJI::get_osd_flight_mode_bitmask(void)
{
    uint32_t mode_mask = AG_MSP_Telem_Backend::get_osd_flight_mode_bitmask();
    const AG_Notify& notify = AP::notify();
...
MSPCommandResult AG_MSP_Telem_DJI::msp_process_out_fc_variant(sbuf_t *dst)
{
    sbuf_write_data(dst, "BTFL", FLIGHT_CONTROLLER_IDENTIFIER_LENGTH);
    return MSP_RESULT_ACK;
}

MSPCommandResult AG_MSP_Telem_DJI::msp_process_out_esc_sensor_data(sbuf_t *dst)
{
...
void AG_MSP_Telem_DJI::update_home_pos(home_state_t &home_state)
{
    AG_MSP_Telem_Backend::update_home_pos(home_state);
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
        return;
    }
...
void AG_MSP_Telem_DJI::update_battery_state(battery_state_t &_battery_state)
{
    AG_MSP_Telem_Backend::update_battery_state(_battery_state);
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
        return;
    }
...
void AG_MSP_Telem_DJI::update_gps_state(gps_state_t &gps_state)
{
    AG_MSP_Telem_Backend::update_gps_state(gps_state);
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
        return;
    }
...
void AG_MSP_Telem_DJI::update_airspeed(airspeed_state_t &airspeed_state)
{
    AG_MSP_Telem_Backend::update_airspeed(airspeed_state);
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
        return;
    }
...
void AG_MSP_Telem_DJI::update_flight_mode_str(char *flight_mode_str, uint8_t size, bool wind_enabled)
{
    AG_MSP_Telem_Backend::update_flight_mode_str(flight_mode_str, size, wind_enabled);
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
        return;
    }
...
bool AG_MSP_Telem_DJI::get_rssi(float &rssi) const
{
    if (!AG_MSP_Telem_Backend::get_rssi(rssi)) {
        return false;
    }
#if OSD_ENABLED
    if (!displaying_stats_screen()) {
    ...
    // Note: return false when min_rssi has not been updated yet by AG_OSD::update_stats()
