...
constexpr uint8_t AG_MSP_Telem_Backend::arrows[8];

using namespace MSP;

AG_MSP_Telem_Backend::AG_MSP_Telem_Backend(AG_HAL::UARTDriver *uart) : AG_RCTelemetry(MSP_TIME_SLOT_MAX)
{
    _msp_port.uart = uart;
}
...
/*
  Scheduler helper
 */
void AG_MSP_Telem_Backend::setup_wfq_scheduler(void)
{
 ...
 */
bool AG_MSP_Telem_Backend::init()
{
    enable_warnings();
    return AG_RCTelemetry::init();
}

bool AG_MSP_Telem_Backend::init_uart()
{
...
void AG_MSP_Telem_Backend::process_outgoing_data()
{
    if (is_scheduler_enabled()) {
        AG_RCTelemetry::run_wfq_scheduler();
    }
}
...
/*
  Scheduler helper
 */
bool AG_MSP_Telem_Backend::is_packet_ready(uint8_t idx, bool queue_empty)
{
    switch (idx) {
 ...
 */
void AG_MSP_Telem_Backend::process_packet(uint8_t idx)
{
    if (idx == EMPTY_SLOT) {
        return;
    }
...
uint8_t AG_MSP_Telem_Backend::calc_cell_count(const float battery_voltage)
{
    return floorf((battery_voltage / CELLFULL) + 1);
}

float AG_MSP_Telem_Backend::get_vspeed_ms(void) const
{
    {
    ...
    AG_Baro &_baro = AP::baro();
    WITH_SEMAPHORE(_baro.get_semaphore());
    return _baro.get_climb_rate();
}

void AG_MSP_Telem_Backend::update_home_pos(home_state_t &home_state)
{
    AG_AHRS &_ahrs = AP::ahrs();
    WITH_SEMAPHORE(_ahrs.get_semaphore());
    Location loc;
...
void AG_MSP_Telem_Backend::update_gps_state(gps_state_t &gps_state)
{
    AG_GPS& gps = AP::gps();
    ...
    gps_state.fix_type = gps.status() >= AG_GPS::GPS_Status::GPS_OK_FIX_3D? 2:0;
...
void AG_MSP_Telem_Backend::update_battery_state(battery_state_t &battery_state)
{
    ...
    const AG_Notify& notify = AP::notify();
...
void AG_MSP_Telem_Backend::update_airspeed(airspeed_state_t &airspeed_state)
{
...
*/
void AG_MSP_Telem_Backend::update_flight_mode_str(char *flight_mode_str, uint8_t size, bool wind_enabled)
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
    ...
    if (wind_enabled) {
        ...
        */
        ...
        invert_wind = osd->screen[msp->_msp_status.current_screen].check_option(AG_OSD::OPTION_INVERTED_WIND);
#endif
        if (invert_wind) {
            v = -v;
        }
...
#if OSD_ENABLED
        units = osd->units == AG_OSD::UNITS_IMPERIAL ?  OSD_UNIT_IMPERIAL : OSD_UNIT_METRIC;
#endif
...
void AG_MSP_Telem_Backend::enable_warnings()
{
    AG_MSP *msp = AP::msp();
    if (msp == nullptr) {
        return;
...
void AG_MSP_Telem_Backend::process_incoming_data()
{
    if (_msp_port.uart == nullptr) {
        return;
    }
...
/*
  send an MSP packet
 */
void AG_MSP_Telem_Backend::msp_send_packet(uint16_t cmd, MSP::msp_version_e msp_version, const void *p, uint16_t size, bool is_request)
{
    uint8_t out_buf[MSP_PORT_OUTBUF_SIZE];
 ...
 */
void AG_MSP_Telem_Backend::msp_process_received_command()
{
    uint8_t out_buf[MSP_PORT_OUTBUF_SIZE];
 ...
 */
MSPCommandResult AG_MSP_Telem_Backend::msp_process_command(msp_packet_t *cmd, msp_packet_t *reply)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_command(uint16_t cmd_msp, sbuf_t *dst)
{
    switch (cmd_msp) {
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_sensor_command(uint16_t cmd_msp, sbuf_t *src)
{
    MSP_UNUSED(src);
...
void AG_MSP_Telem_Backend::msp_handle_opflow(const MSP::msp_opflow_data_message_t &pkt)
{
...
void AG_MSP_Telem_Backend::msp_handle_rangefinder(const MSP::msp_rangefinder_data_message_t &pkt)
{
...
void AG_MSP_Telem_Backend::msp_handle_gps(const MSP::msp_gps_data_message_t &pkt)
{
...
void AG_MSP_Telem_Backend::msp_handle_compass(const MSP::msp_compass_data_message_t &pkt)
{
...
void AG_MSP_Telem_Backend::msp_handle_baro(const MSP::msp_baro_data_message_t &pkt)
{
...
void AG_MSP_Telem_Backend::msp_handle_airspeed(const MSP::msp_airspeed_data_message_t &pkt)
{
...
uint32_t AG_MSP_Telem_Backend::get_osd_flight_mode_bitmask(void)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_api_version(sbuf_t *dst)
{
    const struct {
        uint8_t proto;
        uint8_t major;
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_fc_version(sbuf_t *dst)
{
    const struct {
        uint8_t major;
        uint8_t minor;
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_fc_variant(sbuf_t *dst)
{
    sbuf_write_data(dst, "ARDU", FLIGHT_CONTROLLER_IDENTIFIER_LENGTH);
    return MSP_RESULT_ACK;
}

MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_raw_gps(sbuf_t *dst)
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_comp_gps(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_name(sbuf_t *dst)
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
    ...
    if (notify) {
        ...
        const uint32_t visible_time_ms = AG_HAL::millis() - notify->get_text_updated_millis();
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_status(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_osd_config(sbuf_t *dst)
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
...
#if OSD_ENABLED
    osd_config.units = osd->units == AG_OSD::UNITS_METRIC ? OSD_UNIT_METRIC : OSD_UNIT_IMPERIAL;
#endif
    // Alarms
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_attitude(sbuf_t *dst)
{
    AG_AHRS &ahrs = AP::ahrs();
    WITH_SEMAPHORE(ahrs.get_semaphore());
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_altitude(sbuf_t *dst)
{
    home_state_t home_state;
    update_home_pos(home_state);
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_analog(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_battery_state(sbuf_t *dst)
{
    const AG_MSP *msp = AP::msp();
    if (msp == nullptr) {
        return MSP_RESULT_ERROR;
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_esc_sensor_data(sbuf_t *dst)
{
#if HAL_WITH_ESC_TELEM
    AG_ESC_Telem& telem = AP::esc_telem();
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_rtc(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_rc(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_board_info(sbuf_t *dst)
{
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_build_info(sbuf_t *dst)
{
    const AG_FWVersion &fwver = AP::fwversion();
...
MSPCommandResult AG_MSP_Telem_Backend::msp_process_out_uid(sbuf_t *dst)
{
    uint8_t id[12] {};
...
void AG_MSP_Telem_Backend::hide_osd_items(void)
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
...
void AG_MSP_Telem_Backend::msp_displayport_heartbeat()
{
...
void AG_MSP_Telem_Backend::msp_displayport_grab()
{
    msp_displayport_heartbeat();
}

void AG_MSP_Telem_Backend::msp_displayport_release()
{
...
void AG_MSP_Telem_Backend::msp_displayport_clear_screen()
{
...
void AG_MSP_Telem_Backend::msp_displayport_draw_screen()
{
    const uint8_t subcmd[] = { msp_displayport_subcmd_e::MSP_DISPLAYPORT_DRAW_SCREEN };
    msp_send_packet(MSP_DISPLAYPORT, MSP::MSP_V1, subcmd, sizeof(subcmd), false);
}

void AG_MSP_Telem_Backend::msp_displayport_write_string(uint8_t col, uint8_t row, bool blink, const char *string)
{
...
void AG_MSP_Telem_Backend::msp_displayport_set_options(const uint8_t font_index, const uint8_t screen_resolution)
{
...
bool AG_MSP_Telem_Backend::displaying_stats_screen() const
{
#if OSD_ENABLED
    AG_OSD *osd = AP::osd();
    if (osd == nullptr) {
...
bool AG_MSP_Telem_Backend::get_rssi(float &rssi) const
{
    AG_RSSI* ap_rssi = AP::rssi();
    if (ap_rssi == nullptr) {
        return false;
    }
    if (!ap_rssi->enabled()) {
