...
class AG_ESC_Telem {
public:
    friend class AG_ESC_Telem_Backend;

    AG_ESC_Telem();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_ESC_Telem);

    static const struct AG_Param::GroupInfo var_info[];

    static AG_ESC_Telem *get_singleton();
    ...
    void update_telem_data(const uint8_t esc_index, const AG_ESC_Telem_Backend::TelemetryData& new_data, const uint16_t data_mask);
...
private:
    ...
    // rpm data
    volatile AG_ESC_Telem_Backend::RpmData _rpm_data[ESC_TELEM_MAX_ESCS];
    // telemetry data
    volatile AG_ESC_Telem_Backend::TelemetryData _telem_data[ESC_TELEM_MAX_ESCS];
    ...
    static AG_ESC_Telem *_singleton;
};
