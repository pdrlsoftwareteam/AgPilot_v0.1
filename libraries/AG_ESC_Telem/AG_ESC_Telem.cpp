...
#include <AG_TemperatureSensor/AG_TemperatureSensor_config.h>
...
const AG_Param::GroupInfo AG_ESC_Telem::var_info[] = {
    ...
    // @Range: 0 31
    // @User: Standard
    AP_GROUPINFO("_MAV_OFS", 1, AG_ESC_Telem, mavlink_offset, 0),
    
    AP_GROUPEND
};

AG_ESC_Telem::AG_ESC_Telem()
{
    if (_singleton) {
        AG_HAL::panic("Too many AG_ESC_Telem instances");
    }
    _singleton = this;
...
float AG_ESC_Telem::get_average_motor_rpm(uint32_t servo_channel_mask) const
{
    float rpm_avg = 0.0f;
...
uint8_t AG_ESC_Telem::get_motor_frequencies_hz(uint8_t nfreqs, float* freqs) const
{
    uint8_t valid_escs = 0;
...
uint32_t AG_ESC_Telem::get_active_esc_mask() const {
    uint32_t ret = 0;
...
uint8_t AG_ESC_Telem::get_num_active_escs() const {
...
bool AG_ESC_Telem::are_motors_running(uint32_t servo_channel_mask, float min_rpm) const
{
    const uint32_t now = AG_HAL::micros();

    for (uint8_t i = 0; i < ESC_TELEM_MAX_ESCS; i++) {
        if (BIT_IS_SET(servo_channel_mask, i)) {
            const volatile AG_ESC_Telem_Backend::RpmData& rpmdata = _rpm_data[i];
...
bool AG_ESC_Telem::is_telemetry_active(uint32_t servo_channel_mask) const
{
    for (uint8_t i = 0; i < ESC_TELEM_MAX_ESCS; i++) {
        if (BIT_IS_SET(servo_channel_mask, i)) {
            // no data received
...
bool AG_ESC_Telem::get_rpm(uint8_t esc_index, float& rpm) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS) {
        return false;
    }

    const volatile AG_ESC_Telem_Backend::RpmData& rpmdata = _rpm_data[esc_index];
...
bool AG_ESC_Telem::get_raw_rpm(uint8_t esc_index, float& rpm) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS) {
        return false;
    }
...
bool AG_ESC_Telem::get_temperature(uint8_t esc_index, int16_t& temp) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & (AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE | AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE_EXTERNAL))) {
        return false;
    }
...
bool AG_ESC_Telem::get_motor_temperature(uint8_t esc_index, int16_t& temp) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & (AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE | AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE_EXTERNAL))) {
        return false;
    }
...
bool AG_ESC_Telem::get_highest_motor_temperature(int16_t& temp) const
{
    uint8_t valid_escs = 0;
...
bool AG_ESC_Telem::get_current(uint8_t esc_index, float& amps) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::CURRENT)) {
        return false;
    }
...
bool AG_ESC_Telem::get_voltage(uint8_t esc_index, float& volts) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::VOLTAGE)) {
        return false;
    }
...
bool AG_ESC_Telem::get_consumption_mah(uint8_t esc_index, float& consumption_mah) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::CONSUMPTION)) {
        return false;
    }
...
bool AG_ESC_Telem::get_usage_seconds(uint8_t esc_index, uint32_t& usage_s) const
{
    if (esc_index >= ESC_TELEM_MAX_ESCS
        || AG_HAL::millis() - _telem_data[esc_index].last_update_ms > ESC_TELEM_DATA_TIMEOUT_MS
        || !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::USAGE)) {
        return false;
    }
...
void AG_ESC_Telem::send_esc_telemetry_mavlink(uint8_t mav_chan)
{
#if HAL_GCS_ENABLED
    if (!_have_data) {
...
void AG_ESC_Telem::update_telem_data(const uint8_t esc_index, const AG_ESC_Telem_Backend::TelemetryData& new_data, const uint16_t data_mask)
{
    ...
    const bool has_temperature = (data_mask & AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE_EXTERNAL) ||
        ((data_mask & AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE) && !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE_EXTERNAL));

    const bool has_motor_temperature = (data_mask & AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE_EXTERNAL) ||
        ((data_mask & AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE) && !(_telem_data[esc_index].types & AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE_EXTERNAL));
#else
    const bool has_temperature = (data_mask & AG_ESC_Telem_Backend::TelemetryType::TEMPERATURE);
    const bool has_motor_temperature = (data_mask & AG_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE);
#endif
    ...
    if (data_mask & AG_ESC_Telem_Backend::TelemetryType::VOLTAGE) {
        _telem_data[esc_index].voltage = new_data.voltage;
    }
    if (data_mask & AG_ESC_Telem_Backend::TelemetryType::CURRENT) {
        _telem_data[esc_index].current = new_data.current;
    }
    if (data_mask & AG_ESC_Telem_Backend::TelemetryType::CONSUMPTION) {
        _telem_data[esc_index].consumption_mah = new_data.consumption_mah;
    }
    if (data_mask & AG_ESC_Telem_Backend::TelemetryType::USAGE) {
        _telem_data[esc_index].usage_s = new_data.usage_s;
    }

    _telem_data[esc_index].count++;
    _telem_data[esc_index].types |= data_mask;
    _telem_data[esc_index].last_update_ms = AG_HAL::millis();
}

// record an update to the RPM together with timestamp, this allows the notch values to be slewed
// this should be called by backends when new telemetry values are available
void AG_ESC_Telem::update_rpm(const uint8_t esc_index, const float new_rpm, const float error_rate)
{
    if (esc_index >= ESC_TELEM_MAX_ESCS) {
        return;
    }
    ...
    const uint32_t now = AG_HAL::micros();
    volatile AG_ESC_Telem_Backend::RpmData& rpmdata = _rpm_data[esc_index];
...
*/
void AG_ESC_Telem::set_rpm_scale(const uint8_t esc_index, const float scale_factor)
{
    if (esc_index < ESC_TELEM_MAX_ESCS) {
...
AG_ESC_Telem *AG_ESC_Telem::_singleton = nullptr;

/*
 * Get the AG_ESC_Telem singleton
 */
AG_ESC_Telem *AG_ESC_Telem::get_singleton()
{
    return AG_ESC_Telem::_singleton;
}
...
{
    return *AG_ESC_Telem::get_singleton();
}
