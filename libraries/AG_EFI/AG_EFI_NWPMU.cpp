...
AG_EFI_NWPMU::AG_EFI_NWPMU(AG_EFI &_frontend) :
    CANSensor("NWPMU"),
    AG_EFI_Backend(_frontend)
{
    register_driver(AG_CANManager::Driver_Type_EFI_NWPMU);
}

void AG_EFI_NWPMU::handle_frame(AG_HAL::CANFrame &frame)
{
    const uint32_t id =  frame.id & AG_HAL::CANFrame::MaskExtID;
    ...
    case NWPMU_ID::ECU_6: {
        ...
        struct ecu_6 data;
        memcpy(&data, frame.data, sizeof(data));
        if (!_emitted_version && (AG_HAL::millis() > 10000)) { // don't emit a version early in the boot process
...
void AG_EFI_NWPMU::update()
{
    // copy the data to the front end
    copy_to_frontend();
}

#endif // AG_EFI_NWPWU_ENABLED
