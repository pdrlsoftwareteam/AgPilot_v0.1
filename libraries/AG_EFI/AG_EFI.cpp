...
#include "AG_EFI_Serial_MS.h"
#include "AG_EFI_Serial_Lutan.h"
#include "AG_EFI_NWPMU.h"
#include "AG_EFI_DroneCAN.h"
#include "AG_EFI_Currawong_ECU.h"
#include "AG_EFI_Scripting.h"
...
const AG_Param::GroupInfo AG_EFI::var_info[] = {
    // @Param: _TYPE
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("_TYPE", 1, AG_EFI, type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @Range: 0 1
    // @User: Advanced
    AP_GROUPINFO("_COEF1", 2, AG_EFI, coef1, 0),
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO("_COEF2", 3, AG_EFI, coef2, 0),
    ...
    // @Range: 0 10000
    // @User: Advanced
    AP_GROUPINFO("_FUEL_DENS", 4, AG_EFI, ecu_fuel_density, 0),

    AP_GROUPEND
};

AG_EFI *AG_EFI::singleton;
...
void AG_EFI::init(void)
{
    ...
    case Type::NONE:
        break;
#if AG_EFI_SERIAL_MS_ENABLED
    case Type::MegaSquirt:
        backend = new AG_EFI_Serial_MS(*this);
        break;
#endif
#if AG_EFI_SERIAL_LUTAN_ENABLED
    case Type::Lutan:
        backend = new AG_EFI_Serial_Lutan(*this);
        break;
#endif
    case Type::NWPMU:
#if AG_EFI_NWPWU_ENABLED
        backend = new AG_EFI_NWPMU(*this);
#endif
        break;
    case Type::DroneCAN:
#if AG_EFI_DRONECAN_ENABLED
        backend = new AG_EFI_DroneCAN(*this);
#endif
        break;
    case Type::CurrawongECU:
#if AG_EFI_CURRAWONG_ECU_ENABLED
        backend = new AG_EFI_Currawong_ECU(*this);
#endif
        break;
    case Type::SCRIPTING:
#if AG_EFI_SCRIPTING_ENABLED
        backend = new AG_EFI_Scripting(*this);
#endif
        break;
    default:
...
bool AG_EFI::is_healthy(void) const
{
    return (backend && (AG_HAL::millis() - state.last_updated_ms) < HEALTHY_LAST_RECEIVED_MS);
}
...
/*
  send EFI_STATUS
 */
void AG_EFI::send_mavlink_status(mavlink_channel_t chan)
{
    if (!backend) {
        return;
    }
...
void AG_EFI::get_state(EFI_State &_state)
{
    WITH_SEMAPHORE(sem);
    _state = state;
}
