...
void AG_EFI_DroneCAN::subscribe_msgs(AG_UAVCAN *ap_uavcan)
{
    ...
    if (status_listener_res < 0) {
        AG_HAL::panic("DroneCAN EFI subscriber start problem\n\r");
        return;
    }
}

// Called from frontend to update with the readings received by handler
void AG_EFI_DroneCAN::update()
{
}

// EFI message handler
void AG_EFI_DroneCAN::trampoline_status(AG_UAVCAN *ap_uavcan, uint8_t node_id, const EFIStatusCb &cb)
{
    if (driver == nullptr) {
        return;
    }
 ...
 */
void AG_EFI_DroneCAN::handle_status(const uavcan::equipment::ice::reciprocating::Status &pkt)
{
