...
AG_EFI_Serial_MS::AG_EFI_Serial_MS(AG_EFI &_frontend):
    AG_EFI_Backend(_frontend)
{
    internal_state.estimated_consumed_fuel_volume_cm3 = 0; // Just to be sure
    port = AP::serialmanager().find_serial(AG_SerialManager::SerialProtocol_EFI, 0);
}


void AG_EFI_Serial_MS::update()
{
    if (!port) {
        return;
    }
...
bool AG_EFI_Serial_MS::read_incoming_realtime_data() 
{
    ...
    uint32_t current_time = AG_HAL::millis();
...
void AG_EFI_Serial_MS::send_request(uint8_t table, uint16_t first_offset, uint16_t last_offset)
{
...
uint8_t AG_EFI_Serial_MS::read_byte_CRC32()
{   
...
uint32_t AG_EFI_Serial_MS::CRC32_compute_byte(uint32_t crc, uint8_t data)
{
    crc ^= ~0U;
...
#endif  // AG_EFI_SERIAL_MS_ENABLED
