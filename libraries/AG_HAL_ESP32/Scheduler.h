...
class ESP32::Scheduler : public AG_HAL::Scheduler
{

public:
    Scheduler();
    /* AG_HAL::Scheduler methods */
    void     init() override;
    void     set_callbacks(AG_HAL::HAL::Callbacks *cb)
    {
        callbacks = cb;
    };
    ...
    bool thread_create(AG_HAL::MemberProc, const char *name, uint32_t stack_size, priority_base base, int8_t priority) override;
...
private:
    AG_HAL::HAL::Callbacks *callbacks;
    AG_HAL::Proc _failsafe;

    AG_HAL::MemberProc _timer_proc[ESP32_SCHEDULER_MAX_TIMER_PROCS];
    uint8_t _num_timer_procs;

    AG_HAL::MemberProc _io_proc[ESP32_SCHEDULER_MAX_IO_PROCS];
