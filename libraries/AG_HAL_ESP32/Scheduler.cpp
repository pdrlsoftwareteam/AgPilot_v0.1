...
#include "AG_HAL_ESP32/Scheduler.h"
#include "AG_HAL_ESP32/RCInput.h"
#include "AG_HAL_ESP32/AnalogIn.h"
...
bool Scheduler::thread_create(AG_HAL::MemberProc proc, const char *name, uint32_t stack_size, priority_base base, int8_t priority)
{
    ...
    AG_HAL::MemberProc *tproc = (AG_HAL::MemberProc *)calloc(1, sizeof(proc));
    if (!tproc) {
        return false;
    }
    *tproc = proc;
...
#endif
    if (_initialized) {
        AG_HAL::panic("PANIC: scheduler::system_initialized called more than once");
    }
...
void Scheduler::print_stats(void)
{
    ...
    if (AG_HAL::millis64() - last_run > 60000) {
        char buffer[1024];
        vTaskGetRunTimeStats(buffer);
        printf("\n\n%s\n", buffer);
        heap_caps_print_heap_info(0);
        last_run = AG_HAL::millis64();
    }
