...
// relevent to a vehicle type. Use AG_Param::set_frame_type_flags() to
...
#define PARAM_VEHICLE_INFO                   { "",   (const void *)&AP_PARAM_VEHICLE_NAME,         {group_info : AG_Vehicle::var_info}, 0,                                                  Parameters::k_param_vehicle,        AP_PARAM_GROUP }
...
public:
    ...
    AG_Param(const struct Info *info)
    {
        _var_info = info;
        uint16_t i;
        ...
        if (_singleton != nullptr) {
            AG_HAL::panic("AG_Param must be singleton");
        }
        _singleton = this;
    }
    ...
    // return true if AG_Param has been initialised via setup()
    ...
    ///
    void copy_name_info(const struct AG_Param::Info *info,
    ...
    /// buffer.
    ///
    /// Uses token to look up AG_Param::Info for the variable
    ...
    ///                         it does not exist.
    ///
    static AG_Param * find(const char *name, enum ap_var_type *ptype, uint16_t *flags = nullptr);
    ...
    ///                         it does not exist.
    ///
    static AG_Param * find_by_index(uint16_t idx, enum ap_var_type *ptype, ParamToken *token);

    // by-name equivalent of find_by_index()
    static AG_Param* find_by_name(const char* name, enum ap_var_type *ptype, ParamToken *token);
    ...
    /// @param  name            The full name of the variable to be found.
    ///
    static AG_Param * find_object(const char *name);
    ...
    */
    ...
    static bool find_old_parameter(const struct ConversionInfo *info, AG_Param *value);
    ...
    */
    ...
    ///                     there are none.
    ///
    static AG_Param *      first(ParamToken *token, enum ap_var_type *ptype, float *default_val = nullptr);

    /// Returns the next variable in _var_info, recursing into groups
    /// as needed
    static AG_Param *      next(ParamToken *token, enum ap_var_type *ptype) { return  next(token, ptype, false); }
    static AG_Param *      next(ParamToken *token, enum ap_var_type *ptype, bool skip_disabled, float *default_val = nullptr);

    /// Returns the next scalar variable in _var_info, recursing into groups
    /// as needed
    static AG_Param *       next_scalar(ParamToken *token, enum ap_var_type *ptype, float *default_val = nullptr);
    ...
    static void         show_all(AG_HAL::BetterStream *port, bool showKeyValues=false);

    /// print the value of one variable
    static void         show(const AG_Param *param, 
                             const char *name,
                             enum ap_var_type ptype, 
                             AG_HAL::BetterStream *port);
    ...
    static AG_Param *get_singleton() { return _singleton; }
...
protected:
    ...
    static void add_default(AG_Param *ap, float v);

private:
    static AG_Param *_singleton;

    /// EEPROM header
    ///
    /// This structure is placed at the head of the EEPROM to indicate
    /// that the ROM is formatted for AG_Param.
    ///
 ...
 */
    ...
    static float get_default_value(const AG_Param *object_ptr, const struct GroupInfo &info);
    static float get_default_value(const AG_Param *object_ptr, const struct Info &info);
    ...
    static const struct AG_Param::Info &var_info(uint16_t i) {
    ...
    */
    ...
    static void check_default(AG_Param *ap, float *default_value);
};
...
class AG_ParamT : public AG_Param
{
public:
    ...
    /// AG_ParamT types can implement AG_Param::cast_to_float
    ///
...
///
template<typename T, ap_var_type PT>
class AG_ParamV : public AG_Param
{
public:
...
class AG_ParamA : public AG_Param
{
public:
...
/// Convenience macro for defining instances of the AG_ParamT template.
///
// declare a scalar type
// _t is the base type
// _suffix is the suffix on the AP_* type name
// _pt is the enum ap_var_type type
#define AP_PARAMDEF(_t, _suffix, _pt)   typedef AG_ParamT<_t, _pt> AP_ ## _suffix;
...
// this is used in AG_Math.h
// _t is the base type
// _suffix is the suffix on the AP_* type name
// _pt is the enum ap_var_type type
#define AP_PARAMDEFV(_t, _suffix, _pt)   typedef AG_ParamV<_t, _pt> AP_ ## _suffix;
