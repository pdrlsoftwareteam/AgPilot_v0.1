...
uint16_t AG_Param::sentinal_offset;

// singleton instance
AG_Param *AG_Param::_singleton;

#define ENABLE_DEBUG 0

#if ENABLE_DEBUG
# define FATAL(fmt, args ...) AG_HAL::panic(fmt, ## args);
 # define Debug(fmt, args ...)  do {::printf("%s:%d: " fmt "\n", __FUNCTION__, __LINE__, ## args); } while(0)
#else
 # define FATAL(fmt, args ...) AG_HAL::panic("Bad parameter table");
...
// static member variables for AG_Param.
//

// number of rows in the _var_info[] table
uint16_t AG_Param::_num_vars;

#if AP_PARAM_DYNAMIC_ENABLED
uint16_t AG_Param::_num_vars_base;
AG_Param::Info *AG_Param::_var_info_dynamic;
static const char *_empty_string = "";
uint8_t AG_Param::_dynamic_table_sizes[AP_PARAM_MAX_DYNAMIC];
#endif

// cached parameter count
uint16_t AG_Param::_parameter_count;
uint16_t AG_Param::_count_marker;
uint16_t AG_Param::_count_marker_done;
HAL_Semaphore AG_Param::_count_sem;

// storage and naming information about all types that can be saved
const AG_Param::Info *AG_Param::_var_info;

struct AG_Param::param_override *AG_Param::param_overrides;
uint16_t AG_Param::param_overrides_len;
uint16_t AG_Param::num_param_overrides;
uint16_t AG_Param::num_read_only;

ObjectBuffer_TS<AG_Param::param_save> AG_Param::save_queue{30};
bool AG_Param::registered_save_handler;

AG_Param::defaults_list *AG_Param::default_list;

// we need a dummy object for the parameter save callback
static AG_Param save_dummy;
 ...
 */
const AG_Param::param_defaults_struct AG_Param::param_defaults_data = {
    "PARMDEF",
...
// storage object
StorageAccess AG_Param::_storage(StorageManager::StorageParam);

// backup storage object
StorageAccess AG_Param::_storage_bak(StorageManager::StorageParamBak);

// flags indicating frame type
uint16_t AG_Param::_frame_type_flags;

// write to EEPROM
void AG_Param::eeprom_write_check(const void *ptr, uint16_t ofs, uint8_t size)
{
    _storage.write_block(ofs, ptr, size);
    _storage_bak.write_block(ofs, ptr, size);
}

bool AG_Param::_hide_disabled_groups = true;

// write a sentinal value at the given offset
void AG_Param::write_sentinal(uint16_t ofs)
{
...
*/
uint32_t AG_Param::group_id(const struct GroupInfo *grpinfo, uint32_t base, uint8_t i, uint8_t shift)
{
 ...
 */
bool AG_Param::check_frame_type(uint16_t flags)
{
...
void AG_Param::check_group_info(const struct AG_Param::GroupInfo *  group_info,
                                uint16_t *                          total_size,
                                uint8_t                             group_shift,
...
bool AG_Param::duplicate_key(uint16_t vindex, uint16_t key)
{
 ...
 */
const struct AG_Param::GroupInfo *AG_Param::get_group_info(const struct GroupInfo &ginfo)
{
...
/*
  get group_info pointer for a group
 */
const struct AG_Param::GroupInfo *AG_Param::get_group_info(const struct Info &info)
{
...
void AG_Param::check_var_info(void)
{
...
bool AG_Param::setup(void)
{
    ...
    if (hdr.magic[0] != k_EEPROM_magic0 ||
        ...
        if (hdr2.magic[0] == k_EEPROM_magic0 &&
            ...
            INTERNAL_ERROR(AG_InternalError::error_t::params_restored);
            return true;
        }
...
// check if AG_Param has been initialised
bool AG_Param::initialised(void)
{
 ...
 */
bool AG_Param::adjust_group_offset(uint16_t vindex, const struct GroupInfo &group_info, ptrdiff_t &new_offset)
{
 ...
 */
bool AG_Param::get_base(const struct Info &info, ptrdiff_t &base)
{
...
const struct AG_Param::Info *AG_Param::find_by_header_group(struct Param_header phdr, void **ptr,
                                                            uint16_t vindex,
                                                            ...
                                                            ptrdiff_t group_offset)
{
    ...
    for (uint8_t i=0;
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            const struct AG_Param::Info *ret = find_by_header_group(phdr, ptr, vindex, ginfo,
...
// return the Info structure and a pointer to the variables storage
const struct AG_Param::Info *AG_Param::find_by_header(struct Param_header phdr, void **ptr)
{
...
const struct AG_Param::Info *AG_Param::find_var_info_group(const struct GroupInfo * group_info,
                                                           uint16_t                 vindex,
                                                           uint32_t                 group_base,
                                                           uint8_t                  group_shift,
                                                           ptrdiff_t                group_offset,
                                                           ...
                                                           uint8_t *                idx) const
{
    ...
    for (uint8_t i=0;
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            const struct AG_Param::Info *info;
            ptrdiff_t new_offset = group_offset;
            if (!adjust_group_offset(vindex, group_info[i], new_offset)) {
                continue;
            }
...
const struct AG_Param::Info *AG_Param::find_var_info(uint32_t *                 group_element,
                                                     const struct GroupInfo *   &group_ret,
                                                     struct GroupNesting        &group_nesting,
                                                     uint8_t *                  idx) const
{
    ...
    for (uint16_t i=0; i<_num_vars; i++) {
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            const struct AG_Param::Info *info2;
...
// find the info structure for a variable
const struct AG_Param::Info *AG_Param::find_var_info_token(const ParamToken &token,
                                                           uint32_t *                 group_element,
                                                           const struct GroupInfo *   &group_ret,
                                                           struct GroupNesting        &group_nesting,
                                                           uint8_t *                  idx) const
...
uint8_t AG_Param::type_size(enum ap_var_type type)
{
    switch (type) {
    case AP_PARAM_NONE:
    case AP_PARAM_GROUP:
 ...
 */
uint16_t AG_Param::get_key(const Param_header &phdr)
{
 ...
 */
void AG_Param::set_key(Param_header &phdr, uint16_t key)
{
 ...
 */
bool AG_Param::is_sentinal(const Param_header &phdr)
{
...
bool AG_Param::scan(const AG_Param::Param_header *target, uint16_t *pofs)
{
    struct Param_header phdr;
    uint16_t ofs = sizeof(AG_Param::EEPROM_header);
 ...
 */
void AG_Param::add_vector3f_suffix(char *buffer, size_t buffer_size, uint8_t idx) const
{
...
//
void AG_Param::copy_name_token(const ParamToken &token, char *buffer, size_t buffer_size, bool force_scalar) const
{
    uint32_t group_element;
    const struct GroupInfo *ginfo;
    struct GroupNesting group_nesting {};
    uint8_t idx;
    const struct AG_Param::Info *info = find_var_info_token(token, &group_element, ginfo, group_nesting, &idx);
    if (info == nullptr) {
        *buffer = 0;
        Debug("no info found");
        return;
...
void AG_Param::copy_name_info(const struct AG_Param::Info *info,
                              const struct GroupInfo *ginfo,
...
AG_Param *
AG_Param::find_group(const char *name, uint16_t vindex, ptrdiff_t group_offset,
                     const struct GroupInfo *group_info, enum ap_var_type *ptype)
{
    ...
    for (uint8_t i=0;
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            AG_Param *ap = find_group(name+strlen(group_info[i].name), vindex, new_offset, ginfo, ptype);
            if (ap != nullptr) {
                return ap;
            }
            ...
            *ptype = (enum ap_var_type)type;
            return (AG_Param *)(base + group_info[i].offset + group_offset);
        } else if (type == AP_PARAM_VECTOR3F) {
...
//
AG_Param *
AG_Param::find(const char *name, enum ap_var_type *ptype, uint16_t *flags)
{
    for (uint16_t i=0; i<_num_vars; i++) {
        const auto &info = var_info(i);
        uint8_t type = info.type;
        if (type == AP_PARAM_GROUP) {
            ...
            AG_Param *ap = find_group(name + len, i, 0, group_info, ptype);
            if (ap != nullptr) {
                if (flags != nullptr) {
...
//
AG_Param *
AG_Param::find_by_index(uint16_t idx, enum ap_var_type *ptype, ParamToken *token)
{
    AG_Param *ap;
    uint16_t count=0;
    for (ap=AG_Param::first(token, ptype);
         ap && count < idx;
         ap=AG_Param::next_scalar(token, ptype)) {
        count++;
    }
    return ap;    
}
...
AG_Param* AG_Param::find_by_name(const char* name, enum ap_var_type *ptype, ParamToken *token)
{
    AG_Param *ap;
    uint16_t count = 0;
    for (ap = AG_Param::first(token, ptype);
...
*/
bool AG_Param::find_key_by_pointer_group(const void *ptr, uint16_t vindex,
                                         const struct GroupInfo *group_info,
...
/*
  Find a variable by pointer, returning key. This is used for loading pointer variables
*/
bool AG_Param::find_key_by_pointer(const void *ptr, uint16_t &key)
{
    for (uint16_t i=0; i<_num_vars; i++) {
        const auto &info = var_info(i);
...
*/
bool AG_Param::find_top_level_key_by_pointer(const void *ptr, uint16_t &key)
{
    for (uint16_t i=0; i<_num_vars; i++) {
        const auto &info = var_info(i);
        if (info.type != AP_PARAM_GROUP) {
...
*/
bool AG_Param::get_param_by_index(void *obj_ptr, uint8_t idx, ap_var_type old_ptype, void *pvalue)
{
    uint16_t key;
    ...
    return AG_Param::find_old_parameter(&type_info, (AG_Param *)pvalue);
}
...
//
AG_Param *
AG_Param::find_object(const char *name)
{
    for (uint16_t i=0; i<_num_vars; i++) {
        const auto &info = var_info(i);
...
void AG_Param::notify() const {
    uint32_t group_element = 0;
    const struct GroupInfo *ginfo;
    struct GroupNesting group_nesting {};
    uint8_t idx;

    const struct AG_Param::Info *info = find_var_info(&group_element, ginfo, group_nesting, &idx);
    if (info == nullptr) {
...
*/
void AG_Param::save_sync(bool force_save, bool send_to_gcs)
{
    uint32_t group_element = 0;
    const struct GroupInfo *ginfo;
    struct GroupNesting group_nesting {};
    ...
    if (idx != 0) {
        ap = (const AG_Param *)((ptrdiff_t)ap) - (idx*sizeof(float));
    }
...
*/
void AG_Param::save(bool force_save)
{
 ...
 */
void AG_Param::save_io_handler(void)
{
    struct param_save p;
...
bool AG_Param::load(void)
{
    ...
    ap = this;
    if (idx != 0) {
        ap = (AG_Param *)((ptrdiff_t)ap) - (idx*sizeof(float));
    }
...
bool AG_Param::configured_in_storage(void) const
{
    uint32_t group_element = 0;
    const struct GroupInfo *ginfo;
    struct GroupNesting group_nesting {};
...
bool AG_Param::configured_in_defaults_file(bool &read_only) const
{
...
bool AG_Param::configured(void) const
{
    bool read_only;
    return configured_in_defaults_file(read_only) || configured_in_storage();
}

bool AG_Param::is_read_only(void) const
{
    if (num_read_only == 0) {
        return false;
    }
...
bool AG_Param::verifySha256Checksum()
{
...
//    frame_type = (AP_Int8 *)AG_Param::find(pname[0], &ptype);
//    frame_class = (AP_Int8 *)AG_Param::find(pname[1], &ptype);
...
//	   AG_Param * combined_value = 0;
   	EXPECT_DELAY_MS(1000);
	PdrlBootPlugin::calculateCodeCksm();
	PdrlBootPlugin::startSha2();
		for(int i=0;i<lockParamCount;i++)
		{
			AG_Param *vp = AG_Param::find(pname[i], &ptype);
			if(vp != NULL)
			{
...
// set a AG_Param variable to a specified value
void AG_Param::set_value(enum ap_var_type type, void *ptr, float value)
{
    switch (type) {
    case AP_PARAM_INT8:
...
void AG_Param::setup_object_defaults(const void *object_pointer, const struct GroupInfo *group_info)
{
    ...
    for (uint8_t i=0;
        ...
        if (type <= AP_PARAM_FLOAT) {
            ...
            set_value((enum ap_var_type)type, ptr,
                      get_default_value((const AG_Param *)ptr, group_info[i]));
        } else if (type == AP_PARAM_VECTOR3F) {
            // Single default for all components
            void *ptr = (void *)(base + group_info[i].offset);
            const float default_val = get_default_value((const AG_Param *)ptr, group_info[i]);
...
bool AG_Param::set_object_value(const void *object_pointer,
                                const struct GroupInfo *group_info,
...
void AG_Param::setup_sketch_defaults(void)
{
    setup();
    for (uint16_t i=0; i<_num_vars; i++) {
        const auto &info = var_info(i);
        ...
        if (type <= AP_PARAM_FLOAT) {
            ...
            if (get_base(info, base)) {
                set_value((enum ap_var_type)type, (void*)base,
                          get_default_value((const AG_Param *)base, info));
            }
        }
    }
}
...
bool AG_Param::load_all()
{
    ...
    if (!registered_save_handler) {
        ...
        hal.scheduler->register_io_process(FUNCTOR_BIND((&save_dummy), &AG_Param::save_io_handler, void));
    }
 ...
 */
void AG_Param::reload_defaults_file(bool last_pass)
{
#if AP_PARAM_MAX_EMBEDDED_PARAM > 0
    ...
    if (default_file) {
        if (load_defaults_file(default_file, last_pass)) {
            ...
            AG_HAL::panic("Failed to load defaults from %s\n", default_file);
        }
    }
#endif
 ...
 */
void AG_Param::load_object_from_eeprom(const void *object_pointer, const struct GroupInfo *group_info)
{
    struct Param_header phdr;
    uint16_t key;
...
AG_Param *AG_Param::first(ParamToken *token, enum ap_var_type *ptype, float *default_val)
{
    token->key = 0;
    ...
    if (default_val != nullptr) {
        *default_val = get_default_value((AG_Param *)base, var_info(0));
    }
    check_default((AG_Param *)base, default_val);
#endif
    return (AG_Param *)base;
}
...
/// as needed
AG_Param *AG_Param::next_group(const uint16_t vindex, const struct GroupInfo *group_info,
                               bool *found_current,
    ...
    for (uint8_t i=0;
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            if (*found_current) {
                ...
                AG_Param *ret = (AG_Param*)(base + group_info[i].offset + group_offset);
...
#if AP_PARAM_DEFAULTS_ENABLED
                    if (default_val != nullptr) {
                        *default_val = get_default_value((AG_Param *)ofs, group_info[i]);
                    }
#endif
                    return (AG_Param *)ofs;
                }
...
/// as needed
AG_Param *AG_Param::next(ParamToken *token, enum ap_var_type *ptype, bool skip_disabled, float *default_val)
{
    uint16_t i = token->key;
    ...
    if (type == AP_PARAM_VECTOR3F && token->idx < 3) {
        ...
        AG_Param *ret = (AG_Param *)(((token->idx - 1u)*sizeof(float))+(ptrdiff_t)var_info(i).ptr);
#if AP_PARAM_DEFAULTS_ENABLED
        if (default_val != nullptr) {
    ...
    for (; i<_num_vars; i++) {
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            AG_Param *ap = next_group(i, group_info, &found_current, 0, 0, 0, token, ptype, skip_disabled, default_val);
            if (ap != nullptr) {
                return ap;
            }
        } else {
...
#if AP_PARAM_DEFAULTS_ENABLED
            if (default_val != nullptr) {
                *default_val = get_default_value((AG_Param *)info.ptr, info);
            }
#endif
            return (AG_Param *)(info.ptr);
        }
    }
    return nullptr;
}
...
/// as needed
AG_Param *AG_Param::next_scalar(ParamToken *token, enum ap_var_type *ptype, float *default_val)
{
    AG_Param *ap;
    enum ap_var_type type;
...
float AG_Param::cast_to_float(enum ap_var_type type) const
{
    switch (type) {
    case AP_PARAM_INT8:
 ...
 */
bool AG_Param::find_old_parameter(const struct ConversionInfo *info, AG_Param *value)
{
    // find the old value in EEPROM.
    uint16_t pofs;
    AG_Param::Param_header header;
...
void AG_Param::convert_old_parameter(const struct ConversionInfo *info, float scaler, uint8_t flags)
{
    uint8_t old_value[type_size(info->type)];
    AG_Param *ap = (AG_Param *)&old_value[0];
...
void AG_Param::convert_old_parameters(const struct ConversionInfo *conversion_table, uint8_t table_size, uint8_t flags)
{
...
void AG_Param::convert_class(uint16_t param_key, void *object_pointer,
                                    ...
                                    uint16_t old_index, uint16_t old_top_element, bool is_top_level)
{
    ...
    for (uint8_t i=0; group_info[i].type != AP_PARAM_NONE; i++) {
        ...
        if (!AG_Param::find_old_parameter(&info, ap)) {
            // the parameter wasn't set in the old eeprom
            continue;
        }

        AG_Param *ap2 = (AG_Param *)(group_info[i].offset + (uint8_t *)object_pointer);
...
*/
bool AG_Param::convert_parameter_width(ap_var_type old_ptype)
{
    ...
    AG_Param *old_ap = (AG_Param *)&old_value[0];
 ...
 */
void AG_Param::set_float(float value, enum ap_var_type var_type)
{
 ...
 */
bool AG_Param::parse_param_line(char *line, char **vname, float &value, bool &read_only)
{
    if (line[0] == '#') {
        return false;
    }
...
bool AG_Param::count_defaults_in_file(const char *filename, uint16_t &num_defaults)
{
...
bool AG_Param::read_param_defaults_file(const char *filename, bool last_pass, uint16_t &idx)
{
    // try opening the file both in the posix filesystem and using AP::FS
    int file_apfs = AP::FS().open(filename, O_RDONLY, true);
    if (file_apfs == -1) {
        AG_HAL::panic("AG_Param: Failed to re-open defaults file");
        return false;
    }
    ...
    while (AP::FS().fgets(line, sizeof(line)-1, file_apfs)) {
        ...
        enum ap_var_type var_type;
        AG_Param *vp = find(pname, &var_type);
        if (!vp) {
            if (last_pass) {
#if ENABLE_DEBUG
 ...
 */
bool AG_Param::load_defaults_file(const char *filename, bool last_pass)
{
    if (filename == nullptr) {
        return false;
    }
    ...
    if (mutable_filename == nullptr) {
        AG_HAL::panic("AG_Param: Failed to allocate mutable string");
    }
    ...
    if (param_overrides == nullptr) {
        AG_HAL::panic("AG_Param: Failed to allocate overrides");
        return false;
    }
 ...
 */
bool AG_Param::count_embedded_param_defaults(uint16_t &count)
{
 ...
 *             ignored but if this is set a warning will be emitted
 */
void AG_Param::load_embedded_param_defaults(bool last_pass)
{
    delete[] param_overrides;
 ...
 */
float AG_Param::get_default_value(const AG_Param *vp, const struct GroupInfo &info)
{
    for (uint16_t i=0; i<num_param_overrides; i++) {
...
float AG_Param::get_default_value(const AG_Param *vp, const struct Info &info)
{
    for (uint16_t i=0; i<num_param_overrides; i++) {
        if (vp == param_overrides[i].object_ptr) {
            return param_overrides[i].value;
...
void AG_Param::send_parameter(const char *name, enum ap_var_type var_type, uint8_t idx) const
{
...
/*
  ...
  to be thread safe
 */
uint16_t AG_Param::count_parameters(void)
{
    ...
    while ((_parameter_count == 0 ||
        ...
        for (vp = AG_Param::first(&token, nullptr);
             vp != nullptr;
             vp = AG_Param::next_scalar(&token, nullptr)) {
            count++;
        }
 ...
 */
void AG_Param::invalidate_count(void)
{
 ...
 */
bool AG_Param::set_default_by_name(const char *name, float value)
{
    enum ap_var_type vtype;
    AG_Param *vp = find(name, &vtype);
    if (vp == nullptr) {
        return false;
    }
    switch (vtype) {
 ...
 */
void AG_Param::set_defaults_from_table(const struct defaults_table_struct *table, uint8_t count)
{
    for (uint8_t i=0; i<count; i++) {
        if (!AG_Param::set_default_by_name(table[i].name, table[i].value)) {
            AG_BoardConfig::config_error("param deflt fail:%s", table[i].name);
        }
    }
}
...
/*
  set a value by name
 */
bool AG_Param::set_by_name(const char *name, float value)
{
    enum ap_var_type vtype;
    AG_Param *vp = find(name, &vtype);
    if (vp == nullptr) {
...
/*
  get a value by name
 */
bool AG_Param::get(const char *name, float &value)
{
    enum ap_var_type vtype;
    AG_Param *vp = find(name, &vtype);
    if (vp == nullptr) {
 ...
 */
bool AG_Param::set_and_save_by_name(const char *name, float value)
{
    enum ap_var_type vtype;
    AG_Param *vp = find(name, &vtype);
    if (vp == nullptr) {
...
/*
  set and save a value by name
 */
bool AG_Param::set_and_save_by_name_ifchanged(const char *name, float value)
{
    enum ap_var_type vtype;
    AG_Param *vp = find(name, &vtype);
    if (vp == nullptr) {
...
#if AP_PARAM_DEFAULTS_ENABLED
void AG_Param::check_default(AG_Param *ap, float *default_value)
{
...
void AG_Param::add_default(AG_Param *ap, float v)
{
...
void AG_Param::show(const AG_Param *ap, const char *s,
                    enum ap_var_type type, AG_HAL::BetterStream *port)
{
    switch (type) {
    case AP_PARAM_INT8:
...
// print the value of all variables
void AG_Param::show(const AG_Param *ap, const ParamToken &token,
                    enum ap_var_type type, AG_HAL::BetterStream *port)
{
...
// print the value of all variables
void AG_Param::show_all(AG_HAL::BetterStream *port, bool showKeyValues)
{
    ParamToken token;
    AG_Param *ap;
    enum ap_var_type type;
    ...
    for (ap=AG_Param::first(&token, &type, &default_value);
         ap;
         ap=AG_Param::next_scalar(&token, &type, &default_value)) {
        if (showKeyValues) {
...
*/
bool AG_Param::add_table(uint8_t _key, const char *prefix, uint8_t num_params)
{
...
/*
 ...
 a key in add_table()
*/
bool AG_Param::load_int32(uint16_t key, uint32_t group_element, int32_t &value)
{
    struct Param_header phdr;
 ...
 */
bool AG_Param::add_param(uint8_t _key, uint8_t param_num, const char *pname, float default_value)
{
    if (_var_info_dynamic == nullptr) {
