...
const AG_Param::GroupInfo AG_SmartRTL::var_info[] = {
    // @Param: ACCURACY
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO("ACCURACY", 0, AG_SmartRTL, _accuracy, SMARTRTL_ACCURACY_DEFAULT),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("POINTS", 1, AG_SmartRTL, _points_max, SMARTRTL_POINTS_DEFAULT),
    ...
    // @User: Standard
    AP_GROUPINFO("OPTIONS", 2, AG_SmartRTL, _options, 0),
...
AG_SmartRTL::AG_SmartRTL(bool example_mode) :
...
void AG_SmartRTL::init()
{
    ...
    if (!_example_mode){
        ...
        hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&AG_SmartRTL::run_background_cleanup, void));
    }
}

// returns number of points on the path
uint16_t AG_SmartRTL::get_num_points() const
{
    return _path_points_count;
}

// get next point on the path to home, returns true on success
bool AG_SmartRTL::pop_point(Vector3f& point)
{
    // check we are active
    if (!_active) {
        return false;
...
bool AG_SmartRTL::peek_point(Vector3f& point)
{
    // check we are active
    if (!_active) {
        return false;
...
void AG_SmartRTL::set_home(bool position_ok)
{
...
void AG_SmartRTL::set_home(bool position_ok, const Vector3f& current_pos)
{
    if (_path == nullptr) {
        return;
    }
    ...
    _last_good_position_ms = AG_HAL::millis();
    _active = true;
    _home_saved = true;
}

// call this at 3hz (or higher) regardless of what mode the vehicle is in
void AG_SmartRTL::update(bool position_ok, bool save_position)
{
...
void AG_SmartRTL::update(bool position_ok, const Vector3f& current_pos)
{
    if (!_active) {
        return;
    }
    ...
    if (position_ok) {
        ...
        } else if (AG_HAL::millis() - _last_position_save_ms > SMARTRTL_TIMEOUT) {
        ...
        if (AG_HAL::millis() - _last_good_position_ms > SMARTRTL_TIMEOUT) {
...
bool AG_SmartRTL::request_thorough_cleanup(ThoroughCleanupType clean_type)
{
    ...
    if (_thorough_clean_request_ms == 0) {
        _thorough_clean_request_ms = AG_HAL::millis();
...
void AG_SmartRTL::cancel_request_for_thorough_cleanup()
{
    _thorough_clean_request_ms = 0;
}
...
bool AG_SmartRTL::add_point(const Vector3f& point)
{
    // get semaphore
    if (!_path_sem.take_nonblocking()) {
...
void AG_SmartRTL::run_background_cleanup()
{
    if (!_active) {
        return;
    }
...
void AG_SmartRTL::routine_cleanup(uint16_t path_points_count, uint16_t path_points_completed_limit)
{
...
bool AG_SmartRTL::thorough_cleanup(uint16_t path_points_count, ThoroughCleanupType clean_type)
{
...
void AG_SmartRTL::detect_simplifications()
{
...
void AG_SmartRTL::detect_loops()
{
    ...
    while (AG_HAL::micros() - start_time_us < SMARTRTL_PRUNING_LOOP_TIME_US) {
...
// path_points_count is _path_points_count but passed in to avoid having to take the semaphore
void AG_SmartRTL::restart_simplify_if_new_points(uint16_t path_points_count)
{
...
void AG_SmartRTL::restart_pruning_if_new_points()
{
    // any difference in the number of points is because of new points being added to path
...
void AG_SmartRTL::restart_simplification(uint16_t path_points_count)
{
...
void AG_SmartRTL::reset_simplification()
{
...
void AG_SmartRTL::restart_pruning(uint16_t path_points_count)
{
...
void AG_SmartRTL::reset_pruning()
{
    restart_pruning(0);
...
void AG_SmartRTL::remove_points_by_simplify_bitmask()
{
...
bool AG_SmartRTL::remove_points_by_loops(uint16_t num_points_to_remove)
{
...
bool AG_SmartRTL::add_loop(uint16_t start_index, uint16_t end_index, const Vector3f& midpoint)
{
...
*/
AG_SmartRTL::dist_point AG_SmartRTL::segment_segment_dist(const Vector3f &p1, const Vector3f &p2, const Vector3f &p3, const Vector3f &p4)
{
...
void AG_SmartRTL::deactivate(SRTL_Actions action, const char *reason)
{
    _active = false;
    log_action(action);
...
// logging
void AG_SmartRTL::log_action(SRTL_Actions action, const Vector3f &point) const
{
    if (!_example_mode) {
...
bool AG_SmartRTL::loops_overlap(const prune_loop_t &loop1, const prune_loop_t &loop2) const
{
...
bool AG_SmartRTL::use_pilot_yaw(void) const
{
