...
void AP_GyroFFT::init(uint16_t loop_rate_hz)
{
    ...
    if (allocation_count * FFT_DEFAULT_WINDOW_SIZE > hal.util->available_memory() / 2) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "AP_GyroFFT: disabled, required %u bytes", (unsigned int)allocation_count * FFT_DEFAULT_WINDOW_SIZE);
        return;
    } else if (allocation_count * _window_size > hal.util->available_memory() / 2) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "AP_GyroFFT: req alloc %u bytes (free=%u)", (unsigned int)allocation_count * _window_size, (unsigned int)hal.util->available_memory());
    ...
    if (_sample_mode == 0) {
        ...
        for (uint8_t axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            if (!_downsampled_gyro_data[axis].set_size(_window_size + _samples_per_frame)) {
                gcs().send_text(AGPILOT_SEVERITY_WARNING, "Failed to allocate window for AP_GyroFFT");
                return;
            }
        }
    }
    ...
    if (_state == nullptr) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "Failed to initialize DSP engine");
        return;
    }
...
bool AP_GyroFFT::pre_arm_check(char *failure_msg, const uint8_t failure_msg_len)
{
    ...
    if (_state->_bin_resolution > 50.0f) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: resolution is %.1fHz, increase length", _state->_bin_resolution);
    ...
    if (_calibrated) {
        ...
        gcs().send_text(AGPILOT_SEVERITY_INFO, "FFT: calibrated %.1fKHz/%.1fHz/%.1fHz", _fft_sampling_rate_hz * 0.001f,
...
void AP_GyroFFT::start_notch_tune()
{
    ...
    if (!hal.dsp->fft_start_average(_state)) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: Unable to start FFT averaging");
    }
    // throttle averaging for average fft calculation
...
void AP_GyroFFT::stop_notch_tune()
{
    ...
    gcs().send_text(AGPILOT_SEVERITY_INFO, "FFT: Found peaks at %.1f/%.1f/%.1fHz", freqs[0], freqs[1], freqs[2]);
    gcs().send_text(AGPILOT_SEVERITY_NOTICE, "FFT: Selected %.1fHz\n", harmonic);

    // if we don't have a throttle value then all bets are off
    if (is_zero(_avg_throttle_out) || is_zero(harmonic)) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: Unable to calculate notch: need stable hover");
    ...
    if (!_ins->setup_throttle_gyro_harmonic_notch(harmonic, (float)_fft_min_hz.get(), _avg_throttle_out, harmonics)) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: Unable to set throttle notch with %.1fHz/%.2f",
        ...
        gcs().send_text(AGPILOT_SEVERITY_NOTICE, "FFT: Notch frequency %.1fHz and ref %.2f selected", harmonic, _avg_throttle_out);
...
void AP_GyroFFT::write_log_messages()
{
    ...
    if ((now - _last_output_ms) > 1000) {
        ...
        WITH_SEMAPHORE(_sem);
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: f:%.1f, fr:%.1f, b:%u, fd:%.1f",
        ...
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: bw:%.1f, e:%.1f, r:%.1f, snr:%.1f",
...
float AP_GyroFFT::self_test_bin_frequencies()
{
    if (_state->_window_size * sizeof(float) > hal.util->available_memory() / 2) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: unable to run self-test, required %u bytes", (unsigned int)(_state->_window_size * sizeof(float)));
        return 0.0f;
    }
...
float AP_GyroFFT::self_test(float frequency, FloatBuffer& test_window)
{
    ...
    if (max_bin == 0) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: self-test failed, failed to find frequency %.1f", frequency);
    }
    ...
    if (_thread_state._center_freq_hz[0] < (frequency - MAX(_state->_bin_resolution * 0.5f, 1)) || _thread_state._center_freq_hz[0] > (frequency + MAX(_state->_bin_resolution * 0.5f, 1))) {
        gcs().send_text(AGPILOT_SEVERITY_WARNING, "FFT: self-test failed: wanted %.1f, had %.1f", frequency, _thread_state._center_freq_hz[0]);
    }
#if DEBUG_FFT
    else {
        gcs().send_text(AGPILOT_SEVERITY_INFO, "FFT: self-test succeeded: wanted %.1f, had %.1f", frequency, _thread_state._center_freq_hz[0]);
    }
#endif
