...
class AG_PrecLand_Companion;
class AG_PrecLand_IRLock;
class AG_PrecLand_SITL_Gazebo;
class AG_PrecLand_SITL;

class AG_PrecLand
{
    // declare backends as friends
    friend class AG_PrecLand_Backend;
    friend class AG_PrecLand_Companion;
    friend class AG_PrecLand_IRLock;
    friend class AG_PrecLand_SITL_Gazebo;
    friend class AG_PrecLand_SITL;

public:
    AG_PrecLand();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_PrecLand);

    // return singleton
    static AG_PrecLand *get_singleton() {
        return _singleton;
    }
    ...
    // Parameter getters
    AG_PrecLand_StateMachine::RetryStrictness get_retry_strictness() const { return static_cast<AG_PrecLand_StateMachine::RetryStrictness>(_strict.get()); }
    uint8_t get_max_retry_allowed() const { return _retry_max; }
    float get_min_retry_time_sec() const { return _retry_timeout_sec; }
    AG_PrecLand_StateMachine::RetryAction get_retry_behaviour() const { return static_cast<AG_PrecLand_StateMachine::RetryAction>(_retry_behave.get()); }
...
private:
    ...
    AG_PrecLand_Backend         *_backend;  // pointers to backend precision landing driver

    // write out PREC message to log:
    void Write_Precland();
    uint32_t last_log_ms;  // last time we logged

    static AG_PrecLand *_singleton; //singleton
};
