...
void AG_PrecLand_Companion::init()
{
    // set healthy
    _state.healthy = true;
...
void AG_PrecLand_Companion::update()
{
...
bool AG_PrecLand_Companion::get_los_body(Vector3f& ret) {
    if (have_los_meas()) {
        ret = _los_meas_body;
        return true;
    }
...
uint32_t AG_PrecLand_Companion::los_meas_time_ms() {
    return _los_meas_time_ms;
}

// return true if there is a valid los measurement available
bool AG_PrecLand_Companion::have_los_meas()
{
    return _have_los_meas;
}

// return distance to target
float AG_PrecLand_Companion::distance_to_target()
{
    return _distance_to_target;
}

void AG_PrecLand_Companion::handle_msg(const mavlink_landing_target_t &packet, uint32_t timestamp_ms)
{
