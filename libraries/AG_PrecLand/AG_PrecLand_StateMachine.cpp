#include "AG_PrecLand_StateMachine.h"
...
void AG_PrecLand_StateMachine::init()
{
    AG_PrecLand *_precland = AP::ac_precland();
...
void AG_PrecLand_StateMachine::reset_failed_landing_statemachine()
{
...
AG_PrecLand_StateMachine::Status AG_PrecLand_StateMachine::update(Vector3f &retry_pos_m)
{
    ...
    AG_PrecLand::TargetState precland_target_state =  _precland->get_target_state();

    switch (precland_target_state) {
    case AG_PrecLand::TargetState::TARGET_RECENTLY_LOST:
    ...
    case AG_PrecLand::TargetState::TARGET_NEVER_SEEN:
    ...
    case AG_PrecLand::TargetState::TARGET_OUT_OF_RANGE:
    ...
    case AG_PrecLand::TargetState::TARGET_FOUND:
...
// Parameters: Vector3f "retry_pos_m" is filled with the required location if we need to retry landing.
AG_PrecLand_StateMachine::Status AG_PrecLand_StateMachine::get_target_lost_actions(Vector3f &retry_pos_m)
{
    AG_PrecLand *_precland = AP::ac_precland();
    if (_precland == nullptr) {
        // should never happen
    ...
    case TargetLostAction::DESCEND:
        if (AG_HAL::millis() - _precland->get_last_valid_target_ms() >=_precland->get_min_retry_time_sec() * 1000) {
...
AG_PrecLand_StateMachine::Status AG_PrecLand_StateMachine::retry_landing(Vector3f &retry_pos_m)
{
    AG_PrecLand *_precland = AP::ac_precland();
    if (_precland == nullptr) {
        // should never happen
...
AG_PrecLand_StateMachine::FailSafeAction AG_PrecLand_StateMachine::get_failsafe_actions()
{
    AG_PrecLand *_precland = AP::ac_precland();
    if (_precland == nullptr) {
    ...
    if (!failsafe_initialized) {
        // start the timer
        failsafe_start_ms = AG_HAL::millis();
    ...
    case RetryStrictness::NORMAL:
        if (AG_HAL::millis() - failsafe_start_ms < FAILSAFE_INIT_TIMEOUT_MS) {
