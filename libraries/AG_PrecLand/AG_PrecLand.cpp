...
#include "AG_PrecLand_Backend.h"
#include "AG_PrecLand_Companion.h"
#include "AG_PrecLand_IRLock.h"
...
extern const AG_HAL::HAL& hal;
 ...
 # define AG_PRECLAND_ORIENT_DEFAULT Rotation::ROTATION_NONE
#else
 # define AG_PRECLAND_ORIENT_DEFAULT Rotation::ROTATION_PITCH_270
#endif
...
const AG_Param::GroupInfo AG_PrecLand::var_info[] = {
    // @Param: ENABLED
    // @DisplayName: Precision Land enabled/disabled
    // @Description: Precision Land enabled/disabled
    // @Values: 0:Disabled, 1:Enabled
    // @User: Advanced
    AP_GROUPINFO_FLAGS("ENABLED", 0, AG_PrecLand, _enabled, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    AP_GROUPINFO("TYPE",    1, AG_PrecLand, _type, 0),
    ...
    // @User: Advanced
    // @Units: cdeg
    AP_GROUPINFO("YAW_ALIGN",    2, AG_PrecLand, _yaw_align, 0),
    ...
    // @User: Advanced
    // @Units: cm
    AP_GROUPINFO("LAND_OFS_X",    3, AG_PrecLand, _land_ofs_cm_x, 0),
    ...
    // @User: Advanced
    // @Units: cm
    AP_GROUPINFO("LAND_OFS_Y",    4, AG_PrecLand, _land_ofs_cm_y, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("EST_TYPE",    5, AG_PrecLand, _estimator_type, 1),
    ...
    // @Range: 0.5 5
    // @User: Advanced
    AP_GROUPINFO("ACC_P_NSE", 6, AG_PrecLand, _accel_noise, 2.5f),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("CAM_POS", 7, AG_PrecLand, _cam_offset, 0.0f),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUS",    8, AG_PrecLand, _bus, -1),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("LAG", 9, AG_PrecLand, _lag, 0.02f), // 20ms is the old default buffer size (8 frames @ 400hz/2.5ms)
    ...
    // @Units: m
    // @User: Advanced
    AP_GROUPINFO("XY_DIST_MAX", 10, AG_PrecLand, _xy_max_dist_desc, 2.5f),
    // @Param: STRICT
    ...
    AP_GROUPINFO("STRICT", 11, AG_PrecLand, _strict, 1),
    ...
    // @Range: 0 10
    // @Increment: 1
    AP_GROUPINFO("RET_MAX", 12, AG_PrecLand, _retry_max, 4),
    ...
    // @Range: 0 20
    // @Units: s
    AP_GROUPINFO("TIMEOUT", 13, AG_PrecLand, _retry_timeout_sec, 4),
    ...
    AP_GROUPINFO("RET_BEHAVE", 14, AG_PrecLand, _retry_behave, 0),
    ...
    // @Range: 0 5
    // @Units: m
    AP_GROUPINFO("ALT_MIN", 15, AG_PrecLand, _sensor_min_alt, 0.75),
    ...
    // @Range: 0 50
    // @Units: m
    AP_GROUPINFO("ALT_MAX", 16, AG_PrecLand, _sensor_max_alt, 8),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 17, AG_PrecLand, _options, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FRAME("ORIENT", 18, AG_PrecLand, _orient, AG_PRECLAND_ORIENT_DEFAULT, AP_PARAM_FRAME_ROVER),

    AP_GROUPEND
};

// Default constructor.
AG_PrecLand::AG_PrecLand()
{
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_PrecLand must be singleton");
    }
    _singleton = this;
...
void AG_PrecLand::init(uint16_t update_rate_hz)
{
    ...
    switch ((Type)(_type.get())) {
        ...
        // companion computer
        case Type::COMPANION:
            _backend = new AG_PrecLand_Companion(*this, _backend_state);
            break;
        // IR Lock
        case Type::IRLOCK:
            _backend = new AG_PrecLand_IRLock(*this, _backend_state);
            break;
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        case Type::SITL_GAZEBO:
            _backend = new AG_PrecLand_SITL_Gazebo(*this, _backend_state);
            break;
        case Type::SITL:
            _backend = new AG_PrecLand_SITL(*this, _backend_state);
            break;
#endif
    }
...
void AG_PrecLand::update(float rangefinder_alt_cm, bool rangefinder_alt_valid)
{
    ...
    inertial_data_newest.time_usec = AG_HAL::micros64();
...
void AG_PrecLand::check_target_status(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    if (target_acquired()) {
        // target in sight
    ...
    if (_current_target_state == TargetState::TARGET_RECENTLY_LOST) {
        ...
        if (AP::ahrs().get_relative_position_NE_origin(curr_pos)) {
            ...
            if ((AG_HAL::millis() - _last_valid_target_ms) > LANDING_TARGET_LOST_TIMEOUT_MS) {
...
bool AG_PrecLand::check_if_sensor_in_range(float rangefinder_alt_m, bool rangefinder_alt_valid) const
{
...
bool AG_PrecLand::target_acquired()
{
    if ((AG_HAL::millis()-_last_update_ms) > LANDING_TARGET_TIMEOUT_MS) {
        if (_target_acquired) {
...
bool AG_PrecLand::get_target_position_cm(Vector2f& ret)
{
    if (!target_acquired()) {
        return false;
    }
...
void AG_PrecLand::get_target_position_measurement_cm(Vector3f& ret)
{
    ret = _target_pos_rel_meas_NED*100.0f;
    return;
}

bool AG_PrecLand::get_target_position_relative_cm(Vector2f& ret)
{
    if (!target_acquired()) {
        return false;
    }
...
bool AG_PrecLand::get_target_velocity_relative_cms(Vector2f& ret)
{
    if (!target_acquired()) {
        return false;
    }
...
void AG_PrecLand::get_target_velocity_cms(const Vector2f& vehicle_velocity_cms, Vector2f& target_vel_cms)
{
...
void AG_PrecLand::handle_msg(const mavlink_landing_target_t &packet, uint32_t timestamp_ms)
{
    // run backend update
    if (_backend != nullptr) {
...
void AG_PrecLand::run_estimator(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
        ...
        case EstimatorType::KALMAN_FILTER: {
            ...
            if (construct_pos_meas_using_rangefinder(rangefinder_alt_m, rangefinder_alt_valid)) {
                ...
                if (!_estimator_initialized) {
                    ...
                    _last_update_ms = AG_HAL::millis();
                    _estimator_init_ms = AG_HAL::millis();
...
void AG_PrecLand::check_ekf_init_timeout()
{
    if (!target_acquired() && _estimator_initialized) {
        // we have just got the target in sight
        if (AG_HAL::millis()-_last_update_ms > EKF_INIT_SENSOR_MIN_UPDATE_MS) {
        ...
        } else if (AG_HAL::millis()-_estimator_init_ms > EKF_INIT_TIME_MS) {
...
bool AG_PrecLand::retrieve_los_meas(Vector3f& target_vec_unit_body)
{
...
bool AG_PrecLand::construct_pos_meas_using_rangefinder(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
...
void AG_PrecLand::run_output_prediction()
{
    ...
    _last_valid_target_ms = AG_HAL::millis();
}

// Write a precision landing entry
void AG_PrecLand::Write_Precland()
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
// singleton instance
AG_PrecLand *AG_PrecLand::_singleton;

namespace AP {

AG_PrecLand *ac_precland()
{
    return AG_PrecLand::get_singleton();
}
