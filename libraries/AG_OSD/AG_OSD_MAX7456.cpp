...
#include <AG_OSD/AG_OSD_MAX7456.h>

#if HAL_WITH_OSD_BITMAP
#include <AG_HAL/Util.h>
#include <AG_HAL/Semaphores.h>
#include <AG_HAL/Scheduler.h>
...
AG_OSD_MAX7456::AG_OSD_MAX7456(AG_OSD &osd, AG_HAL::OwnPtr<AG_HAL::Device> dev):
    AG_OSD_Backend(osd), _dev(std::move(dev))
{
    video_signal_reg = VIDEO_MODE_PAL | OSD_ENABLE;
    video_lines = video_lines_pal;
}

bool AG_OSD_MAX7456::init()
{
    uint8_t status = 0xFF;
...
bool AG_OSD_MAX7456::update_font()
{
    ...
    if (font_size != NVM_RAM_SIZE * 256) {
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "AG_OSD: bad font size %u\n", unsigned(font_size));
        delete fd;
        return false;
    }
...
bool AG_OSD_MAX7456::check_font_char(uint8_t chr, const uint8_t* font_data)
{
    buffer_offset = 0;
...
bool AG_OSD_MAX7456::update_font_char(uint8_t chr, const uint8_t* font_data)
{
    uint16_t retry;
    buffer_offset = 0;
    buffer_add_cmd(MAX7456ADD_VM0, 0);
...
AG_OSD_Backend *AG_OSD_MAX7456::probe(AG_OSD &osd, AG_HAL::OwnPtr<AG_HAL::Device> dev)
{
    if (!dev) {
        return nullptr;
    }

    AG_OSD_MAX7456 *backend = new AG_OSD_MAX7456(osd, std::move(dev));
    if (!backend) {
        return nullptr;
    }
    if (!backend->init()) {
...
void AG_OSD_MAX7456::buffer_add_cmd(uint8_t reg, uint8_t arg)
{
...
void AG_OSD_MAX7456::check_reinit()
{
    uint8_t check = 0xFF;
    _dev->get_semaphore()->take_blocking();
        ...
        if (sense & STAT_LOS) {
            ...
            if ((VIN_IS_PAL(sense) && VIDEO_MODE_IS_NTSC(video_signal_reg))
                ...
                if (video_detect_time) {
                    if (AG_HAL::millis() - video_detect_time > VIDEO_SIGNAL_DEBOUNCE_MS) {
                        reinit();
                    }
                } else {
                    // Wait for signal to stabilize
                    video_detect_time = AG_HAL::millis();
                }
            }
        }
...
void AG_OSD_MAX7456::transfer_frame()
{
...
bool AG_OSD_MAX7456::is_dirty(uint8_t x, uint8_t y)
{
...
void AG_OSD_MAX7456::clear()
{
    AG_OSD_Backend::clear();
    memset(frame, ' ', sizeof(frame));
}

void AG_OSD_MAX7456::write(uint8_t x, uint8_t y, const char* text)
{
...
float AG_OSD_MAX7456::get_aspect_ratio_correction() const
{
    switch (_format) {
    case FORMAT_NTSC:
        return 12.0f/18.46f;
