...
constexpr uint8_t AG_OSD_Backend::symbols[AG_OSD_NUM_SYMBOLS];

#define SYM_DIG_OFS_1 0x90
#define SYM_DIG_OFS_2 0xA0


uint8_t AG_OSD_Backend::convert_to_decimal_packed_characters(char* buff, uint8_t size) {
    if (size == 0) {
        return 0;
    }
#if OSD_ENABLED
...
uint8_t AG_OSD_Backend::format_string_for_osd(char* buff, uint8_t size, bool decimal_packed, const char *fmt, va_list ap)
{
    if (size == 0) {
        return 0;
    }
...
void AG_OSD_Backend::write(uint8_t x, uint8_t y, bool blink, const char *fmt, ...)
{
#if OSD_ENABLED
    ...
    IGNORE_RETURN(format_string_for_osd(buff, sizeof(buff), check_option(AG_OSD::OPTION_DECIMAL_PACK), fmt, ap));
    va_end(ap);
 ...
 */
FileData *AG_OSD_Backend::load_font_data(uint8_t font_num)
{
    FileData *fd;
...
void AG_OSD_Backend::init_symbol_set(uint8_t *lookup_table, const uint8_t size)
{
