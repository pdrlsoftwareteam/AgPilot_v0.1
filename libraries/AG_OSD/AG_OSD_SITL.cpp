...
void AG_OSD_SITL::load_font(void)
{
    ...
    for (uint16_t i=0; i<256; i++) {
        ...
        if (!font[i].create(char_width, char_height)) {
            AG_HAL::panic("Failed to create texture");
        }
...
void AG_OSD_SITL::write(uint8_t x, uint8_t y, const char* text)
{
...
void AG_OSD_SITL::flush(void)
{
    counter++;
}

// main loop of graphics thread
void AG_OSD_SITL::update_thread(void)
{
    load_font();
    {
    ...
    if (!w) {
        AG_HAL::panic("Unable to create OSD window");
    }
...
void *AG_OSD_SITL::update_thread_start(void *obj)
{
    ((AG_OSD_SITL *)obj)->update_thread();
    return nullptr;
}

// initialise backend
bool AG_OSD_SITL::init(void)
{
    pthread_create(&thread, NULL, update_thread_start, this);
    return true;
}

AG_OSD_Backend *AG_OSD_SITL::probe(AG_OSD &osd)
{
    AG_OSD_SITL *backend = new AG_OSD_SITL(osd);
    if (!backend) {
        return nullptr;
    }
    if (!backend->init()) {
...
AG_OSD_SITL::AG_OSD_SITL(AG_OSD &osd):
    AG_OSD_Backend(osd)
{
}
