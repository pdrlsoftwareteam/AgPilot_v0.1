...
/*
  parameter object for one setting in AG_OSD
 */
...
const AG_Param::GroupInfo AG_OSD_ParamSetting::var_info[] = {
    // @Param: _EN
    // @DisplayName: Enable
    // @Description: Enable setting
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("_EN", 1, AG_OSD_ParamSetting, enabled, default_enabled),
    ...
    // @Range: 0 29
    // @User: Standard
    AP_GROUPINFO("_X", 2, AG_OSD_ParamSetting, xpos, 2),
    ...
    // @Range: 0 15
    // @User: Standard
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("_Y", 3, AG_OSD_ParamSetting, ypos, default_ypos),
...
// plane parameters
const AG_OSD_ParamSetting::ParamMetadata AG_OSD_ParamSetting::_param_metadata[OSD_PARAM_NUM_TYPES] = {
    { -1, AG_SerialManager::SerialProtocol_NumProtocols - 1,    1, ARRAY_SIZE(SERIAL_PROTOCOL_VALUES), SERIAL_PROTOCOL_VALUES },  // OSD_PARAM_SERIAL_PROTOCOL
...
AG_OSD_ParamSetting::AG_OSD_ParamSetting(uint8_t param_number) :
...
AG_OSD_ParamSetting::AG_OSD_ParamSetting(const Initializer& initializer) :
...
void AG_OSD_ParamSetting::update()
{
    ...
    _current_token = AG_Param::ParamToken {};
    // surely there is a more efficient way than brute-force search
    for (_param = AG_Param::first(&_current_token, &_param_type);
        _param && (AG_Param::get_persistent_key(_current_token.key) != uint16_t(_param_key.get())
            || _current_token.idx != uint8_t(_param_idx.get())
            || _current_token.group_element != uint32_t(_param_group.get()));
        _param = AG_Param::next_scalar(&_current_token, &_param_type)) {
    }
...
bool AG_OSD_ParamSetting::set_by_name(const char* name, uint8_t config_type, float pmin, float pmax, float pincr)
{
    AG_Param::ParamToken token = AG_Param::ParamToken {};
    ap_var_type type;
    AG_Param* param = AG_Param::find_by_name(name, &type, &token);
    ...
    _param_key.set_and_save_ifchanged(AG_Param::get_persistent_key(_current_token.key));
...
void AG_OSD_ParamSetting::guess_ranges(bool force)
{
...
void AG_OSD_ParamSetting::copy_name_camel_case(char* name, size_t len) const
{
    char buf[17];
...
bool AG_OSD_ParamSetting::set_from_metadata()
{
    // check for statically configured setting metadata
...
void AG_OSD_ParamSetting::save_as_new()
{
    _param_group.save();
    _param_key.save();
    _param_idx.save();
