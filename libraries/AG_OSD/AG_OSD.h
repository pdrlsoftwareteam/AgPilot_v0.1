...
#endif
#include <AG_Fence/AG_Fence_config.h>

class AG_OSD_Backend;
class AG_MSP;

#define AG_OSD_NUM_DISPLAY_SCREENS 4
#if OSD_PARAM_ENABLED
#define AG_OSD_NUM_PARAM_SCREENS 2
#else
#define AG_OSD_NUM_PARAM_SCREENS 0
#endif
#define AG_OSD_NUM_SCREENS (AG_OSD_NUM_DISPLAY_SCREENS + AG_OSD_NUM_PARAM_SCREENS)

#define PARAM_INDEX(key, idx, group) (uint32_t(uint32_t(key) << 23 | uint32_t(idx) << 18 | uint32_t(group)))
#define PARAM_TOKEN_INDEX(token) PARAM_INDEX(AG_Param::get_persistent_key(token.key), token.idx, token.group_element)

#define AG_OSD_NUM_SYMBOLS 91
/*
  class to hold one setting
 */
class AG_OSD_Setting
{
public:
    AP_Int8 enabled;
    AP_Int8 xpos;
    ...
    AG_OSD_Setting(bool enabled = 0, uint8_t x = 0, uint8_t y = 0);
...
class AG_OSD_AbstractScreen
{
    friend class AG_OSD;
public:
    // constructor
    AG_OSD_AbstractScreen() {}
    virtual void draw(void) {}

    void set_backend(AG_OSD_Backend *_backend);
...
protected:
    ...
    AG_OSD_Backend *backend;
    AG_OSD *osd;

    static uint8_t symbols_lookup_table[AG_OSD_NUM_SYMBOLS];
};
 ...
 */
class AG_OSD_Screen : public AG_OSD_AbstractScreen
{
public:
    // constructor
    AG_OSD_Screen();
...
private:
    ...
    AG_OSD_Setting altitude{true, 23, 8};
    AG_OSD_Setting bat_volt{true, 24, 1};
    AG_OSD_Setting rssi{true, 1, 1};
    AG_OSD_Setting link_quality{false,1,1};
    AG_OSD_Setting restvolt{false, 24, 2};
    AG_OSD_Setting avgcellvolt{false, 24, 3};
    AG_OSD_Setting avgcellrestvolt{false, 24, 4};
    AG_OSD_Setting current{true, 25, 2};
    AG_OSD_Setting batused{true, 23, 3};
    AG_OSD_Setting sats{true, 1, 3};
    AG_OSD_Setting fltmode{true, 2, 8};
    AG_OSD_Setting message{true, 2, 6};
    AG_OSD_Setting gspeed{true, 2, 14};
    AG_OSD_Setting horizon{true, 14, 8};
    AG_OSD_Setting home{true, 14, 1};
    AG_OSD_Setting throttle{true, 24, 11};
    AG_OSD_Setting heading{true, 13, 2};
    AG_OSD_Setting compass{true, 15, 3};
    AG_OSD_Setting wind{false, 2, 12};
    AG_OSD_Setting aspeed{false, 2, 13};
    AG_OSD_Setting aspd1;
    AG_OSD_Setting aspd2;
    AG_OSD_Setting vspeed{true, 24, 9};
#if HAL_WITH_ESC_TELEM
    AG_OSD_Setting esc_temp {false, 24, 13};
    AG_OSD_Setting esc_rpm{false, 22, 12};
    AG_OSD_Setting esc_amps{false, 24, 14};
#endif
    AG_OSD_Setting gps_latitude{true, 9, 13};
    AG_OSD_Setting gps_longitude{true, 9, 14};
    AG_OSD_Setting roll_angle;
    AG_OSD_Setting pitch_angle;
    AG_OSD_Setting temp;
#if BARO_MAX_INSTANCES > 1
    AG_OSD_Setting btemp;
#endif
    AG_OSD_Setting hdop;
    AG_OSD_Setting waypoint;
    AG_OSD_Setting xtrack_error;
    AG_OSD_Setting dist{false,22,11};
    AG_OSD_Setting stat{false,0,0};
    AG_OSD_Setting flightime{false, 23, 10};
    AG_OSD_Setting climbeff{false,0,0};
    AG_OSD_Setting eff{false, 22, 10};
    AG_OSD_Setting atemp;
    AG_OSD_Setting bat2_vlt;
    AG_OSD_Setting bat2used;
    AG_OSD_Setting current2;
    AG_OSD_Setting clk;
    AG_OSD_Setting callsign;
    AG_OSD_Setting vtx_power;
    AG_OSD_Setting hgt_abvterr{false, 23, 7};
    AG_OSD_Setting fence{false, 14, 9};
    AG_OSD_Setting rngf;
#if HAL_PLUSCODE_ENABLE
    AG_OSD_Setting pluscode;
#endif
    AG_OSD_Setting sidebars{false, 4, 5};

    // MSP OSD only
    AG_OSD_Setting crosshair;
    AG_OSD_Setting home_dist{true, 1, 1};
    AG_OSD_Setting home_dir{true, 1, 1};
    AG_OSD_Setting power{true, 1, 1};
    AG_OSD_Setting cell_volt{true, 1, 1};
    AG_OSD_Setting batt_bar{true, 1, 1};
    AG_OSD_Setting arming{true, 1, 1};
...
public:
    ...
    AG_Param::ParamToken _current_token;
    ...
    AG_OSD_ParamSetting() {};
    AG_OSD_ParamSetting(uint8_t param_number);
    AG_OSD_ParamSetting(const Initializer& initializer);
...
/*
  class to hold one screen of settings
 */
class AG_OSD_ParamScreen : public AG_OSD_AbstractScreen
{
public:
    AG_OSD_ParamScreen(uint8_t index);
    ...
    AG_OSD_ParamSetting* get_setting(uint8_t param_idx);
...
private:
    AG_OSD_ParamSetting params[NUM_PARAMS];
...
{
public:
    friend class AG_OSD_Screen;
    friend class AG_MSP;
    friend class AG_MSP_Telem_Backend;
    friend class AG_OSD_ParamScreen;
    //constructor
    AG_OSD();

    /* Do not allow copies */
    CLASS_NO_COPY(AG_OSD);

    // get singleton instance
    static AG_OSD *get_singleton()
    {
        return _singleton;
    }
    ...
    AG_OSD_Screen screen[AG_OSD_NUM_DISPLAY_SCREENS];
    ...
    AG_OSD_AbstractScreen& get_screen(uint8_t idx) {
#if OSD_PARAM_ENABLED
        if (idx >= AG_OSD_NUM_DISPLAY_SCREENS) {
            return param_screen[idx - AG_OSD_NUM_DISPLAY_SCREENS];
        }
#endif
        return screen[idx];
    }
    ...
    bool is_readonly_screen() const { return current_screen < AG_OSD_NUM_DISPLAY_SCREENS; }
    // get the current screen
    uint8_t get_current_screen() const { return current_screen; };
#endif // OSD_ENABLED
#if OSD_PARAM_ENABLED
    AG_OSD_ParamScreen param_screen[AG_OSD_NUM_PARAM_SCREENS] { 0, 1 };
    // return a setting for use by TX based OSD
    AG_OSD_ParamSetting* get_setting(uint8_t screen_idx, uint8_t param_idx) {
        if (screen_idx >= AG_OSD_NUM_PARAM_SCREENS) {
            return nullptr;
        }
