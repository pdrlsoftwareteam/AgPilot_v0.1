...
#include "AG_OSD_MAX7456.h"
#ifdef WITH_SITL_OSD
#include "AG_OSD_SITL.h"
#endif
#include "AG_OSD_MSP.h"
#include "AG_OSD_MSP_DisplayPort.h"
...
const AG_Param::GroupInfo AG_OSD::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("_TYPE", 1, AG_OSD, osd_type, 0, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Standard
    AP_GROUPINFO("_CHAN", 2, AG_OSD, rc_channel, 0),

    // @Group: 1_
    // @Path: AG_OSD_Screen.cpp
    AP_SUBGROUPINFO(screen[0], "1_", 3, AG_OSD, AG_OSD_Screen),

    // @Group: 2_
    // @Path: AG_OSD_Screen.cpp
    AP_SUBGROUPINFO(screen[1], "2_", 4, AG_OSD, AG_OSD_Screen),

    // @Group: 3_
    // @Path: AG_OSD_Screen.cpp
    AP_SUBGROUPINFO(screen[2], "3_", 5, AG_OSD, AG_OSD_Screen),

    // @Group: 4_
    // @Path: AG_OSD_Screen.cpp
    AP_SUBGROUPINFO(screen[3], "4_", 6, AG_OSD, AG_OSD_Screen),
    ...
    // @User: Standard
    AP_GROUPINFO("_SW_METHOD", 7, AG_OSD, sw_method, AG_OSD::TOGGLE),
    ...
    // @User: Standard
    AP_GROUPINFO("_OPTIONS", 8, AG_OSD, options, OPTION_DECIMAL_PACK),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("_FONT", 9, AG_OSD, font_num, 0),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("_V_OFFSET", 10, AG_OSD, v_offset, 16),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("_H_OFFSET", 11, AG_OSD, h_offset, 32),
    ...
    // @Range: 0 99
    // @User: Standard
    AP_GROUPINFO("_W_RSSI", 12, AG_OSD, warn_rssi, 30),
    ...
    // @Range: 1 30
    // @User: Standard
    AP_GROUPINFO("_W_NSAT", 13, AG_OSD, warn_nsat, 9),
    ...
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_W_BATVOLT", 14, AG_OSD, warn_batvolt, 10.0f),
    ...
    // @User: Standard
    AP_GROUPINFO("_UNITS", 15, AG_OSD, units, 0),
    ...
    // @Range: 1 20
    // @User: Standard
    AP_GROUPINFO("_MSG_TIME", 16, AG_OSD, msgtime_s, 10),
    ...
    // @Range: 0 4
    // @User: Standard
    AP_GROUPINFO("_ARM_SCR", 17, AG_OSD, arm_scr, 0),
    ...
    // @Range: 0 4
    // @User: Standard
    AP_GROUPINFO("_DSARM_SCR", 18, AG_OSD, disarm_scr, 0),
    ...
    // @Range: 0 4
    // @User: Standard
    AP_GROUPINFO("_FS_SCR", 19, AG_OSD, failsafe_scr, 0),
    ...
    // @Range: 0 3000
    // @User: Advanced
    AP_GROUPINFO("_BTN_DELAY", 20, AG_OSD, button_delay_ms, 300),
#endif
    ...
    // @Units: m
    // @User: Standard
    AP_GROUPINFO("_W_TERR", 23, AG_OSD, warn_terr, -1),
#endif
    ...
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_W_AVGCELLV", 24, AG_OSD, warn_avgcellvolt, 3.6f),
   ...
   // @Param: _CELL_COUNT
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("_CELL_COUNT", 25, AG_OSD, cell_count, -1),
    ...
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_W_RESTVOLT", 26, AG_OSD, warn_restvolt, 10.0f),
    ...
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_W_ACRVOLT", 31, AG_OSD, warn_avgcellrestvolt, 3.6f),

#endif //osd enabled
#if OSD_PARAM_ENABLED
    // @Group: 5_
    // @Path: AG_OSD_ParamScreen.cpp
    AP_SUBGROUPINFO(param_screen[0], "5_", 21, AG_OSD, AG_OSD_ParamScreen),

    // @Group: 6_
    // @Path: AG_OSD_ParamScreen.cpp
    AP_SUBGROUPINFO(param_screen[1], "6_", 22, AG_OSD, AG_OSD_ParamScreen),
#endif

#if OSD_ENABLED
    // additional tables to go beyond 63 limit
    AP_SUBGROUPINFO2(screen[0], "1_", 27, AG_OSD, AG_OSD_Screen),
    AP_SUBGROUPINFO2(screen[1], "2_", 28, AG_OSD, AG_OSD_Screen),
    AP_SUBGROUPINFO2(screen[2], "3_", 29, AG_OSD, AG_OSD_Screen),
    AP_SUBGROUPINFO2(screen[3], "4_", 30, AG_OSD, AG_OSD_Screen),
#endif
    AP_GROUPEND
};
...
// singleton instance
AG_OSD *AG_OSD::_singleton;

AG_OSD::AG_OSD()
{
    if (_singleton != nullptr) {
        AG_HAL::panic("AG_OSD must be singleton");
    }
...
void AG_OSD::init()
{
    ...
    case OSD_MAX7456: {
        ...
        AG_HAL::OwnPtr<AG_HAL::Device> spi_dev = std::move(hal.spi->get_device("osd"));
        if (!spi_dev) {
            break;
        }
#if HAL_WITH_OSD_BITMAP
        backend = AG_OSD_MAX7456::probe(*this, std::move(spi_dev));
#endif
        if (backend == nullptr) {
            break;
        }
...
#ifdef WITH_SITL_OSD
    case OSD_SITL: {
        backend = AG_OSD_SITL::probe(*this);
        if (backend == nullptr) {
            break;
        }
    ...
    case OSD_MSP_DISPLAYPORT: {
        backend = AG_OSD_MSP_DisplayPort::probe(*this);
        if (backend == nullptr) {
            break;
        }
    ...
    if (backend != nullptr) {
        ...
        backend->init_symbol_set(AG_OSD_AbstractScreen::symbols_lookup_table, AG_OSD_NUM_SYMBOLS);
        // create thread as higher priority than IO for all backends
        hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AG_OSD::osd_thread, void), "OSD", 1280, AG_HAL::Scheduler::PRIORITY_IO, 1);
    }
#endif
}

#if OSD_ENABLED
void AG_OSD::osd_thread()
{
...
void AG_OSD::update_current_screen()
{
    // Switch on ARM/DISARM event
    if (AG_Notify::flags.armed) {
        if (!was_armed && arm_scr > 0 && arm_scr <= AG_OSD_NUM_DISPLAY_SCREENS && get_screen(arm_scr-1).enabled) {
            current_screen = arm_scr-1;
        }
        was_armed = true;
    } else if (was_armed) {
        if (disarm_scr > 0 && disarm_scr <= AG_OSD_NUM_DISPLAY_SCREENS && get_screen(disarm_scr-1).enabled) {
    ...
    if (AG_Notify::flags.failsafe_radio || AG_Notify::flags.failsafe_battery) {
        if (!was_failsafe && failsafe_scr > 0 && failsafe_scr <= AG_OSD_NUM_DISPLAY_SCREENS && get_screen(failsafe_scr-1).enabled) {
    ...
    case PWM_RANGE:
        for (int i=0; i<AG_OSD_NUM_SCREENS; i++) {
...
#if HAL_GCS_ENABLED
void AG_OSD::handle_msg(const mavlink_message_t &msg, const GCS_MAVLINK& link)
{
    bool found = false;
...
#if OSD_PARAM_ENABLED
        for (uint8_t i = 0; i < AG_OSD_NUM_PARAM_SCREENS; i++) {
            if (packet.osd_screen == i + AG_OSD_NUM_DISPLAY_SCREENS + 1) {
