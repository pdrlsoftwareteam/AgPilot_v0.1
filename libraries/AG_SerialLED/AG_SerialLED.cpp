 ...
 */
/*
  AG_SerialLED for controlling serial connected LEDs using WS2812B protocol
 */

#include "AG_SerialLED.h"
...
AG_SerialLED AG_SerialLED::singleton;

// set number of NeoPixels per pin
bool AG_SerialLED::set_num_neopixel(uint8_t chan, uint8_t num_leds)
{
    if (chan >= 1 && chan <= 16 && num_leds <= AP_SERIALLED_MAX_LEDS) {
        return hal.rcout->set_serial_led_num_LEDs(chan-1, num_leds, AG_HAL::RCOutput::MODE_NEOPIXEL);
    }
    return false;
}

// set number of ProfiLEDs per pin
bool AG_SerialLED::set_num_profiled(uint8_t chan, uint8_t num_leds)
{
    if (chan >= 1 && chan <= 16 && num_leds <= AP_SERIALLED_MAX_LEDS - 2) {
        ...
        return hal.rcout->set_serial_led_num_LEDs(chan-1, num_leds, AG_HAL::RCOutput::MODE_PROFILED, Clock_mask);
    }
    return false;
}

// set RGB value on LED number. LED number -1 is all LEDs. First LED is 0. chan is PWM output, 1..16
bool AG_SerialLED::set_RGB(uint8_t chan, int8_t led, uint8_t red, uint8_t green, uint8_t blue)
{
    if (chan >= 1 && chan <= 16) {
...
bool AG_SerialLED::send(uint8_t chan)
{
    if (chan >= 1 && chan <= 16) {
