...
bool AG_HAL::CANFrame::priorityHigherThan(const CANFrame& rhs) const
{
 ...
 */
int16_t AG_HAL::CANIface::receive(CANFrame& out_frame, uint64_t& out_ts_monotonic, CanIOFlags& out_flags)
{
 ...
 */
int16_t AG_HAL::CANIface::send(const CANFrame& frame, uint64_t tx_deadline, CanIOFlags flags)
{
    auto cb = frame_callback;
    if (cb) {
        if ((flags & IsMAVCAN) == 0) {
            ...
            CanRxItem rx_item;
            rx_item.frame = frame;
            rx_item.timestamp_us = AG_HAL::native_micros64();
            rx_item.flags = AG_HAL::CANIface::IsMAVCAN;
 ...
 */
bool AG_HAL::CANIface::register_frame_callback(FrameCb cb)
{
    frame_callback = cb;
    return true;
}

AG_HAL::CANFrame::CANFrame(uint32_t can_id, const uint8_t* can_data, uint8_t data_len, bool canfd_frame) :
        id(can_id),
        canfd(canfd_frame)
{
...
uint8_t AG_HAL::CANFrame::dataLengthToDlc(uint8_t data_length)
{
    if (data_length <= 8) {
        return data_length;
...
uint8_t AG_HAL::CANFrame::dlcToDataLength(uint8_t dlc)
{
    /*
    Data Length Code      9  10  11  12  13  14  15
    Number of data bytes 12  16  20  24  32  48  64
