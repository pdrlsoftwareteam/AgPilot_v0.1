...
TEST(OwnPtrTest, MoveOwnership)
{
    int *a = new int{42};
    AG_HAL::OwnPtr<int> own(a);
    AG_HAL::OwnPtr<int> own2 = std::move(own);
...
TEST(OwnPtrTest, DeleteOutOfScope)
{
    ...
    {
        AG_HAL::OwnPtr<TestDeleted> own(new TestDeleted{deleted});
    }
...
{
    unsigned int deleted = 0;
    AG_HAL::OwnPtr<TestDeleted> own;

    {
        AG_HAL::OwnPtr<TestDeleted> own2(new TestDeleted{deleted});
        own = std::move(own2);
    }
...
{
    AG_HAL::OwnPtr<TestCall> own(new TestCall{});
...
public:
    TestDestructor(AG_HAL::OwnPtr<TestDeleted> v) : _v(std::move(v)) { }

    AG_HAL::OwnPtr<TestDeleted> _v;
};
...
static AG_HAL::OwnPtr<TestDeleted> create_test_deleted(unsigned int &deleted)
{
    return AG_HAL::OwnPtr<TestDeleted>(new TestDeleted(deleted));
}
...
TEST(OwnPtrTest, Inheritance)
{
    A *a = new A(21);
    B *b = new B();
    AG_HAL::OwnPtr<A> own_a(a);
    AG_HAL::OwnPtr<B> own_b(b);
