 ...
 */
bool AG_HAL::Device::setup_checked_registers(uint8_t nregs, uint8_t frequency)
{
...
void AG_HAL::Device::set_device_type(uint8_t devtype) {
    _bus_id.devid_s.devtype = devtype;
}


bool AG_HAL::Device::read_bank_registers(uint8_t bank, uint8_t first_reg, uint8_t *recv, uint32_t recv_len)
{
    first_reg |= _read_flag;
    return transfer_bank(bank, &first_reg, 1, recv, recv_len);
}

bool AG_HAL::Device::write_bank_register(uint8_t bank, uint8_t reg, uint8_t val, bool checked)
{
 ...
 */
void AG_HAL::Device::set_checked_register(uint8_t reg, uint8_t val)
{
    set_checked_register(0, reg, val);
}

void AG_HAL::Device::set_checked_register(uint8_t bank, uint8_t reg, uint8_t val)
{
    if (_checked.regs == nullptr) {
        return;
    }
 ...
 */
bool AG_HAL::Device::check_next_register(void)
{
    if (_checked.n_set == 0) {
        return true;
    }
 ...
 */
bool AG_HAL::Device::check_next_register(struct checkreg &fail)
{
...
bool AG_HAL::Device::write_register(uint8_t reg, uint8_t val, bool checked)
{
    uint8_t buf[2] = { reg, val };
    if (checked) {
...
bool AG_HAL::Device::read_registers(uint8_t first_reg, uint8_t *recv, uint32_t recv_len)
{
...
bool AG_HAL::Device::transfer_bank(uint8_t bank, const uint8_t *send, uint32_t send_len,
 ...
 */
void AG_HAL::Device::set_read_flag(uint8_t flag)
{
    _read_flag = flag;
}
 ...
 */
uint32_t AG_HAL::Device::make_bus_id(enum BusType bus_type, uint8_t bus, uint8_t address, uint8_t devtype) {
    union DeviceId d {};
 ...
 */
uint32_t AG_HAL::Device::change_bus_id(uint32_t old_id, uint8_t devtype) {
    union DeviceId d;
    d.devid = old_id;
    d.devid_s.devtype = devtype;
    return d.devid;
 ...
 */
uint32_t AG_HAL::Device::get_bus_id_devtype(uint8_t devtype) const {
 ...
 * get bus type
 */
enum AG_HAL::Device::BusType AG_HAL::Device::devid_get_bus_type(uint32_t dev_id) {
    union DeviceId d;
    d.devid = dev_id;
    return d.devid_s.bus_type;
}

uint8_t AG_HAL::Device::devid_get_bus(uint32_t dev_id) {
    union DeviceId d;
    d.devid = dev_id;
    return d.devid_s.bus;
}

uint8_t AG_HAL::Device::devid_get_address(uint32_t dev_id) {
    union DeviceId d;
    d.devid = dev_id;
    return d.devid_s.address;
}

uint8_t AG_HAL::Device::devid_get_devtype(uint32_t dev_id) {
    union DeviceId d;
    d.devid = dev_id;
