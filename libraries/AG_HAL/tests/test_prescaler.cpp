...
protected:
    ...
    {
        const uint32_t prescaler = AG_HAL::RCOutput::calculate_bitrate_prescaler(clock, target_rate, is_dshot);
    ...
    {
        const uint32_t target_rate = 800000 * AG_HAL::RCOutput::NEOP_BIT_WIDTH_TICKS;
        const uint32_t prescaler = AG_HAL::RCOutput::calculate_bitrate_prescaler(clock, target_rate, false);
        ...
        const float bit_1_width_us = 1000000.0f * AG_HAL::RCOutput::NEOP_BIT_1_TICKS / actual_rate;
        const float bit_0_width_us = 1000000.0f * AG_HAL::RCOutput::NEOP_BIT_0_TICKS / actual_rate;
        // assert the output of expected results
        EXPECT_EQ(test_results[test_index].clock, clock);
        EXPECT_EQ(test_results[test_index].target, target_rate);
        EXPECT_EQ(test_results[test_index].rate, clock/(prescaler+1));
...
TEST_P(PrescalerParameterizedTestFixture, DShot150) {
    test_prescaler(GetParam(), 150000 * AG_HAL::RCOutput::DSHOT_BIT_WIDTH_TICKS, true);
}

TEST_P(PrescalerParameterizedTestFixture, DShot300) {
    test_prescaler(GetParam(), 300000 * AG_HAL::RCOutput::DSHOT_BIT_WIDTH_TICKS, true);
}

TEST_P(PrescalerParameterizedTestFixture, DShot600) {
    test_prescaler(GetParam(), 600000 * AG_HAL::RCOutput::DSHOT_BIT_WIDTH_TICKS, true);
}

TEST_P(PrescalerParameterizedTestFixture, DShot1200) {
    test_prescaler(GetParam(), 1200000 * AG_HAL::RCOutput::DSHOT_BIT_WIDTH_TICKS, true);
}
...
TEST_P(PrescalerParameterizedTestFixture, ProfiLED) {
    test_prescaler(GetParam(), 1500000 * AG_HAL::RCOutput::NEOP_BIT_WIDTH_TICKS, false);
}
