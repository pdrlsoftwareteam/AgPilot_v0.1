...
public:
    ...
    virtual AG_HAL::DigitalSource* channel(uint16_t n) = 0;
    ...
    bool detach_interrupt(uint8_t pin) {
        if (attach_interrupt(pin, (irq_handler_fn_t)nullptr, AG_HAL::GPIO::INTERRUPT_NONE)) {
            return true;
        }
        return attach_interrupt(pin, (AG_HAL::Proc)nullptr, AG_HAL::GPIO::INTERRUPT_NONE);
    }
