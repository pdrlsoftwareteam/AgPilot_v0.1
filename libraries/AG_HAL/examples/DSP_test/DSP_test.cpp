...
static AG_HAL::DSP::FFTWindowState* fft;
...
class DSPTest : public AG_HAL::DSP {
public:
...
void do_fft(const float* data)
{
    ...
    const float max_energy = fft->_freq_bins[fft->_peak_data[AG_HAL::DSP::CENTER]._bin];
    ...
    hal.console->printf("FFT: detected frequencies %.1f/%d/[%.1f-%.1f] %.1f/%d/[%.1f-%.1f] %.1f/%d/[%.1f-%.1f]\n",
        fft->_peak_data[AG_HAL::DSP::CENTER]._freq_hz,
        fft->_peak_data[AG_HAL::DSP::CENTER]._bin,
        (fft->_peak_data[AG_HAL::DSP::CENTER]._bin - 0.5) * fft->_bin_resolution,
        (fft->_peak_data[AG_HAL::DSP::CENTER]._bin + 0.5) * fft->_bin_resolution,
        fft->_peak_data[AG_HAL::DSP::LOWER_SHOULDER]._freq_hz,
        fft->_peak_data[AG_HAL::DSP::LOWER_SHOULDER]._bin,
        (fft->_peak_data[AG_HAL::DSP::LOWER_SHOULDER]._bin - 0.5) * fft->_bin_resolution,
        (fft->_peak_data[AG_HAL::DSP::LOWER_SHOULDER]._bin + 0.5) * fft->_bin_resolution,
        fft->_peak_data[AG_HAL::DSP::UPPER_SHOULDER]._freq_hz,
        fft->_peak_data[AG_HAL::DSP::UPPER_SHOULDER]._bin,
        (fft->_peak_data[AG_HAL::DSP::UPPER_SHOULDER]._bin - 0.5) * fft->_bin_resolution,
        (fft->_peak_data[AG_HAL::DSP::UPPER_SHOULDER]._bin + 0.5) * fft->_bin_resolution);
}
