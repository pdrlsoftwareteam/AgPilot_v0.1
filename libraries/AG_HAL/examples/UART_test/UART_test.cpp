 ...
 */
static void setup_uart(AG_HAL::UARTDriver *uart, const char *name)
{
    if (uart == nullptr) {
...
static void test_uart(AG_HAL::UARTDriver *uart, const char *name)
{
    if (uart == nullptr) {
        // that UART doesn't exist on this platform
        return;
    ...
    uart->printf("Hello on UART %s at %.3f seconds\n",
                 name, (double)(AG_HAL::millis() * 0.001f));
}
