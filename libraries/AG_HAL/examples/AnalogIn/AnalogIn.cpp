...
const AG_HAL::HAL& hal = AG_HAL::get_HAL();    //create a reference to AG_HAL::HAL object to get access to hardware specific functions. For more info see <https://ardupilot.org/dev/docs/learning-ardupilot-the-example-sketches.html/>  

AG_HAL::AnalogSource* chan;    //delare a pointer to AnalogSource object. AnalogSource class can be found in : AG_HAL->AnalogIn.h

// the setup function runs once when the board powers up
void setup(void) {
    hal.console->printf("Starting AG_HAL::AnalogIn test\r\n");    //print a starting message
    chan = hal.analogin->channel(0);    //initialization of chan variable. AnalogIn class can be found in : AG_HAL->AnalogIn.h
}
...
AG_HAL_MAIN(); //HAL Macro that declares the main function. For more info see <https://ardupilot.org/dev/docs/learning-ardupilot-the-example-sketches.html/>
