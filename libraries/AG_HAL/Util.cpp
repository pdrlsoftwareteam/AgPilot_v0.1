...
/* Helper class implements AG_HAL::Print so we can use utility/vprintf */
class BufferPrinter : public AG_HAL::BetterStream {
public:
...
int AG_HAL::Util::snprintf(char* str, size_t size, const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
...
int AG_HAL::Util::vsnprintf(char* str, size_t size, const char *format, va_list ap)
{
...
void AG_HAL::Util::set_soft_armed(const bool b)
{
    if (b != soft_armed) {
        soft_armed = b;
        last_armed_change_ms = AG_HAL::millis();
